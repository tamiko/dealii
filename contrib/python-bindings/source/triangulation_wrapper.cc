...
namespace python
{
  namespace internal
  {
    ...
    create_triangulation(const boost::python::list &vertices_list,
                         ...
                         void                      *triangulation)
    {
      Triangulation<dim, spacedim> *tria = static_cast<Triangulation<dim, spacedim> *>(triangulation);
      tria->clear();
      ...
      for (size_t i = 0; i < n_vertices; ++i)
        {
          boost::python::list vertex = boost::python::extract<boost::python::list>(vertices_list[i]);
      ...
      for (size_t i = 0; i < n_cells; ++i)
        {
          boost::python::list vertex_indices = boost::python::extract<boost::python::list>(cells_vertices[i]);

          for (unsigned int v = 0; v < GeometryInfo<dim>::vertices_per_cell; ++v)
            cell_data[i].vertices[v] = boost::python::extract<unsigned int>(vertex_indices[v]);
        }
    ...
    template <int dim, int spacedim>
    void
    generate_hyper_cube(const double left, const double right, const bool colorize, void *triangulation)
    {
      Triangulation<dim, spacedim> *tria = static_cast<Triangulation<dim, spacedim> *>(triangulation);
      tria->clear();
    ...
    template <int dim>
    void
    generate_simplex(std::vector<PointWrapper> &wrapped_points, void *triangulation)
    {
    ...
    template <int dim, int spacedim>
    void
    generate_hyper_rectangle(PointWrapper &p1, PointWrapper &p2, const bool colorize, void *triangulation)
    {
                                              ...
                                              void              *triangulation)
    {
      Triangulation<dim> *tria = static_cast<Triangulation<dim> *>(triangulation);
      tria->clear();
      GridGenerator::hyper_cube_with_cylindrical_hole(*tria, inner_radius, outer_radius, L, repetitions, colorize);
    }
    ...
    template <int dim, int spacedim>
    void
    generate_subdivided_hyper_rectangle(const std::vector<unsigned int> &repetitions,
                                        PointWrapper                    &p1,
                                        PointWrapper                    &p2,
                                        const bool                       colorize,
                                        void                            *triangulation)
    {
      ...
      Triangulation<dim, spacedim> *tria = static_cast<Triangulation<dim, spacedim> *>(triangulation);
      tria->clear();
      GridGenerator::subdivided_hyper_rectangle(*tria, repetitions, point_1, point_2, colorize);
    }
    ...
    template <int dim>
    void
    generate_subdivided_steps_hyper_rectangle(const std::vector<std::vector<double>> &step_sizes,
                                              PointWrapper                           &p1,
                                              PointWrapper                           &p2,
                                              const bool                              colorize,
                                              void                                   *triangulation)
    {
      ...
      Triangulation<dim> *tria = static_cast<Triangulation<dim> *>(triangulation);
      tria->clear();
      GridGenerator::subdivided_hyper_rectangle(*tria, step_sizes, point_1, point_2, colorize);
    }
    ...
    template <int dim>
    void
    generate_subdivided_material_hyper_rectangle(const std::vector<std::vector<double>> &spacing,
                                                 PointWrapper                           &p,
                                                 ...
                                                 void                                   *triangulation)
    {
      ...
      Triangulation<dim> *tria  = static_cast<Triangulation<dim> *>(triangulation);
      tria->clear();
      GridGenerator::subdivided_hyper_rectangle(*tria, spacing, point, material_ids, colorize);
    }
    ...
    template <int dim>
    void
    generate_general_cell(std::vector<PointWrapper> &wrapped_points, const bool colorize, void *triangulation)
    {
      // Cast the PointWrapper objects to Point<dim>
    ...
    template <int dim>
    void
    generate_parallelogram(std::vector<PointWrapper> &wrapped_points, const bool colorize, void *triangulation)
    {
      // Cast the PointWrapper objects to Point<dim>
    ...
    template <int dim>
    void
    generate_parallelepiped(std::vector<PointWrapper> &wrapped_points, const bool colorize, void *triangulation)
    {
      // Cast the PointWrapper objects to Point<dim>
      Point<dim> points[dim];
      for (unsigned int i = 0; i < dim; ++i)
    ...
    template <int dim>
    void
    generate_fixed_subdivided_parallelepiped(unsigned int               n_subdivisions,
    ...
    template <int dim>
    void
    generate_varying_subdivided_parallelepiped(std::vector<unsigned int> &n_subdivisions,
                                               std::vector<PointWrapper> &wrapped_points,
                                               const bool                 colorize,
                                               void                      *triangulation)
    {
    ...
    template <int dim>
    void
    generate_hyper_ball(PointWrapper &center, const double radius, void *triangulation)
    {
      // Cast the PointWrapper object to Point<dim>
    ...
    template <int dim, int spacedim>
    void
    generate_hyper_sphere(PointWrapper &center, const double radius, void *triangulation)
    {
      // Cast the PointWrapper object to Point<dim>
      Point<spacedim> center_point = *(static_cast<Point<spacedim> *>(center.get_point()));
                         ...
                         void          *triangulation)
    {
      ...
      Triangulation<dim> *tria = static_cast<Triangulation<dim> *>(triangulation);
      tria->clear();
      GridGenerator::hyper_shell(*tria, center_point, inner_radius, outer_radius, n_cells, colorize);
    }


    template <int dim>
    void
    generate_quarter_hyper_ball(PointWrapper &center, const double radius, void *triangulation)
    {
      // Cast the PointWrapper object to Point<dim>
      Point<dim> center_point = *(static_cast<Point<dim> *>(center.get_point()));
    ...
    template <int dim>
    void
    generate_half_hyper_ball(PointWrapper &center, const double radius, void *triangulation)
    {
      // Cast the PointWrapper object to Point<dim>
      Point<dim> center_point = *(static_cast<Point<dim> *>(center.get_point()));
                         ...
                         const bool           copy_manifold_ids)
    {
      ...
      for (size_t i = 0; i < n_triangulations; ++i)
        {
          TriangulationWrapper &tria_wrapper = boost::python::extract<TriangulationWrapper &>(triangulations[i]);
          tria_ptrs.push_back(static_cast<const Triangulation<dim, spacedim> *>(tria_wrapper.get_triangulation()));
        }

      GridGenerator::merge_triangulations(tria_ptrs, *tria, duplicated_vertex_tolerance, copy_manifold_ids);
    }
    ...
    {
      Triangulation<dim, spacedim_1> *tria = static_cast<Triangulation<dim, spacedim_1> *>(triangulation);
    ...
    template <int dim, int spacedim>
    void
    convert_hypercube_to_simplex_mesh(void *triangulation, TriangulationWrapper &tria_out)
    {
      Triangulation<dim, spacedim> *tria   = static_cast<Triangulation<dim, spacedim> *>(triangulation);
      Triangulation<dim, spacedim> *tria_2 = static_cast<Triangulation<dim, spacedim> *>(tria_out.get_triangulation());
    ...
    template <int dim, int spacedim>
    void
    replicate_triangulation(TriangulationWrapper &tria_in, const boost::python::list &extents, void *tria_out)
    {
      Triangulation<dim, spacedim> *replicated_triangulation = static_cast<Triangulation<dim, spacedim> *>(tria_out);
      const Triangulation<dim, spacedim> *triangulation =
        static_cast<Triangulation<dim, spacedim> *>(tria_in.get_triangulation());
      ...
      GridGenerator::replicate_triangulation(*triangulation, extents_vector, *replicated_triangulation);
    }
    ...
    template <int dim, int spacedim>
    void
    distort_random(const double factor, const bool keep_boundary, void *triangulation)
    {
      Triangulation<dim, spacedim> *tria = static_cast<Triangulation<dim, spacedim> *>(triangulation);
    ...
    transform(boost::python::object &transformation, void *triangulation)
    {
      ...
      auto func = [&transformation](const Point<spacedim> &p_in) {
        ...
        boost::python::list p_list_out = boost::python::extract<boost::python::list>(transformation(p_list_in));
    ...
    template <int dim, int spacedim>
    std::pair<int, int>
    find_active_cell_around_point(PointWrapper &p, MappingQWrapper &mapping_wrapper, void *triangulation)
    {
      Triangulation<dim, spacedim> *tria = static_cast<Triangulation<dim, spacedim> *>(triangulation);
      ...
      if (mapping_wrapper.get_mapping() != nullptr)
        {
          const MappingQ<dim, spacedim> *mapping =
            static_cast<const MappingQ<dim, spacedim> *>(mapping_wrapper.get_mapping());

          auto cell_pair = GridTools::find_active_cell_around_point(*mapping, *tria, point);
    ...
    template <int dim, int spacedim>
    boost::python::list
    compute_aspect_ratio_of_cells(const MappingQWrapper      &mapping_wrapper,
                                  const QuadratureWrapper    &quadrature_wrapper,
                                  const TriangulationWrapper &triangulation_wrapper)
    {
      const Triangulation<dim, spacedim> *tria =
        static_cast<const Triangulation<dim, spacedim> *>(triangulation_wrapper.get_triangulation());

      const Quadrature<dim> *quad = static_cast<const Quadrature<dim> *>(quadrature_wrapper.get_quadrature());

      const MappingQ<dim, spacedim> *mapping =
        static_cast<const MappingQ<dim, spacedim> *>(mapping_wrapper.get_mapping());

      auto aspect_ratios = GridTools::compute_aspect_ratio_of_cells(*mapping, *tria, *quad);
    ...
    template <int dim, int spacedim>
    boost::python::list
    find_cells_adjacent_to_vertex(const unsigned int vertex_index, TriangulationWrapper &triangulation_wrapper)
    {
      Triangulation<dim, spacedim> *tria =
        static_cast<Triangulation<dim, spacedim> *>(triangulation_wrapper.get_triangulation());

      std::vector<typename Triangulation<dim, spacedim>::active_cell_iterator> adjacent_cells =
        GridTools::find_cells_adjacent_to_vertex(*tria, vertex_index);

      boost::python::list cells;
      for (auto &cell : adjacent_cells)
        cells.append(CellAccessorWrapper(triangulation_wrapper, cell->level(), cell->index()));
    ...
    cells(TriangulationWrapper &triangulation_wrapper)
    {
      ...
      for (auto &cell : tria->cell_iterators())
        cells_list.append(CellAccessorWrapper(triangulation_wrapper, cell->level(), cell->index()));
    ...
    {
      const Triangulation<dim, spacedim> *tria = static_cast<const Triangulation<dim, spacedim> *>(triangulation);
    ...
    template <int dim, int spacedim>
    void
    write(const std::string &filename, const std::string &format, const void *triangulation)
    {
      const Triangulation<dim, spacedim> *tria = static_cast<const Triangulation<dim, spacedim> *>(triangulation);
    ...
    template <int dim, int spacedim>
    void
    read(const std::string &filename, const std::string &format, void *triangulation)
    {
      Triangulation<dim, spacedim> *tria = static_cast<Triangulation<dim, spacedim> *>(triangulation);

      tria->clear();

      typename GridIn<dim, spacedim>::Format input_format = GridIn<dim, spacedim>::Format::Default;
      if (format.compare("msh") == 0)
        input_format = GridIn<dim, spacedim>::Format::msh;
      else if (format.compare("vtk") == 0)
        input_format = GridIn<dim, spacedim>::Format::vtk;
      else
        Assert(false, ExcMessage("Cannot read triangulation of the given format."));
            ...
            if (spacedim == 2)
              {
                ...
                // pointer back first.
                Triangulation<2, 2> *tmp = static_cast<Triangulation<2, 2> *>(triangulation);
                delete tmp;
              }
            else
              {
                Triangulation<2, 3> *tmp = static_cast<Triangulation<2, 3> *>(triangulation);
                delete tmp;
              }
          }
        else
  ...
  void
  TriangulationWrapper::create_triangulation(const boost::python::list &vertices,
                                             const boost::python::list &cells_vertices)
  {
    if ((dim == 2) && (spacedim == 2))
      internal::create_triangulation<2, 2>(vertices, cells_vertices, triangulation);
    else if ((dim == 2) && (spacedim == 3))
      internal::create_triangulation<2, 3>(vertices, cells_vertices, triangulation);
    else
      internal::create_triangulation<3, 3>(vertices, cells_vertices, triangulation);
  }



  void
  TriangulationWrapper::generate_hyper_cube(const double left, const double right, const bool colorize)
  {
    if ((dim == 2) && (spacedim == 2))
  ...
  TriangulationWrapper::generate_simplex(boost::python::list &vertices)
  {
    ...
    AssertThrow(dim == spacedim, ExcMessage("This function is only implemented for dim equal to spacedim."));
    ...
    for (int i = 0; i < dim + 1; ++i)
      {
        ...
        AssertThrow(wrapped_points[i].get_dim() == dim, ExcMessage("Point of wrong dimension."));
      }
  ...
  void
  TriangulationWrapper::generate_subdivided_hyper_cube(const unsigned int repetitions,
                                                       const double       left,
                                                       const double       right)
  {
    if ((dim == 2) && (spacedim == 2))
  ...
  void
  TriangulationWrapper::generate_hyper_rectangle(PointWrapper &p1, PointWrapper &p2, const bool colorize)
  {
    if ((dim == 2) && (spacedim == 2))
  ...
  void
  TriangulationWrapper::generate_subdivided_hyper_rectangle(boost::python::list &repetition_list,
                                                            PointWrapper        &p1,
                                                            PointWrapper        &p2,
                                                            const bool           colorize)
  {
    ...
    if ((dim == 2) && (spacedim == 2))
      internal::generate_subdivided_hyper_rectangle<2, 2>(repetitions, p1, p2, colorize, triangulation);
    else if ((dim == 2) && (spacedim == 3))
      internal::generate_subdivided_hyper_rectangle<2, 3>(repetitions, p1, p2, colorize, triangulation);
    else
      internal::generate_subdivided_hyper_rectangle<3, 3>(repetitions, p1, p2, colorize, triangulation);
  }



  void
  TriangulationWrapper::generate_subdivided_steps_hyper_rectangle(boost::python::list &step_sizes_list,
                                                                  PointWrapper        &p1,
                                                                  PointWrapper        &p2,
                                                                  const bool           colorize)
  {
    AssertThrow(spacedim == dim, ExcMessage("This function is only implemented for dim equal to spacedim."));
    ...
    if (dim == 2)
      internal::generate_subdivided_steps_hyper_rectangle<2>(step_sizes, p1, p2, colorize, triangulation);
    else
      internal::generate_subdivided_steps_hyper_rectangle<3>(step_sizes, p1, p2, colorize, triangulation);
  }



  void
  TriangulationWrapper::generate_subdivided_material_hyper_rectangle(boost::python::list &spacing_list,
                                                                     PointWrapper        &p,
    ...
    if (dim == 2)
      {
        ...
        internal::generate_subdivided_material_hyper_rectangle<2>(spacing, p, material_ids, colorize, triangulation);
      }
    else
      {
        const unsigned int           index_0 = boost::python::len(material_id_list);
        ...
        internal::generate_subdivided_material_hyper_rectangle<3>(spacing, p, material_ids, colorize, triangulation);
      }
  }
  ...
  void
  TriangulationWrapper::generate_general_cell(boost::python::list &vertices, const bool colorize)
  {
    AssertThrow(spacedim == dim, ExcMessage("This function is only implemented for dim equal to spacedim."));
    // Extract the PointWrapper object from the python list
  ...
  void
  TriangulationWrapper::generate_parallelogram(boost::python::list &corners, const bool colorize)
  {
    AssertThrow(spacedim == dim, ExcMessage("This function is only implemented for dim equal to spacedim."));
    // Extract the PointWrapper object from the python list
  ...
  void
  TriangulationWrapper::generate_parallelepiped(boost::python::list &corners, const bool colorize)
  {
    AssertThrow(spacedim == dim, ExcMessage("This function is only implemented for dim equal to spacedim."));
    // Extract the PointWrapper object from the python list
    AssertThrow(boost::python::len(corners) == dim,
  ...
  void
  TriangulationWrapper::generate_fixed_subdivided_parallelepiped(const unsigned int   n_subdivisions,
                                                                 ...
                                                                 const bool           colorize)
  {
    ...
    for (int i = 0; i < dim; ++i)
      wrapped_points[i] = boost::python::extract<PointWrapper>(corners[i]);
    if ((dim == 2) && (spacedim == 2))
      internal::generate_fixed_subdivided_parallelepiped<2>(n_subdivisions, wrapped_points, colorize, triangulation);
    else
      internal::generate_fixed_subdivided_parallelepiped<3>(n_subdivisions, wrapped_points, colorize, triangulation);
  }



  void
  TriangulationWrapper::generate_varying_subdivided_parallelepiped(boost::python::list &n_subdivisions,
                                                                   boost::python::list &corners,
                                                                   const bool           colorize)
  {
    AssertThrow(spacedim == dim, ExcMessage("This function is only implemented for dim equal to spacedim."));
    ...
    for (int i = 0; i < dim; ++i)
      wrapped_points[i] = boost::python::extract<PointWrapper>(corners[i]);
    if (dim == 2)
      internal::generate_varying_subdivided_parallelepiped<2>(subdivisions, wrapped_points, colorize, triangulation);
    else
      internal::generate_varying_subdivided_parallelepiped<3>(subdivisions, wrapped_points, colorize, triangulation);
  }
  ...
  void
  TriangulationWrapper::generate_hyper_ball(PointWrapper &center, const double radius)
  {
    AssertThrow(dim == spacedim, ExcMessage("This function is only implemented for dim equal to spacedim."));
    if (dim == 2)
                                             ...
                                             bool           colorize)
  {
    AssertThrow(dim == spacedim, ExcMessage("This function is only implemented for dim equal to spacedim."));
    if (dim == 2)
      internal::generate_hyper_shell<2>(center, inner_radius, outer_radius, n_cells, colorize, triangulation);
    else
      internal::generate_hyper_shell<3>(center, inner_radius, outer_radius, n_cells, colorize, triangulation);
  }



  void
  TriangulationWrapper::generate_hyper_sphere(PointWrapper &center, const double radius)
  {
    AssertThrow(spacedim == dim + 1, ExcMessage("This function is only implemented for spacedim equal to dim+1."));
  ...
  void
  TriangulationWrapper::generate_quarter_hyper_ball(PointWrapper &center, const double radius)
  {
    AssertThrow(dim == spacedim, ExcMessage("This function is only implemented for dim equal to spacedim."));
    if (dim == 2)
  ...
  void
  TriangulationWrapper::generate_half_hyper_ball(PointWrapper &center, const double radius)
  {
    AssertThrow(dim == spacedim, ExcMessage("This function is only implemented for dim equal to spacedim."));
    if (dim == 2)
  ...
  void
  TriangulationWrapper::generate_hyper_cube_with_cylindrical_hole(const double       inner_radius,
                                                                  const double       outer_radius,
                                                                  const double       L,
                                                                  const unsigned int repetitions,
                                                                  const bool         colorize)
  ...
  void
  TriangulationWrapper::merge_triangulations(boost::python::list &triangulations,
                                             const double         duplicated_vertex_tolerance,
                                             const bool           copy_manifold_ids)
  {
    AssertThrow(boost::python::len(triangulations) >= 2,
                ExcMessage("You must provide at least two triangulations to merge."));
    ...
    for (size_t i = 0; i < n_triangulations; ++i)
      {
        ...
        AssertThrow(dim == tria_wrapper.get_dim(), ExcMessage("All triangulations must have the same dimension."));
        AssertThrow(spacedim == tria_wrapper.get_spacedim(),
  ...
  void
  TriangulationWrapper::convert_hypercube_to_simplex_mesh(TriangulationWrapper &tria_out)
  {
    AssertThrow((tria_out.get_dim() == dim) && (tria_out.get_spacedim() == spacedim),
                ExcMessage("The output Triangulation must be of the same dimension"));

    if ((dim == 2) && (spacedim == 2))
      internal::convert_hypercube_to_simplex_mesh<2, 2>(triangulation, tria_out);
    else if ((dim == 2) && (spacedim == 3))
      internal::convert_hypercube_to_simplex_mesh<2, 3>(triangulation, tria_out);
    else
      internal::convert_hypercube_to_simplex_mesh<3, 3>(triangulation, tria_out);
  }



  void
  TriangulationWrapper::extrude_triangulation(const unsigned int    n_slices,
                                              const double          height,
                                              TriangulationWrapper &triangulation_out)
  {
    AssertThrow((dim == 2) && (spacedim == 2), ExcMessage("Extrude can only be applied to the dim and spacedim two."));
    AssertThrow((triangulation_out.get_dim() == 3) && (triangulation_out.get_spacedim() == 3),
                ExcMessage("The output Triangulation must be of dimension three"));


    Triangulation<2, 2> *tria     = static_cast<Triangulation<2, 2> *>(triangulation);
    Triangulation<3, 3> *tria_out = static_cast<Triangulation<3, 3> *>(triangulation_out.get_triangulation());
  ...
  void
  TriangulationWrapper::replicate_triangulation(TriangulationWrapper &triangulation_in, boost::python::list &extents)
  {
    AssertThrow(boost::python::len(extents) == dim,
                ExcMessage("Size of the extents list must be equal to the triangulation dimension."));

    AssertThrow((triangulation_in.get_dim() == dim) && (triangulation_in.get_spacedim() == spacedim),
                ExcMessage("The input Triangulation must be of the same dimension"));


    if ((dim == 2) && (spacedim == 2))
      internal::replicate_triangulation<2, 2>(triangulation_in, extents, triangulation);
    else if ((dim == 2) && (spacedim == 3))
      internal::replicate_triangulation<2, 3>(triangulation_in, extents, triangulation);
    else
      internal::replicate_triangulation<3, 3>(triangulation_in, extents, triangulation);
  }



  void
  TriangulationWrapper::distort_random(const double factor, const bool keep_boundary)
  {
    if ((dim == 2) && (spacedim == 2))
  ...
  CellAccessorWrapper
  TriangulationWrapper::find_active_cell_around_point(PointWrapper &p, MappingQWrapper mapping)
  {
    std::pair<int, int> level_index_pair;
    if ((dim == 2) && (spacedim == 2))
      level_index_pair = internal::find_active_cell_around_point<2, 2>(p, mapping, triangulation);
    else if ((dim == 2) && (spacedim == 3))
      level_index_pair = internal::find_active_cell_around_point<2, 3>(p, mapping, triangulation);
    else
      level_index_pair = internal::find_active_cell_around_point<3, 3>(p, mapping, triangulation);

    return CellAccessorWrapper(*this, level_index_pair.first, level_index_pair.second);
  }



  boost::python::list
  TriangulationWrapper::find_cells_adjacent_to_vertex(const unsigned int vertex_index)
  {
    if ((dim == 2) && (spacedim == 2))
  ...
  boost::python::list
  TriangulationWrapper::compute_aspect_ratio_of_cells(const MappingQWrapper   &mapping,
                                                      const QuadratureWrapper &quadrature)
  {
    if ((dim == 2) && (spacedim == 2))
      return internal::compute_aspect_ratio_of_cells<2, 2>(mapping, quadrature, *this);
    else if ((dim == 3) && (spacedim == 3))
      return internal::compute_aspect_ratio_of_cells<3, 3>(mapping, quadrature, *this);
    else
      AssertThrow(false, ExcMessage("This combination of dim-spacedim is not supported."));
  }
  ...
  TriangulationWrapper::refine_global(const unsigned int n)
  {
    if ((dim == 2) && (spacedim == 2))
      ...
      {
        Triangulation<2, 3> *tria = static_cast<Triangulation<2, 3> *>(triangulation);
        tria->refine_global(n);
      }
    else
      {
        Triangulation<3, 3> *tria = static_cast<Triangulation<3, 3> *>(triangulation);
        tria->refine_global(n);
      }
  }
  ...
  void
  TriangulationWrapper::write(const std::string &filename, const std::string format) const
  {
    if ((dim == 2) && (spacedim == 2))
  ...
  void
  TriangulationWrapper::read(const std::string &filename, const std::string format) const
  {
    if ((dim == 2) && (spacedim == 2))
  ...
  TriangulationWrapper::load(const std::string &filename)
  {
    ...
    if ((dim == 2) && (spacedim == 2))
      ...
      {
        Triangulation<3> *tria = static_cast<Triangulation<3, 3> *>(triangulation);
  ...
  {
    AssertThrow(dim == manifold.get_dim(),
                ExcMessage("The Triangulation and Manifold should have the same dimension."));
    AssertThrow(spacedim == manifold.get_spacedim(),
                ExcMessage("The Triangulation and Manifold should have the same space dimension."));

    if ((dim == 2) && (spacedim == 2))
      {
        Triangulation<2, 2> *tria = static_cast<Triangulation<2, 2> *>(triangulation);
        Manifold<2, 2>      *m    = static_cast<Manifold<2, 2> *>(manifold.get_manifold());
        tria->set_manifold(number, *m);
      }
    else if ((dim == 2) && (spacedim == 3))
      {
        Triangulation<2, 3> *tria = static_cast<Triangulation<2, 3> *>(triangulation);
        Manifold<2, 3>      *m    = static_cast<Manifold<2, 3> *>(manifold.get_manifold());
        tria->set_manifold(number, *m);
      }
    else
      {
        Triangulation<3, 3> *tria = static_cast<Triangulation<3, 3> *>(triangulation);
        Manifold<3, 3>      *m    = static_cast<Manifold<3, 3> *>(manifold.get_manifold());
        tria->set_manifold(number, *m);
      }
  }
  ...
  TriangulationWrapper::set_mesh_smoothing(const int mesh_smoothing)
  {
    if ((dim == 2) && (spacedim == 2))
      {
        Triangulation<2, 2> *tria = static_cast<Triangulation<2, 2> *>(triangulation);
        tria->set_mesh_smoothing((Triangulation<2, 2>::MeshSmoothing)mesh_smoothing);
      }
    else if ((dim == 2) && (spacedim == 3))
      {
        Triangulation<2, 3> *tria = static_cast<Triangulation<2, 3> *>(triangulation);
        tria->set_mesh_smoothing((Triangulation<2, 3>::MeshSmoothing)mesh_smoothing);
      }
    else
      {
        Triangulation<3, 3> *tria = static_cast<Triangulation<3, 3> *>(triangulation);
        tria->set_mesh_smoothing((Triangulation<3, 3>::MeshSmoothing)mesh_smoothing);
      }
  }
    ...
    if ((dimension.compare("2D") == 0) || (dimension.compare("2d") == 0))
      {
        ...
        if ((spacedimension.compare("2D") == 0) || (spacedimension.compare("2d") == 0))
          {
            spacedim = 2;
            triangulation =
              new Triangulation<2, 2>((Triangulation<2, 2>::MeshSmoothing)mesh_smoothing, check_for_distorted_cells);
          }
        else if ((spacedimension.compare("3D") == 0) || (spacedimension.compare("3d") == 0))
          {
            spacedim = 3;
            triangulation =
              new Triangulation<2, 3>((Triangulation<2, 3>::MeshSmoothing)mesh_smoothing, check_for_distorted_cells);
          }
        else
      ...
      {
        if ((spacedimension.compare("3D") != 0) && (spacedimension.compare("3d") != 0))
          AssertThrow(false, ExcMessage("Spacedimension needs to be 3D."));
        dim      = 3;
        spacedim = 3;
        triangulation =
          new Triangulation<3, 3>((Triangulation<3, 3>::MeshSmoothing)mesh_smoothing, check_for_distorted_cells);
      }
    else
      AssertThrow(false, ExcMessage("Dimension needs to be 2D or 3D."));
  }

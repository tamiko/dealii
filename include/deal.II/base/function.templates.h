...
Function<dim, RangeNumberType>::Function(const unsigned int                                       n_components,
  ...
  : FunctionTime<typename Function<dim, RangeNumberType>::time_type>(initial_time)
...
template <int dim, typename RangeNumberType>
void
Function<dim, RangeNumberType>::vector_values(const std::vector<Point<dim>>             &points,
...
template <int dim, typename RangeNumberType>
void
Function<dim, RangeNumberType>::vector_gradient(const Point<dim>                             &p,
...
template <int dim, typename RangeNumberType>
void
Function<dim, RangeNumberType>::gradient_list(const std::vector<Point<dim>>                &points,
  ...
  std::vector<std::vector<Tensor<1, dim, RangeNumberType>>> &gradients) const
{
  ...
  for (unsigned int i = 0; i < points.size(); ++i)
    {
      Assert(gradients[i].size() == n_components, ExcDimensionMismatch(gradients[i].size(), n_components));
...
template <int dim, typename RangeNumberType>
void
Function<dim, RangeNumberType>::vector_laplacian(const Point<dim> &, Vector<RangeNumberType> &) const
{
  Assert(false, ExcPureFunctionCalled());
}
                                               ...
                                               const unsigned int             component) const
{
  // check whether component is in the valid range is up to the derived
  // class
  Assert(laplacians.size() == points.size(), ExcDimensionMismatch(laplacians.size(), points.size()));
...
template <int dim, typename RangeNumberType>
void
Function<dim, RangeNumberType>::vector_laplacian_list(const std::vector<Point<dim>>        &points,
...
template <int dim, typename RangeNumberType>
void
Function<dim, RangeNumberType>::vector_hessian(const Point<dim>                                      &p,
...
template <int dim, typename RangeNumberType>
void
Function<dim, RangeNumberType>::hessian_list(const std::vector<Point<dim>>                         &points,
  ...
  std::vector<std::vector<SymmetricTensor<2, dim, RangeNumberType>>> &hessians) const
{
  ...
  for (unsigned int i = 0; i < points.size(); ++i)
    {
      Assert(hessians[i].size() == n_components, ExcDimensionMismatch(hessians[i].size(), n_components));
...
{
  template <int dim, typename RangeNumberType>
  ConstantFunction<dim, RangeNumberType>::ConstantFunction(const RangeNumberType value, const unsigned int n_components)
  ...
  template <int dim, typename RangeNumberType>
  ConstantFunction<dim, RangeNumberType>::ConstantFunction(const std::vector<RangeNumberType> &values)
  ...
  template <int dim, typename RangeNumberType>
  RangeNumberType
  ConstantFunction<dim, RangeNumberType>::value(const Point<dim> &, const unsigned int component) const
  {
  ...
  template <int dim, typename RangeNumberType>
  void
  ConstantFunction<dim, RangeNumberType>::vector_value(const Point<dim> &, Vector<RangeNumberType> &return_value) const
  {
    Assert(return_value.size() == this->n_components, ExcDimensionMismatch(return_value.size(), this->n_components));
                                                     ...
                                                     const unsigned int             component) const
  {
    ...
    (void)points;
    AssertIndexRange(component, this->n_components);
    Assert(return_values.size() == points.size(), ExcDimensionMismatch(return_values.size(), points.size()))
  ...
  template <int dim, typename RangeNumberType>
  void
  ConstantFunction<dim, RangeNumberType>::vector_value_list(const std::vector<Point<dim>>        &points,
    ...
    for (unsigned int i = 0; i < points.size(); ++i)
      {
        ...
        std::copy(function_value_vector.begin(), function_value_vector.end(), return_values[i].begin());
      };
  }
  ...
  template <int dim, typename RangeNumberType>
  void
  ConstantFunction<dim, RangeNumberType>::vector_gradient(const Point<dim> &,
                                                          std::vector<Tensor<1, dim, RangeNumberType>> &gradients) const
  {
    Assert(gradients.size() == this->n_components, ExcDimensionMismatch(gradients.size(), this->n_components));
  ...
  template <int dim, typename RangeNumberType>
  void
  ConstantFunction<dim, RangeNumberType>::gradient_list(const std::vector<Point<dim>>                &points,
                                                        std::vector<Tensor<1, dim, RangeNumberType>> &gradients,
                                                        const unsigned int /*component*/) const
  {
    Assert(gradients.size() == points.size(), ExcDimensionMismatch(gradients.size(), points.size()));
  ...
  template <int dim, typename RangeNumberType>
  SymmetricTensor<2, dim, RangeNumberType>
  ConstantFunction<dim, RangeNumberType>::hessian(const Point<dim> &, const unsigned int) const
  {
    return SymmetricTensor<2, dim, RangeNumberType>();
  }
  ...
  template <int dim, typename RangeNumberType>
  RangeNumberType
  ConstantFunction<dim, RangeNumberType>::laplacian(const Point<dim> &, const unsigned int) const
  {
    return 0;
  }
  ...
  template <int dim, typename RangeNumberType>
  RangeNumberType
  IdentityFunction<dim, RangeNumberType>::value(const Point<dim> &p, const unsigned int component) const
  {
    AssertIndexRange(component, this->n_components);
    return p[component];
  }
  ...
  template <int dim, typename RangeNumberType>
  Tensor<1, dim, RangeNumberType>
  IdentityFunction<dim, RangeNumberType>::gradient(const Point<dim> &, const unsigned int component) const
  {
    AssertIndexRange(component, this->n_components);
  ...
  template <int dim, typename RangeNumberType>
  SymmetricTensor<2, dim, RangeNumberType>
  IdentityFunction<dim, RangeNumberType>::hessian(const Point<dim> &, const unsigned int) const
  {
    return SymmetricTensor<2, dim, RangeNumberType>();
  }
  ...
  template <int dim, typename RangeNumberType>
  RangeNumberType
  IdentityFunction<dim, RangeNumberType>::laplacian(const Point<dim> &, const unsigned int) const
  {
    return 0;
  }
} // namespace Functions
...
template <int dim, typename RangeNumberType>
void
ComponentSelectFunction<dim, RangeNumberType>::vector_value(const Point<dim> &,
...
template <int dim, typename RangeNumberType>
void
ComponentSelectFunction<dim, RangeNumberType>::vector_value_list(const std::vector<Point<dim>>        &points,
                                                                 std::vector<Vector<RangeNumberType>> &values) const
{
  Assert(values.size() == points.size(), ExcDimensionMismatch(values.size(), points.size()));
...
ComponentSelectFunction<dim, RangeNumberType>::memory_consumption() const
{
  ...
  return (sizeof(*this) - sizeof(Functions::ConstantFunction<dim, RangeNumberType>) +
...
template <int dim, typename RangeNumberType>
RangeNumberType
ScalarFunctionFromFunctionObject<dim, RangeNumberType>::value(const Point<dim> &p, const unsigned int component) const
{
  (void)component;
...
template <int dim, typename RangeNumberType>
VectorFunctionFromScalarFunctionObject<dim, RangeNumberType>::VectorFunctionFromScalarFunctionObject(
  const std::function<RangeNumberType(const Point<dim> &)> &function_object,
...
template <int dim, typename RangeNumberType>
RangeNumberType
VectorFunctionFromScalarFunctionObject<dim, RangeNumberType>::value(const Point<dim>  &p,
                                                                    const unsigned int component) const
{
  AssertIndexRange(component, this->n_components);
...
template <int dim, typename RangeNumberType>
void
VectorFunctionFromScalarFunctionObject<dim, RangeNumberType>::vector_value(const Point<dim>        &p,
...
VectorFunctionFromTensorFunction<dim, RangeNumberType>::value(const Point<dim> &p, const unsigned int component) const
{
  AssertIndexRange(component, this->n_components);
...
template <int dim, typename RangeNumberType>
inline void
VectorFunctionFromTensorFunction<dim, RangeNumberType>::vector_value(const Point<dim>        &p,
                                                                     Vector<RangeNumberType> &values) const
{
...
VectorFunctionFromTensorFunction<dim, RangeNumberType>::gradient(const Point<dim>  &p,
                                                                 const unsigned int component) const
{
  AssertIndexRange(component, this->n_components);
  ...
  // // <tt>selected_component + dim - 1</tt> elements of the <tt>Vector</tt>
  // // values and pick the correct one
  const Tensor<2, dim, RangeNumberType> tensor_gradient = tensor_function.gradient(p);
  ...
  std::vector<std::vector<Tensor<1, dim, RangeNumberType>>> &gradients) const
{
  ...
  for (unsigned int i = 0; i < points.size(); ++i)
    {
      Assert(gradients[i].size() == this->n_components, ExcDimensionMismatch(gradients[i].size(), this->n_components));
      vector_gradient(points[i], gradients[i]);
    }
}
...
template <int dim, typename RangeNumberType>
FunctionFromFunctionObjects<dim, RangeNumberType>::FunctionFromFunctionObjects(const unsigned int n_components,
...
template <int dim, typename RangeNumberType>
RangeNumberType
FunctionFromFunctionObjects<dim, RangeNumberType>::value(const Point<dim> &p, const unsigned int component) const
{
  AssertIndexRange(component, this->n_components);
...
template <int dim, typename RangeNumberType>
Tensor<1, dim, RangeNumberType>
FunctionFromFunctionObjects<dim, RangeNumberType>::gradient(const Point<dim> &p, const unsigned int component) const
{
  AssertIndexRange(component, this->n_components);

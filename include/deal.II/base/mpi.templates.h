...
#  endif
            const int ierr = MPI_Allreduce(values != output ? values.data() : MPI_IN_PLACE,
                 ...
                 const ArrayView<std::complex<T>>       &output)
      {
        ...
        if (job_supports_mpi())
          {
            const int ierr = MPI_Allreduce(values != output ?
    ...
    {
      T return_value{};
      internal::all_reduce(MPI_SUM, ArrayView<const T>(&t, 1), mpi_communicator, ArrayView<T>(&return_value, 1));
      return return_value;
    }
    ...
    sum(const Tensor<rank, dim, Number> &t, const MPI_Comm mpi_communicator)
    {
      ...
      for (unsigned int i = 0; i < Tensor<rank, dim, Number>::n_independent_components; ++i)
        array[i] = t[Tensor<rank, dim, Number>::unrolled_to_component_indices(i)];
    ...
    {
      T return_value{};
      internal::all_reduce(MPI_MAX, ArrayView<const T>(&t, 1), mpi_communicator, ArrayView<T>(&return_value, 1));
      return return_value;
    }
    ...
    {
      T return_value{};
      internal::all_reduce(MPI_MIN, ArrayView<const T>(&t, 1), mpi_communicator, ArrayView<T>(&return_value, 1));
      return return_value;
    }
    ...
    logical_or(const T &t, const MPI_Comm mpi_communicator)
    {
      ...
      T return_value{};
      internal::all_reduce(MPI_LOR, ArrayView<const T>(&t, 1), mpi_communicator, ArrayView<T>(&return_value, 1));
      return return_value;
    }
    ...
    logical_or(const T &values, const MPI_Comm mpi_communicator, U &results)
    {
      ...
      // this.
      const auto array_view_values  = make_array_view(values.cbegin(), values.cend());
      const auto array_view_results = make_array_view(results.begin(), results.end());

      using const_type = ArrayView<const typename decltype(array_view_values)::value_type>;
      logical_or(static_cast<const_type>(array_view_values), mpi_communicator, array_view_results);
    }
          ...
          for (unsigned int stride = 1; stride < nproc; stride *= 2)
            {
              unsigned int rank_recv =
                (2 * stride) * ((rank + nproc - root_process) % nproc / (2 * stride)) + root_process;
              ...
              if (rank_recv == rank) // process receives data
                {
                  MPI_Status status;
                  const auto ierr_1 = MPI_Probe(rank_send, internal::Tags::compute_union, comm, &status);
                  ...
                  const auto ierr_3 =
                    MPI_Recv(temp.data(), amount, MPI_CHAR, status.MPI_SOURCE, status.MPI_TAG, comm, MPI_STATUS_IGNORE);
              ...
              else if (rank_send == rank) // process sends data
                {
                  ...
                  const auto ierr =
                    MPI_Send(temp.data(), temp.size(), MPI_CHAR, rank_recv, internal::Tags::compute_union, comm);
                  AssertThrowMPI(ierr);
                }
            }
    ...
    template <typename T>
    T
    all_reduce(const T &vec, const MPI_Comm comm, const std::function<T(const T &, const T &)> &combiner)
    {
      if (n_mpi_processes(comm) > 1)
        {
          // 1) perform reduction
    ...
    {
      return Utilities::MPI::all_reduce<std::vector<T>>(vec, comm, [](const auto &set_1, const auto &set_2) {

...
namespace FETools
{
  namespace internal
  {
    ...
    public:
      ...
      template <class InVector>
      void
      extrapolate_parallel(const InVector &u2_relevant, const DoFHandler<dim, spacedim> &dof2, OutVector &u2);
    ...
    private:
      ...
      void
      exchange_data_on_cells(const std::vector<CellData> &cells_to_send, std::vector<CellData> &received_cells) const;
    ...
    public:
      ...
      template <class InVector>
      void
      extrapolate_parallel(const InVector & /*u2_relevant*/, const DoFHandler<1, 1> & /*dof2*/, OutVector & /*u2*/)
      {}
    };
    ...
    public:
      ...
      template <class InVector>
      void
      extrapolate_parallel(const InVector & /*u2_relevant*/, const DoFHandler<1, 2> & /*dof2*/, OutVector & /*u2*/)
      {}
    };
    ...
    public:
      ...
      template <class InVector>
      void
      extrapolate_parallel(const InVector & /*u2_relevant*/, const DoFHandler<1, 3> & /*dof2*/, OutVector & /*u2*/)
      {}
    };
    ...
    template <int dim, int spacedim, class OutVector>
    ExtrapolateImplementation<dim, spacedim, OutVector>::ExtrapolateImplementation()
      : round(0)
    {}
    ...
    template <int dim, int spacedim, class OutVector>
    ExtrapolateImplementation<dim, spacedim, OutVector>::CellData::CellData(const unsigned int dofs_per_cell)
    ...
    template <int dim, int spacedim, class OutVector>
    bool
    ExtrapolateImplementation<dim, spacedim, OutVector>::CellData::operator<(const CellData &rhs) const
    {
    ...
    template <class Archive>
    void
    ExtrapolateImplementation<dim, spacedim, OutVector>::CellData::serialize(Archive &ar, const unsigned int)
    {
      ...
      // bytes
      ar &boost::serialization::make_array(reinterpret_cast<char *>(&quadrant), sizeof(quadrant));
    }
          ...
          for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; ++c)
            {
              if (dealii::internal::p4est::functions<dim>::quadrant_overlaps_tree(
                    const_cast<typename dealii::internal::p4est::types<dim>::tree *>(&tree), &p4est_child[c]) == false)
                {
                  ...
                  if (pos == -1)
                    {
                      ...
                      add_new_need(forest, tree_index, dealii_cell->child(c), p4est_child[c], new_needs);
                    }
                  else
                    {
              ...
              else
                {
                  ...
                  get_interpolated_dof_values(
                    forest, tree, tree_index, dealii_cell->child(c), p4est_child[c], u, tmp1, new_needs);
                }
              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  if (!(on_refined_neighbor && dofs_on_refined_neighbors[indices[j]] > dealii_cell->level()))
                    ::dealii::internal::ElementAccess<OutVector>::set(local_values(j), indices[j], u);
                }
            }
        }
      else
        {
          Assert(local_values.size() == dofs_per_cell, ExcDimensionMismatch(local_values.size(), dofs_per_cell));
          Assert(u.size() == dealii_cell->get_dof_handler().n_dofs(),
                 ExcDimensionMismatch(u.size(), dealii_cell->get_dof_handler().n_dofs()));
          ...
          for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; ++c)
            {
              if (tmp.size() > 0)
                fe.get_prolongation_matrix(c, dealii_cell->refinement_case()).vmult(tmp, local_values);
    ...
    template <int dim, int spacedim, class OutVector>
    void
    ExtrapolateImplementation<dim, spacedim, OutVector>::compute_needs(const DoFHandler<dim, spacedim> &dof2,
                                                                       std::vector<CellData>           &new_needs)
    {
      const parallel::distributed::Triangulation<dim, spacedim> *tr =
        (dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(&dof2.get_triangulation()));
      ...
      for (const auto &cell : dof2.cell_iterators_on_level(0))
        {
          if (dealii::internal::p4est::tree_exists_locally<dim>(
                tr->parallel_forest, tr->coarse_cell_to_p4est_tree_permutation[cell->index()]) == false)
            continue;
          ...
          traverse_tree_recursively(*tr->parallel_forest, *tree, tree_index, cell, p4est_coarse_cell, new_needs);
        }
    }
      ...
      if (p4est_has_children)
        {
          ...
          for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; ++c)
            {
              traverse_tree_recursively(forest, tree, tree_index, dealii_cell->child(c), p4est_child[c], new_needs);
            }
        }
    }
    ...
    template <class InVector>
    void
    ExtrapolateImplementation<dim, spacedim, OutVector>::compute_cells(const DoFHandler<dim, spacedim> &dof2,
                                                                       const InVector                  &u,
                                                                       std::vector<CellData> &cells_to_compute,
                                                                       std::vector<CellData> &computed_cells,
                                                                       std::vector<CellData> &new_needs)
      ...
      for (const auto &cell : dof2.cell_iterators_on_level(0))
        {
          ...
          if ((trees.find(tree_index) == trees.end()) ||
              (dealii::internal::p4est::tree_exists_locally<dim>(tr->parallel_forest, tree_index) == false))
            continue;
      ...
      if (pos != -1)
        {
          ...
          // compute dof values
          get_interpolated_dof_values(
            forest, tree, tree_index, dealii_cell, p4est_cell, u, cells_to_compute[pos].dof_values, tmp);
      ...
      std::vector<CellData>       &received_cells) const
    {
      ...
      const auto create_request = [&cells_to_send](const types::subdomain_id other_rank) -> std::vector<CellData> {
      ...
      const auto process_request = [&received_cells](const unsigned int           other_rank,
      ...
      Utilities::MPI::ConsensusAlgorithms::selector<std::vector<CellData>>(
        std::vector<types::subdomain_id>(destinations.begin(), destinations.end()),
        create_request,
        process_request,
        communicator);
    }
    ...
    template <int dim, int spacedim, class OutVector>
    int
    ExtrapolateImplementation<dim, spacedim, OutVector>::cell_data_search(const CellData              &cell_data,
    ...
    template <int dim, int spacedim, class OutVector>
    void
    ExtrapolateImplementation<dim, spacedim, OutVector>::cell_data_insert(const CellData        &cell_data,
    ...
    ExtrapolateImplementation<dim, spacedim, OutVector>::compute_all_non_local_data(
      ...
      const InVector                  &u)
    {
      std::vector<CellData> cells_we_need, cells_to_compute, received_cells, received_needs, new_needs, computed_cells,
        cells_to_send;
    ...
    template <class InVector>
    void
    ExtrapolateImplementation<dim, spacedim, OutVector>::extrapolate_parallel(const InVector &u2_relevant,
                                                                              const DoFHandler<dim, spacedim> &dof2,
                                                                              OutVector                       &u2)
    {
      const parallel::distributed::Triangulation<dim, spacedim> *tr =
      ...
      Assert(tr != nullptr, ExcMessage("Extrapolate in parallel only works for parallel distributed triangulations!"));
                        ...
                        for (unsigned int i = 0; i < fe.n_dofs_per_face(face); ++i)
                          {
                            ...
                            const unsigned int level =
                              index_stored ? std::max(cell->level(), dofs_on_refined_neighbors[index]) : cell->level();
        ...
        for (const auto &cell : dof2.cell_iterators_on_level(0))
          {
            ...
            queue.push({*tr->parallel_forest, *tree, tree_index, cell, p4est_coarse_cell});
          }
      }
          ...
          if (dealii_cell->has_children())
            {
              ...
              for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; ++c)
                queue.push({forest, tree, tree_index, dealii_cell->child(c), p4est_child[c]});
            }
          queue.pop();
        }
    ...
    struct BlockTypeHelper<VectorType, std::enable_if_t<IsBlockVector<VectorType>::value>>
    {
...
#    ifdef DEAL_II_TRILINOS_WITH_TPETRA
    ...
    void
    reinit_distributed(const DoFHandler<dim, spacedim> &dh, LinearAlgebra::TpetraWrappers::Vector<Number> &vector)
    {
      const parallel::distributed::Triangulation<dim, spacedim> *parallel_tria =
        dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(&dh.get_triangulation());
      Assert(parallel_tria != nullptr, ExcNotImplemented());
...
#    endif
    ...
    template <int dim, int spacedim>
    void
    reinit_distributed(const DoFHandler<dim, spacedim> &dh, LinearAlgebra::EpetraWrappers::Vector &vector)
    {
      const parallel::distributed::Triangulation<dim, spacedim> *parallel_tria =
        dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(&dh.get_triangulation());
      Assert(parallel_tria != nullptr, ExcNotImplemented());
...
#  endif
    ...
    template <int dim, int spacedim, typename Number>
    void
    reinit_distributed(const DoFHandler<dim, spacedim> &dh, LinearAlgebra::distributed::Vector<Number> &vector)
    {
      const parallel::distributed::Triangulation<dim, spacedim> *parallel_tria =
        dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(&dh.get_triangulation());
      Assert(parallel_tria != nullptr, ExcNotImplemented());
    ...
    template <int dim, int spacedim, typename Number>
    void
    reinit_ghosted(const DoFHandler<dim, spacedim> &dh, LinearAlgebra::distributed::Vector<Number> &vector)
    {
      const parallel::distributed::Triangulation<dim, spacedim> *parallel_tria =
        dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(&dh.get_triangulation());
      Assert(parallel_tria != nullptr, ExcNotImplemented());
              ...
              OutVector                                               &u2)
  {
    ...
    if (dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(&dof2.get_triangulation()) != nullptr)
      {
        Assert(dof1.get_fe(0).reference_cell() == ReferenceCells::get_hypercube<dim>(), ExcNotImplemented());
        Assert(dof2.get_fe(0).reference_cell() == ReferenceCells::get_hypercube<dim>(), ExcNotImplemented());

...
namespace FETools
{
              ...
              OutVector                                               &u2)
  {
    ...
    Vector<typename OutVector::value_type> u1_local(dof1.get_fe_collection().max_dofs_per_cell());
    Vector<typename OutVector::value_type> u2_local(dof2.get_fe_collection().max_dofs_per_cell());
    ...
    typename DoFHandler<dim, spacedim>::active_cell_iterator cell1 = dof1.begin_active(), endc1 = dof1.end();
    typename DoFHandler<dim, spacedim>::active_cell_iterator cell2 = dof2.begin_active(), endc2 = dof2.end();
    (void)endc2;
    ...
    for (; cell1 != endc1; ++cell1, ++cell2)
      if ((cell1->subdomain_id() == subdomain_id) || (subdomain_id == numbers::invalid_subdomain_id))
        {
          ...
          const bool hanging_nodes_not_allowed =
            ((cell2->get_fe().n_dofs_per_vertex() != 0) && (constraints.n_constraints() == 0));
          ...
          if (interpolation_matrices[&cell1->get_fe()][&cell2->get_fe()].get() == nullptr)
            {
              auto interpolation_matrix = std::make_unique<FullMatrix<double>>(dofs_per_cell2, dofs_per_cell1);

              get_interpolation_matrix(cell1->get_fe(), cell2->get_fe(), *interpolation_matrix);

              interpolation_matrices[&cell1->get_fe()][&cell2->get_fe()] = std::move(interpolation_matrix);
            }
      ...
      if (locally_owned_dofs.is_element(i))
        {
          ...
          const typename OutVector::value_type val = ::dealii::internal::ElementAccess<OutVector>::get(u2, i);
                   ...
                   OutVector                          &u1_interpolated)
  {
    ...
    Assert(u1.size() == dof1.n_dofs(), ExcDimensionMismatch(u1.size(), dof1.n_dofs()));
    Assert(u1_interpolated.size() == dof1.n_dofs(), ExcDimensionMismatch(u1_interpolated.size(), dof1.n_dofs()));
    ...
    Vector<typename OutVector::value_type> u1_local(dof1.get_fe_collection().max_dofs_per_cell());
    Vector<typename OutVector::value_type> u1_int_local(dof1.get_fe_collection().max_dofs_per_cell());

    const types::subdomain_id subdomain_id = dof1.get_triangulation().locally_owned_subdomain();

    typename DoFHandler<dim, spacedim>::active_cell_iterator cell = dof1.begin_active(), endc = dof1.end();

    // map from possible FE objects in
    // dof1 to the back_interpolation
    // matrices
    std::map<const FiniteElement<dim> *, std::unique_ptr<FullMatrix<double>>> interpolation_matrices;
      ...
      if ((cell->subdomain_id() == subdomain_id) || (subdomain_id == numbers::invalid_subdomain_id))
        {
          ...
          // continuous no hanging node constraints are allowed.
          const bool hanging_nodes_not_allowed =
            (cell->get_fe().n_dofs_per_vertex() != 0) || (fe2.n_dofs_per_vertex() != 0);
          ...
          if (interpolation_matrices[&cell->get_fe()] == nullptr)
            {
              interpolation_matrices[&cell->get_fe()] =
                std::make_unique<FullMatrix<double>>(dofs_per_cell1, dofs_per_cell1);
              get_back_interpolation_matrix(cell->get_fe(), fe2, *interpolation_matrices[&cell->get_fe()]);
            }
  ...
  namespace internal
  {
    ...
    void
    back_interpolate(const DoFHandler<dim, spacedim>                                 &dof1,
                     const AffineConstraints<PETScWrappers::MPI::Vector::value_type> &constraints1,
                     const PETScWrappers::MPI::Vector                                &u1,
                     const DoFHandler<dim, spacedim>                                 &dof2,
                     const AffineConstraints<PETScWrappers::MPI::Vector::value_type> &constraints2,
                     PETScWrappers::MPI::Vector                                      &u1_interpolated)
    {
      ...
      PETScWrappers::MPI::Vector u2(dof2_locally_owned_dofs, dof2_locally_relevant_dofs, u1.get_mpi_communicator());
      u2 = u2_out;
      interpolate(dof2, u2, dof1, constraints1, u1_interpolated);
    }
    ...
    void
    back_interpolate(const DoFHandler<dim, spacedim>                                             &dof1,
                     const AffineConstraints<typename TrilinosWrappers::MPI::Vector::value_type> &constraints1,
                     const TrilinosWrappers::MPI::Vector                                         &u1,
                     const DoFHandler<dim, spacedim>                                             &dof2,
                     const AffineConstraints<typename TrilinosWrappers::MPI::Vector::value_type> &constraints2,
                     TrilinosWrappers::MPI::Vector                                               &u1_interpolated)
    {
      ...
      interpolate(dof1, u1, dof2, constraints2, u2_out);
      TrilinosWrappers::MPI::Vector u2(dof2_locally_owned_dofs, dof2_locally_relevant_dofs, u1.get_mpi_communicator());
      u2 = u2_out;
      interpolate(dof2, u2, dof1, constraints1, u1_interpolated);
    }
    ...
    void
    back_interpolate(const DoFHandler<dim, spacedim>                                                  &dof1,
                     const AffineConstraints<typename TrilinosWrappers::MPI::BlockVector::value_type> &constraints1,
                     const TrilinosWrappers::MPI::BlockVector                                         &u1,
                     const DoFHandler<dim, spacedim>                                                  &dof2,
                     const AffineConstraints<typename TrilinosWrappers::MPI::BlockVector::value_type> &constraints2,
                     TrilinosWrappers::MPI::BlockVector                                               &u1_interpolated)
    {
      ...
      TrilinosWrappers::MPI::Vector u2_out(dof2_locally_owned_dofs, mpi_communicator);
      interpolate(dof1, u1, dof2, constraints2, u2_out);
      TrilinosWrappers::MPI::Vector u2(dof2_locally_owned_dofs, dof2_locally_relevant_dofs, mpi_communicator);
      u2 = u2_out;
      interpolate(dof2, u2, dof1, constraints1, u1_interpolated);
    }
    ...
    void
    back_interpolate(const DoFHandler<dim, spacedim> &,
                     const AffineConstraints<typename LinearAlgebra::EpetraWrappers::Vector::value_type> &,
...
#  ifdef DEAL_II_TRILINOS_WITH_TPETRA
    ...
    void
    back_interpolate(const DoFHandler<dim, spacedim> &,
                     const AffineConstraints<typename LinearAlgebra::TpetraWrappers::Vector<Number>::value_type> &,
...
#  endif
                   ...
                   OutVector                                               &u1_interpolated)
  {
    ...
    if (dof1.get_fe().n_dofs_per_vertex() == 0 && dof2.get_fe().n_dofs_per_vertex() == 0 &&
                           ...
                           OutVector                          &u1_difference)
  {
    Assert(dof1.get_fe(0).n_components() == fe2.n_components(),
           ExcDimensionMismatch(dof1.get_fe(0).n_components(), fe2.n_components()));
    Assert(u1.size() == dof1.n_dofs(), ExcDimensionMismatch(u1.size(), dof1.n_dofs()));
    Assert(u1_difference.size() == dof1.n_dofs(), ExcDimensionMismatch(u1_difference.size(), dof1.n_dofs()));
      ...
      if ((cell->subdomain_id() == subdomain_id) || (subdomain_id == numbers::invalid_subdomain_id))
        {
          ...
          // continuous no hanging node constraints are allowed.
          const bool hanging_nodes_not_allowed =
            (dof1.get_fe().n_dofs_per_vertex() != 0) || (fe2.n_dofs_per_vertex() != 0);
  ...
  namespace internal
  {
    ...
    void
    interpolation_difference(const DoFHandler<dim, spacedim>                                    &dof1,
                             const AffineConstraints<TrilinosWrappers::MPI::Vector::value_type> &constraints1,
                             const TrilinosWrappers::MPI::Vector                                &u1,
                             const DoFHandler<dim, spacedim>                                    &dof2,
                             const AffineConstraints<TrilinosWrappers::MPI::Vector::value_type> &constraints2,

...
namespace internal
{
  ...
  public:
    ...
    template <typename Accessor>
    types::global_cell_index
    translate(const TriaIterator<Accessor> &cell, const types::global_cell_index i) const;
  ...
  private:
    ...
    convert_cell_id_binary_type_to_level_coarse_cell_id(const typename CellId::binary_type &binary_representation);
  ...
  template <int dim>
  CellIDTranslator<dim>::CellIDTranslator(const types::global_cell_index n_coarse_cells,
                                          const types::global_cell_index n_global_levels)
    ...
    , n_global_levels(n_global_levels)
  {
    ...
    for (unsigned int i = 0; i < n_global_levels; ++i)
      max_cell_index += Utilities::pow<std::uint64_t>(GeometryInfo<dim>::max_children_per_cell, i) * n_coarse_cells;

    max_cell_index -= 1;

    AssertThrow(max_cell_index <= std::numeric_limits<types::global_cell_index>::max(),
                ExcMessage("You have exceeded the maximal number of possible indices this function "
                           "can handle. The current setup (n_coarse_cells=" +
                           std::to_string(n_coarse_cells) + ", n_global_levels=" + std::to_string(n_global_levels) +
                           ") requires " + std::to_string(max_cell_index + 1) +
    ...
    for (unsigned int i = 0; i < n_global_levels; ++i)
      tree_sizes.push_back(tree_sizes.back() +
                           Utilities::pow<types::global_cell_index>(GeometryInfo<dim>::max_children_per_cell, i) *
                             n_coarse_cells);
  }
  ...
  CellIDTranslator<dim>::size() const
  {
    return n_coarse_cells *
           (Utilities::pow<types::global_cell_index>(GeometryInfo<dim>::max_children_per_cell, n_global_levels) - 1);
  }
  ...
  {
    static_assert(dim == Accessor::dimension && dim == Accessor::structure_dimension,
    ...
    id += convert_cell_id_binary_type_to_level_coarse_cell_id(
      CellAccessor<Accessor::dimension, Accessor::space_dimension>(*cell).id().template to_binary<dim>());
  ...
  template <typename Accessor>
  types::global_cell_index
  CellIDTranslator<dim>::translate(const TriaIterator<Accessor> &cell, const types::global_cell_index i) const
  {
    static_assert(dim == Accessor::dimension && dim == Accessor::structure_dimension,
                  "The information can only be queried for cells.");

    return (translate(cell) - tree_sizes[cell->level()]) * GeometryInfo<dim>::max_children_per_cell + i +
  ...
  CellIDTranslator<dim>::to_cell_id(const types::global_cell_index id) const
  {
    ...
    for (types::global_cell_index l = 0; l < level; ++l)
      {
        child_indices.push_back(id_temp % GeometryInfo<dim>::max_children_per_cell);
        ...
        for (unsigned int j = 0; j < children_per_value; ++j)
          {
            unsigned int cell_index =
              (((binary_representation[binary_entry] >> (j * dim))) & (GeometryInfo<dim>::max_children_per_cell - 1));
            level_coarse_cell_id = level_coarse_cell_id * GeometryInfo<dim>::max_children_per_cell + cell_index;
            ++child_level;

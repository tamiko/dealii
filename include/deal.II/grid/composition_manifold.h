...
{
  return std::make_unique<CompositionManifold<dim, spacedim, chartdim, intermediate_dim, dim1, dim2>>(*F, *G);
}
  ...
  const Point<chartdim> &chart_point) const
{
  ...
  DerivativeForm<1, intermediate_dim, spacedim> DG = G->push_forward_gradient(F->push_forward(chart_point));

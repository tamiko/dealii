...
namespace internal
{
  namespace TriaAccessorImplementation
  {
    ...
    inline double
    diameter(const boost::container::small_vector<Point<spacedim>, GeometryInfo<dim>::vertices_per_cell> vertices)
    {
      const ReferenceCell reference_cell = ReferenceCell::n_vertices_to_type(dim, vertices.size());
      ...
      if (reference_cell == ReferenceCells::Line)
        ...
        return std::max(
          {(vertices[1] - vertices[0]).norm(), (vertices[2] - vertices[1]).norm(), (vertices[2] - vertices[0]).norm()});
...
inline TriaAccessorBase<structdim, dim, spacedim>::TriaAccessorBase(const Triangulation<dim, spacedim> *tria,
                                                                    const int                           level,
                                                                    const int                           index,
                                                                    const AccessorData *)
...
template <int structdim, int dim, int spacedim>
inline TriaAccessorBase<structdim, dim, spacedim>::TriaAccessorBase(const TriaAccessorBase<structdim, dim, spacedim> &a)
...
TriaAccessorBase<structdim, dim, spacedim>::copy_from(const TriaAccessorBase<structdim, dim, spacedim> &a)
{
  ...
  if (structdim != dim)
    {
      Assert((present_level == 0) || (present_level == -1) || (present_level == -2), ExcInternalError());
    }
}
...
{
  Assert(tria == a.tria || tria == nullptr || a.tria == nullptr, TriaAccessorExceptions::ExcCantCompareIterators());
  return ((tria == a.tria) && (present_level == a.present_level) && (present_index == a.present_index));
}
...
TriaAccessorBase<structdim, dim, spacedim>::operator++()
{
  ...
  if (structdim != dim)
    ...
    {
      while (this->present_index >= static_cast<int>(this->tria->levels[this->present_level]->cells.n_objects()))
        {
      ...
      while (this->present_index < 0)
        {
          ...
          // else
          this->present_index = this->tria->levels[this->present_level]->cells.n_objects() - 1;
        }
    }
}
...
template <int structdim, int dim, int spacedim>
InvalidAccessor<structdim, dim, spacedim>::InvalidAccessor(const void *, const int, const int, const AccessorData *)
{
  Assert(false,
...
template <int structdim, int dim, int spacedim>
InvalidAccessor<structdim, dim, spacedim>::InvalidAccessor(const InvalidAccessor &)
{
  Assert(false,
         ExcMessage("You are attempting an invalid conversion between "
                    "iterator/accessor types. The constructor you call "
...
template <int structdim, int dim, int spacedim>
void
InvalidAccessor<structdim, dim, spacedim>::set_user_index(const unsigned int) const
{
  Assert(false,
...
template <int structdim, int dim, int spacedim>
void
InvalidAccessor<structdim, dim, spacedim>::set_manifold_id(const types::manifold_id) const
{
  Assert(false,
  ...
  namespace TriaAccessorImplementation
  {
    ...
    struct Implementation
    {
      ...
      line_index(const TriaAccessor<2, dim, spacedim> &accessor, const unsigned int i)
      {
        ...
        return accessor.objects().cells[accessor.present_index * max_faces_per_cell + i];
      }
      ...
      {
        const auto pair       = accessor.reference_cell().standard_line_to_face_and_line_index(i);
        const auto quad_index = pair[0];
        const auto line_index =
          accessor.reference_cell().standard_to_real_face_line(pair[1],
                                                               pair[0],
      ...
      inline static bool
      face_orientation(const TriaAccessor<2, 2, spacedim> &accessor, const unsigned int face)
      {
      ...
      inline static bool
      face_orientation(const TriaAccessor<3, 3, 3> &accessor, const unsigned int face)
      {
        return accessor.tria->levels[accessor.present_level]->face_orientations.get_orientation(
      ...
      combined_face_orientation(const TriaAccessor<1, dim, spacedim> & /*accessor*/, const unsigned int /*face*/)
      {
      ...
      template <int dim, int spacedim>
      inline static unsigned char
      combined_face_orientation(const TriaAccessor<2, dim, spacedim> &accessor, const unsigned int face)
      {
        return line_orientation(accessor, face) == true ? ReferenceCell::default_combined_face_orientation() :
      ...
      inline static unsigned char
      combined_face_orientation(const TriaAccessor<3, 3, 3> &accessor, const unsigned int face)
      {
        AssertIndexRange(face, accessor.n_faces());
        return accessor.tria->levels[accessor.present_level]->face_orientations.get_combined_orientation(
          accessor.present_index * GeometryInfo<3>::faces_per_cell + face);
      }
      ...
      {
        AssertIndexRange(face, accessor.n_faces());
        return accessor.tria->levels[accessor.present_level]->face_orientations.get_flip(
          accessor.present_index * GeometryInfo<3>::faces_per_cell + face);
      }
      ...
      inline static bool
      face_rotation(const TriaAccessor<3, 3, 3> &accessor, const unsigned int face)
      {
        AssertIndexRange(face, accessor.n_faces());

        return accessor.tria->levels[accessor.present_level]->face_orientations.get_rotation(
          accessor.present_index * GeometryInfo<3>::faces_per_cell + face);
      }
      ...
      template <int spacedim>
      inline static bool
      line_orientation(const TriaAccessor<2, 2, spacedim> &accessor, const unsigned int line)
      {
        AssertIndexRange(line, accessor.n_lines());
        if (accessor.tria->levels[accessor.present_level]->face_orientations.n_objects() == 0)
      ...
      line_orientation(const TriaAccessor<2, 3, spacedim> &accessor, const unsigned int line)
      {
        ...
        return accessor.tria->faces
          ->quads_line_orientations[accessor.present_index * GeometryInfo<3>::lines_per_face + line];
      }


      inline static bool
      line_orientation(const TriaAccessor<3, 3, 3> &accessor, const unsigned int line)
      {
        Assert(accessor.used(), TriaAccessorExceptions::ExcCellNotUsed());
        AssertIndexRange(line, accessor.n_lines());
        ...
        const auto pair       = accessor.reference_cell().standard_line_to_face_and_line_index(line);
        const auto quad_index = pair[0];
      ...
      template <int structdim, int dim, int spacedim>
      inline static void
      set_combined_face_orientation(const TriaAccessor<structdim, dim, spacedim> &,
                                    const unsigned int,
                                    const unsigned char)
      {
        Assert(false, ExcInternalError());
      ...
      {
        AssertIndexRange(face, accessor.n_faces());
        accessor.tria->levels[accessor.present_level]->face_orientations.set_combined_orientation(
          accessor.present_index * GeometryInfo<3>::faces_per_cell + face, combined_orientation);
      }
      ...
      template <int spacedim>
      inline static void
      set_line_orientation(const TriaAccessor<2, 3, spacedim> &accessor, const unsigned int line, const bool value)
      {
        Assert(accessor.used(), TriaAccessorExceptions::ExcCellNotUsed());
        AssertIndexRange(line, accessor.n_lines());
        Assert(accessor.present_index * GeometryInfo<3>::lines_per_face + line <
        ...
        // quads as part of 3d hexes can have non-standard orientation
        accessor.tria->faces->quads_line_orientations[accessor.present_index * GeometryInfo<3>::lines_per_face + line] =
          value;
      }


      inline static void
      set_line_orientation(const TriaAccessor<3, 3, 3> &, const unsigned int, const bool)
      {
      ...
      vertex_index(const TriaAccessor<1, dim, spacedim> &accessor, const unsigned int corner)
      {
        ...
        return accessor.objects().cells[accessor.present_index * max_faces_per_cell + corner];
      }
      ...
      {
        const auto pair       = accessor.reference_cell().standard_vertex_to_face_and_vertex_index(corner);
        const auto line_index = pair[0];
        const auto vertex_index =
          accessor.reference_cell().standard_to_real_face_vertex(pair[1],
                                                                 pair[0],
                                                                 accessor.line_orientation(line_index));
      ...
      inline static unsigned int
      vertex_index(const TriaAccessor<3, 3, 3> &accessor, const unsigned int corner)
      {
        const auto pair       = accessor.reference_cell().standard_vertex_to_face_and_vertex_index(corner);
      ...
      get_line_indices_of_cell(const TriaAccessor<structdim, dim, spacedim> &cell)
      {
        ...
        if (ref_cell == ReferenceCells::Hexahedron)
            ...
            for (unsigned int f = 4; f < 6; ++f)
              {
                const unsigned char orientation =
                  cell.get_triangulation().levels[cell.level()]->face_orientations.get_combined_orientation(
                ...
                // line_index() call.
                const std::array<unsigned int, 4> my_indices{{ref_cell.standard_to_real_face_line(0, f, orientation),
                ...
                for (unsigned int l = 0; l < 4; ++l)
                  line_indices[4 * (f - 4) + l] = quad->line_index(my_indices[l]);
              }
            for (unsigned int f = 0; f < 2; ++f)
              {
                const unsigned char orientation =
                  cell.get_triangulation().levels[cell.level()]->face_orientations.get_combined_orientation(
                    cell.index() * GeometryInfo<3>::faces_per_cell + f);
                const std::array<unsigned int, 2> my_indices{{ref_cell.standard_to_real_face_line(0, f, orientation),
                                                              ref_cell.standard_to_real_face_line(1, f, orientation)}};
                const auto                        quad = cell.quad(f);
          ...
          {
            std::array<unsigned int, 3>       orientations{{combined_face_orientation(cell, 0),
                                                            combined_face_orientation(cell, 1),
                                                            combined_face_orientation(cell, 2)}};
            const std::array<unsigned int, 6> my_indices{{ref_cell.standard_to_real_face_line(0, 0, orientations[0]),
        ...
        if (ref_cell == ReferenceCells::Hexahedron)
            ...
            for (unsigned int f = 4; f < 6; ++f)
              {
                ...
                const std::array<bool, 4>         my_orientations{
                          {ref_cell.standard_vs_true_line_orientation(0, orientation, quad->line_orientation(my_indices[0])),
                           ref_cell.standard_vs_true_line_orientation(1, orientation, quad->line_orientation(my_indices[1])),
                           ref_cell.standard_vs_true_line_orientation(2, orientation, quad->line_orientation(my_indices[2])),
                           ref_cell.standard_vs_true_line_orientation(3, orientation, quad->line_orientation(my_indices[3]))}};
                for (unsigned int l = 0; l < 4; ++l)
        ...
        else if (ref_cell == ReferenceCells::Tetrahedron)
          {
            ...
            const std::array<unsigned int, 6> my_indices{{ref_cell.standard_to_real_face_line(0, 0, orientations[0]),
                                                          ...
                                                          ref_cell.standard_to_real_face_line(1, 2, orientations[2])}};
            line_orientations[0] =
              ref_cell.standard_vs_true_line_orientation(0,
                                                         orientations[0],
                                                         cell.quad(0)->line_orientation(my_indices[0]));
            line_orientations[1] =
              ref_cell.standard_vs_true_line_orientation(1,
                                                         orientations[0],
                                                         cell.quad(0)->line_orientation(my_indices[1]));
            line_orientations[2] =
              ref_cell.standard_vs_true_line_orientation(2,
                                                         orientations[0],
...
template <int structdim, int dim, int spacedim>
inline TriaAccessor<structdim, dim, spacedim>::TriaAccessor(const Triangulation<dim, spacedim> *parent,
                                                            const int                           level,
                                                            const int                           index,
...
TriaAccessor<structdim, dim, spacedim>::reference_cell() const
{
  if (structdim == 0)
    ...
    return this->tria->levels[this->present_level]->reference_cell[this->present_index];
  else
  ...
  if (structdim == dim)
    {
      ...
      const std::size_t      my_index = static_cast<std::size_t>(this->present_index) * max_vertices_per_cell;
      AssertIndexRange(my_index + corner, this->tria->levels[this->present_level]->cell_vertex_indices_cache.size());
      const unsigned int vertex_index =
        this->tria->levels[this->present_level]->cell_vertex_indices_cache[my_index + corner];
...
template <int structdim, int dim, int spacedim>
inline typename dealii::internal::TriangulationImplementation::Iterators<dim, spacedim>::line_iterator
TriaAccessor<structdim, dim, spacedim>::line(const unsigned int i) const
{
  // checks happen in line_index
  return typename dealii::internal::TriangulationImplementation::Iterators<dim, spacedim>::line_iterator(this->tria,
                                                                                                         0,
                                                                                                         line_index(i));
}
...
template <int structdim, int dim, int spacedim>
inline typename dealii::internal::TriangulationImplementation::Iterators<dim, spacedim>::quad_iterator
TriaAccessor<structdim, dim, spacedim>::quad(const unsigned int i) const
{
  // checks happen in quad_index
  return typename dealii::internal::TriangulationImplementation::Iterators<dim, spacedim>::quad_iterator(this->tria,
                                                                                                         0,
                                                                                                         quad_index(i));
}
...
TriaAccessor<structdim, dim, spacedim>::combined_face_orientation(const unsigned int face) const
{
  return dealii::internal::TriaAccessorImplementation::Implementation::combined_face_orientation(*this, face);
}
...
template <int structdim, int dim, int spacedim>
inline void
TriaAccessor<structdim, dim, spacedim>::set_line_orientation(const unsigned int line, const bool value) const
{
  Assert(used(), TriaAccessorExceptions::ExcCellNotUsed());
  AssertIndexRange(line, this->n_lines());
...
template <int structdim, int dim, int spacedim>
inline void
TriaAccessor<structdim, dim, spacedim>::set_combined_face_orientation(const unsigned int  face,
                                                                      const unsigned char combined_orientation) const
{
  Assert(used(), TriaAccessorExceptions::ExcCellNotUsed());
  AssertIndexRange(face, this->n_faces());

  dealii::internal::TriaAccessorImplementation::Implementation::set_combined_face_orientation(*this,
                                                                                              face,
  ...
  switch (structdim)
    {
      ...
      case 2:
        {
          const RefinementCase<2> this_refinement_case(static_cast<std::uint8_t>(refinement_case()));
...
TriaAccessor<structdim, dim, spacedim>::refinement_case() const
{
  ...
  switch (structdim)
    {
      case 1:
        return (RefinementCase<structdim>(this->objects().children[this->present_index] != -1 ?
                                            // cast the branches
                                            // here first to uchar
                                            // and then (above) to
                                            // RefinementCase<structdim>
      ...
      default:
        ...
        return (static_cast<RefinementCase<structdim>>(this->objects().refinement_cases[this->present_index]));
    }
}
...

{
  ...
  const TriaIterator<TriaAccessor<structdim, dim, spacedim>> q(this->tria,
  ...
  const TriaIterator<TriaAccessor<structdim, dim, spacedim>> &child) const
{
  ...
  Assert(false, ExcMessage("The given child is not a child of the current object."));
  return numbers::invalid_unsigned_int;
}
...
template <int structdim, int dim, int spacedim>
inline void
TriaAccessor<structdim, dim, spacedim>::set_refinement_case(const RefinementCase<structdim> &refinement_case) const
{
  Assert(this->state() == IteratorState::valid,
         TriaAccessorExceptions::ExcDereferenceInvalidObject<TriaAccessor>(*this));
  Assert(static_cast<unsigned int>(this->present_index) < this->objects().refinement_cases.size(),
...
template <int structdim, int dim, int spacedim>
void
TriaAccessor<structdim, dim, spacedim>::set_user_index(const unsigned int p) const
{
  Assert(this->used(), TriaAccessorExceptions::ExcCellNotUsed());
...
template <int structdim, int dim, int spacedim>
void
TriaAccessor<structdim, dim, spacedim>::recursively_set_user_index(const unsigned int p) const
{
  set_user_index(p);
...
template <int structdim, int dim, int spacedim>
void
TriaAccessor<structdim, dim, spacedim>::set_boundary_id_internal(const types::boundary_id boundary_ind) const
{
  Assert(structdim < dim, ExcImpossibleInDim(dim));
  Assert(this->used(), TriaAccessorExceptions::ExcCellNotUsed());
...
template <int structdim, int dim, int spacedim>
void
TriaAccessor<structdim, dim, spacedim>::set_all_boundary_ids(const types::boundary_id boundary_ind) const
{
...
template <int structdim, int dim, int spacedim>
void
TriaAccessor<structdim, dim, spacedim>::set_all_manifold_ids(const types::manifold_id manifold_ind) const
{
...
{
  boost::container::small_vector<Point<spacedim>, GeometryInfo<structdim>::vertices_per_cell> vertices(
    this->n_vertices());

  for (unsigned int v = 0; v < vertices.size(); ++v)
    vertices[v] = this->vertex(v);

  return internal::TriaAccessorImplementation::diameter<structdim, spacedim>(vertices);
}
...
template <int dim, int spacedim>
double
CellAccessor<dim, spacedim>::diameter(const Mapping<dim, spacedim> &mapping) const
{
  return internal::TriaAccessorImplementation::diameter<dim, spacedim>(mapping.get_vertices(
    typename Triangulation<dim, spacedim>::cell_iterator(this->tria, this->level(), this->index())));
}
...
TriaAccessor<structdim, dim, spacedim>::enclosing_ball() const
{
  ...
  if (structdim == 1)
    return std::make_pair((this->vertex(1) + this->vertex(0)) * 0.5, (this->vertex(1) - this->vertex(0)).norm() * 0.5);
      ...
      case 3:
        {
          ...
          const std::vector<double>::const_iterator it        = std::max_element(diagonals.begin(), diagonals.end());
...
inline TriaAccessor<0, dim, spacedim>::TriaAccessor(const TriaAccessor<structdim2, dim2, spacedim2> &)
  : tria(nullptr)
...
template <int dim, int spacedim>
template <int structdim2, int dim2, int spacedim2>
inline TriaAccessor<0, dim, spacedim>::TriaAccessor(const InvalidAccessor<structdim2, dim2, spacedim2> &)
  : tria(nullptr)
  , global_vertex_index(numbers::invalid_unsigned_int)
{
  Assert(false, ExcImpossibleInDim(0));
...
template <int dim, int spacedim>
inline bool
TriaAccessor<0, dim, spacedim>::operator<(const TriaAccessor<0, dim, spacedim> &other) const
{
  Assert(tria == other.tria, TriaAccessorExceptions::ExcCantCompareIterators());
...
template <int dim, int spacedim>
inline unsigned char
TriaAccessor<0, dim, spacedim>::combined_face_orientation(const unsigned int /*face*/)
{
  return 0;
}
...
template <int dim, int spacedim>
inline unsigned int
TriaAccessor<0, dim, spacedim>::child_iterator_to_index(const TriaIterator<TriaAccessor<0, dim, spacedim>> &)
{
  return numbers::invalid_unsigned_int;
}
...
template <int spacedim>
template <int structdim2, int dim2, int spacedim2>
inline TriaAccessor<0, 1, spacedim>::TriaAccessor(const TriaAccessor<structdim2, dim2, spacedim2> &)
  : tria(nullptr)
  , vertex_kind(interior_vertex)
  , global_vertex_index(numbers::invalid_unsigned_int)
{
...
template <int spacedim>
template <int structdim2, int dim2, int spacedim2>
inline TriaAccessor<0, 1, spacedim>::TriaAccessor(const InvalidAccessor<structdim2, dim2, spacedim2> &)
  : tria(nullptr)
  , vertex_kind(interior_vertex)
  , global_vertex_index(numbers::invalid_unsigned_int)
{
...
template <int spacedim>
inline void
TriaAccessor<0, 1, spacedim>::copy_from(const TriaAccessorBase<0, 1, spacedim> &)
{
...
template <int spacedim>
inline bool
TriaAccessor<0, 1, spacedim>::operator<(const TriaAccessor<0, 1, spacedim> &other) const
{
  Assert(tria == other.tria, TriaAccessorExceptions::ExcCantCompareIterators());
...
{
  if (tria->vertex_to_manifold_id_map_1d->find(this->vertex_index()) != tria->vertex_to_manifold_id_map_1d->end())
...
template <int spacedim>
inline unsigned char
TriaAccessor<0, 1, spacedim>::combined_face_orientation(const unsigned int /*face*/)
{
  return 0;
}
...
template <int spacedim>
inline unsigned int
TriaAccessor<0, 1, spacedim>::child_iterator_to_index(const TriaIterator<TriaAccessor<0, 1, spacedim>> &)
{
  return numbers::invalid_unsigned_int;
}
...
{
  Assert(tria->vertex_to_boundary_id_map_1d->find(this->vertex_index()) != tria->vertex_to_boundary_id_map_1d->end(),
...
template <int spacedim>
inline void
TriaAccessor<0, 1, spacedim>::set_all_boundary_ids(const types::boundary_id b) const
{
  set_boundary_id(b);
}
...
template <int dim, int spacedim>
inline CellAccessor<dim, spacedim>::CellAccessor(const TriaAccessor<dim, dim, spacedim> &cell_accessor)
  ...
  namespace CellAccessorImplementation
  {
    ...
    get_face(const dealii::CellAccessor<1, spacedim> &cell, const unsigned int i)
    {
      dealii::TriaAccessor<0, 1, spacedim> a(
        ...
        dealii::internal::TriaAccessorImplementation::Implementation::vertex_index(cell, i));
...
{
  TriaIterator<CellAccessor<dim, spacedim>> q(this->tria, this->present_level + 1, this->child_index(i));
  ...
  const TriaIterator<TriaAccessor<dim - 1, dim, spacedim>> &face) const
{
  ...
  Assert(false, ExcMessage("The given face is not a face of the current cell."));
  return numbers::invalid_unsigned_int;
}
...
template <int dim, int spacedim>
inline boost::container::small_vector<TriaIterator<TriaAccessor<dim - 1, dim, spacedim>>,
...
CellAccessor<dim, spacedim>::face_iterators() const
{
  ...
  for (const unsigned int i : this->face_indices())
    face_iterators[i] = dealii::internal::CellAccessorImplementation::get_face(*this, i);
...
CellAccessor<dim, spacedim>::neighbor_index(const unsigned int face_no) const
{
  ...
  return this->tria->levels[this->present_level]
    ->neighbors[this->present_index * GeometryInfo<dim>::faces_per_cell + face_no]
    .second;
}
...
CellAccessor<dim, spacedim>::refine_flag_set() const
{
  ...
  // flag is not cleared).
  Assert(this->is_active() || !this->tria->levels[this->present_level]->refine_flags[this->present_index],
...
{
  Assert(this->used() && this->is_active(), ExcRefineCellNotActive());
  this->tria->levels[this->present_level]->refine_flags[this->present_index] = RefinementCase<dim>::no_refinement;
}
                                                      ...
                                                      const RefinementCase<dim - 1> &face_refinement_case) const
{
  ...
  AssertIndexRange(face_no, this->n_faces());
  AssertIndexRange(face_refinement_case, RefinementCase<dim>::isotropic_refinement + 1);
  ...
  RefinementCase<dim> new_ref_case =
    (old_ref_case | GeometryInfo<dim>::min_cell_refinement_case_for_face_refinement(face_refinement_case,
                                                                                    face_no,
...
CellAccessor<dim, spacedim>::flag_for_line_refinement(const unsigned int line_no) const
{
  ...
  RefinementCase<dim> old_ref_case = refine_flag_set(),
                      new_ref_case =
                        old_ref_case | GeometryInfo<dim>::min_cell_refinement_case_for_line_refinement(line_no);
  set_refine_flag(new_ref_case);
  // return, whether we had to change the
  // refinement flag
  return new_ref_case != old_ref_case;
  ...
  switch (static_cast<std::uint8_t>(face(face_no)->refinement_case()))
    {
      ...
      case RefinementCase<3>::cut_x:
        if (face(face_no)->child(0)->has_children())
          {
            Assert(face(face_no)->child(0)->refinement_case() == RefinementCase<2>::cut_y, ExcInternalError());
            if (face(face_no)->child(1)->has_children())
              {
                Assert(face(face_no)->child(1)->refinement_case() == RefinementCase<2>::cut_y, ExcInternalError());
      ...
      case RefinementCase<3>::cut_y:
        if (face(face_no)->child(0)->has_children())
          {
            Assert(face(face_no)->child(0)->refinement_case() == RefinementCase<2>::cut_x, ExcInternalError());
            if (face(face_no)->child(1)->has_children())
              {
                Assert(face(face_no)->child(1)->refinement_case() == RefinementCase<2>::cut_x, ExcInternalError());
...
CellAccessor<dim, spacedim>::coarsen_flag_set() const
{
  ...
  // executed and for some reason the refine
  // flag is not cleared).
  Assert(this->is_active() || !this->tria->levels[this->present_level]->coarsen_flags[this->present_index],
         ExcRefineCellNotActive());
...
{
  TriaIterator<CellAccessor<dim, spacedim>> q(this->tria, neighbor_level(face_no), neighbor_index(face_no));
...
{
  Assert(this->is_active(), ExcMessage("is_locally_owned() can only be called on active cells!"));
  ...
  const types::subdomain_id locally_owned_subdomain = this->tria->locally_owned_subdomain();
  return (locally_owned_subdomain == numbers::invalid_subdomain_id || this->subdomain_id() == locally_owned_subdomain);
...
{
  Assert(this->is_active(), ExcMessage("is_ghost() can only be called on active cells!"));
  if (this->has_children())
    return false;
  ...
  return (locally_owned_subdomain != numbers::invalid_subdomain_id && subdomain_id != locally_owned_subdomain &&
...
{
  Assert(this->is_active(), ExcMessage("is_artificial() can only be called on active cells!"));
#ifndef DEAL_II_WITH_MPI
  return false;
#else
  ...
  return (this->tria->locally_owned_subdomain() != numbers::invalid_subdomain_id &&
...
{
  Assert(this->used(), TriaAccessorExceptions::ExcCellNotUsed());
  Assert(this->is_active(), ExcMessage("subdomain_id() can only be called on active cells!"));
...
{
  Assert(this->used(), TriaAccessorExceptions::ExcCellNotUsed());
  Assert(this->is_active(), ExcMessage("global_active_cell_index() can only be called on active cells!"));

  return this->tria->levels[this->present_level]->global_active_cell_indices[this->present_index];
}
...
{
  return this->tria->levels[this->present_level]->global_level_cell_indices[this->present_index];
}

...
private:
  ...
  DeclException1(ArpackExcArpackIdo, int, << "This ido " << arg1 << " is not supported. Check documentation of ARPACK");
  ...
  DeclException1(ArpackExcArpackInfoMaxIt, int, << "Maximum number " << arg1 << " of iterations reached.");
  ...
  , symmetric(symmetric)
{
  ...
  // Check for possible options for asymmetric problems
    ...
    {
      Assert(eigenvalue_of_interest != algebraically_largest,
             ExcMessage("'largest algebraic part' can only be used for symmetric problems!"));
      Assert(eigenvalue_of_interest != algebraically_smallest,
             ExcMessage("'smallest algebraic part' can only be used for symmetric problems!"));
      Assert(eigenvalue_of_interest != both_ends, ExcMessage("'both ends' can only be used for symmetric problems!"));
    }
}
                    ...
                    const unsigned int                 n_eigenvalues)
{
  ...
  if (additional_data.symmetric)
    ...
    Assert(nev + 1 <= eigenvectors.size(), ArpackExcInvalidEigenvectorSizeNonsymmetric(nev, eigenvectors.size()));

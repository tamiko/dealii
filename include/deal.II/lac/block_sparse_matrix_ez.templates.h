...
{
  Assert(n_block_rows() == m.n_block_rows(), ExcDimensionMismatch(n_block_rows(), m.n_block_rows()));
  Assert(n_block_cols() == m.n_block_cols(), ExcDimensionMismatch(n_block_cols(), m.n_block_cols()));
...
BlockSparseMatrixEZ<number>::collect_sizes()
{
  ...
  for (unsigned int c = 1; c < columns; ++c)
    for (unsigned int r = 0; r < rows; ++r)
      Assert(row_sizes[r] == blocks[r][c].m(), ExcDimensionMismatch(row_sizes[r], blocks[r][c].m()));
  ...
  for (unsigned int r = 1; r < rows; ++r)
    for (unsigned int c = 0; c < columns; ++c)
      Assert(col_sizes[c] == blocks[r][c].n(), ExcDimensionMismatch(col_sizes[c], blocks[r][c].n()));

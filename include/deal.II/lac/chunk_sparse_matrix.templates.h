...
namespace internal
{
  ...
  namespace ChunkSparseMatrixImplementation
  {
    ...
    inline void
    chunk_vmult_add(const size_type chunk_size, const MatrixIterator matrix, const SrcIterator src, DstIterator dst)
    {
    ...
    template <typename MatrixIterator, typename SrcIterator, typename DstIterator>
    inline void
    chunk_Tvmult_add(const size_type chunk_size, const MatrixIterator matrix, const SrcIterator src, DstIterator dst)
    {
     ...
     * individual chunk.
     */
    template <typename result_type, typename MatrixIterator, typename SrcIterator1, typename SrcIterator2>
    inline result_type
                          ...
                          OutVector                  &dst)
    {
      ...
      const size_type last_regular_row =
        n_filled_last_rows > 0 ? std::min(m / chunk_size, static_cast<size_type>(end_row)) : end_row;
      ...
      for (unsigned int chunk_row = begin_row; chunk_row < last_regular_row; ++chunk_row)
        {
          const number *const val_end_of_row = &values[rowstart[chunk_row + 1] * chunk_size * chunk_size];
          while (val_ptr != val_end_of_row)
            {
              if (*colnum_ptr != irregular_col)
                chunk_vmult_add(chunk_size, val_ptr, src.begin() + *colnum_ptr * chunk_size, dst_ptr);
              else
      ...
      Assert(std::size_t(val_ptr - values) == rowstart[end_row] * chunk_size * chunk_size, ExcInternalError());
    }
  } // namespace ChunkSparseMatrixImplementation
} // namespace internal
...
template <typename number>
ChunkSparseMatrix<number>::ChunkSparseMatrix(const ChunkSparsityPattern &c, const IdentityMatrix &id)
  : cols(nullptr, "ChunkSparseMatrix")
  , val(nullptr)
  , max_len(0)
{
...
ChunkSparseMatrix<number>::operator=(const double d)
{
  ...
  Assert(cols->sparsity_pattern.compressed || cols->empty(), ChunkSparsityPattern::ExcNotCompressed());
  ...
  // per row on average.
  const unsigned int matrix_size = cols->sparsity_pattern.n_nonzero_elements() * cols->chunk_size * cols->chunk_size;
  ...
  if (matrix_size > grain_size)
    parallel::apply_to_subranges(
      ...
      [this](const unsigned int begin, const unsigned int end) {
        internal::ChunkSparseMatrixImplementation::zero_subrange(begin, end, val.get());
      },
      grain_size);
...
ChunkSparseMatrix<number>::copy_from(const ChunkSparseMatrix<somenumber> &matrix)
{
  ...
  std::copy(matrix.val.get(),
            matrix.val.get() + cols->sparsity_pattern.n_nonzero_elements() * chunk_size * chunk_size,
            val.get());
...
ChunkSparseMatrix<number>::add(const number factor, const ChunkSparseMatrix<somenumber> &matrix)
{
  ...
  const number *const end_ptr    = val.get() + cols->sparsity_pattern.n_nonzero_elements() * chunk_size * chunk_size;
                                            ...
                                            number         *values) const
{
  ...
  const number *val_ptr =
    &val[(it - cols->sparsity_pattern.begin(0)) * chunk_size * chunk_size + (row % chunk_size) * chunk_size];
...
ChunkSparseMatrix<number>::vmult_add(OutVector &dst, const InVector &src) const
{
  ...
  parallel::apply_to_subranges(
    ...
    [this, &src, &dst](const unsigned int begin_row, const unsigned int end_row) {
      internal::ChunkSparseMatrixImplementation::vmult_add_on_subrange(*cols,
                                                                       begin_row,
                                                                       end_row,
                                                                       val.get(),
...
ChunkSparseMatrix<number>::Tvmult_add(OutVector &dst, const InVector &src) const
{
  ...
  const bool rows_have_padding = (m() % cols->chunk_size != 0), cols_have_padding = (n() % cols->chunk_size != 0);
      ...
      while (val_ptr != val_end_of_row)
        {
          if ((cols_have_padding == false) || (*colnum_ptr != cols->sparsity_pattern.n_cols() - 1))
            internal::ChunkSparseMatrixImplementation::chunk_Tvmult_add(cols->chunk_size,
                                                                        val_ptr,
  ...
  for (size_type chunk_row = 0; chunk_row < n_regular_chunk_rows; ++chunk_row)
    {
      ...
      while (val_ptr != val_end_of_row)
        {
          if ((cols_have_padding == false) || (*colnum_ptr != cols->sparsity_pattern.n_cols() - 1))
            result += internal::ChunkSparseMatrixImplementation::chunk_matrix_scalar_product<somenumber>(
...
template <typename somenumber>
somenumber
ChunkSparseMatrix<number>::matrix_scalar_product(const Vector<somenumber> &u, const Vector<somenumber> &v) const
{
  Assert(cols != nullptr, ExcNeedsSparsityPattern());
  Assert(val != nullptr, ExcNotInitialized());
...
ChunkSparseMatrix<number>::l1_norm() const
{
  ...
  for (size_type chunk_row = 0; chunk_row < n_chunk_rows; ++chunk_row)
    for (size_type j = cols->sparsity_pattern.rowstart[chunk_row]; j < cols->sparsity_pattern.rowstart[chunk_row + 1];
         ++j)
      for (size_type r = 0; r < cols->chunk_size; ++r)
        for (size_type s = 0; s < cols->chunk_size; ++s)
          column_sums(cols->sparsity_pattern.colnums[j] * cols->chunk_size + s) +=
            numbers::NumberTraits<number>::abs(val[j * cols->chunk_size * cols->chunk_size + r * cols->chunk_size + s]);
  ...
  for (size_type chunk_row = 0; chunk_row < n_regular_chunk_rows; ++chunk_row)
    {
      ...
      while (val_ptr != val_end_of_row)
        {
          if ((cols_have_padding == false) || (*colnum_ptr != cols->sparsity_pattern.n_cols() - 1))
            internal::ChunkSparseMatrixImplementation::chunk_vmult_subtract(cols->chunk_size,
                                                                            val_ptr,
...
template <typename somenumber>
void
ChunkSparseMatrix<number>::SOR_step(Vector<somenumber> &v, const Vector<somenumber> &b, const number /*om*/) const
{
  (void)v;
  (void)b;
  Assert(cols != nullptr, ExcNeedsSparsityPattern());
...
template <typename somenumber>
void
ChunkSparseMatrix<number>::TSOR_step(Vector<somenumber> &v, const Vector<somenumber> &b, const number /*om*/) const
{
  (void)v;
  (void)b;
  Assert(cols != nullptr, ExcNeedsSparsityPattern());
...
template <typename somenumber>
void
ChunkSparseMatrix<number>::SSOR_step(Vector<somenumber> &v, const Vector<somenumber> &b, const number om) const
{
  SOR_step(v, b, om);
  TSOR_step(v, b, om);
}
...
ChunkSparseMatrix<number>::block_write(std::ostream &out) const
{
  ...
  out.write(reinterpret_cast<const char *>(val.get()),
            reinterpret_cast<const char *>(val.get() + max_len) - reinterpret_cast<const char *>(val.get()));
  out << ']';

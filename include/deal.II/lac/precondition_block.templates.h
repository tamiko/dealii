...
template <typename MatrixType, typename inverse_type>
void
PreconditionBlock<MatrixType, inverse_type>::initialize(const MatrixType &M, const AdditionalData parameters)
{
          ...
          for (; entry != row_end; ++entry)
            {
              ...
              const size_type inverse_permuted_column = permuted ? inverse_permutation[column] : column;
          ...
          for (; entry != row_end; ++entry)
            {
              ...
              const size_type inverse_permuted_column = permuted ? inverse_permutation[column] : column;
              b_cell_row -= entry->value() * prev(column);
              if (!this->inverses_ready() && inverse_permuted_column < block_end && column >= block_start)
                {
                  const size_type column_cell = column - block_start;
                  // We need the
                  // transpose of the
...
template <typename MatrixType, typename inverse_type>
void
PreconditionBlock<MatrixType, inverse_type>::set_permutation(const std::vector<size_type> &new_permutation,
                                                             const std::vector<size_type> &new_inverse_permutation)
{
  ...
  if (!this->inverses_ready())
    {
      ...
      for (unsigned int cell = 0; cell < this->size(); ++cell)
        {
          for (row = cell * this->blocksize, row_cell = 0; row_cell < this->blocksize; ++row_cell, ++row)
            {
              b_cell(row_cell) = src(row);
              for (size_type column_cell = 0, column = cell * this->blocksize; column_cell < this->blocksize;
                   ++column_cell, ++column)
...
template <typename number2>
void
PreconditionBlockJacobi<MatrixType, inverse_type>::vmult(Vector<number2> &dst, const Vector<number2> &src) const
{
...
template <typename number2>
void
PreconditionBlockJacobi<MatrixType, inverse_type>::Tvmult(Vector<number2> &dst, const Vector<number2> &src) const
{
  do_vmult(dst, src, false);
}
...
template <typename number2>
void
PreconditionBlockJacobi<MatrixType, inverse_type>::vmult_add(Vector<number2> &dst, const Vector<number2> &src) const
{
...
template <typename number2>
void
PreconditionBlockJacobi<MatrixType, inverse_type>::Tvmult_add(Vector<number2> &dst, const Vector<number2> &src) const
{
  do_vmult(dst, src, true);
}
...
template <typename number2>
void
PreconditionBlockJacobi<MatrixType, inverse_type>::step(Vector<number2> &dst, const Vector<number2> &src) const
{
...
template <typename number2>
void
PreconditionBlockJacobi<MatrixType, inverse_type>::Tstep(Vector<number2> &dst, const Vector<number2> &src) const
{
  GrowingVectorMemory<Vector<number2>>            mem;
  typename VectorMemory<Vector<number2>>::Pointer aux(mem);
  aux->reinit(dst);
                                                        ...
                                                        const bool) const
{
  ...
  for (unsigned int cell = 0; cell < this->size(); ++cell)
    {
      const size_type permuted_block_start = permuted ? this->permutation[block_start] : block_start;
      ...
      for (row = permuted_block_start, row_cell = 0; row_cell < this->blocksize; ++row_cell, ++row)
        {
          ...
          for (; entry != row_end; ++entry)
            {
              const size_type column                  = entry->column();
              const size_type inverse_permuted_column = permuted ? this->inverse_permutation[column] : column;
...
template <typename number2>
void
PreconditionBlockSOR<MatrixType, inverse_type>::vmult(Vector<number2> &dst, const Vector<number2> &src) const
{
...
template <typename number2>
void
PreconditionBlockSOR<MatrixType, inverse_type>::vmult_add(Vector<number2> &dst, const Vector<number2> &src) const
{
...
template <typename number2>
void
PreconditionBlockSOR<MatrixType, inverse_type>::Tvmult(Vector<number2> &dst, const Vector<number2> &src) const
{
...
template <typename number2>
void
PreconditionBlockSOR<MatrixType, inverse_type>::Tvmult_add(Vector<number2> &dst, const Vector<number2> &src) const
{
...
template <typename number2>
void
PreconditionBlockSOR<MatrixType, inverse_type>::step(Vector<number2> &dst, const Vector<number2> &src) const
{
...
template <typename number2>
void
PreconditionBlockSOR<MatrixType, inverse_type>::Tstep(Vector<number2> &dst, const Vector<number2> &src) const
{
...
template <typename number2>
void
PreconditionBlockSSOR<MatrixType, inverse_type>::vmult(Vector<number2> &dst, const Vector<number2> &src) const
{
  Vector<number2> help;
  help.reinit(dst);
...
template <typename number2>
void
PreconditionBlockSSOR<MatrixType, inverse_type>::Tvmult(Vector<number2> &dst, const Vector<number2> &src) const
{
  Vector<number2> help;
  help.reinit(dst);
...
template <typename number2>
void
PreconditionBlockSSOR<MatrixType, inverse_type>::step(Vector<number2> &dst, const Vector<number2> &src) const
{
  this->forward_step(dst, dst, src, false);
...
template <typename number2>
void
PreconditionBlockSSOR<MatrixType, inverse_type>::Tstep(Vector<number2> &dst, const Vector<number2> &src) const
{
  this->backward_step(dst, dst, src, true);

...
namespace LinearAlgebra
{
  namespace internal
  {
    ...
    {
      static void
      import_elements(const std::shared_ptr<const ::dealii::Utilities::MPI::Partitioner> &communication_pattern,
                      const Number                                                       *values,
    ...
    struct read_write_vector_functions<Number, ::dealii::MemorySpace::Host>
    {
      ...
      import_elements(const std::shared_ptr<const ::dealii::Utilities::MPI::Partitioner> &communication_pattern,
                      ...
                      ::dealii::LinearAlgebra::ReadWriteVector<Number>                   &rw_vector)
      {
        distributed::Vector<Number, ::dealii::MemorySpace::Host> tmp_vector(communication_pattern);

        const unsigned int n_elements = communication_pattern->locally_owned_size();
        ...
        if (operation == VectorOperation::add)
          ...
          for (size_type i = 0; i < stored.n_elements(); ++i)
            rw_vector.local_element(i) = get_min(tmp_vector(stored.nth_index_in_set(i)), rw_vector.local_element(i));
        else if (operation == VectorOperation::max)
          for (size_type i = 0; i < stored.n_elements(); ++i)
            rw_vector.local_element(i) = get_max(tmp_vector(stored.nth_index_in_set(i)), rw_vector.local_element(i));
        else
          for (size_type i = 0; i < stored.n_elements(); ++i)
            rw_vector.local_element(i) = tmp_vector(stored.nth_index_in_set(i));
      }
    ...
    struct read_write_vector_functions<Number, ::dealii::MemorySpace::Default>
    {
                      ...
                      ::dealii::LinearAlgebra::ReadWriteVector<Number>                   &rw_vector)
      {
        ...
        Kokkos::deep_copy(Kokkos::View<Number *, Kokkos::HostSpace>(tmp_vector.begin(), n_elements),
                          Kokkos::View<const Number *, ::dealii::MemorySpace::Default::kokkos_space>(values,
                                                                                                     n_elements));
        tmp_vector.update_ghost_values();
  ...
  ReadWriteVector<Number>::resize_val(const size_type new_alloc_size)
  {
    if (new_alloc_size == 0)
      ...
      {
        Number *new_values;
        Utilities::System::posix_memalign(reinterpret_cast<void **>(&new_values), 64, sizeof(Number) * new_alloc_size);
  ...
  void
  ReadWriteVector<Number>::reinit(const ReadWriteVector<Number2> &v, const bool omit_zeroing_entries)
  {
  ...
  template <typename Number>
  void
  ReadWriteVector<Number>::reinit(const IndexSet &locally_stored_indices, const bool omit_zeroing_entries)
  {
  ...
  ReadWriteVector<Number>::apply(const Functor &func)
  {
    ...
    dealii::internal::VectorOperations::parallel_for(functor, 0, locally_owned_size(), thread_loop_partitioner);
  }
    ...
    if (locally_owned_size() > 0)
      {
        ...
        dealii::internal::VectorOperations::parallel_for(copier, 0, locally_owned_size(), thread_loop_partitioner);
      }
  ...
  namespace internal
  {
                         ...
                         ReadWriteVector<Number> &rw_vector)
    {
      ...
      if (operation == VectorOperation::add)
        ...
        for (size_type i = 0; i < stored.n_elements(); ++i)
          rw_vector.local_element(i) = get_min(values(stored.nth_index_in_set(i)), rw_vector.local_element(i));
      else if (operation == VectorOperation::max)
        for (size_type i = 0; i < stored.n_elements(); ++i)
          rw_vector.local_element(i) = get_max(values(stored.nth_index_in_set(i)), rw_vector.local_element(i));
      else
        for (size_type i = 0; i < stored.n_elements(); ++i)
          rw_vector.local_element(i) = values(stored.nth_index_in_set(i));
    }
    ...
    const std::shared_ptr<const Utilities::MPI::CommunicationPatternBase> &communication_pattern)
  {
    ...
    if (communication_pattern.get() == nullptr)
      {
        comm_pattern = std::make_shared<Utilities::MPI::Partitioner>(vec.locally_owned_elements(),
                                                                     get_stored_elements(),
    ...
    internal::read_write_vector_functions<Number, MemorySpace>::import_elements(comm_pattern,
                                                                                vec.begin(),
                                                                                operation,
                                                                                *this);
  }
  ...
  namespace internal
  {
    ...
    void
    copy_petsc_vector(const PETSC_Number *petsc_start_ptr, const PETSC_Number *petsc_end_ptr, Number *ptr)
    {
    ...
    const std::shared_ptr<const Utilities::MPI::CommunicationPatternBase> & /*communication_pattern*/)
  {
    ...
    Assert(petsc_vec.locally_owned_elements() == stored_elements, StandardExceptions::ExcInvalidState());

    // get a representation of the vector and copy it
    const PetscScalar *start_ptr;
    PetscErrorCode     ierr = VecGetArrayRead(static_cast<const Vec &>(petsc_vec), &start_ptr);
...
#  ifdef DEAL_II_TRILINOS_WITH_TPETRA
    ...
    const std::shared_ptr<const Utilities::MPI::CommunicationPatternBase> &communication_pattern)
  {
    ...
    if (communication_pattern == nullptr)
      {
        ...
        // reused.
        if ((source_elements.size() == source_stored_elements.size()) && (source_elements == source_stored_elements))
          {
            tpetra_comm_pattern = std::dynamic_pointer_cast<const TpetraWrappers::CommunicationPattern>(comm_pattern);
            if (tpetra_comm_pattern == nullptr)
              tpetra_comm_pattern = std::make_shared<const TpetraWrappers::CommunicationPattern>(
    ...
    Tpetra::Vector<Number, int, types::signed_global_dof_index> target_vector(tpetra_export.getSourceMap());
    ...
    switch (operation)
      {
        ...
        case VectorOperation::min:
          ...
          for (size_type i = 0; i < size; ++i)
            {
              Assert(std::imag(new_values[i]) == 0.,
                     ExcMessage("VectorOperation::min is not defined if there is an imaginary part!)"));
              Assert(std::imag(values[i]) == 0.,
                     ExcMessage("VectorOperation::min is not defined if there is an imaginary part!)"));
        ...
        case VectorOperation::max:
          for (size_type i = 0; i < size; ++i)
            {
              Assert(std::imag(new_values[i]) == 0.,
                     ExcMessage("VectorOperation::max is not defined if there is an imaginary part!)"));
              Assert(std::imag(values[i]) == 0.,
                     ExcMessage("VectorOperation::max is not defined if there is an imaginary part!)"));
              if (std::real(new_values[i]) - std::real(values[i]) > 0.0)
                values[i] = new_values[i];
    ...
    if (communication_pattern == nullptr)
      {
        ...
        if ((source_elements.size() == source_stored_elements.size()) && (source_elements == source_stored_elements))
          {
            epetra_comm_pattern = std::dynamic_pointer_cast<const EpetraWrappers::CommunicationPattern>(comm_pattern);
      ...
      {
        epetra_comm_pattern =
          std::dynamic_pointer_cast<const EpetraWrappers::CommunicationPattern>(communication_pattern);
    ...
    if (operation == VectorOperation::insert)
      {
        ...
        AssertThrow(err == 0, ExcMessage("Epetra Import() failed with error code: " + std::to_string(err)));
...
#  endif
  ...
  template <typename Number>
  void
  ReadWriteVector<Number>::import_elements(const LinearAlgebra::CUDAWrappers::Vector<Number> &cuda_vec,
                                           VectorOperation::values                            operation,
    ...
    if (operation == VectorOperation::insert)
      {
        cudaError_t error_code =
          cudaMemcpy(values.get(), cuda_vec.get_values(), n_elements * sizeof(Number), cudaMemcpyDeviceToHost);
    ...
    else if (operation == VectorOperation::add)
      {
        ...
        cudaError_t         error_code =
          cudaMemcpy(tmp.data(), cuda_vec.get_values(), n_elements * sizeof(Number), cudaMemcpyDeviceToHost);
        AssertCuda(error_code);
  ...
  ReadWriteVector<Number>::memory_consumption() const
  {
    ...
    memory += sizeof(Number) * static_cast<std::size_t>(this->locally_owned_size());
  ...
  template <typename Number>
  void
  ReadWriteVector<Number>::print(std::ostream &out, const unsigned int precision, const bool scientific) const
  {
...
#  ifdef DEAL_II_TRILINOS_WITH_TPETRA
  ...
  ReadWriteVector<Number>::create_tpetra_comm_pattern(const IndexSet &source_index_set, const MPI_Comm mpi_comm)
  {
    source_stored_elements = source_index_set;
    TpetraWrappers::CommunicationPattern epetra_comm_pattern(source_stored_elements, stored_elements, mpi_comm);
    comm_pattern =
      std::make_shared<TpetraWrappers::CommunicationPattern>(source_stored_elements, stored_elements, mpi_comm);
...
#  endif
  ...
  ReadWriteVector<Number>::create_epetra_comm_pattern(const IndexSet &source_index_set, const MPI_Comm mpi_comm)
  {
    source_stored_elements = source_index_set;
    EpetraWrappers::CommunicationPattern epetra_comm_pattern(source_stored_elements, stored_elements, mpi_comm);
    comm_pattern =
      std::make_shared<EpetraWrappers::CommunicationPattern>(source_stored_elements, stored_elements, mpi_comm);

  ...
  namespace TensorProductMatrixSymmetricSum
  {
                 ...
                 const std::array<const Number *, dim> &derivative_matrix)
    {
      if (n_rows_1d_templated == n_rows_1d)
        vmult<n_rows_1d_templated>(dst, src, tmp, n_rows_1d, mass_matrix, derivative_matrix);
    ...
    {
      if (n_rows_1d_templated == n_rows_1d)
        apply_inverse<n_rows_1d_templated>(dst, src, n_rows_1d, eigenvectors, eigenvalues, inverted_eigenvalues);
      else if (n_rows_1d_templated < FDM_N_ROWS_MAX)
        ...
        apply_inverse<0>(dst, src, n_rows_1d, eigenvectors, eigenvalues, inverted_eigenvalues);
    }
  } // namespace TensorProductMatrixSymmetricSum
} // namespace internal

...
namespace TrilinosWrappers
{
  ...
  public:
     ...
     */
    void
    initialize(const SparseMatrix &matrix, const AdditionalData &additional_data = AdditionalData());
  };
  ...
  public:
    ...
    struct AdditionalData
    {
       ...
       */
      void
      set_parameters(Teuchos::ParameterList              &parameter_list,
       ...
       */
      void
      set_operator_null_space(Teuchos::ParameterList              &parameter_list,
                              std::unique_ptr<Epetra_MultiVector> &distributed_constant_modes,
                              const Epetra_RowMatrix              &matrix) const;
     ...
     */
    void
    initialize(const Epetra_RowMatrix &matrix, const AdditionalData &additional_data = AdditionalData());
...
#  if defined(DOXYGEN) || defined(DEAL_II_TRILINOS_WITH_MUELU)
  ...
  public:
     ...
     */
    void
    initialize(const Epetra_CrsMatrix &matrix, const AdditionalData &additional_data = AdditionalData());
...
#  ifndef DOXYGEN
  ...
  {
    Assert(dst.trilinos_partitioner().SameAs(preconditioner->OperatorRangeMap()), ExcNonMatchingMaps("dst"));
    Assert(src.trilinos_partitioner().SameAs(preconditioner->OperatorDomainMap()), ExcNonMatchingMaps("src"));
  ...
  {
    AssertDimension(dst.size(), preconditioner->OperatorDomainMap().NumMyElements());
    AssertDimension(src.size(), preconditioner->OperatorRangeMap().NumMyElements());
    Epetra_Vector tril_dst(View, preconditioner->OperatorDomainMap(), dst.begin());
    Epetra_Vector tril_src(View, preconditioner->OperatorRangeMap(), const_cast<double *>(src.begin()));
  ...
  {
    AssertDimension(dst.locally_owned_size(), preconditioner->OperatorDomainMap().NumMyElements());
    AssertDimension(src.locally_owned_size(), preconditioner->OperatorRangeMap().NumMyElements());
    Epetra_Vector tril_dst(View, preconditioner->OperatorDomainMap(), dst.begin());
    Epetra_Vector tril_src(View, preconditioner->OperatorRangeMap(), const_cast<double *>(src.begin()));

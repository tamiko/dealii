...
namespace internal
{
  ...
  public:
    static void
    add(const typename VectorType::value_type value, const types::global_dof_index i, VectorType &V);
  ...
  template <>
  inline void
  ElementAccess<LinearAlgebra::EpetraWrappers::Vector>::add(const double                           value,
                                                            const types::global_dof_index          i,
                                                            LinearAlgebra::EpetraWrappers::Vector &V)
  {
    // Extract local indices in the vector.
    Epetra_FEVector                   vector     = V.trilinos_vector();
    TrilinosWrappers::types::int_type trilinos_i = vector.Map().LID(static_cast<TrilinosWrappers::types::int_type>(i));
  ...
  template <>
  inline void
  ElementAccess<LinearAlgebra::EpetraWrappers::Vector>::set(const double                           value,
                                                            const types::global_dof_index          i,
                                                            LinearAlgebra::EpetraWrappers::Vector &V)
  {
    // Extract local indices in the vector.
  ...
  template <>
  inline double
  ElementAccess<LinearAlgebra::EpetraWrappers::Vector>::get(const LinearAlgebra::EpetraWrappers::Vector &V,
                                                            const types::global_dof_index                i)
  {
    // Extract local indices in the vector.
    Epetra_FEVector                   vector     = V.trilinos_vector();
...
#  ifdef DEAL_II_TRILINOS_WITH_TPETRA
  ...
  {
    // Extract local indices in the vector.
    Tpetra::Vector<NumberType, int, types::signed_global_dof_index> vector = V.trilinos_vector();
    TrilinosWrappers::types::int_type                               trilinos_i =
      vector.getMap()->getLocalElement(static_cast<TrilinosWrappers::types::int_type>(i));
...
#    if !DEAL_II_TRILINOS_VERSION_GTE(13, 2, 0)
    vector.template sync<
      typename Tpetra::Vector<NumberType, int, types::signed_global_dof_index>::device_type::memory_space>();
#    endif
  }
...
#    endif
    ...
    const types::global_dof_index                            i)
  {
    // Extract local indices in the vector.
#    if DEAL_II_TRILINOS_VERSION_GTE(13, 2, 0)
    const Tpetra::Vector<NumberType, int, types::signed_global_dof_index> &vector = V.trilinos_vector();

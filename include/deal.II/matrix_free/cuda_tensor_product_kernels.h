...
namespace CUDAWrappers
{
  namespace internal
  {
    ...
    apply_1d(const Kokkos::TeamPolicy<MemorySpace::Default::kokkos_space::execution_space>::member_type &team_member,
             const Kokkos::View<Number *, MemorySpace::Default::kokkos_space>                            shape_data,
             const ViewTypeIn                                                                            in,
             ViewTypeOut                                                                                 out)
    {
      Number t[n_q_points_1d];
      Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, n_q_points_1d), [&](const int &q) {
        t[q] = 0;
        ...
        for (int k = 0; k < n_q_points_1d; ++k)
          {
            const unsigned int shape_idx  = dof_to_quad ? (q + k * n_q_points_1d) : (k + q * n_q_points_1d);
    ...
    template <int n_q_points_1d,
              ...
              typename ViewTypeOut>
    DEAL_II_HOST_DEVICE void
    apply_2d(const Kokkos::TeamPolicy<MemorySpace::Default::kokkos_space::execution_space>::member_type &team_member,
             const Kokkos::View<Number *, MemorySpace::Default::kokkos_space>                            shape_data,
             const ViewTypeIn                                                                            in,
             ViewTypeOut                                                                                 out)
    {
      using TeamType = Kokkos::TeamPolicy<MemorySpace::Default::kokkos_space::execution_space>::member_type;
      constexpr unsigned int n_q_points = Utilities::pow(n_q_points_1d, 2);

      Number t[n_q_points];
      auto   thread_policy =
        Kokkos::TeamThreadMDRange<Kokkos::Rank<2>, TeamType>(team_member, n_q_points_1d, n_q_points_1d);
      Kokkos::parallel_for(thread_policy, [&](const int i, const int j) {
        ...
        for (int k = 0; k < n_q_points_1d; ++k)
          {
            const unsigned int shape_idx  = dof_to_quad ? (j + k * n_q_points_1d) : (k + j * n_q_points_1d);
            const unsigned int source_idx = (direction == 0) ? (k + n_q_points_1d * i) : (i + n_q_points_1d * k);
      ...
      Kokkos::parallel_for(thread_policy, [&](const int i, const int j) {
        ...
        const unsigned int destination_idx = (direction == 0) ? (j + n_q_points_1d * i) : (i + n_q_points_1d * j);
    ...
    template <int n_q_points_1d,
              ...
              typename ViewTypeOut>
    DEAL_II_HOST_DEVICE void
    apply_3d(const Kokkos::TeamPolicy<MemorySpace::Default::kokkos_space::execution_space>::member_type &team_member,
             const Kokkos::View<Number *, MemorySpace::Default::kokkos_space>                            shape_data,
             const ViewTypeIn                                                                            in,
             ViewTypeOut                                                                                 out)
    {
      ...
      Number t[n_q_points];
      auto   thread_policy =
        Kokkos::TeamThreadMDRange<Kokkos::Rank<3>, TeamType>(team_member, n_q_points_1d, n_q_points_1d, n_q_points_1d);
      Kokkos::parallel_for(thread_policy, [&](const int i, const int j, const int q) {
        const int q_point = i + j * n_q_points_1d + q * n_q_points_1d * n_q_points_1d;
        t[q_point]        = 0;
        ...
        for (int k = 0; k < n_q_points_1d; ++k)
          {
            const unsigned int shape_idx  = dof_to_quad ? (q + k * n_q_points_1d) : (k + q * n_q_points_1d);
            const unsigned int source_idx = (direction == 0) ? (k + n_q_points_1d * (i + n_q_points_1d * j)) :
      ...
      Kokkos::parallel_for(thread_policy, [&](const int i, const int j, const int q) {
        const int          q_point         = i + j * n_q_points_1d + q * n_q_points_1d * n_q_points_1d;
        const unsigned int destination_idx = (direction == 0) ? (q + n_q_points_1d * (i + n_q_points_1d * j)) :
    ...
    template <int dim,
              ...
              typename ViewTypeOut>
    DEAL_II_HOST_DEVICE void
    apply(const Kokkos::TeamPolicy<MemorySpace::Default::kokkos_space::execution_space>::member_type &team_member,
          const Kokkos::View<Number *, MemorySpace::Default::kokkos_space>                            shape_data,
          const ViewTypeIn                                                                            in,
          ViewTypeOut                                                                                 out)
    {
#if KOKKOS_VERSION >= 40000
      if constexpr (dim == 1)
        apply_1d<n_q_points_1d, Number, direction, dof_to_quad, add, in_place>(team_member, shape_data, in, out);
      if constexpr (dim == 2)
        apply_2d<n_q_points_1d, Number, direction, dof_to_quad, add, in_place>(team_member, shape_data, in, out);
      if constexpr (dim == 3)
        apply_3d<n_q_points_1d, Number, direction, dof_to_quad, add, in_place>(team_member, shape_data, in, out);
#else
      constexpr unsigned int n_q_points = Utilities::pow(n_q_points_1d, dim);

      Number t[n_q_points];
      Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, n_q_points), [&](const int &q_point) {
        const unsigned int i = (dim == 1) ? 0 : q_point % n_q_points_1d;
        const unsigned int j = (dim == 3) ? (q_point / n_q_points_1d) % n_q_points_1d : 0;
        const unsigned int q = (dim == 1) ? q_point :
        ...
        for (int k = 0; k < n_q_points_1d; ++k)
          {
            ...
            const unsigned int source_idx = (direction == 0) ? (k + n_q_points_1d * (i + n_q_points_1d * j)) :
                                            (direction == 1) ? (i + n_q_points_1d * (k + n_q_points_1d * j)) :
                                                               (i + n_q_points_1d * (j + n_q_points_1d * k));
            t[q_point] += shape_data[shape_idx] * (in_place ? out(source_idx) : in(source_idx));
          }
      });
    ...
    template <int dim, int fe_degree, int n_q_points_1d, typename Number>
    DEAL_II_HOST_DEVICE
    EvaluatorTensorProduct<evaluate_general, dim, fe_degree, n_q_points_1d, Number>::EvaluatorTensorProduct(
      const TeamHandle                                          &team_member,
      Kokkos::View<Number *, MemorySpace::Default::kokkos_space> shape_values,
      Kokkos::View<Number *, MemorySpace::Default::kokkos_space> shape_gradients,
      Kokkos::View<Number *, MemorySpace::Default::kokkos_space> co_shape_gradients)
    ...
    template <int direction, bool dof_to_quad, bool add, bool in_place, typename ViewTypeIn, typename ViewTypeOut>
    DEAL_II_HOST_DEVICE void
    EvaluatorTensorProduct<evaluate_general, dim, fe_degree, n_q_points_1d, Number>::values(const ViewTypeIn in,
                                                                                            ViewTypeOut      out) const
    {
      apply<dim, n_q_points_1d, Number, direction, dof_to_quad, add, in_place>(team_member, shape_values, in, out);
    }
    ...
    template <int direction, bool dof_to_quad, bool add, bool in_place, typename ViewTypeIn, typename ViewTypeOut>
    DEAL_II_HOST_DEVICE void
    EvaluatorTensorProduct<evaluate_general, dim, fe_degree, n_q_points_1d, Number>::gradients(const ViewTypeIn in,
                                                                                               ViewTypeOut out) const
    {
      apply<dim, n_q_points_1d, Number, direction, dof_to_quad, add, in_place>(team_member, shape_gradients, in, out);
    }
    ...
    template <int direction, bool dof_to_quad, bool add, bool in_place, typename ViewTypeIn, typename ViewTypeOut>
    DEAL_II_HOST_DEVICE void
    EvaluatorTensorProduct<evaluate_general, dim, fe_degree, n_q_points_1d, Number>::co_gradients(const ViewTypeIn in,
                                                                                                  ViewTypeOut out) const
    {
      apply<dim, n_q_points_1d, Number, direction, dof_to_quad, add, in_place>(team_member,
                                                                               co_shape_gradients,
                                                                               in,
                                                                               out);
    }
    ...
    EvaluatorTensorProduct<evaluate_general, dim, fe_degree, n_q_points_1d, Number>::value_at_quad_pts(ViewType u)
    {
      switch (dim)
        {
          case 1:
    ...
    template <typename ViewType>
    DEAL_II_HOST_DEVICE inline void
    EvaluatorTensorProduct<evaluate_general, dim, fe_degree, n_q_points_1d, Number>::integrate_value(ViewType u)
    {
      switch (dim)
        {
          case 1:
    ...
    template <typename ViewTypeIn, typename ViewTypeOut>
    DEAL_II_HOST_DEVICE inline void
    EvaluatorTensorProduct<evaluate_general, dim, fe_degree, n_q_points_1d, Number>::gradient_at_quad_pts(
      const ViewTypeIn u,
      ViewTypeOut      grad_u)
    {
      switch (dim)
        {
          ...
          case 2:
            {
              gradients<0, true, false, false>(u, Kokkos::subview(grad_u, Kokkos::ALL, 0));
              values<0, true, false, false>(u, Kokkos::subview(grad_u, Kokkos::ALL, 1));

              team_member.team_barrier();

              values<1, true, false, true>(Kokkos::subview(grad_u, Kokkos::ALL, 0),
                                           Kokkos::subview(grad_u, Kokkos::ALL, 0));
              gradients<1, true, false, true>(Kokkos::subview(grad_u, Kokkos::ALL, 1),
          ...
          case 3:
            {
              gradients<0, true, false, false>(u, Kokkos::subview(grad_u, Kokkos::ALL, 0));
              values<0, true, false, false>(u, Kokkos::subview(grad_u, Kokkos::ALL, 1));
              values<0, true, false, false>(u, Kokkos::subview(grad_u, Kokkos::ALL, 2));
              ...
              values<2, true, false, true>(Kokkos::subview(grad_u, Kokkos::ALL, 1),
                                           Kokkos::subview(grad_u, Kokkos::ALL, 1));
              gradients<2, true, false, true>(Kokkos::subview(grad_u, Kokkos::ALL, 2),
                                              Kokkos::subview(grad_u, Kokkos::ALL, 2));
    ...
    template <typename ViewType1, typename ViewType2>
    DEAL_II_HOST_DEVICE inline void
    EvaluatorTensorProduct<evaluate_general, dim, fe_degree, n_q_points_1d, Number>::value_and_gradient_at_quad_pts(
      ViewType1 u,
      ViewType2 grad_u)
    {
      switch (dim)
          ...
          case 1:
            {
              ...
              co_gradients<0, true, false, false>(u, Kokkos::subview(grad_u, Kokkos::ALL, 0));
          ...
          case 2:
            {
              ...
              co_gradients<0, true, false, false>(u, Kokkos::subview(grad_u, Kokkos::ALL, 0));
              co_gradients<1, true, false, false>(u, Kokkos::subview(grad_u, Kokkos::ALL, 1));
          ...
          case 3:
            {
              ...
              co_gradients<0, true, false, false>(u, Kokkos::subview(grad_u, Kokkos::ALL, 0));
              co_gradients<1, true, false, false>(u, Kokkos::subview(grad_u, Kokkos::ALL, 1));
              co_gradients<2, true, false, false>(u, Kokkos::subview(grad_u, Kokkos::ALL, 2));
      ...
      switch (dim)
        {
          case 1:
            {
              gradients<0, false, add, false>(Kokkos::subview(grad_u, Kokkos::ALL, dim), u);

              break;
            }
          case 2:
            {
              gradients<0, false, false, true>(Kokkos::subview(grad_u, Kokkos::ALL, 0),
                                               Kokkos::subview(grad_u, Kokkos::ALL, 0));
              ...
              values<1, false, add, false>(Kokkos::subview(grad_u, Kokkos::ALL, 0), u);
              team_member.team_barrier();
          ...
          case 3:
            {
              ...
              values<1, false, false, true>(Kokkos::subview(grad_u, Kokkos::ALL, 0),
                                            Kokkos::subview(grad_u, Kokkos::ALL, 0));
              gradients<1, false, false, true>(Kokkos::subview(grad_u, Kokkos::ALL, 1),
                                               Kokkos::subview(grad_u, Kokkos::ALL, 1));
              ...
              values<2, false, add, false>(Kokkos::subview(grad_u, Kokkos::ALL, 0), u);
              team_member.team_barrier();
              values<2, false, true, false>(Kokkos::subview(grad_u, Kokkos::ALL, 1), u);
              team_member.team_barrier();
    ...
    template <typename ViewType1, typename ViewType2>
    DEAL_II_HOST_DEVICE inline void
    EvaluatorTensorProduct<evaluate_general, dim, fe_degree, n_q_points_1d, Number>::integrate_value_and_gradient(
      ViewType1 u,
      ViewType2 grad_u)
    {
      switch (dim)
        {
          case 1:
            {
              co_gradients<0, false, true, false>(Kokkos::subview(grad_u, Kokkos::ALL, 0), u);
              team_member.team_barrier();
          ...
          case 2:
            {
              co_gradients<1, false, true, false>(Kokkos::subview(grad_u, Kokkos::ALL, 1), u);
              team_member.team_barrier();
              co_gradients<0, false, true, false>(Kokkos::subview(grad_u, Kokkos::ALL, 0), u);
              team_member.team_barrier();
          ...
          case 3:
            {
              co_gradients<2, false, true, false>(Kokkos::subview(grad_u, Kokkos::ALL, 2), u);
              team_member.team_barrier();
              co_gradients<1, false, true, false>(Kokkos::subview(grad_u, Kokkos::ALL, 1), u);
              team_member.team_barrier();
              co_gradients<0, false, true, false>(Kokkos::subview(grad_u, Kokkos::ALL, 0), u);

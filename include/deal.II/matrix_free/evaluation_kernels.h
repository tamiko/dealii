...
namespace internal
{
  ...
  {
    static const EvaluatorVariant variant = EvaluatorSelector<type, (fe_degree + n_q_points_1d > 4)>::variant;
    ...
    static Eval
    create_evaluator_tensor_product(const MatrixFreeFunctions::UnivariateShapeData<Number2> *univariate_shape_data)
    {
   ...
   */
  template <int dim, int fe_degree, int n_q_points_1d, typename Number>
  struct FEEvaluationImpl<MatrixFreeFunctions::tensor_raviart_thomas, dim, fe_degree, n_q_points_1d, Number>
  {
    template <bool integrate>
    static void
    evaluate_or_integrate(const EvaluationFlags::EvaluationFlags evaluation_flag,
                          Number                                *values_dofs_actual,
                          FEEvaluationData<dim, Number, false>  &fe_eval,
  ...
  private:
    ...
    static EvalType
    create_evaluator_tensor_product(const MatrixFreeFunctions::UnivariateShapeData<Number2> &shape_data)
    {
      return EvalType(shape_data.shape_values, shape_data.shape_gradients, shape_data.shape_hessians);
    }

    template <int normal_dir>
    static void
    evaluate_tensor_product_per_component(const EvaluationFlags::EvaluationFlags evaluation_flag,
                                          Number                                *values_dofs_actual,
                                          FEEvaluationData<dim, Number, false>  &fe_eval,
                                          const bool                             add_into_values_array,
    ...
    FEEvaluationData<dim, Number, false>  &fe_eval)
  {
    ...
    std::array<const MatrixFreeFunctions::UnivariateShapeData<Number2> *, 3> univariate_shape_data;
    ...
    const unsigned int temp_size =
      Eval::n_rows_of_product == numbers::invalid_unsigned_int ?
        0 :
        (Eval::n_rows_of_product > Eval::n_columns_of_product ? Eval::n_rows_of_product : Eval::n_columns_of_product);
    Number *temp1 = fe_eval.get_scratch_data().begin();
    Number *temp2;
    if (temp_size == 0)
      {
        temp2 = temp1 + std::max(Utilities::fixed_power<dim>(shape_data.front().fe_degree + 1),
    ...
    const std::size_t n_q_points    = temp_size == 0 ? fe_eval.get_shape_info().n_q_points : Eval::n_columns_of_product;
    const std::size_t dofs_per_comp = (type == MatrixFreeFunctions::truncated_tensor) ?
    ...
    if (type == MatrixFreeFunctions::truncated_tensor)
      {
        const std::size_t n_dofs_per_comp = fe_eval.get_shape_info().dofs_per_component_on_cell;
        Number           *values_dofs_tmp = temp1 + 2 * (std::max(n_dofs_per_comp, n_q_points));
        const int         degree          = fe_degree != -1 ? fe_degree : shape_data.front().fe_degree;
        for (unsigned int c = 0; c < n_components; ++c)
          for (int i = 0, count_p = 0, count_q = 0; i < (dim > 2 ? degree + 1 : 1); ++i)
            {
              for (int j = 0; j < (dim > 1 ? degree + 1 - i : 1); ++j)
                {
                  for (int k = 0; k < degree + 1 - j - i; ++k, ++count_p, ++count_q)
                    values_dofs_tmp[c * dofs_per_comp + count_q] = values_dofs_actual[c * n_dofs_per_comp + count_p];
          ...
          for (unsigned int c = 0; c < n_components; ++c)
            {
              ...
              if (evaluation_flag & EvaluationFlags::gradients)
                eval0.template gradients<0, true, false>(values_dofs, gradients_quad);
              if (evaluation_flag & EvaluationFlags::hessians)
                eval0.template hessians<0, true, false>(values_dofs, hessians_quad);
          ...
          for (unsigned int c = 0; c < n_components; ++c)
            {
              ...
              if (evaluation_flag & EvaluationFlags::hessians)
                {
                  ...
                  if (!(evaluation_flag & EvaluationFlags::gradients))
                    eval0.template gradients<0, true, false>(values_dofs, temp1);
                  eval1.template gradients<1, true, false>(temp1, hessians_quad + 2 * n_q_points);
              ...
              if (evaluation_flag & EvaluationFlags::gradients)
                eval1.template gradients<1, true, false>(temp1, gradients_quad + n_q_points);

              // grad yy
              if (evaluation_flag & EvaluationFlags::hessians)
                eval1.template hessians<1, true, false>(temp1, hessians_quad + n_q_points);
          ...
          for (unsigned int c = 0; c < n_components; ++c)
            {
              ...
              if (evaluation_flag & EvaluationFlags::hessians)
                {
                  ...
                  eval2.template gradients<2, true, false>(temp2, hessians_quad + 4 * n_q_points);

                  // grad xy
                  eval1.template gradients<1, true, false>(temp1, temp2);
                  eval2.template values<2, true, false>(temp2, hessians_quad + 3 * n_q_points);
              ...
              if (evaluation_flag & EvaluationFlags::gradients)
                {
                  eval1.template gradients<1, true, false>(temp1, temp2);
                  eval2.template values<2, true, false>(temp2, gradients_quad + n_q_points);
                }
              ...
              if (evaluation_flag & EvaluationFlags::hessians)
                {
                  ...
                  if (!(evaluation_flag & EvaluationFlags::gradients))
                    eval1.template gradients<1, true, false>(temp1, temp2);
                  eval2.template gradients<2, true, false>(temp2, hessians_quad + 5 * n_q_points);

                  // grad yy
                  eval1.template hessians<1, true, false>(temp1, temp2);
                  eval2.template values<2, true, false>(temp2, hessians_quad + n_q_points);
                }
              ...
              eval1.template values<1, true, false>(temp1, temp2);
              if (evaluation_flag & EvaluationFlags::gradients)
                eval2.template gradients<2, true, false>(temp2, gradients_quad + 2 * n_q_points);

              // grad zz: can use the values applied in x and y direction stored
              // in temp2
              if (evaluation_flag & EvaluationFlags::hessians)
                eval2.template hessians<2, true, false>(temp2, hessians_quad + 2 * n_q_points);
    ...
    if (type == MatrixFreeFunctions::tensor_symmetric_plus_dg0 && (evaluation_flag & EvaluationFlags::values))
      {
        values_quad -= n_components * n_q_points;
        values_dofs -= n_components * dofs_per_comp;
        for (std::size_t c = 0; c < n_components; ++c)
          for (std::size_t q = 0; q < n_q_points; ++q)
            values_quad[c * n_q_points + q] += values_dofs[(c + 1) * dofs_per_comp - 1];
      }
  }
    ...
    const bool                             add_into_values_array)
  {
    ...
    Number *values_dofs =
      (type == MatrixFreeFunctions::truncated_tensor) ?
        temp1 + 2 * (std::max<std::size_t>(fe_eval.get_shape_info().dofs_per_component_on_cell, n_q_points)) :
        values_dofs_actual;
    ...
    switch (dim)
      {
              ...
              if (integration_flag & EvaluationFlags::values)
                {
                  if (add_into_values_array == false)
                    eval0.template values<0, false, false>(values_quad, values_dofs);
                  else
                    eval0.template values<0, false, true>(values_quad, values_dofs);
                }
              if (integration_flag & EvaluationFlags::gradients)
                {
                  if (integration_flag & EvaluationFlags::values || add_into_values_array == true)
                    eval0.template gradients<0, false, true>(gradients_quad, values_dofs);
                  else
                    eval0.template gradients<0, false, false>(gradients_quad, values_dofs);
                }
              if ((integration_flag & EvaluationFlags::hessians) != 0u)
                {
                  if ((integration_flag & EvaluationFlags::values) != 0u ||
                      (integration_flag & EvaluationFlags::gradients) != 0u || add_into_values_array == true)
                    eval0.template hessians<0, false, true>(hessians_quad, values_dofs);
                  else
                    eval0.template hessians<0, false, false>(hessians_quad, values_dofs);
                }
        ...
        case 2:
          for (unsigned int c = 0; c < n_components; ++c)
            {
              if ((integration_flag & EvaluationFlags::values) && !(integration_flag & EvaluationFlags::gradients))
                {
              ...
              if (integration_flag & EvaluationFlags::gradients)
                {
                  eval1.template gradients<1, false, false>(gradients_quad + n_q_points, temp1);
                  if (integration_flag & EvaluationFlags::values)
              ...
              if ((integration_flag & EvaluationFlags::hessians) != 0u)
                {
                  ...
                  // grad yy
                  eval1.template hessians<1, false, false>(hessians_quad + n_q_points, temp1);
                  eval0.template values<0, false, true>(temp1, values_dofs);

                  // grad xy
                  eval1.template gradients<1, false, false>(hessians_quad + 2 * n_q_points, temp1);
                  eval0.template gradients<0, false, true>(temp1, values_dofs);
                }
          ...
          for (unsigned int c = 0; c < n_components; ++c)
            {
              ...
              if (integration_flag & EvaluationFlags::gradients)
                {
                  eval2.template gradients<2, false, false>(gradients_quad + 2 * n_q_points, temp1);
                  if (integration_flag & EvaluationFlags::values)
                    eval2.template values<2, false, true>(values_quad, temp1);
                  eval1.template values<1, false, false>(temp1, temp2);
                  eval2.template values<2, false, false>(gradients_quad + n_q_points, temp1);
              ...
              if ((integration_flag & EvaluationFlags::hessians) != 0u)
                {
                  ...
                  // grad yy
                  eval2.template values<2, false, false>(hessians_quad + n_q_points, temp1);
                  eval1.template hessians<1, false, false>(temp1, temp2);
                  eval0.template values<0, false, true>(temp2, values_dofs);

                  // grad zz
                  eval2.template hessians<2, false, false>(hessians_quad + 2 * n_q_points, temp1);
                  eval1.template values<1, false, false>(temp1, temp2);
                  eval0.template values<0, false, true>(temp2, values_dofs);

                  // grad xy
                  eval2.template values<2, false, false>(hessians_quad + 3 * n_q_points, temp1);
                  eval1.template gradients<1, false, false>(temp1, temp2);
                  eval0.template gradients<0, false, true>(temp2, values_dofs);

                  // grad xz
                  eval2.template gradients<2, false, false>(hessians_quad + 4 * n_q_points, temp1);
                  eval1.template values<1, false, false>(temp1, temp2);
                  eval0.template gradients<0, false, true>(temp2, values_dofs);

                  // grad yz
                  eval2.template gradients<2, false, false>(hessians_quad + 5 * n_q_points, temp1);
                  eval1.template gradients<1, false, false>(temp1, temp2);
                  eval0.template values<0, false, true>(temp2, values_dofs);
                }
          ...
          for (int i = 0, count_p = 0, count_q = 0; i < (dim > 2 ? degree + 1 : 1); ++i)
            {
              for (int j = 0; j < (dim > 1 ? degree + 1 - i : 1); ++j)
                {
                  for (int k = 0; k < degree + 1 - j - i; ++k, ++count_p, ++count_q)
                    values_dofs_actual[c * n_dofs_per_comp + count_p] = values_dofs[c * dofs_per_comp + count_q];
                  count_q += j + i;
                }
              count_q += i * (degree + 1);
            }
  ...
  template <int dim, int fe_degree, int n_q_points_1d, typename Number>
  inline void
  FEEvaluationImpl<MatrixFreeFunctions::tensor_none, dim, fe_degree, n_q_points_1d, Number>::evaluate(
    const unsigned int                     n_components,
    const EvaluationFlags::EvaluationFlags evaluation_flag,
    const Number                          *values_dofs_actual,
    FEEvaluationData<dim, Number, false>  &fe_eval)
  {
    ...
    const std::size_t n_dofs     = fe_eval.get_shape_info().dofs_per_component_on_cell;
    const std::size_t n_q_points = fe_eval.get_shape_info().n_q_points;

    const auto &shape_data = fe_eval.get_shape_info().data;

    using Number2 = typename FEEvaluationData<dim, Number, false>::shape_info_number_type;
    using Eval    = EvaluatorTensorProduct<evaluate_general, 1, 0, 0, Number, Number2>;
    ...
    if (evaluation_flag & EvaluationFlags::values)
      {
        ...
        for (unsigned int c = 0; c < n_components; ++c)
          {
            eval.template values<0, true, false>(values_dofs_actual_ptr, values_quad_ptr);
    ...
    if (evaluation_flag & EvaluationFlags::gradients)
      {
        const auto *const shape_gradients        = shape_data.front().shape_gradients.data();
        ...
        for (unsigned int c = 0; c < n_components; ++c)
          {
            for (unsigned int d = 0; d < dim; ++d)
              {
                Eval eval(nullptr, shape_gradients + n_q_points * n_dofs * d, nullptr, n_dofs, n_q_points);

                eval.template gradients<0, true, false>(values_dofs_actual_ptr, gradients_quad_ptr);
  ...
  template <int dim, int fe_degree, int n_q_points_1d, typename Number>
  inline void
  FEEvaluationImpl<MatrixFreeFunctions::tensor_none, dim, fe_degree, n_q_points_1d, Number>::integrate(
    const unsigned int                     n_components,
    const EvaluationFlags::EvaluationFlags integration_flag,
    Number                                *values_dofs_actual,
    FEEvaluationData<dim, Number, false>  &fe_eval,
    ...
    if (integration_flag & EvaluationFlags::values)
      {
        ...
        for (unsigned int c = 0; c < n_components; ++c)
          {
            if (add_into_values_array == false)
              eval.template values<0, false, false>(values_quad_ptr, values_dofs_actual_ptr);
            else
              eval.template values<0, false, true>(values_quad_ptr, values_dofs_actual_ptr);
            ...
            for (unsigned int d = 0; d < dim; ++d)
              {
                ...
                if ((add_into_values_array == false && !(integration_flag & EvaluationFlags::values)) && d == 0)
                  eval.template gradients<0, false, false>(gradients_quad_ptr, values_dofs_actual_ptr);
                else
                  eval.template gradients<0, false, true>(gradients_quad_ptr, values_dofs_actual_ptr);
  ...
  template <bool integrate>
  inline void
  FEEvaluationImpl<MatrixFreeFunctions::tensor_raviart_thomas, dim, fe_degree, n_q_points_1d, Number>::
    evaluate_or_integrate(const EvaluationFlags::EvaluationFlags evaluation_flag,
                          Number                                *values_dofs_actual,
                          FEEvaluationData<dim, Number, false>  &fe_eval,
                          const bool                             add_into_values_array)
  {
    ...
    evaluate_tensor_product_per_component<0>(
      evaluation_flag, values_dofs_actual, fe_eval, add_into_values_array, std::integral_constant<bool, integrate>());
    // Second component :
    ...
    if (dim == 3)
      {
        // Third component
        evaluate_tensor_product_per_component<2>(evaluation_flag,
                                                 values_dofs_actual,
                                                 fe_eval,
                                          ...
                                          std::integral_constant<bool, false>)
  {
    ...
    using Number2    = typename FEEvaluationData<dim, Number, false>::shape_info_number_type;
    using EvalNormal = EvaluatorTensorProductAnisotropic<evaluate_raviart_thomas,
                                                         dim,
    ...
    using EvalTangent = EvaluatorTensorProductAnisotropic<evaluate_raviart_thomas,
                                                          dim,
    ...
    Eval0 eval0 = create_evaluator_tensor_product<Eval0>(((normal_dir == 0) ? shape_info.data[0] : shape_info.data[1]));
    Eval1 eval1 = create_evaluator_tensor_product<Eval1>(((normal_dir == 1) ? shape_info.data[0] : shape_info.data[1]));
    Eval2 eval2 = create_evaluator_tensor_product<Eval2>(((normal_dir == 2) ? shape_info.data[0] : shape_info.data[1]));

    Number *temp1 = fe_eval.get_scratch_data().begin();
    Number *temp2;

    temp2 = temp1 + std::max(Utilities::fixed_power<dim>(shape_info.data[0].fe_degree + 1),
                                          ...
                                          std::integral_constant<bool, true>)
  {
    ...
    switch (dim)
      {
        case 2:
          if ((evaluation_flag & EvaluationFlags::values) && !(evaluation_flag & EvaluationFlags::gradients))
            {
              eval1.template values<1, false, false>(values_quad, temp1);
              if (add_into_values_array == false)
                eval0.template values<0, false, false>(temp1, values_dofs);
  ...
  struct FEEvaluationImplBasisChange
  {
                 ...
                 const unsigned int            basis_size_2_variable = numbers::invalid_unsigned_int)
    {
      ...
      constexpr int next_dim =
        (dim > 2 || ((basis_size_1 == 0 || basis_size_2 > basis_size_1) && dim > 1)) ? dim - 1 : dim;

      EvaluatorTensorProduct<variant, dim, basis_size_1, (basis_size_1 == 0 ? 0 : basis_size_2), Number, Number2>
                         eval_val(transformation_matrix, {}, {}, basis_size_1_variable, basis_size_2_variable);
      ...
      Assert(np_1 > 0 && np_1 != numbers::invalid_unsigned_int, ExcMessage("Cannot transform with 0-point basis"));
      Assert(np_2 > 0 && np_2 != numbers::invalid_unsigned_int, ExcMessage("Cannot transform with 0-point basis"));
      ...
      for (unsigned int c = n_components; c != 0; --c)
        {
          ...
          if (next_dim < dim)
            for (unsigned int q = np_1; q != 0; --q)
              FEEvaluationImplBasisChange<variant, quantity, next_dim, basis_size_1, basis_size_2>::do_forward(
                1,
                  ...
                  const unsigned int            basis_size_2_variable = numbers::invalid_unsigned_int)
    {
      ...
      Assert(quantity == EvaluatorQuantity::value || quantity == EvaluatorQuantity::hessian, ExcInternalError());
              ...
              if (add_into_result)
                {
                  if (quantity == EvaluatorQuantity::value)
                    ...
                    eval_val.template hessians<0, false, true>(values_in, values_out);
                }
              else
                {
                  if (quantity == EvaluatorQuantity::value)
                    ...
                    eval_val.template hessians<0, false, false>(values_in, values_out);
                }
            }
          else
            {
              if (dim == 1 && add_into_result)
              ...
              else
                {
                  if (quantity == EvaluatorQuantity::value)
                    ...
                    eval_val.template hessians<dim - 1, false, false>(values_in, values_in);
                }
            }
          if (next_dim < dim)
            for (unsigned int q = 0; q < np_1; ++q)
              FEEvaluationImplBasisChange<variant, quantity, next_dim, basis_size_1, basis_size_2>::do_backward(
                1,
                transformation_matrix,
                add_into_result,
            ...
            Number                       *values_out)
    {
      ...
      const unsigned int stride = coefficients.size() == size_per_component ? 0 : 1;

      for (unsigned int q = basis_size_1; q != 0; --q)
        FEEvaluationImplBasisChange<variant, EvaluatorQuantity::value, next_dim, basis_size_1, basis_size_2>::
          do_forward(n_components,
                     transformation_matrix,
                     values_in + (q - 1) * Utilities::pow(basis_size_1, dim - 1),
                     my_scratch + (q - 1) * Utilities::pow(basis_size_2, dim - 1));
      EvaluatorTensorProduct<variant, dim, basis_size_1, basis_size_2, Number, Number2> eval_val(transformation_matrix);
            ...
            for (unsigned int q = 0; q < basis_size_2; ++q)
              for (unsigned int i = ii; i < ii + n_inner_blocks; ++i)
                my_scratch[i + q * n_blocks + c * size_per_component] *=
                  coefficients[i + q * n_blocks + c * stride * size_per_component];
            for (unsigned int i = ii; i < ii + n_inner_blocks; ++i)
  ...
  template <int dim, int fe_degree, typename Number>
  inline void
  FEEvaluationImplCollocation<dim, fe_degree, Number>::evaluate(const unsigned int                     n_components,
                                                                const EvaluationFlags::EvaluationFlags evaluation_flag,
                                                                const Number                          *values_dofs,
                                                                FEEvaluationData<dim, Number, false>  &fe_eval)
  {
    ...
    for (unsigned int c = 0; c < n_components; ++c)
      {
        if ((evaluation_flag & EvaluationFlags::values) != 0u)
          for (unsigned int i = 0; i < n_points; ++i)
            fe_eval.begin_values()[n_points * c + i] = values_dofs[n_points * c + i];
  ...
  FEEvaluationImplCollocation<dim, fe_degree, Number>::do_evaluate(
    ...
    Number                                                  *hessians_quad)
  {
    AssertDimension(shape.shape_gradients_collocation_eo.size(), (fe_degree + 2) / 2 * (fe_degree + 1));
    constexpr std::size_t n_points = Utilities::pow(fe_degree + 1, dim);

    Eval eval({}, shape.shape_gradients_collocation_eo, shape.shape_hessians_collocation_eo);
    if ((evaluation_flag & (EvaluationFlags::gradients | EvaluationFlags::hessians)) != 0u)
      {
        eval.template gradients<0, true, false>(values_dofs, gradients_quad);
        if (dim > 1)
          eval.template gradients<1, true, false>(values_dofs, gradients_quad + n_points);
        if (dim > 2)
          eval.template gradients<2, true, false>(values_dofs, gradients_quad + 2 * n_points);
      }
    if (evaluation_flag & EvaluationFlags::hessians)
      {
        ...
        if (dim > 1)
          {
            eval.template gradients<1, true, false>(gradients_quad, hessians_quad + dim * n_points);
            eval.template hessians<1, true, false>(values_dofs, hessians_quad + n_points);
          }
        if (dim > 2)
          {
            eval.template gradients<2, true, false>(gradients_quad, hessians_quad + 4 * n_points);
            eval.template gradients<2, true, false>(gradients_quad + n_points, hessians_quad + 5 * n_points);
            eval.template hessians<2, true, false>(values_dofs, hessians_quad + 2 * n_points);
          }
      }
  }
    ...
    for (unsigned int c = 0; c < n_components; ++c)
      {
        if ((integration_flag & EvaluationFlags::values) != 0u)
          {
            if (add_into_values_array)
              for (unsigned int i = 0; i < n_points; ++i)
                values_dofs[n_points * c + i] += fe_eval.begin_values()[n_points * c + i];
            else
              for (unsigned int i = 0; i < n_points; ++i)
                values_dofs[n_points * c + i] = fe_eval.begin_values()[n_points * c + i];
          }
        ...
        do_integrate(fe_eval.get_shape_info().data.front(),
                     ...
                     fe_eval.begin_gradients() + c * dim * n_points,
                     fe_eval.begin_hessians() + c * dim * (dim + 1) / 2 * n_points,
                     add_into_values_array || ((integration_flag & EvaluationFlags::values) != 0u));
      }
  }
    ...
    const bool                                               add_into_values_array)
  {
    ...
    if ((integration_flag & EvaluationFlags::hessians) != 0u)
      {
        ...
        // grad yy
        if (dim > 1)
          eval.template hessians<1, false, true>(hessians_quad + n_points, values_dofs);
        // grad zz
        if (dim > 2)
          eval.template hessians<2, false, true>(hessians_quad + 2 * n_points, values_dofs);
        // off-diagonal
        if (dim == 2)
          {
            // grad xy, queue into gradient
            if (integration_flag & EvaluationFlags::gradients)
              eval.template gradients<1, false, true>(hessians_quad + 2 * n_points, gradients_quad);
            else
              eval.template gradients<1, false, false>(hessians_quad + 2 * n_points, gradients_quad);
          }
        if (dim == 3)
          {
            // grad xy, queue into gradient
            if (integration_flag & EvaluationFlags::gradients)
              eval.template gradients<1, false, true>(hessians_quad + 3 * n_points, gradients_quad);
            else
              eval.template gradients<1, false, false>(hessians_quad + 3 * n_points, gradients_quad);

            // grad xz
            eval.template gradients<2, false, true>(hessians_quad + 4 * n_points, gradients_quad);

            // grad yz
            if (integration_flag & EvaluationFlags::gradients)
              eval.template gradients<2, false, true>(hessians_quad + 5 * n_points, gradients_quad + n_points);
            else
              eval.template gradients<2, false, false>(hessians_quad + 5 * n_points, gradients_quad + n_points);
          }
    ...
    if ((integration_flag & (EvaluationFlags::gradients | EvaluationFlags::hessians)) != 0u)
      {
        if (add_into_values_array || (integration_flag & EvaluationFlags::hessians) != 0u)
        ...
        if (dim > 1)
          eval.template gradients<1, false, true>(gradients_quad + n_points, values_dofs);
        if (dim > 2)
          eval.template gradients<2, false, true>(gradients_quad + 2 * n_points, values_dofs);
      }
  }
    ...
    for (unsigned int c = 0; c < n_components; ++c)
      {
        ...
        if (evaluation_flag & (EvaluationFlags::gradients | EvaluationFlags::hessians))
          FEEvaluationImplCollocation<dim, n_q_points_1d - 1, Number>::do_evaluate(
            shape_data,
            evaluation_flag & (EvaluationFlags::gradients | EvaluationFlags::hessians),
            fe_eval.begin_values() + c * n_q_points,
  ...
  template <int dim, int fe_degree, int n_q_points_1d, typename Number>
  inline void
  FEEvaluationImplTransformToCollocation<dim, fe_degree, n_q_points_1d, Number>::integrate(
    const unsigned int                     n_components,
    const EvaluationFlags::EvaluationFlags integration_flag,
    Number                                *values_dofs,
    FEEvaluationData<dim, Number, false>  &fe_eval,
    ...
    for (unsigned int c = 0; c < n_components; ++c)
      {
        ...
        if (integration_flag & (EvaluationFlags::gradients | EvaluationFlags::hessians))
          FEEvaluationImplCollocation<dim, n_q_points_1d - 1, Number>::do_integrate(
            shape_data,
            integration_flag & (EvaluationFlags::gradients | EvaluationFlags::hessians),
            fe_eval.begin_values() + c * n_q_points,
            fe_eval.begin_gradients() + c * dim * n_q_points,
            fe_eval.begin_hessians() + c * dim * (dim + 1) / 2 * n_q_points,
            /*add_into_values_array=*/
        ...
        FEEvaluationImplBasisChange<evaluate_evenodd,
                                    ...
                                    n_q_points_1d>::do_backward(1,
                                                                ...
                                                                add_into_values_array,
                                                                fe_eval.begin_values() + c * n_q_points,
                                                                values_dofs + c * Utilities::pow(fe_degree + 1, dim));
      }
  }
  ...
  {
    return (n_q_points_1d > fe_degree) && (n_q_points_1d < 200) && (n_q_points_1d <= 3 * fe_degree / 2 + 1);
  }
  ...
  struct FEEvaluationImplSelector
  {
        ...
        const bool                             sum_into_values_array = false)
    {
      ...
      Assert(fe_eval.get_shape_info().data.size() == 1 ||
               (fe_eval.get_shape_info().data.size() == dim && element_type == ElementType::tensor_general) ||
               element_type == ElementType::tensor_raviart_thomas,
             ExcNotImplemented());

      if (fe_degree >= 0 && fe_degree + 1 == n_q_points_1d && element_type == ElementType::tensor_symmetric_collocation)
        {
          evaluate_or_integrate<FEEvaluationImplCollocation<dim, fe_degree, Number>>(
            n_components, evaluation_flag, values_dofs, fe_eval, sum_into_values_array);
        }
      // '<=' on type means tensor_symmetric or tensor_symmetric_hermite, see
      // shape_info.h for more details
      else if (fe_degree >= 0 && use_collocation_evaluation(fe_degree, n_q_points_1d) &&
               element_type <= ElementType::tensor_symmetric)
        {
          evaluate_or_integrate<FEEvaluationImplTransformToCollocation<dim, fe_degree, n_q_points_1d, Number>>(
            n_components, evaluation_flag, values_dofs, fe_eval, sum_into_values_array);
        }
      else if (fe_degree >= 0 && element_type <= ElementType::tensor_symmetric_no_collocation)
        {
          evaluate_or_integrate<FEEvaluationImpl<ElementType::tensor_symmetric, dim, fe_degree, n_q_points_1d, Number>>(
            n_components, evaluation_flag, values_dofs, fe_eval, sum_into_values_array);
        }
      else if (element_type == ElementType::tensor_symmetric_plus_dg0)
        {
          evaluate_or_integrate<
            FEEvaluationImpl<ElementType::tensor_symmetric_plus_dg0, dim, fe_degree, n_q_points_1d, Number>>(
            n_components, evaluation_flag, values_dofs, fe_eval, sum_into_values_array);
        }
      else if (element_type == ElementType::truncated_tensor)
        {
          evaluate_or_integrate<FEEvaluationImpl<ElementType::truncated_tensor, dim, fe_degree, n_q_points_1d, Number>>(
            n_components, evaluation_flag, values_dofs, fe_eval, sum_into_values_array);
        }
      else if (element_type == ElementType::tensor_none)
        {
          evaluate_or_integrate<FEEvaluationImpl<ElementType::tensor_none, dim, fe_degree, n_q_points_1d, Number>>(
            n_components, evaluation_flag, values_dofs, fe_eval, sum_into_values_array);
        }
      else if (element_type == ElementType::tensor_raviart_thomas)
        {
          FEEvaluationImpl<ElementType::tensor_raviart_thomas,
                           ...
                           Number>::template evaluate_or_integrate<do_integrate>(evaluation_flag,
        ...
        {
          evaluate_or_integrate<FEEvaluationImpl<ElementType::tensor_general, dim, fe_degree, n_q_points_1d, Number>>(
            n_components, evaluation_flag, values_dofs, fe_eval, sum_into_values_array);
        }
  ...
  private:
    template <typename T>
    static void
    evaluate_or_integrate(const unsigned int                     n_components,
                          const EvaluationFlags::EvaluationFlags evaluation_flag,
                          const Number                          *values_dofs,
                          FEEvaluationData<dim, Number, false>  &fe_eval,
    ...
    evaluate_or_integrate(const unsigned int                     n_components,
                          ...
                          std::integral_constant<bool, true>)
    {
      T::integrate(n_components, evaluation_flag, values_dofs, fe_eval, sum_into_values_array);
    }
  ...
  template <bool symmetric_evaluate, int dim, int fe_degree, int n_q_points_1d, typename Number>
  struct FEFaceEvaluationImpl
  {
    ...
    using Eval = EvaluatorTensorProduct<symmetric_evaluate ? evaluate_evenodd : evaluate_general,
                                        dim - 1,
                                        fe_degree + 1,
                                        n_q_points_1d,
                                        Number,
    ...
    static Eval
    create_evaluator_tensor_product(const MatrixFreeFunctions::UnivariateShapeData<Number2> &data,
                                    ...
                                    const unsigned int                                       direction)
    {
      if (symmetric_evaluate)
        ...
        return Eval(
          data.shape_values, data.shape_gradients, data.shape_hessians, data.fe_degree + 1, data.n_q_points_1d);
      else
        {
          const unsigned int index = direction == 0 ? subface_index % 2 : subface_index / 2;
                     ...
                     const unsigned int                                       subface_index)
    {
      ...
      const std::size_t n_dofs =
        fe_degree > -1 ? Utilities::pow(fe_degree + 1, dim - 1) : Utilities::pow(data.fe_degree + 1, dim - 1);
      const std::size_t n_q_points =
        fe_degree > -1 ? Utilities::pow(n_q_points_1d, dim - 1) : Utilities::pow(data.n_q_points_1d, dim - 1);

      // keep a copy of the original pointer for the case of the Hessians
      Number *values_dofs_ptr = values_dofs;

      if ((evaluation_flag & EvaluationFlags::values) != 0u && ((evaluation_flag & EvaluationFlags::gradients) == 0u))
        for (unsigned int c = 0; c < n_components; ++c)
          {
            switch (dim)
              {
        ...
        for (unsigned int c = 0; c < n_components; ++c)
          {
            switch (dim)
              {
                case 3:
                  if (symmetric_evaluate && use_collocation_evaluation(fe_degree, n_q_points_1d))
                    {
                      eval0.template values<0, true, false>(values_dofs, values_quad);
                      eval0.template values<1, true, false>(values_quad, values_quad);
                      EvaluatorTensorProduct<evaluate_evenodd, dim - 1, n_q_points_1d, n_q_points_1d, Number, Number2>
                        eval_grad({}, data.shape_gradients_collocation_eo, {});
                      eval_grad.template gradients<0, true, false>(values_quad, gradients_quad);
                      eval_grad.template gradients<1, true, false>(values_quad, gradients_quad + n_q_points);
                    }
                  else
                    {
                      // grad x
                      eval0.template gradients<0, true, false>(values_dofs, scratch_data);
                      eval1.template values<1, true, false>(scratch_data, gradients_quad);

                      // grad y
                      eval0.template values<0, true, false>(values_dofs, scratch_data);
                      eval1.template gradients<1, true, false>(scratch_data, gradients_quad + n_q_points);

                      if ((evaluation_flag & EvaluationFlags::values) != 0u)
                        eval1.template values<1, true, false>(scratch_data, values_quad);
                    }
                  // grad z
                  eval0.template values<0, true, false>(values_dofs + n_dofs, scratch_data);
                  eval1.template values<1, true, false>(scratch_data, gradients_quad + (dim - 1) * n_q_points);

                  break;
                case 2:
                  eval0.template values<0, true, false>(values_dofs + n_dofs, gradients_quad + n_q_points);
                  eval0.template gradients<0, true, false>(values_dofs, gradients_quad);
                  if ((evaluation_flag & EvaluationFlags::values) != 0u)
                    eval0.template values<0, true, false>(values_dofs, values_quad);
                  break;
              ...
              switch (dim)
                {
                  case 3:
                    // grad xx
                    eval0.template hessians<0, true, false>(values_dofs, scratch_data);
                    eval1.template values<1, true, false>(scratch_data, hessians_quad);

                    // grad yy
                    eval0.template values<0, true, false>(values_dofs, scratch_data);
                    eval1.template hessians<1, true, false>(scratch_data, hessians_quad + n_q_points);

                    // grad zz
                    eval0.template values<0, true, false>(values_dofs + 2 * n_dofs, scratch_data);
                    eval1.template values<1, true, false>(scratch_data, hessians_quad + 2 * n_q_points);

                    // grad xy
                    eval0.template gradients<0, true, false>(values_dofs, scratch_data);
                    eval1.template gradients<1, true, false>(scratch_data, hessians_quad + 3 * n_q_points);

                    // grad xz
                    eval0.template gradients<0, true, false>(values_dofs + n_dofs, scratch_data);
                    eval1.template values<1, true, false>(scratch_data, hessians_quad + 4 * n_q_points);

                    // grad yz
                    eval0.template values<0, true, false>(values_dofs + n_dofs, scratch_data);
                    eval1.template gradients<1, true, false>(scratch_data, hessians_quad + 5 * n_q_points);
                  ...
                  case 2:
                    ...
                    eval0.template hessians<0, true, false>(values_dofs, hessians_quad);
                    // grad yy
                    eval0.template values<0, true, false>(values_dofs + 2 * n_dofs, hessians_quad + n_q_points);
                    // grad xy
                    eval0.template gradients<0, true, false>(values_dofs + n_dofs, hessians_quad + 2 * n_q_points);
                    break;
                  case 1:
                      ...
                      const unsigned int                                       subface_index)
    {
      ...
      const std::size_t n_dofs = fe_degree > -1 ? Utilities::pow(fe_degree + 1, dim - 1) :
      ...
      if ((integration_flag & EvaluationFlags::values) != 0u && (integration_flag & EvaluationFlags::gradients) == 0u)
        for (unsigned int c = 0; c < n_components; ++c)
          {
            switch (dim)
              {
            ...
            switch (dim)
              {
                case 3:
                  // grad z
                  eval1.template values<1, false, false>(gradients_quad + 2 * n_q_points,
                                                         gradients_quad + 2 * n_q_points);
                  eval0.template values<0, false, false>(gradients_quad + 2 * n_q_points, values_dofs + n_dofs);
                  if (symmetric_evaluate && use_collocation_evaluation(fe_degree, n_q_points_1d))
                    {
                      EvaluatorTensorProduct<evaluate_evenodd, dim - 1, n_q_points_1d, n_q_points_1d, Number, Number2>
                        eval_grad({}, data.shape_gradients_collocation_eo, {});
                      if ((integration_flag & EvaluationFlags::values) != 0u)
                        eval_grad.template gradients<1, false, true>(gradients_quad + n_q_points, values_quad);
                      else
                        eval_grad.template gradients<1, false, false>(gradients_quad + n_q_points, values_quad);
                      eval_grad.template gradients<0, false, true>(gradients_quad, values_quad);
                      eval0.template values<1, false, false>(values_quad, values_quad);
                      eval0.template values<0, false, false>(values_quad, values_dofs);
                    }
                  else
                    {
                      if ((integration_flag & EvaluationFlags::values) != 0u)
                        {
                          eval1.template values<1, false, false>(values_quad, scratch_data);
                          eval1.template gradients<1, false, true>(gradients_quad + n_q_points, scratch_data);
                        }
                      else
                        eval1.template gradients<1, false, false>(gradients_quad + n_q_points, scratch_data);

                      // grad y
                      eval0.template values<0, false, false>(scratch_data, values_dofs);

                      // grad x
                      eval1.template values<1, false, false>(gradients_quad, scratch_data);
                      eval0.template gradients<0, false, true>(scratch_data, values_dofs);
                    }
                  break;
                case 2:
                  eval0.template values<0, false, false>(gradients_quad + n_q_points, values_dofs + n_dofs);
                  eval0.template gradients<0, false, false>(gradients_quad, values_dofs);
                  if ((integration_flag & EvaluationFlags::values) != 0u)
                    eval0.template values<0, false, true>(values_quad, values_dofs);
                  break;
              ...
              switch (dim)
                {
                  case 3:
                    // grad xx
                    eval1.template values<1, false, false>(hessians_quad, scratch_data);
                    if ((integration_flag & (EvaluationFlags::values | EvaluationFlags::gradients)) != 0u)
                      eval0.template hessians<0, false, true>(scratch_data, values_dofs);
                    else
                      eval0.template hessians<0, false, false>(scratch_data, values_dofs);

                    // grad yy
                    eval1.template hessians<1, false, false>(hessians_quad + n_q_points, scratch_data);
                    eval0.template values<0, false, true>(scratch_data, values_dofs);

                    // grad zz
                    eval1.template values<1, false, false>(hessians_quad + 2 * n_q_points, scratch_data);
                    eval0.template values<0, false, false>(scratch_data, values_dofs + 2 * n_dofs);

                    // grad xy
                    eval1.template gradients<1, false, false>(hessians_quad + 3 * n_q_points, scratch_data);
                    eval0.template gradients<0, false, true>(scratch_data, values_dofs);

                    // grad xz
                    eval1.template values<1, false, false>(hessians_quad + 4 * n_q_points, scratch_data);
                    if ((integration_flag & EvaluationFlags::gradients) != 0u)
                      eval0.template gradients<0, false, true>(scratch_data, values_dofs + n_dofs);
                    else
                      eval0.template gradients<0, false, false>(scratch_data, values_dofs + n_dofs);

                    // grad yz
                    eval1.template gradients<1, false, false>(hessians_quad + 5 * n_q_points, scratch_data);
                    eval0.template values<0, false, true>(scratch_data, values_dofs + n_dofs);
                  ...
                  case 2:
                    ...
                    // grad yy
                    eval0.template values<0, false, false>(hessians_quad + n_q_points, values_dofs + 2 * n_dofs);
                    // grad xy
                    if ((integration_flag & EvaluationFlags::gradients) != 0u)
                      eval0.template gradients<0, false, true>(hessians_quad + 2 * n_q_points, values_dofs + n_dofs);
                    else
                      eval0.template gradients<0, false, false>(hessians_quad + 2 * n_q_points, values_dofs + n_dofs);
                    break;
                  case 1:
  ...
  struct FEFaceEvaluationImplRaviartThomas
  {
                                    ...
                                    const unsigned int                                       direction)
    {
      if (subface_index >= GeometryInfo<dim>::max_children_per_cell)
        return EvalType(data.shape_values, data.shape_gradients, data.shape_hessians);
      else
        {
          const unsigned int index = direction == 0 ? subface_index % 2 : subface_index / 2;
    ...
    template <bool integrate>
    static void
    evaluate_or_integrate_in_face(const EvaluationFlags::EvaluationFlags evaluation_flag,
                                  Number                                *values_dofs,
    ...
    template <int normal_dir>
    static inline void
    evaluate_in_face_apply(Number                                *values_dofs,
                           FEEvaluationData<dim, Number, true>   &fe_eval,
                           Number                                *scratch_data,
                           const EvaluationFlags::EvaluationFlags evaluation_flag,
                           ...
                           std::integral_constant<bool, false>)
    {
      ...
      using TempEval0 = typename std::conditional<normal_dir == 0, EvalNormal, EvalTangent>::type;
      using TempEval1 = typename std::conditional<normal_dir == 0, EvalTangent, EvalNormal>::type;
      using Eval0     = typename std::conditional<normal_dir == 2, EvalGeneral, TempEval0>::type;
      using Eval1     = typename std::conditional<normal_dir == 2, EvalGeneral, TempEval1>::type;

      const auto &shape_info = fe_eval.get_shape_info();
      Eval0       eval0 =
        create_evaluator_tensor_product<Eval0>(((normal_dir == 0) ? shape_info.data[0] : shape_info.data[1]),
                                               subface_index,
                                               0);
      Eval1 eval1 =
        create_evaluator_tensor_product<Eval1>(((normal_dir == 1) ? shape_info.data[0] : shape_info.data[1]),
                                               subface_index,
                                               1);
      ...
      const std::size_t     dofs_stride    = (std::is_same_v<Eval0, EvalGeneral>) ? n_dofs_normal : n_dofs_tangent;

      static constexpr dealii::ndarray<unsigned int, 3, 3> component_table = {{{{1, 2, 0}}, {{2, 0, 1}}, {{0, 1, 2}}}};
      const unsigned int                                   component =
        (dim == 2 && normal_dir == 0 && face_direction == 1) ? 0 : component_table[face_direction][normal_dir];

      // Initial offsets
      values_dofs +=
        3 * ((component == 0) ? 0 :
                                ((component == 1) ? ((face_direction == 0) ? n_dofs_normal : n_dofs_tangent) :
      ...
      Number            *gradients_quad = fe_eval.begin_gradients() + dim * n_q_points * shift;
      Number            *hessians_quad  = fe_eval.begin_hessians() + dim * (dim + 1) / 2 * n_q_points * shift;
          ...
          switch (dim)
            {
              case 3:
                ...
                // grad z
                eval0.template values<0, true, false>(values_dofs + dofs_stride, scratch_data);
                eval1.template values<1, true, false>(scratch_data, gradients_quad + 2 * n_q_points);

                break;
              case 2:
                eval0.template values<0, true, false>(values_dofs + dofs_stride, gradients_quad + n_q_points);
                eval0.template gradients<0, true, false>(values_dofs, gradients_quad);
                if ((evaluation_flag & EvaluationFlags::values))
                  eval0.template values<0, true, false>(values_dofs, values_quad);
                break;
          ...
          switch (dim)
            {
              case 3:
                ...
                // grad zz
                eval0.template values<0, true, false>(values_dofs + 2 * dofs_stride, scratch_data);
                eval1.template values<1, true, false>(scratch_data, hessians_quad + 2 * n_q_points);
                ...
                // grad xz
                eval0.template gradients<0, true, false>(values_dofs + dofs_stride, scratch_data);
                eval1.template values<1, true, false>(scratch_data, hessians_quad + 4 * n_q_points);
              ...
              case 2:
                ...
                eval0.template hessians<0, true, false>(values_dofs, hessians_quad);
                // grad yy
                eval0.template values<0, true, false>(values_dofs + 2 * dofs_stride, hessians_quad + n_q_points);
                // grad xy
                eval0.template gradients<0, true, false>(values_dofs + dofs_stride, hessians_quad + 2 * n_q_points);
                break;
              default:
                AssertThrow(false, ExcNotImplemented());
            }
          ...
          switch (dim)
            {
              case 3:
                ...
                eval1.template values<1, false, false>(gradients_quad + 2 * n_q_points,
                                                       gradients_quad + 2 * n_q_points);
                eval0.template values<0, false, false>(gradients_quad + 2 * n_q_points, values_dofs + dofs_stride);
                ...
                break;
              case 2:
                eval0.template values<0, false, false>(gradients_quad + n_q_points, values_dofs + dofs_stride);
                eval0.template gradients<0, false, false>(gradients_quad, values_dofs);
                if (evaluation_flag & EvaluationFlags::values)
                  eval0.template values<0, false, true>(values_quad, values_dofs);
                break;
          ...
          switch (dim)
            {
              case 3:
                // grad xx
                eval1.template values<1, false, false>(hessians_quad, scratch_data);
                if ((evaluation_flag & (EvaluationFlags::values | EvaluationFlags::gradients)))
                  eval0.template hessians<0, false, true>(scratch_data, values_dofs);
                else
                  eval0.template hessians<0, false, false>(scratch_data, values_dofs);
                ...
                // grad zz
                eval1.template values<1, false, false>(hessians_quad + 2 * n_q_points, scratch_data);
                eval0.template values<0, false, false>(scratch_data, values_dofs + 2 * dofs_stride);
                ...
                eval1.template values<1, false, false>(hessians_quad + 4 * n_q_points, scratch_data);
                if ((evaluation_flag & EvaluationFlags::gradients))
                  eval0.template gradients<0, false, true>(scratch_data, values_dofs + dofs_stride);
                else
                  eval0.template gradients<0, false, false>(scratch_data, values_dofs + dofs_stride);

                // grad yz
                eval1.template gradients<1, false, false>(hessians_quad + 5 * n_q_points, scratch_data);
                eval0.template values<0, false, true>(scratch_data, values_dofs + dofs_stride);
              ...
              case 2:
                ...
                // grad yy
                eval0.template values<0, false, false>(hessians_quad + n_q_points, values_dofs + 2 * dofs_stride);
                // grad xy
                if ((evaluation_flag & EvaluationFlags::gradients))
                  eval0.template gradients<0, false, true>(hessians_quad + 2 * n_q_points, values_dofs + dofs_stride);
                else
                  eval0.template gradients<0, false, false>(hessians_quad + 2 * n_q_points, values_dofs + dofs_stride);
                break;
              default:
                AssertThrow(false, ExcNotImplemented());
            }
  ...
  struct FEFaceNormalEvaluationImpl
  {
    ...
    interpolate(const unsigned int                             n_components,
                ...
                const unsigned int                             face_no)
    {
      Assert(static_cast<unsigned int>(fe_degree) == shape_info.data.front().fe_degree || fe_degree == -1,
             ExcInternalError());
      if (shape_info.element_type == MatrixFreeFunctions::tensor_raviart_thomas)
        ...
        interpolate_generic<do_evaluate, add_into_output>(n_components,
                                                          input,
                                                          output,
                                                          flags,
                                                          face_no,
    ...
    interpolate_quadrature(const unsigned int                             n_components,
                           ...
                           const unsigned int                             face_no)
    {
      Assert(static_cast<unsigned int>(fe_degree + 1) == shape_info.data.front().n_q_points_1d || fe_degree == -1,
             ExcInternalError());
  ...
  private:
                        ...
                        const unsigned int                           dofs_per_component_on_face)
    {
      if (face_direction == face_no / 2)
        {
          EvaluatorTensorProduct<evaluate_general, dim, fe_degree + 1, 0, Number, Number2> evalf(
            shape_data[face_no % 2].begin(), nullptr, nullptr, n_points_1d, 0);

          const unsigned int in_stride  = do_evaluate ? dofs_per_component_on_cell : dofs_per_component_on_face;
          const unsigned int out_stride = do_evaluate ? dofs_per_component_on_face : dofs_per_component_on_cell;

          for (unsigned int c = 0; c < n_components; ++c)
            {
              if (flag & EvaluationFlags::hessians)
                evalf.template apply_face<face_direction, do_evaluate, add_into_output, 2>(input, output);
              else if (flag & EvaluationFlags::gradients)
                evalf.template apply_face<face_direction, do_evaluate, add_into_output, 1>(input, output);
              else
                evalf.template apply_face<face_direction, do_evaluate, add_into_output, 0>(input, output);
              input += in_stride;
              output += out_stride;
            }
        }
        ...
        {
          interpolate_generic<do_evaluate, add_into_output, std::min(face_direction + 1, dim - 1)>(
            n_components,
            input,
            output,
            flag,
    ...
    static void
    interpolate_generic_raviart_thomas(const unsigned int                             n_components,
                                       const Number                                  *input,
                                       Number                                        *output,
                                       const EvaluationFlags::EvaluationFlags         flag,
                                       const unsigned int                             face_no,
                                       const MatrixFreeFunctions::ShapeInfo<Number2> &shape_info)
    {
      ...
      if (face_direction == face_no / 2 && !increase_max_der)
        {
          interpolate_generic_raviart_thomas_apply_face<do_evaluate, add_into_output, face_direction, max_derivative>(
    ...
    static inline void
    interpolate_generic_raviart_thomas_apply_face(const MatrixFreeFunctions::ShapeInfo<Number2> &shape_info,
                                                  const unsigned int                             face_no,
                                                  const Number                                  *input,
                                                  Number                                        *output)
    {
      ...
      using Evalf0 = typename std::conditional<face_direction == 0,
                                               ...
                                               EvaluatorTensorProductAnisotropic<evaluate_raviart_thomas,
                                                                                 ...
                                                                                 Number,
                                                                                 Number2>>::type;
      using Evalf1 = typename std::conditional<face_direction == 1,
                                               EvaluatorTensorProductAnisotropic<evaluate_raviart_thomas,
                                                                                 dim,
                                                                                 (fe_degree == -1) ? 1 : fe_degree + 1,
                                                                                 0,
                                               ...
                                               EvaluatorTensorProductAnisotropic<evaluate_raviart_thomas,
                                                                                 ...
                                                                                 Number,
                                                                                 Number2>>::type;
      using Evalf2 = typename std::conditional<face_direction == 2,
                                               EvaluatorTensorProductAnisotropic<evaluate_raviart_thomas,
                                                                                 dim,
                                                                                 (fe_degree == -1) ? 1 : fe_degree + 1,
                                                                                 0,
      ...
      Evalf0 evalf0 =
        create_evaluator_tensor_product<Evalf0>((face_direction == 0) ? shape_info.data[0] : shape_info.data[1],
                                                face_no);
      Evalf1 evalf1 =
        create_evaluator_tensor_product<Evalf1>((face_direction == 1) ? shape_info.data[0] : shape_info.data[1],
                                                face_no);
      Evalf2 evalf2 =
        create_evaluator_tensor_product<Evalf2>((face_direction == 2) ? shape_info.data[0] : shape_info.data[1],
                                                face_no);

      const unsigned int dofs_per_component_on_cell = shape_info.dofs_per_component_on_cell;
      const unsigned int dofs_per_component_on_face = 3 * shape_info.dofs_per_component_on_face;
      ...
      const unsigned int in_stride_after_normal =
        do_evaluate ? dofs_per_component_on_cell : dofs_per_component_on_face - 3 * Utilities::pow(fe_degree, dim - 2);
      const unsigned int out_stride_after_normal =
        do_evaluate ? dofs_per_component_on_face - 3 * Utilities::pow(fe_degree, dim - 2) : dofs_per_component_on_cell;

      evalf0.template apply_face<face_direction, do_evaluate, add_into_output, max_derivative>(input, output);
      ...
      evalf1.template apply_face<face_direction, do_evaluate, add_into_output, max_derivative>(input, output);

      if (dim == 3)
        {
          // stride to next component
          input += (face_direction == 1) ? in_stride_after_normal : in_stride;
          output += (face_direction == 1) ? out_stride_after_normal : out_stride;

          evalf2.template apply_face<face_direction, do_evaluate, add_into_output, max_derivative>(input, output);
        }
    }
  };
  ...
  template <typename VectorizedArrayType, typename Number2>
  void
  do_vectorized_gather(const Number2 *src_ptr, const unsigned int *indices, VectorizedArrayType &dst)
  {
    for (unsigned int v = 0; v < VectorizedArrayType::size(); ++v)
  ...
  template <typename Number, std::size_t width>
  void
  do_vectorized_gather(const Number *src_ptr, const unsigned int *indices, VectorizedArray<Number, width> &dst)
  {
  ...
  template <typename VectorizedArrayType, typename Number2>
  void
  do_vectorized_scatter_add(const VectorizedArrayType src, const unsigned int *indices, Number2 *dst_ptr)
  {
    for (unsigned int v = 0; v < VectorizedArrayType::size(); ++v)
  ...
  template <typename Number, std::size_t width>
  void
  do_vectorized_scatter_add(const VectorizedArray<Number, width> src, const unsigned int *indices, Number *dst_ptr)
  {
          ...
          for (unsigned int d = 0; d < dim; ++d)
            {
              if (integrate)
                for (unsigned int q = 0; q < n_q_points; ++q)
                  tmp_values[q] = gradients_quad[(c * dim + d) * n_q_points + orientation[q]];
              else
                for (unsigned int q = 0; q < n_q_points; ++q)
                  tmp_values[orientation[q]] = gradients_quad[(c * dim + d) * n_q_points + q];
              for (unsigned int q = 0; q < n_q_points; ++q)
            ...
            for (unsigned int d = 0; d < hdim; ++d)
              {
                if (integrate)
                  for (unsigned int q = 0; q < n_q_points; ++q)
                    tmp_values[q] = hessians_quad[(c * hdim + d) * n_q_points + orientation[q]];
                else
                  for (unsigned int q = 0; q < n_q_points; ++q)
                    tmp_values[orientation[q]] = hessians_quad[(c * hdim + d) * n_q_points + q];
                for (unsigned int q = 0; q < n_q_points; ++q)
                  hessians_quad[(c * hdim + d) * n_q_points + q] = tmp_values[q];
              }
          }
      }
  }
  ...
  void
  adjust_for_face_orientation_per_lane(const unsigned int                     dim,
                                       const unsigned int                     n_components,
          ...
          for (unsigned int d = 0; d < dim; ++d)
            {
              ...
              if (integrate)
                for (unsigned int q = 0; q < n_q_points; ++q)
                  tmp_values[q] = gradients_quad[(c * dim + d) * n_q_points + orientation[q]][v];
              else
                for (unsigned int q = 0; q < n_q_points; ++q)
                  tmp_values[orientation[q]] = gradients_quad[(c * dim + d) * n_q_points + q][v];
              for (unsigned int q = 0; q < n_q_points; ++q)
                gradients_quad[(c * dim + d) * n_q_points + q][v] = tmp_values[q];
            }
        if (flag & EvaluationFlags::hessians)
          {
            ...
            for (unsigned int d = 0; d < hdim; ++d)
              {
                if (integrate)
                  for (unsigned int q = 0; q < n_q_points; ++q)
                    tmp_values[q] = hessians_quad[(c * hdim + d) * n_q_points + orientation[q]][v];
                else
                  for (unsigned int q = 0; q < n_q_points; ++q)
                    tmp_values[orientation[q]] = hessians_quad[(c * hdim + d) * n_q_points + q][v];
                for (unsigned int q = 0; q < n_q_points; ++q)
                  hessians_quad[(c * hdim + d) * n_q_points + q][v] = tmp_values[q];
              }
          }
      }
  }
  ...
  struct FEFaceEvaluationImplEvaluateSelector
  {
        ...
        FEEvaluationData<dim, Number, true>   &fe_eval)
    {
      ...
      if (shape_info.element_type == MatrixFreeFunctions::tensor_none)
        {
          Assert((fe_eval.get_dof_access_index() == MatrixFreeFunctions::DoFInfo::dof_access_cell &&
          ...
          if (evaluation_flag & EvaluationFlags::values)
            {
              const auto *const shape_values = &shape_data.shape_values_face(face_no, face_orientation, 0);
          ...
          if (evaluation_flag & EvaluationFlags::gradients)
            {
              ...
              for (unsigned int d = 0; d < dim; ++d)
                shape_gradients[d] = &shape_data.shape_gradients_face(face_no, face_orientation, d, 0);

              for (unsigned int c = 0; c < n_components; ++c)
                {
                  for (unsigned int d = 0; d < dim; ++d)
                    {
                      Eval eval(nullptr, shape_gradients[d], nullptr, n_dofs, n_q_points);
      ...
      const unsigned int dofs_per_face =
        fe_degree > -1 ? Utilities::pow(fe_degree + 1, dim - 1) : Utilities::pow(shape_data.fe_degree + 1, dim - 1);
      ...
      if (fe_eval.get_dof_access_index() == MatrixFreeFunctions::DoFInfo::dof_access_cell &&
      ...
      if (use_vectorization == false)
          ...
          for (unsigned int v = 0; v < Number::size(); ++v)
            {
              ...
              if (fe_eval.get_cell_ids()[v] == numbers::invalid_unsigned_int)
                {
                  for (unsigned int i = 0; i < 3 * n_components * dofs_per_face; ++i)
                    temp[i][v] = 0;
                  continue;
                }

              FEFaceNormalEvaluationImpl<dim, fe_degree, Number>::template interpolate<true, false>(
                n_components, evaluation_flag, shape_info, values_dofs, scratch_data, fe_eval.get_face_no(v));
        ...
        FEFaceNormalEvaluationImpl<dim, fe_degree, Number>::template interpolate<true, false>(
          n_components, evaluation_flag, shape_info, values_dofs, temp, fe_eval.get_face_no());

      const unsigned int     subface_index        = fe_eval.get_subface_index();
      constexpr unsigned int n_q_points_1d_actual = fe_degree > -1 ? n_q_points_1d : 0;
      ...
      if (fe_degree >= 1 && shape_info.element_type == MatrixFreeFunctions::tensor_raviart_thomas)
        {
          FEFaceEvaluationImplRaviartThomas<dim,
                                            (fe_degree == -1) ? 1 : fe_degree,
                                            (n_q_points_1d < 1) ? 1 : n_q_points_1d,
                                            Number>::template evaluate_or_integrate_in_face<false>(evaluation_flag,
                                                                                                   temp,
                                                                                                   fe_eval,
                                                                                                   scratch_data,
                                                                                                   subface_index,
      ...
      else if (fe_degree > -1 && subface_index >= GeometryInfo<dim>::max_children_per_cell &&
               shape_info.element_type <= MatrixFreeFunctions::tensor_symmetric)
        FEFaceEvaluationImpl<true, dim, fe_degree, n_q_points_1d_actual, Number>::evaluate_in_face(
          n_components,
          evaluation_flag,
          shape_data,
          temp,
        ...
        FEFaceEvaluationImpl<false, dim, fe_degree, n_q_points_1d_actual, Number>::evaluate_in_face(
          n_components,
          evaluation_flag,
          shape_data,
          temp,
      ...
      if (use_vectorization == false)
          ...
          for (unsigned int v = 0; v < Number::size(); ++v)
            {
              ...
              if (fe_eval.get_face_orientation(v) != 0)
                adjust_for_face_orientation_per_lane(
                  ...
                  v,
                  evaluation_flag,
                  &fe_eval.get_shape_info().face_orientations_quad(fe_eval.get_face_orientation(v), 0),
                  false,
                  shape_info.n_q_points_face,
                  &temp[0][0],
                  fe_eval.begin_values(),
        ...
        adjust_for_face_orientation(dim,
                                    n_components,
                                    evaluation_flag,
                                    &fe_eval.get_shape_info().face_orientations_quad(fe_eval.get_face_orientation(), 0),
                                    false,
                                    shape_info.n_q_points_face,
                                    temp,
                                    fe_eval.begin_values(),
        ...
        FEEvaluationData<dim, Number, true>   &fe_eval)
    {
      ...
      if (fe_eval.get_dof_access_index() == MatrixFreeFunctions::DoFInfo::dof_access_cell &&
        ...
        use_vectorization =
          ...
          std::all_of(fe_eval.get_cell_ids().begin() + 1, fe_eval.get_cell_ids().end(), [&](const auto &v) {
            return v == fe_eval.get_cell_ids()[0] || v == numbers::invalid_unsigned_int;
          });
      ...
      if (fe_degree >= 1 && shape_info.element_type == MatrixFreeFunctions::tensor_raviart_thomas)
        {
          FEFaceEvaluationImplRaviartThomas<dim,
                                            (fe_degree == -1) ? 1 : fe_degree,
                                            (n_q_points_1d < 1) ? 1 : n_q_points_1d,
                                            Number>::template evaluate_or_integrate_in_face<true>(integration_flag,
                                                                                                  temp,
                                                                                                  fe_eval,
                                                                                                  scratch_data,
                                                                                                  subface_index,
      ...
      else if (fe_degree > -1 && fe_eval.get_subface_index() >= GeometryInfo<dim - 1>::max_children_per_cell &&
               shape_info.element_type <= MatrixFreeFunctions::tensor_symmetric)
        ...
        FEFaceEvaluationImpl<false, dim, fe_degree, n_q_points_1d_actual, Number>::integrate_in_face(
          n_components,
          integration_flag,
          shape_data,
          temp,
      ...
      if (use_vectorization == false)
          ...
          for (unsigned int v = 0; v < Number::size(); ++v)
            {
              ...
              FEFaceNormalEvaluationImpl<dim, fe_degree, Number>::template interpolate<false, false>(
                n_components, integration_flag, shape_info, values_dofs, scratch_data, fe_eval.get_face_no(v));
        ...
        FEFaceNormalEvaluationImpl<dim, fe_degree, Number>::template interpolate<false, false>(
          n_components, integration_flag, shape_info, temp, values_dofs, fe_eval.get_face_no());
      return false;
    }
  };
                                    ...
                                    typename Processor::VectorizedArrayType_                         *temp1)
  {
    ...
    const MatrixFreeFunctions::DoFInfo::DoFAccessIndex dof_access_index = fe_eval.get_dof_access_index();
    ...
    const unsigned int n_filled_lanes = dof_info.n_vectorization_lanes_filled[dof_access_index][cell];
    ...
    if (dim == 3 && n_face_orientations == n_lanes && !all_faces_are_same && fe_eval.is_interior_face() == 0)
      for (unsigned int v = 0; v < n_lanes; ++v)
        {
          ...
          if (shape_data.nodal_at_cell_boundaries && fe_eval.get_face_orientation(v) != 0)
            {
              // ... and in case we detect a re-orientation, go to the other
              // version of this function that actually allows for this
              if (subface_index == GeometryInfo<dim>::max_children_per_cell && check_face_orientations == false)
                {
                  fe_face_evaluation_process_and_io<n_face_orientations, Processor, EvaluationData, true>(
                    proc, n_components, evaluation_flag, global_vector_ptr, sm_ptr, fe_eval, temp1);
                  return;
                }
              orientation[v] = &fe_eval.get_shape_info().face_orientations_dofs(fe_eval.get_face_orientation(v), 0);
            }
        }
    else if (dim == 3 && fe_eval.get_face_orientation() != 0)
      {
        ...
        for (unsigned int v = 0; v < n_face_orientations; ++v)
          orientation[v] = &fe_eval.get_shape_info().face_orientations_dofs(fe_eval.get_face_orientation(), 0);
      }
    ...
    VectorizedArrayType grad_weight =
      shape_data.shape_data_on_face[0][fe_degree + (integrate ? (2 - face_no % 2) : (1 + face_no % 2))];
    ...
    if (fe_degree > 1 && (evaluation_flag & EvaluationFlags::gradients))
      {
        if (n_face_orientations == 1)
          index_array_hermite[0] = &fe_eval.get_shape_info().face_to_cell_index_hermite(face_no, 0);
        else
          {
            for (unsigned int v = 0; v < n_lanes; ++v)
              {
                ...
                grad_weight[v] =
                  shape_data
                    .shape_data_on_face[0][fe_degree + (integrate ? (2 - (face_no % 2)) : (1 + (face_no % 2)))][0];

                index_array_hermite[v] = &fe_eval.get_shape_info().face_to_cell_index_hermite(face_no, 0);
              }
          }
      }
    ...
    if (shape_data.nodal_at_cell_boundaries == true)
      {
        if (n_face_orientations == 1)
          index_array_nodal[0] = &fe_eval.get_shape_info().face_to_cell_index_nodal(face_no, 0);
        else
          {
            for (unsigned int v = 0; v < n_lanes; ++v)
              {
                ...
                index_array_nodal[v] = &fe_eval.get_shape_info().face_to_cell_index_nodal(face_no, 0);
              }
          }
      }
    ...
    const auto reorientate = [&](const unsigned int v, const unsigned int i) {
      return (!check_face_orientations || orientation[v] == nullptr) ? i : orientation[v][i];
    };

    const unsigned int cell_index =
      dof_access_index == MatrixFreeFunctions::DoFInfo::dof_access_cell ? fe_eval.get_cell_ids()[0] : cell * n_lanes;
    ...
    for (unsigned int comp = 0; comp < n_components; ++comp)
      {
        ...
        if (n_face_orientations == 1 && dof_info.index_storage_variants[dof_access_index][cell] ==
                                          MatrixFreeFunctions::DoFInfo::IndexStorageVariants::interleaved_contiguous)
          {
            AssertDimension(dof_info.n_vectorization_lanes_filled[dof_access_index][cell], n_lanes);
            Number2_ *vector_ptr = global_vector_ptr + dof_indices[0] + index_offset * n_lanes;
                ...
                for (unsigned int i = 0; i < dofs_per_face; ++i)
                  {
                    ...
                    proc.value_vectorized(temp1[i_], vector_ptr + ind * n_lanes);
                  }
              }
          }
        ...
        // case 2: contiguous and interleaved indices with fixed stride
                 ...
                 dof_info.index_storage_variants[dof_access_index][cell] ==
                   MatrixFreeFunctions::DoFInfo::IndexStorageVariants::interleaved_contiguous_strided)
          {
            AssertDimension(dof_info.n_vectorization_lanes_filled[dof_access_index][cell], n_lanes);
                ...
                for (unsigned int i = 0; i < dofs_per_face; ++i)
                  {
                    ...
                    const unsigned int i_   = reorientate(0, i);
                    const unsigned int ind1 = index_array_hermite[0][2 * i] * n_lanes;
                    const unsigned int ind2 = index_array_hermite[0][2 * i + 1] * n_lanes;
                    proc.hermite_grad_vectorized_indexed(temp1[i_],
                                                         temp1[i_ + dofs_per_face],
                                                         vector_ptr + ind1,
                                                         vector_ptr + ind2,
                                                         grad_weight,
                ...
                for (unsigned int i = 0; i < dofs_per_face; ++i)
                  {
                    ...
                    proc.value_vectorized_indexed(temp1[i_], vector_ptr + ind, dof_indices);
                  }
              }
          }
        ...
        // case 3: contiguous and interleaved indices with mixed stride
                 ...
                 dof_info.index_storage_variants[dof_access_index][cell] ==
                   MatrixFreeFunctions::DoFInfo::IndexStorageVariants::interleaved_contiguous_mixed_strides)
          {
            const unsigned int *strides = &dof_info.dof_indices_interleave_strides[dof_access_index][cell * n_lanes];
                ...
                if (n_filled_lanes == n_lanes)
                  for (unsigned int i = 0; i < dofs_per_face; ++i)
                    {
                      ...
                      for (unsigned int v = 0; v < n_lanes; ++v)
                        ind1[v] = indices[v] + index_array_hermite[0][2 * i] * strides[v];
                ...
                else
                  {
                    ...
                    for (unsigned int v = 0; v < n_filled_lanes; ++v)
                      for (unsigned int i = 0; i < dofs_per_face; ++i)
                        {
                          const unsigned int i_ = reorientate(n_face_orientations == 1 ? 0 : v, i);
                          proc.hermite_grad(
                            temp1[i_][v],
                            temp1[i_ + dofs_per_face][v],
                            global_vector_ptr[indices[v] +
                                              index_array_hermite[n_face_orientations == 1 ? 0 : v][2 * i] *
                                                strides[v]],
                            global_vector_ptr[indices[v] +
                                              index_array_hermite[n_face_orientations == 1 ? 0 : v][2 * i + 1] *
                                                strides[v]],
                  ...
                  for (unsigned int i = 0; i < dofs_per_face; ++i)
                    {
                      ...
                      DEAL_II_OPENMP_SIMD_PRAGMA
                      for (unsigned int v = 0; v < n_lanes; ++v)
                        ind[v] = indices[v] + index_array_nodal[0][i] * strides[v];
                      const unsigned int i_ = reorientate(0, i);
                      proc.value_vectorized_indexed(temp1[i_], global_vector_ptr, ind);
                    }
                else
                  {
                    if (integrate == false)
                    ...
                    for (unsigned int v = 0; v < n_filled_lanes; ++v)
                      for (unsigned int i = 0; i < dofs_per_face; ++i)
                        proc.value(
                          temp1[reorientate(n_face_orientations == 1 ? 0 : v, i)][v],
                          global_vector_ptr[indices[v] +
                                            index_array_nodal[n_face_orientations == 1 ? 0 : v][i] * strides[v]]);
                  }
              }
          }

        // case 4: contiguous indices without interleaving
        else if (n_face_orientations > 1 || dof_info.index_storage_variants[dof_access_index][cell] ==
                                              MatrixFreeFunctions::DoFInfo::IndexStorageVariants::contiguous)
          {
            Number2_ *vector_ptr = global_vector_ptr + index_offset;

            const bool vectorization_possible = all_faces_are_same && (sm_ptr == nullptr);
            ...
            if (vectorization_possible == false)
                ...
                if (n_face_orientations == 1)
                  {
                    for (unsigned int v = 0; v < n_filled_lanes; ++v)
                      if (sm_ptr == nullptr)
                        ...
                        {
                          const auto &temp = dof_info.dof_indices_contiguous_sm[dof_access_index][cell * n_lanes + v];
                          vector_ptrs[v] =
                            const_cast<Number2_ *>(sm_ptr->operator[](temp.first).data() + temp.second + index_offset);
                        }
                  }
                      ...
                      if (cells[v] != numbers::invalid_unsigned_int)
                        {
                          if (sm_ptr == nullptr)
                            {
                              vector_ptrs[v] = vector_ptr + dof_info.dof_indices_contiguous[dof_access_index][cells[v]];
                            }
                          else
                            {
                              const auto &temp = dof_info.dof_indices_contiguous_sm[dof_access_index][cells[v]];
                              vector_ptrs[v]   = const_cast<Number2_ *>(sm_ptr->operator[](temp.first).data() +
              ...
              {
                for (unsigned int v = 0; v < n_lanes; ++v)
                  reordered_indices[v] = dof_info.dof_indices_contiguous[dof_access_index][fe_eval.get_cell_ids()[v]];
            ...
            if (fe_degree > 1 && (evaluation_flag & EvaluationFlags::gradients))
              {
                if (vectorization_possible)
                ...
                else
                  {
                    ...
                    for (unsigned int v = 0; v < n_filled_lanes; ++v)
                      for (unsigned int i = 0; i < dofs_per_face; ++i)
                        proc.hermite_grad(temp1[reorientate(v, i)][v],
            ...
            else
              {
                if (vectorization_possible)
                ...
                else
                  {
                    ...
                    for (unsigned int v = 0; v < n_filled_lanes; ++v)
                      for (unsigned int i = 0; i < dofs_per_face; ++i)
                        proc.value(temp1[reorientate(v, i)][v], vector_ptrs[v][index_array_nodal[v][i]]);
                  }
              }
          }
        else
  ...
  struct FEFaceEvaluationImplGatherEvaluateSelector
  {
        ...
        FEEvaluationData<dim, VectorizedArrayType, true> &fe_eval)
    {
      ...
      Assert(fe_eval.get_shape_info().element_type <= MatrixFreeFunctions::tensor_symmetric_no_collocation,
             ExcInternalError());

      const unsigned int dofs_per_face = Utilities::pow(fe_degree + 1, dim - 1);

      VectorizedArrayType *temp         = fe_eval.get_scratch_data().begin();
      VectorizedArrayType *scratch_data = temp + 3 * n_components * dofs_per_face;
      ...
      if (fe_eval.get_dof_access_index() == MatrixFreeFunctions::DoFInfo::dof_access_cell &&
        ...
        fe_face_evaluation_process_and_io<1>(p, n_components, evaluation_flag, src_ptr, sm_ptr, fe_eval, temp);

      const unsigned int subface_index = fe_eval.get_subface_index();

      if (subface_index >= GeometryInfo<dim>::max_children_per_cell)
        FEFaceEvaluationImpl<true, dim, fe_degree, n_q_points_1d, VectorizedArrayType>::evaluate_in_face(
          n_components,
          evaluation_flag,
        ...
        FEFaceEvaluationImpl<false, dim, fe_degree, n_q_points_1d, VectorizedArrayType>::evaluate_in_face(
          n_components,
          evaluation_flag,
          fe_eval.get_shape_info().data.front(),
          temp,
      ...
      if (subface_index < GeometryInfo<dim>::max_children_per_cell)
        {
          if (fe_eval.get_dof_access_index() == MatrixFreeFunctions::DoFInfo::dof_access_cell &&
            ...
            adjust_for_face_orientation(dim,
                                        n_components,
                                        evaluation_flag,
                                        &fe_eval.get_shape_info().face_orientations_quad(fe_eval.get_face_orientation(),
                                                                                         0),
                                        false,
                                        Utilities::pow(n_q_points_1d, dim - 1),
                                        temp,
             ...
             MatrixFreeFunctions::DoFInfo::IndexStorageVariants storage)
    {
      ...
      if (fe_degree < 1 || !shape_info.data.front().nodal_at_cell_boundaries ||
          (evaluation_flag & EvaluationFlags::gradients &&
           (fe_degree < 2 || shape_info.data.front().element_type != MatrixFreeFunctions::tensor_symmetric_hermite)) ||
          (evaluation_flag & EvaluationFlags::hessians) || vector_ptr == nullptr ||
          shape_info.data.front().element_type > MatrixFreeFunctions::tensor_symmetric_no_collocation ||
          storage < MatrixFreeFunctions::DoFInfo::IndexStorageVariants::contiguous)
        return false;
      else
        return true;
    }
  ...
  private:
    ...
    struct Processor
    {
      ...
      void
      hermite_grad_vectorized(T0 &temp_1, T0 &temp_2, const T1 src_ptr_1, const T1 src_ptr_2, const T2 &grad_weight)
      {
      ...
      template <typename T0, typename T1, typename T2>
      void
      hermite_grad(T0 &temp_1, T0 &temp_2, const T1 &src_ptr_1, const T1 &src_ptr_2, const T2 &grad_weight)
      {
        // case 3a)
        temp_1 = src_ptr_1;
  ...
  struct FEFaceEvaluationImplIntegrateScatterSelector
  {
        ...
        FEEvaluationData<dim, VectorizedArrayType, true> &fe_eval)
    {
      ...
      if (fe_degree > -1 && fe_eval.get_subface_index() >= GeometryInfo<dim - 1>::max_children_per_cell)
        FEFaceEvaluationImpl<true, dim, fe_degree, n_q_points_1d, VectorizedArrayType>::integrate_in_face(
          n_components,
          integration_flag,
          fe_eval.get_shape_info().data.front(),
          temp,
        ...
        FEFaceEvaluationImpl<false, dim, fe_degree, n_q_points_1d, VectorizedArrayType>::integrate_in_face(
          n_components,
          integration_flag,
          fe_eval.get_shape_info().data.front(),
          temp,
      ...
      if (fe_eval.get_dof_access_index() == MatrixFreeFunctions::DoFInfo::dof_access_cell &&
        ...
        fe_face_evaluation_process_and_io<1>(p, n_components, integration_flag, dst_ptr, sm_ptr, fe_eval, temp);
  ...
  private:
    ...
    struct Processor
    {
      ...
      void
      hermite_grad_vectorized(const T0 &temp_1, const T1 &temp_2, T2 dst_ptr_1, T3 dst_ptr_2, const T4 &grad_weight)
      {
        // case 1a)
      ...
      template <typename T0, typename T1, typename T2>
      void
      hermite_grad(const T0 &temp_1, const T0 &temp_2, T1 &dst_ptr_1, T1 &dst_ptr_2, const T2 &grad_weight)
      {
        // case 3a)
  ...
  struct CellwiseInverseMassMatrixImplBasic
  {
    ...
    run(const unsigned int                          n_components,
        ...
        Number                                     *out_array)
    {
      const unsigned int given_degree = (fe_degree > -1) ? fe_degree : fe_eval.get_shape_info().data.front().fe_degree;
      ...
      EvaluatorTensorProduct<evaluate_evenodd, dim, fe_degree + 1, fe_degree + 1, Number, Number2> evaluator(
        {}, {}, fe_eval.get_shape_info().data.front().inverse_shape_values_eo, given_degree + 1, given_degree + 1);
  ...
  struct CellwiseInverseMassMatrixImplFlexible
  {
        ...
        Number                                     *out_array)
    {
      ...
      Assert(inverse_coefficients.size() > 0 && inverse_coefficients.size() % dofs_per_component == 0,
      ...
      if (!dyadic_coefficients)
        {
          if (inverse_coefficients.size() != dofs_per_component)
            AssertDimension(n_desired_components * dofs_per_component, inverse_coefficients.size());
        }
      else
        {
          AssertDimension(n_desired_components * n_desired_components * dofs_per_component,
      ...
      for (unsigned int d = 0; d < n_comp_outer; ++d)
        {
          ...
          if (dyadic_coefficients)
            {
              const auto n_coeff_components = n_desired_components * n_desired_components;
              if (n_desired_components == dim)
                {
                  for (unsigned int q = 0; q < dofs_per_component; ++q)
                    vmult<dim>(&inv_coefficient[q * n_coeff_components], &in[q], &out[q], dofs_per_component);
                }
              else
                {
                  for (unsigned int q = 0; q < dofs_per_component; ++q)
  ...
  private:
    ...
    {
      const unsigned int n_desired_components = (n_components > -1) ? n_components : n_given_components;

      std::array<Number, dim + 2> tmp = {};
      Assert(n_desired_components <= dim + 2, ExcMessage("Number of components larger than dim+2 not supported."));
      ...
      for (unsigned int d1 = 0; d1 < n_desired_components; ++d1)
        {
          const Number *inv_coeff_row = &inverse_coefficients[d1 * n_desired_components];
  ...
  struct CellwiseInverseMassMatrixImplTransformFromQPoints
  {
        ...
        Number                                     *out_array)
    {
      ...
      const auto &inverse_shape = do_inplace ? fe_eval.get_shape_info().data.front().inverse_shape_values_eo :
                                               fe_eval.get_shape_info().data.front().inverse_shape_values;

      const std::size_t dofs_per_component =
        do_inplace ? Utilities::pow(fe_degree + 1, dim) : fe_eval.get_shape_info().dofs_per_component_on_cell;
      const std::size_t n_q_points =

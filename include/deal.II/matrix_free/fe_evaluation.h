...
class FEEvaluationBase : public FEEvaluationData<dim, VectorizedArrayType, is_face>
{
public:
  using number_type                          = Number;
  using value_type                           = Tensor<1, n_components_, VectorizedArrayType>;
  using gradient_type                        = Tensor<1, n_components_, Tensor<1, dim, VectorizedArrayType>>;
   ...
   */
  void
  submit_divergence(const VectorizedArrayType div_in, const unsigned int q_point);
   ...
   */
  void
  submit_symmetric_gradient(const SymmetricTensor<2, dim, VectorizedArrayType> grad_in, const unsigned int q_point);
   ...
   */
  void
  submit_curl(const Tensor<1, dim == 2 ? 1 : dim, VectorizedArrayType> curl_in, const unsigned int q_point);
...
protected:
   ...
   * components.
   */
  FEEvaluationBase(const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
...
class FEEvaluationAccess : public FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>
{
  static_assert(std::is_same_v<Number, typename VectorizedArrayType::value_type>,
...
public:
  ...
  static constexpr unsigned int dimension    = dim;
  static constexpr unsigned int n_components = n_components_;
  using BaseClass = FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>;
...
protected:
   ...
   */
  FEEvaluationAccess(const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
                     const unsigned int                                  dof_no,
                     const unsigned int                                  first_selected_component,
                     const unsigned int                                  quad_no,
                     const unsigned int                                  fe_degree,
...
public:
  ...
  static constexpr unsigned int dimension = dim;
  using BaseClass                         = FEEvaluationBase<dim, 1, Number, is_face, VectorizedArrayType>;
   ...
   */
  void
  submit_value(const Tensor<1, 1, VectorizedArrayType> val_in, const unsigned int q_point);
...
public:
  ...
  using BaseClass                            = FEEvaluationBase<dim, dim, Number, is_face, VectorizedArrayType>;
   ...
   */
  void
  submit_value(const Tensor<1, dim, VectorizedArrayType> val_in, const unsigned int q_point);
   ...
   */
  void
  submit_gradient(const Tensor<1, dim, Tensor<1, dim, VectorizedArrayType>> grad_in, const unsigned int q_point);
...
public:
  ...
  using BaseClass                         = FEEvaluationBase<1, 1, Number, is_face, VectorizedArrayType>;
   ...
   */
  void
  submit_gradient(const Tensor<2, 1, VectorizedArrayType> grad_in, const unsigned int q_point);
...
protected:
   ...
   * quad_no allow to select the appropriate components.
   */
  FEEvaluationAccess(const MatrixFree<1, Number, VectorizedArrayType> &matrix_free,
                     const unsigned int                                dof_no,
                     const unsigned int                                first_selected_component,
                     const unsigned int                                quad_no,
                     const unsigned int                                fe_degree,
...
class FEEvaluation : public FEEvaluationAccess<dim, n_components_, Number, false, VectorizedArrayType>
{
  static_assert(std::is_same_v<Number, typename VectorizedArrayType::value_type>,
                "Type of Number and of VectorizedArrayType do not match.");
...
public:
   ...
   */
  static bool
  fast_evaluation_supported(const unsigned int given_degree, const unsigned int given_n_q_points_1d);
   ...
   */
  void
  evaluate(const VectorizedArrayType *values_array, const EvaluationFlags::EvaluationFlags evaluation_flag);
  ...
  template <typename VectorType>
  void
  gather_evaluate(const VectorType &input_vector, const EvaluationFlags::EvaluationFlags evaluation_flag);
  ...
  template <typename VectorType>
  void
  integrate_scatter(const EvaluationFlags::EvaluationFlags integration_flag, VectorType &output_vector);
...
template <int dim,
          ...
          typename VectorizedArrayType = VectorizedArray<Number>>
class FEFaceEvaluation : public FEEvaluationAccess<dim, n_components_, Number, true, VectorizedArrayType>
{
  static_assert(std::is_same_v<Number, typename VectorizedArrayType::value_type>,
                "Type of Number and of VectorizedArrayType do not match.");
...
public:
   ...
   */
  void
  integrate(const EvaluationFlags::EvaluationFlags integration_flag, VectorizedArrayType *values_array);
  ...
  template <typename VectorType>
  void
  integrate_scatter(const bool integrate_values, const bool integrate_gradients, VectorType &output_vector);
...
namespace internal
{
  namespace MatrixFreeFunctions
  {
    ...
    struct DGP_dofs_per_component
    {
      ...
      static constexpr unsigned int value = (DGP_dofs_per_component<dim - 1, degree>::value * (degree + dim)) / dim;
    };
...
namespace internal
{
  ...
  template <bool is_face, int dim, typename Number, typename VectorizedArrayType>
  inline typename FEEvaluationData<dim, VectorizedArrayType, is_face>::InitializationData
  extract_initialization_data(const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
                              const unsigned int                                  dof_no,
                              const unsigned int                                  first_selected_component,
                              const unsigned int                                  quad_no,
                              const unsigned int                                  fe_degree,
                              ...
                              const unsigned int                                  face_type)
  {
    typename FEEvaluationData<dim, VectorizedArrayType, is_face>::InitializationData init_data;

    init_data.dof_info = &matrix_free.get_dof_info(dof_no);
    init_data.mapping_data =
      &internal::MatrixFreeFunctions::MappingInfoCellsOrFaces<dim, Number, is_face, VectorizedArrayType>::get(
        matrix_free.get_mapping_info(), quad_no);

    init_data.active_fe_index = fe_degree != numbers::invalid_unsigned_int ?
                                  init_data.dof_info->fe_index_from_degree(first_selected_component, fe_degree) :
                                  (active_fe_index_given != numbers::invalid_unsigned_int ? active_fe_index_given : 0);
    init_data.active_quad_index =
      fe_degree == numbers::invalid_unsigned_int ?
        (active_quad_index_given != numbers::invalid_unsigned_int ?
           active_quad_index_given :
           std::min<unsigned int>(init_data.active_fe_index, init_data.mapping_data->descriptor.size() - 1)) :
        init_data.mapping_data->quad_index_from_n_q_points(n_q_points);

    init_data.shape_info =
    ...
    init_data.descriptor =
      &init_data.mapping_data->descriptor[is_face ? (init_data.active_quad_index * std::max<unsigned int>(1, dim - 1) +
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::FEEvaluationBase(
  const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
  const unsigned int                                  dof_no,
  const unsigned int                                  first_selected_component,
  const unsigned int                                  quad_no,
  ...
  Assert(
    this->dof_info->start_components.back() == 1 ||
      static_cast<int>(n_components_) <=
        static_cast<int>(
          this->dof_info->start_components[this->dof_info->component_to_base_index[first_selected_component] + 1]) -
          first_selected_component,
    ExcMessage(
      "You tried to construct a vector-valued evaluator with " + std::to_string(n_components) +
      " components. However, "
      ...
      std::to_string(
        first_selected_component -
        this->dof_info->start_components[this->dof_info->component_to_base_index[first_selected_component]]) +
...
inline FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::FEEvaluationBase(
  ...
  const FEEvaluationData<dim, VectorizedArrayType, is_face> *other)
  : FEEvaluationData<dim, VectorizedArrayType, is_face>(
      other != nullptr && other->mapped_geometry->get_quadrature() == quadrature ?
        other->mapped_geometry :
        std::make_shared<internal::MatrixFreeFunctions::MappingDataOnTheFly<dim, VectorizedArrayType>>(mapping,
                                                                                                       quadrature,
                                                                                                       update_flags),
      n_components_,
      first_selected_component)
  ...
  , matrix_free(nullptr)
{
  const unsigned int base_element_number = fe.component_to_base_index(first_selected_component).first;
  ...
  this->data = new internal::MatrixFreeFunctions::ShapeInfo<VectorizedArrayType>(
    Quadrature<(is_face ? dim - 1 : dim)>(quadrature), fe, fe.component_to_base_index(first_selected_component).first);
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::FEEvaluationBase(
  const FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType> &other)
  : FEEvaluationData<dim, VectorizedArrayType, is_face>(other)
  , scratch_data_array(other.matrix_free == nullptr ? new AlignedVector<VectorizedArrayType>() :
  ...
  if (other.matrix_free == nullptr)
    {
      ...
      this->data = new internal::MatrixFreeFunctions::ShapeInfo<VectorizedArrayType>(*other.data);

      // Create deep copy of mapped geometry for use in parallel
      this->mapped_geometry =
        std::make_shared<internal::MatrixFreeFunctions::MappingDataOnTheFly<dim, VectorizedArrayType>>(
      ...
      this->jacobian           = this->mapped_geometry->get_data_storage().jacobians[0].begin();
      this->J_value            = this->mapped_geometry->get_data_storage().JxW_values.begin();
      this->jacobian_gradients = this->mapped_geometry->get_data_storage().jacobian_gradients[0].begin();
      this->jacobian_gradients_non_inverse =
        this->mapped_geometry->get_data_storage().jacobian_gradients_non_inverse[0].begin();
      this->quadrature_points = this->mapped_geometry->get_data_storage().quadrature_points.begin();
    }
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType> &
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::operator=(
  const FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType> &other)
{
  // release old memory
...
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::get_matrix_free() const
{
  Assert(matrix_free != nullptr, ExcMessage("FEEvaluation was not initialized with a MatrixFree object!"));
  return *matrix_free;
}
...
namespace internal
{
  ...
  {
    using BaseVectorType = typename ConstBlockVectorSelector<VectorType, std::is_const_v<VectorType>>::BaseVectorType;
  ...
  struct BlockVectorSelector<const std::vector<VectorType>, false>
  {
    ...
    get_vector_component(const std::vector<VectorType> &vec, const unsigned int component)
    {
      AssertIndexRange(component, vec.size());
      return &vec[component];
    }
...
template <typename VectorType, typename VectorOperation>
inline void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::read_write_operation(
  const VectorOperation                                                                                  &operation,
  const std::array<VectorType *, n_components_>                                                          &src,
  const std::array<const std::vector<ArrayView<const typename VectorType::value_type>> *, n_components_> &src_sm,
  const std::bitset<n_lanes>                                                                             &mask,
  const bool apply_constraints) const
{
    ...
    for (unsigned int comp = 0; comp < n_components; ++comp)
      {
        ...
        internal::check_vector_compatibility(*src[comp], *this->matrix_free, *this->dof_info);
      }
  else
    {
      internal::check_vector_compatibility(*src[0], *this->matrix_free, *this->dof_info);
    }
  ...
  if (this->cell != numbers::invalid_unsigned_int)
    {
      AssertIndexRange(this->cell, dof_info.index_storage_variants[this->dof_access_index].size());
      ...
      if (accesses_exterior_dofs)
        {
          ...
          const unsigned int                       n_filled_lanes =
            dof_info.n_vectorization_lanes_filled[internal::MatrixFreeFunctions::DoFInfo::dof_access_cell][this->cell];
          // we have to check all filled lanes which are active in the mask
          for (unsigned int v = 0; v < n_filled_lanes; ++v)
            if (mask[v] == true &&
                dof_info.index_storage_variants[internal::MatrixFreeFunctions::DoFInfo::dof_access_cell]
                                               [cells[v] / n_lanes] <
                  internal::MatrixFreeFunctions::DoFInfo::IndexStorageVariants::contiguous)
              is_contiguous = false;
        } // or if cell/face batch is not contiguous
      else if (dof_info.index_storage_variants[is_face ?
                                                 this->dof_access_index :
                                                 internal::MatrixFreeFunctions::DoFInfo::dof_access_cell][this->cell] <
               internal::MatrixFreeFunctions::DoFInfo::IndexStorageVariants::contiguous)
        {
          is_contiguous = false;
        }
  ...
  if (is_face == false)
    {
      if (!dof_info.hanging_node_constraint_masks.empty() && !dof_info.hanging_node_constraint_masks_comp.empty() &&
          dof_info.hanging_node_constraint_masks_comp[this->active_fe_index][this->first_selected_component])
        for (unsigned int v = 0; v < n_lanes; ++v)
  ...
  std::integral_constant<bool, internal::is_vectorizable<VectorType, Number>::value> vector_selector;

  const bool use_vectorized_path = !(masking_is_active || has_hn_constraints || accesses_exterior_dofs);
  ...
  for (unsigned int c = 0; c < n_components; ++c)
    values_dofs[c] = const_cast<VectorizedArrayType *>(this->values_dofs) + c * dofs_per_component;

  if (this->cell != numbers::invalid_unsigned_int &&
      dof_info.index_storage_variants[is_face ? this->dof_access_index :
                                                internal::MatrixFreeFunctions::DoFInfo::dof_access_cell][this->cell] ==
      ...
      use_vectorized_path)
    {
      const unsigned int *dof_indices =
        ...
        dof_info.row_starts[this->cell * this->n_fe_components * n_lanes].first +
        this->dof_info->component_dof_indices_offset[this->active_fe_index][this->first_selected_component] * n_lanes;
      if (n_components == 1 || this->n_fe_components == 1)
        for (unsigned int i = 0; i < dofs_per_component; ++i, dof_indices += n_lanes)
          for (unsigned int comp = 0; comp < n_components; ++comp)
            operation.process_dof_gather(dof_indices, *src[comp], 0, values_dofs[comp][i], vector_selector);
      else
        for (unsigned int comp = 0; comp < n_components; ++comp)
          for (unsigned int i = 0; i < dofs_per_component; ++i, dof_indices += n_lanes)
            operation.process_dof_gather(dof_indices, *src[0], 0, values_dofs[comp][i], vector_selector);
      return;
    }
  ...
  const unsigned int n_components_read = this->n_fe_components > 1 ? n_components : 1;
      ...
      for (unsigned int v = 0; v < n_lanes; ++v)
        {
          ...
          const std::pair<unsigned int, unsigned int> *my_index_start =
            &dof_info.row_starts[cells[v] * this->n_fe_components + this->first_selected_component];
          ...
          if (my_index_start[n_components_read].second != my_index_start[0].second)
            has_constraints = true;

          dof_indices[v] = dof_info.dof_indices.data() + my_index_start[0].first;
        }
    }
  else
    {
      for (unsigned int v = 0; v < n_lanes; ++v)
        {
          ...
          Assert(my_index_start[n_components_read].first == my_index_start[0].first ||
                   my_index_start[0].first < dof_info.dof_indices.size(),
                 ExcIndexRange(0, my_index_start[0].first, dof_info.dof_indices.size()));
          dof_indices[v] = dof_info.dof_indices.data() + my_index_start[0].first;
        }
    }

  if (std::count_if(cells.begin(), cells.end(), [](const auto i) { return i != numbers::invalid_unsigned_int; }) <
      n_lanes)
    for (unsigned int comp = 0; comp < n_components; ++comp)
  ...
  for (unsigned int v = 0; v < n_lanes; ++v)
    {
      ...
      const unsigned int cell_dof_index        = cell_index * this->n_fe_components + this->first_selected_component;
      const unsigned int n_components_read     = this->n_fe_components > 1 ? n_components : 1;
      unsigned int       index_indicators      = dof_info.row_starts[cell_dof_index].second;
      unsigned int       next_index_indicators = dof_info.row_starts[cell_dof_index + 1].second;
      ...
      if (apply_constraints == false &&
          (dof_info.row_starts[cell_dof_index].second !=
           ...
           ((dof_info.hanging_node_constraint_masks.size() > 0 &&
            ...
            dof_info.hanging_node_constraint_masks_comp[this->active_fe_index][this->first_selected_component])))
        {
          Assert(dof_info.row_starts_plain_indices[cell_index] != numbers::invalid_unsigned_int, ExcNotInitialized());
          dof_indices[v] =
          ...
          for (; index_indicators != next_index_indicators; ++index_indicators)
            {
              ...
              for (unsigned int j = 0; j < indicator.first; ++j)
                for (unsigned int comp = 0; comp < n_components; ++comp)
                  operation.process_dof(dof_indices[v][j], *src[comp], values_dofs[comp][ind_local + j][v]);
              ...
              const Number *data_val = this->matrix_free->constraint_pool_begin(indicator.second);
              const Number *end_pool = this->matrix_free->constraint_pool_end(indicator.second);
          ...
          for (unsigned int comp = 0; comp < n_components; ++comp)
            {
              ...
              if (apply_constraints == true && comp + 1 < n_components)
                next_index_indicators = dof_info.row_starts[cell_dof_index + comp + 2].second;
            }
        }
    }
}
...
template <typename VectorType, typename VectorOperation>
inline void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::read_write_operation_global(
  const VectorOperation                         &operation,
  ...
  for (unsigned int comp = 0; comp < n_components; ++comp)
    {
      for (unsigned int i = 0; i < dofs_per_component; ++i, ++index)
        {
          operation.process_empty(this->values_dofs[comp * dofs_per_component + i]);
...
template <typename VectorType, typename VectorOperation>
inline void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::read_write_operation_contiguous(
  const VectorOperation                                                                                  &operation,
  const std::array<VectorType *, n_components_>                                                          &src,
  const std::array<const std::vector<ArrayView<const typename VectorType::value_type>> *, n_components_> &vectors_sm,
  const std::bitset<n_lanes>                                                                             &mask) const
{
  ...
  const internal::MatrixFreeFunctions::DoFInfo::DoFAccessIndex                       ind =
    is_face ? this->dof_access_index : internal::MatrixFreeFunctions::DoFInfo::dof_access_cell;
      ...
      if (n_components == 1 || this->n_fe_components == 1)
        for (unsigned int comp = 0; comp < n_components; ++comp)
          operation.process_dofs_vectorized(
            dofs_per_component, dof_index, *src[comp], values_dofs[comp], vector_selector);
      else
        operation.process_dofs_vectorized(
          dofs_per_component * n_components, dof_index, *src[0], values_dofs[0], vector_selector);
      return;
    }
  ...
  const unsigned int n_filled_lanes = dof_info.n_vectorization_lanes_filled[ind][this->cell];

  const bool use_vectorized_path = n_filled_lanes == n_lanes && n_active_lanes == n_lanes && !accesses_exterior_dofs;
  ...
  if (vectors_sm[0] != nullptr)
    {
      const auto compute_vector_ptrs = [&](const unsigned int comp) {
        ...
        const auto upper_bound = std::min<unsigned int>(n_filled_lanes, n_lanes);
        for (unsigned int v = 0; v < upper_bound; ++v)
          {
            ...
            Assert(ind < dof_info.dof_indices_contiguous_sm.size(),
                   ExcIndexRange(ind, 0, dof_info.dof_indices_contiguous_sm.size()));
            Assert(cells[v] < dof_info.dof_indices_contiguous_sm[ind].size(),
                   ExcIndexRange(cells[v], 0, dof_info.dof_indices_contiguous_sm[ind].size()));

            const auto &temp = dof_info.dof_indices_contiguous_sm[ind][cells[v]];
            ...
            if (temp.first != numbers::invalid_unsigned_int)
              vector_ptrs[v] = const_cast<typename VectorType::value_type *>(
                ...
                dof_info.component_dof_indices_offset[this->active_fe_index][this->first_selected_component]);
            else
              vector_ptrs[v] = nullptr;
          }
      ...
      if (use_vectorized_path)
        {
          if (n_components == 1 || this->n_fe_components == 1)
          ...
          else
            {
              ...
              operation.process_dofs_vectorized_transpose(dofs_per_component * n_components,
                                                          vector_ptrs,
                                                          &values_dofs[0][0],
                                                          vector_selector);
            }
        ...
        for (unsigned int comp = 0; comp < n_components; ++comp)
          {
            auto vector_ptrs = compute_vector_ptrs((n_components == 1 || this->n_fe_components == 1) ? comp : 0);
            ...
            if (n_components == 1 || this->n_fe_components == 1)
              {
                for (unsigned int v = 0; v < n_filled_lanes; ++v)
                  if (mask[v] == true)
                    for (unsigned int i = 0; i < dofs_per_component; ++i)
                      operation.process_dof(vector_ptrs[v][i], values_dofs[comp][i][v]);
              }
            else
              {
                for (unsigned int v = 0; v < n_filled_lanes; ++v)
                  if (mask[v] == true)
                    for (unsigned int i = 0; i < dofs_per_component; ++i)
                      operation.process_dof(vector_ptrs[v][i + comp * dofs_per_component], values_dofs[comp][i][v]);
              }
          }
      return;
    }
  ...
  std::fill(dof_indices.begin(), dof_indices.end(), numbers::invalid_unsigned_int);

  Assert(n_filled_lanes <= n_lanes, ExcInternalError());
  for (unsigned int v = 0; v < n_filled_lanes; ++v)
    {
      Assert(mask[v] == false || cells[v] != numbers::invalid_unsigned_int, ExcNotImplemented());
      if (mask[v] == true)
        dof_indices[v] =
      ...
      if (dof_info.index_storage_variants[ind][this->cell] ==
          ...
          if (n_components == 1 || this->n_fe_components == 1)
            for (unsigned int comp = 0; comp < n_components; ++comp)
              operation.process_dofs_vectorized_transpose(
                dofs_per_component, dof_indices.data(), *src[comp], values_dofs[comp], vector_selector);
          else
            operation.process_dofs_vectorized_transpose(
              dofs_per_component * n_components, dof_indices.data(), *src[0], &values_dofs[0][0], vector_selector);
        }
            ...
            for (unsigned int i = 0; i < dofs_per_component; ++i)
              {
                for (unsigned int comp = 0; comp < n_components; ++comp)
                  operation.process_dof_gather(
                    dof_indices.data(), *src[comp], i * n_lanes, values_dofs[comp][i], vector_selector);
              }
          else
            for (unsigned int comp = 0; comp < n_components; ++comp)
              for (unsigned int i = 0; i < dofs_per_component; ++i)
      ...
      else
        {
          Assert(dof_info.index_storage_variants[ind][this->cell] ==
                 ...
                 ExcNotImplemented());
          const unsigned int *offsets = &dof_info.dof_indices_interleave_strides[ind][n_lanes * this->cell];
          if (n_components == 1 || this->n_fe_components == 1)
            for (unsigned int i = 0; i < dofs_per_component; ++i)
              {
                for (unsigned int comp = 0; comp < n_components; ++comp)
                  operation.process_dof_gather(
                    dof_indices.data(), *src[comp], 0, values_dofs[comp][i], vector_selector);
            ...
            for (unsigned int comp = 0; comp < n_components; ++comp)
              for (unsigned int i = 0; i < dofs_per_component; ++i)
                {
                  operation.process_dof_gather(dof_indices.data(), *src[0], 0, values_dofs[comp][i], vector_selector);
                  DEAL_II_OPENMP_SIMD_PRAGMA
                  for (unsigned int v = 0; v < n_lanes; ++v)
                    dof_indices[v] += offsets[v];
                }
        ...
        if (accesses_exterior_dofs)
          {
            for (unsigned int v = 0; v < n_filled_lanes; ++v)
              if (mask[v] == true)
                {
                  if (dof_info.index_storage_variants[ind][cells[v] / VectorizedArrayType::size()] ==
                      internal::MatrixFreeFunctions::DoFInfo::IndexStorageVariants::contiguous)
                    {
                      if (n_components == 1 || this->n_fe_components == 1)
                        {
                        ...
                        {
                          for (unsigned int i = 0; i < dofs_per_component; ++i)
                            operation.process_dof(dof_indices[v] + i + comp * dofs_per_component,
                                                  *src[0],
                                                  values_dofs[comp][i][v]);
                        }
                    }
                    ...
                    {
                      const unsigned int offset = dof_info.dof_indices_interleave_strides[ind][cells[v]];
                      AssertIndexRange(offset, VectorizedArrayType::size() + 1);
                      if (n_components == 1 || this->n_fe_components == 1)
                        {
                          for (unsigned int i = 0; i < dofs_per_component; ++i)
                            operation.process_dof(dof_indices[v] + i * offset, *src[comp], values_dofs[comp][i][v]);
                        }
                      else
                        {
                          for (unsigned int i = 0; i < dofs_per_component; ++i)
                            operation.process_dof(dof_indices[v] + (i + comp * dofs_per_component) * offset,
                                                  *src[0],
                                                  values_dofs[comp][i][v]);
                        }
                    }
            ...
            if (dof_info.index_storage_variants[ind][this->cell] ==
            ...
            else
              {
                const unsigned int *offsets =
                  &dof_info.dof_indices_interleave_strides[ind][VectorizedArrayType::size() * this->cell];
                for (unsigned int v = 0; v < n_filled_lanes; ++v)
                  ...
                  for (unsigned int v = 0; v < n_filled_lanes; ++v)
                    {
                      if (mask[v] == true)
                        for (unsigned int i = 0; i < dofs_per_component; ++i)
                          operation.process_dof(dof_indices[v] + i * offsets[v], *src[comp], values_dofs[comp][i][v]);
                    }
                else
                  {
                    for (unsigned int v = 0; v < n_filled_lanes; ++v)
                      if (mask[v] == true)
                        for (unsigned int i = 0; i < dofs_per_component; ++i)
                          operation.process_dof(dof_indices[v] + (i + comp * dofs_per_component) * offsets[v],
                                                *src[0],
                                                values_dofs[comp][i][v]);
                  }
              }
...
namespace internal
{
  ...
  template <typename VectorType, std::enable_if_t<has_shared_vector_data<VectorType>, VectorType> * = nullptr>
                         ...
                         const internal::MatrixFreeFunctions::DoFInfo *dof_info)
  {
    ...
    if (is_valid_mode_for_sm && dof_info->dof_indices_contiguous_sm[0 /*any index (<3) should work*/].size() > 0 &&
        active_fe_index == 0)
  ...
  template <typename VectorType, std::enable_if_t<!has_shared_vector_data<VectorType>, VectorType> * = nullptr>
  const std::vector<ArrayView<const typename VectorType::value_type>> *
  get_shared_vector_data(VectorType *, const bool, const unsigned int, const internal::MatrixFreeFunctions::DoFInfo *)
  {
    return nullptr;
  }
  ...
  std::pair<
    std::array<typename internal::BlockVectorSelector<VectorType, IsBlockVector<VectorType>::value>::BaseVectorType *,
               n_components>,
    std::array<const std::vector<
                 ArrayView<const typename internal::BlockVectorSelector<VectorType, IsBlockVector<VectorType>::value>::
                  ...
                  const internal::MatrixFreeFunctions::DoFInfo *dof_info)
  {
    ...
    std::pair<
      std::array<typename internal::BlockVectorSelector<VectorType, IsBlockVector<VectorType>::value>::BaseVectorType *,
                 n_components>,
      std::array<const std::vector<ArrayView<const typename internal::BlockVectorSelector<
                   VectorType,
    ...
    for (unsigned int d = 0; d < n_components; ++d)
      src_data.first[d] =
        internal::BlockVectorSelector<VectorType, IsBlockVector<VectorType>::value>::get_vector_component(
          src, d + first_index);

    for (unsigned int d = 0; d < n_components; ++d)
      src_data.second[d] =
        get_shared_vector_data(const_cast<typename internal::BlockVectorSelector<
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::apply_hanging_node_constraints(
  const bool transpose) const
{
  if (this->dof_info == nullptr || this->dof_info->hanging_node_constraint_masks.empty() ||
      this->dof_info->hanging_node_constraint_masks_comp.empty() ||
      this->dof_info->hanging_node_constraint_masks_comp[this->active_fe_index][this->first_selected_component] ==
        false)
    return; // nothing to do with faces

  std::array<internal::MatrixFreeFunctions::compressed_constraint_kind, n_lanes> constraint_mask;
  ...
  for (unsigned int v = 0; v < n_lanes; ++v)
    {
      if (cells[v] == numbers::invalid_unsigned_int)
        {
          constraint_mask[v] = internal::MatrixFreeFunctions::unconstrained_compressed_constraint_kind;
          continue;
        }

      const unsigned int cell_index = cells[v];
      const auto         mask       = this->dof_info->hanging_node_constraint_masks[cell_index];
      constraint_mask[v]            = mask;

      hn_available |= (mask != internal::MatrixFreeFunctions::unconstrained_compressed_constraint_kind);
    }

  if (hn_available == false)
    return; // no hanging node on cell batch -> nothing to do

  internal::FEEvaluationHangingNodesFactory<dim, Number, VectorizedArrayType>::apply(n_components,
...
template <typename VectorType>
inline void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::read_dof_values(
  const VectorType           &src,
  const unsigned int          first_index,
  const std::bitset<n_lanes> &mask)
{
  const auto src_data =
    internal::get_vector_data<n_components_>(src,
                                             first_index,
                                             this->dof_access_index ==
...
template <typename VectorType>
inline void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::read_dof_values_plain(
  const VectorType           &src,
  const unsigned int          first_index,
  const std::bitset<n_lanes> &mask)
{
...
template <typename VectorType>
inline void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::distribute_local_to_global(
  VectorType                 &dst,
  const unsigned int          first_index,
  const std::bitset<n_lanes> &mask) const
{
...
#  endif
  ...
  const auto dst_data =
    internal::get_vector_data<n_components_>(dst,
                                             first_index,
                                             this->dof_access_index ==
                                               internal::MatrixFreeFunctions::DoFInfo::dof_access_cell,
                                             this->active_fe_index,
  ...
  internal::VectorDistributorLocalToGlobal<Number, VectorizedArrayType> distributor;
...
template <typename VectorType>
inline void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::set_dof_values(
  VectorType                 &dst,
  const unsigned int          first_index,
  const std::bitset<n_lanes> &mask) const
{
...
template <typename VectorType>
inline void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::set_dof_values_plain(
  VectorType                 &dst,
  const unsigned int          first_index,
  const std::bitset<n_lanes> &mask) const
{
...
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::get_dof_value(const unsigned int dof) const
{
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE Tensor<1, n_components_, VectorizedArrayType>
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::get_value(const unsigned int q_point) const
{
#  ifdef DEBUG
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE Tensor<1, n_components_, Tensor<1, dim, VectorizedArrayType>>
                             FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::get_gradient(
...
#  endif
  ...
  AssertIndexRange(q_point, this->n_quadrature_points);
  Assert(this->jacobian != nullptr, internal::ExcMatrixFreeAccessToUninitializedMappingField("update_gradients"));
  const std::size_t                                             nqp = this->n_quadrature_points;
  ...
  if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
    {
      for (unsigned int d = 0; d < dim; ++d)
        for (unsigned int comp = 0; comp < n_components; ++comp)
          grad_out[comp][d] = this->gradients_quad[(comp * dim + d) * nqp + q_point] * this->jacobian[0][d][d];
    }
  // cell with general/affine Jacobian
  else
    {
      ...
      for (unsigned int comp = 0; comp < n_components; ++comp)
        for (unsigned int d = 0; d < dim; ++d)
          {
            grad_out[comp][d] = jac[d][0] * this->gradients_quad[(comp * dim) * nqp + q_point];
            for (unsigned int e = 1; e < dim; ++e)
              grad_out[comp][d] += jac[d][e] * this->gradients_quad[(comp * dim + e) * nqp + q_point];
          }
    }
  return grad_out;
}
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE Tensor<1, n_components_, VectorizedArrayType>
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::get_normal_derivative(
  const unsigned int q_point) const
{
  AssertIndexRange(q_point, this->n_quadrature_points);
#  ifdef DEBUG
...
#  endif
  ...
  Assert(this->normal_x_jacobian != nullptr,
         internal::ExcMatrixFreeAccessToUninitializedMappingField("update_gradients"));
  ...
  if (this->cell_type == internal::MatrixFreeFunctions::cartesian)
    for (unsigned int comp = 0; comp < n_components; ++comp)
      grad_out[comp] =
        this->gradients_quad[(comp * dim + dim - 1) * nqp + q_point] * (this->normal_x_jacobian[0][dim - 1]);
  else
    {
      const std::size_t index = this->cell_type <= internal::MatrixFreeFunctions::affine ? 0 : q_point;
      for (unsigned int comp = 0; comp < n_components; ++comp)
        {
          grad_out[comp] = this->gradients_quad[comp * dim * nqp + q_point] * this->normal_x_jacobian[index][0];
          for (unsigned int d = 1; d < dim; ++d)
            grad_out[comp] +=
              this->gradients_quad[(comp * dim + d) * nqp + q_point] * this->normal_x_jacobian[index][d];
        }
    }
  return grad_out;
}
...
namespace internal
{
    ...
    for (unsigned int d = 0; d < 2; ++d)
      {
        ...
        tmp[1][d] = (jac[d][0] * hessians[2 * nqp] + jac[d][1] * hessians[1 * nqp]);
      }
  }
                         ...
                         VectorizedArrayType (&tmp)[3][3])
  {
    for (unsigned int d = 0; d < 3; ++d)
      {
        tmp[0][d] = (jac[d][0] * hessians[0 * nqp] + jac[d][1] * hessians[3 * nqp] + jac[d][2] * hessians[4 * nqp]);
        tmp[1][d] = (jac[d][0] * hessians[3 * nqp] + jac[d][1] * hessians[1 * nqp] + jac[d][2] * hessians[5 * nqp]);
        tmp[2][d] = (jac[d][0] * hessians[4 * nqp] + jac[d][1] * hessians[5 * nqp] + jac[d][2] * hessians[2 * nqp]);
      }
  }
} // namespace internal
...
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::get_hessian(
  const unsigned int q_point) const
{
#  ifdef DEBUG
...
#  endif
  ...
  Assert(this->jacobian != nullptr, internal::ExcMatrixFreeAccessToUninitializedMappingField("update_hessian"));
  const Tensor<2, dim, VectorizedArrayType> &jac =
    this->jacobian[this->cell_type <= internal::MatrixFreeFunctions::affine ? 0 : q_point];
  ...
  if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
    {
      for (unsigned int comp = 0; comp < n_components; ++comp)
        {
          for (unsigned int d = 0; d < dim; ++d)
            hessian_out[comp][d][d] = this->hessians_quad[(comp * hdim + d) * nqp + q_point] * (jac[d][d] * jac[d][d]);
          switch (dim)
            {
              case 1:
                break;
              case 2:
                hessian_out[comp][0][1] =
                  this->hessians_quad[(comp * hdim + 2) * nqp + q_point] * (jac[0][0] * jac[1][1]);
                break;
              case 3:
                hessian_out[comp][0][1] =
                  this->hessians_quad[(comp * hdim + 3) * nqp + q_point] * (jac[0][0] * jac[1][1]);
                hessian_out[comp][0][2] =
                  this->hessians_quad[(comp * hdim + 4) * nqp + q_point] * (jac[0][0] * jac[2][2]);
                hessian_out[comp][1][2] =
                  this->hessians_quad[(comp * hdim + 5) * nqp + q_point] * (jac[1][1] * jac[2][2]);
                break;
              default:
      ...
      for (unsigned int comp = 0; comp < n_components; ++comp)
        {
          ...
          internal::hessian_unit_times_jac(jac, this->hessians_quad + comp * hdim * nqp + q_point, nqp, tmp);
      ...
      for (unsigned int comp = 0; comp < n_components; ++comp)
        {
          ...
          for (unsigned int d = 0; d < dim; ++d)
            for (unsigned int e = 0; e < dim; ++e)
              hessian_out[comp][d][d] += jac_grad[d][e] * this->gradients_quad[(comp * dim + e) * nqp + q_point];

          // add off-diagonal part of J' * grad(u)
          for (unsigned int d = 0, count = dim; d < dim; ++d)
            for (unsigned int e = d + 1; e < dim; ++e, ++count)
              for (unsigned int f = 0; f < dim; ++f)
                hessian_out[comp][d][e] += jac_grad[count][f] * this->gradients_quad[(comp * dim + f) * nqp + q_point];
...
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::get_hessian_diagonal(
  const unsigned int q_point) const
{
  ...
  if (this->cell_type == internal::MatrixFreeFunctions::cartesian)
    {
      for (unsigned int comp = 0; comp < n_components; ++comp)
        for (unsigned int d = 0; d < dim; ++d)
          hessian_out[comp][d] = this->hessians_quad[(comp * hdim + d) * nqp + q_point] * (jac[d][d] * jac[d][d]);
    }
  // cell with general Jacobian, but constant within the cell
  else if (this->cell_type == internal::MatrixFreeFunctions::affine)
    {
      ...
      for (unsigned int comp = 0; comp < n_components; ++comp)
        {
          ...
          for (unsigned int d = 0; d < dim; ++d)
            for (unsigned int e = 0; e < dim; ++e)
              hessian_out[comp][d] += jac_grad[d][e] * this->gradients_quad[(comp * dim + e) * nqp + q_point];
        }
    }
  return hessian_out;
}
...
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::get_laplacian(
  const unsigned int q_point) const
{
...
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::submit_dof_value(
  const Tensor<1, n_components_, VectorizedArrayType> val_in,
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::submit_value(
  const Tensor<1, n_components_, VectorizedArrayType> val_in,
...
#  endif
  AssertIndexRange(q_point, this->n_quadrature_points);
  Assert(this->J_value != nullptr, internal::ExcMatrixFreeAccessToUninitializedMappingField("update_values"));
#  ifdef DEBUG
...
#  endif
  ...
  if (this->cell_type <= internal::MatrixFreeFunctions::affine)
    {
      const VectorizedArrayType JxW = this->J_value[0] * this->quadrature_weights[q_point];
      for (unsigned int comp = 0; comp < n_components; ++comp)
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::submit_gradient(
...
#  endif
  AssertIndexRange(q_point, this->n_quadrature_points);
  Assert(this->J_value != nullptr, internal::ExcMatrixFreeAccessToUninitializedMappingField("update_gradients"));
  Assert(this->jacobian != nullptr, internal::ExcMatrixFreeAccessToUninitializedMappingField("update_gradients"));
#  ifdef DEBUG
      ...
      for (unsigned int d = 0; d < dim; ++d)
        {
          ...
          for (unsigned int comp = 0; comp < n_components; ++comp)
            this->gradients_quad[(comp * dim + d) * nqp + q_point] = grad_in[comp][d] * factor;
        }
    }
  else
    {
      const Tensor<2, dim, VectorizedArrayType> jac =
        this->cell_type > internal::MatrixFreeFunctions::affine ? this->jacobian[q_point] : this->jacobian[0];
      const VectorizedArrayType JxW = this->cell_type > internal::MatrixFreeFunctions::affine ?
        ...
        for (unsigned int d = 0; d < dim; ++d)
          {
            ...
            this->gradients_quad[(comp * dim + d) * nqp + q_point] = new_val * JxW;
          }
    }
}
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::submit_normal_derivative(
...
#  endif
  ...
  if (this->cell_type == internal::MatrixFreeFunctions::cartesian)
    {
      const VectorizedArrayType JxW_jac =
        this->J_value[0] * this->quadrature_weights[q_point] * this->normal_x_jacobian[0][dim - 1];
      for (unsigned int comp = 0; comp < n_components; ++comp)
        {
          for (unsigned int d = 0; d < dim - 1; ++d)
            this->gradients_quad[(comp * dim + d) * nqp + q_point] = VectorizedArrayType();
          this->gradients_quad[(comp * dim + dim - 1) * nqp + q_point] = grad_in[comp] * JxW_jac;
        }
    }
  else
    {
      const unsigned int index = this->cell_type <= internal::MatrixFreeFunctions::affine ? 0 : q_point;
      const Tensor<1, dim, VectorizedArrayType> jac = this->normal_x_jacobian[index];
      const VectorizedArrayType                 JxW = (this->cell_type <= internal::MatrixFreeFunctions::affine) ?
      ...
      for (unsigned int comp = 0; comp < n_components; ++comp)
        {
          for (unsigned int d = 0; d < dim; ++d)
            this->gradients_quad[(comp * dim + d) * nqp + q_point] = (grad_in[comp] * JxW) * jac[d];
        }
    }
}
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::submit_hessian(
  const Tensor<1, n_components_, Tensor<2, dim, VectorizedArrayType>> hessian_in,
  const unsigned int                                                  q_point)
{
#  ifdef DEBUG
  Assert(this->is_reinitialized, ExcNotInitialized());
#  endif
  AssertIndexRange(q_point, this->n_quadrature_points);
  Assert(this->J_value != nullptr, internal::ExcMatrixFreeAccessToUninitializedMappingField("update_hessians"));
  Assert(this->jacobian != nullptr, internal::ExcMatrixFreeAccessToUninitializedMappingField("update_hessians"));
#  ifdef DEBUG
      ...
      for (unsigned int d = 0; d < dim; ++d)
        {
          ...
          for (unsigned int comp = 0; comp < n_components; ++comp)
            this->hessians_quad[(comp * hdim + d) * nqp + q_point] = hessian_in[comp][d][d] * factor;
        }
      ...
      for (unsigned int comp = 0; comp < n_components; ++comp)
        {
          ...
          // diagonal part
          for (unsigned int d = 0; d < dim; ++d)
            this->hessians_quad[(comp * hdim + d) * nqp + q_point] = tmp2[d][d] * JxW;

          // off diagonal part
          for (unsigned int d = 0, off_diag = dim; d < dim; ++d)
            for (unsigned int e = d + 1; e < dim; ++e, ++off_diag)
              this->hessians_quad[(comp * hdim + off_diag) * nqp + q_point] = (tmp2[d][e] + tmp2[e][d]) * JxW;
        }
    }
  else
    {
          ...
          for (unsigned int d = 0; d < dim; ++d)
            {
              ...
              for (unsigned int e = 0, count = dim; e < dim; ++e)
                for (unsigned int f = e + 1; f < dim; ++f, ++count)
                  sum += (hessian_in[comp][e][f] + hessian_in[comp][f][e]) * jac_grad[count][d];
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline Tensor<1, n_components_, VectorizedArrayType>
FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::integrate_value() const
{
#  ifdef DEBUG
  Assert(this->is_reinitialized, ExcNotInitialized());
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline FEEvaluationAccess<dim, n_components_, Number, is_face, VectorizedArrayType>::FEEvaluationAccess(
  const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
  const unsigned int                                  dof_no,
  const unsigned int                                  first_selected_component,
  const unsigned int                                  quad_no,
  ...
  const unsigned int                                  active_quad_index,
  const unsigned int                                  face_type)
  : FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>(matrix_free,
                                                                               dof_no,
                                                                               first_selected_component,
                                                                               quad_no,
                                                                               fe_degree,
...
inline FEEvaluationAccess<dim, n_components_, Number, is_face, VectorizedArrayType>::FEEvaluationAccess(
  ...
  const unsigned int                                         first_selected_component,
  const FEEvaluationData<dim, VectorizedArrayType, is_face> *other)
  : FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>(mapping,
                                                                               fe,
                                                                               quadrature,
                                                                               update_flags,
                                                                               first_selected_component,
...
template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline FEEvaluationAccess<dim, n_components_, Number, is_face, VectorizedArrayType>::FEEvaluationAccess(
  const FEEvaluationAccess<dim, n_components_, Number, is_face, VectorizedArrayType> &other)
  : FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>(other)
{}



template <int dim, int n_components_, typename Number, bool is_face, typename VectorizedArrayType>
inline FEEvaluationAccess<dim, n_components_, Number, is_face, VectorizedArrayType> &
FEEvaluationAccess<dim, n_components_, Number, is_face, VectorizedArrayType>::operator=(
  const FEEvaluationAccess<dim, n_components_, Number, is_face, VectorizedArrayType> &other)
{
  this->FEEvaluationBase<dim, n_components_, Number, is_face, VectorizedArrayType>::operator=(other);
  return *this;
}
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::FEEvaluationAccess(
  const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
  const unsigned int                                  dof_no,
  const unsigned int                                  first_selected_component,
  const unsigned int                                  quad_no,
  ...
  const unsigned int                                  active_quad_index,
  const unsigned int                                  face_type)
  : FEEvaluationBase<dim, 1, Number, is_face, VectorizedArrayType>(matrix_free,
                                                                   dof_no,
                                                                   first_selected_component,
                                                                   quad_no,
                                                                   fe_degree,
...
inline FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::FEEvaluationAccess(
  ...
  const unsigned int                                         first_selected_component,
  const FEEvaluationData<dim, VectorizedArrayType, is_face> *other)
  : FEEvaluationBase<dim, 1, Number, is_face, VectorizedArrayType>(mapping,
                                                                   fe,
                                                                   quadrature,
                                                                   update_flags,
                                                                   first_selected_component,
...
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::operator=(
  const FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType> &other)
{
  this->FEEvaluationBase<dim, 1, Number, is_face, VectorizedArrayType>::operator=(other);
  return *this;
}
...
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::get_dof_value(const unsigned int dof) const
{
  AssertIndexRange(dof, this->data->dofs_per_component_on_cell);
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE VectorizedArrayType
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::get_value(const unsigned int q_point) const
{
#  ifdef DEBUG
  Assert(this->values_quad_initialized == true, internal::ExcAccessToUninitializedField());
#  endif
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE VectorizedArrayType
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::get_normal_derivative(
  const unsigned int q_point) const
{
...
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::get_gradient(const unsigned int q_point) const
{
...
#  endif
  ...
  if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
    {
      for (unsigned int d = 0; d < dim; ++d)
        grad_out[d] = this->gradients_quad[d * nqp + q_point] * this->jacobian[0][d][d];
    }
  // cell with general/affine Jacobian
  else
    {
...
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::get_hessian(const unsigned int q_point) const
{
...
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::get_hessian_diagonal(const unsigned int q_point) const
{
...
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::get_laplacian(const unsigned int q_point) const
{
...
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::submit_dof_value(const VectorizedArrayType val_in,
                                                                                   const unsigned int        dof)
{
#  ifdef DEBUG
  this->dof_values_initialized = true;
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline void DEAL_II_ALWAYS_INLINE
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::submit_value(const VectorizedArrayType val_in,
                                                                               const unsigned int        q_point)
{
#  ifdef DEBUG
  Assert(this->is_reinitialized, ExcNotInitialized());
#  endif
  AssertIndexRange(q_point, this->n_quadrature_points);
  Assert(this->J_value != nullptr, internal::ExcMatrixFreeAccessToUninitializedMappingField("update_value"));
#  ifdef DEBUG
  this->values_quad_submitted = true;
#  endif
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::submit_normal_derivative(
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::submit_gradient(
  const Tensor<1, dim, VectorizedArrayType> grad_in,
  const unsigned int                        q_point)
{
#  ifdef DEBUG
  Assert(this->is_reinitialized, ExcNotInitialized());
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::submit_hessian(
  const Tensor<2, dim, VectorizedArrayType> hessian_in,
  const unsigned int                        q_point)
{
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline VectorizedArrayType
FEEvaluationAccess<dim, 1, Number, is_face, VectorizedArrayType>::integrate_value() const
{
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::FEEvaluationAccess(
  const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
  const unsigned int                                  dof_no,
  const unsigned int                                  first_selected_component,
  const unsigned int                                  quad_no,
  ...
  const unsigned int                                  active_quad_index,
  const unsigned int                                  face_type)
  : FEEvaluationBase<dim, dim, Number, is_face, VectorizedArrayType>(matrix_free,
                                                                     dof_no,
                                                                     first_selected_component,
                                                                     quad_no,
                                                                     fe_degree,
...
inline FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::FEEvaluationAccess(
  ...
  const unsigned int                                         first_selected_component,
  const FEEvaluationData<dim, VectorizedArrayType, is_face> *other)
  : FEEvaluationBase<dim, dim, Number, is_face, VectorizedArrayType>(mapping,
                                                                     fe,
                                                                     quadrature,
                                                                     update_flags,
                                                                     first_selected_component,
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::FEEvaluationAccess(
  const FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType> &other)
...
FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::operator=(
  const FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType> &other)
{
  this->FEEvaluationBase<dim, dim, Number, is_face, VectorizedArrayType>::operator=(other);
  return *this;
}
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE Tensor<1, dim, VectorizedArrayType>
FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::get_value(const unsigned int q_point) const
{
  if (this->data->element_type == internal::MatrixFreeFunctions::ElementType::tensor_raviart_thomas)
    {
      ...
      if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
        {
          ...
          const VectorizedArrayType                 inv_det =
            (dim == 2) ? this->jacobian[0][0][0] * this->jacobian[0][1][1] :
                                         this->jacobian[0][0][0] * this->jacobian[0][1][1] * this->jacobian[0][2][2];

          // J * u * det(J^-1)
          for (unsigned int comp = 0; comp < n_components; ++comp)
            value_out[comp] = this->values_quad[comp * nqp + q_point] * jac[comp][comp] * inv_det;
        }
      else
        {
          ...
          const Tensor<2, dim, VectorizedArrayType> &inv_t_jac =
            (this->cell_type > internal::MatrixFreeFunctions::affine) ? this->jacobian[q_point] : this->jacobian[0];
          const Tensor<2, dim, VectorizedArrayType> &jac = (this->cell_type > internal::MatrixFreeFunctions::affine) ?
                                                             transpose(invert(inv_t_jac)) :
                                                             this->jacobian[1];

          // Derivatives are reordered for faces. Need to take this into account
          const VectorizedArrayType inv_det =
            (is_face && dim == 2 && this->get_face_no() < 2) ? -determinant(inv_t_jac) : determinant(inv_t_jac);
          // J * u * det(J^-1)
          for (unsigned int comp = 0; comp < n_components; ++comp)
            {
              value_out[comp] = this->values_quad[q_point] * jac[comp][0] * inv_det;
              for (unsigned int e = 1; e < dim; ++e)
                value_out[comp] += this->values_quad[e * nqp + q_point] * jac[comp][e] * inv_det;
            }
        }
      return value_out;
    }
...
FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::get_gradient(const unsigned int q_point) const
{
  if (this->data->element_type == internal::MatrixFreeFunctions::ElementType::tensor_raviart_thomas)
    {
      // Piola transform is required
...
#  endif
      ...
      if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
        {
          // Cartesian cell
          const Tensor<2, dim, VectorizedArrayType> &inv_t_jac = this->jacobian[0];
          const Tensor<2, dim, VectorizedArrayType> &jac       = this->jacobian[1];
          const VectorizedArrayType                  inv_det =
            (dim == 2) ? this->jacobian[0][0][0] * this->jacobian[0][1][1] :
                                          this->jacobian[0][0][0] * this->jacobian[0][1][1] * this->jacobian[0][2][2];
          ...
          for (unsigned int d = 0; d < dim; ++d)
            for (unsigned int comp = 0; comp < n_components; ++comp)
              grad_out[comp][d] =
                this->gradients_quad[(comp * dim + d) * nqp + q_point] * inv_t_jac[d][d] * jac[comp][comp] * inv_det;
        }
      else if (this->cell_type <= internal::MatrixFreeFunctions::affine)
        {
          // Affine cell
            ...
            for (unsigned int d = 0; d < dim; ++d)
              {
                ...
                for (unsigned int f = 0; f < dim; ++f)
                  for (unsigned int e = 0; e < dim; ++e)
                    tmp +=
                      jac[comp][f] * inv_t_jac[d][e] * inv_det * this->gradients_quad[(f * dim + e) * nqp + q_point];
      ...
      else
        {
          ...
          Assert(this->jacobian_gradients_non_inverse != nullptr,
                 internal::ExcMatrixFreeAccessToUninitializedMappingField("update_hessians"));

          const auto                                &jac_grad  = this->jacobian_gradients_non_inverse[q_point];
          const Tensor<2, dim, VectorizedArrayType> &inv_t_jac = this->jacobian[q_point];
            ...
            for (unsigned int d = 0; d < dim; ++d)
              {
                ...
                for (unsigned int f = 0; f < dim; ++f)
                  for (unsigned int e = 0; e < dim; ++e)
                    tmp += t_jac[f][comp] * inv_t_jac[d][e] * this->gradients_quad[(f * dim + e) * nqp + q_point];
          ...
          // Contribution from values
          {
            ...
            for (unsigned int i = 0; i < dim; ++i)
              for (unsigned int j = 0; j < dim; ++j)
                {
                  tmp = jac_grad[0][i] * inv_t_jac[j][0] * this->values_quad[q_point];
                  for (unsigned int f = 1; f < dim; ++f)
                    tmp += jac_grad[f][i] * inv_t_jac[j][f] * this->values_quad[f * nqp + q_point];
                  ...
                  for (unsigned int f = 0; f < dim; ++f)
                    for (unsigned int n = 0; n < dim; ++n)
                      for (unsigned int m = 0; m < dim; ++m)
                        tmp += inv_t_jac[m][f] * jac_grad[f][m] * inv_t_jac[j][f] * t_jac[n][i] *
              ...
              for (unsigned int j = 0; j < dim; ++j)
                {
                  ...
                  for (unsigned int r = 0, f = dim; r < dim; ++r)
                    for (unsigned int k = r + 1; k < dim; ++k, ++f)
                      {
                        tmp += jac_grad[f][i] * (inv_t_jac[j][k] * this->values_quad[r * nqp + q_point] +
                                                 inv_t_jac[j][r] * this->values_quad[k * nqp + q_point]);
                        for (unsigned int n = 0; n < dim; ++n)
                          for (unsigned int m = 0; m < dim; ++m)
                            tmp -= jac_grad[f][m] * t_jac[n][i] * this->values_quad[n * nqp + q_point] *
                                   (inv_t_jac[m][k] * inv_t_jac[j][r] + inv_t_jac[m][r] * inv_t_jac[j][k]);
                      }
                  grad_out[i][j] += tmp * inv_det;
                }
          }
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE VectorizedArrayType
FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::get_divergence(const unsigned int q_point) const
{
#  ifdef DEBUG
  Assert(this->gradients_quad_initialized == true, internal::ExcAccessToUninitializedField());
#  endif
      ...
      if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
        {
          ...
          for (unsigned int d = 1; d < dim; ++d)
            divergence += this->gradients_quad[(dim * d + d) * nqp + q_point] * inv_det;
        }
      else
        {
          // General cell
          // Derivatives are reordered for faces. Need to take this into account
          const VectorizedArrayType inv_det =
            determinant(this->jacobian[this->cell_type > internal::MatrixFreeFunctions::affine ? q_point : 0]) *
      ...
      if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
        {
          ...
          for (unsigned int d = 1; d < dim; ++d)
            divergence += this->gradients_quad[(dim * d + d) * nqp + q_point] * this->jacobian[0][d][d];
        }
      else
        {
          // cell with general/constant Jacobian
          const Tensor<2, dim, VectorizedArrayType> &jac =
            this->cell_type == internal::MatrixFreeFunctions::general ? this->jacobian[q_point] : this->jacobian[0];
          divergence = jac[0][0] * this->gradients_quad[q_point];
          for (unsigned int e = 1; e < dim; ++e)
            divergence += jac[0][e] * this->gradients_quad[e * nqp + q_point];
          for (unsigned int d = 1; d < dim; ++d)
            for (unsigned int e = 0; e < dim; ++e)
              divergence += jac[d][e] * this->gradients_quad[(d * dim + e) * nqp + q_point];
        }
    }
  return divergence;
}
...
inline DEAL_II_ALWAYS_INLINE SymmetricTensor<2, dim, VectorizedArrayType>
                             FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::get_symmetric_gradient(
  const unsigned int q_point) const
{
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE Tensor<1, (dim == 2 ? 1 : dim), VectorizedArrayType>
FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::get_curl(const unsigned int q_point) const
{
  // copy from generic function into dim-specialization function
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE Tensor<2, dim, VectorizedArrayType>
                             FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::get_hessian_diagonal(
  const unsigned int q_point) const
{
...
FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::get_hessian(const unsigned int q_point) const
{
#  ifdef DEBUG
  Assert(this->hessians_quad_initialized == true, internal::ExcAccessToUninitializedField());
#  endif
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::submit_value(
  const Tensor<1, dim, VectorizedArrayType> val_in,
  const unsigned int                        q_point)
{
  if (this->data->element_type == internal::MatrixFreeFunctions::ElementType::tensor_raviart_thomas)
    {
      ...
      if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
        {
          ...
          for (unsigned int comp = 0; comp < n_components; ++comp)
            this->values_quad[comp * nqp + q_point] = val_in[comp] * weight * jac[comp][comp];
        }
      else
        {
          // Affine or general cell
          ...
          const VectorizedArrayType fac =
            (!is_face) ? this->quadrature_weights[q_point] :
                         (((this->cell_type > internal::MatrixFreeFunctions::affine) ?
                             this->J_value[q_point] :
                             this->J_value[0] * this->quadrature_weights[q_point]) *
                          ((dim == 2 && this->get_face_no() < 2) ? -determinant(inv_t_jac) : determinant(inv_t_jac)));

          // J^T * u * factor
          for (unsigned int comp = 0; comp < n_components; ++comp)
            {
              this->values_quad[comp * nqp + q_point] = val_in[0] * jac[0][comp] * fac;
              for (unsigned int e = 1; e < dim; ++e)
                this->values_quad[comp * nqp + q_point] += val_in[e] * jac[e][comp] * fac;
            }
        }
    }
  else
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::submit_gradient(
  const Tensor<2, dim, VectorizedArrayType> grad_in,
  const unsigned int                        q_point)
{
  if (this->data->element_type == internal::MatrixFreeFunctions::ElementType::tensor_raviart_thomas)
    {
            ...
            for (unsigned int d = 0; d < dim; ++d)
              {
                ...
                this->gradients_quad[(comp * dim + d) * nqp + q_point] = tmp * fac;
              }
        }
      else
        {
                  ...
                  for (unsigned int j = 0; j < dim; ++j)
                    {
                      ...
                      for (unsigned int m = 0; m < dim; ++m)
                        for (unsigned int k = 0; k < dim; ++k)
                          tmp -= inv_t_jac[m][k] * jac_grad[k][m] * inv_t_jac[j][k] * t_jac[f][i] * grad_in[i][j];
                    }
              ...
              for (unsigned int k = r + 1; k < dim; ++k, ++f)
                {
                  ...
                  this->values_from_gradients_quad[r * nqp + q_point] += tmp * fac;
                  ...
                  this->values_from_gradients_quad[k * nqp + q_point] += tmp * fac;
                }
            ...
            for (unsigned int n = 0; n < dim; ++n)
              {
                ...
                this->values_from_gradients_quad[n * nqp + q_point] -= tmp * fac;
              }
          }
        }
    }
  ...
  if (this->data->element_type == internal::MatrixFreeFunctions::ElementType::tensor_raviart_thomas)
    {
      ...
      FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::submit_gradient(grad, q_point);
    }
  else
    {
      BaseClass::submit_gradient(grad_in, q_point);
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::submit_divergence(const VectorizedArrayType div_in,
                                                                                      const unsigned int        q_point)
{
#  ifdef DEBUG
  Assert(this->is_reinitialized, ExcNotInitialized());
...
#  endif
  ...
  if (this->data->element_type == internal::MatrixFreeFunctions::ElementType::tensor_raviart_thomas)
      ...
      for (unsigned int d = 0; d < dim; ++d)
        {
          ...
          for (unsigned int e = d + 1; e < dim; ++e)
            {
              this->gradients_quad[(dim * d + e) * nqp + q_point] = VectorizedArrayType();
              this->gradients_quad[(dim * e + d) * nqp + q_point] = VectorizedArrayType();
            }
        }
  ...
  else
    {
      if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
        {
          const VectorizedArrayType fac = this->J_value[0] * this->quadrature_weights[q_point] * div_in;
          for (unsigned int d = 0; d < dim; ++d)
            {
              this->gradients_quad[(d * dim + d) * nqp + q_point] = (fac * this->jacobian[0][d][d]);
              for (unsigned int e = d + 1; e < dim; ++e)
                {
                  this->gradients_quad[(d * dim + e) * nqp + q_point] = VectorizedArrayType();
                  this->gradients_quad[(e * dim + d) * nqp + q_point] = VectorizedArrayType();
                }
            }
        }
      else
        {
          const Tensor<2, dim, VectorizedArrayType> jac =
            this->cell_type == internal::MatrixFreeFunctions::general ? this->jacobian[q_point] : this->jacobian[0];
          const VectorizedArrayType fac = (this->cell_type == internal::MatrixFreeFunctions::general ?
                                             this->J_value[q_point] :
                                             this->J_value[0] * this->quadrature_weights[q_point]) *
                                          div_in;
          for (unsigned int d = 0; d < dim; ++d)
            {
              for (unsigned int e = 0; e < dim; ++e)
                this->gradients_quad[(d * dim + e) * nqp + q_point] = jac[d][e] * fac;
            }
        }
    }
}
...
template <int dim, typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationAccess<dim, dim, Number, is_face, VectorizedArrayType>::submit_symmetric_gradient(
  const SymmetricTensor<2, dim, VectorizedArrayType> sym_grad,
  const unsigned int                                 q_point)
{
  AssertThrow(this->data->element_type != internal::MatrixFreeFunctions::ElementType::tensor_raviart_thomas,
              ExcNotImplemented());
  ...
  if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
    {
      ...
      for (unsigned int e = 0, counter = dim; e < dim; ++e)
        for (unsigned int d = e + 1; d < dim; ++d, ++counter)
          {
            const VectorizedArrayType value                     = sym_grad.access_raw_entry(counter) * JxW;
            this->gradients_quad[(e * dim + d) * nqp + q_point] = value * this->jacobian[0][d][d];
            this->gradients_quad[(d * dim + e) * nqp + q_point] = value * this->jacobian[0][e][e];
          }
    }
  // general/affine cell type
  else
    {
      const VectorizedArrayType                 JxW = this->cell_type == internal::MatrixFreeFunctions::general ?
                                                        this->J_value[q_point] :
                                                        this->J_value[0] * this->quadrature_weights[q_point];
      const Tensor<2, dim, VectorizedArrayType> jac =
        this->cell_type == internal::MatrixFreeFunctions::general ? this->jacobian[q_point] : this->jacobian[0];
...
template <typename Number, bool is_face, typename VectorizedArrayType>
inline FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::FEEvaluationAccess(
  ...
  const unsigned int                                active_quad_index,
  const unsigned int                                face_type)
  : FEEvaluationBase<1, 1, Number, is_face, VectorizedArrayType>(matrix_free,
                                                                 dof_no,
                                                                 first_selected_component,
                                                                 quad_no,
                                                                 fe_degree,
...
inline FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::FEEvaluationAccess(
  ...
  const unsigned int                                       first_selected_component,
  const FEEvaluationData<1, VectorizedArrayType, is_face> *other)
  : FEEvaluationBase<1, 1, Number, is_face, VectorizedArrayType>(mapping,
                                                                 fe,
                                                                 quadrature,
                                                                 update_flags,
                                                                 first_selected_component,
...
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::operator=(
  const FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType> &other)
{
  this->FEEvaluationBase<1, 1, Number, is_face, VectorizedArrayType>::operator=(other);
  return *this;
}
...
template <typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE VectorizedArrayType
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::get_dof_value(const unsigned int dof) const
{
  AssertIndexRange(dof, this->data->dofs_per_component_on_cell);
  return this->values_dofs[dof];
}
...
template <typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE VectorizedArrayType
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::get_value(const unsigned int q_point) const
{
#  ifdef DEBUG
  Assert(this->values_quad_initialized == true, internal::ExcAccessToUninitializedField());
#  endif
...
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::get_gradient(const unsigned int q_point) const
{
...
template <typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE VectorizedArrayType
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::get_divergence(const unsigned int q_point) const
{
...
template <typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE VectorizedArrayType
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::get_normal_derivative(const unsigned int q_point) const
{
  return BaseClass::get_normal_derivative(q_point)[0];
}
...
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::get_hessian(const unsigned int q_point) const
{
  return BaseClass::get_hessian(q_point)[0];
}
...
template <typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE Tensor<1, 1, VectorizedArrayType>
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::get_hessian_diagonal(const unsigned int q_point) const
{
  return BaseClass::get_hessian_diagonal(q_point)[0];
}
...
template <typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE VectorizedArrayType
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::get_laplacian(const unsigned int q_point) const
{
  return BaseClass::get_laplacian(q_point)[0];
}
...
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::submit_dof_value(const VectorizedArrayType val_in,
                                                                                 const unsigned int        dof)
{
#  ifdef DEBUG
  this->dof_values_initialized = true;
...
template <typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::submit_value(const VectorizedArrayType val_in,
                                                                             const unsigned int        q_point)
{
#  ifdef DEBUG
  Assert(this->is_reinitialized, ExcNotInitialized());
...
template <typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::submit_gradient(const VectorizedArrayType grad_in,
                                                                                const unsigned int        q_point)
{
#  ifdef DEBUG
  Assert(this->is_reinitialized, ExcNotInitialized());
...
template <typename Number, bool is_face, typename VectorizedArrayType>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::submit_normal_derivative(
  const VectorizedArrayType grad_in,
  const unsigned int        q_point)
{
  Tensor<1, 1, VectorizedArrayType> grad;
...
template <typename Number, bool is_face, typename VectorizedArrayType>
inline VectorizedArrayType
FEEvaluationAccess<1, 1, Number, is_face, VectorizedArrayType>::integrate_value() const
{
  return BaseClass::integrate_value()[0];
}
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
inline FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::FEEvaluation(
  const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
...
inline FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::FEEvaluation(
  ...
  const unsigned int                                  quad_no,
  const unsigned int                                  first_selected_component)
  : FEEvaluation(matrix_free, dof_no, quad_no, first_selected_component, matrix_free.get_cell_active_fe_index(range))
{}
...
inline FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::FEEvaluation(
  ...
  const UpdateFlags         update_flags,
  const unsigned int        first_selected_component)
  : BaseClass(mapping, fe, quadrature, update_flags, first_selected_component, nullptr)
  , dofs_per_component(this->data->dofs_per_component_on_cell)
  , dofs_per_cell(this->data->dofs_per_component_on_cell * n_components_)
  , n_q_points(this->data->n_q_points)
{
...
inline FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::FEEvaluation(
  ...
  const UpdateFlags         update_flags,
  const unsigned int        first_selected_component)
  : BaseClass(StaticMappingQ1<dim>::mapping, fe, quadrature, update_flags, first_selected_component, nullptr)
  , dofs_per_component(this->data->dofs_per_component_on_cell)
  , dofs_per_cell(this->data->dofs_per_component_on_cell * n_components_)
  , n_q_points(this->data->n_q_points)
{
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
inline FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType> &
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::operator=(
  const FEEvaluation &other)
{
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
inline void
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::check_template_arguments(
  const unsigned int dof_no,
  const unsigned int first_selected_component)
{
  (void)dof_no;
  ...
  Assert(this->data->dofs_per_component_on_cell > 0,
         ExcMessage("There is nothing useful you can do with an FEEvaluation object with "
...
#  ifdef DEBUG
  ...
  if ((static_cast<unsigned int>(fe_degree) != numbers::invalid_unsigned_int &&
       static_cast<unsigned int>(fe_degree) != this->data->data.front().fe_degree) ||
      n_q_points != this->n_quadrature_points)
    {
      ...
      // points
      unsigned int proposed_dof_comp = numbers::invalid_unsigned_int, proposed_fe_comp = numbers::invalid_unsigned_int,
                   proposed_quad_comp = numbers::invalid_unsigned_int;
      if (dof_no != numbers::invalid_unsigned_int)
        {
          if (static_cast<unsigned int>(fe_degree) == this->data->data.front().fe_degree)
            {
              proposed_dof_comp = dof_no;
              proposed_fe_comp  = first_selected_component;
            }
          else
            for (unsigned int no = 0; no < this->matrix_free->n_components(); ++no)
              for (unsigned int nf = 0; nf < this->matrix_free->n_base_elements(no); ++nf)
                if (this->matrix_free->get_shape_info(no, 0, nf, this->active_fe_index, 0).data.front().fe_degree ==
          ...
          if (n_q_points == this->mapping_data->descriptor[this->active_quad_index].n_q_points)
            ...
            for (unsigned int no = 0; no < this->matrix_free->get_mapping_info().cell_data.size(); ++no)
              if (this->matrix_free->get_mapping_info().cell_data[no].descriptor[this->active_quad_index].n_q_points ==
                  n_q_points)
                {
      ...
      if (proposed_dof_comp != numbers::invalid_unsigned_int && proposed_quad_comp != numbers::invalid_unsigned_int)
        {
      ...
      message += "    Did you mean FEEvaluation<dim,";
      message += Utilities::int_to_string(this->data->data.front().fe_degree) + ",";
      ...
      message += ")?\n";
      std::string correct_pos;
      if (this->data->data.front().fe_degree != static_cast<unsigned int>(fe_degree))
        correct_pos = " ^";
      else
        correct_pos = "  ";
      ...
      Assert(static_cast<unsigned int>(fe_degree) == this->data->data.front().fe_degree &&
               n_q_points == this->n_quadrature_points,
             ExcMessage(message));
    }
  if (dof_no != numbers::invalid_unsigned_int)
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
inline void
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::reinit(
...
inline void
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::reinit(
  const std::array<unsigned int, n_lanes> &cell_ids)
{
  Assert(this->dof_info != nullptr, ExcNotInitialized());
  Assert(this->mapping_data != nullptr, ExcNotInitialized());
  ...
  for (unsigned int v = 0; v < n_lanes; ++v)
    {
      ...
      this->cell_type =
        std::max(this->cell_type, this->matrix_free->get_mapping_info().get_cell_type(cell_index / n_lanes));
    }
  ...
  auto &this_jacobian_gradients_non_inverse_data = mapping_storage.jacobian_gradients_non_inverse[0];
  ...
  if (this->cell_type <= internal::MatrixFreeFunctions::GeometryType::affine)
  ...
  else
    {
      ...
      if (this->mapping_data->jacobian_gradients_non_inverse[0].size() > 0)
        this_jacobian_gradients_non_inverse_data.resize_fast(this->n_quadrature_points);
  ...
  this->jacobian_gradients_non_inverse = this_jacobian_gradients_non_inverse_data.data();
      ...
      if (this->cell_type <= internal::MatrixFreeFunctions::GeometryType::affine)
        {
          ...
          if (this->mapping_data->JxW_values.size() > 0)
            this_J_value_data[q][v] = this->mapping_data->JxW_values[offsets + q][lane];

          if (this->mapping_data->jacobians[0].size() > 0)
            for (unsigned int q = 0; q < 2; ++q)
              for (unsigned int i = 0; i < dim; ++i)
                for (unsigned int j = 0; j < dim; ++j)
                  this_jacobian_data[q][i][j][v] = this->mapping_data->jacobians[0][offsets + q][i][j][lane];

          if (this->mapping_data->jacobian_gradients[0].size() > 0)
            for (unsigned int i = 0; i < dim * (dim + 1) / 2; ++i)
              for (unsigned int j = 0; j < dim; ++j)
                this_jacobian_gradients_data[q][i][j][v] =
                  this->mapping_data->jacobian_gradients[0][offsets + q][i][j][lane];

          if (this->mapping_data->jacobian_gradients_non_inverse[0].size() > 0)
            for (unsigned int i = 0; i < dim * (dim + 1) / 2; ++i)
              for (unsigned int j = 0; j < dim; ++j)
                this_jacobian_gradients_non_inverse_data[q][i][j][v] =
                  this->mapping_data->jacobian_gradients_non_inverse[0][offsets + q][i][j][lane];

          if (this->mapping_data->quadrature_points.size() > 0)
            for (unsigned int i = 0; i < dim; ++i)
              this_quadrature_points_data[q][i][v] =
                this->mapping_data
                  ->quadrature_points[this->mapping_data->quadrature_point_offsets[cell_batch_index] + q][i][lane];
        }
      else
        {
          // general case that at least one cell is not Cartesian or affine
          const auto cell_type = this->matrix_free->get_mapping_info().get_cell_type(cell_batch_index);

          for (unsigned int q = 0; q < this->n_quadrature_points; ++q)
            {
              const unsigned int q_src = (cell_type <= internal::MatrixFreeFunctions::GeometryType::affine) ? 0 : q;

              if (this->mapping_data->JxW_values.size() > 0)
                this_J_value_data[q][v] = this->mapping_data->JxW_values[offsets + q_src][lane];

              if (this->mapping_data->jacobians[0].size() > 0)
                for (unsigned int i = 0; i < dim; ++i)
                  for (unsigned int j = 0; j < dim; ++j)
                    this_jacobian_data[q][i][j][v] = this->mapping_data->jacobians[0][offsets + q_src][i][j][lane];

              if (this->mapping_data->jacobian_gradients[0].size() > 0)
                for (unsigned int i = 0; i < dim * (dim + 1) / 2; ++i)
                  for (unsigned int j = 0; j < dim; ++j)
                    this_jacobian_gradients_data[q][i][j][v] =
                      this->mapping_data->jacobian_gradients[0][offsets + q_src][i][j][lane];

              if (this->mapping_data->jacobian_gradients_non_inverse[0].size() > 0)
                for (unsigned int i = 0; i < dim * (dim + 1) / 2; ++i)
                  for (unsigned int j = 0; j < dim; ++j)
                    this_jacobian_gradients_non_inverse_data[q][i][j][v] =
                      this->mapping_data->jacobian_gradients_non_inverse[0][offsets + q_src][i][j][lane];

              if (this->mapping_data->quadrature_points.size() > 0)
                {
                  if (cell_type <= internal::MatrixFreeFunctions::GeometryType::affine)
                    {
                      ...
                      Point<dim, VectorizedArrayType> point =
                        this->mapping_data
                          ->quadrature_points[this->mapping_data->quadrature_point_offsets[cell_batch_index] + 0];

                      const Tensor<2, dim, VectorizedArrayType> &jac = this->mapping_data->jacobians[0][offsets + 1];
                      if (cell_type == internal::MatrixFreeFunctions::cartesian)
                        for (unsigned int d = 0; d < dim; ++d)
                          point[d] += jac[d][d] * static_cast<Number>(this->descriptor->quadrature.point(q)[d]);
                      else
                        for (unsigned int d = 0; d < dim; ++d)
                          for (unsigned int e = 0; e < dim; ++e)
                            point[d] += jac[d][e] * static_cast<Number>(this->descriptor->quadrature.point(q)[e]);
                  ...
                  else
                    {
                      ...
                      for (unsigned int i = 0; i < dim; ++i)
                        this_quadrature_points_data[q][i][v] =
                          this->mapping_data
                            ->quadrature_points[this->mapping_data->quadrature_point_offsets[cell_batch_index] + q][i]
                                               [lane];
                    }
                }
            }
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
inline void
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::evaluate(
  ...
  const EvaluationFlags::EvaluationFlags evaluation_flag)
{
  const bool hessians_on_general_cells =
    evaluation_flag & EvaluationFlags::hessians && (this->cell_type > internal::MatrixFreeFunctions::affine);
  ...
  if (this->data->element_type == internal::MatrixFreeFunctions::ElementType::tensor_raviart_thomas &&
      evaluation_flag & EvaluationFlags::gradients && (this->cell_type > internal::MatrixFreeFunctions::affine))
    evaluation_flag_actual |= EvaluationFlags::values;

  if (fe_degree > -1)
    {
      SelectEvaluator<dim, fe_degree, n_q_points_1d, VectorizedArrayType>::evaluate(n_components,
    ...
    {
      internal::FEEvaluationFactory<dim, VectorizedArrayType>::evaluate(n_components,
                                                                        evaluation_flag_actual,
...
namespace internal
{
  ...
  template <
    ...
    std::enable_if_t<internal::has_begin<VectorType> && !IsBlockVector<VectorType>::value, VectorType> * = nullptr>
  VectorizedArrayType *
  check_vector_access_inplace(const EvaluatorType &fe_eval, VectorType &vector)
  {
    ...
    if (std::is_same_v<typename VectorType::value_type, Number> &&
        dof_info.index_storage_variants[internal::MatrixFreeFunctions::DoFInfo::dof_access_cell][cell] ==
          internal::MatrixFreeFunctions::DoFInfo::IndexStorageVariants::interleaved_contiguous &&
        reinterpret_cast<std::size_t>(
          vector.begin() + dof_info.dof_indices_contiguous[internal::MatrixFreeFunctions::DoFInfo::dof_access_cell]
          ...
          0)
      {
        return reinterpret_cast<VectorizedArrayType *>(
          vector.begin() +
          dof_info.dof_indices_contiguous[internal::MatrixFreeFunctions::DoFInfo::dof_access_cell]
                                         [cell * VectorizedArrayType::size()] +
          dof_info.component_dof_indices_offset[fe_eval.get_active_fe_index()][fe_eval.get_first_selected_component()] *
...
template <typename VectorType>
inline void
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::gather_evaluate(
  const VectorType                      &input_vector,
  const EvaluationFlags::EvaluationFlags evaluation_flag)
{
  const VectorizedArrayType *src_ptr =
    internal::check_vector_access_inplace<Number, const VectorizedArrayType>(*this, input_vector);
  if (src_ptr != nullptr)
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
inline void
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::integrate(
...
#  ifdef DEBUG
  ...
  if ((integration_flag & EvaluationFlags::hessians) != 0u)
    Assert(this->hessians_quad_submitted == true, internal::ExcAccessToUninitializedField());
#  endif
  Assert(this->matrix_free != nullptr || this->mapped_geometry->is_initialized(), ExcNotInitialized());

  Assert((integration_flag & ~(EvaluationFlags::values | EvaluationFlags::gradients | EvaluationFlags::hessians)) == 0,
  ...
  if (integration_flag & EvaluationFlags::hessians && (this->cell_type > internal::MatrixFreeFunctions::affine))
    {
  ...
  if (this->data->element_type == internal::MatrixFreeFunctions::ElementType::tensor_raviart_thomas &&
      integration_flag & EvaluationFlags::gradients && this->cell_type > internal::MatrixFreeFunctions::affine &&
  ...
  if (fe_degree > -1)
    {
      SelectEvaluator<dim, fe_degree, n_q_points_1d, VectorizedArrayType>::integrate(
        n_components, integration_flag_actual, values_array, *this, sum_into_values_array);
    }
  else
    {
...
template <typename VectorType>
inline void
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::integrate_scatter(
  const EvaluationFlags::EvaluationFlags integration_flag,
  VectorType                            &destination)
{
  VectorizedArrayType *dst_ptr = internal::check_vector_access_inplace<Number, VectorizedArrayType>(*this, destination);
  if (dst_ptr != nullptr)
...
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::dof_indices() const
{
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
inline FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::FEFaceEvaluation(
  const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
  const bool                                          is_interior_face,
  const unsigned int                                  dof_no,
  const unsigned int                                  quad_no,
...
inline FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::FEFaceEvaluation(
  ...
  : FEFaceEvaluation(matrix_free,
                     ...
                     quad_no,
                     first_selected_component,
                     matrix_free.get_face_active_fe_index(range, is_interior_face),
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
inline void
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::reinit(
  const unsigned int face_index)
{
  ...
  this->dof_access_index = this->is_interior_face() ? internal::MatrixFreeFunctions::DoFInfo::dof_access_face_interior :
                                                      internal::MatrixFreeFunctions::DoFInfo::dof_access_face_exterior;
  Assert(this->mapping_data != nullptr, ExcNotInitialized());

  if (face_index >= this->matrix_free->get_task_info().face_partition_data.back() &&
      face_index < this->matrix_free->get_task_info().boundary_partition_data.back())
    Assert(this->is_interior_face(),
           ExcMessage("Boundary faces do not have a neighbor. When looping over "
  ...
  this->jacobian_gradients   = this->mapping_data->jacobian_gradients[!this->is_interior_face()].data() + offsets;
  this->jacobian_gradients_non_inverse =
  ...
  if (this->mapping_data->quadrature_point_offsets.empty() == false)
    {
      ...
      this->quadrature_points =
        this->mapping_data->quadrature_points.data() + this->mapping_data->quadrature_point_offsets[this->cell];
    }
  ...
  const unsigned int face_number)
{
  ...
  AssertIndexRange(cell_index, this->matrix_free->get_mapping_info().cell_type.size());
  Assert(this->mapped_geometry == nullptr,
         ExcMessage("FEEvaluation was initialized without a matrix-free object."
                    " Integer indexing is not possible"));
  if (this->mapped_geometry != nullptr)
  ...
  this->cell_type        = this->matrix_free->get_mapping_info().faces_by_cells_type[cell_index][face_number];
  this->cell             = cell_index;
  ...
  if (this->is_interior_face() == false)
      ...
      for (unsigned int i = 0; i < n_lanes; ++i)
        {
          ...
          // compute face ID
          unsigned int face_index = this->matrix_free->get_cell_and_face_to_plain_faces()(cell_index, face_number, i);
          ...
          if (face_identifies_as_interior != orientation_interior_face)
            {
              constexpr std::array<std::uint8_t, 8> table{{0, 1, 2, 3, 6, 5, 4, 7}};
  ...
  else
    {
      ...
      for (unsigned int i = 0; i < n_lanes; ++i)
        this->cell_ids[i] = cell_index * n_lanes + i;
      for (unsigned int i = 0; i < n_lanes; ++i)
        this->face_ids[i] = this->matrix_free->get_cell_and_face_to_plain_faces()(cell_index, face_number, i);
    }

  const unsigned int offsets = this->matrix_free->get_mapping_info()
                                 .face_data_by_cells[this->quad_no]
                                 .data_index_offsets[cell_index * GeometryInfo<dim>::faces_per_cell + face_number];
  AssertIndexRange(offsets, this->matrix_free->get_mapping_info().face_data_by_cells[this->quad_no].JxW_values.size());
  this->J_value = &this->matrix_free->get_mapping_info().face_data_by_cells[this->quad_no].JxW_values[offsets];
  this->normal_vectors =
    &this->matrix_free->get_mapping_info().face_data_by_cells[this->quad_no].normal_vectors[offsets];
  this->jacobian = &this->matrix_free->get_mapping_info()
                      .face_data_by_cells[this->quad_no]
                      .jacobians[!this->is_interior_face()][offsets];
  this->normal_x_jacobian = &this->matrix_free->get_mapping_info()
                               .face_data_by_cells[this->quad_no]
  ...
  if (this->matrix_free->get_mapping_info().face_data_by_cells[this->quad_no].quadrature_point_offsets.empty() == false)
    {
      const unsigned int index = this->cell * GeometryInfo<dim>::faces_per_cell + this->face_numbers[0];
      AssertIndexRange(
        index, this->matrix_free->get_mapping_info().face_data_by_cells[this->quad_no].quadrature_point_offsets.size());
      this->quadrature_points =
        this->matrix_free->get_mapping_info().face_data_by_cells[this->quad_no].quadrature_points.data() +
        this->matrix_free->get_mapping_info().face_data_by_cells[this->quad_no].quadrature_point_offsets[index];
    }
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
inline void
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::evaluate(
  const EvaluationFlags::EvaluationFlags evaluation_flag)
{
#  ifdef DEBUG
...
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::evaluate(
  ...
  const EvaluationFlags::EvaluationFlags evaluation_flag)
{
  Assert((evaluation_flag & ~(EvaluationFlags::values | EvaluationFlags::gradients | EvaluationFlags::hessians)) == 0,
  ...
  if (fe_degree > -1)
    internal::FEFaceEvaluationImplEvaluateSelector<dim, VectorizedArrayType>::template run<fe_degree, n_q_points_1d>(
      n_components, evaluation_flag_actual, values_array, *this);
  else
    internal::FEFaceEvaluationFactory<dim, VectorizedArrayType>::evaluate(n_components,
                                                                          evaluation_flag_actual,
                                                                          values_array,
                                                                          *this);
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
inline void
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::integrate(
  const EvaluationFlags::EvaluationFlags integration_flag)
{
  integrate(integration_flag, this->values_dofs);
  ...
  VectorizedArrayType                   *values_array)
{
  ...
  if (fe_degree > -1)
    internal::FEFaceEvaluationImplIntegrateSelector<dim, VectorizedArrayType>::template run<fe_degree, n_q_points_1d>(
      n_components, integration_flag_actual, values_array, *this);
  else
    internal::FEFaceEvaluationFactory<dim, VectorizedArrayType>::integrate(n_components,
...
template <typename VectorType>
inline void
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::gather_evaluate(
  const VectorType                      &input_vector,
  const EvaluationFlags::EvaluationFlags evaluation_flag)
{
  Assert((evaluation_flag & ~(EvaluationFlags::values | EvaluationFlags::gradients | EvaluationFlags::hessians)) == 0,
  ...
  if (this->data->data.front().fe_degree > 0 &&
      fast_evaluation_supported(this->data->data.front().fe_degree, this->data->data.front().n_q_points_1d) &&
      internal::FEFaceEvaluationImplGatherEvaluateSelector<dim, typename VectorType::value_type, VectorizedArrayType>::
        supports(evaluation_flag,
                 *this->data,
                 internal::get_beginning<typename VectorType::value_type>(input_vector),
                 this->dof_info->index_storage_variants[this->dof_access_index][this->cell]))
    {
      if (fe_degree > -1)
        {
          internal::FEFaceEvaluationImplGatherEvaluateSelector<dim,
                                                               typename VectorType::value_type,
        ...
        {
          internal::FEFaceEvaluationGatherFactory<dim, typename VectorType::value_type, VectorizedArrayType>::evaluate(
            n_components,
            evaluation_flag,
            internal::get_beginning<typename VectorType::value_type>(input_vector),
            shared_vector_data,
...
template <typename VectorType>
inline void
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::integrate_scatter(
...
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::integrate_scatter(
  ...
  VectorType                            &destination)
{
  Assert((this->dof_access_index == internal::MatrixFreeFunctions::DoFInfo::dof_access_cell &&
          this->is_interior_face() == false) == false,
         ExcNotImplemented());

  const auto shared_vector_data =
    internal::get_shared_vector_data(&destination,
                                     this->dof_access_index == internal::MatrixFreeFunctions::DoFInfo::dof_access_cell,
                                     this->active_fe_index,
                                     this->dof_info);
  ...
  if (this->data->data.front().fe_degree > 0 &&
      ...
      internal::FEFaceEvaluationImplGatherEvaluateSelector<dim, typename VectorType::value_type, VectorizedArrayType>::
        supports(integration_flag,
                 *this->data,
                 internal::get_beginning<typename VectorType::value_type>(destination),
                 this->dof_info->index_storage_variants[this->dof_access_index][this->cell]))
    {
      if (fe_degree > -1)
        {
          internal::FEFaceEvaluationImplIntegrateScatterSelector<dim,
                                                                 typename VectorType::value_type,
                                                                 VectorizedArrayType>::template run<fe_degree,
                                                                                                    n_q_points_1d>(
            n_components,
        ...
        {
          internal::FEFaceEvaluationGatherFactory<dim, typename VectorType::value_type, VectorizedArrayType>::integrate(
            n_components,
            integration_flag,
            internal::get_beginning<typename VectorType::value_type>(destination),
            shared_vector_data,
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
inline std_cxx20::ranges::iota_view<unsigned int, unsigned int>
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::dof_indices() const
{
  return {0U, dofs_per_cell};
}
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
bool
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::fast_evaluation_supported(
  const unsigned int given_degree,
  const unsigned int given_n_q_points_1d)
{
  return fe_degree == -1 ?
           internal::FEEvaluationFactory<dim, VectorizedArrayType>::fast_evaluation_supported(given_degree,
                                                                                              given_n_q_points_1d) :
           true;
}
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
bool
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::fast_evaluation_supported(
  const unsigned int given_degree,
  const unsigned int given_n_q_points_1d)
{
  return fe_degree == -1 ?
           internal::FEFaceEvaluationFactory<dim, VectorizedArrayType>::fast_evaluation_supported(given_degree,
                                                                                                  given_n_q_points_1d) :
           true;
}
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
bool
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::at_boundary() const
{
  Assert(this->dof_access_index != internal::MatrixFreeFunctions::DoFInfo::dof_access_cell, ExcNotImplemented());

  if (this->is_interior_face() == false)
    return false;
  else if (this->cell < this->matrix_free->n_inner_face_batches())
    return false;
  else if (this->cell < (this->matrix_free->n_inner_face_batches() + this->matrix_free->n_boundary_face_batches()))
    return true;
  else
    return false;
}
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number, typename VectorizedArrayType>
types::boundary_id
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number, VectorizedArrayType>::boundary_id() const
{
  Assert(this->dof_access_index != internal::MatrixFreeFunctions::DoFInfo::dof_access_cell, ExcNotImplemented());

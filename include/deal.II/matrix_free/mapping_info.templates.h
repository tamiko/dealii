  ...
  namespace MatrixFreeFunctions
  {
      ...
      const bool                                                 piola_transform)
    {
      ...
      this->update_flags_cells =
        MappingInfoStorage<dim, dim, VectorizedArrayType>::compute_update_flags(update_flags_cells,
                                                                                quad,
                                                                                piola_transform);
      ...
      this->update_flags_boundary_faces =
        ((update_flags_inner_faces | update_flags_boundary_faces) & update_quadrature_points ?
           ...
           update_default) |
        ((((update_flags_inner_faces | update_flags_boundary_faces) & (update_jacobian_grads | update_hessians)) !=
            0u ||
          (piola_transform && ((update_flags_inner_faces | update_flags_boundary_faces) &
          ...
          for (unsigned int hpq = 0; hpq < n_hp_quads; ++hpq)
            {
              ...
              if (flag)
                for (unsigned int i = 1; i < dim; ++i)
                  flag &= quad[my_q][hpq].get_tensor_basis()[0] == quad[my_q][hpq].get_tensor_basis()[i];

              if (flag == false)
                {
                  cell_data[my_q].descriptor[hpq].initialize(quad[my_q][hpq]);
                  const auto quad_face = get_unique_face_quadratures(quad[my_q][hpq]);

                  if (quad_face.first.size() > 0) // line, quad
                    {
                      face_data[my_q].descriptor[hpq * scale].initialize(quad_face.first);
                      face_data_by_cells[my_q].descriptor[hpq * scale].initialize(quad_face.first);
                    }

                  if (quad_face.second.size() > 0) // triangle
                    {
                      AssertDimension(dim, 3);
                      face_data[my_q].descriptor[hpq * scale + (is_mixed_mesh ? 1 : 0)].initialize(quad_face.second);
                      face_data_by_cells[my_q].descriptor[hpq * scale + (is_mixed_mesh ? 1 : 0)].initialize(
                        quad_face.second);
                    }

                  const auto face_quadrature_collection = get_face_quadrature_collection(quad[my_q][hpq]);

                  face_data[my_q].q_collection[hpq] = face_quadrature_collection.second;

                  reference_cell_types[my_q][hpq] = face_quadrature_collection.first;
                }
              else
                {
                  cell_data[my_q].descriptor[hpq].initialize(quad[my_q][hpq].get_tensor_basis()[0]);

                  const auto quad_face = quad[my_q][hpq].get_tensor_basis()[0];
                  face_data[my_q].descriptor[hpq * scale].initialize(quad_face);
                  face_data[my_q].q_collection[hpq] = dealii::hp::QCollection<dim - 1>(quad_face);
                  face_data_by_cells[my_q].descriptor[hpq * scale].initialize(quad_face);
                  reference_cell_types[my_q][hpq] = ReferenceCells::get_hypercube<dim>();
                }
            }
        }
      ...
      if (active_fe_index.empty() && !cells.empty() && mapping->size() == 1 &&
      ...
      else
        {
          ...
          initialize_faces(tria, cells, face_info.faces, active_fe_index, *mapping);
    ...
    MappingInfo<dim, Number, VectorizedArrayType>::update_mapping(
      ...
      const std::shared_ptr<dealii::hp::MappingCollection<dim>> &mapping)
    {
      AssertDimension(cells.size() / VectorizedArrayType::size(), cell_type.size());
    ...
    store_vectorized_array(const VectorizedArrayType1 value, const unsigned int offset, VectorizedArrayType2 &result)
    {
      static_assert(VectorizedArrayType2::size() >= VectorizedArrayType1::size(),
      ...
      for (unsigned int d = 0; d < dim; ++d)
        for (unsigned int e = 0; e < dim; ++e)
          for (unsigned int f = 0; f < dim; ++f)
            {
              inv_jac_grad[f][e][d] = (inv_jac_permut[f][0] * jac_grad[d][e][0]);
              for (unsigned int g = 1; g < dim; ++g)
                inv_jac_grad[f][e][d] += (inv_jac_permut[f][g] * jac_grad[d][e][g]);
            }
    ...
    namespace ExtractCellHelper
    {
      ...
      struct CompressedCellData
      {
        CompressedCellData(const double expected_size)
          : data(FloatingPointComparator<VectorizedArrayType>(expected_size * std::numeric_limits<double>::epsilon() *
                                                              1024.))
        {}
      ...
      template <int dim, typename Number, typename VectorizedArrayType>
      LocalData<dim, Number, VectorizedArrayType>::LocalData(const double jac_size_in)
      ...
      template <int dim, typename Number, typename VectorizedArrayType>
      void
      LocalData<dim, Number, VectorizedArrayType>::resize(const unsigned int size)
      {
      ...
      evaluate_on_cell(const dealii::Triangulation<dim>                                              &tria,
                       ...
                       LocalData<dim, typename VectorizedArrayType::value_type, VectorizedArrayType> &cell_data)
      {
        ...
        const double zero_tolerance_double = cell_data.jac_size * std::numeric_limits<double>::epsilon() * 1024.;
        for (unsigned int j = 0; j < VectorizedArrayType::size(); ++j)
          {
            typename dealii::Triangulation<dim>::cell_iterator cell_it(&tria, cells[j].first, cells[j].second);
                ...
                if (cell_t[j] == general)
                  {
                    ...
                    if (cell_cartesian == false && n_q_points == 1 && update_flags & update_jacobian_grads)
                      {
            ...
            for (unsigned int q = 0; q < n_q_points; ++q)
              {
                ...
                for (unsigned int d = 0; d < dim; ++d)
                  for (unsigned int e = 0; e < dim; ++e)
                    cell_data.general_jac[q][d][e][j] = std::fabs(jac[d][e]) < zero_tolerance_double ? 0. : jac[d][e];
      ...
      initialize_cell_range(const std::pair<unsigned int, unsigned int>                      cell_range,
                            ...
                            std::pair<std::vector<MappingInfoStorage<dim, dim, VectorizedArrayType>>,
                                      CompressedCellData<dim, Number, VectorizedArrayType>> &data)
      {
        ...
        // FEValues
        std::vector<std::vector<std::shared_ptr<dealii::FEValues<dim>>>> fe_values(mapping_info.cell_data.size());

        const unsigned int max_active_fe_index =
          active_fe_index.size() > 0 ? *std::max_element(active_fe_index.begin(), active_fe_index.end()) : 0;
        ...
        const unsigned int end_cell = std::min(mapping_info.cell_type.size(), std::size_t(cell_range.second));
          ...
          for (unsigned int my_q = 0; my_q < mapping_info.cell_data.size(); ++my_q)
            {
              ...
              const unsigned int hp_quad_index    = mapping_info.cell_data[my_q].descriptor.size() == 1 ? 0 : fe_index;
              const unsigned int hp_mapping_index = mapping.size() == 1 ? 0 : fe_index;
              const unsigned int n_q_points       = mapping_info.cell_data[my_q].descriptor[hp_quad_index].n_q_points;
              if (fe_values[my_q][fe_index].get() == nullptr)
                fe_values[my_q][fe_index] = std::make_shared<dealii::FEValues<dim>>(
                  mapping[hp_mapping_index],
                  dummy_fe,
                  mapping_info.cell_data[my_q].descriptor[hp_quad_index].quadrature,
                  update_flags_feval);
              ...
              evaluate_on_cell(
                tria, &cells[cell * VectorizedArrayType::size()], my_q, cell_t_prev, cell_t, fe_val, cell_data);
              ...
              if (mapping_info.cell_type[cell] <= affine)
                {
                  if (my_q == 0)
                    {
                      std::pair<Tensor<2, dim, Tensor<1, VectorizedArrayType::size(), Number>>, unsigned int> new_entry;
              ...
              if (mapping_info.get_cell_type(cell) == general)
                {
                  for (unsigned int q = 0; q < n_q_points; ++q)
                    {
                      Tensor<2, dim, VectorizedArrayType> &jac           = cell_data.general_jac[q];
                      Tensor<3, dim, VectorizedArrayType> &jacobian_grad = cell_data.general_jac_grad[q];
                      for (unsigned int j = 0; j < VectorizedArrayType::size(); ++j)
                        if (cell_t[j] < general)
                          {
                            for (unsigned int d = 0; d < dim; ++d)
                      ...
                      data.first[my_q].JxW_values.push_back(determinant(jac) * fe_val.get_quadrature().weight(q));
                      Tensor<2, dim, VectorizedArrayType> inv_jac = transpose(invert(jac));
                      data.first[my_q].jacobians[0].push_back(inv_jac);

                      if (update_flags & update_jacobian_grads)
                        {
                          data.first[my_q].jacobian_gradients[0].push_back(
                            process_jacobian_gradient(inv_jac, inv_jac, jacobian_grad));
                          Tensor<1, dim *(dim + 1) / 2, Tensor<1, dim, VectorizedArrayType>> jac_grad_sym;
                          ...
                          data.first[my_q].jacobian_gradients_non_inverse[0].push_back(jac_grad_sym);
                        }
                    }
                }
              ...
              if (update_flags & update_quadrature_points)
                {
                  ...
                  if (mapping_info.get_cell_type(cell) < general)
                      ...
                      for (unsigned int v = 0; v < VectorizedArrayType::size(); ++v)
                        {
                          typename dealii::Triangulation<dim>::cell_iterator cell_it(
                            &tria,
                            cells[cell * VectorizedArrayType::size() + v].first,
                            cells[cell * VectorizedArrayType::size() + v].second);
                          const Point<dim> p =
                            mapping[hp_mapping_index].transform_unit_to_real_cell(cell_it, Point<dim>());
                          for (unsigned int d = 0; d < dim; ++d)
                            quad_point[d][v] = p[d];
                        }
                    ...
                    {
                      for (unsigned int q = 0; q < n_q_points; ++q)
                        data.first[my_q].quadrature_points.push_back(cell_data.quadrature_points[q]);
                    }
                }
      ...
      void
      merge_compressed_data(const CONTAINER &source, CONTAINER &destination, std::vector<unsigned int> &indices)
      {
      ...
      template <int structdim, int dim, typename Number, typename VectorizedArrayType>
      void
      copy_data(const unsigned int                                       first_cell,
                ...
                MappingInfoStorage<structdim, dim, VectorizedArrayType> &data_cells_local,
                MappingInfoStorage<structdim, dim, VectorizedArrayType> &data_cells)
      {
        ...
        for (unsigned int lcell = 0; lcell < data_cells_local.data_index_offsets.size(); ++lcell)
          {
            ...
            data_cells.data_index_offsets[cell] =
              cell_type[cell] <= affine ?
                (dim == structdim ? 2 : 1) * indices_compressed[data_cells_local.data_index_offsets[lcell]] :
        ...
        if (data_cells.quadrature_point_offsets.empty() == false)
          {
            Point<dim, VectorizedArrayType> *out_point = &data_cells.quadrature_points[data_shift[1]];
            for (const Point<dim, VectorizedArrayType> *point = data_cells_local.quadrature_points.begin();
        ...
        for (unsigned int i = 0; i < 2; ++i)
          {
            ...
            std::copy(data_cells_local.jacobian_gradients_non_inverse[i].begin(),
                      data_cells_local.jacobian_gradients_non_inverse[i].end(),
                      data_cells.jacobian_gradients_non_inverse[i].begin() + data_shift[0]);
      ...
      template <int dim>
      void
      mapping_q_query_fe_values(const unsigned int                                        begin_cell,
                                ...
                                AlignedVector<std::array<Tensor<2, dim>, dim + 1>>       &jacobians_on_stencil)
      {
        ...
        const unsigned int n_mapping_points = Utilities::pow(mapping_degree + 1, dim);

        FEValues<dim> fe_values(mapping_q, dummy_fe, quadrature, update_quadrature_points | update_jacobians);

        for (unsigned int cell = begin_cell; cell < end_cell; ++cell)
          {
            typename dealii::Triangulation<dim>::cell_iterator cell_it(&tria,
                                                                       cell_array[cell].first,
                                                                       cell_array[cell].second);
            fe_values.reinit(cell_it);
            for (unsigned int d = 0; d < dim; ++d)
              for (unsigned int q = 0; q < n_mapping_points; ++q)
                plain_quadrature_points[(cell * dim + d) * n_mapping_points + q] = fe_values.quadrature_point(q)[d];

            // store the first, second, n-th and n^2-th one along a
            // stencil-like pattern
            std::array<Tensor<2, dim, double>, dim + 1> &my_jacobians = jacobians_on_stencil[cell];
            my_jacobians[0]                                           = Tensor<2, dim, double>(fe_values.jacobian(0));
            for (unsigned int d = 0, skip = 1; d < dim; ++d, skip *= (mapping_degree + 1))
              my_jacobians[1 + d] = Tensor<2, dim, double>(fe_values.jacobian(skip));
            ...
            for (unsigned int q = 1; q < n_mapping_points; ++q)
              for (unsigned int d = 0; d < dim; ++d)
                for (unsigned int e = 0; e < dim; ++e)
                  if (std::abs(fe_values.jacobian(q)[d][e] - fe_values.jacobian(0)[d][e]) > 1e-12 * jacobian_size)
                    {
                      type = general;
                      goto endloop;
                    }
      ...
      mapping_q_find_compression(const double                                              jacobian_size,
                                 const AlignedVector<std::array<Tensor<2, dim>, dim + 1>> &jacobians_on_stencil,
                                 ...
                                 std::vector<GeometryType>                                &preliminary_cell_type)
      {
        ...
        // evaluation
        std::map<std::array<Tensor<2, dim>, dim + 1>, unsigned int, FloatingPointComparator<double>>
          compressed_jacobians(
            FloatingPointComparator<double>(1e4 * jacobian_size * std::numeric_limits<double>::epsilon() * 1024.));
        ...
        for (unsigned int cell = 0; cell < jacobians_on_stencil.size(); ++cell)
          {
            ...
            const auto position      = compressed_jacobians.find(jacobians_on_stencil[cell]);
            ...
            if (!add_this_cell)
              {
                ...
                const double *ptr_origin   = plain_quadrature_points.data() + position->second * dim * n_mapping_points;
                const double *ptr_mine     = plain_quadrature_points.data() + cell * dim * n_mapping_points;
                for (unsigned int d = 0; d < dim; ++d)
                  {
                    const double translate_d = ptr_origin[d * n_mapping_points] - ptr_mine[d * n_mapping_points];
                    for (unsigned int q = 1; q < n_mapping_points; ++q)
                      max_distance = std::max(std::abs(ptr_origin[d * n_mapping_points + q] -
                                                       ptr_mine[d * n_mapping_points + q] - translate_d),
                                              max_distance);
                  }
              ...
              compressed_jacobians.insert(std::make_pair(jacobians_on_stencil[cell], cell));
            ...
            if (add_this_cell)
            ...
            else
              {
                ...
                preliminary_cell_type[cell] = preliminary_cell_type[position->second];
              }
          }
       ...
       * MappingQ.
       */
      template <int dim, typename Number, typename VectorizedArrayType, typename VectorizedDouble>
      void
      mapping_q_compute_range(const unsigned int                                 begin_cell,
                              const unsigned int                                 end_cell,
                              const std::vector<GeometryType>                   &cell_type,
                              ...
                              MappingInfoStorage<dim, dim, VectorizedArrayType> &my_data)
      {
        ...
        const unsigned int     n_mapping_points = shape_info.dofs_per_component_on_cell;
              ...
              if (cell_type[cell] > affine || process_cell[cell])
                {
                  ...
                  for (unsigned int v = 0; v < n_lanes_d; ++v)
                    start_indices[v] = (cell * n_lanes + vv + v) * n_mapping_points * dim;
                  vectorized_load_and_transpose(n_mapping_points * dim,
                                                plain_quadrature_points.data(),
                                                start_indices,
                                                eval.begin_dof_values());

                  FEEvaluationFactory<dim, VectorizedDouble>::evaluate(dim,
              ...
              if (update_flags_cells & update_quadrature_points)
                {
                  Point<dim, VectorizedArrayType> *quadrature_points =
                    my_data.quadrature_points.data() + my_data.quadrature_point_offsets[cell];
                  if (cell_type[cell] <= affine)
                    for (unsigned int d = 0; d < dim; ++d)
                      for (unsigned int v = 0; v < n_lanes_d; ++v)
                        quadrature_points[0][d][vv + v] =
                          plain_quadrature_points[(dim * (cell * n_lanes + vv + v) + d) * n_mapping_points];
                  else
                    for (unsigned int d = 0; d < dim; ++d)
                      for (unsigned int q = 0; q < n_q_points; ++q)
                        store_vectorized_array(eval.begin_values()[q + d * n_q_points], vv, quadrature_points[q][d]);
                }

              const unsigned int n_points = cell_type[cell] <= affine ? 1 : n_q_points;
              if (process_cell[cell])
                for (unsigned int q = 0; q < n_points; ++q)
                  {
                    const unsigned int               idx = my_data.data_index_offsets[cell] + q;
                    Tensor<2, dim, VectorizedDouble> jac;
                    for (unsigned int d = 0; d < dim; ++d)
                      for (unsigned int e = 0; e < dim; ++e)
                        jac[d][e] = eval.begin_gradients()[q + (d * dim + e) * n_q_points];
                    ...
                    if (cell_type[cell] <= affine)
                      {
                        store_vectorized_array(jac_det, vv, my_data.JxW_values[idx]);

                        for (unsigned int d = 0; d < dim; ++d)
                          for (unsigned int e = 0; e < dim; ++e)
                            store_vectorized_array(jac[d][e], vv, my_data.jacobians[0][idx + 1][d][e]);
                      }
                    else
                      {
                        const double weight = my_data.descriptor[0].quadrature.weight(q);
                        store_vectorized_array(jac_det * weight, vv, my_data.JxW_values[idx]);
                      }
                    for (unsigned int d = 0; d < dim; ++d)
                      for (unsigned int e = 0; e < dim; ++e)
                        store_vectorized_array(inv_jac[d][e], vv, my_data.jacobians[0][idx][d][e]);

                    if (update_flags_cells & update_jacobian_grads && cell_type[cell] > affine)
                      {
                        Tensor<3, dim, VectorizedDouble> jac_grad;
                        for (unsigned int d = 0; d < dim; ++d)
                          {
                            for (unsigned int e = 0; e < dim; ++e)
                              jac_grad[d][e][e] = eval.begin_hessians()[q + (d * hess_dim + e) * n_q_points];
                            for (unsigned int c = dim, e = 0; e < dim; ++e)
                              for (unsigned int f = e + 1; f < dim; ++f, ++c)
                                jac_grad[d][e][f] = jac_grad[d][f][e] =
                                  eval.begin_hessians()[q + (d * hess_dim + c) * n_q_points];
                            const auto inv_jac_grad = process_jacobian_gradient(inv_jac, inv_jac, jac_grad);
                            for (unsigned int d = 0; d < hess_dim; ++d)
                              for (unsigned int e = 0; e < dim; ++e)
                                store_vectorized_array(inv_jac_grad[d][e],
                                                       vv,
                            ...
                            for (unsigned int d = 0; d < dim; ++d)
                              for (unsigned int f = 0; f < dim; ++f)
                                store_vectorized_array(jac_grad[f][d][d],
                                                       vv,
                                                       my_data.jacobian_gradients_non_inverse[0][idx][d][f]);

                            // then the upper-diagonal part
                            for (unsigned int d = 0, count = dim; d < dim; ++d)
                              for (unsigned int e = d + 1; e < dim; ++e, ++count)
                                for (unsigned int f = 0; f < dim; ++f)
                                  store_vectorized_array(jac_grad[f][d][e],
                                                         vv,
                                                         my_data.jacobian_gradients_non_inverse[0][idx][count][f]);
                          }
                      }
                  }
            }
      ...
      const dealii::hp::MappingCollection<dim>                 &mapping)
    {
      ...
      unsigned int work_per_chunk =
        std::max(8U, (n_cell_batches + MultithreadInfo::n_threads() - 1) / MultithreadInfo::n_threads());

      std::vector<std::pair<std::vector<MappingInfoStorage<dim, dim, VectorizedArrayType>>,
                            ExtractCellHelper::CompressedCellData<dim, Number, VectorizedArrayType>>>
        data_cells_local;
        ...
        while (cell_range.first < n_cell_batches)
          {
            data_cells_local.push_back(
              std::make_pair(std::vector<MappingInfoStorage<dim, dim, VectorizedArrayType>>(cell_data.size()),
                             ExtractCellHelper::CompressedCellData<dim, Number, VectorizedArrayType>(
                               ExtractCellHelper::get_jacobian_size(tria))));
            tasks += Threads::new_task(&ExtractCellHelper::initialize_cell_range<dim, Number, VectorizedArrayType>,
                                       cell_range,
                                       tria,
                                       cells,
                                       active_fe_index,
      ...
      for (unsigned int i = 0; i < data_cells_local.size(); ++i)
        ExtractCellHelper::merge_compressed_data(data_cells_local[i].second.data,
      ...
      for (unsigned int my_q = 0; my_q < cell_data.size(); ++my_q)
        {
          ...
          cell_data[my_q].JxW_values.resize_fast(shift.back()[0] +
          ...
          if (update_flags_cells & update_jacobian_grads)
            {
              ...
              cell_data[my_q].jacobian_gradients_non_inverse[0].resize_fast(cell_data[my_q].JxW_values.size());
            }
          if (update_flags_cells & update_quadrature_points)
            {
          ...
          Threads::TaskGroup<> tasks;
          for (unsigned int i = 0; i < data_cells_local.size(); ++i)
            tasks += Threads::new_task(&ExtractCellHelper::copy_data<dim, dim, Number, VectorizedArrayType>,
                                       work_per_chunk * i,
                                       shift[i],
                                       indices_compressed[i],
                                       cell_type,
              ...
              for (const auto &it : data_cells_local[0].second.data)
                {
                  ...
                  cell_data[my_q].jacobians[0][2 * index]     = transpose(invert(jac));
    ...
    namespace ExtractFaceHelper
    {
      ...
      struct CompressedFaceData
      {
        ...
        CompressedFaceData(const Number jacobian_size)
          : data(FloatingPointComparator<VectorizedArrayType>(512. / jacobian_size *
        ...
        // memory allocation.
        std::map<Tensor<1, 2 * dim * dim + dim + 1, Tensor<1, VectorizedArrayType::size(), Number>>,
                 unsigned int,
                 FloatingPointComparator<VectorizedArrayType>>
          data;
                                      ...
                                      const ReferenceCell reference_cell = ReferenceCells::Invalid)
      {
        ...
        if ((reference_cell == ReferenceCells::Invalid || reference_cell == ReferenceCells::get_hypercube<dim>()) ==
            false)
          {
            return index;
          }
      ...
      template <int dim, typename Number, typename VectorizedArrayType>
      void
      initialize_face_range(const std::pair<unsigned int, unsigned int>                         face_range,
                            const dealii::Triangulation<dim>                                   &tria,
                            const std::vector<std::pair<unsigned int, unsigned int>>           &cells,
                            ...
                            MappingInfo<dim, Number, VectorizedArrayType>                      &mapping_info,
                            std::pair<std::vector<MappingInfoStorage<dim - 1, dim, VectorizedArrayType>>,
                                      CompressedFaceData<dim, Number, VectorizedArrayType>>    &data)
      {
        std::vector<std::vector<std::shared_ptr<FE_Nothing<dim>>>> dummy_fe(mapping_info.reference_cell_types.size());
        for (unsigned int my_q = 0; my_q < mapping_info.reference_cell_types.size(); ++my_q)
          {
            const unsigned int n_hp_quads = mapping_info.reference_cell_types[my_q].size();
            dummy_fe[my_q].resize(n_hp_quads);

            for (unsigned int hpq = 0; hpq < n_hp_quads; ++hpq)
              dummy_fe[my_q][hpq] = std::make_shared<FE_Nothing<dim>>(mapping_info.reference_cell_types[my_q][hpq]);
          }
        ...
        std::vector<std::vector<std::shared_ptr<FEFaceValues<dim>>>> fe_boundary_face_values_container(
          mapping_info.face_data.size());
        for (unsigned int my_q = 0; my_q < mapping_info.face_data.size(); ++my_q)
          fe_boundary_face_values_container[my_q].resize(max_active_fe_index + 1);
        ...
        ExtractCellHelper::LocalData<dim, Number, VectorizedArrayType> face_data(
          ExtractCellHelper::get_jacobian_size(tria));
          ...
          for (unsigned int my_q = 0; my_q < mapping_info.face_data.size(); ++my_q)
            {
              ...
              const unsigned int fe_index =
                active_fe_index.size() > 0 ?
                  active_fe_index[faces[face].cells_interior[0] / VectorizedArrayType::size()] :
                  0;
              const unsigned int hp_quad_index    = mapping_info.cell_data[my_q].descriptor.size() == 1 ? 0 : fe_index;
              const unsigned int hp_mapping_index = mapping_in.size() == 1 ? 0 : fe_index;

              const auto &mapping    = mapping_in[hp_mapping_index];
              const auto &quadrature = mapping_info.face_data[my_q].q_collection[hp_quad_index];

              const bool is_boundary_face = faces[face].cells_exterior[0] == numbers::invalid_unsigned_int;

              if (is_boundary_face && fe_boundary_face_values_container[my_q][fe_index] == nullptr)
                fe_boundary_face_values_container[my_q][fe_index] = std::make_shared<FEFaceValues<dim>>(
                  mapping, *dummy_fe[my_q][hp_quad_index], quadrature, mapping_info.update_flags_boundary_faces);
              else if (fe_face_values_container[my_q][fe_index] == nullptr)
                fe_face_values_container[my_q][fe_index] = std::make_shared<FEFaceValues<dim>>(
                  mapping, *dummy_fe[my_q][hp_quad_index], quadrature, mapping_info.update_flags_inner_faces);
              ...
              for (unsigned int v = 0; v < VectorizedArrayType::size(); ++v)
                {
                  ...
                  if (faces[face].cells_interior[v] != numbers::invalid_unsigned_int)
                    {
                      typename dealii::Triangulation<dim>::cell_iterator cell_it(
                        &tria, cells[faces[face].cells_interior[v]].first, cells[faces[face].cells_interior[v]].second);
                      ...
                      for (unsigned int q = 0; q < n_q_points; ++q)
                        {
                          if (std::abs(fe_face_values.JxW(q) * fe_face_values.get_quadrature().weight(0) -
                              ...
                              2048. * std::numeric_limits<double>::epsilon() * fe_face_values.JxW(0) *
                            ...
                            for (unsigned int e = 0; e < dim; ++e)
                              {
                                ...
                                const unsigned int ee =
                                  reorder_face_derivative_indices<dim>(faces[face].interior_face_no,
                                                                       e,
                      ...
                      for (unsigned int q = 0; q < n_q_points; ++q)
                        {
                          ...
                          for (unsigned int d = 0; d < dim; ++d)
                            for (unsigned int e = 0; e < dim; ++e)
                              {
                                face_data.general_jac[q][d][e][v] = face_data.general_jac[q][d][e][0];
                                face_data.normal_vectors[q][d][v] = face_data.normal_vectors[q][d][0];
                              }
                          if (fe_face_values.get_update_flags() & update_quadrature_points)
                            for (unsigned int d = 0; d < dim; ++d)
                              face_data.quadrature_points[q][d][v] = face_data.quadrature_points[q][d][0];
                        }
                    }
                  if (is_boundary_face == false && faces[face].cells_exterior[v] != numbers::invalid_unsigned_int)
                    {
                      typename dealii::Triangulation<dim>::cell_iterator cell_it(
                        &tria, cells[faces[face].cells_exterior[v]].first, cells[faces[face].cells_exterior[v]].second);
                      ...
                      if (faces[face].subface_index >= GeometryInfo<dim>::max_children_per_cell)
                        {
                          ...
                          const unsigned int fe_index =
                            active_fe_index.size() > 0 ?
                              active_fe_index[faces[face].cells_exterior[0] / VectorizedArrayType::size()] :
                              0;
                          const unsigned int hp_quad_index =
                            mapping_info.cell_data[my_q].descriptor.size() == 1 ? 0 : fe_index;
                          const unsigned int hp_mapping_index = mapping_in.size() == 1 ? 0 : fe_index;

                          const auto &mapping    = mapping_in[hp_mapping_index];
                          const auto &quadrature = mapping_info.face_data[my_q].q_collection[hp_quad_index];

                          if (fe_face_values_container[my_q][fe_index] == nullptr)
                            fe_face_values_container[my_q][fe_index] =
                          ...
                          fe_face_values_container[my_q][fe_index]->reinit(cell_it, faces[face].exterior_face_no);

                          actual_fe_face_values = fe_face_values_container[my_q][fe_index].get();
                        }
                      else
                        {
                          if (fe_subface_values_container[my_q][0] == nullptr)
                            fe_subface_values_container[my_q][0] =
                              std::make_shared<FESubfaceValues<dim>>(mapping,
                                                                     *dummy_fe[my_q][hp_quad_index],
                                                                     quadrature,
                                                                     mapping_info.update_flags_inner_faces);
                          fe_subface_values_container[my_q][0]->reinit(cell_it,
                            ...
                            for (unsigned int e = 0; e < dim; ++e)
                              {
                                if (std::abs(inv_jac[d][e] - jacobian_0[d][e]) >
                                  ...
                                  JxW_is_similar = false;
                                const unsigned int ee =
                                  reorder_face_derivative_indices<dim>(faces[face].exterior_face_no,
                                                                       e,
                                                                       cell_it->reference_cell());
                  ...
                  // Fill up with 'known' values
                    ...
                    {
                      Assert(faces[face].cells_exterior[0] != numbers::invalid_unsigned_int, ExcInternalError());
                      for (unsigned int q = 0; q < n_q_points; ++q)
                        for (unsigned int d = 0; d < dim; ++d)
                          for (unsigned int e = 0; e < dim; ++e)
              ...
              if (fe_face_values.get_update_flags() & update_quadrature_points)
                {
                  data.first[my_q].quadrature_point_offsets.push_back(data.first[my_q].quadrature_points.size());
                  if (fe_face_values.get_update_flags() & update_quadrature_points)
                    for (unsigned int q = 0; q < n_q_points; ++q)
                  ...
                  if (my_q == 0)
                    {
                      ...
                      new_entry.second = data.second.data.size();
                      for (unsigned int v = 0; v < VectorizedArrayType::size(); ++v)
                        new_entry.first[2 * dim * dim + dim][v] = face_data.JxW_values[0][v] /
                                    ...
                                    MappingInfoStorage<dim - 1, dim, VectorizedArrayType>              &data_faces)
      {
        for (unsigned int face = first_face; face < last_face; ++face)
          {
            const bool         is_boundary_face = faces[face].cells_exterior[0] == numbers::invalid_unsigned_int;
            const unsigned int n_q_points_work  = face_type[face] > affine ? data_faces.descriptor[0].n_q_points : 1;
      ...
      template <int dim, typename Number, typename VectorizedArrayType, typename VectorizedDouble>
      void
      mapping_q_compute_range(const unsigned int                                                  begin_face,
                              ...
                              MappingInfoStorage<dim - 1, dim, VectorizedArrayType> &my_data)
      {
        ...
        FEEvaluationData<dim, VectorizedDouble, true> eval_int(shape_info, true);
        FEEvaluationData<dim, VectorizedDouble, true> eval_ext(shape_info, false);

        AlignedVector<VectorizedDouble> evaluation_data_int, evaluation_data_ext;
          ...
          for (unsigned vv = 0; vv < n_lanes; vv += n_lanes_d)
            {
              ...
              unsigned int start_indices[n_lanes_d];
              for (unsigned int v = 0; v < n_lanes_d; ++v)
                if (faces[face].cells_interior[vv + v] != numbers::invalid_unsigned_int)
                  start_indices[v] = faces[face].cells_interior[vv + v] * n_mapping_points * dim;
                else
                  start_indices[v] = faces[face].cells_interior[0] * n_mapping_points * dim;
              ...
              FEFaceEvaluationFactory<dim, VectorizedDouble>::evaluate(
                ...
                EvaluationFlags::values | EvaluationFlags::gradients |
                  (update_flags_faces & update_jacobian_grads ? EvaluationFlags::hessians : EvaluationFlags::nothing),
                eval_int.begin_dof_values(),
                eval_int);
              ...
              if (update_flags_faces & update_quadrature_points)
                for (unsigned int q = 0; q < n_q_points; ++q)
                  for (unsigned int d = 0; d < dim; ++d)
                    store_vectorized_array(eval_int.begin_values()[d * n_q_points + q],
                                           vv,
                                           my_data.quadrature_points[my_data.quadrature_point_offsets[face] + q][d]);
              ...
              const auto compute_jacobian_grad = [&](unsigned int                                   face_no,
                                                     int                                            is_exterior,
                                                     unsigned int                                   q,
                ...
                for (unsigned int d = 0; d < dim; ++d)
                  for (unsigned int e = 0; e < dim; ++e)
                    {
                      const unsigned int ee       = ExtractFaceHelper::reorder_face_derivative_indices<dim>(face_no, e);
                      inv_transp_jac_permut[d][e] = inv_jac[ee][d];
                    }
                Tensor<2, dim, VectorizedDouble> jacobi;
                for (unsigned int e = 0; e < dim; ++e)
                  for (unsigned int d = 0; d < dim; ++d)
                    jacobi[d][e] = eval.begin_gradients()[(d * dim + e) * n_q_points + q];
                Tensor<2, dim, VectorizedDouble> inv_transp_jac = transpose(invert(jacobi));
                Tensor<3, dim, VectorizedDouble> jac_grad;
                for (unsigned int d = 0; d < dim; ++d)
                  {
                    for (unsigned int e = 0; e < dim; ++e)
                    ...
                    const auto inv_jac_grad =
                      process_jacobian_gradient(inv_transp_jac_permut, inv_transp_jac, jac_grad);
                    for (unsigned int e = 0; e < dim; ++e)
                      {
                        for (unsigned int d = 0; d < hess_dim; ++d)
                          store_vectorized_array(inv_jac_grad[d][e],
                                                 vv,
                                                 my_data.jacobian_gradients[is_exterior][offset + q][d][e]);
                      }
                    ...
                    for (unsigned int d = 0; d < dim; ++d)
                      for (unsigned int f = 0; f < dim; ++f)
                        store_vectorized_array(jac_grad[f][d][d],
                                               vv,
                                               my_data.jacobian_gradients_non_inverse[is_exterior][offset + q][d][f]);
                      ...
                      for (unsigned int e = d + 1; e < dim; ++e, ++count)
                        for (unsigned int f = 0; f < dim; ++f)
                          store_vectorized_array(
                            jac_grad[f][d][e],
                            vv,
                            my_data.jacobian_gradients_non_inverse[is_exterior][offset + q][count][f]);
                  }
              };

              // go through the faces and fill the result
              const unsigned int n_points_compute = face_type[face] <= affine ? 1 : n_q_points;
              for (unsigned int q = 0; q < n_points_compute; ++q)
                {
                  ...
                  for (unsigned int e = 0; e < dim; ++e)
                    {
                      const unsigned int ee =
                        ExtractFaceHelper::reorder_face_derivative_indices<dim>(interior_face_no, e);
                      for (unsigned int d = 0; d < dim; ++d)
                        jac[d][ee] = eval_int.begin_gradients()[(d * dim + e) * n_q_points + q];
                    }
                  ...
                  for (unsigned int e = 0; e < dim; ++e)
                    {
                      const unsigned int ee =
                        ExtractFaceHelper::reorder_face_derivative_indices<dim>(interior_face_no, e);
                      for (unsigned int d = 0; d < dim; ++d)
                        store_vectorized_array(inv_jac[ee][d], vv, my_data.jacobians[0][offset + q][d][e]);
                    }
                    ...
                    for (unsigned int e = 0; e < dim; ++e)
                      {
                        const unsigned int ee =
                          ExtractFaceHelper::reorder_face_derivative_indices<dim>(interior_face_no, e);
                        for (unsigned int d = 0; d < dim; ++d)
                          store_vectorized_array(jac[d][ee], vv, my_data.jacobians[0][offset + q + 1][d][e]);
                      }

                  if (update_flags_faces & update_jacobian_grads)
                    {
                      compute_jacobian_grad(interior_face_no, 0, q, inv_jac, eval_int);
                    }

                  std::array<Tensor<1, dim, VectorizedDouble>, dim - 1> tangential_vectors;
                  for (unsigned int d = 0; d != dim - 1; ++d)
                    for (unsigned int e = 0; e < dim; ++e)
                      for (unsigned int f = 0; f < dim; ++f)
                        tangential_vectors[d][e] +=
                          jac[e][f] * GeometryInfo<dim>::unit_tangential_vectors[interior_face_no][d][f];
                  ...
                  if (dim == 1)
                    ...
                    boundary_form = cross_product_3d(tangential_vectors[0], tangential_vectors[1]);
                  else
                    Assert(false, ExcNotImplemented());

                  const VectorizedDouble JxW =
                    boundary_form.norm() *
                    (face_type[face] <= affine ? 1. : my_data.descriptor[0].quadrature.weight(q));

                  store_vectorized_array(JxW, vv, my_data.JxW_values[offset + q]);

                  const Tensor<1, dim, VectorizedDouble> normal = boundary_form / boundary_form.norm();

                  for (unsigned int d = 0; d < dim; ++d)
                    store_vectorized_array(normal[d], vv, my_data.normal_vectors[offset + q][d]);

                  my_data.normals_times_jacobians[0][offset + q] =
                    my_data.normal_vectors[offset + q] * my_data.jacobians[0][offset + q];
                }

              if (faces[face].cells_exterior[0] != numbers::invalid_unsigned_int)
                {
                  for (unsigned int v = 0; v < n_lanes_d; ++v)
                    if (faces[face].cells_exterior[vv + v] != numbers::invalid_unsigned_int)
                      start_indices[v] = faces[face].cells_exterior[vv + v] * n_mapping_points * dim;
                    else
                      start_indices[v] = faces[face].cells_exterior[0] * n_mapping_points * dim;
                  ...
                  FEFaceEvaluationFactory<dim, VectorizedDouble>::evaluate(
                    dim,
                    EvaluationFlags::values | EvaluationFlags::gradients |
                      (update_flags_faces & update_jacobian_grads ? EvaluationFlags::hessians :
                                                                    EvaluationFlags::nothing),
                    eval_ext.begin_dof_values(),
                    eval_ext);
                  ...
                  for (unsigned int q = 0; q < n_points_compute; ++q)
                    {
                      const unsigned int               exterior_face_no = faces[face].exterior_face_no;
                      Tensor<2, dim, VectorizedDouble> jac;
                      for (unsigned int e = 0; e < dim; ++e)
                        {
                          const unsigned int ee =
                            ExtractFaceHelper::reorder_face_derivative_indices<dim>(exterior_face_no, e);
                          for (unsigned int d = 0; d < dim; ++d)
                            jac[d][ee] = eval_ext.begin_gradients()[(d * dim + e) * n_q_points + q];
                        }
                      Tensor<2, dim, VectorizedDouble> inv_jac = invert(jac);
                      for (unsigned int e = 0; e < dim; ++e)
                        {
                          const unsigned int ee =
                            ExtractFaceHelper::reorder_face_derivative_indices<dim>(exterior_face_no, e);
                          for (unsigned int d = 0; d < dim; ++d)
                            store_vectorized_array(inv_jac[ee][d], vv, my_data.jacobians[1][offset + q][d][e]);
                        }
                      if (face_type[face] <= affine)
                        for (unsigned int e = 0; e < dim; ++e)
                          {
                            const unsigned int ee =
                              ExtractFaceHelper::reorder_face_derivative_indices<dim>(exterior_face_no, e);
                            for (unsigned int d = 0; d < dim; ++d)
                              store_vectorized_array(jac[d][ee], vv, my_data.jacobians[1][offset + q + 1][d][e]);
                          }

                      if (update_flags_faces & update_jacobian_grads)
                        {
                          compute_jacobian_grad(exterior_face_no, 1, q, inv_jac, eval_ext);
                        }

                      my_data.normals_times_jacobians[1][offset + q] =
                        my_data.normal_vectors[offset + q] * my_data.jacobians[1][offset + q];
                    }
                }
            }
      }
      ...
      const dealii::hp::MappingCollection<dim>                           &mapping)
    {
      ...
      // work
      unsigned int work_per_chunk =
        std::max(std::size_t(8), (faces.size() + MultithreadInfo::n_threads() - 1) / MultithreadInfo::n_threads());

      std::vector<std::pair<std::vector<MappingInfoStorage<dim - 1, dim, VectorizedArrayType>>,
                            ExtractFaceHelper::CompressedFaceData<dim, Number, VectorizedArrayType>>>
        data_faces_local;
        ...
        while (face_range.first < faces.size())
          {
            data_faces_local.push_back(
              std::make_pair(std::vector<MappingInfoStorage<dim - 1, dim, VectorizedArrayType>>(face_data.size()),
                             ExtractFaceHelper::CompressedFaceData<dim, Number, VectorizedArrayType>(
                               ExtractCellHelper::get_jacobian_size(tria))));
            tasks += Threads::new_task(&ExtractFaceHelper::initialize_face_range<dim, Number, VectorizedArrayType>,
                                       face_range,
                                       tria,
                                       cells,
                                       faces,
      ...
      for (unsigned int i = 0; i < data_faces_local.size(); ++i)
        ExtractCellHelper::merge_compressed_data(data_faces_local[i].second.data,
                                                 data_faces_local[0].second.data,
                                                 indices_compressed[i]);

      const UpdateFlags update_flags_common = update_flags_boundary_faces | update_flags_inner_faces;
      ...
      for (unsigned int my_q = 0; my_q < face_data.size(); ++my_q)
        {
          ...
          face_data[my_q].JxW_values.resize_fast(shift.back()[0] +
          ...
          if (update_flags_common & update_jacobian_grads)
            {
              ...
              face_data[my_q].jacobian_gradients_non_inverse[0].resize_fast(face_data[my_q].JxW_values.size());
              face_data[my_q].jacobian_gradients_non_inverse[1].resize_fast(face_data[my_q].JxW_values.size());
            }
          ...
          Threads::TaskGroup<> tasks;
          for (unsigned int i = 0; i < data_faces_local.size(); ++i)
            tasks += Threads::new_task(&ExtractCellHelper::copy_data<dim - 1, dim, Number, VectorizedArrayType>,
                                       work_per_chunk * i,
                                       shift[i],
                                       indices_compressed[i],
                                       face_type,
          ...
          if (my_q == 0)
            {
              const Number jac_size = ExtractCellHelper::get_jacobian_size(tria);
              for (const auto &it : data_faces_local[0].second.data)
                {
                  ...
                  for (unsigned int i = 0; i < 2; ++i)
                    for (unsigned int d = 0; d < dim; ++d)
                      for (unsigned int e = 0; e < dim; ++e)
                        for (unsigned int v = 0; v < VectorizedArrayType::size(); ++v)
                          face_data[my_q].jacobians[i][it.second][d][e][v] = it.first[i * dim * dim + d * dim + e][v];

                  // normal vectors; invert previously applied scaling
                  for (unsigned int d = 0; d < dim; ++d)
                    for (unsigned int v = 0; v < VectorizedArrayType::size(); ++v)
                      face_data[my_q].normal_vectors[it.second][d][v] = it.first[2 * dim * dim + d][v] * jac_size;
                }
            }
          else
            {
          ...
          for (unsigned int i = 0; i < data_faces_local.size(); ++i)
            tasks +=
              Threads::new_task(&ExtractFaceHelper::compute_normal_times_jacobian<dim, Number, VectorizedArrayType>,
                                work_per_chunk * i,
      ...
      const FaceInfo<VectorizedArrayType::size()>              &face_info)
    {
      ...
      const MappingQ<dim> *mapping_q = dynamic_cast<const MappingQ<dim> *>(&this->mapping_collection->operator[](0));
      Assert(mapping_q != nullptr, ExcInternalError());

      const unsigned int    mapping_degree   = mapping_q->get_degree();
      const unsigned int    n_mapping_points = Utilities::pow(mapping_degree + 1, dim);
      AlignedVector<double> plain_quadrature_points(cell_array.size() * n_mapping_points * dim);
      ...
      {
        AlignedVector<std::array<Tensor<2, dim>, dim + 1>> jacobians_on_stencil(cell_array.size());

        // Create as many chunks of cells as we have threads and spawn the
        // work
        unsigned int work_per_chunk =
          std::max(std::size_t(1),
                   (cell_array.size() + MultithreadInfo::n_threads() - 1) / MultithreadInfo::n_threads());
        ...
        std::size_t          offset = 0;
        Threads::TaskGroup<> tasks;
        for (unsigned int t = 0; t < MultithreadInfo::n_threads(); ++t, offset += work_per_chunk)
          tasks += Threads::new_task(&ExtractCellHelper::mapping_q_query_fe_values<dim>,
                                     offset,
        ...
        tasks.join_all();
        cell_data_index = ExtractCellHelper::mapping_q_find_compression(
          jacobian_size, jacobians_on_stencil, n_mapping_points, plain_quadrature_points, preliminary_cell_type);
      }
      ...
      using VectorizedDouble = VectorizedArray<double,
                                               ((std::is_same_v<Number, float> && VectorizedArrayType::size() > 1) ?
      ...
      std::vector<ShapeInfo<VectorizedDouble>> shape_infos(cell_data.size());
      {
        ...
        for (unsigned int my_q = 0; my_q < cell_data.size(); ++my_q)
          shape_infos[my_q].reinit(cell_data[my_q].descriptor[0].quadrature, fe_geometry);
      }
      ...
      for (unsigned int cell = 0; cell < cell_array.size(); cell += n_lanes)
        {
          std::pair<std::array<unsigned int, n_lanes>, unsigned int> data_indices;
          ...
          cell_type[cell / n_lanes] =
            *std::max_element(preliminary_cell_type.data() + cell, preliminary_cell_type.data() + cell + n_lanes);
        }
      ...
      for (unsigned int my_q = 0; my_q < cell_data.size(); ++my_q)
        {
          MappingInfoStorage<dim, dim, VectorizedArrayType> &my_data = cell_data[my_q];
          ...
          for (unsigned int cell = 0; cell < cell_type.size(); ++cell)
            {
              if (process_cell[cell] == false)
                my_data.data_index_offsets[cell] = my_data.data_index_offsets[cell_data_index_vect[cell]];
              else
                my_data.data_index_offsets[cell] = max_size;
              max_size =
                std::max(max_size, my_data.data_index_offsets[cell] + (cell_type[cell] <= affine ? 2 : n_q_points));
            }
          ...
          if (update_flags_cells & update_quadrature_points)
            {
              ...
              for (unsigned int cell = 1; cell < cell_type.size(); ++cell)
                if (cell_type[cell - 1] <= affine)
                  my_data.quadrature_point_offsets[cell] = my_data.quadrature_point_offsets[cell - 1] + 1;
                else
                  my_data.quadrature_point_offsets[cell] = my_data.quadrature_point_offsets[cell - 1] + n_q_points;
              my_data.quadrature_points.resize_fast(my_data.quadrature_point_offsets.back() +
          ...
          dealii::parallel::apply_to_subranges(
            ...
            [&](const unsigned int begin, const unsigned int end) {
              ExtractCellHelper::mapping_q_compute_range<dim, Number, VectorizedArrayType, VectorizedDouble>(
                begin,
                end,
                cell_type,
                process_cell,
            ...
            std::max(cell_type.size() / MultithreadInfo::n_threads() / 2, std::size_t(2U)));
        }

      const std::vector<FaceToCellTopology<VectorizedArrayType::size()>> &faces = face_info.faces;
      if (faces.empty())
        return;

      // step 5: find compression of faces with vectorization
      std::map<std::array<unsigned int, 2 * n_lanes + 3>, unsigned int> compressed_faces;
      ...
      for (unsigned int face = 0; face < faces.size(); ++face)
        {
          std::pair<std::array<unsigned int, 2 * n_lanes + 3>, unsigned int> data_indices;
          for (unsigned int i = 0; i < n_lanes; ++i)
            if (faces[face].cells_interior[i] != numbers::invalid_unsigned_int)
              data_indices.first[i] = cell_data_index[faces[face].cells_interior[i]];
            else
          ...
          for (unsigned int i = 0; i < n_lanes; ++i)
            if (faces[face].cells_exterior[i] != numbers::invalid_unsigned_int)
              data_indices.first[n_lanes + i] = cell_data_index[faces[face].cells_exterior[i]];
          ...
          for (unsigned int i = 0; i < n_lanes; ++i)
            if (faces[face].cells_interior[i] != numbers::invalid_unsigned_int)
              face_type[face] = std::max(face_type[face], preliminary_cell_type[faces[face].cells_interior[i]]);
          for (unsigned int i = 0; i < n_lanes; ++i)
            if (faces[face].cells_exterior[i] != numbers::invalid_unsigned_int)
              face_type[face] = std::max(face_type[face], preliminary_cell_type[faces[face].cells_exterior[i]]);
        }
      ...
      for (unsigned int my_q = 0; my_q < face_data.size(); ++my_q)
        {
          MappingInfoStorage<dim - 1, dim, VectorizedArrayType> &my_data = face_data[my_q];
          ...
          for (unsigned int face = 0; face < face_type.size(); ++face)
            {
              if (process_face[face] == false)
                my_data.data_index_offsets[face] = my_data.data_index_offsets[face_data_index_vect[face]];
              else
                my_data.data_index_offsets[face] = max_size;
              max_size =
                std::max(max_size, my_data.data_index_offsets[face] + (face_type[face] <= affine ? 2 : n_q_points));
            }
          ...
          if (update_flags_common & update_quadrature_points)
            {
              ...
              for (unsigned int face = 1; face < faces.size(); ++face)
                my_data.quadrature_point_offsets[face] = n_q_points + my_data.quadrature_point_offsets[face - 1];
              my_data.quadrature_points.resize_fast(face_type.size() * n_q_points);
            }
          ...
          dealii::parallel::apply_to_subranges(
            ...
            face_type.size(),
            [&](const unsigned int begin, const unsigned int end) {
              ExtractFaceHelper::mapping_q_compute_range<dim, Number, VectorizedArrayType, VectorizedDouble>(
                begin,
                end,
                faces,
                face_type,
            ...
            std::max(face_type.size() / MultithreadInfo::n_threads() / 2, std::size_t(2U)));
        }
      ...
      for (unsigned int my_q = 1; my_q < face_data.size(); ++my_q)
        if (face_data[my_q].descriptor[0].n_q_points > face_data[quad_with_most_points].descriptor[0].n_q_points)
        ...
        [&](const unsigned int begin, const unsigned int end) {
          for (unsigned int face = begin; face < end; ++face)
            if (face_type[face] == general)
              {
                const unsigned int n_q_points = face_data[quad_with_most_points].descriptor[0].n_q_points;
      ...
      initialize_faces_by_cells(tria, cell_array, face_info, *this->mapping_collection);
    }
      ...
      const dealii::hp::MappingCollection<dim>                 &mapping_in)
    {
      ...
      UpdateFlags            update_flags =
        (update_flags_faces_by_cells & update_quadrature_points ? update_quadrature_points : update_default) |
        update_normal_vectors | update_JxW_values | update_jacobians;

      const auto compute_neighbor_index =
        [&face_info](const unsigned int cell, const unsigned int face, const unsigned int lane) {
          constexpr unsigned int n_lanes    = VectorizedArrayType::size();
          const unsigned int     cell_this  = cell * n_lanes + lane;
          unsigned int           face_index = face_info.cell_and_face_to_plain_faces(cell, face, lane);
          if (face_index == numbers::invalid_unsigned_int)
            return numbers::invalid_unsigned_int;

          unsigned int cell_neighbor = face_info.faces[face_index / n_lanes].cells_interior[face_index % n_lanes];
          if (cell_neighbor == cell_this)
            cell_neighbor = face_info.faces[face_index / n_lanes].cells_exterior[face_index % n_lanes];
        ...
        for (const unsigned int face : GeometryInfo<dim>::face_indices())
          {
            ...
            for (unsigned int v = 0; v < n_lanes; ++v)
              {
                const unsigned int cell_neighbor = compute_neighbor_index(cell, face, v);
                if (cell_neighbor != numbers::invalid_unsigned_int)
                  faces_by_cells_type[cell][face] =
                    std::max(faces_by_cells_type[cell][face], cell_type[cell_neighbor / n_lanes]);
              }
          }
      ...
      for (unsigned int my_q = 0; my_q < n_quads; ++my_q)
        {
          ...
          face_data_by_cells[my_q].data_index_offsets.resize(cell_type.size() * GeometryInfo<dim>::faces_per_cell);
          if (update_flags & update_quadrature_points)
            face_data_by_cells[my_q].quadrature_point_offsets.resize(cell_type.size() *
            ...
            for (const unsigned int face : GeometryInfo<dim>::face_indices())
              {
                if (faces_by_cells_type[i][face] <= affine)
                  {
                    face_data_by_cells[my_q].data_index_offsets[i * GeometryInfo<dim>::faces_per_cell + face] =
                      storage_length;
                    ++storage_length;
                  }
                else
                ...
                if (update_flags & update_quadrature_points)
                  face_data_by_cells[my_q].quadrature_point_offsets[i * GeometryInfo<dim>::faces_per_cell + face] =
                    (i * GeometryInfo<dim>::faces_per_cell + face) * face_data_by_cells[my_q].descriptor[0].n_q_points;
              }
          face_data_by_cells[my_q].JxW_values.resize_fast(storage_length * GeometryInfo<dim>::faces_per_cell);
          face_data_by_cells[my_q].jacobians[0].resize_fast(storage_length * GeometryInfo<dim>::faces_per_cell);
          face_data_by_cells[my_q].jacobians[1].resize_fast(storage_length * GeometryInfo<dim>::faces_per_cell);
          if (update_flags & update_normal_vectors)
            face_data_by_cells[my_q].normal_vectors.resize_fast(storage_length * GeometryInfo<dim>::faces_per_cell);
          if (update_flags & update_normal_vectors && update_flags & update_jacobians)
            face_data_by_cells[my_q].normals_times_jacobians[0].resize_fast(storage_length *
                                                                            GeometryInfo<dim>::faces_per_cell);
          if (update_flags & update_normal_vectors && update_flags & update_jacobians)
            face_data_by_cells[my_q].normals_times_jacobians[1].resize_fast(storage_length *
                                                                            GeometryInfo<dim>::faces_per_cell);
          if (update_flags & update_jacobian_grads)
            {
              face_data_by_cells[my_q].jacobian_gradients[0].resize_fast(storage_length *
                                                                         GeometryInfo<dim>::faces_per_cell);
              face_data_by_cells[my_q].jacobian_gradients_non_inverse[0].resize_fast(storage_length *
                                                                                     GeometryInfo<dim>::faces_per_cell);
            }

          if (update_flags & update_quadrature_points)
            face_data_by_cells[my_q].quadrature_points.resize_fast(
              cell_type.size() * GeometryInfo<dim>::faces_per_cell * face_data_by_cells[my_q].descriptor[0].n_q_points);
        }

      FE_Nothing<dim> dummy_fe;
      // currently no hp-indices implemented
      const unsigned int                                                   fe_index = 0;
      std::vector<std::vector<std::shared_ptr<dealii::FEFaceValues<dim>>>> fe_face_values(face_data_by_cells.size());
      for (unsigned int i = 0; i < fe_face_values.size(); ++i)
        fe_face_values[i].resize(face_data_by_cells[i].descriptor.size());
      std::vector<std::vector<std::shared_ptr<dealii::FEFaceValues<dim>>>> fe_face_values_neigh(
          ...
          for (const unsigned int face : GeometryInfo<dim>::face_indices())
            {
              if (fe_face_values[my_q][fe_index].get() == nullptr)
                fe_face_values[my_q][fe_index] = std::make_shared<dealii::FEFaceValues<dim>>(
                  mapping, dummy_fe, face_data[my_q].q_collection[fe_index], update_flags);
              if (fe_face_values_neigh[my_q][fe_index].get() == nullptr)
                fe_face_values_neigh[my_q][fe_index] = std::make_shared<dealii::FEFaceValues<dim>>(
                  mapping, dummy_fe, face_data[my_q].q_collection[fe_index], update_flags);
              dealii::FEFaceValues<dim> &fe_val       = *fe_face_values[my_q][fe_index];
              dealii::FEFaceValues<dim> &fe_val_neigh = *fe_face_values_neigh[my_q][fe_index];
              const unsigned int         offset =
                face_data_by_cells[my_q].data_index_offsets[cell * GeometryInfo<dim>::faces_per_cell + face];
              ...
              for (unsigned int v = 0; v < n_lanes; ++v)
                {
                  ...
                  if (cell_neighbor != numbers::invalid_unsigned_int)
                    {
                      typename dealii::Triangulation<dim>::cell_iterator cell_it_neigh(&tria,
                                                                                       cells[cell_neighbor].first,
                                                                                       cells[cell_neighbor].second);
                      fe_val_neigh.reinit(cell_it_neigh,
                                          cell_it->at_boundary(face) ? cell_it->periodic_neighbor_face_no(face) :
                  ...
                  if (my_cell_type <= affine)
                    {
                      ...
                      if (update_flags & update_jacobians)
                        {
                          ...
                          for (unsigned int d = 0; d < dim; ++d)
                            for (unsigned int e = 0; e < dim; ++e)
                              {
                                const unsigned int ee =
                                  ExtractFaceHelper::reorder_face_derivative_indices<dim>(face, e);
                                face_data_by_cells[my_q].jacobians[0][offset][d][e][v] = inv_jac[d][ee];
                              }
                        }
                      if (cell_neighbor != numbers::invalid_unsigned_int && (update_flags & update_jacobians))
                        for (unsigned int q = 0; q < fe_val.n_quadrature_points; ++q)
                          {
                            DerivativeForm<1, dim, dim> inv_jac = fe_val_neigh.jacobian(q).covariant_form();
                            for (unsigned int d = 0; d < dim; ++d)
                              for (unsigned int e = 0; e < dim; ++e)
                                {
                                  const unsigned int ee = ExtractFaceHelper::reorder_face_derivative_indices<dim>(
                                    fe_val_neigh.get_face_number(), e);
                                  face_data_by_cells[my_q].jacobians[1][offset][d][e][v] = inv_jac[d][ee];
                                }
                          }
                      if (update_flags & update_jacobian_grads)
                        {
                      ...
                      if (update_flags & update_normal_vectors)
                        for (unsigned int d = 0; d < dim; ++d)
                          face_data_by_cells[my_q].normal_vectors[offset][d][v] = fe_val.normal_vector(0)[d];
                    }
                  // copy data for general data type
                  else
                    {
                              ...
                              for (unsigned int e = 0; e < dim; ++e)
                                {
                                  const unsigned int ee =
                                    ExtractFaceHelper::reorder_face_derivative_indices<dim>(face, e);
                                  face_data_by_cells[my_q].jacobians[0][offset + q][d][e][v] = inv_jac[d][ee];
                                }
                          }
                      if (cell_neighbor != numbers::invalid_unsigned_int && (update_flags & update_jacobians))
                        for (unsigned int q = 0; q < fe_val.n_quadrature_points; ++q)
                              ...
                              for (unsigned int e = 0; e < dim; ++e)
                                {
                                  const unsigned int ee = ExtractFaceHelper::reorder_face_derivative_indices<dim>(
                                    fe_val_neigh.get_face_number(), e);
                                  face_data_by_cells[my_q].jacobians[1][offset + q][d][e][v] = inv_jac[d][ee];
                                }
                          }
                      if (update_flags & update_jacobian_grads)
                        {
                      ...
                      if (update_flags & update_normal_vectors)
                        for (unsigned int q = 0; q < fe_val.n_quadrature_points; ++q)
                          for (unsigned int d = 0; d < dim; ++d)
                            face_data_by_cells[my_q].normal_vectors[offset + q][d][v] = fe_val.normal_vector(q)[d];
                    }
                  if (update_flags & update_quadrature_points)
                    for (unsigned int q = 0; q < fe_val.n_quadrature_points; ++q)
                      for (unsigned int d = 0; d < dim; ++d)
                        face_data_by_cells[my_q].quadrature_points[face_data_by_cells[my_q].quadrature_point_offsets
                                                                     [cell * GeometryInfo<dim>::faces_per_cell + face] +
                                                                   q][d][v] = fe_val.quadrature_point(q)[d];
                }
              if (update_flags & update_normal_vectors && update_flags & update_jacobians)
                for (unsigned int q = 0; q < (my_cell_type <= affine ? 1 : fe_val.n_quadrature_points); ++q)
              ...
              if (update_flags & update_normal_vectors && update_flags & update_jacobians)
                for (unsigned int q = 0; q < (my_cell_type <= affine ? 1 : fe_val.n_quadrature_points); ++q)
                  face_data_by_cells[my_q].normals_times_jacobians[1][offset + q] =
                    face_data_by_cells[my_q].normal_vectors[offset + q] *
    ...
    MappingInfo<dim, Number, VectorizedArrayType>::memory_consumption() const
    {
      ...
      memory += faces_by_cells_type.capacity() * GeometryInfo<dim>::faces_per_cell * sizeof(GeometryType);
      memory += sizeof(*this);
      return memory;
    }
    ...
    void
    MappingInfo<dim, Number, VectorizedArrayType>::print_memory_consumption(StreamType     &out,
                                                                            const TaskInfo &task_info) const
    {
      ...
      task_info.print_memory_statistics(out,
                                        faces_by_cells_type.capacity() * GeometryInfo<dim>::faces_per_cell *
                                          sizeof(GeometryType));

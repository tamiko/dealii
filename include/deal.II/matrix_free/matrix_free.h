...
public:
  ...
  struct AdditionalData
  {
    ...
    AdditionalData(const AdditionalData &other)
      ...
      , mapping_update_flags_boundary_faces(other.mapping_update_flags_boundary_faces)
      , mapping_update_flags_inner_faces(other.mapping_update_flags_inner_faces)
      , mapping_update_flags_faces_by_cells(other.mapping_update_flags_faces_by_cells)
      ...
      , overlap_communication_computation(other.overlap_communication_computation)
      ...
      , cell_vectorization_categories_strict(other.cell_vectorization_categories_strict)
   ...
   */
  void
  copy_from(const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free_base);
  ...
  void
  cell_loop(const std::function<void(const MatrixFree<dim, Number, VectorizedArrayType> &,
                                     OutVector &,
                                     const InVector &,
  ...
  template <typename OutVector, typename InVector>
  void
  loop(const std::function<void(const MatrixFree<dim, Number, VectorizedArrayType> &,
                                OutVector &,
                                const InVector &,
                                const std::pair<unsigned int, unsigned int> &)> &cell_operation,
  ...
  loop_cell_centric(
    const std::function<
      void(const MatrixFree &, OutVector &, const InVector &, const std::pair<unsigned int, unsigned int> &)>
                           &cell_operation,
    OutVector              &dst,
    const InVector         &src,
    const bool              zero_dst_vector        = false,
   ...
   */
  unsigned int
  get_cell_active_fe_index(const std::pair<unsigned int, unsigned int> range) const;

  /**
   * In the hp-adaptive case, return the active FE index of a face range.
   */
  unsigned int
  get_face_active_fe_index(const std::pair<unsigned int, unsigned int> range, const bool is_interior_face = true) const;
   ...
   */
  void
  renumber_dofs(std::vector<types::global_dof_index> &renumbering, const unsigned int dof_handler_index = 0);
  ...
  get_faces_by_cells_boundary_id(const unsigned int cell_batch_index, const unsigned int face_number) const;
   ...
   */
  std::pair<int, int>
  get_cell_level_and_index(const unsigned int cell_batch_index, const unsigned int lane_index) const;
   ...
   */
  unsigned int
  get_matrix_free_cell_index(const typename Triangulation<dim>::cell_iterator &cell) const;
   ...
   */
  unsigned int
  get_dofs_per_cell(const unsigned int dof_handler_index = 0, const unsigned int hp_active_fe_index = 0) const;

  /**
   * Return the number of quadrature points per cell for a given hp-index.
   */
  unsigned int
  get_n_q_points(const unsigned int quad_index = 0, const unsigned int hp_active_fe_index = 0) const;
   ...
   */
  unsigned int
  get_dofs_per_face(const unsigned int dof_handler_index = 0, const unsigned int hp_active_fe_index = 0) const;
   ...
   */
  unsigned int
  get_n_q_points_face(const unsigned int quad_index = 0, const unsigned int hp_active_fe_index = 0) const;

  /**
   * Return the quadrature rule for given hp-index.
   */
  const Quadrature<dim> &
  get_quadrature(const unsigned int quad_index = 0, const unsigned int hp_active_fe_index = 0) const;

  /**
   * Return the quadrature rule for given hp-index.
   */
  const Quadrature<dim - 1> &
  get_face_quadrature(const unsigned int quad_index = 0, const unsigned int hp_active_fe_index = 0) const;
   ...
   */
  unsigned int
  get_cell_range_category(const std::pair<unsigned int, unsigned int> cell_batch_range) const;
   ...
   */
  std::pair<unsigned int, unsigned int>
  get_face_range_category(const std::pair<unsigned int, unsigned int> face_batch_range) const;
...
private:
  ...
  template <typename number2>
  void
  initialize_indices(const std::vector<const AffineConstraints<number2> *> &constraint,
                     const std::vector<IndexSet>                           &locally_owned_set,
                     const AdditionalData                                  &additional_data);
   ...
   */
  void
  initialize_dof_handlers(const std::vector<const DoFHandler<dim, dim> *> &dof_handlers,
                          const AdditionalData                            &additional_data);
   ...
   * objects.
   */
  mutable Threads::ThreadLocalStorage<std::list<std::pair<bool, AlignedVector<VectorizedArrayType>>>> scratch_pad;
...
template <typename T>
inline void
MatrixFree<dim, Number, VectorizedArrayType>::initialize_cell_data_vector(AlignedVector<T> &vec) const
{
...
template <typename T>
inline void
MatrixFree<dim, Number, VectorizedArrayType>::initialize_face_data_vector(AlignedVector<T> &vec) const
{
  vec.resize(this->n_inner_face_batches() + this->n_boundary_face_batches() + this->n_ghost_inner_face_batches());
}
...
template <typename VectorType>
inline void
MatrixFree<dim, Number, VectorizedArrayType>::initialize_dof_vector(VectorType &vec, const unsigned int comp) const
{
  static_assert(IsBlockVector<VectorType>::value == false, "This function is not supported for block vectors.");

  Assert(task_info.n_procs == 1, ExcMessage("This function can only be used in serial."));
...
template <typename Number2>
inline void
MatrixFree<dim, Number, VectorizedArrayType>::initialize_dof_vector(LinearAlgebra::distributed::Vector<Number2> &vec,
...
MatrixFree<dim, Number, VectorizedArrayType>::get_vector_partitioner(const unsigned int comp) const
{
  AssertIndexRange(comp, n_components());
...
MatrixFree<dim, Number, VectorizedArrayType>::get_constrained_dofs(const unsigned int comp) const
{
  AssertIndexRange(comp, n_components());
...
template <int dim, typename Number, typename VectorizedArrayType>
inline unsigned int
MatrixFree<dim, Number, VectorizedArrayType>::n_base_elements(const unsigned int dof_no) const
{
  AssertDimension(dof_handlers.size(), dof_info.size());
...
MatrixFree<dim, Number, VectorizedArrayType>::get_boundary_id(const unsigned int face_batch_index) const
{
...
MatrixFree<dim, Number, VectorizedArrayType>::get_faces_by_cells_boundary_id(const unsigned int cell_batch_index,
                                                                             const unsigned int face_number) const
{
  AssertIndexRange(cell_batch_index, n_cell_batches());
  AssertIndexRange(face_number, GeometryInfo<dim>::faces_per_cell);
  Assert(face_info.cell_and_face_boundary_id.size(0) >= n_cell_batches(), ExcNotInitialized());
  std::array<types::boundary_id, VectorizedArrayType::size()> result;
  result.fill(numbers::invalid_boundary_id);
  for (unsigned int v = 0; v < n_active_entries_per_cell_batch(cell_batch_index); ++v)
    result[v] = face_info.cell_and_face_boundary_id(cell_batch_index, face_number, v);
  return result;
}
...
template <int dim, typename Number, typename VectorizedArrayType>
inline const internal::MatrixFreeFunctions::MappingInfo<dim, Number, VectorizedArrayType> &
...
MatrixFree<dim, Number, VectorizedArrayType>::get_dof_info(const unsigned int dof_index) const
{
...
template <int dim, typename Number, typename VectorizedArrayType>
inline const Number *
MatrixFree<dim, Number, VectorizedArrayType>::constraint_pool_begin(const unsigned int row) const
{
...
template <int dim, typename Number, typename VectorizedArrayType>
inline const Number *
MatrixFree<dim, Number, VectorizedArrayType>::constraint_pool_end(const unsigned int row) const
{
  AssertIndexRange(row, constraint_pool_row_index.size() - 1);
...
template <int dim, typename Number, typename VectorizedArrayType>
inline bool
MatrixFree<dim, Number, VectorizedArrayType>::at_irregular_cell(const unsigned int cell_batch_index) const
{
  AssertIndexRange(cell_batch_index, task_info.cell_partition_data.back());
  return VectorizedArrayType::size() > 1 &&
         cell_level_index[(cell_batch_index + 1) * VectorizedArrayType::size() - 1] ==
           cell_level_index[(cell_batch_index + 1) * VectorizedArrayType::size() - 2];
}
  ...
  if (is_interior_face)
    {
      const unsigned int index =
        fe_indices[face_info.faces[range.first].cells_interior[0] / VectorizedArrayType::size()];

      for (unsigned int i = range.first; i < range.second; ++i)
        AssertDimension(index, fe_indices[face_info.faces[i].cells_interior[0] / VectorizedArrayType::size()]);
    ...
    {
      const unsigned int index =
        fe_indices[face_info.faces[range.first].cells_exterior[0] / VectorizedArrayType::size()];

      for (unsigned int i = range.first; i < range.second; ++i)
        AssertDimension(index, fe_indices[face_info.faces[i].cells_exterior[0] / VectorizedArrayType::size()]);
...
template <int dim, typename Number, typename VectorizedArrayType>
inline unsigned int
MatrixFree<dim, Number, VectorizedArrayType>::n_active_entries_per_cell_batch(const unsigned int cell_batch_index) const
{
  Assert(!dof_info.empty(), ExcNotInitialized());
  AssertIndexRange(cell_batch_index, task_info.cell_partition_data.back());
  const std::vector<unsigned char> &n_lanes_filled =
    dof_info[0].n_vectorization_lanes_filled[internal::MatrixFreeFunctions::DoFInfo::dof_access_cell];
...
template <int dim, typename Number, typename VectorizedArrayType>
inline unsigned int
MatrixFree<dim, Number, VectorizedArrayType>::n_active_entries_per_face_batch(const unsigned int face_batch_index) const
{
  AssertIndexRange(face_batch_index, face_info.faces.size());
  Assert(!dof_info.empty(), ExcNotInitialized());
  const std::vector<unsigned char> &n_lanes_filled =
    dof_info[0].n_vectorization_lanes_filled[internal::MatrixFreeFunctions::DoFInfo::dof_access_face_interior];
...
template <int dim, typename Number, typename VectorizedArrayType>
inline unsigned int
MatrixFree<dim, Number, VectorizedArrayType>::get_dofs_per_cell(const unsigned int dof_handler_index,
...
template <int dim, typename Number, typename VectorizedArrayType>
inline unsigned int
MatrixFree<dim, Number, VectorizedArrayType>::get_n_q_points(const unsigned int quad_index,
                                                             const unsigned int active_fe_index) const
{
...
template <int dim, typename Number, typename VectorizedArrayType>
inline unsigned int
MatrixFree<dim, Number, VectorizedArrayType>::get_dofs_per_face(const unsigned int dof_handler_index,
                                                                const unsigned int active_fe_index) const
{
...
template <int dim, typename Number, typename VectorizedArrayType>
inline unsigned int
MatrixFree<dim, Number, VectorizedArrayType>::get_n_q_points_face(const unsigned int quad_index,
                                                                  const unsigned int active_fe_index) const
{
...
MatrixFree<dim, Number, VectorizedArrayType>::get_locally_owned_set(const unsigned int dof_handler_index) const
{
...
template <int dim, typename Number, typename VectorizedArrayType>
inline const IndexSet &
MatrixFree<dim, Number, VectorizedArrayType>::get_ghost_set(const unsigned int dof_handler_index) const
{
...
MatrixFree<dim, Number, VectorizedArrayType>::get_shape_info(const unsigned int dof_handler_index,
...
template <int dim, typename Number, typename VectorizedArrayType>
inline const internal::MatrixFreeFunctions::FaceToCellTopology<VectorizedArrayType::size()> &
MatrixFree<dim, Number, VectorizedArrayType>::get_face_info(const unsigned int face_batch_index) const
{
  AssertIndexRange(face_batch_index, face_info.faces.size());
...
MatrixFree<dim, Number, VectorizedArrayType>::get_cell_and_face_to_plain_faces() const
{
...
MatrixFree<dim, Number, VectorizedArrayType>::get_quadrature(const unsigned int quad_index,
                                                             const unsigned int active_fe_index) const
{
  AssertIndexRange(quad_index, mapping_info.cell_data.size());
...
MatrixFree<dim, Number, VectorizedArrayType>::get_face_quadrature(const unsigned int quad_index,
                                                                  const unsigned int active_fe_index) const
{
  AssertIndexRange(quad_index, mapping_info.face_data.size());
...
template <int dim, typename Number, typename VectorizedArrayType>
inline unsigned int
MatrixFree<dim, Number, VectorizedArrayType>::get_cell_category(const unsigned int cell_batch_index) const
{
...
template <int dim, typename Number, typename VectorizedArrayType>
inline std::pair<unsigned int, unsigned int>
MatrixFree<dim, Number, VectorizedArrayType>::get_face_category(const unsigned int face_batch_index) const
{
  AssertIndexRange(face_batch_index, face_info.faces.size());
  if (dof_info[0].cell_active_fe_index.empty())
  ...
  for (unsigned int v = 0; v < VectorizedArrayType::size() &&
                           face_info.faces[face_batch_index].cells_interior[v] != numbers::invalid_unsigned_int;
       ++v)
    result.first = std::max(result.first,
                            dof_info[0].cell_active_fe_index[face_info.faces[face_batch_index].cells_interior[v] /
                                                             VectorizedArrayType::size()]);
  if (face_info.faces[face_batch_index].cells_exterior[0] != numbers::invalid_unsigned_int)
    for (unsigned int v = 0; v < VectorizedArrayType::size() &&
                             face_info.faces[face_batch_index].cells_exterior[v] != numbers::invalid_unsigned_int;
         ++v)
      result.second = std::max(result.second,
                               dof_info[0].cell_active_fe_index[face_info.faces[face_batch_index].cells_exterior[v] /
                                                                VectorizedArrayType::size()]);
  else
...
{
  using list_type = std::list<std::pair<bool, AlignedVector<VectorizedArrayType>>>;
...
template <int dim, typename Number, typename VectorizedArrayType>
AlignedVector<Number> *
MatrixFree<dim, Number, VectorizedArrayType>::acquire_scratch_data_non_threadsafe() const
{
  for (typename std::list<std::pair<bool, AlignedVector<Number>>>::iterator it = scratch_pad_non_threadsafe.begin();
...
template <int dim, typename Number, typename VectorizedArrayType>
void
MatrixFree<dim, Number, VectorizedArrayType>::release_scratch_data_non_threadsafe(
  ...
  const typename MatrixFree<dim, Number, VectorizedArrayType>::AdditionalData &additional_data)
{
  ...
  std::vector<IndexSet> locally_owned_sets =
    internal::MatrixFreeImplementation::extract_locally_owned_index_sets(dof_handlers, additional_data.mg_level);
...
{
  std::vector<IndexSet> locally_owned_set =
    internal::MatrixFreeImplementation::extract_locally_owned_index_sets(dof_handler, additional_data.mg_level);
  std::vector<hp::QCollection<dim>> quad_hp;
  quad_hp.emplace_back(quad);
...
namespace internal
{
  ...
  struct VectorDataExchange
  {
    ...
    VectorDataExchange(
      ...
      const typename dealii::MatrixFree<dim, Number, VectorizedArrayType>::DataAccessOnFaces vector_face_access,
      const unsigned int                                                                     n_components)
      : matrix_free(matrix_free)
      , vector_face_access(matrix_free.get_task_info().face_partition_data.empty() ?
                             dealii::MatrixFree<dim, Number, VectorizedArrayType>::DataAccessOnFaces::unspecified :
                             vector_face_access)
...
#  endif
    {
      ...
      if (this->vector_face_access !=
        ...
        for (unsigned int c = 0; c < matrix_free.n_components(); ++c)
          AssertDimension(matrix_free.get_dof_info(c).vector_exchanger_face_variants.size(), 5);
    }
...
#  endif
    ...
    template <typename VectorType>
    unsigned int
    find_vector_in_mf(const VectorType &vec, const bool check_global_compatibility = true) const
    {
      // case 1: vector was set up with MatrixFree::initialize_dof_vector()
      for (unsigned int c = 0; c < matrix_free.n_components(); ++c)
        if (vec.get_partitioner().get() == matrix_free.get_dof_info(c).vector_partitioner.get())
          return c;
      ...
      Assert(false, ExcNotImplemented("Could not find partitioner that fits vector"));
    ...
    {
      AssertDimension(matrix_free.get_dof_info(mf_component).vector_exchanger_face_variants.size(), 5);
      if (vector_face_access == dealii::MatrixFree<dim, Number, VectorizedArrayType>::DataAccessOnFaces::none)
        return *matrix_free.get_dof_info(mf_component).vector_exchanger_face_variants[0];
      else if (vector_face_access == dealii::MatrixFree<dim, Number, VectorizedArrayType>::DataAccessOnFaces::values)
        return *matrix_free.get_dof_info(mf_component).vector_exchanger_face_variants[1];
      else if (vector_face_access == dealii::MatrixFree<dim, Number, VectorizedArrayType>::DataAccessOnFaces::gradients)
        return *matrix_free.get_dof_info(mf_component).vector_exchanger_face_variants[2];
      else if (vector_face_access ==
               dealii::MatrixFree<dim, Number, VectorizedArrayType>::DataAccessOnFaces::values_all_faces)
        return *matrix_free.get_dof_info(mf_component).vector_exchanger_face_variants[3];
      else if (vector_face_access ==
               dealii::MatrixFree<dim, Number, VectorizedArrayType>::DataAccessOnFaces::gradients_all_faces)
        return *matrix_free.get_dof_info(mf_component).vector_exchanger_face_variants[4];
      else
     ...
     */
    template <typename VectorType, std::enable_if_t<is_not_parallel_vector<VectorType>, VectorType> * = nullptr>
    void
    update_ghost_values_start(const unsigned int /*component_in_block_vector*/, const VectorType & /*vec*/)
    {}
     ...
     */
    template <typename VectorType,
              std::enable_if_t<!has_update_ghost_values_start<VectorType> && !is_not_parallel_vector<VectorType>,
                               VectorType> * = nullptr>
    void
    update_ghost_values_start(const unsigned int component_in_block_vector, const VectorType &vec)
    {
      ...
      Assert(matrix_free.get_task_info().allow_ghosted_vectors_in_loops || ghosts_set == false, ExcNotImplemented());
     ...
     */
    template <typename VectorType,
              std::enable_if_t<has_update_ghost_values_start<VectorType> && !has_exchange_on_subset<VectorType>,
                               VectorType> * = nullptr>
    void
    update_ghost_values_start(const unsigned int component_in_block_vector, const VectorType &vec)
    {
...
#  ifdef DEAL_II_WITH_MPI
          ...
          if (part.n_ghost_indices() == 0 && part.n_import_indices() == 0 && part.n_import_sm_procs() == 0)
            return;
          ...
          part.export_to_ghosted_array_start(
            ...
            ArrayView<Number>(const_cast<Number *>(vec.begin()) + part.locally_owned_size(),
                              matrix_free.get_dof_info(mf_component).vector_partitioner->n_ghost_indices()),
            ArrayView<Number>(tmp_data[component_in_block_vector]->begin(), part.n_import_indices()),
...
#  endif
     ...
     */
    template <typename VectorType, std::enable_if_t<!has_update_ghost_values_start<VectorType>, VectorType> * = nullptr>
    void
    update_ghost_values_finish(const unsigned int /*component_in_block_vector*/, const VectorType & /*vec*/)
    {}
...
#  endif
    ...
    template <typename VectorType, std::enable_if_t<is_not_parallel_vector<VectorType>, VectorType> * = nullptr>
    void
    compress_start(const unsigned int /*component_in_block_vector*/, VectorType & /*vec*/)
    {}
    ...
    template <
      typename VectorType,
      std::enable_if_t<!has_compress_start<VectorType> && !is_not_parallel_vector<VectorType>, VectorType> * = nullptr>
    void
    compress_start(const unsigned int component_in_block_vector, VectorType &vec)
    {
      (void)component_in_block_vector;
    ...
    template <
      typename VectorType,
      std::enable_if_t<has_compress_start<VectorType> && !has_exchange_on_subset<VectorType>, VectorType> * = nullptr>
    void
    compress_start(const unsigned int component_in_block_vector, VectorType &vec)
    {
      (void)component_in_block_vector;
...
#  endif
     ...
     * the split into _start() and finish() stages and serial vectors
     */
    template <typename VectorType, std::enable_if_t<!has_compress_start<VectorType>, VectorType> * = nullptr>
    void
    compress_finish(const unsigned int /*component_in_block_vector*/, VectorType & /*vec*/)
    {}
...
#  ifdef DEAL_II_WITH_MPI
          ...
          if (part.n_ghost_indices() != 0 || part.n_import_indices() != 0 || part.n_import_sm_procs() != 0)
            {
              part.import_from_ghosted_array_finish(
                ...
                ArrayView<Number>(vec.begin() + part.locally_owned_size(),
                                  matrix_free.get_dof_info(mf_component).vector_partitioner->n_ghost_indices()),
                ArrayView<const Number>(tmp_data[component_in_block_vector]->begin(), part.n_import_indices()),
                this->requests[component_in_block_vector]);
          ...
          if (Utilities::MPI::job_supports_mpi())
            {
              const int ierr = MPI_Barrier(matrix_free.get_task_info().communicator_sm);
              AssertThrowMPI(ierr);
            }
#  endif
        }
     ...
     */
    template <typename VectorType,
              std::enable_if_t<!has_exchange_on_subset<VectorType> && !is_not_parallel_vector<VectorType>, VectorType>
                * = nullptr>
    void
    reset_ghost_values(const VectorType &vec) const
    {
     ...
     */
    template <typename VectorType, std::enable_if_t<has_exchange_on_subset<VectorType>, VectorType> * = nullptr>
    void
    reset_ghost_values(const VectorType &vec) const
    {
      static_assert(std::is_same_v<Number, typename VectorType::value_type>,
          ...
          if (part.n_ghost_indices() > 0)
            {
              part.reset_ghost_values(
                ArrayView<Number>(const_cast<LinearAlgebra::distributed::Vector<Number> &>(vec).begin() +
...
#  endif
    ...
    zero_vector_region(const unsigned int range_index, VectorType &vec) const
    {
      ...
      if (range_index == numbers::invalid_unsigned_int)
      ...
      else
        {
          ...
          const internal::MatrixFreeFunctions::DoFInfo &dof_info     = matrix_free.get_dof_info(mf_component);
          ...
          for (unsigned int id = dof_info.vector_zero_range_list_index[range_index];
            ...
            std::memset(vec.begin() + dof_info.vector_zero_range_list[id].first,
                        0,
                        (dof_info.vector_zero_range_list[id].second - dof_info.vector_zero_range_list[id].first) *
                          sizeof(Number));
        }
    }
     ...
     */
    template <typename VectorType,
              std::enable_if_t<!has_exchange_on_subset<VectorType>, VectorType> * = nullptr,
...
#  endif
  ...
  template <typename VectorStruct,
            std::enable_if_t<!has_communication_block_size<VectorStruct>, VectorStruct> * = nullptr>
    ...
    if (get_communication_block_size(vec) < vec.n_blocks())
      {
        ...
        Assert(exchanger.matrix_free.get_task_info().allow_ghosted_vectors_in_loops || ghosts_set == false,
               ExcNotImplemented());
  ...
  template <int dim, typename VectorStruct, typename Number, typename VectorizedArrayType>
  inline void
  ...
  template <int dim, typename VectorStruct, typename Number, typename VectorizedArrayType>
  inline void
  compress_start(std::vector<VectorStruct> &vec, VectorDataExchange<dim, Number, VectorizedArrayType> &exchanger)
  {
    unsigned int component_index = 0;
    for (unsigned int comp = 0; comp < vec.size(); ++comp)
      {
  ...
  template <int dim, typename VectorStruct, typename Number, typename VectorizedArrayType>
  inline void
  compress_finish(std::vector<VectorStruct> &vec, VectorDataExchange<dim, Number, VectorizedArrayType> &exchanger)
  {
    unsigned int component_index = 0;
    for (unsigned int comp = 0; comp < vec.size(); ++comp)
      {
  ...
  template <int dim,
            ...
            std::enable_if_t<IsBlockVector<VectorStruct>::value, VectorStruct> * = nullptr>
  inline void
  reset_ghost_values(const VectorStruct &vec, VectorDataExchange<dim, Number, VectorizedArrayType> &exchanger)
  {
  ...
  inline void
  apply_operation_to_constrained_dofs(const std::vector<unsigned int> &, const VectorStruct1 &, VectorStruct2 &)
  {}

  template <typename Number>
  inline void
  apply_operation_to_constrained_dofs(const std::vector<unsigned int>                  &constrained_dofs,
  ...
  namespace MatrixFreeFunctions
  {
    ...
    {
      using function_type = void (Container::*)(const MF &,
                                                OutVector &,
                                                const InVector &,
                                                const std::pair<unsigned int, unsigned int> &) const;
    };
  ...
  template <typename MF, typename InVector, typename OutVector, typename Container, bool is_constant>
  class MFWorker : public MFWorkerInterface
  {
  public:
    // An alias to make the arguments further down more readable
    using function_type =
      typename MatrixFreeFunctions::InterfaceSelector<MF, InVector, OutVector, Container, is_constant>::function_type;
    ...
    cell(const std::pair<unsigned int, unsigned int> &cell_range) override
    {
      if (cell_function != nullptr && cell_range.second > cell_range.first)
        for (unsigned int i = 0; i < matrix_free.n_active_fe_indices(); ++i)
          {
            const auto cell_subrange = matrix_free.create_cell_subrange_hp_by_index(cell_range, i);

            if (cell_subrange.second <= cell_subrange.first)
              continue;

            (container.*cell_function)(matrix_free, this->dst, this->src, cell_subrange);
          }
    }
      ...
      for (unsigned int i = ptr[range_index]; i < ptr[range_index + 1]; ++i)
        {
          ...
          (container.*fu)(matrix_free, this->dst, this->src, std::make_pair(data[2 * i], data[2 * i + 1]));
        }
    }
  ...
  public:
    ...
    cell_loop_pre_range(const unsigned int range_index) override
    {
      if (operation_before_loop)
        {
          const internal::MatrixFreeFunctions::DoFInfo &dof_info = matrix_free.get_dof_info(dof_handler_index_pre_post);
          if (range_index == numbers::invalid_unsigned_int)
            {
              // Case with threaded loop -> currently no overlap implemented
              dealii::parallel::apply_to_subranges(0U,
            ...
            {
              AssertIndexRange(range_index, dof_info.cell_loop_pre_list_index.size() - 1);
              for (unsigned int id = dof_info.cell_loop_pre_list_index[range_index];
                   id != dof_info.cell_loop_pre_list_index[range_index + 1];
                   ++id)
                operation_before_loop(dof_info.cell_loop_pre_list[id].first, dof_info.cell_loop_pre_list[id].second);
            }
        }
    }
      ...
      if (operation_after_loop)
        {
          ...
          // last range
          const std::vector<unsigned int> &partition_row_index = matrix_free.get_task_info().partition_row_index;
          if (range_index == partition_row_index[partition_row_index.size() - 2] - 1)
            apply_operation_to_constrained_dofs(matrix_free.get_constrained_dofs(dof_handler_index_pre_post), src, dst);
          ...
          if (range_index == numbers::invalid_unsigned_int)
            ...
            {
              AssertIndexRange(range_index, dof_info.cell_loop_post_list_index.size() - 1);
              for (unsigned int id = dof_info.cell_loop_post_list_index[range_index];
                   id != dof_info.cell_loop_post_list_index[range_index + 1];
                   ++id)
                operation_after_loop(dof_info.cell_loop_post_list[id].first, dof_info.cell_loop_post_list[id].second);
            }
        }
    }
  ...
  private:
    ...
    const InVector                                                                                &src;
    OutVector                                                                                     &dst;
    VectorDataExchange<MF::dimension, typename MF::value_type, typename MF::vectorized_value_type> src_data_exchanger;
    VectorDataExchange<MF::dimension, typename MF::value_type, typename MF::vectorized_value_type> dst_data_exchanger;
  ...
  {
    using function_type =
      std::function<void(const MF &, OutVector &, const InVector &, const std::pair<unsigned int, unsigned int> &)>;

    MFClassWrapper(const function_type cell, const function_type face, const function_type boundary)
      : cell(cell)
      , face(face)
      , boundary(boundary)
    {}
...
{
  using Wrapper = internal::MFClassWrapper<MatrixFree<dim, Number, VectorizedArrayType>, InVector, OutVector>;
  Wrapper wrap(cell_operation, nullptr, nullptr);
  internal::MFWorker<MatrixFree<dim, Number, VectorizedArrayType>, InVector, OutVector, Wrapper, true> worker(
    *this,
    src,
    dst,
    zero_dst_vector,
...
inline void
MatrixFree<dim, Number, VectorizedArrayType>::cell_loop(
  void (CLASS::*function_pointer)(const MatrixFree<dim, Number, VectorizedArrayType> &,
                                  OutVector &,
                                  const InVector &,
                                  const std::pair<unsigned int, unsigned int> &) const,
  const CLASS    *owning_class,
  ...
  const bool      zero_dst_vector) const
{
  internal::MFWorker<MatrixFree<dim, Number, VectorizedArrayType>, InVector, OutVector, CLASS, true> worker(
...
inline void
MatrixFree<dim, Number, VectorizedArrayType>::loop(
  void (CLASS::*cell_operation)(const MatrixFree<dim, Number, VectorizedArrayType> &,
                                OutVector &,
                                const InVector &,
                                const std::pair<unsigned int, unsigned int> &) const,
  void (CLASS::*face_operation)(const MatrixFree<dim, Number, VectorizedArrayType> &,
                                OutVector &,
                                const InVector &,
                                const std::pair<unsigned int, unsigned int> &) const,
  void (CLASS::*boundary_operation)(const MatrixFree<dim, Number, VectorizedArrayType> &,
                                    OutVector &,
                                    const InVector &,
                                    const std::pair<unsigned int, unsigned int> &) const,
  const CLASS            *owning_class,
...
MatrixFree<dim, Number, VectorizedArrayType>::cell_loop(
  ...
  const bool      zero_dst_vector) const
{
  internal::MFWorker<MatrixFree<dim, Number, VectorizedArrayType>, InVector, OutVector, CLASS, false> worker(
    *this, src, dst, zero_dst_vector, *owning_class, function_pointer, nullptr, nullptr);
  task_info.loop(worker);
}

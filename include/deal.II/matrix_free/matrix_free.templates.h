  ...
  const unsigned int                           dof_handler_index) const
{
  ...
  const std::vector<unsigned int> &fe_indices = dof_info[dof_handler_index].cell_active_fe_index;

  if (fe_indices.empty() == true || dof_handlers[dof_handler_index]->get_fe_collection().size() == 1)
    return range;
  else
    {
      ...
      return_range.first =
        std::lower_bound(fe_indices.begin() + range.first, fe_indices.begin() + range.second, fe_index) -
        fe_indices.begin();
      return_range.second =
        std::lower_bound(fe_indices.begin() + return_range.first, fe_indices.begin() + range.second, fe_index + 1) -
        fe_indices.begin();
      Assert(return_range.first >= range.first && return_range.second <= range.second, ExcInternalError());
      return return_range;
    }
}
...
namespace
{
  ...
  {
  public:
    FaceRangeCompartor(const std::vector<unsigned int> &fe_indices, const bool include, const bool only_face_type)
    ...
    bool
    operator()(const internal::MatrixFreeFunctions::FaceToCellTopology<vectorization_width> &face,
               ...
               const std::array<unsigned int, 3>                                            &fe_index)
    {
      ...
      const std::array<unsigned int, 3> fe_index_face = {{face_type,
...
template <int dim, typename Number, typename VectorizedArrayType>
void
MatrixFree<dim, Number, VectorizedArrayType>::renumber_dofs(std::vector<types::global_dof_index> &renumbering,
...
MatrixFree<dim, Number, VectorizedArrayType>::get_dof_handler(const unsigned int dof_handler_index) const
{
...
MatrixFree<dim, Number, VectorizedArrayType>::get_affine_constraints(const unsigned int dof_handler_index) const
{
  AssertIndexRange(dof_handler_index, n_components());
...
MatrixFree<dim, Number, VectorizedArrayType>::get_cell_iterator(const unsigned int cell_batch_index,
                                                                ...
                                                                const unsigned int dof_handler_index) const
{
  ...
  AssertIndexRange(lane_index, n_active_entries_per_cell_batch(cell_batch_index));

  std::pair<unsigned int, unsigned int> index =
    cell_level_index[cell_batch_index * VectorizedArrayType::size() + lane_index];
  return typename DoFHandler<dim>::cell_iterator(&dof_handlers[dof_handler_index]->get_triangulation(),
                                                 index.first,
                                                 index.second,
...
template <int dim, typename Number, typename VectorizedArrayType>
std::pair<int, int>
MatrixFree<dim, Number, VectorizedArrayType>::get_cell_level_and_index(const unsigned int cell_batch_index,
                                                                       const unsigned int lane_index) const
{
  AssertIndexRange(cell_batch_index, task_info.cell_partition_data.back());
  AssertIndexRange(lane_index, n_active_entries_per_cell_batch(cell_batch_index));

  std::pair<int, int> level_index_pair = cell_level_index[cell_batch_index * VectorizedArrayType::size() + lane_index];
...
{
  return mf_cell_indices[(this->get_mg_level() == numbers::invalid_unsigned_int) ? cell->active_cell_index() :
                                                                                   cell->index()];
}
...
MatrixFree<dim, Number, VectorizedArrayType>::get_face_iterator(const unsigned int face_batch_index,
                                                                const unsigned int lane_index,
                                                                const bool         interior,
                                                                const unsigned int fe_component) const
{
  AssertIndexRange(fe_component, dof_handlers.size());
  AssertIndexRange(face_batch_index, n_inner_face_batches() + (interior ? n_boundary_face_batches() : 0));

  AssertIndexRange(lane_index, n_active_entries_per_face_batch(face_batch_index));

  const internal::MatrixFreeFunctions::FaceToCellTopology<VectorizedArrayType::size()> face2cell_info =
    get_face_info(face_batch_index);

  const unsigned int cell_index =
    interior ? face2cell_info.cells_interior[lane_index] : face2cell_info.cells_exterior[lane_index];

  std::pair<unsigned int, unsigned int> index = cell_level_index[cell_index];

  return {typename DoFHandler<dim>::cell_iterator(
            &dof_handlers[fe_component]->get_triangulation(), index.first, index.second, &*dof_handlers[fe_component]),
          interior ? face2cell_info.interior_face_no : face2cell_info.exterior_face_no};
}
...
template <int dim, typename Number, typename VectorizedArrayType>
void
MatrixFree<dim, Number, VectorizedArrayType>::copy_from(const MatrixFree<dim, Number, VectorizedArrayType> &v)
{
  clear();
...
namespace internal
{
  ...
  void
  store_affine_constraints(const dealii::AffineConstraints<Number2> *,
  ...
  void
  store_affine_constraints(const dealii::AffineConstraints<Number>               *affine_constraints,
                           SmartPointer<const dealii::AffineConstraints<Number>> &stored_constraints)
  {
...
MatrixFree<dim, Number, VectorizedArrayType>::internal_reinit(
  ...
  const typename MatrixFree<dim, Number, VectorizedArrayType>::AdditionalData &additional_data)
{
  ...
  // gradients and Hessians for quadrature points.
  {
    ...
    for (unsigned int no = 0; no < dof_handler.size(); ++no)
      n_fe_in_collection = std::max(n_fe_in_collection, dof_handler[no]->get_fe_collection().size());
        ...
        for (unsigned int fe_no = 0; fe_no < dof_handler[no]->get_fe_collection().size(); ++fe_no)
          for (unsigned int nq = 0; nq < n_quad; ++nq)
            for (unsigned int q_no = 0; q_no < quad[nq].size(); ++q_no)
              shape_info(c, nq, fe_no, q_no).reinit(quad[nq][q_no], dof_handler[no]->get_fe(fe_no), b);
  }
  ...
  if (additional_data.initialize_indices == true)
    {
      ...
      task_info.allow_ghosted_vectors_in_loops = additional_data.allow_ghosted_vectors_in_loops;
      ...
      for (const auto &constraint : constraints)
        Assert(constraint->is_closed(task_info.communicator),
               ExcMessage("You have provided a non-empty AffineConstraints object that has not "
      ...
      for (unsigned int no = 0; no < dof_handler.size(); ++no)
        {
          ...
          dof_info[no].global_base_element_offset =
            no > 0 ? dof_info[no - 1].global_base_element_offset + dof_handler[no - 1]->get_fe(0).n_base_elements() : 0;
        }
      ...
      if (additional_data.tasks_parallel_scheme != AdditionalData::none && MultithreadInfo::n_threads() > 1)
        {
  ...
  // initialize bare structures
  else if (dof_info.size() != dof_handler.size())
    {
      ...
      task_info.create_blocks_serial(dummy, 1, false, dummy, false, dummy, dummy, dummy2);
      ...
      for (unsigned int i = 0; i < dof_info.size(); ++i)
        {
          ...
          dof_info[i].dofs_per_cell.push_back(dof_handler[i]->get_fe(0).n_dofs_per_cell());
          ...
          dof_info[i].n_vectorization_lanes_filled[internal::MatrixFreeFunctions::DoFInfo::dof_access_cell].resize(
            cell_level_index.size() / VectorizedArrayType::size(), VectorizedArrayType::size());
          if (n_regular_cells < cell_level_index.size())
            dof_info[i].n_vectorization_lanes_filled[internal::MatrixFreeFunctions::DoFInfo::dof_access_cell]
  ...
  if (task_info.scheme != internal::MatrixFreeFunctions::TaskInfo::TasksParallelScheme::partition_color)
    {
      // ... for cells
      {
        ...
        for (unsigned int i = 0; i < task_info.cell_partition_data.size() - 1; ++i)
          {
            if (task_info.cell_partition_data[i + 1] > task_info.cell_partition_data[i])
              {
                std::pair<unsigned int, unsigned int> range{task_info.cell_partition_data[i],
                                                            task_info.cell_partition_data[i + 1]};

                if (range.second > range.first)
                  for (unsigned int i = 0; i < this->n_active_fe_indices(); ++i)
                    {
                      const auto cell_subrange = this->create_cell_subrange_hp_by_index(range, i);
      ...
      if (task_info.face_partition_data.empty() == false)
        {
          ...
          const auto create_inner_face_subrange_hp_by_index = [&](const std::pair<unsigned int, unsigned int> &range,
            ...
            if (fe_indices.empty() == true && n_face_types == 1)
              ...
              {
                const bool only_face_type = dof_info[dof_handler_index].max_fe_index == 0 ||
                                            dof_handlers[dof_handler_index]->get_fe_collection().size() == 1 ||
                ...
                return_range.first =
                  std::lower_bound(face_info.faces.begin() + range.first,
                                   ...
                                   std::array<unsigned int, 3>{{face_type, fe_index_interior, fe_index_exterior}},
          ...
          for (unsigned int i = 0; i < task_info.face_partition_data.size() - 1; ++i)
            {
              if (task_info.face_partition_data[i + 1] > task_info.face_partition_data[i])
                {
                  std::pair<unsigned int, unsigned int> range{task_info.face_partition_data[i],
                                                              task_info.face_partition_data[i + 1]};

                  if (range.second > range.first)
                    for (unsigned int t = 0; t < std::max<unsigned int>(dim - 1, 1); ++t)
                      for (unsigned int i = 0; i < this->n_active_fe_indices(); ++i)
                        for (unsigned int j = 0; j < this->n_active_fe_indices(); ++j)
                          {
                            const auto subrange = create_inner_face_subrange_hp_by_index(range, t, i, j);
      ...
      if (task_info.boundary_partition_data.empty() == false)
        {
          ...
          const auto create_boundary_face_subrange_hp_by_index = [&](const std::pair<unsigned int, unsigned int> &range,
                                                                     const unsigned int face_type,
                                                                     const unsigned int fe_index,
                                                                     const unsigned int dof_handler_index =
                                                                       0) -> std::pair<unsigned int, unsigned int> {
            ...
            if (fe_indices.empty() == true && n_face_types == 1)
            ...
            else
              {
                ...
                std::pair<unsigned int, unsigned int> return_range;
                return_range.first = std::lower_bound(face_info.faces.begin() + range.first,
                                                      face_info.faces.begin() + range.second,
                                                      std::array<unsigned int, 2>{{face_type, fe_index}},
                                                      FaceRangeCompartor(fe_indices, false, only_face_type)) -
                                     face_info.faces.begin();
                return_range.second = std::lower_bound(face_info.faces.begin() + return_range.first,
                                                       face_info.faces.begin() + range.second,
                                                       std::array<unsigned int, 2>{{face_type, fe_index}},
                                                       FaceRangeCompartor(fe_indices, true, only_face_type)) -
                                      face_info.faces.begin();
          ...
          for (unsigned int i = 0; i < task_info.boundary_partition_data.size() - 1; ++i)
            {
              if (task_info.boundary_partition_data[i + 1] > task_info.boundary_partition_data[i])
                {
                  std::pair<unsigned int, unsigned int> range{task_info.boundary_partition_data[i],
                  ...
                  if (range.second > range.first)
                    for (unsigned int t = 0; t < std::max<unsigned int>(dim - 1, 1); ++t)
                      for (unsigned int i = 0; i < this->n_active_fe_indices(); ++i)
                        {
                          const auto cell_subrange = create_boundary_face_subrange_hp_by_index(range, t, i);
  ...
  if (additional_data.initialize_mapping == true)
    {
      if (dof_handler.size() > 1)
        {
          ...
          for (unsigned int i = 1; i < dof_handler.size(); ++i)
            {
              Assert(dof_handler[0]->has_hp_capabilities() == dof_handler[i]->has_hp_capabilities(),
                     ExcNotImplemented());

              if (dof_handler[0]->has_hp_capabilities())
                {
                  Assert(dof_info[0].cell_active_fe_index == dof_info[i].cell_active_fe_index, ExcNotImplemented());
                }
            }
        }
      ...
      mapping_info.initialize(dof_handler[0]->get_triangulation(),
                              cell_level_index,
                              face_info,
                              dof_handler[0]->has_hp_capabilities() ? dof_info[0].cell_active_fe_index :
  ...
  // set up map: deal.II index -> MatrixFree index
  {
    ...
    for (unsigned int cell = 0; cell < n_cell_batches() + n_ghost_cell_batches(); ++cell)
      for (unsigned int v = 0; v < n_active_entries_per_cell_batch(cell); ++v)
        {
          const auto tria_cell = get_cell_iterator(cell, v);
          mf_cell_indices[(mg_level == numbers::invalid_unsigned_int) ? tria_cell->active_cell_index() :
                                                                        tria_cell->index()] =
...
template <int dim, typename Number, typename VectorizedArrayType>
void
MatrixFree<dim, Number, VectorizedArrayType>::update_mapping(const Mapping<dim> &mapping)
{
...
template <int dim, typename Number, typename VectorizedArrayType>
void
MatrixFree<dim, Number, VectorizedArrayType>::update_mapping(const std::shared_ptr<hp::MappingCollection<dim>> &mapping)
{
  AssertDimension(shape_info.size(1), mapping_info.cell_data.size());
  mapping_info.update_mapping(
    dof_handlers[0]->get_triangulation(), cell_level_index, face_info, dof_info[0].cell_active_fe_index, mapping);
}
...
bool
MatrixFree<dim, Number, VectorizedArrayType>::is_supported(const FiniteElement<dim, spacedim> &fe)
{
...
namespace internal
{
  namespace MatrixFreeFunctions
  {
    ...
    void
    resolve_cell(const InIterator &cell, std::vector<std::pair<unsigned int, unsigned int>> &cell_its)
    {
  ...
  if (level == numbers::invalid_unsigned_int)
    {
      ...
      Assert(task_info.n_procs > 1 || cell_level_index.size() == tria.n_active_cells(), ExcInternalError());
    }
  else
    {
...
#  endif
                             ...
                             DynamicSparsityPattern                &connectivity_direct)
  {
    ...
    for (unsigned int cell = begin; cell < end; ++cell)
      {
        new_indices.clear();
        typename dealii::Triangulation<dim>::cell_iterator dcell(&tria,
                                                                 cell_level_index[cell].first,
                                                                 cell_level_index[cell].second);
        for (auto f : dcell->face_indices())
          {
            // Only inner faces couple different cells
            if (dcell->at_boundary(f) == false &&
                dcell->neighbor_or_periodic_neighbor(f)->level_subdomain_id() == dcell->level_subdomain_id())
              {
                std::pair<unsigned int, unsigned int> level_index(dcell->neighbor_or_periodic_neighbor(f)->level(),
    ...
    for (unsigned int block = begin; block < end; ++block)
      {
        ...
        std::sort(new_indices.begin(), new_indices.end());
        connectivity.add_entries(block, new_indices.begin(), std::unique(new_indices.begin(), new_indices.end()), true);
      }
  }
  ...
  std::vector<bool>
  compute_dof_info(const std::vector<const dealii::AffineConstraints<number> *> &constraint,
                   const std::vector<IndexSet>                                  &locally_owned_dofs,
                   ...
                   const bool                                                    use_vector_data_exchanger_full)
  {
    ...
    std::vector<std::vector<std::vector<unsigned int>>> lexicographic(n_dof_handlers);
    ...
    if (use_fast_hanging_node_algorithm)
      for (unsigned int no = 0; no < n_dof_handlers; ++no)
        {
          const dealii::hp::FECollection<dim> &fes = dof_handlers[no]->get_fe_collection();
    ...
    for (unsigned int no = 0; no < n_dof_handlers; ++no)
      {
        ...
        for (unsigned int fe_index = 0; fe_index < fes.size(); ++fe_index)
          {
            ...
            dof_info[no].dofs_per_face.push_back(
              fe.n_dofs_per_face(0)); // we assume that all faces have the same number of dofs
            ...
            for (unsigned int c = 0; c < dof_info[no].n_base_elements; ++c)
              {
                dof_info[no].n_components[c] = fe.element_multiplicity(c) * fe.base_element(c).n_components();
                for (unsigned int l = 0; l < dof_info[no].n_components[c]; ++l)
                  {
                    ...
                    dof_info[no].component_dof_indices_offset[fe_index].push_back(
                      ...
                      shape_infos(dof_info[no].global_base_element_offset + c, fe_index).dofs_per_component_on_cell);
                ...
                const auto &lex =
                  shape_infos(dof_info[no].global_base_element_offset + c, fe_index).lexicographic_numbering;
                lexicographic[no][fe_index].insert(lexicographic[no][fe_index].end(), lex.begin(), lex.end());
              }
        ...
        dof_info[no].vector_partitioner =
          std::make_shared<Utilities::MPI::Partitioner>(locally_owned_dofs[no], task_info.communicator);

        if (use_vector_data_exchanger_full == false)
          dof_info[no].vector_exchanger =
            std::make_shared<internal::MatrixFreeFunctions::VectorDataExchange::PartitionerWrapper>(
              dof_info[no].vector_partitioner);
        else
          dof_info[no].vector_exchanger =
            std::make_shared<internal::MatrixFreeFunctions::VectorDataExchange::Full>(dof_info[no].vector_partitioner,
        ...
        for (const auto &line : constraint[no]->get_lines())
          if (dof_info[no].vector_partitioner->in_local_range(line.index))
            dof_info[no].constrained_dofs.push_back(dof_info[no].vector_partitioner->global_to_local(line.index));
      }

    // extract all the global indices associated with the computation, and form
    // the ghost indices
    std::vector<unsigned int>                                                 subdomain_boundary_cells;
    std::unique_ptr<dealii::internal::MatrixFreeFunctions::HangingNodes<dim>> hanging_nodes;

    if (dim > 1 && mg_level == numbers::invalid_unsigned_int)
      {
        hanging_nodes = std::make_unique<dealii::internal::MatrixFreeFunctions::HangingNodes<dim>>(tria);
        for (unsigned int no = 0; no < n_dof_handlers; ++no)
          {
            dof_info[no].hanging_node_constraint_masks.resize(n_active_cells);

            dof_info[no].hanging_node_constraint_masks_comp =
              hanging_nodes->compute_supported_components(dof_handlers[no]->get_fe_collection());

            if ([](const auto &supported_components) {
                  return std::none_of(supported_components.begin(), supported_components.end(), [](const auto &a) {
    ...
    for (unsigned int counter = 0; counter < n_active_cells; ++counter)
      {
        bool cell_at_subdomain_boundary =
          (face_setup.at_processor_boundary.size() > counter && face_setup.at_processor_boundary[counter]) ||
        ...
        for (unsigned int no = 0; no < n_dof_handlers; ++no)
          {
            ...
            if (mg_level == numbers::invalid_unsigned_int)
              {
                typename DoFHandler<dim>::active_cell_iterator cell_it(&tria,
                                                                       cell_level_index[counter].first,
                                                                       cell_level_index[counter].second,
                                                                       &dofh);
                const unsigned int fe_index      = dofh.get_fe_collection().size() > 1 ? cell_it->active_fe_index() : 0;
                ...
                if (dofh.get_fe_collection().size() > 1)
                  ...
                  {
                    AssertIndexRange(cell_it->active_cell_index(), cell_vectorization_category.size());
                ...
                for (unsigned int i = 0; i < dofs_per_cell; ++i)
                  local_dof_indices[i] = local_dof_indices_resolved[lexicographic[no][fe_index][i]];

                if (dim > 1 && use_fast_hanging_node_algorithm)
                  {
                    local_dof_indices_resolved = local_dof_indices;

                    cell_has_hanging_node_constraints = dof_info[no].process_hanging_node_constraints(
                      *hanging_nodes, lexicographic[no], counter, cell_it, local_dof_indices_resolved);
                  }
              }
            // we are requested to use a multigrid level
            else
              {
                ...
                typename DoFHandler<dim>::cell_iterator cell_it(&tria,
                                                                cell_level_index[counter].first,
                                                                cell_level_index[counter].second,
                                                                &dofh);
                ...
                if (cell_categorization_enabled)
                  {
                    AssertIndexRange(cell_it->index(), cell_vectorization_category.size());
                    dof_info[no].cell_active_fe_index[counter] =
                ...
                local_dof_indices.resize(dofs_per_cell);
                for (unsigned int i = 0; i < dofs_per_cell; ++i)
                  local_dof_indices[i] = local_dof_indices_resolved[lexicographic[no][0][i]];
              }

            dof_info[no].read_dof_indices(cell_has_hanging_node_constraints ? local_dof_indices_resolved :
                                                                              local_dof_indices,
                                          local_dof_indices,
      ...
      for (unsigned int no = 0; no < n_dof_handlers; ++no)
        {
          ...
          if (std::all_of(vec.begin(), vec.end(), [](const auto i) {
                return i == internal::MatrixFreeFunctions::unconstrained_compressed_constraint_kind;
              }))
            vec.clear();
        }
          ...
          if (do_face_integrals && mg_level != numbers::invalid_unsigned_int)
            {
              ...
              if (mg_level + 1 < tria.n_global_levels())
                for (const auto &cell : dof_handlers[no]->cell_iterators_on_level(mg_level + 1))
                  if (cell->level_subdomain_id() == task_info.my_pid)
                    for (const unsigned int f : cell->face_indices())
                      if ((cell->at_boundary(f) == false || cell->has_periodic_neighbor(f) == true) &&
                        ...
                        {
                          dof_indices.resize(cell->neighbor_or_periodic_neighbor(f)->get_fe().n_dofs_per_cell());
                          cell->neighbor_or_periodic_neighbor(f)->get_mg_dof_indices(dof_indices);
                          for (const auto dof_index : dof_indices)
                            dof_info[no].ghost_dofs.push_back(dof_index);
                        }
            }
          dof_info[no].assign_ghosts(cells_with_ghosts, task_info.communicator_sm, use_vector_data_exchanger_full);
        }
    }

    const bool hp_functionality_enabled =
      std::any_of(dof_handlers.begin(), dof_handlers.end(), [](const auto &dh) { return dh->has_hp_capabilities(); });

    const unsigned int         n_lanes = task_info.vectorization_length;
    std::vector<unsigned int>  renumbering;
    std::vector<unsigned char> irregular_cells;

    Assert(task_info.scheme == internal::MatrixFreeFunctions::TaskInfo::none || cell_vectorization_category.empty(),
           ExcMessage(
    ...
    if (task_info.scheme == internal::MatrixFreeFunctions::TaskInfo::none)
      {
        const bool   strict_categories = cell_vectorization_categories_strict || hp_functionality_enabled;
        ...
        std::vector<unsigned int> parent_relation(task_info.n_active_cells + task_info.n_ghost_cells,
                                                  numbers::invalid_unsigned_int);

        constexpr unsigned int max_children_per_cell = GeometryInfo<dim>::max_children_per_cell;
        const unsigned int     n_levels = mg_level == numbers::invalid_unsigned_int ? tria.n_levels() - 1 : mg_level;
        std::vector<std::vector<std::pair<unsigned int, std::array<unsigned int, max_children_per_cell>>>> cell_parents(
          n_levels);
        ...
        for (unsigned int c = 0; c < cell_level_index_end_local; ++c)
          if (cell_level_index[c].first > 0)
            {
              typename Triangulation<dim>::cell_iterator cell(&tria,
    ...
    else
      {
        ...
        unsigned int n_cell_batches_before = *(task_info.cell_partition_data.end() - 2);
        unsigned int n_ghost_slots         = *(task_info.cell_partition_data.end() - 1) - n_cell_batches_before;
            ...
            if (hp_functionality_enabled)
              {
                ...
                irregular_cells.resize(task_info.cell_partition_data.back() + 2 * dof_info[0].max_fe_index);
                ...
                for (counter = 0; counter < std::min(start_nonboundary * n_lanes, task_info.n_active_cells); counter++)
                  {
                    AssertIndexRange(counter, renumbering.size());
                    AssertIndexRange(renumbering[counter], dof_info[0].cell_active_fe_index.size());
                    renumbering_fe_index[dof_info[0].cell_active_fe_index[renumbering[counter]]].push_back(
                      renumbering[counter]);
                  }
                counter = 0;
                for (unsigned int j = 0; j < dof_info[0].max_fe_index; ++j)
                  {
                    ...
                    irregular_cells[renumbering_fe_index[j].size() / n_lanes + n_cell_batches_before] =
                      renumbering_fe_index[j].size() % n_lanes;
                    n_cell_batches_before += (renumbering_fe_index[j].size() + n_lanes - 1) / n_lanes;
                    renumbering_fe_index[j].resize(0);
                  }

                for (counter = start_nonboundary * n_lanes; counter < task_info.n_active_cells; counter++)
                  {
                ...
                *(task_info.cell_partition_data.end() - 1) = n_cell_batches_before + n_ghost_slots;
              }
          }

        task_info.n_blocks =
          (*(task_info.cell_partition_data.end() - 2) + task_info.block_size - 1) / task_info.block_size;
...
#  endif
            ...
            dealii::parallel::apply_to_subranges(
              ...
              [&cell_level_index, &map](const unsigned int begin, const unsigned int end) {
            ...
            dealii::parallel::apply_to_subranges(
              ...
              [&cell_level_index, &tria, &map, &connectivity_direct](const unsigned int begin, const unsigned int end) {
                fill_connectivity_subrange<dim>(begin, end, tria, cell_level_index, map, connectivity_direct);
              },
              20);
            ...
            dealii::parallel::apply_to_subranges(
              0,
              task_info.n_active_cells,
              [&connectivity_direct, &connectivity](const unsigned int begin, const unsigned int end) {
                fill_connectivity_indirect_subrange(begin, end, connectivity_direct, connectivity);
              },
              20);
#endif
          }
        ...
        task_info.make_thread_graph(
          dof_info[0].cell_active_fe_index, connectivity, renumbering, irregular_cells, hp_functionality_enabled);
...
#endif
    {
      ...
      for (unsigned int i = 0; i < task_info.cell_partition_data.back(); ++i)
        {
          unsigned int n_comp = (irregular_cells[i] > 0) ? irregular_cells[i] : n_lanes;
      ...
      AssertDimension(cell_level_index.size(), task_info.cell_partition_data.back() * n_lanes);
    }

    std::vector<unsigned int> constraint_pool_row_index;
    constraint_pool_row_index.resize(1, 0);
    for (const auto &it : constraint_values.constraints)
      constraint_pool_row_index.push_back(constraint_pool_row_index.back() + it.first.size());
  ...
  const AdditionalData                                  &additional_data)
{
  ...
  Table<2, internal::MatrixFreeFunctions::ShapeInfo<double>> shape_info_dummy(shape_info.size(0), shape_info.size(2));
  {
    ...
    for (unsigned int no = 0, c = 0; no < dof_handlers.size(); ++no)
      for (unsigned int b = 0; b < dof_handlers[no]->get_fe(0).n_base_elements(); ++b, ++c)
        for (unsigned int fe_no = 0; fe_no < dof_handlers[no]->get_fe_collection().size(); ++fe_no)
          shape_info_dummy(c, fe_no).reinit(dof_handlers[no]->get_fe(fe_no).reference_cell() ==
  ...
  const std::vector<bool>                                 is_fe_dg =
    internal::compute_dof_info(constraint,
                               locally_owned_dofs,
                               dof_handlers,
                               shape_info_dummy,
  ...
  if ((additional_data.mapping_update_flags_inner_faces | additional_data.mapping_update_flags_boundary_faces) !=
      update_default)
    {
      ...
      if (task_info.scheme == internal::MatrixFreeFunctions::TaskInfo::none)
        {
          ...
          if (task_info.partition_row_index[2] < task_info.face_partition_data.size())
      ...
      else
        {
          ...
          std::fill(hard_vectorization_boundary.begin(), hard_vectorization_boundary.end(), true);
        }
      ...
      internal::MatrixFreeFunctions::collect_faces_vectorization(face_setup.inner_ghost_faces,
                                                                 hard_vectorization_boundary,
                                                                 task_info.ghost_face_partition_data,
                                                                 face_info.faces,
                                                                 dof_info[0].cell_active_fe_index);
      hard_vectorization_boundary.clear();
      hard_vectorization_boundary.resize(task_info.refinement_edge_face_partition_data.size(), false);
      internal::MatrixFreeFunctions::collect_faces_vectorization(face_setup.refinement_edge_faces,
                                                                 hard_vectorization_boundary,
      ...
      cell_level_index.resize(cell_level_index.size() +
                              VectorizedArrayType::size() * (task_info.refinement_edge_face_partition_data[1] -
      ...
      face_info.cell_and_face_to_plain_faces.reinit(TableIndices<3>(task_info.cell_partition_data.back(),
                                                                    GeometryInfo<dim>::faces_per_cell,
                                                                    VectorizedArrayType::size()),
                                                    true);
      face_info.cell_and_face_to_plain_faces.fill(numbers::invalid_unsigned_int);
      face_info.cell_and_face_boundary_id.reinit(TableIndices<3>(task_info.cell_partition_data.back(),
                                                                 GeometryInfo<dim>::faces_per_cell,
                                                                 VectorizedArrayType::size()),
                                                 true);
      face_info.cell_and_face_boundary_id.fill(numbers::invalid_boundary_id);

      for (unsigned int f = 0; f < task_info.ghost_face_partition_data.back(); ++f)
        for (unsigned int v = 0;
             v < VectorizedArrayType::size() && face_info.faces[f].cells_interior[v] != numbers::invalid_unsigned_int;
             ++v)
          {
            TableIndices<3> index(face_info.faces[f].cells_interior[v] / VectorizedArrayType::size(),
            ...
            face_info.cell_and_face_to_plain_faces(index) = f * VectorizedArrayType::size() + v;
            if (face_info.faces[f].cells_exterior[v] != numbers::invalid_unsigned_int)
              {
                TableIndices<3> index(face_info.faces[f].cells_exterior[v] / VectorizedArrayType::size(),
                                      face_info.faces[f].exterior_face_no,
                                      face_info.faces[f].cells_exterior[v] % VectorizedArrayType::size());
                // Assert(cell_and_face_to_plain_faces(index) ==
                // numbers::invalid_unsigned_int,
                //       ExcInternalError("Should only visit each face once"));
                face_info.cell_and_face_to_plain_faces(index) = f * VectorizedArrayType::size() + v;
              ...
              face_info.cell_and_face_boundary_id(index) = types::boundary_id(face_info.faces[f].exterior_face_no);
          }

      // compute tighter index sets for various sets of face integrals
      unsigned int count = 0;
      for (auto &di : dof_info)
        di.compute_tight_partitioners(shape_info_dummy,
                                      *(task_info.cell_partition_data.end() - 2) * VectorizedArrayType::size(),
                                      VectorizedArrayType::size(),
    ...
    for (const auto &di : dof_info)
      {
        ...
        for (const auto &v : di.index_storage_variants[2])
          is_non_buffering_sm_supported &=
            (v == internal::MatrixFreeFunctions::DoFInfo::IndexStorageVariants::contiguous);
      }

    is_non_buffering_sm_supported =
      Utilities::MPI::min(static_cast<unsigned int>(is_non_buffering_sm_supported), task_info.communicator);
    ...
    if (is_non_buffering_sm_supported)
      {
        ...
        const auto sm_procs =
          Utilities::MPI::mpi_processes_within_communicator(task_info.communicator, communicator_sm);

        // get my rank within the shared-memory domain
        const unsigned int my_sm_pid =
          std::distance(sm_procs.begin(), std::find(sm_procs.begin(), sm_procs.end(), task_info.my_pid));
        ...
        cell_indices_contiguous_sm[2] = [&]() {
          // result
          std::vector<std::pair<unsigned int, unsigned int>> cells(di.dof_indices_contiguous[2].size(),
                                                                   {numbers::invalid_unsigned_int,
                                                                    numbers::invalid_unsigned_int});

          // locally-owned cells
          std::vector<std::pair<unsigned int, types::global_cell_index>> cells_locally_owned;
          cells_locally_owned.reserve(n_cell_batches() * n_lanes);

          // shared-ghost cells (categorized according to sm-rank)
          std::vector<std::vector<std::pair<unsigned int, types::global_cell_index>>> cells_shared_ghosts(
            sm_procs.size());

          for (unsigned int cell = 0; cell < n_cell_batches() + n_ghost_cell_batches(); ++cell)
            for (unsigned int v = 0; v < this->n_active_entries_per_cell_batch(cell); ++v)
              {
                ...
                const unsigned int local_cell_index =
                  di.dof_indices_contiguous[2][index] / this->get_dofs_per_cell(di_index);
                ...
                if (cell < n_cell_batches())
                  {
                    // locally-owned cell
                    cells_locally_owned.emplace_back(local_cell_index, global_cell_index);
                  }
                else
                  {
                    // ghost cell
                    ...
                    if (ptr != sm_procs.end())
                      {
                        ...
                        cells_shared_ghosts[sm_rank].emplace_back(index, global_cell_index);
                      }
                    else
                      {
          ...
          std::sort(cells_locally_owned.begin(), cells_locally_owned.end(), [](const auto &a, const auto &b) {
          ...
          const auto create_request = [&](const auto other_rank) {
            ...
            std::sort(source.begin(), source.end(), [](const auto &a, const auto &b) { return a.second < b.second; });
          ...
          const auto process_answer = [&](const auto other_rank, const auto &answer) {
            Assert(answer.size() == cells_shared_ghosts[other_rank].size(), ExcInternalError());
            for (unsigned int i = 0; i < answer.size(); ++i)
              {
                cells[cells_shared_ghosts[other_rank][i].first] = {other_rank, answer[i]};
              }
          };

          Utilities::MPI::ConsensusAlgorithms::Selector<std::vector<dealii::types::global_dof_index>,
                                                        std::vector<unsigned int>>()
            .run(targets, create_request, answer_request, process_answer, communicator_sm);
        ...
        for (unsigned int i = 0; i < 2; ++i)
          {
            cell_indices_contiguous_sm[i].assign(di.dof_indices_contiguous[i].size(),
                                                 {numbers::invalid_unsigned_int, numbers::invalid_unsigned_int});

            for (unsigned int face_index = 0; face_index < cell_indices_contiguous_sm[i].size(); ++face_index)
              {
                const unsigned int cell_index =
                  (i == 0) ? get_face_info(face_index / n_lanes).cells_interior[face_index % n_lanes] :
                             get_face_info(face_index / n_lanes).cells_exterior[face_index % n_lanes];

                if (cell_index == numbers::invalid_unsigned_int)
                  continue;

                cell_indices_contiguous_sm[i][face_index] = cell_indices_contiguous_sm[2][cell_index];
              }
          }
...
void
MatrixFree<dim, Number, VectorizedArrayType>::print_memory_consumption(StreamType &out) const
{
  ...
  if (Utilities::MPI::sum(face_info.faces.size(), task_info.communicator) > 0)
    {
      ...
      task_info.print_memory_statistics(out, MemoryConsumption::memory_consumption(face_info.faces));
    }

...
namespace MatrixFreeTools
{
  ...
  void
  categorize_by_boundary_ids(const Triangulation<dim> &tria, AdditionalData &additional_data);
  ...
  compute_diagonal(
    ...
    const std::function<void(FEEvaluation<dim, fe_degree, n_q_points_1d, n_components, Number, VectorizedArrayType> &)>
                      &local_vmult,
  ...
  template <typename CLASS,
            ...
            typename VectorType>
  void
  compute_diagonal(const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
                   VectorType                                         &diagonal_global,
                   void (CLASS::*cell_operation)(
                     FEEvaluation<dim, fe_degree, n_q_points_1d, n_components, Number, VectorizedArrayType> &) const,
  ...
  template <typename CLASS,
            ...
            typename MatrixType>
  void
  compute_matrix(const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
                 const AffineConstraints<Number>                    &constraints,
                 MatrixType                                         &matrix,
                 void (CLASS::*cell_operation)(
                   FEEvaluation<dim, fe_degree, n_q_points_1d, n_components, Number, VectorizedArrayType> &) const,
  ...
  public:
           ...
           const AdditionalData                               &additional_data = AdditionalData())
    {
      ...
      const auto &fe_collection = matrix_free.get_dof_handler(additional_data.dof_index).get_fe_collection();
    ...
    {
      const auto ebd_cell_operation = [&](const auto &matrix_free, auto &dst, const auto &src, const auto &range) {
      ...
      matrix_free->template cell_loop<VectorTypeOut, VectorTypeIn>(ebd_cell_operation, dst, src, zero_dst_vector);
    }
         ...
         const bool                                                                zero_dst_vector = false) const
    {
      ...
      const auto ebd_internal_or_boundary_face_operation =
        [&](const auto &matrix_free, auto &dst, const auto &src, const auto &range) {
          ...
          const unsigned int type = static_cast<unsigned int>(category.first == fe_index_valid) +
          ...
          if (type == 1) // boundary face
            boundary_operation(matrix_free, dst, src, range, category.first == fe_index_valid);
          else if (type == 2) // internal face
            face_operation(matrix_free, dst, src, range);
        };

      matrix_free->template loop<VectorTypeOut, VectorTypeIn>(ebd_cell_operation,
  ...
  categorize_by_boundary_ids(const Triangulation<dim> &tria, AdditionalData &additional_data)
  {
    ...
    if (is_mg)
      additional_data.cell_vectorization_category.assign(std::distance(tria.begin(level), tria.end(level)), 0);
    else
      additional_data.cell_vectorization_category.assign(tria.n_active_cells(), 0);
...

    {
      ...
      int          offset = 1;
      for (unsigned int i = 0; i < GeometryInfo<dim>::faces_per_cell; i++, offset = offset * n_bids)
        factors[i] = offset;
    }
      ...
      for (unsigned int i = 0; i < GeometryInfo<dim>::faces_per_cell; ++i)
        {
          ...
          if (face->at_boundary() && !cell->has_periodic_neighbor(i))
            c_num +=
              factors[i] * (1 + std::distance(bids.begin(), std::find(bids.begin(), bids.end(), face->boundary_id())));
        }
      return c_num;
    };
        ...
        for (auto cell = tria.begin_active(); cell != tria.end(); ++cell)
          {
            if (cell->is_locally_owned())
              additional_data.cell_vectorization_category[cell->active_cell_index()] = to_category(cell);
          }
      }
    else
      {
        for (auto cell = tria.begin(level); cell != tria.end(level); ++cell)
          {
            if (cell->is_locally_owned_on_level())
              additional_data.cell_vectorization_category[cell->index()] = to_category(cell);
          }
      }
    ...
    additional_data.mapping_update_flags_faces_by_cells =
      additional_data.mapping_update_flags_inner_faces | additional_data.mapping_update_flags_boundary_faces;
  }
  ...
  namespace internal
  {
    ...
    public:
      ...
      void
      initialize(FEEvaluation<dim, fe_degree, n_q_points_1d, n_components, Number, VectorizedArrayType> &phi)
      {
      ...
      reinit(const unsigned int cell)
      {
        ...
        // vector
        const unsigned int first_selected_component = n_fe_components == 1 ? 0 : phi->get_first_selected_component();

        const unsigned int n_lanes_filled = matrix_free.n_active_entries_per_cell_batch(cell);
        ...
        // constraints, weight)
        std::vector<std::tuple<unsigned int, unsigned int, Number>> locally_relevant_constraints,
        ...
        for (unsigned int v = 0; v < n_lanes_filled; ++v)
          {
            ...
            const unsigned int start = (cell * n_lanes + v) * n_fe_components + first_selected_component;
            dof_indices              = dof_info.dof_indices.data() + dof_info.row_starts[start].first;
            ...
            if (n_components == 1 || n_fe_components == 1)
              {
                ...
                for (; index_indicators != next_index_indicators; ++index_indicators, ++ind_local)
                  {
                    const std::pair<unsigned short, unsigned short> indicator =
                      dof_info.constraint_indicator[index_indicators];

                    for (unsigned int j = 0; j < indicator.first; ++j, ++ind_local)
                      locally_relevant_constraints.emplace_back(ind_local, dof_indices[j], 1.0);

                    dof_indices += indicator.first;

                    const Number *data_val = matrix_free.constraint_pool_begin(indicator.second);
                    const Number *end_pool = matrix_free.constraint_pool_end(indicator.second);

                    for (; data_val != end_pool; ++data_val, ++dof_indices)
                      locally_relevant_constraints.emplace_back(ind_local, *dof_indices, *data_val);
                  }

                AssertIndexRange(ind_local, dofs_per_component + 1);

                for (; ind_local < dofs_per_component; ++dof_indices, ++ind_local)
                  locally_relevant_constraints.emplace_back(ind_local, *dof_indices, 1.0);
              }
            else
              {
                ...
                for (unsigned int comp = 0; comp < n_components; ++comp)
                  {
                    ...
                    for (; index_indicators != next_index_indicators; ++index_indicators, ++ind_local)
                      {
                        ...
                        for (unsigned int j = 0; j < indicator.first; ++j, ++ind_local)
                          locally_relevant_constraints.emplace_back(comp * dofs_per_component + ind_local,
                                                                    dof_indices[j],
                                                                    1.0);
                        dof_indices += indicator.first;
                    ...
                    if (comp + 1 < n_components)
                      next_index_indicators = dof_info.row_starts[start + comp + 2].second;
                  }
              }
            ...
            for (unsigned int i = 1; i < locally_relevant_constraints.size(); ++i)
              Assert(std::get<0>(locally_relevant_constraints[i]) >= std::get<0>(locally_relevant_constraints[i - 1]),
                     ExcInternalError());
            ...
            if (dof_info.hanging_node_constraint_masks.size() > 0 &&
                ...
                dof_info.hanging_node_constraint_masks_comp[phi->get_active_fe_index()][first_selected_component])
              {
                const auto mask = dof_info.hanging_node_constraint_masks[cell * n_lanes + v];

                // cell has hanging nodes
                if (mask != dealii::internal::MatrixFreeFunctions::unconstrained_compressed_constraint_kind)
                  {
                    // check if hanging node internpolation matrix has been set
                    // up
                    if (locally_relevant_constraints_hn_map.find(mask) == locally_relevant_constraints_hn_map.end())
                      fill_constraint_type_into_map(mask);

                    const auto &locally_relevant_constraints_hn = locally_relevant_constraints_hn_map[mask];

                    locally_relevant_constraints_tmp.clear();
                    if (locally_relevant_constraints_tmp.capacity() < locally_relevant_constraints.size())
                      locally_relevant_constraints_tmp.reserve(locally_relevant_constraints.size() +
                    ...
                    constraint_position.assign(phi->dofs_per_cell, numbers::invalid_unsigned_int);
                    for (auto &a : locally_relevant_constraints)
                      if (constraint_position[std::get<0>(a)] == numbers::invalid_unsigned_int)
                        constraint_position[std::get<0>(a)] = std::distance(locally_relevant_constraints.data(), &a);
                    ...
                    for (const auto &hn : locally_relevant_constraints_hn)
                      if (constraint_position[std::get<1>(hn)] != numbers::invalid_unsigned_int)
                        {
                          AssertIndexRange(constraint_position[std::get<1>(hn)], locally_relevant_constraints.size());
                          auto other = locally_relevant_constraints.begin() + constraint_position[std::get<1>(hn)];
                          AssertDimension(std::get<0>(*other), std::get<1>(hn));

                          for (; other != locally_relevant_constraints.end() && std::get<0>(*other) == std::get<1>(hn);
                               ++other)
                            locally_relevant_constraints_tmp.emplace_back(std::get<0>(hn),
                                                                          std::get<1>(*other),
            ...
            std::sort(locally_relevant_constraints.begin(),
                      ...
                      [](const auto &a, const auto &b) {
                        if (std::get<1>(a) < std::get<1>(b))
                          return true;
                        return (std::get<1>(a) == std::get<1>(b)) && (std::get<0>(a) < std::get<0>(b));
                      });
            ...
            auto &c_pool = c_pools[v];
            {
              ...
              if (locally_relevant_constraints.size() > 0)
                c_pool.row_lid_to_gid.emplace_back(std::get<1>(locally_relevant_constraints.front()));
              ...
              AssertDimension(c_pool.inverse_lookup_rows.back(), c_pool.col.size());
              ...
              for (unsigned int row = 0; row < c_pool.row.size() - 1; ++row)
                for (unsigned int col = c_pool.row[row]; col < c_pool.row[row + 1]; ++col)
                  {
                    const unsigned int index = c_pool.col[col];
                    c_pool.inverse_lookup_origins[c_pool.inverse_lookup_rows[index] + inverse_lookup_count[index]] =
        ...
        for (unsigned int v = 0; v < n_lanes_filled; ++v)
          diagonals_local_constrained[v].assign(c_pools[v].row_lid_to_gid.size() *
                                                  (n_fe_components == 1 ? n_components : 1),
                                                Number(0.0));
      }

      void
      fill_constraint_type_into_map(const dealii::internal::MatrixFreeFunctions::compressed_constraint_kind mask)
      {
        ...
        // first guess
        const unsigned int degree = phi->get_shape_info().data.front().fe_degree;
        ...
        std::array<dealii::internal::MatrixFreeFunctions::compressed_constraint_kind, VectorizedArrayType::size()>
          constraint_mask;
        constraint_mask.fill(dealii::internal::MatrixFreeFunctions::unconstrained_compressed_constraint_kind);
        ...
        for (unsigned int i = 0; i < dofs_per_component; ++i)
          {
            ...
            dealii::internal::FEEvaluationHangingNodesFactory<dim, Number, VectorizedArrayType>::apply(
              1, degree, phi->get_shape_info(), false, constraint_mask, values_dofs.data());

            const Number tolerance = std::max(Number(1e-12), std::numeric_limits<Number>::epsilon() * 16);
            for (unsigned int j = 0; j < dofs_per_component; ++j)
              if (std::abs(values_dofs[j][0]) > tolerance &&
                  (j != i || std::abs(values_dofs[j][0] - Number(1)) > tolerance))
        ...
        for (unsigned int c = 1; c < n_components; ++c)
          for (unsigned int i = 0; i < n_hn_constraints; ++i)
            constraints_hn[c * n_hn_constraints + i] =
              std::make_tuple(std::get<0>(constraints_hn[i]) + c * dofs_per_component,
      ...
      submit()
      {
        ...
        const unsigned int n_fe_components = phi->get_dof_info().start_components.back();
        const unsigned int comp            = n_fe_components == 1 ? i / dofs_per_component : 0;
        const unsigned int i_comp          = n_fe_components == 1 ? (i % dofs_per_component) : i;

        // apply local constraint matrix from left and from right:
        // loop over all rows of transposed constrained matrix
        for (unsigned int v = 0;
             v < phi->get_matrix_free().n_active_entries_per_cell_batch(phi->get_current_cell_index());
             ++v)
          {
            const auto &c_pool = c_pools[v];

            for (unsigned int jj = c_pool.inverse_lookup_rows[i_comp]; jj < c_pool.inverse_lookup_rows[i_comp + 1];
                 ++jj)
              {
                ...
                for (unsigned int k = c_pool.row[j]; k < c_pool.row[j + 1]; ++k)
                  temp += c_pool.val[k] * phi->begin_dof_values()[comp * dofs_per_component + c_pool.col[k]][v];

                // apply constraint matrix from the right
                diagonals_local_constrained[v][j + comp * c_pools[v].row_lid_to_gid.size()] +=
      ...
      inline void
      distribute_local_to_global(std::array<VectorType *, n_components> &diagonal_global)
      {
        ...
        for (unsigned int v = 0;
          ...
          for (unsigned int j = 0; j < c_pools[v].row.size() - 1; ++j)
            for (unsigned int comp = 0; comp < (n_fe_components == 1 ? static_cast<unsigned int>(n_components) : 1);
                 ++comp)
    ...
    private:
      ...
      std::map<dealii::internal::MatrixFreeFunctions::compressed_constraint_kind,
    ...
    const unsigned int first_selected_component)
  {
    ...
    std::array<
      typename dealii::internal::BlockVectorSelector<VectorType, IsBlockVector<VectorType>::value>::BaseVectorType *,
      n_components>
      diagonal_global_components;

    for (unsigned int d = 0; d < n_components; ++d)
      diagonal_global_components[d] =
        dealii::internal::BlockVectorSelector<VectorType, IsBlockVector<VectorType>::value>::get_vector_component(
          diagonal_global, d + first_selected_component);
      ...
      for (unsigned int comp = 0; comp < n_components; ++comp)
        {
          ...
          dealii::internal::check_vector_compatibility(*diagonal_global_components[comp], matrix_free, dof_info);
        }
    else
      {
        dealii::internal::check_vector_compatibility(*diagonal_global_components[0], matrix_free, dof_info);
      }

    using Helper =
      internal::ComputeDiagonalHelper<dim, fe_degree, n_q_points_1d, n_components, Number, VectorizedArrayType>;
  ...
  compute_diagonal(const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
                   ...
                   const unsigned int first_selected_component)
  {
    compute_diagonal<dim, fe_degree, n_q_points_1d, n_components, Number, VectorizedArrayType, VectorType>(
      matrix_free,
      diagonal_global,
  ...
  namespace internal
  {
    ...
    template <typename MatrixType,
              ...
              std::enable_if_t<std::is_same_v<
                typename std::remove_const<typename std::remove_reference<typename MatrixType::value_type>::type>::type,
                typename std::remove_const<typename std::remove_reference<Number>::type>::type>> * = nullptr>
    const AffineConstraints<typename MatrixType::value_type> &
    create_new_affine_constraints_if_needed(const MatrixType &,
                                            const AffineConstraints<Number> &constraints,
    ...
    create_new_affine_constraints_if_needed(
      const MatrixType &,
      const AffineConstraints<Number>                                     &constraints,
      std::unique_ptr<AffineConstraints<typename MatrixType::value_type>> &new_constraints)
    {
      new_constraints = std::make_unique<AffineConstraints<typename MatrixType::value_type>>();
  ...
  compute_matrix(
    ...
    const unsigned int first_selected_component)
  {
    std::unique_ptr<AffineConstraints<typename MatrixType::value_type>> constraints_for_matrix;
    const AffineConstraints<typename MatrixType::value_type>           &constraints =
      internal::create_new_affine_constraints_if_needed(matrix, constraints_in, constraints_for_matrix);

    matrix_free.template cell_loop<MatrixType, MatrixType>(
      [&](const auto &, auto &dst, const auto &, const auto range) {
        FEEvaluation<dim, fe_degree, n_q_points_1d, n_components, Number, VectorizedArrayType> integrator(
          matrix_free, range, dof_no, quad_no, first_selected_component);
        ...
        std::array<FullMatrix<typename MatrixType::value_type>, VectorizedArrayType::size()> matrices;

        std::fill_n(matrices.begin(),
                    VectorizedArrayType::size(),
                    FullMatrix<typename MatrixType::value_type>(dofs_per_cell, dofs_per_cell));

        const auto lexicographic_numbering = matrix_free
        ...
        for (auto cell = range.first; cell < range.second; ++cell)
          {
            ...
            for (unsigned int v = 0; v < n_filled_lanes; ++v)
              {
                const auto cell_v = matrix_free.get_cell_iterator(cell, v, dof_no);
                ...
                constraints.distribute_local_to_global(matrices[v], dof_indices_mf, dst);
              }
          }
      },
      matrix,
  ...
  compute_matrix(const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free,
                 ...
                 const unsigned int first_selected_component)
  {
    compute_matrix<dim, fe_degree, n_q_points_1d, n_components, Number, VectorizedArrayType, MatrixType>(
      matrix_free,
      constraints,
      matrix,
      [&](auto &feeval) { (owning_class->*cell_operation)(feeval); },

...
namespace MeshWorker
{
  ...
  cell_action(
    ...
    const std::function<void(DOFINFO &, DOFINFO &, typename INFOBOX::CellInfo &, typename INFOBOX::CellInfo &)>
                      &face_worker,
    const LoopControl &loop_control)
  {
    ...
    types::subdomain_id csid = (cell->is_level_cell()) ? cell->level_subdomain_id() : cell->subdomain_id();
          ...
          if (cell->at_boundary(face_no) && !cell->has_periodic_neighbor(face_no))
          ...
          else if (integrate_interior_face)
            {
              ...
              const bool own_neighbor =
                ignore_subdomain || (neighbid == cell->get_triangulation().locally_owned_subdomain());
              ...
              if ((!periodic_neighbor && cell->neighbor_is_coarser(face_no)) ||
                  (periodic_neighbor && cell->periodic_neighbor_is_coarser(face_no)))
                {
                  ...
                  dof_info.exterior[face_no].reinit(neighbor, nface, neighbor_face_no.first, neighbor_face_no.second);
                  info.subface.reinit(dof_info.exterior[face_no]);

                  face_worker(dof_info.interior[face_no], dof_info.exterior[face_no], info.face, info.subface);
                }
              else
                {
                  ...
                  if (own_cell && own_neighbor && loop_control.own_faces == LoopControl::one && (neighbor < cell))
                    continue;
                  ...
                  // the face.
                  if (own_cell && !own_neighbor && loop_control.faces_to_ghost == LoopControl::one && (neighbid < csid))
                    continue;
                  ...
                  dof_info.interior[face_no].reinit(cell, face, face_no);
                  info.face.reinit(dof_info.interior[face_no]);
                  dof_info.exterior[face_no].reinit(neighbor, neighbor->face(neighbor_face_no), neighbor_face_no);
                  info.neighbor.reinit(dof_info.exterior[face_no]);

                  face_worker(dof_info.interior[face_no], dof_info.exterior[face_no], info.face, info.neighbor);
                }
            }
        } // faces
    // Call the callback function in
       ...
       const LoopControl &lctrl = LoopControl())
  {
    ...
    WorkStream::run(
      ...
      [&cell_worker, &boundary_worker, &face_worker, &lctrl](ITERATOR                  cell,
                                                             INFOBOX                  &info,
                                                             DoFInfoBox<dim, DOFINFO> &dof_info) {
        cell_action<INFOBOX, DOFINFO, dim, spacedim, ITERATOR>(
          cell, dof_info, info, cell_worker, boundary_worker, face_worker, lctrl);
      },
                   ...
                   const LoopControl                    &lctrl = LoopControl())
  {
    ...
    if (integrator.use_cell)
      cell_worker = [&integrator](DoFInfo<dim, spacedim> &dof_info, IntegrationInfo<dim, spacedim> &integration_info) {
        integrator.cell(dof_info, integration_info);
      };
    if (integrator.use_boundary)
      boundary_worker = [&integrator](DoFInfo<dim, spacedim>         &dof_info,
                                      IntegrationInfo<dim, spacedim> &integration_info) {
        integrator.boundary(dof_info, integration_info);
      };
    if (integrator.use_face)
      face_worker = [&integrator](DoFInfo<dim, spacedim>         &dof_info_1,
        ...
        integrator.face(dof_info_1, dof_info_2, integration_info_1, integration_info_2);
      };

    loop<dim, spacedim>(begin, end, dof_info, box, cell_worker, boundary_worker, face_worker, assembler, lctrl);
  }

...
namespace internal
{
  namespace MGTransfer
  {
                  ...
                  MGLevelObject<BlockVector<number>> &v)
    {
      ...
      Assert(target_component.size() == n_blocks, ExcDimensionMismatch(target_component.size(), n_blocks));
      const unsigned int max_block       = *std::max_element(target_component.begin(), target_component.end());
      const unsigned int n_target_blocks = max_block + 1;

      std::vector<std::vector<types::global_dof_index>> ndofs(dof_handler.get_triangulation().n_levels(),
                  ...
                  MGLevelObject<TrilinosWrappers::MPI::Vector> &v)
    {
      ...
      for (unsigned int level = v.min_level(); level <= v.max_level(); ++level)
        {
          v[level].reinit(dof_handler.locally_owned_mg_dofs(level), tria->get_communicator());
        }
    }
#endif
                  ...
                  MGLevelObject<PETScWrappers::MPI::Vector> &v)
    {
      const dealii::parallel::TriangulationBase<dim, spacedim> *tria =
        (dynamic_cast<const dealii::parallel::TriangulationBase<dim, spacedim> *>(&dof_handler.get_triangulation()));
      AssertThrow(tria != nullptr,
                  ExcMessage("multigrid with parallel PETSc vectors only works with a parallel Triangulation!"));
...
namespace internal
{
  ...
  template <typename T>
  void
  copy_vector(const std::vector<std::pair<types::global_dof_index, types::global_dof_index>> &, const T &src, T &dst)
  {
    dst = src;
  }
} // namespace internal
...
void
MGLevelGlobalTransfer<VectorType>::copy_to_mg(const DoFHandler<dim, spacedim> &dof_handler,
  ...
  for (unsigned int level = dst.max_level() + 1; level != dst.min_level();)
    {
      ...
      using dof_pair_iterator =
        std::vector<std::pair<types::global_dof_index, types::global_dof_index>>::const_iterator;
...
void
MGLevelGlobalTransfer<VectorType>::copy_from_mg(const DoFHandler<dim, spacedim> &dof_handler,
                                                OutVector                       &dst,
...
template <int dim, class OutVector, int spacedim>
void
MGLevelGlobalTransfer<VectorType>::copy_from_mg_add(const DoFHandler<dim, spacedim> &dof_handler,
                                                    OutVector                       &dst,
                                                    const MGLevelObject<VectorType> &src) const
{
  assert_built(dof_handler);
...
template <int dim, int spacedim>
void
MGLevelGlobalTransfer<VectorType>::assert_built(const DoFHandler<dim, spacedim> &dof_handler) const
{
  (void)dof_handler;
  Assert(copy_indices.size() == dof_handler.get_triangulation().n_global_levels(),
  ...
  const bool                                                 solution_transfer) const
{
  ...
  for (unsigned int level = dst.min_level(); level <= dst.max_level(); ++level)
    ...
    else if ((perform_plain_copy == false && perform_renumbered_plain_copy == false) || level != dst.max_level())
      dst[level] = 0;
  ...
  if (perform_plain_copy)
    ...
    {
      AssertDimension(dst[dst.max_level()].locally_owned_size(), src.locally_owned_size());
      AssertDimension(this_copy_indices.back().n_cols(), src.locally_owned_size());
      ...
      for (unsigned int i = 0; i < this_copy_indices.back().n_cols(); ++i)
        dst_level.local_element(this_copy_indices.back()(1, i)) = src.local_element(i);
      return;
    }

  // the ghosted vector should already have the correct local size (but
  // different parallel layout)
  AssertDimension(ghosted_global_vector.locally_owned_size(), src.locally_owned_size());
  ...
  for (unsigned int level = dst.max_level() + 1; level != dst.min_level();)
    {
      ...
      auto copy_unknowns = [&](const Table<2, unsigned int> &indices) {
        for (unsigned int i = 0; i < indices.n_cols(); ++i)
          dst_level.local_element(indices(1, i)) = this_ghosted_global_vector.local_element(indices(0, i));
      };
  ...
  if (perform_plain_copy)
    {
      ...
      AssertDimension(src[src.max_level()].locally_owned_size(), dst.locally_owned_size());
  ...
  else if (perform_renumbered_plain_copy)
    {
      ...
      for (unsigned int i = 0; i < copy_indices.back().n_cols(); ++i)
        dst.local_element(i) = src_level.local_element(copy_indices.back()(1, i));
      return;
    }
  ...
  for (unsigned int level = src.min_level(); level <= src.max_level(); ++level)
    {
      ...
      if (ghosted_level_vector[level].size() > 0)
        AssertDimension(ghosted_level_vector[level].locally_owned_size(), src[level].locally_owned_size());
      ...
      const auto *const ghosted_vector_ptr = (ghosted_level_vector[level].size() > 0) ? &ghosted_vector : &src[level];
  ...
  for (unsigned int level = src.min_level(); level <= src.max_level(); ++level)
    {
      ...
      auto copy_unknowns = [&](const Table<2, unsigned int> &indices) {
        for (unsigned int i = 0; i < indices.n_cols(); ++i)
          dst.local_element(indices(0, i)) += ghosted_vector.local_element(indices(1, i));
      };

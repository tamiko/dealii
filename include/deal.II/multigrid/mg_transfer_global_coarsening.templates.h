...
namespace
{
  ...
  public:
    ...
    CellTransferFactory(const unsigned int degree_fine, const unsigned int degree_coarse)
  ...
  public:
    ...
    run_full(const unsigned int n_dofs_fine, const unsigned int n_dofs_coarse)
    {
      ...
      internal::FEEvaluationImplBasisChange<internal::evaluate_general, internal::EvaluatorQuantity::value, 1, 0, 0>::
        do_forward(1, prolongation_matrix, evaluation_data_coarse, evaluation_data_fine, n_dofs_coarse, n_dofs_fine);
    }
  ...
  public:
    ...
    run_full(const unsigned int n_dofs_fine, const unsigned int n_dofs_coarse)
    {
      ...
      internal::FEEvaluationImplBasisChange<internal::evaluate_general, internal::EvaluatorQuantity::value, 1, 0, 0>::
        do_backward(
          1, prolongation_matrix, false, evaluation_data_fine, evaluation_data_coarse, n_dofs_coarse, n_dofs_fine);
    }
  ...
  public:
    template <int degree_fine, int degree_coarse>
    void
    run(const unsigned int = numbers::invalid_unsigned_int, const unsigned int = numbers::invalid_unsigned_int)
    {}
  };
...
namespace internal
{
  namespace
  {
    ...
    void loop_over_active_or_level_cells(const MeshType &tria, const unsigned int level, const OPType &op)
    {
                     ...
                     std::vector<unsigned int> &local_dof_indices)
    {
      ...
      const unsigned int n_components       = local_dof_indices.size() / Utilities::fixed_power<dim>(fe_degree + 1);
        ...
        for (unsigned int k = 0; k < (dim > 2 ? (fe_degree + 1) : 1); ++k)
          for (unsigned int j = 0; j < (dim > 1 ? (fe_degree + 1) : 1); ++j)
            for (unsigned int i = 0; i < (fe_degree + 1); ++i, ++m)
              local_dof_indices[m] =
                c * n_scalar_cell_dofs + k * n_child_dofs_1d * n_child_dofs_1d + j * n_child_dofs_1d + i + shift;
    }
                      ...
                      const unsigned int fe_degree)
    {
      std::vector<std::vector<unsigned int>> cell_local_children_indices(
        GeometryInfo<dim>::max_children_per_cell, std::vector<unsigned int>(n_dofs_per_cell_coarse));
      for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; c++)
        get_child_offset<dim>(c, fe_shift_1d, fe_degree, cell_local_children_indices[c]);
    ...
    get_child_offsets_general(const unsigned int n_dofs_per_cell_coarse)
    {
      std::vector<std::vector<unsigned int>> cell_local_children_indices(
        GeometryInfo<dim>::max_children_per_cell, std::vector<unsigned int>(n_dofs_per_cell_coarse));
      for (unsigned int c = 0, k = 0; c < GeometryInfo<dim>::max_children_per_cell; c++)
    ...
    create_1D_fe(const FiniteElement<dim, spacedim> &fe)
    {
      std::string fe_name = fe.get_name();
      {
        ...
        Assert(fe_name[template_starts + 1] == (dim == 1 ? '1' : (dim == 2 ? '2' : '3')), ExcInternalError());
    ...
    template <int dim, int spacedim>
    FullMatrix<double>
    get_restriction_matrix(const FiniteElement<dim, spacedim> &fe, const unsigned int child)
    {
    ...
    template <int dim>
    bool
    use_fast_hanging_node_algorithm(const DoFHandler<dim> &dof_handler_coarse, const unsigned int mg_level_coarse)
    {
      // algorithm is only needed on active levels
      bool use_fast_hanging_node_algorithm = mg_level_coarse == numbers::invalid_unsigned_int;

      // algorithm can be only used on meshes consisting of hypercube and
      // simplices
      if (use_fast_hanging_node_algorithm)
        {
          const auto &reference_cells = dof_handler_coarse.get_triangulation().get_reference_cells();
          use_fast_hanging_node_algorithm =
            std::all_of(reference_cells.begin(), reference_cells.end(), [](const auto &r) {
      ...
      if (use_fast_hanging_node_algorithm)
        {
          ...
          use_fast_hanging_node_algorithm &= std::all_of(fes.begin(), fes.end(), [&fes](const auto &fe) {
      ...
      if (use_fast_hanging_node_algorithm)
        {
          const std::vector<std::vector<bool>> supported_components =
            internal::MatrixFreeFunctions::HangingNodes<dim>::compute_supported_components(
              dof_handler_coarse.get_fe_collection());

          use_fast_hanging_node_algorithm &= std::any_of(supported_components.begin(),
  ...
  {
  public:
    FineDoFHandlerViewCell(const std::function<bool()>                                       &has_children_function,
                           const std::function<void(std::vector<types::global_dof_index> &)> &get_dof_indices_function,
  ...
  public:
    FineDoFHandlerView(const DoFHandler<dim> &dof_handler_fine,
      ...
      , communicator(dof_handler_fine.get_communicator() /*TODO: fix for different comms*/)
      , cell_id_translator(dof_handler_fine.get_triangulation().n_global_coarse_cells(),
                           dof_handler_fine.get_triangulation().n_global_levels())
    {
      AssertDimension(dof_handler_fine.get_triangulation().n_global_coarse_cells(),
      ...
      AssertIndexRange(dof_handler_coarse.get_triangulation().n_global_levels(),
                       dof_handler_fine.get_triangulation().n_global_levels() + 1);
    }
           ...
           const bool      check_if_elements_in_is_dst_remote_exist = false)
    {
      ...
      if (check_if_elements_in_is_dst_remote_exist)
        {
          ...
          std::vector<unsigned int> owning_ranks_of_ghosts(is_dst_remote_potentially_relevant.n_elements());

          {
            Utilities::MPI::internal::ComputeIndexOwner::ConsensusAlgorithmsPayload process(
              is_dst_locally_owned, is_dst_remote_potentially_relevant, communicator, owning_ranks_of_ghosts, false);

            Utilities::MPI::ConsensusAlgorithms::Selector<
              std::vector<std::pair<types::global_cell_index, types::global_cell_index>>,
          ...
          for (unsigned i = 0; i < is_dst_remote_potentially_relevant.n_elements(); ++i)
            if (owning_ranks_of_ghosts[i] != numbers::invalid_unsigned_int)
              is_dst_remote.add_index(is_dst_remote_potentially_relevant.nth_index_in_set(i));
        }
      ...
      Utilities::MPI::internal::ComputeIndexOwner::ConsensusAlgorithmsPayload process(
        is_dst_locally_owned, is_dst_remote, communicator, is_dst_remote_owners, true);

      Utilities::MPI::ConsensusAlgorithms::
        Selector<std::vector<std::pair<types::global_cell_index, types::global_cell_index>>, std::vector<unsigned int>>
          consensus_algorithm;
      consensus_algorithm.run(process, communicator);
      ...
      this->is_extended_locally_owned = mg_level_fine == numbers::invalid_unsigned_int ?
      ...
      std::map<unsigned int, std::vector<types::global_dof_index>> indices_to_be_sent;
        ...
        for (const auto &i : targets_with_indexset)
          {
            ...
            std::vector<types::global_dof_index> &buffer = indices_to_be_sent[i.first];

            for (auto cell_id : i.second)
              {
                typename DoFHandler<dim>::cell_iterator cell(*dof_handler_fine.get_triangulation().create_cell_iterator(
            ...
            const auto ierr_1 = MPI_Isend(buffer.data(),
                                          buffer.size(),
        ...
        for (const auto id : is_dst_locally_owned)
          {
            ...
            typename DoFHandler<dim>::cell_iterator cell_(
              *dof_handler_fine.get_triangulation().create_cell_iterator(cell_id), &dof_handler_fine);
      ...
      {
        std::map<unsigned int, std::vector<types::global_dof_index>> rank_to_ids;
        for (unsigned int i = 0; i < is_dst_remote_owners.size(); ++i)
          rank_to_ids[is_dst_remote_owners[i]].push_back(is_dst_remote.nth_index_in_set(i));
        ...
        for (const auto &pair : rank_to_ids)
          {
            ...
            MPI_Status status;
            const int  ierr_1 = MPI_Probe(MPI_ANY_SOURCE,
                                         Utilities::MPI::internal::Tags::fine_dof_handler_view_reinit,
                                         communicator,
                                         &status);
            AssertThrowMPI(ierr_1);
            ...
            int       message_length;
            const int ierr_2 =
              MPI_Get_count(&status, Utilities::MPI::mpi_type_id_for_type<decltype(*buffer.data())>, &message_length);
            AssertThrowMPI(ierr_2);

            buffer.resize(message_length);

            const int ierr_3 = MPI_Recv(buffer.data(),
                                        buffer.size(),
                                        Utilities::MPI::mpi_type_id_for_type<decltype(*buffer.data())>,
                                        status.MPI_SOURCE,
                                        Utilities::MPI::internal::Tags::fine_dof_handler_view_reinit,
            ...
            for (unsigned int i = 0; i < buffer.size();)
              {
                const unsigned int dofs_per_cell = dof_handler_fine.get_fe(buffer[i]).n_dofs_per_cell();
                ++i;
            ...
            for (unsigned int i = 0, k = 0; i < ids.size(); ++i)
              {
                ...
                indices.resize(dof_handler_fine.get_fe(active_fe_index).n_dofs_per_cell());
        ...
        if (!requests.empty())
          {
            const int ierr_1 = MPI_Waitall(requests.size(), requests.data(), MPI_STATUSES_IGNORE);
            AssertThrowMPI(ierr_1);
          }
      }
#endif
      ...
      this->is_extended_ghosts =
        IndexSet(mg_level_fine == numbers::invalid_unsigned_int ? dof_handler_fine.n_dofs() :
                                                                  dof_handler_fine.n_dofs(mg_level_fine));
      this->is_extended_ghosts.add_indices(ghost_indices.begin(), ghost_indices.end());
    ...
    get_cell(const typename DoFHandler<dim>::cell_iterator &cell) const
    {
      ...
      const bool is_cell_locally_owned = this->is_dst_locally_owned.is_element(id);
      ...
      return FineDoFHandlerViewCell(
        ...
        [cell, is_cell_locally_owned, is_cell_remotly_owned, id, this](auto &dof_indices) {
          if (is_cell_locally_owned)
            {
              typename DoFHandler<dim>::cell_iterator cell_fine(
                *dof_handler_fine.get_triangulation().create_cell_iterator(cell->id()), &dof_handler_fine);
              if (mg_level_fine == numbers::invalid_unsigned_int)
        ...
        [cell, is_cell_locally_owned, is_cell_remotly_owned, id, this]() -> unsigned int {
          if (is_cell_locally_owned)
            {
    ...
    FineDoFHandlerViewCell
    get_cell(const typename DoFHandler<dim>::cell_iterator &cell, const unsigned int c) const
    {
      ...
      return FineDoFHandlerViewCell(
        ...
        [cell, is_cell_locally_owned, is_cell_remotely_owned, c, id, this](auto &dof_indices) {
          if (is_cell_locally_owned)
            {
  ...
  private:
    ...
    std::map<unsigned int, std::pair<unsigned int, std::vector<types::global_dof_index>>> map;
  };
  ...
  public:
    ...
    {
      Assert((mg_level_fine == numbers::invalid_unsigned_int && mg_level_coarse == numbers::invalid_unsigned_int) ||
      ...
      const auto fine_operation = [&](const auto &cell) {
        is_dst_locally_owned.add_index(this->cell_id_translator.translate(cell));
      };

      const auto coarse_operation = [&](const auto &cell) {
        is_src_locally_owned.add_index(this->cell_id_translator.translate(cell));
      ...
      loop_over_active_or_level_cells(tria_src, mg_level_coarse, coarse_operation);

      this->reinit(is_dst_locally_owned, is_dst_remote, is_src_locally_owned, true);
    }
  };
  ...
  public:
    PermutationFineDoFHandlerView(const DoFHandler<dim> &dof_handler_dst,
                                  ...
                                  const unsigned int     mg_level_fine,
                                  const unsigned int     mg_level_coarse)
      : internal::FineDoFHandlerView<dim>(dof_handler_dst, dof_handler_src, mg_level_fine)
    {
      // get reference to triangulations
      const auto &tria_dst = dof_handler_dst.get_triangulation();
      const auto &tria_src = dof_handler_src.get_triangulation();
      ...
      this->reinit(is_dst_locally_owned, is_dst_remote, is_src_locally_owned, false);
    }
  };
  ...
  class MGTwoLevelTransferImplementation
  {
    template <int dim, typename Number>
    static void
    compute_weights(const dealii::AffineConstraints<Number>                                   &constraints_fine,
                    const MGTwoLevelTransfer<dim, LinearAlgebra::distributed::Vector<Number>> &transfer,
                    LinearAlgebra::distributed::Vector<Number>                                &touch_count)
    {
      ...
      for (unsigned int i = 0; i < touch_count.locally_owned_size(); ++i)
        touch_count.local_element(i) =
          constraints_fine.is_constrained(touch_count.get_partitioner()->local_to_global(i)) ?
            Number(0.) :
    ...
    template <int dim, typename Number>
    static void
    compress_weights(MGTwoLevelTransfer<dim, LinearAlgebra::distributed::Vector<Number>> &transfer)
    {
      ...
      for (const auto &scheme : transfer.schemes)
        for (unsigned int cell = 0; cell < scheme.n_coarse_cells;
          ...
          if (!compute_weights_fe_q_dofs_by_entity<dim, -1, Number>(
                weight_ptr, transfer.n_components, scheme.degree_fine + 1, mask_ptr->data()))
            return;
          ...
          for (unsigned int cell = 0; cell < scheme.n_coarse_cells; cell += n_lanes)
            {
              const unsigned int n_lanes_filled =
                (cell + n_lanes > scheme.n_coarse_cells) ? (scheme.n_coarse_cells - cell) : n_lanes;
  ...
  public:
                              ...
                              const unsigned int                       mg_level_coarse)
    {
      IndexSet locally_relevant_dofs =
        (mg_level_coarse == numbers::invalid_unsigned_int) ?
          ...
          DoFTools::extract_locally_active_level_dofs(dof_handler_coarse, mg_level_coarse);
      ...
      return std::make_shared<Utilities::MPI::Partitioner>(mg_level_coarse == numbers::invalid_unsigned_int ?
                              ...
                              MGTwoLevelTransfer<dim, LinearAlgebra::distributed::Vector<Number>> &transfer)
    {
      ...
      loop_over_active_or_level_cells(dof_handler_fine, mg_level_fine, [&](const auto &cell) {
        min_active_fe_indices[0] = std::min<unsigned int>(min_active_fe_indices[0], cell->active_fe_index());
        max_active_fe_indices[0] = std::max<unsigned int>(max_active_fe_indices[0], cell->active_fe_index());
      });

      loop_over_active_or_level_cells(dof_handler_coarse, mg_level_coarse, [&](const auto &cell) {
        min_active_fe_indices[1] = std::min<unsigned int>(min_active_fe_indices[1], cell->active_fe_index());
        max_active_fe_indices[1] = std::max<unsigned int>(max_active_fe_indices[1], cell->active_fe_index());
      });

      const auto comm = dof_handler_fine.get_communicator();

      Assert(comm == dof_handler_coarse.get_communicator(), ExcNotImplemented());
      ...
      // create partitioners and vectors for internal purposes
      {
        // ... for fine mesh
        {
          transfer.partitioner_fine =
            std::make_shared<Utilities::MPI::Partitioner>(view.locally_owned_dofs(),
        ...
        // ... coarse mesh (needed since user vector might be const)
        {
          transfer.partitioner_coarse =
            create_coarse_partitioner(dof_handler_coarse, constraints_coarse, mg_level_coarse);
      ...
      const auto process_cells = [&](const auto &fu_non_refined, const auto &fu_refined) {
        loop_over_active_or_level_cells(dof_handler_coarse, mg_level_coarse, [&](const auto &cell_coarse) {
          if (mg_level_coarse == numbers::invalid_unsigned_int)
            {
      ...
      const bool is_feq =
        fe_fine.n_base_elements() == 1 && (dynamic_cast<const FE_Q<dim> *>(&fe_fine.base_element(0)) != nullptr);

      // number of dofs on coarse and fine cells
      transfer.schemes[0].n_dofs_per_cell_coarse   = transfer.schemes[0].n_dofs_per_cell_fine =
        transfer.schemes[1].n_dofs_per_cell_coarse = fe_coarse.n_dofs_per_cell();
      transfer.schemes[1].n_dofs_per_cell_fine =
        is_feq ? (fe_fine.n_components() * Utilities::pow(2 * fe_fine.degree + 1, dim)) :
                 (fe_coarse.n_dofs_per_cell() * GeometryInfo<dim>::max_children_per_cell);

      // degree of FE on coarse and fine cell
      transfer.schemes[0].degree_coarse = transfer.schemes[0].degree_fine = transfer.schemes[1].degree_coarse =
        fe_coarse.degree;
      transfer.schemes[1].degree_fine = is_feq ? (fe_coarse.degree * 2) : (fe_coarse.degree * 2 + 1);
      ...
      // count coarse cells for each scheme (0, 1)
      {
        ...
        process_cells([&](const auto &, const auto &) { transfer.schemes[0].n_coarse_cells++; },
      ...
      const auto cell_local_children_indices =
        (reference_cell == ReferenceCells::get_hypercube<dim>()) ?
          get_child_offsets<dim>(transfer.schemes[0].n_dofs_per_cell_coarse,
                                 ...
                                 fe_fine.degree) :
          get_child_offsets_general<dim>(transfer.schemes[0].n_dofs_per_cell_coarse);

      std::vector<unsigned int> n_dof_indices_fine(transfer.schemes.size() + 1);
      std::vector<unsigned int> n_dof_indices_coarse(transfer.schemes.size() + 1);

      for (unsigned int i = 0; i < transfer.schemes.size(); ++i)
        {
          n_dof_indices_fine[i + 1]   = transfer.schemes[i].n_dofs_per_cell_fine * transfer.schemes[i].n_coarse_cells;
          n_dof_indices_coarse[i + 1] = transfer.schemes[i].n_dofs_per_cell_coarse * transfer.schemes[i].n_coarse_cells;
        }
      ...
      // indices
      {
        std::vector<types::global_dof_index> local_dof_indices(transfer.schemes[0].n_dofs_per_cell_coarse);
        ...
        if (reference_cell == ReferenceCells::get_hypercube<dim>())
          ...
          {
            const auto dummy_quadrature = reference_cell.template get_gauss_type_quadrature<dim>(1);
            internal::MatrixFreeFunctions::ShapeInfo<Number> shape_info;
            shape_info.reinit(dummy_quadrature, fe_fine, 0);
            lexicographic_numbering_fine = shape_info.lexicographic_numbering;
            shape_info.reinit(dummy_quadrature, fe_coarse, 0);
        ...
        // ------------------------------ indices ------------------------------
        std::vector<types::global_dof_index> level_dof_indices_fine_0(transfer.schemes[0].n_dofs_per_cell_fine);
        std::vector<types::global_dof_index> level_dof_indices_fine_1(transfer.schemes[1].n_dofs_per_cell_fine);

        unsigned int cell_no_0 = 0;
        unsigned int cell_no_1 = transfer.schemes[0].n_coarse_cells;

        transfer.constraint_info_coarse.reinit(dof_handler_coarse,
                                               transfer.schemes[0].n_coarse_cells + transfer.schemes[1].n_coarse_cells,
                                               constraints_coarse.n_constraints() > 0 &&
                                                 use_fast_hanging_node_algorithm(dof_handler_coarse, mg_level_coarse));

        transfer.constraint_info_fine.reinit(transfer.schemes[0].n_coarse_cells + transfer.schemes[1].n_coarse_cells);
        ...
        process_cells(
          [&](const auto &cell_coarse, const auto &cell_fine) {
            // parent
            {
              transfer.constraint_info_coarse.read_dof_indices(
                cell_no_0, mg_level_coarse, cell_coarse, constraints_coarse, transfer.partitioner_coarse);
            }

            // child
            {
              cell_fine.get_dof_indices(local_dof_indices);
              for (unsigned int i = 0; i < transfer.schemes[0].n_dofs_per_cell_coarse; i++)
                level_dof_indices_fine_0[i] = local_dof_indices[lexicographic_numbering_fine[i]];

              transfer.constraint_info_fine.read_dof_indices(cell_no_0,
          ...
          [&](const auto &cell_coarse, const auto &cell_fine, const auto c) {
            ...
            if (c == 0)
              {
                transfer.constraint_info_coarse.read_dof_indices(
                  cell_no_1, mg_level_coarse, cell_coarse, constraints_coarse, transfer.partitioner_coarse);

                level_dof_indices_fine_1.assign(level_dof_indices_fine_1.size(), numbers::invalid_dof_index);
              }

            // child
            {
              cell_fine.get_dof_indices(local_dof_indices);
              for (unsigned int i = 0; i < transfer.schemes[1].n_dofs_per_cell_coarse; ++i)
                {
                  const auto index = local_dof_indices[lexicographic_numbering_fine[i]];

                  Assert(level_dof_indices_fine_1[cell_local_children_indices[c][i]] == numbers::invalid_dof_index ||
                           level_dof_indices_fine_1[cell_local_children_indices[c][i]] == index,
                         ExcInternalError());
            ...
            if (c + 1 == GeometryInfo<dim>::max_children_per_cell)
              {
                transfer.constraint_info_fine.read_dof_indices(cell_no_1,
        ...
        if (reference_cell == ReferenceCells::get_hypercube<dim>())
          {
            ...
            const unsigned int shift =
              is_feq ? (fe->n_dofs_per_cell() - fe->n_dofs_per_vertex()) : (fe->n_dofs_per_cell());
            const unsigned int n_child_dofs_1d =
              is_feq ? (fe->n_dofs_per_cell() * 2 - fe->n_dofs_per_vertex()) : (fe->n_dofs_per_cell() * 2);

            {
              transfer.schemes[1].prolongation_matrix_1d.resize(fe->n_dofs_per_cell() * n_child_dofs_1d);

              for (unsigned int c = 0; c < GeometryInfo<1>::max_children_per_cell; ++c)
                for (unsigned int i = 0; i < fe->n_dofs_per_cell(); ++i)
                  for (unsigned int j = 0; j < fe->n_dofs_per_cell(); ++j)
                    transfer.schemes[1].prolongation_matrix_1d[i * n_child_dofs_1d + j + c * shift] =
                      fe->get_prolongation_matrix(c)(renumbering[j], renumbering[i]);
            }
            {
              transfer.schemes[1].restriction_matrix_1d.resize(fe->n_dofs_per_cell() * n_child_dofs_1d);
              ...
              for (unsigned int c = 0; c < GeometryInfo<1>::max_children_per_cell; ++c)
                {
                  ...
                  for (unsigned int i = 0; i < fe->n_dofs_per_cell(); ++i)
                    for (unsigned int j = 0; j < fe->n_dofs_per_cell(); ++j)
                      transfer.schemes[1].restriction_matrix_1d[i * n_child_dofs_1d + j + c * shift] +=
        ...
        else
          {
            ...
            {
              transfer.schemes[1].prolongation_matrix.resize(n_dofs_per_cell * n_dofs_per_cell *
              ...
              for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; ++c)
                for (unsigned int i = 0; i < n_dofs_per_cell; ++i)
                  for (unsigned int j = 0; j < n_dofs_per_cell; ++j)
                    transfer.schemes[1]
                      .prolongation_matrix[i * n_dofs_per_cell * GeometryInfo<dim>::max_children_per_cell + j +
                                           c * n_dofs_per_cell] = fe.get_prolongation_matrix(c)(j, i);
            }
            {
              transfer.schemes[1].restriction_matrix.resize(n_dofs_per_cell * n_dofs_per_cell *
                                                            GeometryInfo<dim>::max_children_per_cell);
              ...
              for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; ++c)
                {
                  ...
                  for (unsigned int i = 0; i < n_dofs_per_cell; ++i)
                    for (unsigned int j = 0; j < n_dofs_per_cell; ++j)
                      transfer.schemes[1]
                        .restriction_matrix[i * n_dofs_per_cell * GeometryInfo<dim>::max_children_per_cell + j +
                                            c * n_dofs_per_cell] += matrix(i, j);
                }
            }
          }
      }
      ...
      if (transfer.fine_element_is_continuous)
        {
          ...
          unsigned int *dof_indices_fine_0 = transfer.constraint_info_fine.dof_indices.data() + n_dof_indices_fine[0];
          unsigned int *dof_indices_fine_1 = transfer.constraint_info_fine.dof_indices.data() + n_dof_indices_fine[1];

          process_cells(
            [&](const auto &, const auto &) {
              for (unsigned int i = 0; i < transfer.schemes[0].n_dofs_per_cell_fine; ++i)
                weights_0[i] = weight_vector.local_element(dof_indices_fine_0[i]);
            ...
            [&](const auto &, const auto &, const auto c) {
              for (unsigned int i = 0; i < transfer.schemes[1].n_dofs_per_cell_coarse; ++i)
                weights_1[cell_local_children_indices[c][i]] =
                  weight_vector.local_element(dof_indices_fine_1[cell_local_children_indices[c][i]]);

              // move pointers (only once at the end)
              if (c + 1 == GeometryInfo<dim>::max_children_per_cell)
                {
                  dof_indices_fine_1 += transfer.schemes[1].n_dofs_per_cell_fine;
                               ...
                               MGTwoLevelTransfer<dim, LinearAlgebra::distributed::Vector<Number>> &transfer)
    {
      Assert(mg_level_fine == numbers::invalid_unsigned_int ||
               mg_level_fine <= MGTools::max_level_for_coarse_mesh(dof_handler_fine.get_triangulation()),
             ExcMessage("Polynomial transfer is only allowed on the active level "
                        "(numbers::invalid_unsigned_int) or on refinement levels without "
                        "hanging nodes."));
      Assert(mg_level_coarse == numbers::invalid_unsigned_int ||
               mg_level_coarse <= MGTools::max_level_for_coarse_mesh(dof_handler_coarse.get_triangulation()),
             ExcMessage("Polynomial transfer is only allowed on the active level "
                        "(numbers::invalid_unsigned_int) or on refinement levels without "
                        "hanging nodes."));
      ...
      // TODO: adjust assert
      AssertDimension(dof_handler_fine.get_triangulation().n_global_active_cells(),
      ...
      transfer.n_components = dof_handler_fine.get_fe_collection().n_components();
      ...
      transfer.fine_element_is_continuous =
        std::all_of(dof_handler_fine.get_fe_collection().begin(),
                    ...
                    [](const auto &fe) { return fe.n_dofs_per_cell() == 0 || fe.n_dofs_per_vertex() > 0; });
      ...
      Assert(transfer.fine_element_is_continuous != fine_element_is_discontinuous, ExcNotImplemented());
#endif

      const bool is_feq = std::all_of(dof_handler_fine.get_fe_collection().begin(),
                                      dof_handler_fine.get_fe_collection().end(),
                                      [](const auto &fe) {
      ...
      std::map<std::pair<unsigned int, unsigned int>, unsigned int> fe_index_pairs;

      process_cells([&](const auto &cell_coarse, const auto &cell_fine) {
        fe_index_pairs.emplace(std::pair<unsigned int, unsigned int>(cell_coarse->active_fe_index(),
      ...
      // extract number of coarse cells
      {
        ...
        process_cells([&](const auto &cell_coarse, const auto &cell_fine) {
          transfer
            .schemes[fe_index_pairs[std::pair<unsigned int, unsigned int>(cell_coarse->active_fe_index(),
                                                                          cell_fine->active_fe_index())]]
            .n_coarse_cells++;
        });
      }
      ...
      for (const auto &fe_index_pair : fe_index_pairs)
        {
          transfer.schemes[fe_index_pair.second].n_dofs_per_cell_coarse =
            dof_handler_coarse.get_fe(fe_index_pair.first.first).n_dofs_per_cell();
          transfer.schemes[fe_index_pair.second].n_dofs_per_cell_fine =
            dof_handler_fine.get_fe(fe_index_pair.first.second).n_dofs_per_cell();
      ...
      {
        transfer.partitioner_fine = std::make_shared<Utilities::MPI::Partitioner>(view.locally_owned_dofs(),
                                                                                  view.locally_relevant_dofs(),
                                                                                  dof_handler_fine.get_communicator());
        transfer.vec_fine.reinit(transfer.partitioner_fine);
      }
      ...
      {
        std::vector<std::vector<unsigned int>>            lexicographic_numbering_fine(fe_index_pairs.size());
        std::vector<std::vector<unsigned int>>            lexicographic_numbering_coarse(fe_index_pairs.size());
        std::vector<std::vector<types::global_dof_index>> local_dof_indices_coarse(fe_index_pairs.size());
        std::vector<std::vector<types::global_dof_index>> local_dof_indices_coarse_lex(fe_index_pairs.size());
        std::vector<std::vector<types::global_dof_index>> local_dof_indices_fine(fe_index_pairs.size());
        std::vector<std::vector<types::global_dof_index>> local_dof_indices_fine_lex(fe_index_pairs.size());
        ...
        for (const auto &fe_index_pair : fe_index_pairs)
          {
            ...
            n_dof_indices_fine[fe_index_pair.second + 1] = transfer.schemes[fe_index_pair.second].n_dofs_per_cell_fine *
                                                           transfer.schemes[fe_index_pair.second].n_coarse_cells;
            n_dof_indices_coarse[fe_index_pair.second + 1] =
              transfer.schemes[fe_index_pair.second].n_dofs_per_cell_coarse *
              transfer.schemes[fe_index_pair.second].n_coarse_cells;
            cell_no[fe_index_pair.second + 1] = transfer.schemes[fe_index_pair.second].n_coarse_cells;

            const auto reference_cell = dof_handler_fine.get_fe(fe_index_pair.first.second).reference_cell();

            Assert(reference_cell == dof_handler_coarse.get_fe(fe_index_pair.first.first).reference_cell(),
                   ExcNotImplemented());
            ...
            if (reference_cell == ReferenceCells::get_hypercube<dim>())
              {
                const Quadrature<1> dummy_quadrature(std::vector<Point<1>>(1, Point<1>()));
                internal::MatrixFreeFunctions::ShapeInfo<VectorizedArray<Number>> shape_info;
                shape_info.reinit(dummy_quadrature, dof_handler_fine.get_fe(fe_index_pair.first.second), 0);
                lexicographic_numbering_fine[fe_index_pair.second] = shape_info.lexicographic_numbering;

                shape_info.reinit(dummy_quadrature, dof_handler_coarse.get_fe(fe_index_pair.first.first), 0);
                lexicographic_numbering_coarse[fe_index_pair.second] = shape_info.lexicographic_numbering;
              }
            else
              {
                const auto dummy_quadrature = reference_cell.template get_gauss_type_quadrature<dim>(1);
        ...
        process_cells([&](const auto &cell_coarse, const auto &cell_fine) {
          const auto fe_pair_no = fe_index_pairs[std::pair<unsigned int, unsigned int>(cell_coarse->active_fe_index(),
                                                                                       cell_fine->active_fe_index())];

          // parent
          {
            transfer.constraint_info_coarse.read_dof_indices(
              cell_no[fe_pair_no], mg_level_coarse, cell_coarse, constraints_coarse, transfer.partitioner_coarse);
          }
          ...
          // child
          {
            ...
            for (unsigned int i = 0; i < transfer.schemes[fe_pair_no].n_dofs_per_cell_fine; ++i)
              local_dof_indices_fine_lex[fe_pair_no][i] =
                local_dof_indices_fine[fe_pair_no][lexicographic_numbering_fine[fe_pair_no][i]];

            transfer.constraint_info_fine.read_dof_indices(cell_no[fe_pair_no],
      ...
      for (const auto &fe_index_pair_ : fe_index_pairs)
        {
          ...
          AssertDimension(dof_handler_fine.get_fe(fe_index_pair.second).n_base_elements(), 1);
          AssertDimension(dof_handler_coarse.get_fe(fe_index_pair.first).n_base_elements(), 1);

          const auto reference_cell = dof_handler_fine.get_fe(fe_index_pair_.first.second).reference_cell();

          Assert(reference_cell == dof_handler_coarse.get_fe(fe_index_pair_.first.first).reference_cell(),
                 ExcNotImplemented());

          if (reference_cell == ReferenceCells::get_hypercube<dim>() &&
              (dof_handler_coarse.get_fe(fe_index_pair.first) != dof_handler_fine.get_fe(fe_index_pair.second)) &&
              (dof_handler_coarse.get_fe(fe_index_pair.first).n_dofs_per_cell() != 0 &&
               dof_handler_fine.get_fe(fe_index_pair.second).n_dofs_per_cell() != 0))
            {
              const auto fe_fine = create_1D_fe(dof_handler_fine.get_fe(fe_index_pair.second).base_element(0));

              std::vector<unsigned int> renumbering_fine(fe_fine->n_dofs_per_cell());
              {
                ...
                for (unsigned int i = 0; i < fe_fine->dofs_per_line; ++i)
                  renumbering_fine[i + fe_fine->n_dofs_per_vertex()] =
                    GeometryInfo<1>::vertices_per_cell * fe_fine->n_dofs_per_vertex() + i;
                if (fe_fine->n_dofs_per_vertex() > 0)
                  renumbering_fine[fe_fine->n_dofs_per_cell() - fe_fine->n_dofs_per_vertex()] =
                    fe_fine->n_dofs_per_vertex();
              }

              const auto fe_coarse = create_1D_fe(dof_handler_coarse.get_fe(fe_index_pair.first).base_element(0));

              std::vector<unsigned int> renumbering_coarse(fe_coarse->n_dofs_per_cell());
              {
                ...
                for (unsigned int i = 0; i < fe_coarse->dofs_per_line; ++i)
                  renumbering_coarse[i + fe_coarse->n_dofs_per_vertex()] =
                    GeometryInfo<1>::vertices_per_cell * fe_coarse->n_dofs_per_vertex() + i;
                if (fe_coarse->n_dofs_per_vertex() > 0)
                  renumbering_coarse[fe_coarse->n_dofs_per_cell() - fe_coarse->n_dofs_per_vertex()] =
...

              {
                ...
                transfer.schemes[fe_index_no].prolongation_matrix_1d.resize(fe_fine->n_dofs_per_cell() *
              ...
              {
                FullMatrix<double> matrix(fe_coarse->n_dofs_per_cell(), fe_fine->n_dofs_per_cell());
                FETools::get_projection_matrix(*fe_fine, *fe_coarse, matrix);
                transfer.schemes[fe_index_no].restriction_matrix_1d.resize(fe_fine->n_dofs_per_cell() *
                                                                           fe_coarse->n_dofs_per_cell());
                ...
                for (unsigned int i = 0, k = 0; i < fe_coarse->n_dofs_per_cell(); ++i)
                  for (unsigned int j = 0; j < fe_fine->n_dofs_per_cell(); ++j, ++k)
                   ...
                   (dof_handler_coarse.get_fe(fe_index_pair.first).n_dofs_per_cell() != 0 &&
                    dof_handler_fine.get_fe(fe_index_pair.second).n_dofs_per_cell() != 0))
            {
              const auto &fe_fine = dof_handler_fine.get_fe(fe_index_pair.second).base_element(0);

              const auto &fe_coarse = dof_handler_coarse.get_fe(fe_index_pair.first).base_element(0);

              {
                FullMatrix<double> matrix(fe_fine.n_dofs_per_cell(), fe_coarse.n_dofs_per_cell());
                FETools::get_projection_matrix(fe_coarse, fe_fine, matrix);
                transfer.schemes[fe_index_no].prolongation_matrix.resize(fe_fine.n_dofs_per_cell() *
                                                                         fe_coarse.n_dofs_per_cell());

                for (unsigned int i = 0, k = 0; i < fe_coarse.n_dofs_per_cell(); ++i)
                  for (unsigned int j = 0; j < fe_fine.n_dofs_per_cell(); ++j, ++k)
                    transfer.schemes[fe_index_no].prolongation_matrix[k] = matrix(j, i);
              }

              {
                FullMatrix<double> matrix(fe_coarse.n_dofs_per_cell(), fe_fine.n_dofs_per_cell());
                FETools::get_projection_matrix(fe_fine, fe_coarse, matrix);
                transfer.schemes[fe_index_no].restriction_matrix.resize(fe_fine.n_dofs_per_cell() *
                                                                        fe_coarse.n_dofs_per_cell());

                for (unsigned int i = 0, k = 0; i < fe_coarse.n_dofs_per_cell(); ++i)
                  for (unsigned int j = 0; j < fe_fine.n_dofs_per_cell(); ++j, ++k)
                    transfer.schemes[fe_index_no].restriction_matrix[k] = matrix(i, j);
              }
            }
        }
      ...
      if (transfer.fine_element_is_continuous)
        {
          ...
          for (unsigned int i = 0; i < fe_index_pairs.size(); ++i)
            {
              level_dof_indices_fine_[i] = transfer.constraint_info_fine.dof_indices.data() + n_dof_indices_fine[i];
          ...
          process_cells([&](const auto &cell_coarse, const auto &cell_fine) {
            ...
            for (unsigned int i = 0; i < transfer.schemes[fe_pair_no].n_dofs_per_cell_fine; i++)
              weights_[fe_pair_no][i] = weight_vector.local_element(level_dof_indices_fine_[fe_pair_no][i]);

            level_dof_indices_fine_[fe_pair_no] += transfer.schemes[fe_pair_no].n_dofs_per_cell_fine;
            weights_[fe_pair_no] += transfer.schemes[fe_pair_no].n_dofs_per_cell_fine;
          });
  ...
  {
    SimpleVectorDataExchange(const std::shared_ptr<const Utilities::MPI::Partitioner> &embedded_partitioner,
    ...
    update_ghost_values_start(const VectorType &vec) const
    {
      ...
      embedded_partitioner->template export_to_ghosted_array_start<Number, MemorySpace::Host>(
        ...
        dealii::ArrayView<Number>(const_cast<Number *>(vec.begin()) + embedded_partitioner->locally_owned_size(),
    ...
    zero_out_ghost_values(const VectorType &vec) const
    {
      ...
      ArrayView<Number> ghost_array(const_cast<LinearAlgebra::distributed::Vector<Number> &>(vec).begin() +
...
namespace MGTransferGlobalCoarseningTools
{
  ...
  create_geometric_coarsening_sequence(const Triangulation<dim, spacedim> &fine_triangulation_in)
  {
    ...
    Assert((dynamic_cast<const parallel::fullydistributed::Triangulation<dim, spacedim> *>(&fine_triangulation_in) ==
            nullptr),
           ExcMessage("Triangulations of type parallel::fullydistributed::Triangulation are "
                      "not supported by this function!"));

    const auto create_new_empty_triangulation = [&]() -> std::shared_ptr<Triangulation<dim, spacedim>> {
#ifdef DEAL_II_WITH_P4EST
      if (const auto fine_triangulation =
            dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(&fine_triangulation_in))
        return std::make_shared<parallel::distributed::Triangulation<dim, spacedim>>(
          fine_triangulation->get_communicator(), fine_triangulation->get_mesh_smoothing());
      else
#endif
#ifdef DEAL_II_WITH_MPI
        if (const auto fine_triangulation =
              dynamic_cast<const parallel::shared::Triangulation<dim, spacedim> *>(&fine_triangulation_in))
...
#endif
        return std::make_shared<Triangulation<dim, spacedim>>(fine_triangulation_in.get_mesh_smoothing());
    };
  ...
  template <int dim, int spacedim>
  std::vector<std::shared_ptr<const Triangulation<dim, spacedim>>>
  create_geometric_coarsening_sequence(Triangulation<dim, spacedim>                         &fine_triangulation_in,
                                       ...
                                       const bool repartition_fine_triangulation)
  {
    ...
    const auto fine_triangulation =
      dynamic_cast<parallel::distributed::Triangulation<dim, spacedim> *>(&fine_triangulation_in);

    Assert(fine_triangulation, ExcNotImplemented());

    const auto comm = fine_triangulation->get_communicator();

    parallel::distributed::Triangulation<dim, spacedim> temp_triangulation(comm,
                                                                           fine_triangulation->get_mesh_smoothing());

    if (keep_fine_triangulation == true && repartition_fine_triangulation == false)
      {
        coarse_grid_triangulations.back().reset(&fine_triangulation_in, [](auto *) {
          // empty deleter, since
    ...
    else
      {
        ...
        const auto construction_data =
          repartition_fine_triangulation ?
            ...
            TriangulationDescription::Utilities::create_description_from_triangulation(*fine_triangulation, comm);

        // create new triangulation
        const auto new_fine_triangulation =
          std::make_shared<parallel::fullydistributed::Triangulation<dim, spacedim>>(comm);

        for (const auto i : fine_triangulation->get_manifold_ids())
          if (i != numbers::flat_manifold_id)
            new_fine_triangulation->set_manifold(i, fine_triangulation->get_manifold(i));
    ...
    auto *temp_triangulation_ptr = keep_fine_triangulation ? &temp_triangulation : fine_triangulation;
                                       ...
                                       const bool repartition_fine_triangulation)
  {
    ...
    return create_geometric_coarsening_sequence(const_cast<Triangulation<dim, spacedim> &>(fine_triangulation_in),
                                                policy,
                                                true,
                                                repartition_fine_triangulation);
  }
...
template <typename Number>
void
MGTwoLevelTransferBase<LinearAlgebra::distributed::Vector<Number>>::prolongate_and_add(
  ...
  const LinearAlgebra::distributed::Vector<Number> &src) const
{
  ...
  Assert(vec_fine_ptr->get_partitioner().get() == this->partitioner_fine.get(), ExcInternalError());

  const bool        use_src_inplace = this->vec_coarse.size() == 0;
  const auto *const vec_coarse_ptr  = use_src_inplace ? &src : &this->vec_coarse;
  Assert(vec_coarse_ptr->get_partitioner().get() == this->partitioner_coarse.get(), ExcInternalError());
...
template <int dim, typename Number>
void
MGTwoLevelTransfer<dim, LinearAlgebra::distributed::Vector<Number>>::prolongate_and_add_internal(
  LinearAlgebra::distributed::Vector<Number>       &dst,
  const LinearAlgebra::distributed::Vector<Number> &src) const
{
  ...
  for (const auto &scheme : schemes)
    {
      ...
      const bool needs_interpolation =
        (scheme.prolongation_matrix.empty() && scheme.prolongation_matrix_1d.empty()) == false;

      evaluation_data_fine.clear();
      evaluation_data_coarse.clear();

      const unsigned int max_n_dofs_per_cell = std::max(scheme.n_dofs_per_cell_fine, scheme.n_dofs_per_cell_coarse);
      evaluation_data_fine.resize(max_n_dofs_per_cell);
      evaluation_data_coarse.resize(max_n_dofs_per_cell);

      CellTransferFactory cell_transfer(scheme.degree_fine, scheme.degree_coarse);

      const unsigned int n_scalar_dofs_fine   = scheme.n_dofs_per_cell_fine / n_components;
      const unsigned int n_scalar_dofs_coarse = scheme.n_dofs_per_cell_coarse / n_components;
      ...
      for (unsigned int cell = 0; cell < scheme.n_coarse_cells; cell += n_lanes)
        {
          ...
          constraint_info_coarse.read_write_operation(reader,
                                                      src,
                                                      ...
                                                      true);
          constraint_info_coarse.apply_hanging_node_constraints(cell_counter,
                                                                n_lanes_filled,
                                                                false,
                                                                evaluation_data_coarse);
          ...
          if (needs_interpolation)
            for (int c = n_components - 1; c >= 0; --c)
              {
                CellProlongator<dim, VectorizedArrayType> cell_prolongator(scheme.prolongation_matrix,
                ...
                if (scheme.prolongation_matrix_1d.size() > 0)
                  ...
                  cell_prolongator.run_full(n_scalar_dofs_fine, n_scalar_dofs_coarse);
              }
          else
          ...
          // weight
          if (this->fine_element_is_continuous && this->weights_compressed.size() > 0)
            {
              internal::weight_fe_q_dofs_by_entity<dim, -1, VectorizedArrayType>(weights_compressed,
                                                                                 n_components,
          ...
          constraint_info_fine.read_write_operation(
            writer, dst, evaluation_data_fine.data(), cell_counter, n_lanes_filled, scheme.n_dofs_per_cell_fine, false);
...
template <typename Number>
void
MGTwoLevelTransferBase<LinearAlgebra::distributed::Vector<Number>>::restrict_and_add(
  LinearAlgebra::distributed::Vector<Number>       &dst,
  const LinearAlgebra::distributed::Vector<Number> &src) const
{
  ...
  this->zero_out_ghost_values(*vec_coarse_ptr); // since we might add into the
...
template <int dim, typename Number>
void
MGTwoLevelTransfer<dim, LinearAlgebra::distributed::Vector<Number>>::restrict_and_add_internal(
  LinearAlgebra::distributed::Vector<Number>       &dst,
  const LinearAlgebra::distributed::Vector<Number> &src) const
{
  const unsigned int n_lanes = VectorizedArrayType::size();
      ...
      for (unsigned int cell = 0; cell < scheme.n_coarse_cells; cell += n_lanes)
        {
          ...
          internal::VectorReader<Number, VectorizedArrayType> reader;
          constraint_info_fine.read_write_operation(
            reader, src, evaluation_data_fine.data(), cell_counter, n_lanes_filled, scheme.n_dofs_per_cell_fine, false);
          ...
          if (needs_interpolation)
            for (int c = n_components - 1; c >= 0; --c)
              {
                CellRestrictor<dim, VectorizedArrayType> cell_restrictor(scheme.prolongation_matrix,
                                                                         scheme.prolongation_matrix_1d,
                                                                         evaluation_data_fine.begin() +
                                                                           c * n_scalar_dofs_fine,
                                                                         evaluation_data_coarse.begin() +
                ...
                if (scheme.prolongation_matrix_1d.size() > 0)
                  ...
                  cell_restrictor.run_full(n_scalar_dofs_fine, n_scalar_dofs_coarse);
              }
          else
          ...
          constraint_info_coarse.apply_hanging_node_constraints(cell_counter,
                                                                ...
                                                                true,
                                                                evaluation_data_coarse);
          constraint_info_coarse.read_write_operation(writer,
                                                      dst,
                                                      evaluation_data_coarse.data(),
                                                      cell_counter,
                                                      n_lanes_filled,
...
template <int dim, typename Number>
void
MGTwoLevelTransfer<dim, LinearAlgebra::distributed::Vector<Number>>::interpolate(
  LinearAlgebra::distributed::Vector<Number>       &dst,
  const LinearAlgebra::distributed::Vector<Number> &src) const
{
  const unsigned int n_lanes = VectorizedArrayType::size();
  ...
  for (const auto &scheme : schemes)
    {
      ...
      if (scheme.n_dofs_per_cell_fine == 0 || scheme.n_dofs_per_cell_coarse == 0)
        {
      ...
      for (unsigned int cell = 0; cell < scheme.n_coarse_cells; cell += n_lanes)
        {
          ...
          if (needs_interpolation)
            for (int c = n_components - 1; c >= 0; --c)
              {
                CellRestrictor<dim, VectorizedArrayType> cell_restrictor(scheme.restriction_matrix,
...
namespace internal
{
  ...
  {
    bool
    is_partitioner_contained(const std::shared_ptr<const Utilities::MPI::Partitioner> &partitioner,
                             const std::shared_ptr<const Utilities::MPI::Partitioner> &external_partitioner)
    {
      ...
      const int ghosts_locally_contained =
        ((external_partitioner->ghost_indices() & partitioner->ghost_indices()) == partitioner->ghost_indices()) ? 1 :
                                                                                                                   0;

      // check if ghost values are contained in external partititioner
      return Utilities::MPI::min(ghosts_locally_contained, partitioner->get_mpi_communicator()) == 1;
    }

    std::shared_ptr<Utilities::MPI::Partitioner>
    create_embedded_partitioner(const std::shared_ptr<const Utilities::MPI::Partitioner> &partitioner,
                                const std::shared_ptr<const Utilities::MPI::Partitioner> &larger_partitioner)
    {
      ...
      embedded_partitioner->set_ghost_indices(partitioner->ghost_indices(), larger_partitioner->ghost_indices());
...
void
MGTwoLevelTransferBase<LinearAlgebra::distributed::Vector<Number>>::internal_enable_inplace_operations_if_possible(
  ...
  std::vector<unsigned int>                                                          &dof_indices_fine)
{
  ...
  else if (internal::is_partitioner_contained(this->partitioner_coarse, external_partitioner_coarse))
    {
      this->vec_coarse.reinit(0);

      for (auto &i : constraint_info_coarse.dof_indices)
        i = external_partitioner_coarse->global_to_local(this->partitioner_coarse->local_to_global(i));

      for (auto &i : constraint_info_coarse.plain_dof_indices)
        i = external_partitioner_coarse->global_to_local(this->partitioner_coarse->local_to_global(i));

      this->partitioner_coarse_embedded =
        internal::create_embedded_partitioner(this->partitioner_coarse, external_partitioner_coarse);
  ...
  else if (internal::is_partitioner_contained(this->partitioner_fine, external_partitioner_fine))
    {
      this->vec_fine.reinit(0);

      for (auto &i : dof_indices_fine)
        i = external_partitioner_fine->global_to_local(this->partitioner_fine->local_to_global(i));

      this->partitioner_fine_embedded =
        internal::create_embedded_partitioner(this->partitioner_fine, external_partitioner_fine);
...
template <int dim, typename Number>
void
MGTwoLevelTransfer<dim, LinearAlgebra::distributed::Vector<Number>>::enable_inplace_operations_if_possible(
  const std::shared_ptr<const Utilities::MPI::Partitioner> &external_partitioner_coarse,
  const std::shared_ptr<const Utilities::MPI::Partitioner> &external_partitioner_fine)
{
  this->internal_enable_inplace_operations_if_possible(external_partitioner_coarse,
...
template <int dim, typename Number>
void
MGTwoLevelTransfer<dim, LinearAlgebra::distributed::Vector<Number>>::reinit_geometric_transfer(
  const DoFHandler<dim>           &dof_handler_fine,
  const DoFHandler<dim>           &dof_handler_coarse,
  ...
  const unsigned int               mg_level_coarse)
{
  internal::MGTwoLevelTransferImplementation::reinit_geometric_transfer(
    dof_handler_fine, dof_handler_coarse, constraints_fine, constraints_coarse, mg_level_fine, mg_level_coarse, *this);
}
...
template <int dim, typename Number>
void
MGTwoLevelTransfer<dim, LinearAlgebra::distributed::Vector<Number>>::reinit_polynomial_transfer(
  const DoFHandler<dim>           &dof_handler_fine,
  const DoFHandler<dim>           &dof_handler_coarse,
  const AffineConstraints<Number> &constraints_fine,
  const AffineConstraints<Number> &constraints_coarse,
  ...
  const unsigned int               mg_level_coarse)
{
  ...
  bool do_polynomial_transfer =
    (mg_level_fine != numbers::invalid_unsigned_int) || (mg_level_coarse != numbers::invalid_unsigned_int);
  ...
  if (do_polynomial_transfer == false)
    {
      ...
      std::vector<unsigned int> owning_ranks(is_locally_owned_coarse.n_elements());

      Utilities::MPI::internal::ComputeIndexOwner::ConsensusAlgorithmsPayload process(
        is_locally_owned_fine, is_locally_owned_coarse, communicator, owning_ranks, false);
      ...
      do_polynomial_transfer = Utilities::MPI::min(static_cast<unsigned int>(all_cells_found), communicator) == 1;
    }

  if (do_polynomial_transfer)
    internal::MGTwoLevelTransferImplementation::reinit_polynomial_transfer(dof_handler_fine,
                                                                           dof_handler_coarse,
                                                                           constraints_fine,
                                                                           constraints_coarse,
                                                                           mg_level_fine,
    ...
    internal::MGTwoLevelTransferImplementation::reinit_geometric_transfer(dof_handler_fine,
                                                                          dof_handler_coarse,
                                                                          constraints_fine,
                                                                          constraints_coarse,
                                                                          mg_level_fine,
...
template <int dim, typename Number>
bool
MGTwoLevelTransfer<dim, LinearAlgebra::distributed::Vector<Number>>::fast_polynomial_transfer_supported(
...
template <int dim, typename Number>
std::size_t
MGTwoLevelTransfer<dim, LinearAlgebra::distributed::Vector<Number>>::memory_consumption() const
{
  std::size_t size = 0;
...
template <typename Number>
void
MGTwoLevelTransferBase<LinearAlgebra::distributed::Vector<Number>>::update_ghost_values(
  const LinearAlgebra::distributed::Vector<Number> &vec) const
{
  if ((vec.get_partitioner().get() == this->partitioner_coarse.get()) && (this->partitioner_coarse_embedded != nullptr))
    internal::SimpleVectorDataExchange<Number>(this->partitioner_coarse_embedded, this->buffer_coarse_embedded)
      .update_ghost_values(vec);
  else if ((vec.get_partitioner().get() == this->partitioner_fine.get()) &&
           (this->partitioner_fine_embedded != nullptr))
    internal::SimpleVectorDataExchange<Number>(this->partitioner_fine_embedded, this->buffer_fine_embedded)
      .update_ghost_values(vec);
  else
...
template <typename Number>
void
MGTwoLevelTransferBase<LinearAlgebra::distributed::Vector<Number>>::zero_out_ghost_values(
  const LinearAlgebra::distributed::Vector<Number> &vec) const
{
  if ((vec.get_partitioner().get() == this->partitioner_coarse.get()) && (this->partitioner_coarse_embedded != nullptr))
    internal::SimpleVectorDataExchange<Number>(this->partitioner_coarse_embedded, this->buffer_coarse_embedded)
...
template <int dim, typename Number>
void
MGTransferMF<dim, Number>::initialize_constraints(const MGConstrainedDoFs &mg_constrained_dofs)
{
  ...
  const SmartPointer<const MGConstrainedDoFs> &mg_constrained_dofs)
{
  ...
  MGLevelObject<AffineConstraints<typename VectorType::value_type>> constraints(min_level, max_level);

  if (mg_constrained_dofs)
    for (unsigned int l = min_level; l <= max_level; ++l)
      mg_constrained_dofs->merge_constraints(constraints[l],
                                             l,
  ...
  if (this->external_partitioners.size() > 0)
    {
      ...
      for (unsigned int l = min_level + 1; l <= max_level; ++l)
        transfer[l]->enable_inplace_operations_if_possible(this->external_partitioners[l - 1 - min_level],
  ...
  else
    {
      ...
      for (unsigned int l = min_level + 1; l <= max_level; ++l)
        {
          if (l == min_level + 1)
            this->external_partitioners.push_back(transfer[l]->partitioner_coarse);
...
template <int dim, typename Number>
void
MGTransferMF<dim, Number>::build(const std::function<void(const unsigned int, VectorType &)> &initialize_dof_vector)
{
  if (initialize_dof_vector)
    {
      ...
      std::vector<std::shared_ptr<const Utilities::MPI::Partitioner>> external_partitioners(n_levels);

      for (unsigned int l = min_level; l <= max_level; ++l)
        {
          LinearAlgebra::distributed::Vector<typename VectorType::value_type> vector;
...
template <int dim, typename Number>
void
MGTransferMF<dim, Number>::build(const DoFHandler<dim>                                       &dof_handler,
...
template <int dim, typename Number>
void
MGTransferMF<dim, Number>::prolongate(const unsigned int to_level, VectorType &dst, const VectorType &src) const
{
  dst = 0;
...
template <int dim, typename Number>
void
MGTransferMF<dim, Number>::prolongate_and_add(const unsigned int to_level, VectorType &dst, const VectorType &src) const
{
...
template <int dim, typename Number>
void
MGTransferMF<dim, Number>::restrict_and_add(const unsigned int from_level, VectorType &dst, const VectorType &src) const
{
...
template <int dim, typename Number>
MGTransferBlockMF<dim, Number>::MGTransferBlockMF(const MGTransferMF<dim, Number> &transfer_operator)
...
template <int dim, typename Number>
MGTransferBlockMF<dim, Number>::MGTransferBlockMF(const MGConstrainedDoFs &mg_constrained_dofs)
  : MGTransferBlockMatrixFreeBase<dim, Number, MGTransferMF<dim, Number>>(true)
{
...
template <int dim, typename Number>
MGTransferBlockMF<dim, Number>::MGTransferBlockMF(const std::vector<MGConstrainedDoFs> &mg_constrained_dofs)
...
template <int dim, typename Number>
void
MGTransferBlockMF<dim, Number>::initialize_constraints(const MGConstrainedDoFs &mg_constrained_dofs)
{
...
template <int dim, typename Number>
void
MGTransferBlockMF<dim, Number>::initialize_constraints(const std::vector<MGConstrainedDoFs> &mg_constrained_dofs)
{
  this->transfer_operators_internal.clear();
  this->transfer_operators.clear();
...
template <int dim, typename Number>
void
MGTransferBlockMF<dim, Number>::build(const std::vector<const DoFHandler<dim> *> &dof_handler)
{
...
MGTransferBlockMF<dim, Number>::get_matrix_free_transfer(const unsigned int b) const
{
...
namespace internal
{
  namespace
  {
    ...
    fill_mapping_info(const Utilities::MPI::RemotePointEvaluation<dim> &rpe)
    {
      ...
      std::vector<typename Triangulation<dim>::active_cell_iterator> cell_iterators;
      std::vector<std::vector<Point<dim>>>                           unit_points_vector;

      for (unsigned int i = 0; i < cell_data.cells.size(); ++i)
        {
          typename Triangulation<dim>::active_cell_iterator cell(&rpe.get_triangulation(),
      ...
      auto mapping_info =
        std::make_shared<NonMatching::MappingInfo<dim, dim, Number>>(rpe.get_mapping(), update_values);
    ...
    std::tuple<std::vector<unsigned int>, std::vector<unsigned int>, std::vector<types::global_dof_index>>
    support_point_indices_to_dof_indices(const DoFHandler<dim, spacedim>         &dof_handler,
                                         ...
                                         const dealii::AffineConstraints<Number> &constraint)
    {
      ...
      Assert((dynamic_cast<const FE_DGQ<dim, spacedim> *>(&dof_handler.get_fe().base_element(0)) != nullptr) ||
               (dynamic_cast<const FE_Q<dim, spacedim> *>(&dof_handler.get_fe().base_element(0)) != nullptr),
             ExcMessage("Function expects FE_DGQ of FE_Q elements in dof_handler."));

      Assert((dynamic_cast<const FE_Q<dim, spacedim> *>(&dof_handler_support_points.get_fe()) != nullptr) ||
               ((dynamic_cast<const FE_DGQ<dim, spacedim> *>(&dof_handler_support_points.get_fe()) != nullptr) &&
                dof_handler_support_points.get_fe().degree == 0),
             ExcMessage("Function expects FE_DGQ&&degree==0 or FE_Q in dof_handler_support_points."));

      Assert(dof_handler_support_points.get_fe().n_components() == 1,
             ExcMessage("dof_handler_support_points needs element with exactly one component."));

      Assert(&dof_handler.get_triangulation() == &dof_handler_support_points.get_triangulation(),
             ExcMessage("DoFHandlers need the same underlying triangulation."));

      Assert(dof_handler.get_fe().degree == dof_handler_support_points.get_fe().degree,
             ExcMessage("DoFHandlers need the same degree."));

      Assert(dof_handler.get_fe().is_primitive(), ExcMessage("Only primitive elements are allowed."));

      const auto degree                  = dof_handler.get_fe().degree;
      const auto dofs_per_cell           = dof_handler.get_fe().n_dofs_per_cell();
      const auto support_points_per_cell = dof_handler_support_points.get_fe().n_dofs_per_cell();

      std::vector<std::pair<unsigned int, types::global_dof_index>> support_point_dofs;
      ...
      // fill support_point_dofs
      {
        ...
        const bool needs_conversion = dof_handler.get_fe().conforming_space == FiniteElementData<dim>::Conformity::L2 &&
                                      (dof_handler.get_fe().degree > 0);
        std::vector<unsigned int> lexicographic_to_hierarchic;
        if (needs_conversion)
          lexicographic_to_hierarchic = FETools::lexicographic_to_hierarchic_numbering<dim>(degree);

        const Utilities::MPI::Partitioner partitioner_support_points(dof_handler_support_points.locally_owned_dofs(),
                                                                     dof_handler_support_points.get_communicator());

        const Utilities::MPI::Partitioner partitioner_dof(dof_handler.locally_owned_dofs(),
                                                          DoFTools::extract_locally_relevant_dofs(dof_handler),
                                                          dof_handler.get_communicator());

        std::vector<bool> dof_processed(partitioner_dof.locally_owned_size() + partitioner_dof.n_ghost_indices(),
                                        false);


        std::vector<types::global_dof_index>                          support_point_indices(support_points_per_cell);
        std::vector<types::global_dof_index>                          dof_indices(dofs_per_cell);
        std::vector<std::pair<unsigned int, types::global_dof_index>> support_point_dofs_comp;
        ...
        for (const auto &cell : dof_handler.active_cell_iterators())
          {
            if (cell->is_locally_owned() || cell->is_ghost())
              {
                const auto cell_support_point = cell->as_dof_handler_iterator(dof_handler_support_points);
                ...
                for (unsigned int i = 0; i < support_point_indices.size(); ++i)
                  if (partitioner_support_points.in_local_range(support_point_indices[i]))
                    {
                      for (unsigned int c = 0; c < n_components; ++c)
                        {
                          const auto global_dof_idx =
                            needs_conversion ? dof_indices[dof_handler.get_fe().component_to_system_index(
                                                 c, lexicographic_to_hierarchic[i])] :
                                               dof_indices[dof_handler.get_fe().component_to_system_index(c, i)];

                          const auto local_dof_idx = partitioner_dof.global_to_local(global_dof_idx);
                          ...
                          if (dof_processed[local_dof_idx] == false)
                            {
                              if (!constraint.is_constrained(global_dof_idx))
                                support_point_dofs_comp.emplace_back(
                                  partitioner_support_points.global_to_local(support_point_indices[i]), global_dof_idx);

                              dof_processed[local_dof_idx] = true;
                            }
                        }

                      Assert(support_point_dofs_comp.size() == 0 || support_point_dofs_comp.size() == n_components,
                             ExcNotImplemented());
      ...
      std::stable_sort(support_point_dofs.begin(), support_point_dofs.end(), [](const auto &a, const auto &b) {
      ...
      support_point_indices.reserve(dof_handler_support_points.n_locally_owned_dofs());
      ...
      return std::make_tuple(std::move(support_point_indices), std::move(dof_ptrs), std::move(dof_indices));
    }
    ...
    create_support_point_dof_handler(const DoFHandler<dim, spacedim> &dof_handler)
    {
      ...
      if (n_components == 1 && (fe.conforming_space == FiniteElementData<dim>::Conformity::H1 || degree == 0))
        {
          // in case a DG space of order 0 is provided, DoFs indices are always
          // uniquely assigned to support points (they are always defined in the
          // center of the element) and are never shared at vertices or faces.
          return std::shared_ptr<const DoFHandler<dim, spacedim>>(&dof_handler, [](auto *) {});
        }
      else
        {
          ...
          // center of the cell.
          auto dof_handler_support_points = std::make_shared<DoFHandler<dim, spacedim>>(tria);

          if (degree == 0)
            dof_handler_support_points->distribute_dofs(FE_DGQ<dim, spacedim>(degree));
          else
            dof_handler_support_points->distribute_dofs(FE_Q<dim, spacedim>(degree));
    ...
    template <int dim, typename Number>
    std::tuple<std::vector<Point<dim>>, std::vector<unsigned int>, std::vector<types::global_dof_index>>
    collect_unconstrained_unique_support_points(const DoFHandler<dim>                   &dof_handler,
                                                ...
                                                const dealii::AffineConstraints<Number> &constraint)
    {
      ...
      const auto dof_handler_support_points = create_support_point_dof_handler(dof_handler);

      // compute mapping: index of locally owned support points to (global) DoF
      // indices
      const auto support_point_dofs_crs =
        support_point_indices_to_dof_indices(dof_handler, *dof_handler_support_points, constraint);

      const std::vector<unsigned int> &local_support_point_indices = std::get<0>(support_point_dofs_crs);

      // compute locally owned support points
      std::vector<Point<dim>> points;
      points.resize(local_support_point_indices.size());

      const auto                locally_owned_support_point = dof_handler_support_points->locally_owned_dofs();
      std::vector<unsigned int> indices_state(locally_owned_support_point.n_elements(), numbers::invalid_unsigned_int);

      AssertIndexRange(local_support_point_indices.size(), indices_state.size() + 1);
      ...
      FEValues<dim> fe_values(mapping,
                              fe_support_point,
                              Quadrature<dim>(fe_support_point.get_unit_support_points()),
                              update_quadrature_points);

      std::vector<types::global_dof_index> dof_indices(fe_support_point.n_dofs_per_cell());

      for (const auto &cell : dof_handler_support_points->active_cell_iterators() | IteratorFilters::LocallyOwnedCell())
        {
          ...
          for (const unsigned int q : fe_values.quadrature_point_indices())
            if (locally_owned_support_point.is_element(dof_indices[q]))
              {
                const auto index = locally_owned_support_point.index_within_set(dof_indices[q]);
...
template <int dim, typename Number>
void
MGTwoLevelTransferNonNested<dim, LinearAlgebra::distributed::Vector<Number>>::reinit(
  const DoFHandler<dim>           &dof_handler_fine,
  const DoFHandler<dim>           &dof_handler_coarse,
...
{
  AssertThrow(dof_handler_coarse.get_fe().has_support_points(), ExcNotImplemented());
  Assert(dof_handler_coarse.get_fe().n_components() > 0 && dof_handler_fine.get_fe().n_components() > 0,
         ExcNotImplemented());

  this->fine_element_is_continuous = dof_handler_fine.get_fe().n_dofs_per_vertex() > 0;

  // collect points, ptrs, and global indices
  const auto points_ptrs_indices =
    internal::collect_unconstrained_unique_support_points(dof_handler_fine, mapping_fine, constraint_fine);
  ...
  // create partitioners and internal vectors
  {
    this->partitioner_coarse =
      internal::MGTwoLevelTransferImplementation::create_coarse_partitioner(dof_handler_coarse,
                                                                            constraint_coarse,
  ...
  }
  {
    ...
    if (!this->fine_element_is_continuous && dof_handler_fine.get_fe().degree != 0)
      locally_relevant_dofs.add_indices(global_dof_indices.begin(), global_dof_indices.end());

    this->partitioner_fine.reset(new Utilities::MPI::Partitioner(dof_handler_fine.locally_owned_dofs(),
                                                                 locally_relevant_dofs,
                                                                 dof_handler_fine.get_communicator()));
  ...
  if (dof_handler_fine.get_fe().n_components() == 1 &&
      (this->fine_element_is_continuous || dof_handler_fine.get_fe().degree == 0))
  ...
  for (unsigned int i = 0; i < global_dof_indices.size(); ++i)
    this->level_dof_indices_fine[i] = this->partitioner_fine->global_to_local(global_dof_indices[i]);
  ...
  constraint_info.reinit(dof_handler_coarse, cell_data.cells.size(), false /*TODO*/);

  for (unsigned int i = 0; i < cell_data.cells.size(); ++i)
    {
      typename DoFHandler<dim>::active_cell_iterator cell(&rpe.get_triangulation(),
                                                          cell_data.cells[i].first,
                                                          cell_data.cells[i].second,
                                                          &dof_handler_coarse);

      constraint_info.read_dof_indices(
        i, numbers::invalid_unsigned_int, cell, constraint_coarse, this->partitioner_coarse);
    }
  ...
  if (const auto fe = dynamic_cast<const FE_Q<dim> *>(&fe_base))
    fe_coarse = std::make_unique<FESystem<dim>>(FE_DGQ<dim>(fe->get_degree()), n_components);
...
void
MGTwoLevelTransferNonNested<dim, LinearAlgebra::distributed::Vector<Number>>::prolongate_and_add_internal_comp(
  LinearAlgebra::distributed::Vector<Number>       &dst,
  const LinearAlgebra::distributed::Vector<Number> &src) const
{
  using Traits     = internal::FEPointEvaluation::EvaluatorTypeTraits<dim, n_components, Number>;
  ...
  const auto evaluation_function = [&](auto &values, const auto &cell_data) {
    ...
    FEPointEvaluation<n_components, dim, dim, Number> evaluator(*mapping_info, *fe_coarse);
    ...
    for (unsigned int cell = 0; cell < cell_data.cells.size(); ++cell)
      {
        ...
        for (const auto q : evaluator.quadrature_point_indices())
          values[q + cell_data.reference_point_ptrs[cell]] = evaluator.get_value(q);
      }
  };

  rpe.template evaluate_and_process<value_type>(evaluation_point_results, buffer, evaluation_function);
      ...
      if (level_dof_indices_fine_ptrs.empty())
        {
          for (unsigned int c = 0; c < n_components; ++c)
            {
              AssertIndexRange(n_components * j + c, this->level_dof_indices_fine.size());
              dst.local_element(this->level_dof_indices_fine[n_components * j + c]) +=
                internal::access(evaluation_point_results[j], c);
            }
        }
      else
        {
          for (unsigned int i = this->level_dof_indices_fine_ptrs[j]; i < this->level_dof_indices_fine_ptrs[j + 1]; ++i)
            for (unsigned int c = 0; c < n_components; ++c)
              {
                AssertIndexRange(n_components * i + c, this->level_dof_indices_fine.size());
                dst.local_element(this->level_dof_indices_fine[n_components * i + c]) +=
                  internal::access(evaluation_point_results[j], c);
              }
        }
    }
...
template <int dim, typename Number>
void
MGTwoLevelTransferNonNested<dim, LinearAlgebra::distributed::Vector<Number>>::prolongate_and_add_internal(
  LinearAlgebra::distributed::Vector<Number>       &dst,
  const LinearAlgebra::distributed::Vector<Number> &src) const
{
...
template <int n_components>
void
MGTwoLevelTransferNonNested<dim, LinearAlgebra::distributed::Vector<Number>>::restrict_and_add_internal_comp(
  LinearAlgebra::distributed::Vector<Number>       &dst,
  const LinearAlgebra::distributed::Vector<Number> &src) const
{
  using Traits     = internal::FEPointEvaluation::EvaluatorTypeTraits<dim, n_components, Number>;
          ...
          for (unsigned int c = 0; c < n_components; ++c)
            {
              ...
              internal::access(evaluation_point_results[j], c) =
                src.local_element(this->level_dof_indices_fine[n_components * j + c]);
            }
        }
      else
        {
            ...
            for (unsigned int c = 0; c < n_components; ++c)
              {
                AssertIndexRange(n_components * i + c, this->level_dof_indices_fine.size());
                internal::access(evaluation_point_results[j], c) +=
                  src.local_element(this->level_dof_indices_fine[n_components * i + c]);
              }
        }
    }
  ...
  const auto evaluation_function = [&](const auto &values, const auto &cell_data) {
    std::vector<Number>                               solution_values;
    FEPointEvaluation<n_components, dim, dim, Number> evaluator(*mapping_info, *fe_coarse);
    ...
    for (unsigned int cell = 0; cell < cell_data.cells.size(); ++cell)
      {
        ...
        for (const auto q : evaluator.quadrature_point_indices())
          evaluator.submit_value(values[q + cell_data.reference_point_ptrs[cell]], q);
  ...
  rpe.template process_and_evaluate<value_type>(evaluation_point_results, buffer, evaluation_function);
}
...
template <int dim, typename Number>
void
MGTwoLevelTransferNonNested<dim, LinearAlgebra::distributed::Vector<Number>>::restrict_and_add_internal(
  LinearAlgebra::distributed::Vector<Number>       &dst,
  const LinearAlgebra::distributed::Vector<Number> &src) const
{
  if (this->fe_coarse->n_components() == 1)
...
template <int dim, typename Number>
void
MGTwoLevelTransferNonNested<dim, LinearAlgebra::distributed::Vector<Number>>::interpolate(
  LinearAlgebra::distributed::Vector<Number>       &dst,
  const LinearAlgebra::distributed::Vector<Number> &src) const
{
  AssertThrow(false, ExcNotImplemented());
...
template <int dim, typename Number>
void
MGTwoLevelTransferNonNested<dim, LinearAlgebra::distributed::Vector<Number>>::enable_inplace_operations_if_possible(
  const std::shared_ptr<const Utilities::MPI::Partitioner> &external_partitioner_coarse,
  const std::shared_ptr<const Utilities::MPI::Partitioner> &external_partitioner_fine)
{
  this->internal_enable_inplace_operations_if_possible(external_partitioner_coarse,
...
template <int dim, typename Number>
std::size_t
MGTwoLevelTransferNonNested<dim, LinearAlgebra::distributed::Vector<Number>>::memory_consumption() const
{
  std::size_t size = 0;

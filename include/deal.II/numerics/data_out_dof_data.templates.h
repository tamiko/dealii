...
namespace internal
{
  namespace DataOutImplementation
  {
     ...
     */
    inline void
    generate_simplex_evaluation_points_recursively(const std::vector<Point<2>> &bounding_vertices,
                                                   const unsigned int           n_subdivisions,
                                                   std::vector<Point<2>>       &evaluation_points)
    {
      ...
      const auto generate_inbetween_points = [&](const Point<2> &p0, const Point<2> &p1) {
        std::vector<Point<2>> line_points;

        for (unsigned int i = 1; i < n_subdivisions; ++i)
          line_points.push_back(p0 + (p1 - p0) / n_subdivisions * i);

        evaluation_points.insert(evaluation_points.end(), line_points.begin(), line_points.end());

        return line_points;
      };

      const auto line_points_0 = generate_inbetween_points(bounding_vertices[0], bounding_vertices[1]);
      const auto line_points_1 = generate_inbetween_points(bounding_vertices[1], bounding_vertices[2]);
      const auto line_points_2 = generate_inbetween_points(bounding_vertices[2], bounding_vertices[0]);
      ...
      generate_simplex_evaluation_points_recursively(
        ...
        {{Point<2>(line_points_0[line_points_0.size() - 2][0], line_points_1[0][1]),
          Point<2>(line_points_0[0][0], line_points_1[line_points_1.size() - 2][1]),
    ...
    generate_simplex_evaluation_points(const unsigned int n_subdivisions)
    {
      ...
      generate_simplex_evaluation_points_recursively({{Point<2>(0.0, 0.0), Point<2>(1.0, 0.0), Point<2>(0.0, 1.0)}},
                                                     n_subdivisions,
                                                     evaluation_points);
      ...
      std::vector<std::shared_ptr<dealii::hp::FEFaceValues<dim, spacedim>>>       &x_fe_face_values)
    {
      ...
      static const auto has_fe_with_reference_cell = [](const dealii::hp::FECollection<dim, spacedim> &fe_collection,
      ...
      const bool needs_hypercube_setup =
        std::any_of(finite_elements.begin(),
                    finite_elements.end(),
                    [](const std::shared_ptr<dealii::hp::FECollection<dim, spacedim>> &fe_collection) {
                      return has_fe_with_reference_cell(*fe_collection, ReferenceCells::get_hypercube<dim>());
                    });
      const bool needs_simplex_setup =
        (dim > 1 && std::any_of(finite_elements.begin(),
                                finite_elements.end(),
                                [](const std::shared_ptr<dealii::hp::FECollection<dim, spacedim>> &fe_collection) {
                                  return has_fe_with_reference_cell(*fe_collection, ReferenceCells::get_simplex<dim>());
                                }));
      const bool needs_wedge_setup =
        (dim == 3 && std::any_of(finite_elements.begin(),
                                 finite_elements.end(),
                                 [](const std::shared_ptr<dealii::hp::FECollection<dim, spacedim>> &fe_collection) {
                                   return has_fe_with_reference_cell(*fe_collection, ReferenceCells::Wedge);
                                 }));
      const bool needs_pyramid_setup =
        (dim == 3 && std::any_of(finite_elements.begin(),
                                 finite_elements.end(),
                                 [](const std::shared_ptr<dealii::hp::FECollection<dim, spacedim>> &fe_collection) {
                                   return has_fe_with_reference_cell(*fe_collection, ReferenceCells::Pyramid);
                                 }));
      ...
      if (use_face_values == false)
        {
          ...
          if (needs_simplex_setup)
            {
              if (dim == 2)
                quadrature_simplex =
                  std::make_unique<Quadrature<dim>>(generate_simplex_evaluation_points<dim>(n_subdivisions));
              else
                quadrature_simplex =
                  std::make_unique<Quadrature<dim>>(FE_SimplexP<dim>(n_subdivisions).get_unit_support_points());
            }

          if (needs_hypercube_setup)
            {
              quadrature_hypercube = std::make_unique<QIterated<dim>>(QTrapezoid<1>(), n_subdivisions);
            }
          ...
          if (needs_wedge_setup)
            {
              ...
              quadrature_wedge =
                std::make_unique<Quadrature<dim>>(ReferenceCells::Wedge.get_nodal_type_quadrature<dim>());
            }
          ...
          if (needs_pyramid_setup)
            {
              ...
              quadrature_pyramid =
                std::make_unique<Quadrature<dim>>(ReferenceCells::Pyramid.get_nodal_type_quadrature<dim>());
            }
              ...
              if (x_fe_values[i].get() == nullptr)
                {
                  ...
                  for (unsigned int j = 0; j < finite_elements[i]->size(); ++j)
                    {
                      const ReferenceCell reference_cell = (*finite_elements[i])[j].reference_cell();
                  ...
                  x_fe_values[i] = std::make_shared<dealii::hp::FEValues<dim, spacedim>>(
                    ...
                    *finite_elements[i],
                    ...
                    (needs_simplex_setup || needs_wedge_setup || needs_pyramid_setup) ?
          ...
          return std::max({needs_wedge_setup ? quadrature_wedge->size() : 0,
      ...
      else // build FEFaceValues objects instead
        {
          ...
          Assert(needs_pyramid_setup == false && needs_wedge_setup == false, ExcNotImplemented());
          ...
          // quadrature rule).
          if ((dim == 3) && (needs_simplex_setup || needs_pyramid_setup || needs_wedge_setup))
            {
              quadrature_simplex =
                std::make_unique<Quadrature<dim - 1>>(generate_simplex_evaluation_points<dim - 1>(n_subdivisions));
            }

          if ((dim < 3) || (needs_hypercube_setup || needs_pyramid_setup || needs_wedge_setup))
            {
              quadrature_hypercube = std::make_unique<QIterated<dim - 1>>(QTrapezoid<1>(), n_subdivisions);
            }
                  ...
                  for (unsigned int j = 0; j < finite_elements[i]->size(); ++j)
                    {
                      ...
                      if ((dim < 3) || (reference_cell.is_hyper_cube() || (reference_cell == ReferenceCells::Wedge) ||
                      ...
                      if ((dim == 3) && (reference_cell.is_simplex() || (reference_cell == ReferenceCells::Wedge) ||
                                         (reference_cell == ReferenceCells::Pyramid)))
                        quadrature.push_back(*quadrature_simplex);
                    }

                  x_fe_face_values[i] = std::make_shared<dealii::hp::FEFaceValues<dim, spacedim>>(mapping_collection,
                                                                                                  *finite_elements[i],
                                                                                                  quadrature,
                                                                                                  update_flags);
                }
          ...
          // Return maximal number of evaluation points:
          return std::max({(dim == 3) && (needs_simplex_setup || needs_pyramid_setup || needs_wedge_setup) ?
                             quadrature_simplex->size() :
                             0,
                           (dim < 3) || needs_hypercube_setup ? quadrature_hypercube->size() : 0});
        }
    }
    ...
    {
      const unsigned int n_q_points = setup_parallel_data_base_internal(
        n_subdivisions, mapping, finite_elements, update_flags, use_face_values, x_fe_values, x_fe_face_values);
      ...
      for (unsigned int dataset = 0; dataset < n_postprocessor_outputs.size(); ++dataset)
        if (n_postprocessor_outputs[dataset] != 0)
          postprocessed_values[dataset].resize(n_q_points, dealii::Vector<double>(n_postprocessor_outputs[dataset]));
    }
      ...
      for (unsigned int dataset = 0; dataset < dof_data.size(); ++dataset)
        {
          const bool is_duplicate =
            std::any_of(finite_elements.cbegin(),
                        finite_elements.cbegin() + dataset,
                        [&](const std::shared_ptr<dealii::hp::FECollection<dim, spacedim>> &fe) {
                          return finite_elements[dataset].get() == fe.get();
                        });
              ...
              if (cell->is_active())
                {
                  const typename DoFHandler<dim, spacedim>::active_cell_iterator dh_cell(
                    &cell->get_triangulation(), cell->level(), cell->index(), dof_data[dataset]->dof_handler);
    ...
    ParallelDataBase<dim, spacedim>::resize_system_vectors(const unsigned int n_components)
    {
      ...
      AssertDimension(patch_values_system.solution_values.size(), patch_values_system.solution_gradients.size());
      AssertDimension(patch_values_system.solution_values.size(), patch_values_system.solution_hessians.size());
...
namespace internal
{
  namespace DataOutImplementation
  {
    ...
    Tensor<rank, dim>
    get_component(const Tensor<rank, dim, NumberType> &value, const ComponentExtractor extract_component)
    {
      Assert(extract_component == ComponentExtractor::real_part,
             ExcMessage("You cannot extract anything other than the real "
                        "part from a real number."));
    ...
    DataEntryBase<dim, spacedim>::DataEntryBase(
      ...
      : dof_handler(dofs, typeid(dealii::DataOut_DoFData<dim, dim, spacedim, spacedim>).name())
      , names(names_in)
    ...
    template <int dim, int spacedim>
    DataEntryBase<dim, spacedim>::DataEntryBase(const DoFHandler<dim, spacedim>   *dofs,
      ...
      , n_output_variables(names.size())
    {
      Assert(data_postprocessor->get_names().size() == data_postprocessor->get_data_component_interpretation().size(),
    ...
    namespace
    {
      ...
      void
      copy_locally_owned_data_from(const VectorType &src, LinearAlgebra::distributed::Vector<Number> &dst)
      {
      ...
      {
        const IndexSet &locally_owned_dofs = (level == numbers::invalid_unsigned_int) ?
                                               dof_handler.locally_owned_dofs() :
                                               dof_handler.locally_owned_mg_dofs(level);

        const IndexSet locally_relevant_dofs = (level == numbers::invalid_unsigned_int) ?
                                                 DoFTools::extract_locally_relevant_dofs(dof_handler) :
                                                 DoFTools::extract_locally_relevant_level_dofs(dof_handler, level);

        std::vector<types::global_dof_index> n_indices_per_block(src.n_blocks());

        for (unsigned int b = 0; b < src.n_blocks(); ++b)
          n_indices_per_block[b] = src.get_block_indices().block_size(b);

        const auto locally_owned_dofs_b    = locally_owned_dofs.split_by_block(n_indices_per_block);
        const auto locally_relevant_dofs_b = locally_relevant_dofs.split_by_block(n_indices_per_block);
        ...
        for (unsigned int b = 0; b < src.n_blocks(); ++b)
          {
            Assert(src.block(b).locally_owned_elements().is_contiguous(),
                   ExcMessage("Using non-contiguous vector blocks is not currently "
            ...
            dst.block(b).reinit(locally_owned_dofs_b[b], locally_relevant_dofs_b[b], dof_handler.get_communicator());
                        ...
                        const unsigned int                               level = numbers::invalid_unsigned_int)
      {
        ...
        Assert(locally_owned_dofs.is_contiguous(),
               ExcMessage("You are trying to add a non-block vector with non-contiguous "
                          "locally-owned index sets. This is not possible. Please "
                          "consider to use an adequate block vector!"));

        dst.reinit(1);

        dst.block(0).reinit(locally_owned_dofs, locally_relevant_dofs, dof_handler.get_communicator());
      ...
      template <typename VectorType,
                ...
                std::enable_if_t<IsBlockVector<VectorType>::value, VectorType> * = nullptr>
      void
      create_cell_vector(const VectorType &src, LinearAlgebra::distributed::BlockVector<Number> &dst)
      {
        dst.reinit(src.n_blocks());
    ...
    public:
       ...
       */
      virtual double
      get_cell_data_value(const unsigned int cell_number, const ComponentExtractor extract_component) const override;
      ...
      get_function_gradients(const FEValuesBase<dim, spacedim>             &fe_patch_values,
                             const ComponentExtractor                       extract_component,
                             std::vector<std::vector<Tensor<1, spacedim>>> &patch_gradients_system) const override;
      ...
      get_function_hessians(const FEValuesBase<dim, spacedim>             &fe_patch_values,
                            const ComponentExtractor                       extract_component,
                            std::vector<std::vector<Tensor<2, spacedim>>> &patch_hessians_system) const override;
    ...
    template <int dim, int spacedim, typename ScalarType>
    template <typename VectorType>
    DataEntry<dim, spacedim, ScalarType>::DataEntry(const DoFHandler<dim, spacedim>   *dofs,
                                                    const VectorType                  *data,
                                                    const DataPostprocessor<spacedim> *data_postprocessor)
    ...
    template <int dim, int spacedim, typename ScalarType>
    double
    DataEntry<dim, spacedim, ScalarType>::get_cell_data_value(const unsigned int       cell_number,
                                                              const ComponentExtractor extract_component) const
    {
      return get_component(
        internal::ElementAccess<LinearAlgebra::distributed::BlockVector<ScalarType>>::get(vector, cell_number),
        extract_component);
    }
    ...
    template <int dim, int spacedim, typename ScalarType>
    void
    DataEntry<dim, spacedim, ScalarType>::get_function_values(const FEValuesBase<dim, spacedim> &fe_patch_values,
                                                              const ComponentExtractor           extract_component,
    ...
    DataEntry<dim, spacedim, ScalarType>::get_function_gradients(
      const FEValuesBase<dim, spacedim>             &fe_patch_values,
      const ComponentExtractor                       extract_component,
      std::vector<std::vector<Tensor<1, spacedim>>> &patch_gradients_system) const
    {
      if constexpr (std::is_same_v<ScalarType, double>)
        {
          (void)extract_component;
      ...
      else
        {
          ...
          std::vector<std::vector<Tensor<1, spacedim, ScalarType>>> tmp(n_eval_points);
          for (unsigned int i = 0; i < n_eval_points; ++i)
    ...
    DataEntry<dim, spacedim, ScalarType>::get_function_hessians(
      const FEValuesBase<dim, spacedim>             &fe_patch_values,
      const ComponentExtractor                       extract_component,
      std::vector<std::vector<Tensor<2, spacedim>>> &patch_hessians_system) const
    {
      if constexpr (std::is_same_v<ScalarType, double>)
        {
          (void)extract_component;
      ...
      else
        {
          ...
          std::vector<std::vector<Tensor<2, spacedim, ScalarType>>> tmp(n_eval_points);
          for (unsigned int i = 0; i < n_eval_points; ++i)
            tmp[i].resize(n_components);
    ...
    template <int dim, int spacedim, typename ScalarType>
    void
    DataEntry<dim, spacedim, ScalarType>::get_function_hessians(const FEValuesBase<dim, spacedim> &fe_patch_values,
                                                                const ComponentExtractor           extract_component,
                                                                std::vector<Tensor<2, spacedim>>  &patch_hessians) const
    {
      if constexpr (std::is_same_v<ScalarType, double>)
        ...
        {
          std::vector<Tensor<2, spacedim, ScalarType>> tmp(patch_hessians.size());
    ...
    {
      return (vector.memory_consumption() + MemoryConsumption::memory_consumption(this->names));
    }
    ...
    public:
      ...
      is_complex_valued() const override
      {
        Assert(numbers::NumberTraits<ScalarType>::is_complex == false, ExcNotImplemented());
        return numbers::NumberTraits<ScalarType>::is_complex;
      }
    ...
    private:
      MGLevelObject<LinearAlgebra::distributed::BlockVector<ScalarType>> vectors;
    ...
    template <int dim, int spacedim, typename ScalarType>
    double
    MGDataEntry<dim, spacedim, ScalarType>::get_cell_data_value(const unsigned int       cell_number,
                                                                const ComponentExtractor extract_component) const
    {
      Assert(false, ExcNotImplemented());
    ...
    template <int dim, int spacedim, typename ScalarType>
    void
    MGDataEntry<dim, spacedim, ScalarType>::get_function_values(const FEValuesBase<dim, spacedim> &fe_patch_values,
                                                                const ComponentExtractor           extract_component,
                                                                std::vector<double>               &patch_values) const
    {
      Assert(extract_component == ComponentExtractor::real_part, ExcNotImplemented());
      ...
      const unsigned int dofs_per_cell = this->dof_handler->get_fe(0).n_dofs_per_cell();
      ...
      std::vector<dealii::Vector<double>> &patch_values_system) const
    {
      ...
      typename DoFHandler<dim, spacedim>::level_cell_iterator dof_cell(&fe_patch_values.get_cell()->get_triangulation(),
                                                                       fe_patch_values.get_cell()->level(),
                                                                       fe_patch_values.get_cell()->index(),
                                                                       this->dof_handler);
      ...
      for (unsigned int q = 0; q < n_eval_points; ++q)
        {
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            for (unsigned int c = 0; c < n_components; ++c)
              patch_values_system[q](c) += values[i] * fe_patch_values.shape_value_component(i, q, c);
        }
    }
...
template <int dim, int patch_dim, int spacedim, int patch_spacedim>
void
DataOut_DoFData<dim, patch_dim, spacedim, patch_spacedim>::attach_dof_handler(const DoFHandler<dim, spacedim> &d)
{
  Assert(dof_data.empty(), Exceptions::DataOutImplementation::ExcOldDataStillPresent());
  Assert(cell_data.empty(), Exceptions::DataOutImplementation::ExcOldDataStillPresent());

  triangulation = SmartPointer<const Triangulation<dim, spacedim>>(&d.get_triangulation(), typeid(*this).name());
  dofs          = SmartPointer<const DoFHandler<dim, spacedim>>(&d, typeid(*this).name());
}
  ...
  const Triangulation<dim, spacedim> &tria)
{
  ...
  triangulation = SmartPointer<const Triangulation<dim, spacedim>>(&tria, typeid(*this).name());
}
  ...
  const DataPostprocessor<spacedim> &data_postprocessor)
{
  ...
  if (triangulation != nullptr)
    ...
    {
      triangulation =
        SmartPointer<const Triangulation<dim, spacedim>>(&dof_handler.get_triangulation(), typeid(*this).name());
    }
  ...
  auto new_entry = std::make_unique<
    internal::DataOutImplementation::
      DataEntry<dim, spacedim, typename numbers::NumberTraits<typename VectorType::value_type>::double_type>>(
...
template <typename VectorType>
void
DataOut_DoFData<dim, patch_dim, spacedim, patch_spacedim>::add_data_vector_internal(
  const DoFHandler<dim, spacedim>                                             *dof_handler,
  ...
  const bool                                                                   deduce_output_names)
{
  ...
  auto new_entry = std::make_unique<
    internal::DataOutImplementation::
      DataEntry<dim, spacedim, typename numbers::NumberTraits<typename VectorType::value_type>::double_type>>(
    dof_handler, &data_vector, deduced_names, data_component_interpretation, actual_type);
  ...
  const std::vector<DataComponentInterpretation::DataComponentInterpretation> &data_component_interpretation_)
{
  ...
  Assert(deduced_names.size() == n_components, ExcMessage("Invalid number of names given."));

  const std::vector<DataComponentInterpretation::DataComponentInterpretation> &data_component_interpretation =
    (data_component_interpretation_.size() != 0 ? data_component_interpretation_ :
                                                  std::vector<DataComponentInterpretation::DataComponentInterpretation>(
                                                    n_components, DataComponentInterpretation::component_is_scalar));

  Assert(data_component_interpretation.size() == n_components,
         ExcMessage("Invalid number of entries in data_component_interpretation."));

  auto new_entry =
    std::make_unique<internal::DataOutImplementation::MGDataEntry<dim, spacedim, typename VectorType::value_type>>(
...
template <int dim, int patch_dim, int spacedim, int patch_spacedim>
void
DataOut_DoFData<dim, patch_dim, spacedim, patch_spacedim>::clear_input_data_references()
{
...
DataOut_DoFData<dim, patch_dim, spacedim, patch_spacedim>::get_dataset_names() const
{
  ...
  for (const auto &input_data : dof_data)
    if (input_data->is_complex_valued() == false || (input_data->postprocessor != nullptr))
      {
                ...
                case DataComponentInterpretation::component_is_part_of_tensor:
                  {
                    ...
                    for (unsigned int tensor_comp = 0; tensor_comp < size; ++tensor_comp)
                      names.push_back(input_data->names[i + tensor_comp] + "_re");
                    for (unsigned int tensor_comp = 0; tensor_comp < size; ++tensor_comp)
                      names.push_back(input_data->names[i + tensor_comp] + "_im");
...
DataOut_DoFData<dim, patch_dim, spacedim, patch_spacedim>::get_nonscalar_data_ranges() const
{
  std::vector<
    std::tuple<unsigned int, unsigned int, std::string, DataComponentInterpretation::DataComponentInterpretation>>
    ranges;
          ...
          case DataComponentInterpretation::component_is_scalar:
            {
              ...
              ++i;
              output_component += (input_data->is_complex_valued() && (input_data->postprocessor == nullptr) ? 2 : 1);
          ...
          case DataComponentInterpretation::component_is_part_of_vector:
            {
              ...
              Assert(i + patch_spacedim <= input_data->n_output_variables,
                     Exceptions::DataOutImplementation::ExcInvalidVectorDeclaration(i, input_data->names[i]));
              for (unsigned int dd = 1; dd < patch_spacedim; ++dd)
                Assert(input_data->data_component_interpretation[i + dd] ==
              ...
              if (input_data->is_complex_valued() == false || (input_data->postprocessor != nullptr))
                {
                  ranges.emplace_back(std::forward_as_tuple(output_component,
          ...
          case DataComponentInterpretation::component_is_part_of_tensor:
            {
              ...
              Assert(i + size <= input_data->n_output_variables,
                     Exceptions::DataOutImplementation::ExcInvalidTensorDeclaration(i, input_data->names[i]));
...
{
  std::vector<std::shared_ptr<dealii::hp::FECollection<dim, spacedim>>> finite_elements(this->dof_data.size());
  for (unsigned int i = 0; i < this->dof_data.size(); ++i)
    {
      Assert(dof_data[i]->dof_handler != nullptr, Exceptions::DataOutImplementation::ExcNoDoFHandlerSelected());
      ...
      if (duplicate == false)
        finite_elements[i] = std::make_shared<dealii::hp::FECollection<dim, spacedim>>(
      ...
      for (const auto &reference_cell : reference_cells)
        {
          if (reference_cell.is_hyper_cube())
            finite_elements.emplace_back(
              std::make_shared<dealii::hp::FECollection<dim, spacedim>>(FE_DGQ<dim, spacedim>(0)));
          else if (reference_cell.is_simplex())
            finite_elements.emplace_back(
              std::make_shared<dealii::hp::FECollection<dim, spacedim>>(FE_SimplexDGP<dim, spacedim>(1)));
          else if (reference_cell == ReferenceCells::Wedge)
            finite_elements.emplace_back(
              std::make_shared<dealii::hp::FECollection<dim, spacedim>>(FE_WedgeDGP<dim, spacedim>(1)));
          else if (reference_cell == ReferenceCells::Pyramid)
            finite_elements.emplace_back(
              std::make_shared<dealii::hp::FECollection<dim, spacedim>>(FE_PyramidDGP<dim, spacedim>(1)));
          else
            Assert(false, ExcNotImplemented());
        }
    }
...
template <int dim, int patch_dim, int spacedim, int patch_spacedim>
std::size_t
DataOut_DoFData<dim, patch_dim, spacedim, patch_spacedim>::memory_consumption() const
{
  return (DataOutInterface<patch_dim, patch_spacedim>::memory_consumption() +
          MemoryConsumption::memory_consumption(dofs) + MemoryConsumption::memory_consumption(patches));
}

...
namespace internal
{
  ...
  ParallelData<dim, spacedim, number>::ParallelData(
    ...
    , fe_face_values_cell(mapping,
                          ...
                          update_gradients | update_JxW_values |
                            (need_quadrature_points ? update_quadrature_points : UpdateFlags()) | update_normal_vectors)
    , fe_face_values_neighbor(mapping, finite_element, face_quadratures, update_gradients | update_normal_vectors)
    , fe_subface_values(mapping, finite_element, face_quadratures, update_gradients | update_normal_vectors)
    ...
    , psi(n_solution_vectors,
          std::vector<std::vector<Tensor<1, spacedim, number>>>(face_quadratures.max_n_quadrature_points(),
    ...
    , coefficient_values(face_quadratures.max_n_quadrature_points(), dealii::Vector<double>(fe.n_components()))
  ...
  template <int dim, int spacedim, typename number>
  void
  ParallelData<dim, spacedim, number>::resize(const unsigned int active_fe_index)
  {
  ...
  copy_local_to_global(
    const std::map<typename DoFHandler<dim, spacedim>::face_iterator, std::vector<double>> &local_face_integrals,
    std::map<typename DoFHandler<dim, spacedim>::face_iterator, std::vector<double>>       &face_integrals)
  {
    // now copy locally computed elements into the global map
    for (typename std::map<typename DoFHandler<dim, spacedim>::face_iterator, std::vector<double>>::const_iterator p =
  ...
  template <int dim, int spacedim, typename number>
  std::vector<double>
  integrate_over_face(ParallelData<dim, spacedim, number>                     &parallel_data,
                      ...
                      dealii::hp::FEFaceValues<dim, spacedim>                 &fe_face_values_cell)
  {
    ...
    parallel_data.normal_vectors = fe_face_values_cell.get_present_fe_values().get_normal_vectors();
          ...
          for (unsigned int component = 0; component < n_components; ++component)
            for (unsigned int p = 0; p < n_q_points; ++p)
              parallel_data.phi[n][p][component] +=
                (parallel_data.neighbor_psi[n][p][component] * parallel_data.neighbor_normal_vectors[p]);
      }
    ...
    if (parallel_data.coefficients != nullptr)
      {
        ...
        if (parallel_data.coefficients->n_components == 1)
          {
            parallel_data.coefficients->value_list(fe_face_values_cell.get_present_fe_values().get_quadrature_points(),
                                                   parallel_data.coefficient_values1);
            for (unsigned int n = 0; n < n_solution_vectors; ++n)
              for (unsigned int component = 0; component < n_components; ++component)
                for (unsigned int point = 0; point < n_q_points; ++point)
                  parallel_data.phi[n][point][component] *= parallel_data.coefficient_values1[point];
          }
        else
          // vector-valued coefficient
          {
            parallel_data.coefficients->vector_value_list(
              fe_face_values_cell.get_present_fe_values().get_quadrature_points(), parallel_data.coefficient_values);
            for (unsigned int n = 0; n < n_solution_vectors; ++n)
              for (unsigned int component = 0; component < n_components; ++component)
                for (unsigned int point = 0; point < n_q_points; ++point)
                  parallel_data.phi[n][point][component] *= parallel_data.coefficient_values[point](component);
          }
      }
    ...
    if (face->at_boundary() == true)
      ...
      // and boundary function
      {
        ...
        Assert(parallel_data.neumann_bc->find(boundary_id) != parallel_data.neumann_bc->end(), ExcInternalError());
        ...
        if (n_components == 1)
          {
            ...
            parallel_data.neumann_bc->find(boundary_id)
              ->second->value_list(fe_face_values_cell.get_present_fe_values().get_quadrature_points(), g);
          ...
          {
            std::vector<dealii::Vector<number>> g(n_q_points, dealii::Vector<number>(n_components));
            parallel_data.neumann_bc->find(boundary_id)
              ->second->vector_value_list(fe_face_values_cell.get_present_fe_values().get_quadrature_points(), g);
  ...
  template <int dim, int spacedim>
  double
  boundary_face_factor(const typename DoFHandler<dim, spacedim>::active_cell_iterator &cell,
    ...
    switch (strategy)
      {
        ...
        case KellyErrorEstimator<dim, spacedim>::face_diameter_over_twice_max_degree:
          {
  ...
  template <int dim, int spacedim>
  double
  regular_face_factor(const typename DoFHandler<dim, spacedim>::active_cell_iterator &cell,
                      const unsigned int                                              face_no,
                      const dealii::hp::FEFaceValues<dim, spacedim>                  &fe_face_values_cell,
        ...
        case KellyErrorEstimator<dim, spacedim>::face_diameter_over_twice_max_degree:
          {
            ...
            const double neighbor_degree =
              fe_face_values_neighbor.get_fe_collection()[cell->neighbor(face_no)->active_fe_index()].degree;
  ...
  template <int dim, int spacedim>
  double
  irregular_face_factor(const typename DoFHandler<dim, spacedim>::active_cell_iterator &cell,
                        const typename DoFHandler<dim, spacedim>::active_cell_iterator &neighbor_child,
                        const unsigned int                                              face_no,
        ...
        case KellyErrorEstimator<dim, spacedim>::face_diameter_over_twice_max_degree:
          {
            ...
            return cell->face(face_no)->child(subface_no)->diameter() / std::max(neighbor_child_degree, cell_degree) /
                   2.0;
          }
        default:
          {
  ...
  template <int dim, int spacedim>
  double
  cell_factor(const typename DoFHandler<dim, spacedim>::active_cell_iterator &cell,
              const unsigned int /*face_no*/,
  ...
  integrate_over_regular_face(
    ...
    std::map<typename DoFHandler<dim, spacedim>::face_iterator, std::vector<double>> &local_face_integrals,
    ...
    const typename KellyErrorEstimator<dim, spacedim>::Strategy                       strategy)
  {
    ...
    for (unsigned int n = 0; n < n_solution_vectors; ++n)
      fe_face_values_cell.get_present_fe_values().get_function_gradients(*solutions[n], parallel_data.psi[n]);
    ...
    if (face->at_boundary() == false)
      // internal face; integrate jump of gradient across this face
      {
        ...
        const typename DoFHandler<dim, spacedim>::active_cell_iterator neighbor = cell->neighbor(face_no);
        ...
        factor =
          regular_face_factor<dim, spacedim>(cell, face_no, fe_face_values_cell, fe_face_values_neighbor, strategy);

        // get gradients on neighbor cell
        for (unsigned int n = 0; n < n_solution_vectors; ++n)
          {
            fe_face_values_neighbor.get_present_fe_values().get_function_gradients(*solutions[n],
                                                                                   parallel_data.neighbor_psi[n]);
          }

        parallel_data.neighbor_normal_vectors = fe_face_values_neighbor.get_present_fe_values().get_normal_vectors();
      }
    else
      {
        factor = boundary_face_factor<dim, spacedim>(cell, face_no, fe_face_values_cell, strategy);
      }
    ...
    const typename KellyErrorEstimator<dim, spacedim>::Strategy                       strategy)
  {
    ...
    for (unsigned int subface_no = 0; subface_no < face->n_children(); ++subface_no)
      {
        ...
        const double factor = irregular_face_factor<dim, spacedim>(
        ...
        for (unsigned int n = 0; n < n_solution_vectors; ++n)
          fe_subface_values.get_present_fe_values().get_function_gradients(*solutions[n], parallel_data.psi[n]);

        // store the gradient from the neighbor's side in @p{neighbor_psi}
        for (unsigned int n = 0; n < n_solution_vectors; ++n)
          fe_face_values.get_present_fe_values().get_function_gradients(*solutions[n], parallel_data.neighbor_psi[n]);

        // call generic evaluate function
        parallel_data.neighbor_normal_vectors = fe_subface_values.get_present_fe_values().get_normal_vectors();
    ...
    for (unsigned int subface_no = 0; subface_no < face->n_children(); ++subface_no)
      {
        Assert(local_face_integrals.find(face->child(subface_no)) != local_face_integrals.end(), ExcInternalError());
    ...
    for (const unsigned int face_no : cell->face_indices())
      {
        ...
        if ((face->has_children() == false) && !cell->at_boundary(face_no) &&
            (!cell->neighbor_is_coarser(face_no) &&
             (cell->neighbor(face_no)->index() < cell->index() ||
              (cell->neighbor(face_no)->index() == cell->index() && cell->neighbor(face_no)->level() < cell->level()))))
          continue;
        ...
        if (face->at_boundary() &&
            (parallel_data.neumann_bc->find(face->boundary_id()) == parallel_data.neumann_bc->end()))
          {
            ...
            if (face->has_children() == false)
            ...
            else
              {
                for (unsigned int sf = 0; sf < face->n_children(); ++sf)
                  if (((cell->neighbor_child_on_subface(face_no, sf)->subdomain_id() == subdomain_id) &&
                       (material_id == numbers::invalid_material_id)) ||
                      ((cell->neighbor_child_on_subface(face_no, sf)->material_id() == material_id) &&
                       (subdomain_id == numbers::invalid_subdomain_id)))
                    {
                      care_for_cell = true;
                      break;
  ...
  const Strategy            strategy)
{
  ...
  if (const auto *triangulation =
        dynamic_cast<const parallel::DistributedTriangulationBase<dim, spacedim> *>(&dof_handler.get_triangulation()))
    {
  ...
  Assert((coefficients == nullptr) || (coefficients->n_components == n_components) || (coefficients->n_components == 1),
  ...
  const internal::ParallelData<dim, spacedim, Number> parallel_data(dof_handler.get_fe_collection(),
                                                                    face_quadratures,
                                                                    mapping_collection,
                                                                    ...
                                                                    component_mask,
                                                                    coefficients);
  std::map<typename DoFHandler<dim, spacedim>::face_iterator, std::vector<double>> sample_local_face_integrals;

  // now let's work on all those cells:
  WorkStream::run(
    dof_handler.begin_active(),
    static_cast<typename DoFHandler<dim, spacedim>::active_cell_iterator>(dof_handler.end()),
    [&solutions,
     strategy](const typename DoFHandler<dim, spacedim>::active_cell_iterator                   &cell,
               internal::ParallelData<dim, spacedim, Number>                                    &parallel_data,
               std::map<typename DoFHandler<dim, spacedim>::face_iterator, std::vector<double>> &local_face_integrals) {
      internal::estimate_one_cell(cell, parallel_data, local_face_integrals, solutions, strategy);
    },
    [&face_integrals](
      const std::map<typename DoFHandler<dim, spacedim>::face_iterator, std::vector<double>> &local_face_integrals) {
        ...
        for (const unsigned int face_no : cell->face_indices())
          {
            ...
            const double factor = internal::cell_factor<dim, spacedim>(cell, face_no, dof_handler, strategy);

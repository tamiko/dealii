...
namespace Functions
{
  ...
  FEFieldFunction<dim, VectorType, spacedim>::FEFieldFunction(const DoFHandler<dim, spacedim> &mydh,
  ...
  template <int dim, typename VectorType, int spacedim>
  void
  FEFieldFunction<dim, VectorType, spacedim>::vector_value(const Point<dim>                        &p,
                                                           Vector<typename VectorType::value_type> &values) const
  {
    ...
    typename DoFHandler<dim, spacedim>::active_cell_iterator cell = cell_hint.get();
    if (cell == dh->end())
    ...
    if (!qp)
      {
        const std::pair<typename dealii::internal::ActiveCellIterator<dim, dim, DoFHandler<dim, spacedim>>::type,
                        Point<dim>>
          my_pair = GridTools::find_active_cell_around_point(mapping, *dh, p);
        AssertThrow(my_pair.first.state() == IteratorState::valid && !my_pair.first->is_artificial(),
  ...
  FEFieldFunction<dim, VectorType, spacedim>::value(const Point<dim> &p, const unsigned int comp) const
  {
  ...
  FEFieldFunction<dim, VectorType, spacedim>::gradient(const Point<dim> &p, const unsigned int comp) const
  {
  ...
  template <int dim, typename VectorType, int spacedim>
  void
  FEFieldFunction<dim, VectorType, spacedim>::vector_laplacian(const Point<dim>                        &p,
                                                               Vector<typename VectorType::value_type> &values) const
  {
    Assert(values.size() == this->n_components, ExcDimensionMismatch(values.size(), this->n_components));
    typename DoFHandler<dim, spacedim>::active_cell_iterator cell = cell_hint.get();
  ...
  template <int dim, typename VectorType, int spacedim>
  typename VectorType::value_type
  FEFieldFunction<dim, VectorType, spacedim>::laplacian(const Point<dim> &p, const unsigned int comp) const
  {
    ...
    std::vector<Vector<typename VectorType::value_type>> &values) const
  {
    ...
    hp::FEValues<dim>          fe_v(mapping_collection, dh->get_fe_collection(), quadrature_collection, update_values);
    for (unsigned int i = 0; i < n_cells; ++i)
      {
  ...
  template <int dim, typename VectorType, int spacedim>
  void
  FEFieldFunction<dim, VectorType, spacedim>::value_list(const std::vector<Point<dim>>                &points,
    ...
    std::vector<std::vector<Tensor<1, dim, typename VectorType::value_type>>> &values) const
  {
    ...
    // Now gather all the information we need
    hp::MappingCollection<dim> mapping_collection(mapping);
    hp::FEValues<dim> fe_v(mapping_collection, dh->get_fe_collection(), quadrature_collection, update_gradients);
    for (unsigned int i = 0; i < n_cells; ++i)
      {
        AssertThrow(!cells[i]->is_artificial(), VectorTools::ExcPointNotAvailableHere());
    ...
    const unsigned int                                            component) const
  {
    ...
    std::vector<std::vector<Tensor<1, dim, typename VectorType::value_type>>> vvalues(
      points.size(), std::vector<Tensor<1, dim, typename VectorType::value_type>>(this->n_components));
    ...
    std::vector<Vector<typename VectorType::value_type>> &values) const
  {
    ...
    // Now gather all the information we need
    hp::MappingCollection<dim> mapping_collection(mapping);
    hp::FEValues<dim> fe_v(mapping_collection, dh->get_fe_collection(), quadrature_collection, update_hessians);
    // Now gather all the information we need
    for (unsigned int i = 0; i < n_cells; ++i)
      {
        AssertThrow(!cells[i]->is_artificial(), VectorTools::ExcPointNotAvailableHere());
  ...
  template <int dim, typename VectorType, int spacedim>
  void
  FEFieldFunction<dim, VectorType, spacedim>::laplacian_list(const std::vector<Point<dim>>                &points,
                                                             std::vector<typename VectorType::value_type> &values,
                                                             const unsigned int component) const
  {
    Assert(points.size() == values.size(), ExcDimensionMismatch(points.size(), values.size()));
    ...
    std::vector<std::vector<unsigned int>>                                &maps) const
  {
    ...
    auto        cell_qpoint_map = GridTools::compute_point_locations_try_all(cache, points, cell_hint.get());
    ...
    AssertThrow(std::get<3>(cell_qpoint_map).empty(), VectorTools::ExcPointNotAvailableHere());
    return cells.size();
  }

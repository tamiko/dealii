                       ...
                       const Direction         direction)
{
  ...
  // wake up the first few time levels
  for (int step = -static_cast<int>(timestepping_data.look_ahead); step < 0; ++step)
    for (int look_ahead = 0; look_ahead <= static_cast<int>(timestepping_data.look_ahead); ++look_ahead)
      switch (direction)
        {
          case forward:
  ...
  // make the last few timesteps sleep
  for (int step = n_timesteps; step < static_cast<int>(n_timesteps + timestepping_data.look_back); ++step)

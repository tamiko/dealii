...
namespace VectorTools
{
  ...
  DEAL_II_CXX20_REQUIRES((concepts::is_dealii_vector_type<VectorType> &&
                          concepts::is_triangulation_or_dof_handler<MeshType<dim, spacedim>>))
  std::vector<typename FEPointEvaluation<n_components, dim, spacedim, typename VectorType::value_type>::
  ...
  DEAL_II_CXX20_REQUIRES((concepts::is_dealii_vector_type<VectorType> &&
                          concepts::is_triangulation_or_dof_handler<MeshType<dim, spacedim>>))
  std::vector<typename FEPointEvaluation<n_components, dim, spacedim, typename VectorType::value_type>::
                value_type> point_values(const Utilities::MPI::RemotePointEvaluation<dim, spacedim> &cache,
                                         const MeshType<dim, spacedim>                              &mesh,
                                         const VectorType                                           &vector,
                                         const EvaluationFlags::EvaluationFlags flags = EvaluationFlags::avg,
                                         const unsigned int                     first_selected_component = 0);
  ...
  DEAL_II_CXX20_REQUIRES((concepts::is_dealii_vector_type<VectorType> &&
                          concepts::is_triangulation_or_dof_handler<MeshType<dim, spacedim>>))
  std::vector<typename FEPointEvaluation<n_components, dim, spacedim, typename VectorType::value_type>::
                gradient_type> point_gradients(const Utilities::MPI::RemotePointEvaluation<dim, spacedim> &cache,
                                               const MeshType<dim, spacedim>                              &mesh,
                                               const VectorType                                           &vector,
                                               const EvaluationFlags::EvaluationFlags flags = EvaluationFlags::avg,
                                               const unsigned int                     first_selected_component = 0);
  ...
  DEAL_II_CXX20_REQUIRES((concepts::is_dealii_vector_type<VectorType> &&
                          concepts::is_triangulation_or_dof_handler<MeshType<dim, spacedim>>))
  inline std::vector<typename FEPointEvaluation<n_components, dim, spacedim, typename VectorType::value_type>::
                       value_type> point_values(const Mapping<dim>                                   &mapping,
                                                const MeshType<dim, spacedim>                        &mesh,
                                                const VectorType                                     &vector,
                                                const std::vector<Point<spacedim>>                   &evaluation_points,
                                                ...
                                                const unsigned int first_selected_component)
  {
    ...
    return point_values<n_components>(cache, mesh, vector, flags, first_selected_component);
  }
                                                      ...
                                                      const unsigned int first_selected_component)
  {
    ...
    return point_gradients<n_components>(cache, mesh, vector, flags, first_selected_component);
  }
  ...
  namespace internal
  {
    ...
    template <typename T>
    T
    reduce(const EvaluationFlags::EvaluationFlags &flags, const ArrayView<const T> &values)
    {
      switch (flags)
        {
          case EvaluationFlags::avg:
            {
              return std::accumulate(values.begin(), values.end(), T{}) / (T(1.0) * values.size());
            }
    ...
    reduce(const EvaluationFlags::EvaluationFlags &flags, const ArrayView<const Tensor<rank, dim, Number>> &values)
    {
      switch (flags)
        {
          case EvaluationFlags::avg:
            {
              return std::accumulate(values.begin(), values.end(), Tensor<rank, dim, Number>{}) /
    ...
    reduce(const EvaluationFlags::EvaluationFlags                                    &flags,
           const ArrayView<const Tensor<1, n_components, Tensor<rank, dim, Number>>> &values)
    {
      switch (flags)
        {
          case EvaluationFlags::avg:
    ...
    template <int n_components, int dim, int spacedim, typename VectorType, typename value_type>
    void
    process_cell(
      const unsigned int                                                             i,
      const typename Utilities::MPI::RemotePointEvaluation<dim, spacedim>::CellData &cell_data,
      ...
      const unsigned int                                                             first_selected_component,
      const std::function<
        value_type(const FEPointEvaluation<n_components, dim, spacedim, typename VectorType::value_type> &,
      ...
      std::vector<std::unique_ptr<FEPointEvaluation<n_components, dim, spacedim, typename VectorType::value_type>>>
        &evaluators)
    {
      if (evaluators.empty())
      ...
      if (evaluators[cell->active_fe_index()] == nullptr)
        evaluators[cell->active_fe_index()] =
          std::make_unique<FEPointEvaluation<n_components, dim, spacedim, typename VectorType::value_type>>(
            cache.get_mapping(), cell->get_fe(), update_flags, first_selected_component);
      ...
      for (unsigned int q = 0; q < unit_points.size(); ++q)
        values[q + cell_data.reference_point_ptrs[i]] = process_quadrature_point(evaluator, q);
    }
    ...
    Number
    get_value(const Triangulation<dim, spacedim>                                &tria,
    ...
    get_value(const Triangulation<dim, spacedim>                                &tria,
              ...
              const typename Triangulation<dim, spacedim>::active_cell_iterator &cell)
    {
      const auto distributed_tria = dynamic_cast<const parallel::TriangulationBase<dim, spacedim> *>(&tria);

      const bool use_distributed_path =
        (distributed_tria == nullptr) ?
          false :
          (vector.get_partitioner().get() == distributed_tria->global_active_cell_index_partitioner().lock().get());
    ...
    {
      Assert(false, ExcMessage("A cell-data vector can only have a single component."));
    }
      ...
      std::vector<std::unique_ptr<FEPointEvaluation<n_components, dim, spacedim, typename VectorType::value_type>>> &)
    {
      ...
      Assert(n_components == 1 && first_selected_component == 0,
             ExcMessage("A cell-data vector can only have a single component."));

      Assert(evaluation_flags == dealii::EvaluationFlags::EvaluationFlags::values,
      ...
      for (unsigned int q = cell_data.reference_point_ptrs[i]; q < cell_data.reference_point_ptrs[i + 1]; ++q)
        set_value(values[q], value);
    }



    template <int n_components, int dim, int spacedim, typename MeshType, typename VectorType, typename value_type>
    DEAL_II_CXX20_REQUIRES(
      concepts::is_dealii_vector_type<VectorType> &&concepts::is_triangulation_or_dof_handler<MeshType>)
                   ...
                   const unsigned int &)> process_quadrature_point)
    {
      ...
      Assert(&mesh.get_triangulation() == &cache.get_triangulation(),
      ...
      const auto evaluation_point_results = [&]() {
        ...
        const auto evaluation_function = [&](auto &values, const auto &cell_data) {
          ...
          for (unsigned int i = 0; i < cell_data.cells.size(); ++i)
            process_cell<n_components, dim, spacedim, VectorType, value_type>(i,
                                                                              cell_data,
                                                                              cache,
                                                                              mesh,
                                                                              vector,
        ...
        cache.template evaluate_and_process<value_type>(evaluation_point_results, buffer, evaluation_function);
          ...
          for (unsigned int i = 0; i < ptr.size() - 1; ++i)
            {
              ...
              unique_evaluation_point_results[i] =
                reduce(flags, ArrayView<const value_type>(evaluation_point_results.data() + ptr[i], n_entries));
            }
                                                ...
                                                const unsigned int first_selected_component)
  {
    return internal::evaluate_at_points<
      ...
      VectorType,
      typename FEPointEvaluation<n_components, dim, spacedim, typename VectorType::value_type>::value_type>(
      cache,
      mesh,
      vector,
      flags,
      ...
      [](const auto &evaluator, const auto &q) { return evaluator.get_value(q); });
  }
                                                      ...
                                                      const unsigned int                     first_selected_component)
  {
    return internal::evaluate_at_points<
      ...
      MeshType<dim, spacedim>,
      VectorType,
      typename FEPointEvaluation<n_components, dim, spacedim, typename VectorType::value_type>::gradient_type>(
      cache,
      mesh,
      vector,
      flags,
      ...
      [](const auto &evaluator, const unsigned &q) { return evaluator.get_gradient(q); });
  }

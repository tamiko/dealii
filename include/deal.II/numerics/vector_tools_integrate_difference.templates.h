...
namespace VectorTools
{
  namespace internal
  {
    ...
    struct IDScratchData
    {
      ...
      std::vector<std::vector<Tensor<1, spacedim, Number>>> tmp_vector_gradients;
    ...
    template <int dim, int spacedim, typename Number>
    IDScratchData<dim, spacedim, Number>::IDScratchData(const dealii::hp::MappingCollection<dim, spacedim> &mapping,
                                                        const dealii::hp::FECollection<dim, spacedim>      &fe,
                                                        const dealii::hp::QCollection<dim>                 &q,
                                                        const UpdateFlags update_flags)
    ...
    template <int dim, int spacedim, typename Number>
    void
    IDScratchData<dim, spacedim, Number>::resize_vectors(const unsigned int n_q_points, const unsigned int n_components)
    {
      ...
      tmp_vector_gradients.resize(n_q_points, std::vector<Tensor<1, spacedim, Number>>(n_components));
    }
      ...
      if (update_flags & update_values)
        {
          ...
          if (fe_is_system)
            {
              exact_solution.vector_value_list(fe_values.get_quadrature_points(), data.tmp_vector_values);
      ...
      if (update_flags & update_gradients)
        {
          ...
          if (fe_is_system)
            {
              exact_solution.vector_gradient_list(fe_values.get_quadrature_points(), data.tmp_vector_gradients);
              for (const auto i : fe_values.quadrature_point_indices())
      ...
      switch (norm)
        {
            ...
            for (const auto q : fe_values.quadrature_point_indices())
              {
                ...
                for (unsigned int k = 0; k < n_components; ++k)
                  if (data.weight_vectors[q](k) != 0)
                    sum +=
                      std::pow(static_cast<double>(numbers::NumberTraits<Number>::abs_square(data.psi_values[q](k))),
                               exponent / 2.) *
            ...
            for (const auto q : fe_values.quadrature_point_indices())
              {
                ...
                for (unsigned int k = 0; k < n_components; ++k)
                  if (data.weight_vectors[q](k) != 0)
                    sum += numbers::NumberTraits<Number>::abs_square(data.psi_values[q](k)) * data.weight_vectors[q](k);
                diff += sum * fe_values.JxW(q);
              }
            // Compute the root only, if no derivative values are added later
            if (norm == L2_norm)
          ...
          case W1infty_norm:
            for (const auto q : fe_values.quadrature_point_indices())
              for (unsigned int k = 0; k < n_components; ++k)
                if (data.weight_vectors[q](k) != 0)
                  diff = std::max(diff, double(std::abs(data.psi_values[q](k) * data.weight_vectors[q](k))));
            break;
            ...
            for (const auto q : fe_values.quadrature_point_indices())
              {
                ...
                for (unsigned int k = 0; k < n_components; ++k)
                  if (data.weight_vectors[q](k) != 0)
                    sum += std::pow(data.psi_grads[q][k].norm_square(), exponent / 2.) * data.weight_vectors[q](k);
                diff += sum * fe_values.JxW(q);
              }
            diff = std::pow(diff, 1. / exponent);
            break;
                ...
                for (unsigned int k = 0; k < n_components; ++k)
                  if (data.weight_vectors[q](k) != 0)
                    for (unsigned int d = 0; d < dim; ++d)
                      t = std::max(t, double(std::abs(data.psi_grads[q][k][d]) * data.weight_vectors[q](k)));
    ...
    static void do_integrate_difference(const dealii::hp::MappingCollection<dim, spacedim> &mapping,
        ...
        if (cell->is_locally_owned())
          {
            ...
            difference(cell->active_cell_index()) = integrate_difference_inner<dim, spacedim, Number>(
  ...
  template <int dim, typename Number, class OutVector, int spacedim>
  DEAL_II_CXX20_REQUIRES(concepts::is_writable_dealii_vector_type<OutVector>)
  void integrate_difference(const dealii::hp::MappingCollection<dim, spacedim> &mapping,
                            const DoFHandler<dim, spacedim>                    &dof,
                            const ReadVector<Number>                           &fe_function,
                            const Function<spacedim, Number>                   &exact_solution,
                            OutVector                                          &difference,
                            ...
                            const double                                        exponent)
  {
    internal::do_integrate_difference(mapping, dof, fe_function, exact_solution, difference, q, norm, weight, exponent);
  }
  ...
  double compute_global_error(const Triangulation<dim, spacedim> &tria,
                              ...
                              const double                        exponent)
  {
    Assert(cellwise_error.size() == tria.n_active_cells(), ExcMessage("input vector cell_error has invalid size!"));
#ifdef DEBUG
    {

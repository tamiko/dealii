...
namespace
{
  ...
  double
  calculate_next_time(const double current_time, const double step_size, const double end_time)
  {
...
DiscreteTime::DiscreteTime(const double start_time, const double end_time, const double desired_start_step_size)
  : start_time{start_time}
  , end_time{end_time}
  , current_time{start_time}
  , next_time{calculate_next_time(start_time, desired_start_step_size, end_time)}
...
{
  Assert(next_step_size > 0, ExcMessage("Only positive time step size is allowed."));
  next_time = current_time + next_step_size;
  Assert(next_time <= end_time, ExcMessage("Time step size is too large. The next time cannot exceed the end time."));
}

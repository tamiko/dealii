...
namespace Functions
{
  ...
  CSpline<dim>::CSpline(const std::vector<double> &x_, const std::vector<double> &y_)
    ...
    , acc(gsl_interp_accel_alloc(), [](gsl_interp_accel *p) { gsl_interp_accel_free(p); })
              ...
              [](gsl_spline *p) { gsl_spline_free(p); })
  {
    ...
    gsl_spline_init(cspline.get(), interpolation_points.data(), interpolation_values.data(), n);
  }

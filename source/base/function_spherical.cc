...
namespace Functions
{
...

  {
    ...
    const double c_utheta2 =
      sg[0] / r + ((sin_phi != 0.) ? (cos_phi * sg[2]) / (r2 * sin_phi) + sh[1] / (r2 * sin_phi2) : 0.);
    const double c_utheta_ur   = ((sin_phi != 0.) ? (r * sh[3] - sg[1]) / (r2 * sin_phi) : 0.);
    const double c_utheta_uphi = ((sin_phi != 0.) ? (sh[5] * sin_phi - cos_phi * sg[1]) / (r2 * sin_phi2) : 0.);
  ...
  template <int dim>
  double
  Spherical<dim>::svalue(const std::array<double, dim> & /* sp */, const unsigned int /*component*/) const
  {
  ...
  Spherical<dim>::sgradient(const std::array<double, dim> & /* sp */, const unsigned int /*component*/) const
  {
    AssertThrow(false, ExcNotImplemented());
  ...
  template <int dim>
  std::array<double, 6>
  Spherical<dim>::shessian(const std::array<double, dim> & /* sp */, const unsigned int /*component*/) const
  {
    AssertThrow(false, ExcNotImplemented());

...
namespace Utilities
{
  namespace MPI
  {
    ...
    Partitioner::Partitioner(const types::global_dof_index local_size,
      ...
      : global_size(Utilities::MPI::sum<types::global_dof_index>(local_size, communicator))
      ...
      , have_ghost_indices(true)
    {
      ...
      const int ierr = MPI_Exscan(
        &local_size, &prefix_sum, 1, Utilities::MPI::mpi_type_id_for_type<decltype(prefix_sum)>, MPI_SUM, communicator);
      AssertThrowMPI(ierr);
#endif
    ...
    Partitioner::Partitioner(const IndexSet &locally_owned_indices, const MPI_Comm communicator_in)
      : global_size(static_cast<types::global_dof_index>(locally_owned_indices.size()))
      , n_ghost_indices_data(0)
      , n_import_indices_data(0)
      , n_ghost_indices_in_larger_set(0)
    ...
    Partitioner::set_owned_indices(const IndexSet &locally_owned_indices)
    {
      ...
      if (locally_owned_indices.n_elements() > 0)
        local_range_data =
          std::pair<types::global_dof_index, types::global_dof_index>(locally_owned_indices.nth_index_in_set(0),
    ...
    void
    Partitioner::set_ghost_indices(const IndexSet &ghost_indices_in, const IndexSet &larger_ghost_index_set)
    {
      ...
      // in any case.
      Assert(ghost_indices_in.n_elements() == 0 || ghost_indices_in.size() == locally_owned_range_data.size(),
      ...
      have_ghost_indices = Utilities::MPI::max(n_ghost_indices_data, communicator) > 0;
      ...
      {
        const int ierr = MPI_Exscan(&my_size, &my_shift, 1, DEAL_II_DOF_INDEX_MPI_TYPE, MPI_SUM, communicator);
        AssertThrowMPI(ierr);
      }
      ...
      if (local_range_data.first == 0 && my_shift != 0)
        {
          const types::global_dof_index old_locally_owned_size = locally_owned_size();
          local_range_data.first                               = my_shift;
          local_range_data.second                              = my_shift + old_locally_owned_size;
        }

      std::vector<unsigned int> owning_ranks_of_ghosts(ghost_indices_data.n_elements());

      // set up dictionary
      internal::ComputeIndexOwner::ConsensusAlgorithmsPayload process(locally_owned_range_data,
                                                                      ghost_indices_data,
                                                                      communicator,
            ...
            for (auto i : owning_ranks_of_ghosts)
              {
                Assert(i >= ghost_targets_data.back().first,
                       ExcInternalError("Expect result of ConsensusAlgorithms::Process to be "
                                        "sorted."));
        ...
        if (i.second.n_elements() > 0)
          {
            ...
            import_indices_chunks_by_rank_data.push_back(import_indices_chunks_by_rank_data.back() +
      ...
      for (const auto &i : import_data)
        {
          ...
          for (auto interval = i.second.begin_intervals(); interval != i.second.end_intervals(); ++interval)
            import_indices_data.emplace_back(*interval->begin() - local_range_data.first,
                                             interval->last() + 1 - local_range_data.first);
        }
...
#  ifdef DEBUG
      ...
      // same
      AssertDimension(Utilities::MPI::sum(import_targets_data.size(), communicator),
      ...
      // (ghost_indices_ref).
      const std::vector<types::global_dof_index> ghost_indices_ref = ghost_indices_data.get_index_vector();
      ...
      const std::vector<types::global_dof_index> my_indices = locally_owned_range_data.get_index_vector();
      std::vector<MPI_Request>                   requests;
      n_ghost_indices_in_larger_set = n_ghost_indices_data;
      export_to_ghosted_array_start(127,
                                    ArrayView<const types::global_dof_index>(my_indices.data(), my_indices.size()),
      ...
      int       flag = 0;
      const int ierr = MPI_Testall(requests.size(), requests.data(), &flag, MPI_STATUSES_IGNORE);
      AssertThrowMPI(ierr);
      Assert(flag == 1, ExcMessage("MPI found unfinished requests. Check communication setup"));
...
#  endif
          ...
          for (const dealii::IndexSet::size_type index : ghost_indices_data)
            {
              ...
              Assert(larger_ghost_index_set.index_within_set(index) <
                       static_cast<types::global_dof_index>(std::numeric_limits<unsigned int>::max()),
                     ExcMessage("Index overflow: This class supports at most 2^32-1 ghost elements"));
              expanded_numbering.push_back(larger_ghost_index_set.index_within_set(index));
            }
              ...
              for (unsigned int ii = 0; ii < ghost_targets_data[p].second; ++ii)
                {
                  ...
                  if (expanded_numbering[i] == last_index + 1)
                    ...
                    ghost_indices_subset.emplace_back(expanded_numbering[i], expanded_numbering[i] + 1);
    ...
    Partitioner::memory_consumption() const
    {
      ...
      memory += sizeof(import_indices_plain_dev) +
                sizeof(*import_indices_plain_dev.begin()) * import_indices_plain_dev.capacity();
      ...
      memory += MemoryConsumption::memory_consumption(n_ghost_indices_in_larger_set);
      ...
      for (unsigned int i = 0; i < n_import_targets; ++i)
        {
          ...
          import_indices_plain_dev.emplace_back("import_indices_plain_dev" + std::to_string(i), chunk_size);
          Kokkos::deep_copy(import_indices_plain_dev.back(),
                            Kokkos::View<unsigned int *, Kokkos::HostSpace>(import_indices_plain_host.data(),
                                                                            chunk_size));
        }
    }

...
PathSearch::find(const std::string &filename, const char *open_mode)
{
  ...
  if (debug > 2)
    deallog << "PathSearch[" << cls << "] " << my_path_list.size() << " directories " << my_suffix_list.size()
            << " suffixes" << std::endl;
...
PathSearch::add_path(const std::string &path, Position pos)
{
  if (pos == back)
    ...
    {
      std::vector<std::string>::iterator i = std::find(my_path_list.begin(), my_path_list.end(), empty);
...
PathSearch::add_suffix(const std::string &suffix, Position pos)
{
  if (pos == back)
    ...
    {
      std::vector<std::string>::iterator i = std::find(my_suffix_list.begin(), my_suffix_list.end(), empty);

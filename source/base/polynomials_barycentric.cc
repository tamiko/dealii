...
namespace internal
{
  ...
  template <int dim>
  unsigned int
  get_degree(const std::vector<typename BarycentricPolynomials<dim>::PolyType> &polys)
  {
...
BarycentricPolynomials<dim>::get_fe_p_basis(const unsigned int degree)
{
  ...
  switch (degree)
    {
      case 0:
        polys.push_back(0 * BarycentricPolynomial<dim, double>::monomial(0) + 1);
        break;
      case 1:
        {
          ...
          for (const unsigned int l : reference_cell.line_indices())
            {
              ...
              polys.push_back(4 * BarycentricPolynomial<dim, double>::monomial(v0) *
...
template <int dim>
BarycentricPolynomials<dim>::BarycentricPolynomials(const std::vector<PolyType> &polynomials)
  : ScalarPolynomialsBase<dim>(internal::get_degree<dim>(polynomials), polynomials.size())
{
  polys = polynomials;
      ...
      for (unsigned int d0 = 0; d0 < dim; ++d0)
        for (unsigned int d1 = 0; d1 < dim; ++d1)
          for (unsigned int d2 = 0; d2 < dim; ++d2)
            poly_third_derivatives[i][d0][d1][d2] = poly_hessians[i][d0][d1].derivative(d2);
        ...
        for (unsigned int d1 = 0; d1 < dim; ++d1)
          for (unsigned int d2 = 0; d2 < dim; ++d2)
            for (unsigned int d3 = 0; d3 < dim; ++d3)
              poly_fourth_derivatives[i][d0][d1][d2][d3] = poly_third_derivatives[i][d0][d1][d2].derivative(d3);
    }
}
...
template <int dim>
void
BarycentricPolynomials<dim>::evaluate(const Point<dim>            &unit_point,
  ...
  for (std::size_t i = 0; i < polys.size(); ++i)
    {
      ...
      if (third_derivatives.size() == this->n())
        for (unsigned int d0 = 0; d0 < dim; ++d0)
          for (unsigned int d1 = 0; d1 < dim; ++d1)
            for (unsigned int d2 = 0; d2 < dim; ++d2)
              third_derivatives[i][d0][d1][d2] = poly_third_derivatives[i][d0][d1][d2].value(unit_point);
      ...
      if (fourth_derivatives.size() == this->n())
        for (unsigned int d0 = 0; d0 < dim; ++d0)
          for (unsigned int d1 = 0; d1 < dim; ++d1)
            for (unsigned int d2 = 0; d2 < dim; ++d2)
              for (unsigned int d3 = 0; d3 < dim; ++d3)
                fourth_derivatives[i][d0][d1][d2][d3] = poly_fourth_derivatives[i][d0][d1][d2][d3].value(unit_point);
    }
}
...
template <int dim>
double
BarycentricPolynomials<dim>::compute_value(const unsigned int i, const Point<dim> &p) const
{
...
template <int dim>
Tensor<1, dim>
BarycentricPolynomials<dim>::compute_1st_derivative(const unsigned int i, const Point<dim> &p) const
{
  Tensor<1, dim> result;
  for (unsigned int d = 0; d < dim; ++d)
...
template <int dim>
Tensor<2, dim>
BarycentricPolynomials<dim>::compute_2nd_derivative(const unsigned int i, const Point<dim> &p) const
{
  Tensor<2, dim> result;
  for (unsigned int d0 = 0; d0 < dim; ++d0)
    for (unsigned int d1 = 0; d1 < dim; ++d1)
...
template <int dim>
Tensor<3, dim>
BarycentricPolynomials<dim>::compute_3rd_derivative(const unsigned int i, const Point<dim> &p) const
{
  Tensor<3, dim> result;
  for (unsigned int d0 = 0; d0 < dim; ++d0)
    for (unsigned int d1 = 0; d1 < dim; ++d1)
...
template <int dim>
Tensor<4, dim>
BarycentricPolynomials<dim>::compute_4th_derivative(const unsigned int i, const Point<dim> &p) const
{
  Tensor<4, dim> result;
  for (unsigned int d0 = 0; d0 < dim; ++d0)
    for (unsigned int d1 = 0; d1 < dim; ++d1)
      for (unsigned int d2 = 0; d2 < dim; ++d2)
        for (unsigned int d3 = 0; d3 < dim; ++d3)
          result[d0][d1][d2][d3] = poly_fourth_derivatives[i][d0][d1][d2][d3].value(p);
...
template <int dim>
Tensor<1, dim>
BarycentricPolynomials<dim>::compute_grad(const unsigned int i, const Point<dim> &p) const
{
...
template <int dim>
Tensor<2, dim>
BarycentricPolynomials<dim>::compute_grad_grad(const unsigned int i, const Point<dim> &p) const
{
...
BarycentricPolynomials<dim>::memory_consumption() const
{
  ...
  return ScalarPolynomialsBase<dim>::memory_consumption() + poly_memory +
         MemoryConsumption::memory_consumption(poly_grads) + MemoryConsumption::memory_consumption(poly_hessians) +

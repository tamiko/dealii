...
namespace
{
  ...
  create_rt_polynomials(const unsigned int dim, const unsigned int normal_degree, const unsigned int tangential_degree)
  {
    std::vector<std::vector<Polynomials::Polynomial<double>>> pols(dim);
    if (normal_degree > 0)
      pols[0] = Polynomials::generate_complete_Lagrange_basis(QGaussLobatto<1>(normal_degree + 1).get_points());
    else
      pols[0] = Polynomials::generate_complete_Lagrange_basis(QMidpoint<1>().get_points());
    if (tangential_degree > 0)
      for (unsigned int d = 1; d < dim; ++d)
        pols[d] = Polynomials::generate_complete_Lagrange_basis(QGaussLobatto<1>(tangential_degree + 1).get_points());
    else
      for (unsigned int d = 1; d < dim; ++d)
        pols[d] = Polynomials::generate_complete_Lagrange_basis(QMidpoint<1>().get_points());
...
template <int dim>
PolynomialsRaviartThomas<dim>::PolynomialsRaviartThomas(const unsigned int normal_degree,
  ...
  , polynomial_space(create_rt_polynomials(dim, normal_degree, tangential_degree))
{
  ...
  lexicographic_to_hierarchic = get_lexicographic_numbering(normal_degree, tangential_degree);

  hierarchic_to_lexicographic = Utilities::invert_permutation(lexicographic_to_hierarchic);
  ...
  if (dim == 2)
    {
      ...
      for (unsigned int j = 0; j < normal_degree + 1; ++j)
        for (unsigned int i = 0; i < tangential_degree + 1; ++i)
          renumber_aniso[1][j * (tangential_degree + 1) + i] = j + i * (normal_degree + 1);
    }
  if (dim == 3)
    {
      ...
      for (unsigned int k = 0; k < tangential_degree + 1; ++k)
        for (unsigned int j = 0; j < normal_degree + 1; ++j)
          for (unsigned int i = 0; i < tangential_degree + 1; ++i)
            renumber_aniso[1][(k * (normal_degree + 1) + j) * (tangential_degree + 1) + i] =
      ...
      for (unsigned int k = 0; k < normal_degree + 1; ++k)
        for (unsigned int j = 0; j < tangential_degree + 1; ++j)
          for (unsigned int i = 0; i < tangential_degree + 1; ++i)
            renumber_aniso[2][(k * (tangential_degree + 1) + j) * (tangential_degree + 1) + i] =
...
template <int dim>
void
PolynomialsRaviartThomas<dim>::evaluate(const Point<dim>            &unit_point,
  ...
  for (unsigned int d = 0; d < dim; ++d)
    {
      ...
      polynomial_space.evaluate(p, p_values, p_grads, p_grad_grads, p_third_derivatives, p_fourth_derivatives);

      for (unsigned int i = 0; i < p_values.size(); ++i)
        values[lexicographic_to_hierarchic[i + d * n_sub]][d] = p_values[renumber_aniso[d][i]];

      for (unsigned int i = 0; i < p_grads.size(); ++i)
        for (unsigned int d1 = 0; d1 < dim; ++d1)
          grads[lexicographic_to_hierarchic[i + d * n_sub]][d][(d1 + d) % dim] = p_grads[renumber_aniso[d][i]][d1];

      for (unsigned int i = 0; i < p_grad_grads.size(); ++i)
        for (unsigned int d1 = 0; d1 < dim; ++d1)
          for (unsigned int d2 = 0; d2 < dim; ++d2)
            grad_grads[lexicographic_to_hierarchic[i + d * n_sub]][d][(d1 + d) % dim][(d2 + d) % dim] =
        ...
        for (unsigned int d1 = 0; d1 < dim; ++d1)
          for (unsigned int d2 = 0; d2 < dim; ++d2)
            for (unsigned int d3 = 0; d3 < dim; ++d3)
              third_derivatives[lexicographic_to_hierarchic[i + d * n_sub]][d][(d1 + d) % dim][(d2 + d) % dim]
                               [(d3 + d) % dim] = p_third_derivatives[renumber_aniso[d][i]][d1][d2][d3];
          ...
          for (unsigned int d2 = 0; d2 < dim; ++d2)
            for (unsigned int d3 = 0; d3 < dim; ++d3)
              for (unsigned int d4 = 0; d4 < dim; ++d4)
                fourth_derivatives[lexicographic_to_hierarchic[i + d * n_sub]][d][(d1 + d) % dim][(d2 + d) % dim]
                                  [(d3 + d) % dim][(d4 + d) % dim] =
...
template <int dim>
unsigned int
PolynomialsRaviartThomas<dim>::n_polynomials(const unsigned int normal_degree, const unsigned int tangential_degree)
{
  return dim * (normal_degree + 1) * Utilities::pow(tangential_degree + 1, dim - 1);
}
...
PolynomialsRaviartThomas<dim>::get_lexicographic_numbering(const unsigned int normal_degree,
                                                           const unsigned int tangential_degree)
{
  const unsigned int        n_dofs_face = Utilities::pow(tangential_degree + 1, dim - 1);
  ...
  for (unsigned int j = 0; j < n_dofs_face; ++j)
    {
      ...
      if (normal_degree > 1)
        for (unsigned int i = n_dofs_face * 2 * dim; i < n_dofs_face * 2 * dim + normal_degree - 1; ++i)
  ...
  for (unsigned int k = 0; k < layers; ++k)
    {
      ...
      for (unsigned int j = n_dofs_face * 2; j < n_dofs_face * 2 + tangential_degree + 1; ++j)
        lexicographic_numbering.push_back(j + k_add);

      if (normal_degree > 1)
        for (unsigned int i = n_dofs_face * (2 * dim + (normal_degree - 1));
             i < n_dofs_face * (2 * dim + (normal_degree - 1)) + (normal_degree - 1) * (tangential_degree + 1);
             ++i)
          {
            lexicographic_numbering.push_back(i + k_add * tangential_degree);
          }
      for (unsigned int j = n_dofs_face * 3; j < n_dofs_face * 3 + tangential_degree + 1; ++j)
        lexicographic_numbering.push_back(j + k_add);
    }
  ...
  if (dim == 3)
    {
      ...
      if (normal_degree > 1)
        for (unsigned int i = 6 * n_dofs_face + n_dofs_face * 2 * (normal_degree - 1);
...
PolynomialsRaviartThomas<dim>::get_polynomial_support_points() const
{
  ...
  const Quadrature<1>     tangential(tangential_degree == 0 ?
                                   static_cast<Quadrature<1>>(QMidpoint<1>()) :
                                   static_cast<Quadrature<1>>(QGaussLobatto<1>(tangential_degree + 1)));
  const Quadrature<1>     normal(normal_degree == 0 ? static_cast<Quadrature<1>>(QMidpoint<1>()) :
                                                  static_cast<Quadrature<1>>(QGaussLobatto<1>(normal_degree + 1)));
  const QAnisotropic<dim> quad =
    (dim == 1 ? QAnisotropic<dim>(normal) :
                (dim == 2 ? QAnisotropic<dim>(normal, tangential) : QAnisotropic<dim>(normal, tangential, tangential)));
  ...
  for (unsigned int d = 0; d < dim; ++d)
    for (unsigned int i = 0; i < n_sub; ++i)
      for (unsigned int c = 0; c < dim; ++c)
        points[lexicographic_to_hierarchic[i + d * n_sub]][(c + d) % dim] = quad.point(renumber_aniso[d][i])[c];
  return points;
}

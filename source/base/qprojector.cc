...
namespace internal
{
  namespace QProjector
  {
    namespace
    {
       ...
       */
      void
      project_to_hex_face_and_append(const std::vector<Point<2>> &points,
                                     ...
                                     const unsigned int           subface_no = 0)
      {
        ...
        const unsigned int xi_index = (1 + face_no / 2) % 3, eta_index = (2 + face_no / 2) % 3,
      ...
      std::vector<Point<2>>
      mutate_points_with_offset(const std::vector<Point<2>> &points, const unsigned int offset)
      {
        switch (offset)
          {
            case 0:
      ...
      {
        static const unsigned int offset[2][2][2] = {{{4, 5},   // face_orientation=false; face_flip=false;
      ...
      select_subface_no_and_refinement_case(const unsigned int             subface_no,
                                            const bool                     face_orientation,
                                            const bool                     face_flip,
                                            const bool                     face_rotation,
                                            const internal::SubfaceCase<3> ref_case)
      {
        ...
        static const RefinementCase<dim - 1> equivalent_refine_case[internal::SubfaceCase<dim>::case_isotropic + 1]
        ...
        static const unsigned int equivalent_subface_number[internal::SubfaceCase<dim>::case_isotropic + 1]
                                                           [GeometryInfo<3>::max_children_per_face] = {
                                                             // case_none, see above
                                                             {0, 1, 2, 3},
                                                             // case_x
        ...
        // exchanged.
        static const RefinementCase<dim - 1> ref_case_permutation[4] = {RefinementCase<dim - 1>::no_refinement,
                                                                        RefinementCase<dim - 1>::cut_y,
                                                                        RefinementCase<dim - 1>::cut_x,
                                                                        RefinementCase<dim - 1>::cut_xy};
        ...
        const unsigned int final_subface_no = GeometryInfo<dim>::child_cell_on_face(RefinementCase<dim>(final_ref_case),
                                                                                    4,
                                                                                    equ_subface_no,
                                                                                    face_orientation,
                                                                                    face_flip,
                               ...
                               std::vector<Point<2>> &q_points)
{
  ...
  AssertIndexRange(face_no, GeometryInfo<dim>::faces_per_cell);
  Assert(q_points.size() == quadrature.size(), ExcDimensionMismatch(q_points.size(), quadrature.size()));
        ...
        switch (face_no)
          {
            case 0:
              ...
              break;
            case 1:
              q_points[p] = Point<dim>(1 - quadrature.point(p)(0), quadrature.point(p)(0));
              break;
            case 2:
                               ...
                               std::vector<Point<3>> &q_points)
{
  ...
  Assert(q_points.size() == quadrature.size(), ExcDimensionMismatch(q_points.size(), quadrature.size()));
  q_points.clear();
  internal::QProjector::project_to_hex_face_and_append(quadrature.get_points(), face_no, q_points);
}
                                        ...
                                        const bool           face_rotation)
{
  ...
  const Quadrature<2> mutation =
    internal::QProjector::mutate_quadrature(quadrature, face_orientation, face_flip, face_rotation);
        ...
        switch (face_no)
          {
              ...
              switch (subface_no)
                {
                  case 0:
                    ...
                    break;
                  case 1:
                    q_points[p] = Point<dim>(0.5 + quadrature.point(p)(0) / 2, 0);
                    break;
                  default:
                    Assert(false, ExcInternalError());
                }
            ...
            case 1:
              switch (subface_no)
                {
                  case 0:
                    q_points[p] = Point<dim>(1 - quadrature.point(p)(0) / 2, quadrature.point(p)(0) / 2);
                    break;
                  case 1:
                    q_points[p] = Point<dim>(0.5 - quadrature.point(p)(0) / 2, 0.5 + quadrature.point(p)(0) / 2);
                    break;
                  default:
                    Assert(false, ExcInternalError());
                }
              ...
              switch (subface_no)
                {
                  case 0:
                    ...
                    break;
                  case 1:
                    q_points[p] = Point<dim>(0, 0.5 - quadrature.point(p)(0) / 2);
                    break;
                  default:
                    Assert(false, ExcInternalError());
                }
...
template <int dim>
Quadrature<dim>
QProjector<dim>::project_to_oriented_subface(const ReferenceCell       &reference_cell,
                                             const Quadrature<dim - 1> &quadrature,
                                             const unsigned int         face_no,
                                             const unsigned int         subface_no,
                                             const bool,
                                             ...
                                             const internal::SubfaceCase<dim>)
{
  return QProjector<dim>::project_to_subface(
    reference_cell, quadrature, face_no, subface_no, RefinementCase<dim - 1>::isotropic_refinement);
}
                                           ...
                                           const internal::SubfaceCase<3> ref_case)
{
  ...
  const std::pair<unsigned int, RefinementCase<2>> final_subface_no_and_ref_case =
  ...
  return QProjector<3>::project_to_subface(
    reference_cell, mutation, face_no, final_subface_no_and_ref_case.first, final_subface_no_and_ref_case.second);
}
...
template <>
Quadrature<1>
QProjector<1>::project_to_all_faces(const ReferenceCell &reference_cell, const hp::QCollection<0> &quadrature)
{
  ...
  for (unsigned int face = 0; face < n_faces; ++face)
    {
      project_to_face(reference_cell, quadrature[quadrature.size() == 1 ? 0 : face], face, help);
  ...
  for (unsigned int face = 0; face < n_faces; ++face)
    std::copy(quadrature[quadrature.size() == 1 ? 0 : face].get_weights().begin(),
...
template <>
Quadrature<2>
QProjector<2>::project_to_all_faces(const ReferenceCell &reference_cell, const hp::QCollection<1> &quadrature)
{
  if (reference_cell == ReferenceCells::Triangle)
    {
      const auto support_points_line = [](const auto &face, const auto &orientation) -> std::vector<Point<2>> {
        ...
        const auto                temp = ReferenceCells::Line.permute_by_combined_orientation(vertices, orientation);
        return std::vector<Point<2>>(temp.begin(), temp.begin() + face.first.size());
      };
        ...
        for (unsigned int orientation = 0; orientation < 2; ++orientation)
          {
            ...
            // orientation
            std::vector<Point<2>> support_points = support_points_line(face, orientation);

            // the quadrature rule to be projected ...
            const auto &sub_quadrature_points  = quadrature[quadrature.size() == 1 ? 0 : face_no].get_points();
            const auto &sub_quadrature_weights = quadrature[quadrature.size() == 1 ? 0 : face_no].get_weights();
            ...
            for (unsigned int j = 0; j < sub_quadrature_points.size(); ++j)
              {
                ...
                for (unsigned int i = 0; i < 2; ++i)
                  mapped_point += support_points[i] * poly.compute_value(i, sub_quadrature_points[j]);
...
template <>
Quadrature<3>
QProjector<3>::project_to_all_faces(const ReferenceCell &reference_cell, const hp::QCollection<2> &quadrature)
{
  const auto process = [&](const std::vector<std::vector<Point<3>>> &faces) {
    ...
    const TensorProductPolynomials<2> poly_quad(
      Polynomials::generate_complete_Lagrange_basis({Point<1>(0.0), Point<1>(1.0)}));
    ...
    for (unsigned int face_no = 0; face_no < faces.size(); ++face_no)
      {
        ...
        const auto &poly = (n_linear_shape_functions == 3 ? static_cast<const ScalarPolynomialsBase<2> &>(poly_tri) :
                                                            static_cast<const ScalarPolynomialsBase<2> &>(poly_quad));

        // ... and over all possible orientations
        for (unsigned char orientation = 0; orientation < reference_cell.n_face_orientations(face_no); ++orientation)
          {
            const auto &face = faces[face_no];

            const boost::container::small_vector<Point<3>, 8> support_points =
              reference_cell.face_reference_cell(face_no).permute_by_combined_orientation<Point<3>>(face, orientation);
            ...
            for (unsigned int j = 0; j < sub_quadrature_points.size(); ++j)
              {
                ...
                const double scaling = [&]() {
                  ...
                  for (unsigned int i = 0; i < n_linear_shape_functions; ++i)
                    shape_derivatives[i] = poly.compute_1st_derivative(i, sub_quadrature_points[j]);

                  for (unsigned int k = 0; k < n_linear_shape_functions; ++k)
                    for (unsigned int i = 0; i < spacedim; ++i)
                      for (unsigned int j = 0; j < dim_; ++j)
                        DX_t[j][i] += shape_derivatives[k][j] * support_points[k][i];
  ...
  if ((reference_cell == ReferenceCells::Tetrahedron) || (reference_cell == ReferenceCells::Wedge) ||
      (reference_cell == ReferenceCells::Pyramid))
    {
      std::vector<std::vector<Point<3>>> face_vertex_locations(reference_cell.n_faces());
      for (const unsigned int f : reference_cell.face_indices())
        {
          face_vertex_locations[f].resize(reference_cell.face_reference_cell(f).n_vertices());
          for (const unsigned int v : reference_cell.face_reference_cell(f).vertex_indices())
            face_vertex_locations[f][v] = reference_cell.face_vertex_location<3>(f, v);
        }
  ...
  else
    {
      ...
      for (unsigned int offset = 0; offset < 8; ++offset)
        {
          const auto mutation = internal::QProjector::mutate_points_with_offset(quadrature[0].get_points(), offset);
          ...
          for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
            {
              ...
              internal::QProjector::project_to_hex_face_and_append(
                q_index > 0 ? internal::QProjector::mutate_points_with_offset(quadrature[face].get_points(), offset) :
                              mutation,
                face,
                q_points);
...
template <>
Quadrature<1>
QProjector<1>::project_to_all_subfaces(const ReferenceCell &reference_cell, const Quadrature<0> &quadrature)
{
  Assert(reference_cell == ReferenceCells::Line, ExcNotImplemented());
  (void)reference_cell;
  ...
  for (unsigned int face = 0; face < n_faces; ++face)
    for (unsigned int subface = 0; subface < subfaces_per_face; ++subface)
      std::copy(quadrature.get_weights().begin(), quadrature.get_weights().end(), std::back_inserter(weights));
...
template <>
Quadrature<2>
QProjector<2>::project_to_all_subfaces(const ReferenceCell &reference_cell, const SubQuadrature &quadrature)
{
  if (reference_cell == ReferenceCells::Triangle || reference_cell == ReferenceCells::Tetrahedron)
...
template <>
Quadrature<3>
QProjector<3>::project_to_all_subfaces(const ReferenceCell &reference_cell, const SubQuadrature &quadrature)
{
  if (reference_cell == ReferenceCells::Triangle || reference_cell == ReferenceCells::Tetrahedron)
  ...
  for (unsigned int offset = 0; offset < 8; ++offset)
    {
      const auto mutation = internal::QProjector::mutate_points_with_offset(quadrature.get_points(), offset);

      // project to each face and copy results
      for (unsigned int face = 0; face < n_faces; ++face)
        for (unsigned int ref_case = RefinementCase<dim - 1>::cut_xy; ref_case >= RefinementCase<dim - 1>::cut_x;
             --ref_case)
          for (unsigned int subface = 0; subface < GeometryInfo<dim - 1>::n_children(RefinementCase<dim - 1>(ref_case));
               ++subface)
            {
              internal::QProjector::project_to_hex_face_and_append(
                mutation, face, q_points, RefinementCase<dim - 1>(ref_case), subface);
...
{
  Assert(reference_cell == ReferenceCells::get_hypercube<dim>(), ExcNotImplemented());
  (void)reference_cell;
...
template <int dim>
Quadrature<dim>
QProjector<dim>::project_to_all_children(const ReferenceCell &reference_cell, const Quadrature<dim> &quadrature)
{
  Assert(reference_cell == ReferenceCells::get_hypercube<dim>(), ExcNotImplemented());
  (void)reference_cell;

  const unsigned int n_points = quadrature.size(), n_children = GeometryInfo<dim>::max_children_per_cell;
  ...
  for (unsigned int child = 0; child < n_children; ++child)
    {
      Quadrature<dim> help = project_to_child(reference_cell, quadrature, child);
                                         ...
                                         const unsigned int   n_quadrature_points)
{
  if (reference_cell == ReferenceCells::Triangle || reference_cell == ReferenceCells::Tetrahedron)
    {
      if (dim == 2)
        return {(2 * face_no + (face_orientation ? 1 : 0)) * n_quadrature_points};
      else if (dim == 3)
        {
          ...
          return {(reference_cell.n_face_orientations(face_no) * face_no + orientation) * n_quadrature_points};
        }
    }
      ...
      case 3:
        {
          ...
          return ((face_no + offset[face_orientation][face_flip][face_rotation]) * n_quadrature_points);
        }
                                         ...
                                         const hp::QCollection<dim - 1> &quadrature)
{
  if (reference_cell == ReferenceCells::Triangle || reference_cell == ReferenceCells::Tetrahedron ||
      reference_cell == ReferenceCells::Wedge || reference_cell == ReferenceCells::Pyramid)
    {
      ...
      static const std::array<unsigned int, 5> scale_pyramid = {{8, 6, 6, 6, 6}};

      const auto &scale = (reference_cell == ReferenceCells::Triangle) ?
                            scale_tri :
                            ((reference_cell == ReferenceCells::Tetrahedron) ?
                               scale_tet :
                               ((reference_cell == ReferenceCells::Wedge) ? scale_wedge : scale_pyramid));
      ...
      if (dim == 2)
        return {offset + face_orientation * quadrature[quadrature.size() == 1 ? 0 : face_no].size()};
      else if (dim == 3)
        {
          return {offset + internal::combined_face_orientation(face_orientation, face_rotation, face_flip) *
      ...
      case 3:
        {
          ...
          if (quadrature.size() == 1)
            return (face_no + offset[face_orientation][face_flip][face_rotation] * GeometryInfo<dim>::faces_per_cell) *
                   quadrature[0].size();
          else
            {
              ...
              return (n_points_i + offset[face_orientation][face_flip][face_rotation] * n_points);
            }
        }
...
QProjector<1>::DataSetDescriptor::subface(const ReferenceCell &reference_cell,
                                          const unsigned int   face_no,
                                          const unsigned int   subface_no,
                                          const bool,
                                          const bool,
...
QProjector<2>::DataSetDescriptor::subface(const ReferenceCell &reference_cell,
                                          const unsigned int   face_no,
                                          const unsigned int   subface_no,
                                          const bool,
                                          const bool,
...
QProjector<3>::DataSetDescriptor::subface(const ReferenceCell           &reference_cell,
                                          const unsigned int             face_no,
                                          const unsigned int             subface_no,
                                          const bool                     face_orientation,
                                          const bool                     face_flip,
                                          ...
                                          const internal::SubfaceCase<3> ref_case)
{
  ...
  return (((face_no * total_subfaces_per_face + ref_case_offset[final_subface_no_and_ref_case.second - 1] +

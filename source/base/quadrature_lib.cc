...
namespace internal
{
  namespace QGaussLobatto
  {
    ...
    compute_quadrature_weights(const std::vector<long double> &x, const int alpha, const int beta)
    {
      ...
      const long double factor = std::pow(2., alpha + beta + 1) * gamma(alpha + q) * gamma(beta + q) /
  ...
  : Quadrature<1>(7)
{
  ...
  static const double wts[]  = {41. / 840., 216. / 840., 27. / 840., 272. / 840., 27. / 840., 216. / 840., 41. / 840.};
...
QGaussOneOverR<2>::quad_size(const Point<2> &singularity, const unsigned int n)
{
  ...
  for (unsigned int d = 0; d < 2; ++d)
    on_edge = on_edge || (std::abs(singularity[d]) < eps || std::abs(singularity[d] - 1.0) < eps);
  const bool on_vertex = on_edge && std::abs((singularity - Point<2>(.5, .5)).norm_square() - .5) < eps;
  if (on_vertex)
    return 2 * n * n;
  else if (on_edge)
    return 4 * n * n;
        ...
        for (unsigned int q = 0; q < quads[box].size(); ++q, ++q_id)
          {
            ...
            this->quadrature_points[q_id] = origins[box] + Point<2>(dist[0] * qp[0], dist[1] * qp[1]);
  ...
  : Quadrature<dim>(quad)
{
  ...
  std::sort(permutation.begin(), permutation.end(), [this](const unsigned int x, const unsigned int y) {
...
QTelles<dim>::QTelles(const Quadrature<1> &base_quad,
  ...
  : // We need the explicit implementation if dim == 1. If dim > 1 we use the
    ...
    // dimensions.
  Quadrature<dim>(dim == 2 ? QAnisotropic<dim>(QTelles<1>(base_quad, Point<1>(singularity[0])),
  ...
  for (unsigned int q = 0; q < quadrature_points.size(); ++q)
    {
      ...
      double eta   = (Utilities::fixed_power<3>(gamma - gamma_bar) + gamma_bar * (gamma_bar * gamma_bar + 3)) /
...
Quadrature<spacedim>
QSimplex<dim>::compute_affine_transformation(const std::array<Point<spacedim>, dim + 1> &vertices) const
{
  Assert(dim <= spacedim, ExcMessage("Invalid combination of dim and spacedim ."));
  ...
  for (unsigned int i = 0; i < this->size(); ++i)
    {
      qp[i] = Point<spacedim>(vertices[0] + apply_transformation(B, this->point(i)));
      w[i]  = J * this->weight(i);
    }
...
template <int spacedim>
Quadrature<spacedim>
QSimplex<dim>::mapped_quadrature(const std::vector<std::array<Point<spacedim>, dim + 1>> &simplices) const
{
  Assert(!(dim == 1 && spacedim == 1), ExcMessage("This function is not supposed to work in 1D-1d case."));
  Assert(dim <= spacedim, ExcMessage("Invalid combination of dim and spacedim ."));
  ...
  for (const auto &simplex : simplices)
    {
      ...
      std::transform(rule.get_weights().begin(), rule.get_weights().end(), std::back_inserter(ws), [&](const double w) {
        return w;
      });
    }
...
QTrianglePolar::QTrianglePolar(const Quadrature<1> &radial_quadrature, const Quadrature<1> &angular_quadrature)
...
QDuffy::QDuffy(const Quadrature<1> &radial_quadrature, const Quadrature<1> &angular_quadrature, const double beta)
  : QSimplex<2>(Quadrature<2>())
{
  const QAnisotropic<2> base(radial_quadrature, angular_quadrature);
  this->quadrature_points.resize(base.size());
    ...
    for (unsigned int start = 0; start < (dim > 2 ? 2 : 1); ++start)
      {
        for (unsigned int i = 0; i < dim; ++i)
          vertices[i + 1] = GeometryInfo<dim>::unit_cell_vertex(GeometryInfo<dim>::face_to_cell_vertices(f, i + start));
        ...
        if (quad.size())
          {
            this->quadrature_points.insert(this->quadrature_points.end(),
                                           ...
                                           quad.get_points().end());
            this->weights.insert(this->weights.end(), quad.get_weights().begin(), quad.get_weights().end());
          }
      }
}
  ...
  : QSimplex<dim>(Quadrature<dim>())
{
  ...
  else if (dim == 2)
    {
      if (n_points_1D == 1)
      ...
      else if (n_points_1D == 2)
        {
          ...
          this->quadrature_points.emplace_back(0.17855872826361643, 0.1550510257216822);
          this->quadrature_points.emplace_back(0.07503111022260812, 0.6449489742783178);
          this->quadrature_points.emplace_back(0.6663902460147014, 0.1550510257216822);
          this->quadrature_points.emplace_back(0.28001991549907407, 0.6449489742783178);
        ...
        {
          Quadrature<dim>::operator=(QWitherdenVincentSimplex<dim>(n_points_1D));
        }
    }
  else if (dim == 3)
    {
      ...
      // Applications, 2010
      else if (n_points_1D == 2)
        {
          ...
          this->quadrature_points.emplace_back(0.1620014916985245, 0.1838503504920977, 0.01271836631368145);
          this->quadrature_points.emplace_back(0.01090521221118924, 0.2815238021235462, 0.3621268299455338);
          this->quadrature_points.emplace_back(0.1901170024392839, 0.01140332944455717, 0.3586207204668839);
          this->quadrature_points.emplace_back(0.170816925164989, 0.1528181430909273, 0.6384932999617267);
          this->quadrature_points.emplace_back(0.1586851632274406, 0.5856628056552158, 0.1308471689520965);
          this->quadrature_points.emplace_back(0.5712260521491151, 0.1469183900871696, 0.1403728057942107);
        }
  ...
  Assert(this->quadrature_points.size() > 0,
         ExcNotImplemented("QGaussSimplex is currently only implemented for "
...
template <int dim>
QWitherdenVincentSimplex<dim>::QWitherdenVincentSimplex(const unsigned int n_points_1D, const bool use_odd_order)
  : QSimplex<dim>(Quadrature<dim>())
{
        ...
        switch (dim)
          {
            case 2:
              if (use_odd_order)
                ...
                {
                  // WV-2, 2d
                  process_point_1(1.6666666666666669e-01, 3.3333333333333331e-01);
                }
              break;
            case 3:
              if (use_odd_order)
                ...
                {
                  // WV-2, 3d
                  process_point_1(1.3819660112501050e-01, 2.5000000000000000e-01);
                }
              break;
            default:
              Assert(false, ExcNotImplemented());
        ...
        switch (dim)
          {
            ...
            case 3:
              if (use_odd_order)
                {
                  // WV-3, 3d
                  process_point_1(3.2816330251638171e-01, 1.3621784253708741e-01);
                  process_point_1(1.0804724989842859e-01, 1.1378215746291261e-01);
                }
              else
                {
                  // WV-5 (no WV-4 in 3d)
        ...
        switch (dim)
          {
            case 2:
              if (use_odd_order)
                ...
                {
                  // WV-6, 2d
                  process_point_1(6.3089014491502227e-02, 5.0844906370206819e-02);
                  process_point_1(2.4928674517091043e-01, 1.1678627572637937e-01);
                  process_point_3(5.3145049844816938e-02, 3.1035245103378439e-01, 8.2851075618373571e-02);
                }
              break;
            case 3:
              if (use_odd_order)
                {
                  // WV-5, 3d
                  process_point_1(3.1088591926330061e-01, 1.1268792571801590e-01);
                  process_point_1(9.2735250310891248e-02, 7.3493043116361956e-02);
                  process_point_2(4.5503704125649642e-02, 4.2546020777081472e-02);
                }
              else
                {
                  // WV-6, 3d
                  process_point_1(4.0673958534611372e-02, 1.0077211055320640e-02);
                  process_point_1(3.2233789014227548e-01, 5.5357181543654717e-02);
                  process_point_1(2.1460287125915201e-01, 3.9922750258167487e-02);
                  process_point_3(6.3661001875017442e-02, 6.0300566479164919e-01, 4.8214285714285710e-02);
                }
              break;
            default:
              Assert(false, ExcInternalError());
              ...
              if (use_odd_order)
                {
                  ...
                  process_point_3(4.7036644652595216e-02, 1.9868331479735168e-01, 5.5878732903199779e-02);
                }
              else
                {
                  // WV-8, 2d
                  ...
                  process_point_1(5.0547228317030957e-02, 3.2458497623198079e-02);
                  process_point_1(4.5929258829272313e-01, 9.5091634267284619e-02);
                  process_point_1(1.7056930775176021e-01, 1.0321737053471824e-01);
                  process_point_3(8.3947774099575878e-03, 2.6311282963463811e-01, 2.7230314174434993e-02);
                }
              break;
            case 3:
              if (use_odd_order)
                {
                  ...
                  process_point_1(3.1570114977820279e-01, 4.2329581209967028e-02);
                  process_point_2(5.0489822598396350e-02, 3.1896927832857580e-02);
                  process_point_3(1.8883383102600099e-01, 5.7517163758699996e-01, 3.7207130728334620e-02);
                  process_point_3(2.1265472541483140e-02, 8.1083024109854862e-01, 8.1107708299033420e-03);
                }
              else
                {
                  // WV-8, 3d
                  process_point_1(1.0795272496221089e-01, 2.6426650908408830e-02);
                  process_point_1(1.8510948778258660e-01, 5.2031747563738531e-02);
                  process_point_1(4.2316543684767283e-02, 7.5252561535401989e-03);
                  process_point_1(3.1418170912403898e-01, 4.1763782856934897e-02);
                  process_point_2(4.3559132858383021e-01, 3.6280930261308818e-02);
                  process_point_3(2.1433930127130570e-02, 7.1746406342630831e-01, 7.1569028908444327e-03);
                  process_point_3(2.0413933387602909e-01, 5.8379737830214440e-01, 1.5453486150960340e-02);
                }
              break;
            default:
              Assert(false, ExcInternalError());
              ...
              if (use_odd_order)
                {
                  ...
                  process_point_3(3.6838412054736258e-02, 2.2196298916076568e-01, 4.3283539377289376e-02);
                }
              else
                {
                  // WV-10, 2d
                  ...
                  process_point_1(3.2055373216943517e-02, 1.3352968813149567e-02);
                  process_point_1(1.4216110105656438e-01, 4.5957963604744731e-02);
                  process_point_3(2.8367665339938453e-02, 1.6370173373718250e-01, 2.5297757707288385e-02);
                  process_point_3(2.9619889488729734e-02, 3.6914678182781102e-01, 3.4184648162959429e-02);
                  process_point_3(1.4813288578382056e-01, 3.2181299528883545e-01, 6.3904906396424044e-02);
                }
              break;
            case 3:
              if (use_odd_order)
                {
                  ...
                  process_point_1(6.1981697552226933e-10, 6.4319281759256394e-05);
                  process_point_1(1.6077453539526160e-01, 2.3173338462425461e-02);
                  process_point_1(3.2227652182142102e-01, 2.9562912335429289e-02);
                  process_point_1(4.5108918345413578e-02, 8.0639799796161822e-03);
                  process_point_2(1.1229654600437609e-01, 3.8134080103702457e-02);
                  process_point_3(4.5887144875245922e-01, 2.5545792330413102e-03, 8.3844221982985519e-03);
                  process_point_3(3.3775870685338598e-02, 7.1835032644207453e-01, 1.0234559352745330e-02);
                  process_point_3(1.8364136980992790e-01, 3.4415910578175279e-02, 2.0524915967988139e-02);
                }
              else
                {
                  // WV-10, 3d
                  ...
                  process_point_1(3.1225006869518868e-01, 2.6937059992268701e-02);
                  process_point_1(1.1430965385734609e-01, 9.8691597167933822e-03);
                  process_point_3(4.1043073921896539e-01, 1.6548602561961109e-01, 1.1393881220195230e-02);
                  process_point_3(6.1380088247906528e-03, 9.4298876734520487e-01, 3.6194434433925362e-04);
                  process_point_3(1.2105018114558939e-01, 4.7719037990428043e-01, 2.5739731980456069e-02);
                  process_point_3(3.2779468216442620e-02, 5.9425626948000698e-01, 1.0135871679755789e-02);
                  process_point_3(3.2485281564823047e-02, 8.0117728465834437e-01, 6.5761472770359038e-03);
                  process_point_3(1.7497934218393901e-01, 6.2807184547536599e-01, 1.2907035798861989e-02);
                }
              break;
            default:
              Assert(false, ExcNotImplemented());
        ...
        if (use_odd_order)
          {
            ...
            process_point_3(7.3254276860644785e-03, 1.4932478865208237e-01, 1.0290289572953278e-02);
            process_point_3(4.6010500165429957e-02, 2.8958112563770588e-01, 4.0332476640500554e-02);
          }
        else
          {
            // WV-12, 2d
            ...
            process_point_3(2.1382490256170616e-02, 1.2727971723358933e-01, 1.5083677576511438e-02);
            process_point_3(2.3034156355267121e-02, 2.9165567973834094e-01, 2.1783585038607559e-02);
            process_point_3(1.1629601967792658e-01, 2.5545422863851736e-01, 4.3227363659414209e-02);
          }
        break;
      case 7:
        ...
        if (use_odd_order)
          {
            ...
            process_point_3(5.1263891023823893e-03, 2.7251581777342970e-01, 9.5906810035432631e-03);
            process_point_3(2.4370186901093827e-02, 1.1092204280346341e-01, 1.4965401105165668e-02);
            process_point_3(8.7895483032197297e-02, 1.6359740106785048e-01, 2.4179039811593819e-02);
            process_point_3(6.8012243554206653e-02, 3.0844176089211778e-01, 3.4641276140848373e-02);
          }
        else
          {
            // WV-14, 2d
            ...
            process_point_3(1.2683309328720416e-03, 1.1897449769695684e-01, 5.0102288385006719e-03);
            process_point_3(1.4646950055654417e-02, 2.9837288213625779e-01, 1.4436308113533840e-02);
            process_point_3(5.7124757403647919e-02, 1.7226668782135557e-01, 2.4665753212563674e-02);
            process_point_3(9.2916249356971847e-02, 3.3686145979634496e-01, 3.8571510787060684e-02);
          }
        break;
      default:
        Assert(false, ExcNotImplemented());
  ...
  for (unsigned int permutation_n = 0; permutation_n < b_weights.size(); ++permutation_n)
    {
      for (const std::array<double, dim + 1> &b_point : b_point_permutations[permutation_n])
        {
...
namespace
{
  ...
  Quadrature<1>
  setup_qiterated_1D(const Quadrature<1> &base_quad, const unsigned int n_copies)
  {
...
template <int dim>
QIteratedSimplex<dim>::QIteratedSimplex(const Quadrature<dim> &base_quad, const unsigned int n_copies)
{
  switch (dim)
    {
      case 1:
        static_cast<Quadrature<dim> &>(*this) = setup_qiterated_1D(base_quad, n_copies);
        break;
      case 2:
      case 3:
        {
          const auto n_refinements = static_cast<unsigned int>(std::round(std::log2(n_copies)));
          Assert((1u << n_refinements) == n_copies, ExcMessage("The number of copies must be a power of 2."));
          Triangulation<dim> tria;
          const auto         reference_cell = ReferenceCells::get_simplex<dim>();
          GridGenerator::reference_cell(tria, reference_cell);
          tria.refine_global(n_refinements);
          const Mapping<dim> &mapping = reference_cell.template get_default_linear_mapping<dim>();
          FE_Nothing<dim>     fe(reference_cell);

          FEValues<dim>           fe_values(mapping, fe, base_quad, update_quadrature_points | update_JxW_values);
          ...
          static_cast<Quadrature<dim> &>(*this) = Quadrature<dim>(points, weights);
  ...
  : Quadrature<dim>()
{
  ...
  for (unsigned int i = 0; i < quad_line.size(); ++i)
    for (unsigned int j = 0; j < quad_tri.size(); ++j)
      {
        this->quadrature_points.emplace_back(quad_tri.point(j)[0], quad_tri.point(j)[1], quad_line.point(i)[0]);
        this->weights.emplace_back(quad_tri.weight(j) * quad_line.weight(i));
      }

  AssertDimension(this->quadrature_points.size(), this->weights.size());
  Assert(this->quadrature_points.size() > 0, ExcMessage("No valid quadrature points!"));
}
...
QSimplex<1>::compute_affine_transformation(const std::array<Point<1>, 1 + 1> &vertices) const;

template Quadrature<2>
QSimplex<1>::compute_affine_transformation(const std::array<Point<2>, 1 + 1> &vertices) const;

template Quadrature<2>
QSimplex<2>::compute_affine_transformation(const std::array<Point<2>, 2 + 1> &vertices) const;

template Quadrature<3>
QSimplex<1>::compute_affine_transformation(const std::array<Point<3>, 1 + 1> &vertices) const;

template Quadrature<3>
QSimplex<2>::compute_affine_transformation(const std::array<Point<3>, 2 + 1> &vertices) const;

template Quadrature<3>
QSimplex<3>::compute_affine_transformation(const std::array<Point<3>, 3 + 1> &vertices) const;

template Quadrature<2>
QSimplex<1>::mapped_quadrature(const std::vector<std::array<Point<2>, 1 + 1>> &simplices) const;

template Quadrature<3>
QSimplex<1>::mapped_quadrature(const std::vector<std::array<Point<3>, 1 + 1>> &simplices) const;

template Quadrature<2>
QSimplex<2>::mapped_quadrature(const std::vector<std::array<Point<2>, 2 + 1>> &simplices) const;

template Quadrature<3>
QSimplex<2>::mapped_quadrature(const std::vector<std::array<Point<3>, 2 + 1>> &simplices) const;

template Quadrature<3>
QSimplex<3>::mapped_quadrature(const std::vector<std::array<Point<3>, 3 + 1>> &simplices) const;
#endif

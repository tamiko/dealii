...
namespace internal
{
  ...
  TableEntry::get_default_constructed_copy() const
  {
    ...
    // one.
    std::visit([](auto &arg) { arg = std::remove_reference_t<decltype(arg)>(); }, new_entry.value);
  ...
  while (entries.size() < size)
    {
      ...
      max_length = std::max(max_length, static_cast<unsigned int>(entry.get_cached_string().size()));
    }
}
...
TableHandler::declare_column(const std::string &key)
{
  ...
  Assert(columns.find(key) == columns.end(),
         ExcMessage("You are trying to declare a column with key <" + key + "> but such a column already exists."));
    ...
    while (column.second.entries.size() < max_col_length)
      {
        ...
        column.second.max_length =
          std::max(column.second.max_length, static_cast<unsigned int>(entry.get_cached_string().size()));
      }
}
...
TableHandler::add_column_to_supercolumn(const std::string &key, const std::string &superkey)
{
  ...
  if (supercolumns.count(superkey) == 0u)
    {
      std::pair<std::string, std::vector<std::string>> new_column(superkey, std::vector<std::string>());
  ...
  else
    {
      ...
      const auto order_iter = std::find(column_order.begin(), column_order.end(), key);
...
{
  Assert(columns.count(key), ExcColumnNotExistent(key));
  Assert(tex_format == "l" || tex_format == "c" || tex_format == "r", ExcUndefinedTexFormat(tex_format));
  ...
  if (auto_fill_mode == true)
    {
      ...
      for (std::map<std::string, Column>::const_iterator p = columns.begin(); p != columns.end(); ++p)
          ...
          for (unsigned int j = 0; j < n_cols; ++j)
            {
              ...
              column_widths[j]       = std::max(column_widths[j], static_cast<unsigned int>(key.size()));
          ...
          for (const auto &key : column_order)
            {
              ...
              const unsigned int front_padding = (width - key.size()) / 2,
                                 rear_padding  = (width - key.size()) - front_padding;
  ...
  for (const auto &key : column_order)
    {
      ...
      const std::map<std::string, std::vector<std::string>>::const_iterator super_iter = supercolumns.find(key);
          ...
          for (unsigned int k = 0; k < n_subcolumns; ++k)
            {
              ...
              const std::map<std::string, Column>::const_iterator col_iter = columns.find(super_iter->second[k]);
              Assert(col_iter != columns.end(), ExcInternalError());
      ...
      if (super_iter != supercolumns.end())
        {
          ...
          std::map<std::string, std::string>::const_iterator tex_super_cap_iter = tex_supercaptions.find(key);

...
namespace CGALWrappers
{
  ...
  namespace internal
  {
    ...
    void
    mark_domains(CDT &ct, Face_handle start, int index, std::list<CDT::Edge> &border)
    {
    ...
    std::optional<std::variant<CGALPoint2, CGALSegment2, CGALTriangle2, std::vector<CGALPoint2>>>
    compute_intersection_triangle_triangle(const ArrayView<const Point<2>> &triangle0,
                                           const ArrayView<const Point<2>> &triangle1)
    {
      ...
      return convert_boost_to_std(CGAL::intersection(cgal_triangle0, cgal_triangle1));
    }


    std::optional<std::variant<CGALPoint2, CGALSegment2>>
    compute_intersection_triangle_segment(const ArrayView<const Point<2>> &triangle,
...
#  if DEAL_II_CGAL_VERSION_GTE(5, 5, 0)
      ...
      return convert_boost_to_std(CGAL::intersection(cgal_segment, cgal_tetrahedron));
#  else
      Assert(false, ExcMessage("This function requires a version of CGAL greater or equal than 5.5."));
      (void)tetrahedron;
      (void)segment;
      return {};
#  endif
    ...
    // tetra, triangle
    std::optional<std::variant<CGALPoint3, CGALSegment3, CGALTriangle3, std::vector<CGALPoint3>>>
    compute_intersection_tetra_triangle(const ArrayView<const Point<3>> &tetrahedron,
...
#  if DEAL_II_CGAL_VERSION_GTE(5, 5, 0)
      ...
      return convert_boost_to_std(CGAL::intersection(cgal_triangle, cgal_tetrahedron));
#  else
...
#  endif
                                   ...
                                   const double                     tol)
    {
      ...
      const auto intersection_test = internal::compute_intersection_rect_rect(quad0, quad1);
          ...
          if (size_poly == 3)
          ...
          else if (size_poly >= 4)
            {
              ...
              for (Face_handle f : cdt.finite_face_handles())
                {
                  if (f->info().in_domain() && CGAL::to_double(cdt.triangle(f).area()) > tol)
                    {
                      collection.push_back({{CGALWrappers::cgal_point_to_dealii_point<2>(cdt.triangle(f).vertex(0)),
                                             CGALWrappers::cgal_point_to_dealii_point<2>(cdt.triangle(f).vertex(1)),
                                             CGALWrappers::cgal_point_to_dealii_point<2>(cdt.triangle(f).vertex(2))}});
                    }
                }
              return collection;
            }
                                   ...
                                   const double                     tol)
    {
      ...
      std::transform(quad.begin(), quad.end(), pts.begin(), &CGALWrappers::dealii_point_to_cgal_point<CGALPoint2, 2>);

      CGALPolygon poly(pts.begin(), pts.end());

      CGALSegment2 segm(CGALWrappers::dealii_point_to_cgal_point<CGALPoint2>(line[0]),
          ...
          if (f->info().in_domain() && CGAL::to_double(cdt.triangle(f).area()) > tol &&
              CGAL::do_intersect(segm, cdt.triangle(f)))
            {
              ...
              if (const CGALSegment2 *s = boost::get<CGALSegment2>(&*intersection))
                {
                  vertices.push_back({{CGALWrappers::cgal_point_to_dealii_point<2>((*s)[0]),
...
#  if DEAL_II_CGAL_VERSION_GTE(5, 5, 0)
      ...
      std::transform(hexa.begin(),
                     hexa.end(),
                     pts.begin(),
                     &CGALWrappers::dealii_point_to_cgal_point<CGALPoint3_exact, 3>);

      CGALSegment3_exact cgal_segment(CGALWrappers::dealii_point_to_cgal_point<CGALPoint3_exact>(line[0]),
      ...
      for (const auto &c : cgal_triangulation.finite_cell_handles())
        {
          ...
          if (CGAL::do_intersect(cgal_segment, cgal_tetrahedron))
            {
              const auto intersection = CGAL::intersection(cgal_segment, cgal_tetrahedron);
              if (const CGALSegment3_exact *s = boost::get<CGALSegment3_exact>(&*intersection))
                {
                  if (s->squared_length() > tol * tol)
                    {
                      vertices.push_back({{CGALWrappers::cgal_point_to_dealii_point<3>(s->vertex(0)),
          ...
          for (const auto &f : triangulation_quad.finite_facets())
            {
              if (CGAL::do_intersect(tet, triangulation_quad.triangle(f)))
                {
                  const auto intersection = CGAL::intersection(triangulation_quad.triangle(f), tet);

                  if (const CGALTriangle3_exact *t = boost::get<CGALTriangle3_exact>(&*intersection))
                    {
                  ...
                  if (const std::vector<CGALPoint3_exact> *vps =
                        boost::get<std::vector<CGALPoint3_exact>>(&*intersection))
                    {
                      ...
                      for (auto it = tria_inter.finite_facets_begin(); it != tria_inter.finite_facets_end(); ++it)
                        {
...
#  endif
                                   ...
                                   const double                     tol)
    {
      ...
      std::transform(hexa0.begin(),
                     hexa0.end(),
                     pts_hex0.begin(),
                     &CGALWrappers::dealii_point_to_cgal_point<CGALPoint3_exact, 3>);

      std::transform(hexa1.begin(),
                     hexa1.end(),
                     pts_hex1.begin(),
                     &CGALWrappers::dealii_point_to_cgal_point<CGALPoint3_exact, 3>);
      ...
      for (const auto &c0 : tria0.finite_cell_handles())
        {
          ...
          const auto &tetg0 =
            CGAL::make_tetrahedron(tet0.vertex(0), tet0.vertex(1), tet0.vertex(2), tet0.vertex(3), surf0);
          (void)tetg0; // instead of C++ 17s [[maybe unused]]
          for (const auto &c1 : tria1.finite_cell_handles())
            {
              const auto &tet1 = tria1.tetrahedron(c1);
              const auto &tetg1 =
                CGAL::make_tetrahedron(tet1.vertex(0), tet1.vertex(1), tet1.vertex(2), tet1.vertex(3), surf1);
              (void)tetg1; // instead of C++ 17s [[maybe unused]]
              namespace PMP                = CGAL::Polygon_mesh_processing;
              const bool test_intersection = PMP::corefine_and_compute_intersection(surf0, surf1, sm);
              if (PMP::volume(sm) > tol && test_intersection)
                {
                  ...
                  for (const auto &c : triangulation_hexa.finite_cell_handles())
                    {
                      const auto &tet = triangulation_hexa.tetrahedron(c);
                      vertices.push_back({{CGALWrappers::cgal_point_to_dealii_point<3>(tet.vertex(0)),
                                ...
                                const double                            tol)
  {
    ...
    Assert(n_vertices0 > 0 || n_vertices1 > 0,
           ExcMessage("The intersection cannot be computed as at least one of the two cells has no vertices."));

    if constexpr (dim0 == 2 && dim1 == 2 && spacedim == 2)
      {
        if (n_vertices0 == 4 && n_vertices1 == 4)
          {
            return internal::compute_intersection_quad_quad(vertices0, vertices1, tol);
          }
      }
    else if constexpr (dim0 == 2 && dim1 == 1 && spacedim == 2)
      {
        if (n_vertices0 == 4 && n_vertices1 == 2)
          {
            return internal::compute_intersection_quad_line(vertices0, vertices1, tol);
          }
      }
    else if constexpr (dim0 == 3 && dim1 == 1 && spacedim == 3)
      {
        if (n_vertices0 == 8 && n_vertices1 == 2)
          {
            return internal::compute_intersection_hexa_line(vertices0, vertices1, tol);
          }
      }
    else if constexpr (dim0 == 3 && dim1 == 2 && spacedim == 3)
      {
        if (n_vertices0 == 8 && n_vertices1 == 4)
          {
            return internal::compute_intersection_hexa_quad(vertices0, vertices1, tol);
          }
      }
    else if constexpr (dim0 == 3 && dim1 == 3 && spacedim == 3)
      {
        if (n_vertices0 == 8 && n_vertices1 == 8)
          {
            return internal::compute_intersection_hexa_hexa(vertices0, vertices1, tol);
          }
      }
    else
      {
  ...
  compute_intersection_of_cells(const typename Triangulation<dim0, spacedim>::cell_iterator &cell0,
                                ...
                                const double                                                 tol)
  {
    Assert(mapping0.get_vertices(cell0).size() == ReferenceCells::get_hypercube<dim0>().n_vertices(),
           ExcNotImplemented());
    Assert(mapping1.get_vertices(cell1).size() == ReferenceCells::get_hypercube<dim1>().n_vertices(),
           ExcNotImplemented());

    const auto &vertices0 = CGALWrappers::get_vertices_in_cgal_order(cell0, mapping0);
    const auto &vertices1 = CGALWrappers::get_vertices_in_cgal_order(cell1, mapping1);

    return compute_intersection_of_cells<dim0, dim1, spacedim>(vertices0, vertices1, tol);
  }
...
template <int dim0, int dim1, int spacedim, int n_components0, int n_components1>
std::vector<std::array<Point<spacedim>, dim1 + 1>>
compute_intersection_of_cells(const std::array<Point<spacedim>, n_components0> &vertices0,

...
namespace Differentiation
{
  namespace AD
  {
    ...
    HelperBase<ADNumberTypeCode, ScalarType>::HelperBase(const unsigned int n_independent_variables,
                                                         const unsigned int n_dependent_variables)
      : independent_variable_values(n_independent_variables, dealii::internal::NumberType<scalar_type>::value(0.0))
      ...
      , registered_marked_dependent_variables(n_dependent_variables, false)
    {
      ...
      Assert(ADNumberTypeCode != NumberTypes::none,
             ExcMessage("Floating point/arithmetic numbers have no derivatives."));
      Assert(AD::ADNumberTraits<ad_type>::n_supported_derivative_levels >= 1,
             ExcMessage("The AD number type does not support the calculation of any derivatives."));

      // Tapeless mode must be configured before any active live
      // variables are created.
      if (AD::is_tapeless_ad_number<ad_type>::value)
        {
          configure_tapeless_mode(n_independent_variables, false /*ensure_persistent_setting*/);
        }
      ...
      // initializer.
      dependent_variables.resize(n_dependent_variables, dealii::internal::NumberType<ad_type>::value(0.0));
    }
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::reset_registered_independent_variables()
    {
      std::fill(registered_independent_variable_values.begin(), registered_independent_variable_values.end(), false);
    }
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::reset_registered_dependent_variables(const bool flag)
    {
      std::fill(registered_marked_dependent_variables.begin(), registered_marked_dependent_variables.end(), flag);
    }
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::set_sensitivity_value(const unsigned int index, const scalar_type &value)
    {
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::mark_independent_variable(const unsigned int index, ad_type &out) const
    {
      Assert(index < n_independent_variables(), ExcInternalError());
      Assert(registered_independent_variable_values[index] == true, ExcInternalError());

      if (index > 0)
        {
          Assert(registered_marked_independent_variables[index - 1] == true,
                 ExcMessage("Need to extract sensitivities in the order they're created."));
        }

      if (ADNumberTraits<ad_type>::is_taped == true)
        {
          Assert(active_tape_index() != Numbers<ad_type>::invalid_tape_index, ExcMessage("Invalid tape index"));
          Assert(is_recording() == true,
                 ExcMessage("The marking of independent variables is only valid "
                            "during recording."));
        }

      internal::Marking<ad_type>::independent_variable(independent_variable_values[index],
                                                       index,
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::finalize_sensitive_independent_variables() const
    {
      ...
      if (this->independent_variables.empty())
        {
          this->independent_variables.resize(this->n_independent_variables(),
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::initialize_non_sensitive_independent_variable(const unsigned int index,
                                                                                            ad_type &out) const
    {
      if (ADNumberTraits<ad_type>::is_taped == true)
        {
      ...
      Assert(is_recording() == false,
             ExcMessage("The initialization of non-sensitive independent variables is "
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    unsigned int
    HelperBase<ADNumberTypeCode, ScalarType>::n_registered_independent_variables() const
    {
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    unsigned int
    HelperBase<ADNumberTypeCode, ScalarType>::n_registered_dependent_variables() const
    {
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    typename Types<typename HelperBase<ADNumberTypeCode, ScalarType>::ad_type>::tape_index
    ...
    HelperBase<ADNumberTypeCode, ScalarType>::print(std::ostream &stream) const
    {
      ...
      for (unsigned int i = 0; i < n_independent_variables(); ++i)
        stream << registered_independent_variable_values[i] << (i < (n_independent_variables() - 1) ? "," : "");
      stream << std::endl;
      ...
      for (unsigned int i = 0; i < n_independent_variables(); ++i)
        stream << registered_marked_independent_variables[i] << (i < (n_independent_variables() - 1) ? "," : "")
               << std::flush;
      stream << std::endl;

      stream << "Dependent variable values: " << '\n';
      for (unsigned int i = 0; i < n_dependent_variables(); ++i)
        stream << dependent_variables[i] << (i < (n_dependent_variables() - 1) ? "," : "");
      stream << std::endl;

      stream << "Registered dependent variables: " << '\n';
      for (unsigned int i = 0; i < n_dependent_variables(); ++i)
        stream << registered_marked_dependent_variables[i] << (i < (n_dependent_variables() - 1) ? "," : "");
      stream << std::endl;
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::print_values(std::ostream &stream) const
    {
      for (unsigned int i = 0; i < n_independent_variables(); ++i)
        stream << independent_variable_values[i] << (i < (n_independent_variables() - 1) ? "," : "") << std::flush;
      stream << std::endl;
    }
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::print_tape_stats(const typename Types<ad_type>::tape_index tape_index,
                                                               std::ostream                             &stream) const
    {
      if (ADNumberTraits<ad_type>::is_tapeless == true)
        return;

      Assert(is_registered_tape(tape_index), ExcMessage("Tape number not registered"));
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::reset(const unsigned int n_independent_variables,
                                                    const unsigned int n_dependent_variables,
                                                    const bool         clear_registered_tapes)
    {
      const unsigned int new_n_independent_variables =
        (n_independent_variables != dealii::numbers::invalid_unsigned_int ? n_independent_variables :
                                                                            this->n_independent_variables());
      const unsigned int new_n_dependent_variables =
        (n_dependent_variables != dealii::numbers::invalid_unsigned_int ? n_dependent_variables :
      ...
      if (AD::is_tapeless_ad_number<ad_type>::value)
        {
          configure_tapeless_mode(new_n_independent_variables, false /*ensure_persistent_setting*/);
        }
      if (AD::is_taped_ad_number<ad_type>::value)
        taped_driver.reset(clear_registered_tapes);

      independent_variable_values =
        std::vector<scalar_type>(new_n_independent_variables, dealii::internal::NumberType<scalar_type>::value(0.0));
      registered_independent_variable_values  = std::vector<bool>(new_n_independent_variables, false);
      registered_marked_independent_variables = std::vector<bool>(new_n_independent_variables, false);
      dependent_variables =
        std::vector<ad_type>(new_n_dependent_variables, dealii::internal::NumberType<ad_type>::value(0.0));
      registered_marked_dependent_variables = std::vector<bool>(new_n_dependent_variables, false);
    }
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::configure_tapeless_mode(const unsigned int n_independent_variables,
                                                                      const bool         ensure_persistent_setting)
    {
      ...
      TapelessDrivers<ad_type, scalar_type>::initialize_global_environment(n_independent_variables);
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    bool
    HelperBase<ADNumberTypeCode, ScalarType>::active_tape_requires_retaping() const
    {
      if (ADNumberTraits<ad_type>::is_tapeless == true)
        return false;
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::activate_tape(const typename Types<ad_type>::tape_index tape_index,
                                                            const bool                                read_mode)
    {
      if (ADNumberTraits<ad_type>::is_taped == true)
        {
          Assert(tape_index != Numbers<ad_type>::invalid_tape_index, ExcMessage("Invalid tape index"));
      ...
      const typename Types<ad_type>::tape_buffer_sizes tbufsize)
    {
      ...
      if (ADNumberTraits<ad_type>::is_taped == true)
        taped_driver.set_tape_buffer_sizes(obufsize, lbufsize, vbufsize, tbufsize);
    }
      ...
      if (ADNumberTraits<ad_type>::is_taped == true)
        {
          if (overwrite_tape != true)
            {
              Assert(is_recording() == false, ExcMessage("Already recording..."));
            }
          ...
          if (is_registered_tape(tape_index) == false || overwrite_tape == true)
            {
              ...
              // Start taping
              taped_driver.start_taping(active_tape_index(), keep_independent_values);
            ...
            {
              Assert(is_recording() == false, ExcMessage("Tape recording is unexpectedly still enabled."));
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::stop_recording_operations(const bool write_tapes_to_file)
    {
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    HelperBase<ADNumberTypeCode, ScalarType>::register_dependent_variable(const unsigned int index, const ad_type &func)
    {
      ...
      if (ADNumberTraits<ad_type>::is_taped == true)
        {
          Assert(active_tape_index() != Numbers<ad_type>::invalid_tape_index, ExcMessage("Invalid tape index"));
          Assert(is_recording() == true, ExcMessage("Must be recording when registering dependent variables."));
        }
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    CellLevelBase<ADNumberTypeCode, ScalarType>::CellLevelBase(const unsigned int n_independent_variables,
                                                               const unsigned int n_dependent_variables)
      : HelperBase<ADNumberTypeCode, ScalarType>(n_independent_variables, n_dependent_variables)
    {}
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    CellLevelBase<ADNumberTypeCode, ScalarType>::register_dof_values(const std::vector<scalar_type> &dof_values)
    {
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    const std::vector<typename CellLevelBase<ADNumberTypeCode, ScalarType>::ad_type> &
    CellLevelBase<ADNumberTypeCode, ScalarType>::get_sensitive_dof_values() const
    {
      if (ADNumberTraits<ad_type>::is_taped == true)
        {
          Assert(this->active_tape_index() != Numbers<ad_type>::invalid_tape_index, ExcMessage("Invalid tape index"));
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    CellLevelBase<ADNumberTypeCode, ScalarType>::set_dof_values(const std::vector<scalar_type> &values)
    {
      if (ADNumberTraits<ad_type>::is_taped == true)
        {
          Assert(this->active_tape_index() != Numbers<ad_type>::invalid_tape_index, ExcMessage("Invalid tape index"));
      ...
      Assert(values.size() == this->n_independent_variables(),
             ExcMessage("Vector size does not match number of independent variables"));
      for (unsigned int i = 0; i < this->n_independent_variables(); ++i)
        HelperBase<ADNumberTypeCode, ScalarType>::set_sensitivity_value(i, values[i]);
    }
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    EnergyFunctional<ADNumberTypeCode, ScalarType>::EnergyFunctional(const unsigned int n_independent_variables)
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    EnergyFunctional<ADNumberTypeCode, ScalarType>::register_energy_functional(const ad_type &energy)
    {
      Assert(this->n_dependent_variables() == 1, ExcInternalError());
      HelperBase<ADNumberTypeCode, ScalarType>::register_dependent_variable(0, energy);
    }
    ...
    {
      if ((ADNumberTraits<ad_type>::is_taped == true && this->taped_driver.keep_independent_values() == false) ||
          ADNumberTraits<ad_type>::is_tapeless == true)
        {
          Assert(this->n_registered_independent_variables() == this->n_independent_variables(),
                 ExcMessage("Not all values of sensitivities have been registered or subsequently set!"));
        }
      Assert(this->n_registered_dependent_variables() == this->n_dependent_variables(),
             ExcMessage("Not all dependent variables have been registered."));

      Assert(this->n_dependent_variables() == 1,
             ExcMessage("The EnergyFunctional class expects there to be only one dependent variable."));

      if (ADNumberTraits<ad_type>::is_taped == true)
        {
          Assert(this->active_tape_index() != Numbers<ad_type>::invalid_tape_index, ExcMessage("Invalid tape index"));
          Assert(this->is_recording() == false, ExcMessage("Cannot compute value while tape is being recorded."));
          ...
          return this->taped_driver.value(this->active_tape_index(), this->independent_variable_values);
        }
      else
        {
          Assert(ADNumberTraits<ad_type>::is_tapeless == true, ExcInternalError());
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    EnergyFunctional<ADNumberTypeCode, ScalarType>::compute_residual(Vector<scalar_type> &gradient) const
    {
      if ((ADNumberTraits<ad_type>::is_taped == true && this->taped_driver.keep_independent_values() == false) ||
          ADNumberTraits<ad_type>::is_tapeless == true)
        {
      ...
      if (gradient.size() != this->n_independent_variables())
        gradient.reinit(this->n_independent_variables(), true /*omit_zeroing_entries*/);

      if (ADNumberTraits<ad_type>::is_taped == true)
        {
          Assert(this->active_tape_index() != Numbers<ad_type>::invalid_tape_index, ExcMessage("Invalid tape index"));
          Assert(this->is_recording() == false, ExcMessage("Cannot compute gradient while tape is being recorded."));
          Assert(this->independent_variable_values.size() == this->n_independent_variables(),
                 ExcDimensionMismatch(this->independent_variable_values.size(), this->n_independent_variables()));

          this->taped_driver.gradient(this->active_tape_index(), this->independent_variable_values, gradient);
        }
      else
        {
          Assert(ADNumberTraits<ad_type>::is_tapeless == true, ExcInternalError());
          ...
          this->tapeless_driver.gradient(this->independent_variables, this->dependent_variables, gradient);
        }
    }
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    EnergyFunctional<ADNumberTypeCode, ScalarType>::compute_linearization(FullMatrix<scalar_type> &hessian) const
    {
      ...
      // we'll be overwriting them immediately in the succeeding call to
      // Drivers::hessian().
      if (hessian.m() != this->n_independent_variables() || hessian.n() != this->n_independent_variables())
        hessian.reinit({this->n_independent_variables(), this->n_independent_variables()},
                       true /*omit_default_initialization*/);

      if (ADNumberTraits<ad_type>::is_taped == true)
        {
          Assert(this->active_tape_index() != Numbers<ad_type>::invalid_tape_index, ExcMessage("Invalid tape index"));
          Assert(this->is_recording() == false, ExcMessage("Cannot compute hessian while tape is being recorded."));
          Assert(this->independent_variable_values.size() == this->n_independent_variables(),
                 ExcDimensionMismatch(this->independent_variable_values.size(), this->n_independent_variables()));

          this->taped_driver.hessian(this->active_tape_index(), this->independent_variable_values, hessian);
        }
      else
        {
          Assert(ADNumberTraits<ad_type>::is_tapeless == true, ExcInternalError());
          ...
          this->tapeless_driver.hessian(this->independent_variables, this->dependent_variables, hessian);
        }
    }
    ...
    ResidualLinearization<ADNumberTypeCode, ScalarType>::ResidualLinearization(
      ...
      const unsigned int n_dependent_variables)
      : CellLevelBase<ADNumberTypeCode, ScalarType>(n_independent_variables, n_dependent_variables)
    {}
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    ResidualLinearization<ADNumberTypeCode, ScalarType>::register_residual_vector(const std::vector<ad_type> &residual)
    {
      ...
      for (unsigned int i = 0; i < this->n_dependent_variables(); ++i)
        HelperBase<ADNumberTypeCode, ScalarType>::register_dependent_variable(i, residual[i]);
    }
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    ResidualLinearization<ADNumberTypeCode, ScalarType>::compute_residual(Vector<scalar_type> &values) const
    {
      if ((ADNumberTraits<ad_type>::is_taped == true && this->taped_driver.keep_independent_values() == false) ||
          ADNumberTraits<ad_type>::is_tapeless == true)
        {
      ...
      if (values.size() != this->n_dependent_variables())
        values.reinit(this->n_dependent_variables(), true /*omit_zeroing_entries*/);

      if (ADNumberTraits<ad_type>::is_taped == true)
        {
          Assert(this->active_tape_index() != Numbers<ad_type>::invalid_tape_index, ExcMessage("Invalid tape index"));
          Assert(this->is_recording() == false, ExcMessage("Cannot compute values while tape is being recorded."));
          Assert(this->independent_variable_values.size() == this->n_independent_variables(),
                 ExcDimensionMismatch(this->independent_variable_values.size(), this->n_independent_variables()));
    ...
    template <enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    ResidualLinearization<ADNumberTypeCode, ScalarType>::compute_linearization(FullMatrix<scalar_type> &jacobian) const
    {
      if ((ADNumberTraits<ad_type>::is_taped == true && this->taped_driver.keep_independent_values() == false) ||
          ADNumberTraits<ad_type>::is_tapeless == true)
        {
      ...
      if (jacobian.m() != this->n_dependent_variables() || jacobian.n() != this->n_independent_variables())
        jacobian.reinit({this->n_dependent_variables(), this->n_independent_variables()},
                        true /*omit_default_initialization*/);
      ...
      if (ADNumberTraits<ad_type>::is_taped == true)
      ...
      else
        {
          ...
          this->tapeless_driver.jacobian(this->independent_variables, this->dependent_variables, jacobian);
        }
    }
    ...
    PointLevelFunctionsBase<dim, ADNumberTypeCode, ScalarType>::PointLevelFunctionsBase(
      const unsigned int n_independent_variables,
      const unsigned int n_dependent_variables)
      : HelperBase<ADNumberTypeCode, ScalarType>(n_independent_variables, n_dependent_variables)
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    PointLevelFunctionsBase<dim, ADNumberTypeCode, ScalarType>::reset(const unsigned int n_independent_variables,
                                                                      const unsigned int n_dependent_variables,
                                                                      const bool         clear_registered_tapes)
    {
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    bool
    PointLevelFunctionsBase<dim, ADNumberTypeCode, ScalarType>::is_symmetric_independent_variable(
      const unsigned int index) const
    {
      Assert(index < symmetric_independent_variables.size(), ExcInternalError());
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    unsigned int
    PointLevelFunctionsBase<dim, ADNumberTypeCode, ScalarType>::n_symmetric_independent_variables() const
    {
      return std::count(symmetric_independent_variables.begin(), symmetric_independent_variables.end(), true);
    }
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    PointLevelFunctionsBase<dim, ADNumberTypeCode, ScalarType>::register_independent_variables(
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    const std::vector<typename PointLevelFunctionsBase<dim, ADNumberTypeCode, ScalarType>::ad_type> &
    PointLevelFunctionsBase<dim, ADNumberTypeCode, ScalarType>::get_sensitive_variables() const
    {
      if (ADNumberTraits<ad_type>::is_taped == true)
        {
          Assert(this->active_tape_index() != Numbers<ad_type>::invalid_tape_index, ExcMessage("Invalid tape index"));
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    PointLevelFunctionsBase<dim, ADNumberTypeCode, ScalarType>::set_sensitivity_value(const unsigned int index,
                                                                                      const bool symmetric_component,
                                                                                      const scalar_type &value)
    {
      HelperBase<ADNumberTypeCode, ScalarType>::set_sensitivity_value(index, value);
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    PointLevelFunctionsBase<dim, ADNumberTypeCode, ScalarType>::set_independent_variables(
      const std::vector<scalar_type> &values)
    {
      if (ADNumberTraits<ad_type>::is_taped == true)
        {
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    ScalarFunction<dim, ADNumberTypeCode, ScalarType>::ScalarFunction(const unsigned int n_independent_variables)
      : PointLevelFunctionsBase<dim, ADNumberTypeCode, ScalarType>(n_independent_variables, 1)
    {}
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    ScalarFunction<dim, ADNumberTypeCode, ScalarType>::register_dependent_variable(const ad_type &func)
    {
      Assert(this->n_dependent_variables() == 1, ExcInternalError());
      HelperBase<ADNumberTypeCode, ScalarType>::register_dependent_variable(0, func);
    }
    ...
    ScalarFunction<dim, ADNumberTypeCode, ScalarType>::compute_value() const
    {
      ...
      Assert(this->n_dependent_variables() == 1,
             ExcMessage("The ScalarFunction class expects there to be only one dependent variable."));
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    ScalarFunction<dim, ADNumberTypeCode, ScalarType>::compute_gradient(Vector<scalar_type> &gradient) const
    {
      if ((ADNumberTraits<ad_type>::is_taped == true && this->taped_driver.keep_independent_values() == false) ||
          ADNumberTraits<ad_type>::is_tapeless == true)
        {
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    ScalarFunction<dim, ADNumberTypeCode, ScalarType>::compute_hessian(FullMatrix<scalar_type> &hessian) const
    {
      Assert(AD::ADNumberTraits<ad_type>::n_supported_derivative_levels >= 2,
             ExcMessage("Cannot computed function Hessian: AD number type does "
                        "not support the calculation of second order derivatives."));
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    Tensor<0, dim, typename ScalarFunction<dim, ADNumberTypeCode, ScalarType>::scalar_type>
    ScalarFunction<dim, ADNumberTypeCode, ScalarType>::extract_hessian_component(
      ...
      const FEValuesExtractors::Scalar &extractor_col)
    {
      ...
      const std::vector<unsigned int> col_index_set(internal::extract_field_component_indices<dim>(extractor_col));
      Assert(row_index_set.size() == 1, ExcInternalError());
      Assert(col_index_set.size() == 1, ExcInternalError());

      internal::set_tensor_entry(out, 0, hessian[row_index_set[0]][col_index_set[0]]);
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    SymmetricTensor<4, dim, typename ScalarFunction<dim, ADNumberTypeCode, ScalarType>::scalar_type>
    ScalarFunction<dim, ADNumberTypeCode, ScalarType>::extract_hessian_component(
      const FullMatrix<scalar_type>                &hessian,
      ...
      const FEValuesExtractors::SymmetricTensor<2> &extractor_col)
    {
      ...
      for (unsigned int r = 0; r < row_index_set.size(); ++r)
        for (unsigned int c = 0; c < col_index_set.size(); ++c)
          {
            internal::set_tensor_entry(out, r, c, hessian[row_index_set[r]][col_index_set[c]]);
          }
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    VectorFunction<dim, ADNumberTypeCode, ScalarType>::VectorFunction(const unsigned int n_independent_variables,
                                                                      const unsigned int n_dependent_variables)
      : PointLevelFunctionsBase<dim, ADNumberTypeCode, ScalarType>(n_independent_variables, n_dependent_variables)
    {}
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    VectorFunction<dim, ADNumberTypeCode, ScalarType>::register_dependent_variables(const std::vector<ad_type> &funcs)
    {
      Assert(funcs.size() == this->n_dependent_variables(),
             ExcMessage("Vector size does not match number of dependent variables"));
      for (unsigned int i = 0; i < this->n_dependent_variables(); ++i)
        HelperBase<ADNumberTypeCode, ScalarType>::register_dependent_variable(i, funcs[i]);
    }
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    VectorFunction<dim, ADNumberTypeCode, ScalarType>::compute_values(Vector<scalar_type> &values) const
    {
      if ((ADNumberTraits<ad_type>::is_taped == true && this->taped_driver.keep_independent_values() == false) ||
          ADNumberTraits<ad_type>::is_tapeless == true)
        {
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    void
    VectorFunction<dim, ADNumberTypeCode, ScalarType>::compute_jacobian(FullMatrix<scalar_type> &jacobian) const
    {
      if ((ADNumberTraits<ad_type>::is_taped == true && this->taped_driver.keep_independent_values() == false) ||
          ADNumberTraits<ad_type>::is_tapeless == true)
        {
      ...
      if (ADNumberTraits<ad_type>::is_taped == true)
        {
          Assert(this->active_tape_index() != Numbers<ad_type>::invalid_tape_index, ExcMessage("Invalid tape index"));
          Assert(this->is_recording() == false, ExcMessage("Cannot compute Jacobian while tape is being recorded."));
          Assert(this->independent_variable_values.size() == this->n_independent_variables(),
                 ExcDimensionMismatch(this->independent_variable_values.size(), this->n_independent_variables()));
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    Tensor<0, dim, typename VectorFunction<dim, ADNumberTypeCode, ScalarType>::scalar_type>
      ...
      const FEValuesExtractors::Scalar &extractor_col)
    {
      ...
      internal::set_tensor_entry(out, 0, jacobian[row_index_set[0]][col_index_set[0]]);
    ...
    template <int dim, enum AD::NumberTypes ADNumberTypeCode, typename ScalarType>
    SymmetricTensor<4, dim, typename VectorFunction<dim, ADNumberTypeCode, ScalarType>::scalar_type>
    VectorFunction<dim, ADNumberTypeCode, ScalarType>::extract_jacobian_component(
      const FullMatrix<scalar_type>                &jacobian,
      const FEValuesExtractors::SymmetricTensor<2> &extractor_row,
      const FEValuesExtractors::SymmetricTensor<2> &extractor_col)
    {
      ...
      for (unsigned int r = 0; r < row_index_set.size(); ++r)
        for (unsigned int c = 0; c < col_index_set.size(); ++c)
          {
            internal::set_tensor_entry(out, r, c, jacobian[row_index_set[r]][col_index_set[c]]);
          }

...
#  ifndef DOXYGEN
      ...
      convert_basic_vector_to_expression_vector(const SE::vec_basic &symbol_vector)
      {
      ...
      convert_basic_pair_vector_to_expression_pair_vector(const SymEngine::vec_pair &symbol_value_vector)
      {
        ...
        for (const auto &entry : symbol_value_vector)
          symb_val_vec.emplace_back(SD::Expression(entry.first), SD::Expression(entry.second));
        return symb_val_vec;
      }

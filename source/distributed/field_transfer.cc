...
namespace parallel
{
  namespace distributed
  {
    namespace experimental
    {
      ...
      FieldTransfer<dim, VectorType, spacedim>::FieldTransfer(const DoFHandler<dim, spacedim> &dof)
        : dof_handler(dof)
      {
        ...
        const auto coarsening_strategy = [this](
                                           const typename dealii::Triangulation<dim, spacedim>::cell_iterator &parent,
                                           const std::vector<Vector<Number>> &children_values) {
          // get the equivalent DoFCellAccessor
          typename DoFHandler<dim, spacedim>::cell_iterator dof_cell_iterator(&dof_handler.get_triangulation(),
                                                                              parent->level(),
                                                                              parent->index(),
                                                                              &dof_handler);
          ...
          if (dof_handler.has_hp_capabilities())
            fe_index = dealii::internal::hp::DoFHandlerImplementation::dominated_future_fe_on_children<dim, spacedim>(
              dof_cell_iterator);

          const auto &fe = dof_handler.get_fe(fe_index);
          Assert(fe.n_dofs_per_cell() > 0, ExcMessage("Cannot coarsen onto a FiniteElement with no DoFs."));
          AssertDimension(dof_cell_iterator->n_children(), children_values.size());

          const auto         child_iterators = dof_cell_iterator->child_iterators();
          const unsigned int n_children_with_fe_nothing =
            std::count_if(child_iterators.begin(), child_iterators.end(), [](const auto &child_cell) {
              return child_cell->get_fe().n_dofs_per_cell() == 0;
            });

          Assert(n_children_with_fe_nothing == 0 || n_children_with_fe_nothing == dof_cell_iterator->n_children(),
                 ExcMessage("Coarsening is only supported for parent cells where either all"
          ...
          // interpolate.
          for (unsigned int child = 0; child < dof_cell_iterator->n_children(); ++child)
            {
              // interpolate the previously stored values on a child to the
              // mother cell
              fe.get_restriction_matrix(child, dof_cell_iterator->refinement_case()).vmult(tmp, children_values[child]);
        ...
        cell_data_transfer = std::make_unique<CellDataTransfer<dim, spacedim, std::vector<Vector<Number>>>>(
          dynamic_cast<dealii::parallel::distributed::Triangulation<dim, spacedim> &>(
            const_cast<dealii::Triangulation<dim, spacedim> &>(dof_handler.get_triangulation())),
          false,
          &dealii::AdaptationStrategies::Refinement::preserve<dim, spacedim, Vector<Number>>,
          coarsening_strategy);
      }
      ...
      template <int dim, typename VectorType, int spacedim>
      void
      FieldTransfer<dim, VectorType, spacedim>::prepare_for_coarsening_and_refinement(
        const VectorType  &in,
        const unsigned int fe_nothing_index)
      {
        ...
        for (const auto &cell : dof_handler.active_cell_iterators())
          {
            if ((cell->is_locally_owned()) && (cell->active_fe_index() != fe_nothing_index))
              {
        ...
        cell_data_transfer->prepare_for_coarsening_and_refinement(data_to_transfer);
      }
      ...
      template <int dim, typename VectorType, int spacedim>
      void
      FieldTransfer<dim, VectorType, spacedim>::interpolate(const Number                    &new_value,
                                                            const AffineConstraints<Number> &affine_constraints,
                                                            VectorType                      &out)
      {
        const unsigned int          dofs_per_cell = dof_handler.get_fe_collection().max_dofs_per_cell();
        std::vector<Vector<Number>> transferred_data(dof_handler.get_triangulation().n_active_cells(),
        ...
        for (const auto &cell : dof_handler.active_cell_iterators())
          {
            if ((cell->is_locally_owned()) && (transferred_data[cell_i][0] != std::numeric_limits<Number>::infinity()))
              {
        ...
        std::vector<std::vector<std::pair<types::global_dof_index, Number>>> constraint_lines;
        ...
        for (auto constrained_dof : locally_owned_dofs)
          if (affine_constraints.is_constrained(constrained_dof))
            {
              auto              *constraint = affine_constraints.get_constraint_entries(constrained_dof);
              const unsigned int line_size  = constraint->size();
              bool               add_line   = false;
              for (unsigned int i = 0; i < line_size; ++i)
                {
                  types::global_dof_index constraining_dof = (*constraint)[i].first;
                  // If one of the constraining value is infinity, we need to
                  // reverse the relationship
                  if (out[constraining_dof] == std::numeric_limits<Number>::infinity())
                    {
                      add_line = true;
                      break;
                    }
                  ...
                  for (unsigned int i = 0; i < line_size; ++i)
                    {
                      ...
                      if (out[constraining_dof] == std::numeric_limits<Number>::infinity())
                        {
                          auto constraining_dof_map_it = std::find(dofs_map.begin(), dofs_map.end(), constraining_dof);
                        ...
                        {
                          val -= out[constraining_dof] * (*constraint)[i].second;
                        }
                    }
        ...
        if (n_rows > 0)
          {
                ...
                for (unsigned int j = 0; j < n_cols; ++j)
                  {
                    if (j < constraint_lines[i].size())
                      {
                        auto constraint_it = std::find(dofs_map.begin(), dofs_map.end(), constraint_lines[i][j].first);
                        constraints_matrix(i, constraint_it - dofs_map.begin()) = constraint_lines[i][j].second;
                      }
                  }
            ...
            constraints_matrix.vmult(new_constrained_values, constraint_values_vec);

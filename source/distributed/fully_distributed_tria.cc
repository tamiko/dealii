...
namespace parallel
{
  namespace fullydistributed
  {
    ...
    Triangulation<dim, spacedim>::Triangulation(const MPI_Comm mpi_communicator)
      ...
      , partitioner([](dealii::Triangulation<dim, spacedim> &tria, const unsigned int n_partitions) {
      ...
      , currently_processing_prepare_coarsening_and_refinement_for_internal_usage(false)
    {}
      ...
      const TriangulationDescription::Description<dim, spacedim> &construction_data)
    {
      ...
      Assert(construction_data.comm == this->mpi_communicator, ExcMessage("MPI communicators do not match!"));
      ...
      if (settings & TriangulationDescription::Settings::construct_multigrid_hierarchy)
        this->set_mesh_smoothing(static_cast<typename dealii::Triangulation<dim, spacedim>::MeshSmoothing>(
      ...
      if (construction_data.coarse_cell_vertices.empty())
        {
          ...
          //    and global cells
          this->coarse_cell_id_to_coarse_cell_index_vector.emplace_back(numbers::invalid_coarse_cell_id, 0);
          this->coarse_cell_index_to_coarse_cell_id_vector.emplace_back(numbers::invalid_coarse_cell_id);
        }
      else
        {
          // 1) store `coarse-cell index to coarse-cell id`-mapping
          this->coarse_cell_index_to_coarse_cell_id_vector = construction_data.coarse_cell_index_to_coarse_cell_id;

          // 2) set up `coarse-cell id to coarse-cell index`-mapping
          std::map<types::coarse_cell_id, unsigned int> coarse_cell_id_to_coarse_cell_index_vector;
          for (unsigned int i = 0; i < construction_data.coarse_cell_index_to_coarse_cell_id.size(); ++i)
            coarse_cell_id_to_coarse_cell_index_vector[construction_data.coarse_cell_index_to_coarse_cell_id[i]] = i;
          ...
          currently_processing_prepare_coarsening_and_refinement_for_internal_usage = true;
          currently_processing_create_triangulation_for_internal_usage              = true;
          dealii::Triangulation<dim, spacedim>::create_triangulation(construction_data);
          currently_processing_prepare_coarsening_and_refinement_for_internal_usage = false;
          currently_processing_create_triangulation_for_internal_usage              = false;
    ...
    void Triangulation<dim, spacedim>::copy_triangulation(const dealii::Triangulation<dim, spacedim> &other_tria)
    {
      ...
      // partitioned
      if (dynamic_cast<const dealii::parallel::TriangulationBase<dim, spacedim> *>(&other_tria) == nullptr)
        {
          // actually copy the serial triangulation
          serial_tria.copy_triangulation(other_tria);

          // partition triangulation
          this->partitioner(serial_tria, dealii::Utilities::MPI::n_mpi_processes(this->mpi_communicator));
      ...
      const auto construction_data =
        TriangulationDescription::Utilities::create_description_from_triangulation(*other_tria_ptr,
    ...
    void Triangulation<dim, spacedim>::repartition()
    {
      ...
      const auto construction_data = TriangulationDescription::Utilities::create_description_from_triangulation(
        *this, this->partitioner_distributed->partition(*this), this->settings);
    ...
    {
      Assert(currently_processing_prepare_coarsening_and_refinement_for_internal_usage,
             ExcMessage("No coarsening and refinement is supported!"));

      return dealii::Triangulation<dim, spacedim>::prepare_coarsening_and_refinement();
    }
    ...
    {
      const std::size_t mem = this->dealii::parallel::TriangulationBase<dim, spacedim>::memory_consumption() +
                              MemoryConsumption::memory_consumption(coarse_cell_id_to_coarse_cell_index_vector) +
                              MemoryConsumption::memory_consumption(coarse_cell_index_to_coarse_cell_id_vector);
      return mem;
    }
    ...
    {
      return (settings & TriangulationDescription::Settings::construct_multigrid_hierarchy);
    }
      ...
      const types::coarse_cell_id coarse_cell_id) const
    {
      ...
      if (coarse_cell_index != coarse_cell_id_to_coarse_cell_index_vector.cend())
    ...
    {
      AssertIndexRange(coarse_cell_index, coarse_cell_index_to_coarse_cell_id_vector.size());

      const auto coarse_cell_id = coarse_cell_index_to_coarse_cell_id_vector[coarse_cell_index];
    ...
    void Triangulation<dim, spacedim>::update_cell_relations()
    {
      ...
      for (const auto &cell : this->active_cell_iterators())
        if (cell->is_locally_owned())
          this->local_cell_relations.emplace_back(cell, ::dealii::CellStatus::cell_will_persist);
    }
    ...
    void Triangulation<dim, spacedim>::save(const std::string &filename) const
    {
      ...
      const int mpisize = Utilities::MPI::n_mpi_processes(this->mpi_communicator);
      ...
      int ierr =
        MPI_Exscan(&n_locally_owned_cells, &global_first_cell, 1, MPI_UNSIGNED, MPI_SUM, this->mpi_communicator);
      AssertThrowMPI(ierr);
      ...
      if (myrank == 0)
        {
          ...
          f << "version nproc n_attached_fixed_size_objs n_attached_variable_size_objs n_global_active_cells"
            << std::endl
            << 4 << " " << Utilities::MPI::n_mpi_processes(this->mpi_communicator) << " "
            << this->cell_attached_data.pack_callbacks_fixed.size() << " "
            << this->cell_attached_data.pack_callbacks_variable.size() << " " << this->n_global_active_cells()
            << std::endl;
        }

      // Save cell attached data.
      this->save_attached_data(global_first_cell, this->n_global_active_cells(), filename);
      ...
      // Save triangulation description.
      {
        ...
        // Open file.
        MPI_File fh;
        ierr = MPI_File_open(this->mpi_communicator, fname_tria.c_str(), MPI_MODE_CREATE | MPI_MODE_WRONLY, info, &fh);
        AssertThrowMPI(ierr);
        ...
        // Create construction data.
        const auto construction_data =
          TriangulationDescription::Utilities::create_description_from_triangulation(*this,
                                                                                     this->mpi_communicator,
                                                                                     this->settings);
        ...
        const std::uint64_t global_position = mpisize * sizeof(std::uint64_t) + offset;

        // Write buffers to file.
        ierr = dealii::Utilities::MPI::LargeCount::File_write_at_c(fh,
                                                                   global_position,
                                                                   buffer.data(),
    ...
    {
#ifdef DEAL_II_WITH_MPI
      Assert(this->n_cells() == 0, ExcMessage("load() only works if the Triangulation is empty!"));
      ...
      unsigned int version, numcpus, attached_count_fixed, attached_count_variable, n_global_active_cells;
      {
        std::string   fname = std::string(filename) + ".info";
        ...
        f >> version >> numcpus >> attached_count_fixed >> attached_count_variable >> n_global_active_cells;
      }

      AssertThrow(version == 4, ExcMessage("Incompatible version found in .info file."));
      ...
      // Load description and construct the triangulation.
      {
        ...
        MPI_File fh;
        ierr = MPI_File_open(this->mpi_communicator, fname_tria.c_str(), MPI_MODE_RDONLY, info, &fh);
        AssertThrowMPI(ierr);
        ...
        ierr = dealii::Utilities::MPI::LargeCount::File_read_at_c(fh,
                                                                  global_position,
                                                                  buffer.data(),
                                                                  buffer.size(), // local buffer
                                                                  MPI_CHAR,
        ...
        auto construction_data =
          dealii::Utilities::template unpack<TriangulationDescription::Description<dim, spacedim>>(buffer, false);
    ...
    void Triangulation<dim, spacedim>::update_number_cache()
    {
      ...
      for (const auto &cell : this->active_cell_iterators())
        if (!cell->is_artificial())
          number_of_global_coarse_cells = std::max(number_of_global_coarse_cells, cell->id().get_coarse_cell_id());

      number_of_global_coarse_cells = Utilities::MPI::max(number_of_global_coarse_cells, this->mpi_communicator) + 1;

      this->number_cache.number_of_global_coarse_cells = number_of_global_coarse_cells;
    }

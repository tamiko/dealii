...
namespace internal
{
  namespace p4est
  {
    namespace
    {
      ...
      cell_from_quad(const dealii::parallel::distributed::Triangulation<dim, spacedim> *triangulation,
      ...
      template <int dim, int spacedim>
      void
      find_ghosts_corner(typename dealii::internal::p4est::iter<dim>::corner_info *info, void *user_data)
      {
        int   i, j;
        int   nsides = info->sides.elem_count;
        auto *sides  = reinterpret_cast<typename dealii::internal::p4est::iter<dim>::corner_side *>(info->sides.array);
        ...
        for (i = 0; i < nsides; ++i)
          {
            if (sides[i].is_ghost)
              {
                typename dealii::parallel::distributed::Triangulation<dim, spacedim>::cell_iterator cell =
                  cell_from_quad(triangulation, sides[i].treeid, *(sides[i].quad));
                Assert(cell->is_ghost(), ExcMessage("ghost quad did not find ghost cell"));
                dealii::types::subdomain_id *subid = static_cast<dealii::types::subdomain_id *>(sc_array_push(subids));
        ...
        subdomain_ids = reinterpret_cast<dealii::types::subdomain_id *>(subids->array);
            ...
            if (!sides[i].is_ghost)
              {
                ...
                for (j = 0; j < nsubs; ++j)
                  {
                    (*vertices_with_ghost_neighbors)[cell->vertex_index(sides[i].corner)].insert(subdomain_ids[j]);
                  }
              }
          }
      ...
      template <int dim, int spacedim>
      void
      find_ghosts_edge(typename dealii::internal::p4est::iter<dim>::edge_info *info, void *user_data)
      {
        int   i, j, k;
        int   nsides = info->sides.elem_count;
        auto *sides  = reinterpret_cast<typename dealii::internal::p4est::iter<dim>::edge_side *>(info->sides.array);
                ...
                for (j = 0; j < 2; ++j)
                  {
                    if (sides[i].is.hanging.is_ghost[j])
                      {
                        typename dealii::parallel::distributed::Triangulation<dim, spacedim>::cell_iterator cell =
                          cell_from_quad(triangulation, sides[i].treeid, *(sides[i].is.hanging.quad[j]));
      ...
      template <int dim, int spacedim>
      void
      find_ghosts_face(typename dealii::internal::p4est::iter<dim>::face_info *info, void *user_data)
      {
        int   i, j, k;
        int   nsides = info->sides.elem_count;
        auto *sides  = reinterpret_cast<typename dealii::internal::p4est::iter<dim>::face_side *>(info->sides.array);
        FindGhosts<dim, spacedim> *fg     = static_cast<FindGhosts<dim, spacedim> *>(user_data);
        sc_array_t                *subids = fg->subids;
        const dealii::parallel::distributed::Triangulation<dim, spacedim> *triangulation = fg->triangulation;
        int                                                                nsubs;
    ...
    int (&functions<2>::quadrant_overlaps_tree)(types<2>::tree           *tree,
                                                const types<2>::quadrant *q) = p4est_quadrant_overlaps_tree;

    void (&functions<2>::quadrant_set_morton)(types<2>::quadrant *quadrant,
                                              int                 level,
                                              std::uint64_t       id) = p4est_quadrant_set_morton;
    ...
    int (&functions<2>::quadrant_is_sibling)(const types<2>::quadrant *q1,
                                             const types<2>::quadrant *q2) = p4est_quadrant_is_sibling;

    int (&functions<2>::quadrant_is_ancestor)(const types<2>::quadrant *q1,
                                              const types<2>::quadrant *q2) = p4est_quadrant_is_ancestor;

    int (&functions<2>::quadrant_ancestor_id)(const types<2>::quadrant *q, int level) = p4est_quadrant_ancestor_id;
    ...
    types<2>::connectivity *(&functions<2>::connectivity_new_copy)(types<2>::topidx        num_vertices,
                                                                   types<2>::topidx        num_trees,
                                                                   types<2>::topidx        num_corners,
    ...
    void (&functions<2>::connectivity_destroy)(p4est_connectivity_t *connectivity) = p4est_connectivity_destroy;
    ...
    types<2>::forest *(&functions<2>::copy_forest)(types<2>::forest *input, int copy_data) = p4est_copy;
    ...
    void (&functions<2>::save)(const char *filename, types<2>::forest *p4est, int save_data) = p4est_save;
    ...
    int (&functions<2>::connectivity_is_valid)(types<2>::connectivity *connectivity) = p4est_connectivity_is_valid;
    ...
    std::size_t (&functions<2>::forest_memory_used)(types<2>::forest *p4est) = p4est_memory_used;

    std::size_t (&functions<2>::connectivity_memory_used)(types<2>::connectivity *p4est) =
    ...
    void (&functions<2>::transfer_fixed)(const types<2>::gloidx *dest_gfq,
                                         ...
                                         void                   *dest_data,
                                         const void             *src_data,
                                         std::size_t             data_size) = p4est_transfer_fixed;

    types<2>::transfer_context *(&functions<2>::transfer_fixed_begin)(const types<2>::gloidx *dest_gfq,
                                                                      const types<2>::gloidx *src_gfq,
                                                                      MPI_Comm                mpicomm,
                                                                      int                     tag,
                                                                      void                   *dest_data,
    ...
    void (&functions<2>::transfer_fixed_end)(types<2>::transfer_context *tc) = p4est_transfer_fixed_end;
    ...
    types<2>::transfer_context *(&functions<2>::transfer_custom_begin)(const types<2>::gloidx *dest_gfq,
                                                                       const types<2>::gloidx *src_gfq,
                                                                       MPI_Comm                mpicomm,
                                                                       int                     tag,
                                                                       void                   *dest_data,
    ...
    void (&functions<2>::transfer_custom_end)(types<2>::transfer_context *tc) = p4est_transfer_custom_end;
...
#  endif
    ...
    void (&functions<2>::quadrant_coord_to_vertex)(types<2>::connectivity  *connectivity,
    ...
    int (&functions<3>::quadrant_overlaps_tree)(types<3>::tree           *tree,
                                                const types<3>::quadrant *q) = p8est_quadrant_overlaps_tree;

    void (&functions<3>::quadrant_set_morton)(types<3>::quadrant *quadrant,
                                              int                 level,
                                              std::uint64_t       id) = p8est_quadrant_set_morton;
    ...
    int (&functions<3>::quadrant_is_sibling)(const types<3>::quadrant *q1,
                                             const types<3>::quadrant *q2) = p8est_quadrant_is_sibling;

    int (&functions<3>::quadrant_is_ancestor)(const types<3>::quadrant *q1,
                                              const types<3>::quadrant *q2) = p8est_quadrant_is_ancestor;

    int (&functions<3>::quadrant_ancestor_id)(const types<3>::quadrant *q, int level) = p8est_quadrant_ancestor_id;
    ...
    types<3>::connectivity *(&functions<3>::connectivity_new_copy)(types<3>::topidx        num_vertices,
                                                                   types<3>::topidx        num_trees,
                                                                   types<3>::topidx        num_edges,
                                                                   types<3>::topidx        num_corners,
                                                                   const double           *vertices,
    ...
    void (&functions<3>::connectivity_destroy)(p8est_connectivity_t *connectivity) = p8est_connectivity_destroy;
    ...
    types<3>::forest *(&functions<3>::copy_forest)(types<3>::forest *input, int copy_data) = p8est_copy;
    ...
    void (&functions<3>::save)(const char *filename, types<3>::forest *p4est, int save_data) = p8est_save;
    ...
    int (&functions<3>::connectivity_is_valid)(types<3>::connectivity *connectivity) = p8est_connectivity_is_valid;
    ...
    std::size_t (&functions<3>::forest_memory_used)(types<3>::forest *p4est) = p8est_memory_used;

    std::size_t (&functions<3>::connectivity_memory_used)(types<3>::connectivity *p4est) =
    ...
    void (&functions<3>::transfer_fixed)(const types<3>::gloidx *dest_gfq,
                                         ...
                                         void                   *dest_data,
                                         const void             *src_data,
                                         std::size_t             data_size) = p8est_transfer_fixed;

    types<3>::transfer_context *(&functions<3>::transfer_fixed_begin)(const types<3>::gloidx *dest_gfq,
                                                                      const types<3>::gloidx *src_gfq,
                                                                      MPI_Comm                mpicomm,
                                                                      int                     tag,
                                                                      void                   *dest_data,
    ...
    void (&functions<3>::transfer_fixed_end)(types<3>::transfer_context *tc) = p8est_transfer_fixed_end;
    ...
    types<3>::transfer_context *(&functions<3>::transfer_custom_begin)(const types<3>::gloidx *dest_gfq,
                                                                       const types<3>::gloidx *src_gfq,
                                                                       MPI_Comm                mpicomm,
                                                                       int                     tag,
                                                                       void                   *dest_data,
    ...
    void (&functions<3>::transfer_custom_end)(types<3>::transfer_context *tc) = p8est_transfer_custom_end;
...
#  endif
    ...
    void (&functions<3>::quadrant_coord_to_vertex)(types<3>::connectivity  *connectivity,
    ...
    template <int dim>
    bool
    quadrant_is_equal(const typename types<dim>::quadrant &q1, const typename types<dim>::quadrant &q2)
    {
    ...
    template <int dim>
    bool
    quadrant_is_ancestor(const typename types<dim>::quadrant &q1, const typename types<dim>::quadrant &q2)
    {
    ...
    {
      return functions<2>::connectivity_new_copy(connectivity->num_vertices,
    ...
    {
      return functions<3>::connectivity_new_copy(connectivity->num_vertices,
                                                 connectivity->num_trees,
    ...
    template <>
    bool
    quadrant_is_equal<1>(const typename types<1>::quadrant &q1, const typename types<1>::quadrant &q2)
    {
      return q1 == q2;
    }
    ...
    template <>
    bool
    quadrant_is_ancestor<1>(const types<1>::quadrant &q1, const types<1>::quadrant &q2)
    {
      // determine level of quadrants
      const int level_1 = (q1 << types<1>::max_n_child_indices_bits) >> types<1>::max_n_child_indices_bits;
      const int level_2 = (q2 << types<1>::max_n_child_indices_bits) >> types<1>::max_n_child_indices_bits;
      ...
      const int truncated_id_1 = (q1 >> (types<1>::n_bits - 1 - level_1)) << (types<1>::n_bits - 1 - level_1);
      const int truncated_id_2 = (q2 >> (types<1>::n_bits - 1 - level_1)) << (types<1>::n_bits - 1 - level_1);
    ...
    init_quadrant_children<1>(
      ...
      typename types<1>::quadrant (&p4est_children)[dealii::GeometryInfo<1>::max_children_per_cell])
    {
      // determine the current level of quadrant
      const int level_parent = (q << types<1>::max_n_child_indices_bits) >> types<1>::max_n_child_indices_bits;

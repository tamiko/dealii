...
namespace
{
  ...
  get_vertex_to_cell_mappings(
    ...
    std::vector<std::list<std::pair<typename Triangulation<dim, spacedim>::active_cell_iterator, unsigned int>>>
      &vertex_to_cell)
  {
  ...
  set_vertex_and_cell_info(
    ...
    const std::vector<std::list<std::pair<typename Triangulation<dim, spacedim>::active_cell_iterator, unsigned int>>>
                                                       &vertex_to_cell,
    ...
    typename internal::p4est::types<dim>::connectivity *connectivity)
  {
    ...
    Assert(triangulation.get_used_vertices().size() == triangulation.get_vertices().size(), ExcInternalError());
    Assert(std::find(triangulation.get_used_vertices().begin(), triangulation.get_used_vertices().end(), false) ==
      ...
      for (unsigned int v = 0; v < triangulation.n_vertices(); ++v)
        {
          ...
          connectivity->vertices[3 * v + 2] = (spacedim == 2 ? 0 : triangulation.get_vertices()[v][2]);
        }
                  ...
                  case 3:
                    {
                      ...
                      typename Triangulation<dim>::active_cell_iterator cell_list[2] = {cell, cell->neighbor(f)};
                      ...
                      unsigned int g_idx =
                        cell_list[smaller_idx]->vertex_index(GeometryInfo<dim>::face_to_cell_vertices(
    ...
    std::partial_sum(vertex_touch_count.begin(), vertex_touch_count.end(), &connectivity->ctt_offset[1]);
    ...
    for (unsigned int v = 0; v < triangulation.n_vertices(); ++v)
      {
        ...
        typename std::list<
          std::pair<typename Triangulation<dim, spacedim>::active_cell_iterator, unsigned int>>::const_iterator p =
                                           ...
                                           const std::vector<std::vector<bool>>                       &marked_vertices)
  {
    if (dealii_cell->level_subdomain_id() == numbers::artificial_subdomain_id)
      {
        ...
        // |   | M |
        // *---*---*
        if (!used && dealii_cell->is_active() && dealii_cell->is_artificial() == false &&
        ...
        if (!used && dealii_cell->is_active() && dealii_cell->is_artificial() == false && dealii_cell->level() > 0)
          {
            for (const unsigned int v : GeometryInfo<dim>::vertex_indices())
              {
                if (marked_vertices[dealii_cell->level() - 1][dealii_cell->vertex_index(v)])
        ...
        if (used)
          {
            int owner =
              internal::p4est::functions<dim>::comm_find_owner(&forest, tree_index, &p4est_cell, my_subdomain);
    ...
    if (dealii_cell->has_children())
      {
        typename internal::p4est::types<dim>::quadrant p4est_child[GeometryInfo<dim>::max_children_per_cell];
        ...
        for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; ++c)
          {
            determine_level_subdomain_id_recursively<dim, spacedim>(
              tree, tree_index, dealii_cell->child(c), p4est_child[c], forest, my_subdomain, marked_vertices);
          }
      }
  }
        ...
        else
          {
            ...
            for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; ++c)
              if (internal::p4est::functions<dim>::quadrant_overlaps_tree(
                    const_cast<typename internal::p4est::types<dim>::tree *>(&tree), &p4est_child[c]) == false)
                {
                  ...
                  dealii_cell->child(c)->recursively_set_subdomain_id(numbers::artificial_subdomain_id);
                }
              else
                {
...
#  ifdef P4EST_SEARCH_LOCAL
  ...
  private:
    ...
    public:
      ...
      void
      set_cell_vertices(typename internal::p4est::types<dim>::forest              *forest,
                        typename internal::p4est::types<dim>::topidx               which_tree,
                        typename internal::p4est::types<dim>::quadrant            *quadrant,
                        const typename internal::p4est::types<dim>::quadrant_coord quad_length_on_level);
  ...
  template <int dim>
  int
  PartitionSearch<dim>::local_quadrant_fn(typename internal::p4est::types<dim>::forest   *forest,
                                          typename internal::p4est::types<dim>::topidx    which_tree,
                                          typename internal::p4est::types<dim>::quadrant *quadrant,
                                          int /* rank_begin */,
                                          int /* rank_end */,
                                          void * /* this is always nullptr */ point)
  {
    ...
    PartitionSearch<dim> *this_object = reinterpret_cast<PartitionSearch<dim> *>(forest->user_pointer);
    ...
    const typename internal::p4est::types<dim>::quadrant_coord quad_length_on_level =
      1 << (static_cast<typename internal::p4est::types<dim>::quadrant_coord>(
            ...
            static_cast<typename internal::p4est::types<dim>::quadrant_coord>(quadrant->level));

    this_object->quadrant_data.set_cell_vertices(forest, which_tree, quadrant, quad_length_on_level);
  ...
  template <int dim>
  int
  PartitionSearch<dim>::local_point_fn(typename internal::p4est::types<dim>::forest *forest,
                                       typename internal::p4est::types<dim>::topidx /* which_tree */,
                                       typename internal::p4est::types<dim>::quadrant * /* quadrant */,
                                       int   rank_begin,
                                       int   rank_end,
                                       void *point)
  {
    ...
    Point<dim> this_point = (dim == 2 ? Point<dim>(this_point_dptr[0], this_point_dptr[1]) :
                                        Point<dim>(this_point_dptr[0], this_point_dptr[1], this_point_dptr[2]));

    // use reference mapping to decide whether this point is in this quadrant
    const bool is_in_this_quadrant = this_object->quadrant_data.is_in_this_quadrant(this_point);
  ...
  template <int dim>
  bool
  PartitionSearch<dim>::QuadrantData::is_in_this_quadrant(const Point<dim> &p) const
  {
  ...
  template <int dim>
  Point<dim>
  PartitionSearch<dim>::QuadrantData::map_real_to_unit_cell(const Point<dim> &p) const
  {
    Assert(is_reference_mapping_initialized,
           dealii::ExcMessage("Cell vertices and mapping coefficients must be fully "
    ...
    if (dim == 2)
      {
        for (unsigned int alpha = 0; alpha < GeometryInfo<dim>::vertices_per_cell; ++alpha)
          {
            const Point<dim> &p_ref = GeometryInfo<dim>::unit_cell_vertex(alpha);

            p_out += (quadrant_mapping_matrix(alpha, 0) + quadrant_mapping_matrix(alpha, 1) * p(0) +
                      quadrant_mapping_matrix(alpha, 2) * p(1) + quadrant_mapping_matrix(alpha, 3) * p(0) * p(1)) *
                     p_ref;
          }
      }
    else
        ...
        for (unsigned int alpha = 0; alpha < GeometryInfo<dim>::vertices_per_cell; ++alpha)
          {
            ...
            p_out +=
              (quadrant_mapping_matrix(alpha, 0) + quadrant_mapping_matrix(alpha, 1) * p(0) +
               quadrant_mapping_matrix(alpha, 2) * p(1) + quadrant_mapping_matrix(alpha, 3) * p(2) +
               quadrant_mapping_matrix(alpha, 4) * p(0) * p(1) + quadrant_mapping_matrix(alpha, 5) * p(1) * p(2) +
  ...
  PartitionSearch<dim>::QuadrantData::QuadrantData()
    ...
    , quadrant_mapping_matrix(GeometryInfo<dim>::vertices_per_cell, GeometryInfo<dim>::vertices_per_cell)
  ...
  {
    Assert(are_vertices_initialized,
           dealii::ExcMessage("Cell vertices must be initialized before the cell mapping can be filled."));

    FullMatrix<double> point_matrix(GeometryInfo<dim>::vertices_per_cell, GeometryInfo<dim>::vertices_per_cell);
        ...
        for (unsigned int alpha = 0; alpha < GeometryInfo<dim>::vertices_per_cell; ++alpha)
          {
            ...
            point_matrix(3, alpha) = cell_vertices[alpha](0) * cell_vertices[alpha](1);
          }
        ...
        for (unsigned int alpha = 0; alpha < GeometryInfo<dim>::vertices_per_cell; ++alpha)
          {
            ...
            point_matrix(4, alpha) = cell_vertices[alpha](0) * cell_vertices[alpha](1);
            point_matrix(5, alpha) = cell_vertices[alpha](1) * cell_vertices[alpha](2);
            point_matrix(6, alpha) = cell_vertices[alpha](0) * cell_vertices[alpha](2);
            point_matrix(7, alpha) = cell_vertices[alpha](0) * cell_vertices[alpha](1) * cell_vertices[alpha](2);
          }
  ...
  PartitionSearch<2>::QuadrantData::set_cell_vertices(
    ...
    const typename internal::p4est::types<2>::quadrant_coord quad_length_on_level)
  {
    ...
    vertex_index = 1;
    internal::p4est::functions<dim>::quadrant_coord_to_vertex(
      forest->connectivity, which_tree, quadrant->x + quad_length_on_level, quadrant->y, corner_point);
    ...
    vertex_index = 2;
    internal::p4est::functions<dim>::quadrant_coord_to_vertex(
      forest->connectivity, which_tree, quadrant->x, quadrant->y + quad_length_on_level, corner_point);
     ...
     */
    vertex_index = 3;
    internal::p4est::functions<dim>::quadrant_coord_to_vertex(forest->connectivity,
                                                              which_tree,
  ...
  PartitionSearch<3>::QuadrantData::set_cell_vertices(
    ...
    const typename internal::p4est::types<3>::quadrant_coord quad_length_on_level)
  {
    ...
    unsigned int vertex_index = 0;
    internal::p4est::functions<dim>::quadrant_coord_to_vertex(
      forest->connectivity, which_tree, quadrant->x, quadrant->y, quadrant->z, corner_point);
    ...
    vertex_index = 1;
    internal::p4est::functions<dim>::quadrant_coord_to_vertex(
      forest->connectivity, which_tree, quadrant->x + quad_length_on_level, quadrant->y, quadrant->z, corner_point);
    ...
    vertex_index = 2;
    internal::p4est::functions<dim>::quadrant_coord_to_vertex(
      forest->connectivity, which_tree, quadrant->x, quadrant->y + quad_length_on_level, quadrant->z, corner_point);
    ...
    vertex_index = 4;
    internal::p4est::functions<dim>::quadrant_coord_to_vertex(
      forest->connectivity, which_tree, quadrant->x, quadrant->y, quadrant->z + quad_length_on_level, corner_point);
...
#  endif //  P4EST_SEARCH_LOCAL defined
  ...
  {
    return ((current_refine_pointer == refine_list.end()) && (current_coarsen_pointer == coarsen_list.end()));
  }
    ...
    for (unsigned int c = 0; c < triangulation.n_cells(0); ++c)
      {
        ...
        const typename Triangulation<dim, spacedim>::cell_iterator cell(&triangulation, 0, coarse_cell_index);
  ...
  template <int dim, int spacedim>
  void
  RefineAndCoarsenList<dim, spacedim>::build_lists(const typename Triangulation<dim, spacedim>::cell_iterator &cell,
                                                   const typename internal::p4est::types<dim>::quadrant &p4est_cell,
                                                   const types::subdomain_id                             my_subdomain)
  {
    if (cell->is_active())
  ...
  template <int dim, int spacedim>
  int
  RefineAndCoarsenList<dim, spacedim>::refine_callback(typename internal::p4est::types<dim>::forest *forest,
                                                       typename internal::p4est::types<dim>::topidx  coarse_cell_index,
                                                       typename internal::p4est::types<dim>::quadrant *quadrant)
  {
    ...
    Assert(coarse_cell_index <= this_object->current_refine_pointer->p.which_tree, ExcInternalError());
    ...
    // pointer
    Assert(internal::p4est::functions<dim>::quadrant_compare(quadrant, &*this_object->current_refine_pointer) <= 0,
           ExcInternalError());

    // now, if the p4est cell is one in the list, it is supposed to be refined
    if (internal::p4est::functions<dim>::quadrant_is_equal(quadrant, &*this_object->current_refine_pointer))
      {
  ...
  template <int dim, int spacedim>
  int
  RefineAndCoarsenList<dim, spacedim>::coarsen_callback(typename internal::p4est::types<dim>::forest *forest,
                                                        typename internal::p4est::types<dim>::topidx  coarse_cell_index,
                                                        typename internal::p4est::types<dim>::quadrant *children[])
  {
    RefineAndCoarsenList<dim, spacedim> *this_object =
    ...
    Assert(coarse_cell_index <= this_object->current_coarsen_pointer->p.which_tree, ExcInternalError());
    ...
    // make sure that the p4est loop over cells hasn't gotten ahead of our own
    // pointer
    Assert(internal::p4est::functions<dim>::quadrant_compare(children[0], &*this_object->current_coarsen_pointer) <= 0,
           ExcInternalError());

    // now, if the p4est cell is one in the list, it is supposed to be
    // coarsened
    if (internal::p4est::functions<dim>::quadrant_is_equal(children[0], &*this_object->current_coarsen_pointer))
      {
        ...
        for (unsigned int c = 1; c < GeometryInfo<dim>::max_children_per_cell; ++c)
          {
            Assert(internal::p4est::functions<dim>::quadrant_is_equal(children[c],
                                                                      &*this_object->current_coarsen_pointer),
                   ExcInternalError());
            ++this_object->current_coarsen_pointer;
          }
                                               ...
                                               typename internal::p4est::types<dim>::quadrant *)
  {
    ...
    Assert(this_object->current_pointer >= this_object->cell_weights_list.begin(), ExcInternalError());
  ...
  template <int dim, int spacedim>
  using cell_relation_t = typename std::pair<typename dealii::Triangulation<dim, spacedim>::cell_iterator, CellStatus>;
  ...
  template <int dim, int spacedim>
  inline void
  add_single_cell_relation(std::vector<cell_relation_t<dim, spacedim>>                &cell_rel,
  ...
  template <int dim, int spacedim>
  void
  update_cell_relations_recursively(std::vector<cell_relation_t<dim, spacedim>>                  &cell_rel,
                                    const typename dealii::internal::p4est::types<dim>::tree     &tree,
                                    const typename Triangulation<dim, spacedim>::cell_iterator   &dealii_cell,
                                    const typename dealii::internal::p4est::types<dim>::quadrant &p4est_cell)
  {
    ...
    if (idx == -1 && (dealii::internal::p4est::functions<dim>::quadrant_overlaps_tree(
                        const_cast<typename dealii::internal::p4est::types<dim>::tree *>(&tree), &p4est_cell) == false))
    ...
    if (p4est_has_children && dealii_cell->has_children())
      {
        // recurse further
        typename dealii::internal::p4est::types<dim>::quadrant p4est_child[GeometryInfo<dim>::max_children_per_cell];
        ...
        dealii::internal::p4est::functions<dim>::quadrant_childrenv(&p4est_cell, p4est_child);

        for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; ++c)
          {
            update_cell_relations_recursively<dim, spacedim>(cell_rel, tree, dealii_cell->child(c), p4est_child[c]);
          }
      }
    else if (!p4est_has_children && !dealii_cell->has_children())
      {
        ...
        add_single_cell_relation<dim, spacedim>(cell_rel, tree, idx, dealii_cell, CellStatus::cell_will_persist);
      }
                                 ...
                                 // dealii_cell has no children
      {
        ...
        for (unsigned int i = 0; i < GeometryInfo<dim>::max_children_per_cell; ++i)
          {
            child_idx = sc_array_bsearch(const_cast<sc_array_t *>(&tree.quadrants),
                                         &p4est_child[i],
                                         dealii::internal::p4est::functions<dim>::quadrant_compare);

            cell_status = (i == 0) ? CellStatus::cell_will_be_refined : CellStatus::cell_invalid;

            add_single_cell_relation<dim, spacedim>(cell_rel, tree, child_idx, dealii_cell, cell_status);
          }
      }
         ...
         // children, and the dealii_cell does
      {
        ...
        add_single_cell_relation<dim, spacedim>(
          cell_rel, tree, idx, dealii_cell, CellStatus::children_will_be_coarsened);
      }
  }
} // namespace
...
namespace parallel
{
  namespace distributed
  {
                                                            ...
                                                            const SubCellData                  &subcelldata)
    {
      ...
      Assert(this->all_reference_cells_are_hyper_cube(),
             ExcMessage("The class parallel::distributed::Triangulation only supports meshes "
    ...
    {
      return settings & Triangulation<dim, spacedim>::construct_multigrid_hierarchy;
    }
    ...
    {
      return settings & Triangulation<dim, spacedim>::communicate_vertices_to_p4est;
    }
    ...
    void Triangulation<dim, spacedim>::execute_transfer(
      const typename dealii::internal::p4est::types<dim>::forest *parallel_forest,
      const typename dealii::internal::p4est::types<dim>::gloidx *previous_global_first_quadrant)
    {
      Assert(this->data_serializer.sizes_fixed_cumulative.size() > 0, ExcMessage("No data has been packed!"));

      // Resize memory according to the data that we will receive.
      this->data_serializer.dest_data_fixed.resize(parallel_forest->local_num_quadrants *
                                                   this->data_serializer.sizes_fixed_cumulative.back());

      // Execute non-blocking fixed size transfer.
      typename dealii::internal::p4est::types<dim>::transfer_context *tf_context;
      tf_context = dealii::internal::p4est::functions<dim>::transfer_fixed_begin(
      ...
      if (this->data_serializer.variable_size_data_stored)
        {
          // Resize memory according to the data that we will receive.
          this->data_serializer.dest_sizes_variable.resize(parallel_forest->local_num_quadrants);

          // Execute fixed size transfer of data sizes for variable size
          // transfer.
          dealii::internal::p4est::functions<dim>::transfer_fixed(parallel_forest->global_first_quadrant,
                                                                  previous_global_first_quadrant,
                                                                  parallel_forest->mpicomm,
                                                                  1,
...
#  endif
          ...
          dealii::internal::p4est::functions<dim>::transfer_custom(parallel_forest->global_first_quadrant,
                                                                   previous_global_first_quadrant,
                                                                   parallel_forest->mpicomm,
                                                                   1,
    ...
    {
      Assert(parallel_forest != nullptr, ExcMessage("Can't produce output when no forest is created yet."));

      AssertThrow(are_vertices_communicated_to_p4est(),
                  ExcMessage("To use this function the triangulation's flag "
                             "Settings::communicate_vertices_to_p4est must be set."));

      dealii::internal::p4est::functions<dim>::vtk_write_file(parallel_forest, nullptr, file_basename.c_str());
    }
    ...
    {
      Assert(this->cell_attached_data.n_attached_deserialize == 0,
             ExcMessage("Not all SolutionTransfer objects have been deserialized after the last call to load()."));
      Assert(this->n_cells() > 0, ExcMessage("Can not save() an empty Triangulation."));

      const int myrank = Utilities::MPI::this_mpi_process(this->mpi_communicator);
      ...
      if (this->my_subdomain == 0)
        {
          ...
          f << "version nproc n_attached_fixed_size_objs n_attached_variable_size_objs n_coarse_cells" << std::endl
            << 5 << " " << Utilities::MPI::n_mpi_processes(this->mpi_communicator) << " "
            << this->cell_attached_data.pack_callbacks_fixed.size() << " "
            << this->cell_attached_data.pack_callbacks_variable.size() << " " << this->n_cells(0) << std::endl;
        }
      ...
      dealii::internal::p4est::functions<dim>::save(filename.c_str(), parallel_forest, false);
    ...
    void Triangulation<dim, spacedim>::load(const std::string &filename)
    {
      ...
      unsigned int version, numcpus, attached_count_fixed, attached_count_variable, n_coarse_cells;
      {
        std::string   fname = std::string(filename) + ".info";
        ...
        f >> version >> numcpus >> attached_count_fixed >> attached_count_variable >> n_coarse_cells;
      }

      AssertThrow(version == 5, ExcMessage("Incompatible version found in .info file."));
      ...
      this->cell_attached_data.n_attached_deserialize = attached_count_fixed + attached_count_variable;

      parallel_forest = dealii::internal::p4est::functions<dim>::load_ext(
        filename.c_str(), this->mpi_communicator, 0, 0, 1, 0, this, &connectivity);
    ...
    template <int dim, int spacedim>
    DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
    void Triangulation<dim, spacedim>::load(const typename dealii::internal::p4est::types<dim>::forest *forest)
    {
      Assert(this->n_cells() > 0,
             ExcMessage("load() only works if the Triangulation already contains "
                        "a coarse mesh!"));
      Assert(this->n_cells() == forest->trees->elem_count,
             ExcMessage("Coarse mesh of the Triangulation does not match the one "
      ...
      parallel_forest               = dealii::internal::p4est::functions<dim>::copy_forest(temp, false);
    ...
    {
      Assert(parallel_forest != nullptr, ExcMessage("Can't produce a check sum when no forest is created yet."));
    ...
    template <int dim, int spacedim>
    DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
    const typename dealii::internal::p4est::types<dim>::forest *Triangulation<dim, spacedim>::get_p4est() const
    {
      Assert(parallel_forest != nullptr, ExcMessage("The forest has not been allocated yet."));
    ...
    template <int dim, int spacedim>
    DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
    typename dealii::internal::p4est::types<dim>::tree *Triangulation<dim, spacedim>::init_tree(
      const int dealii_coarse_cell_index) const
    {
      const unsigned int tree_index = coarse_cell_to_p4est_tree_permutation[dealii_coarse_cell_index];
...
#  ifndef DOXYGEN
    ...
    Triangulation<2, 2>::copy_new_triangulation_to_p4est(std::integral_constant<int, 2>)
    {
      ...
      // it)
      std::vector<unsigned int> vertex_touch_count;
      std::vector<std::list<std::pair<Triangulation<dim, spacedim>::active_cell_iterator, unsigned int>>>
        vertex_to_cell;
      ...
      connectivity = dealii::internal::p4est::functions<2>::connectivity_new(
        (set_vertex_info == true ? this->n_vertices() : 0), this->n_cells(0), this->n_vertices(), num_vtt);
      ...
      parallel_forest =
        dealii::internal::p4est::functions<2>::new_forest(this->mpi_communicator,
                                                          connectivity,
    ...
    Triangulation<3, 3>::copy_new_triangulation_to_p4est(std::integral_constant<int, 3>)
    {
      ...
      // it)
      std::vector<unsigned int>                                                               vertex_touch_count;
      std::vector<std::list<std::pair<Triangulation<3>::active_cell_iterator, unsigned int>>> vertex_to_cell;
      get_vertex_to_cell_mappings(*this, vertex_touch_count, vertex_to_cell);
      const dealii::internal::p4est::types<2>::locidx num_vtt =
        std::accumulate(vertex_touch_count.begin(), vertex_touch_count.end(), 0u);

      std::vector<unsigned int>                                                               edge_touch_count;
      std::vector<std::list<std::pair<Triangulation<3>::active_cell_iterator, unsigned int>>> edge_to_cell;
      ...
      connectivity =
        dealii::internal::p4est::functions<3>::connectivity_new((set_vertex_info == true ? this->n_vertices() : 0),
                                                                this->n_cells(0),
                                                                this->n_active_lines(),
                                                                num_ett,
                                                                this->n_vertices(),
      ...
      for (Triangulation<dim, spacedim>::active_cell_iterator cell = this->begin_active(); cell != this->end(); ++cell)
        {
          const unsigned int index = coarse_cell_to_p4est_tree_permutation[cell->index()];
          for (unsigned int e = 0; e < GeometryInfo<3>::lines_per_cell; ++e)
            connectivity->tree_to_edge[index * GeometryInfo<3>::lines_per_cell + deal_to_p4est_line_index[e]] =
              cell->line(e)->index();
        }
      ...
      std::partial_sum(edge_touch_count.begin(), edge_touch_count.end(), &connectivity->ett_offset[1]);
      ...
      for (unsigned int v = 0; v < this->n_active_lines(); ++v)
        {
          ...
          std::list<std::pair<Triangulation<dim, spacedim>::active_cell_iterator, unsigned int>>::const_iterator p =
            edge_to_cell[v].begin();
          for (unsigned int c = 0; c < edge_touch_count[v]; ++c, ++p)
            {
              connectivity->edge_to_tree[connectivity->ett_offset[v] + c] =
                coarse_cell_to_p4est_tree_permutation[p->first->index()];
              connectivity->edge_to_edge[connectivity->ett_offset[v] + c] = deal_to_p4est_line_index[p->second];
            }
        }
      ...
      // now create a forest out of the connectivity data structure
      parallel_forest =
        dealii::internal::p4est::functions<3>::new_forest(this->mpi_communicator,
                                                          connectivity,
                                                          /* minimum initial number of quadrants per tree */ 0,
                                                          /* minimum level of upfront refinement */ 0,
                                                          /* use uniform upfront refinement */ 1,
...
#  endif
                ...
                for (unsigned int v = 0; v < GeometryInfo<dim - 1>::vertices_per_cell; ++v)
                  {
                    ...
                    // cell[0] into account
                    const unsigned int vface0 = GeometryInfo<dim>::standard_to_real_face_vertex(v,
                                                                                                face_orientation[0],
                                                                                                face_orientation[1],
                                                                                                face_orientation[2]);
                    ...
                    topological_vertex_numbering[cell_1->face(face_no_1)->vertex_index(vface0)] =
                      topological_vertex_numbering[cell_2->face(face_no_2)->vertex_index(v)] = min_index;
                  }
              }
          }
        ...
        while (continue_iterating)
          {
            ...
            for (; cell != endc; ++cell)
              {
                if (cell->refine_flag_set())
                  ...
                  for (const unsigned int vertex : GeometryInfo<dim>::vertex_indices())
                    vertex_level[topological_vertex_numbering[cell->vertex_index(vertex)]] =
                      std::max(vertex_level[topological_vertex_numbering[cell->vertex_index(vertex)]], cell->level());
                else
                  {
            ...
            for (cell = tria.last_active(); cell != endc; --cell)
              if (cell->refine_flag_set() == false)
                {
                  for (const unsigned int vertex : GeometryInfo<dim>::vertex_indices())
                    if (vertex_level[topological_vertex_numbering[cell->vertex_index(vertex)]] >= cell->level() + 1)
                      {
                        // remove coarsen flag...
                        cell->clear_coarsen_flag();
            ...
            for (const auto &cell : tria.cell_iterators())
              {
                ...
                for (unsigned int child = 0; child < n_children; ++child)
                  if (cell->child(child)->is_active() && cell->child(child)->coarsen_flag_set())
                    ++flagged_children;
    ...
    bool Triangulation<dim, spacedim>::prepare_coarsening_and_refinement()
    {
      ...
      do
        {
          n_changes += this->dealii::Triangulation<dim, spacedim>::prepare_coarsening_and_refinement();
          this->update_periodic_face_map();
          ...
          // message.
          ++loop_counter;
          AssertThrow(loop_counter < 32,
                      ExcMessage("Infinite loop in "
    ...
    void Triangulation<dim, spacedim>::copy_local_forest_to_triangulation()
    {
        ...
        while (this->n_levels() > 1)
          {
            ...
            for (const auto &cell : this->active_cell_iterators_on_level(this->n_levels() - 1))
              {
                cell->set_coarsen_flag();
              }
            try
      ...
      parallel_ghost = dealii::internal::p4est::functions<dim>::ghost_new(
        parallel_forest,
        (dim == 2 ? typename dealii::internal::p4est::types<dim>::balance_type(P4EST_CONNECT_CORNER) :
      ...
      do
        {
          ...
          for (unsigned int g_idx = 0; g_idx < parallel_ghost->ghosts.elem_count; ++g_idx)
            {
              while (g_idx >= static_cast<unsigned int>(parallel_ghost->proc_offsets[ghost_owner + 1]))
                ++ghost_owner;
              while (g_idx >= static_cast<unsigned int>(parallel_ghost->tree_offsets[ghost_tree + 1]))
                ++ghost_tree;
          ...
          mesh_changed = std::any_of(this->begin_active(),
                                     active_cell_iterator{this->end()},
                                     [](const CellAccessor<dim, spacedim> &cell) {
                                       return cell.refine_flag_set() || cell.coarsen_flag_set();
                                     });
...
#  ifdef DEBUG
      ...
      for (const auto &cell : this->active_cell_iterators())
        {
          if (cell->subdomain_id() != this->my_subdomain && cell->subdomain_id() != numbers::artificial_subdomain_id)
            ++num_ghosts;
        }
...
#  endif
          ...
          for (unsigned int lvl = this->n_levels(); lvl > 0;)
            {
              ...
              for (const auto &cell : this->cell_iterators_on_level(lvl))
                {
                  if ((cell->is_active() && cell->subdomain_id() == this->locally_owned_subdomain()) ||
          ...
          for (const auto &cell : this->cell_iterators_on_level(0))
            {
              ...
              determine_level_subdomain_id_recursively<dim, spacedim>(
                *tree, tree_index, cell, p4est_coarse_cell, *parallel_forest, this->my_subdomain, marked_vertices);
            }
      ...
      // many non-artificial cells as parallel_forest->local_num_quadrants)
      {
        ...
        if (Utilities::MPI::n_mpi_processes(this->mpi_communicator) == 1)
          {
            Assert(static_cast<unsigned int>(parallel_forest->local_num_quadrants) == total_local_cells,
                   ExcInternalError());
          }
        else
          {
...
#  ifdef DEBUG
        ...
        Assert(static_cast<unsigned int>(parallel_forest->local_num_quadrants) == n_owned, ExcInternalError());
#  endif
      }
    ...
    template <int dim, int spacedim>
    DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
    std::vector<types::subdomain_id> Triangulation<dim, spacedim>::find_point_owner_rank(
      const std::vector<Point<dim>> &points)
    {
#  ifndef P4EST_SEARCH_LOCAL
      (void)points;
      AssertThrow(false, ExcMessage("This function is only available if p4est is version 2.2 and higher."));
      // Just return to satisfy compiler
      return std::vector<unsigned int>(1, dealii::numbers::invalid_subdomain_id);
#  else
      // We can only use this function if vertices are communicated to p4est
      AssertThrow(this->are_vertices_communicated_to_p4est(),
                  ExcMessage("Vertices need to be communicated to p4est to use this "
      ...
      for (const auto &manifold_id : this->get_manifold_ids())
        {
          AssertThrow(manifold_id == numbers::flat_manifold_id,
                      ExcMessage("This function can only be used if the triangulation "
      ...
      point_sc_array = sc_array_new_count(sizeof(double[dim + 1]), points.size());
      ...
      for (size_t i = 0; i < points.size(); ++i)
        {
          ...
          double *this_sc_point = static_cast<double *>(sc_array_index_ssize_t(point_sc_array, i));
      ...
      dealii::internal::p4est::functions<dim>::search_partition(parallel_forest,
      ...
      std::vector<types::subdomain_id> owner_rank(points.size(), numbers::invalid_subdomain_id);
...
#  endif
      ...
      // much memory
      RefineAndCoarsenList<dim, spacedim> refine_and_coarsen_list(*this,
      ...
      dealii::internal::p4est::functions<dim>::refine(parallel_forest,
                                                      /* refine_recursive */ false,
                                                      &RefineAndCoarsenList<dim, spacedim>::refine_callback,
                                                      /*init_callback=*/nullptr);
      dealii::internal::p4est::functions<dim>::coarsen(parallel_forest,
      ...
      std::vector<typename dealii::internal::p4est::types<dim>::gloidx> previous_global_first_quadrant;
      ...
      if (this->cell_attached_data.n_attached_data_sets > 0)
        {
          ...
          std::memcpy(previous_global_first_quadrant.data(),
                      parallel_forest->global_first_quadrant,
                      sizeof(typename dealii::internal::p4est::types<dim>::gloidx) * (parallel_forest->mpisize + 1));
        }
      ...
      if (!(settings & no_automatic_repartitioning))
        {
          ...
          if (this->signals.weight.empty())
          ...
          else
            {
              ...
              Assert(Utilities::MPI::sum(std::accumulate(cell_weights.begin(), cell_weights.end(), std::uint64_t(0)),
      ...
      if (this->cell_attached_data.n_attached_data_sets > 0)
        {
          this->data_serializer.pack_data(this->local_cell_relations,
      ...
      if (this->cell_attached_data.n_attached_data_sets > 0)
        {
          this->execute_transfer(parallel_forest, previous_global_first_quadrant.data());
          ...
          for (unsigned int lvl = 0; lvl < this->n_global_levels(); ++lvl)
            {
              ...
              for (; cell != endc; ++cell)
                if (cell->level() == static_cast<int>(lvl) || cell->is_active())
                  {
                    const bool is_level_artificial = (cell->level_subdomain_id() == numbers::artificial_subdomain_id);
...
#  endif
    ...
    template <int dim, int spacedim>
    DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
    unsigned int Triangulation<dim, spacedim>::coarse_cell_id_to_coarse_cell_index(
      ...
      const std::vector<dealii::GridTools::PeriodicFacePair<cell_iterator>> &periodicity_vector)
    {
      ...
      for (const auto &face_pair : periodicity_vector)
        {
          ...
          const unsigned int tree_left  = coarse_cell_to_p4est_tree_permutation[first_cell->index()];
          const unsigned int tree_right = coarse_cell_to_p4est_tree_permutation[second_cell->index()];
          ...
          if (dim == 2)
          ...
          else
            {
              ...
              for (unsigned int i = 0; i < GeometryInfo<dim>::vertices_per_face; ++i)
                {
                  const unsigned int first_dealii_idx_on_cell = GeometryInfo<dim>::face_to_cell_vertices(
              ...
              const unsigned int     second_dealii_idx_on_face =
                lower_idx == 0 ? left_to_right[face_pair.orientation.to_ulong()][first_dealii_idx_on_face] :
                                     right_to_left[face_pair.orientation.to_ulong()][first_dealii_idx_on_face];
              const unsigned int second_dealii_idx_on_cell = GeometryInfo<dim>::face_to_cell_vertices(
      ...
      Assert(dealii::internal::p4est::functions<dim>::connectivity_is_valid(connectivity) == 1, ExcInternalError());

      // now create a forest out of the connectivity data structure
      dealii::internal::p4est::functions<dim>::destroy(parallel_forest);
      parallel_forest =
        dealii::internal::p4est::functions<dim>::new_forest(this->mpi_communicator,
                                                            connectivity,
                                                            /* minimum initial number of quadrants per tree */ 0,
                                                            /* minimum level of upfront refinement */ 0,
                                                            /* use uniform upfront refinement */ 1,
    ...
    {
      std::size_t mem =
        this->dealii::parallel::TriangulationBase<dim, spacedim>::memory_consumption() +
        MemoryConsumption::memory_consumption(triangulation_has_content) +
        MemoryConsumption::memory_consumption(connectivity) + MemoryConsumption::memory_consumption(parallel_forest) +
        MemoryConsumption::memory_consumption(this->cell_attached_data.n_attached_data_sets) +
        ...
        MemoryConsumption::memory_consumption(p4est_tree_to_coarse_cell_permutation) + memory_consumption_p4est();
    ...
    template <int dim, int spacedim>
    DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
    void Triangulation<dim, spacedim>::copy_triangulation(const dealii::Triangulation<dim, spacedim> &other_tria)
    {
      Assert((dynamic_cast<const dealii::parallel::distributed::Triangulation<dim, spacedim> *>(&other_tria)) ||
      ...
      try
        {
          dealii::parallel::TriangulationBase<dim, spacedim>::copy_triangulation(other_tria);
        }
      catch (const typename dealii::Triangulation<dim, spacedim>::DistortedCellList &)
        {
          // the underlying triangulation should not be checking for distorted
      ...
      if (const dealii::parallel::distributed::Triangulation<dim, spacedim> *other_distributed =
            dynamic_cast<const dealii::parallel::distributed::Triangulation<dim, spacedim> *>(&other_tria))
        {
          // copy parallel distributed specifics
          settings                              = other_distributed->settings;
          triangulation_has_content             = other_distributed->triangulation_has_content;
          coarse_cell_to_p4est_tree_permutation = other_distributed->coarse_cell_to_p4est_tree_permutation;
          p4est_tree_to_coarse_cell_permutation = other_distributed->p4est_tree_to_coarse_cell_permutation;

          // create deep copy of connectivity graph
          typename dealii::internal::p4est::types<dim>::connectivity *temp_connectivity =
            const_cast<typename dealii::internal::p4est::types<dim>::connectivity *>(other_distributed->connectivity);
          connectivity = dealii::internal::p4est::copy_connectivity<dim>(temp_connectivity);

          // create deep copy of parallel forest
          typename dealii::internal::p4est::types<dim>::forest *temp_forest =
            const_cast<typename dealii::internal::p4est::types<dim>::forest *>(other_distributed->parallel_forest);
          parallel_forest               = dealii::internal::p4est::functions<dim>::copy_forest(temp_forest, false);
          parallel_forest->connectivity = connectivity;
          parallel_forest->user_pointer = this;
        }
      else
      ...
      for (const auto &cell : this->cell_iterators_on_level(0))
        {
          ...
          update_cell_relations_recursively<dim, spacedim>(this->local_cell_relations, *tree, cell, p4est_coarse_cell);
        }
    }
    ...
    std::vector<unsigned int> Triangulation<dim, spacedim>::get_cell_weights() const
    {
      ...
      // correctly
      Assert(this->local_cell_relations.size() == static_cast<unsigned int>(parallel_forest->local_num_quadrants),
             ExcInternalError());
    ...
    Triangulation<1, spacedim>::Triangulation(
      ...
      const Settings /*settings*/)
      : dealii::parallel::DistributedTriangulationBase<1, spacedim>(mpi_communicator, smooth_grid, false)
    {
      Assert(false, ExcNotImplemented());
    }
    ...
    template <int spacedim>
    DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<1, spacedim>))
    const std::vector<types::global_dof_index> &Triangulation<1, spacedim>::get_p4est_tree_to_coarse_cell_permutation()
      const
    {
    ...
    template <int spacedim>
    DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<1, spacedim>))
    std::map<unsigned int, std::set<dealii::types::subdomain_id>> Triangulation<1, spacedim>::
    ...
    template <int spacedim>
    DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<1, spacedim>))
    std::vector<bool> Triangulation<1, spacedim>::mark_locally_active_vertices_on_level(const unsigned int) const
    {
      Assert(false, ExcNotImplemented());
    ...
    template <int spacedim>
    DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<1, spacedim>))
    unsigned int Triangulation<1, spacedim>::coarse_cell_id_to_coarse_cell_index(const types::coarse_cell_id) const
    {
      Assert(false, ExcNotImplemented());
      return 0;
    }
    ...
    template <int spacedim>
    DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<1, spacedim>))
    types::coarse_cell_id Triangulation<1, spacedim>::coarse_cell_index_to_coarse_cell_id(const unsigned int) const
    {
      Assert(false, ExcNotImplemented());
      return 0;
    }
...
namespace parallel
{
  ...
  {
    template <int dim, int spacedim>
    TemporarilyMatchRefineFlags<dim, spacedim>::TemporarilyMatchRefineFlags(dealii::Triangulation<dim, spacedim> &tria)
      : distributed_tria(dynamic_cast<dealii::parallel::distributed::Triangulation<dim, spacedim> *>(&tria))
    {
#ifdef DEAL_II_WITH_P4EST

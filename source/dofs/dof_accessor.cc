...
DoFInvalidAccessor<structdim, dim, spacedim>::DoFInvalidAccessor(const void *,
                                                                 const int,
                                                                 const int,
                                                                 const AccessorData *)
{
...
DoFInvalidAccessor<structdim, dim, spacedim>::dof_index(const unsigned int, const types::fe_index) const
{
...
void
DoFCellAccessor<dim, spacedim, lda>::set_dof_indices(const std::vector<types::global_dof_index> &local_dof_indices)
{
  ...
  internal::DoFAccessorImplementation::Implementation::template set_dof_indices<dim, spacedim, lda, dim>(
...
DoFCellAccessor<dim, spacedim, lda>::neighbor_child_on_subface(const unsigned int face,
                                                               const unsigned int subface) const
{
  ...
  return TriaIterator<DoFCellAccessor<dim, spacedim, lda>>(*q, this->dof_handler);
}
...
template <int dim, int spacedim, bool lda>
TriaIterator<DoFCellAccessor<dim, spacedim, lda>>
DoFCellAccessor<dim, spacedim, lda>::periodic_neighbor_child_on_subface(const unsigned int face,
                                                                        const unsigned int subface) const
{
  const TriaIterator<CellAccessor<dim, spacedim>> q =
...
DoFCellAccessor<dim, spacedim, lda>::periodic_neighbor(const unsigned int face) const
{
  const TriaIterator<CellAccessor<dim, spacedim>> q = CellAccessor<dim, spacedim>::periodic_neighbor(face);
  return TriaIterator<DoFCellAccessor<dim, spacedim, lda>>(*q, this->dof_handler);
}
...
template <int dim, int spacedim, bool lda>
inline TriaIterator<DoFCellAccessor<dim, spacedim, lda>>
DoFCellAccessor<dim, spacedim, lda>::neighbor_or_periodic_neighbor(const unsigned int face) const
{
  const TriaIterator<CellAccessor<dim, spacedim>> q = CellAccessor<dim, spacedim>::neighbor_or_periodic_neighbor(face);
  return TriaIterator<DoFCellAccessor<dim, spacedim, lda>>(*q, this->dof_handler);
}
  ...
  const types::fe_index                 fe_index_) const
{
  ...
  Assert(static_cast<unsigned int>(this->level()) < this->dof_handler->object_dof_indices.size(),
  ...
  Assert(this->fe_index_is_active(fe_index) ||
           (this->dof_handler->get_fe(fe_index).n_dofs_per_cell() ==
            this->n_vertices() * this->dof_handler->get_fe(fe_index).n_dofs_per_vertex()),
         ExcInternalError());

  // now do the actual work
  dealii::internal::DoFAccessorImplementation::Implementation::get_dof_indices(*this, dof_indices, fe_index);
}
  ...
  const types::fe_index                 fe_index_) const
{
  ...
  internal::DoFAccessorImplementation::Implementation::get_mg_dof_indices(*this, level, dof_indices, fe_index);
}
  ...
  const types::fe_index                       fe_index_)
{
  ...
  internal::DoFAccessorImplementation::Implementation::set_mg_dof_indices(*this, level, dof_indices, fe_index);
}
...
inline void
DoFCellAccessor<dimension_, space_dimension_, level_dof_access>::get_dof_indices(
  std::vector<types::global_dof_index> &dof_indices) const
{
  Assert(this->is_active(), ExcMessage("get_dof_indices() only works on active cells."));
  ...
  dealii::internal::DoFAccessorImplementation::Implementation::get_dof_indices(*this,
                                                                               dof_indices,
...
template <int dimension_, int space_dimension_, bool level_dof_access>
inline void
DoFCellAccessor<dimension_, space_dimension_, level_dof_access>::get_mg_dof_indices(
  std::vector<types::global_dof_index> &dof_indices) const
{
  Assert(this->dof_handler->mg_vertex_dofs.size() > 0,
         ExcMessage("Multigrid DoF indices can only be accessed after "
                    "DoFHandler::distribute_mg_dofs() has been called!"));
  DoFAccessor<dimension_, dimension_, space_dimension_, level_dof_access>::get_mg_dof_indices(this->level(),
                                                                                              dof_indices);
}
...
template <int dimension_, int space_dimension_, bool level_dof_access>
inline void
DoFCellAccessor<dimension_, space_dimension_, level_dof_access>::set_mg_dof_indices(
  const std::vector<types::global_dof_index> &dof_indices)
{
  Assert(this->dof_handler->mg_vertex_dofs.size() > 0,
         ExcMessage("Multigrid DoF indices can only be accessed after "
                    "DoFHandler::distribute_mg_dofs() has been called!"));
  DoFAccessor<dimension_, dimension_, space_dimension_, level_dof_access>::set_mg_dof_indices(this->level(),
                                                                                              dof_indices);
}

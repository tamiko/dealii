...
namespace internal
{
  ...
  policy_to_string(const dealii::internal::DoFHandlerImplementation::Policy::PolicyBase<dim, spacedim> &policy)
  {
    ...
    if (dynamic_cast<const typename dealii::internal::DoFHandlerImplementation::Policy::Sequential<dim, spacedim> *>(
          &policy) ||
        dynamic_cast<const typename dealii::internal::DoFHandlerImplementation::Policy::Sequential<dim, spacedim> *>(
          &policy))
      policy_name = "Policy::Sequential<";
    else if (dynamic_cast<
               const typename dealii::internal::DoFHandlerImplementation::Policy::ParallelDistributed<dim, spacedim> *>(
               &policy) ||
             dynamic_cast<
               const typename dealii::internal::DoFHandlerImplementation::Policy::ParallelDistributed<dim, spacedim> *>(
               &policy))
      policy_name = "Policy::ParallelDistributed<";
               ...
               const typename dealii::internal::DoFHandlerImplementation::Policy::ParallelShared<dim, spacedim> *>(
               &policy) ||
             dynamic_cast<
               const typename dealii::internal::DoFHandlerImplementation::Policy::ParallelShared<dim, spacedim> *>(
               &policy))
  ...
  namespace DoFHandlerImplementation
  {
    ...
    struct Implementation
    {
      ...
      {
        return std::min(static_cast<types::global_dof_index>(3 * dof_handler.fe_collection.max_dofs_per_vertex() +
      ...
      template <int dim, int spacedim>
      static void
      reserve_cells(DoFHandler<dim, spacedim> &dof_handler, const unsigned int n_inner_dofs_per_cell)
      {
        for (unsigned int i = 0; i < dof_handler.tria->n_levels(); ++i)
          {
            dof_handler.object_dof_ptr[i][dim].assign(dof_handler.tria->n_raw_cells(i) + 1, 0);

            for (const auto &cell : dof_handler.tria->cell_iterators_on_level(i))
              if (cell->is_active() && !cell->is_artificial())
                dof_handler.object_dof_ptr[i][dim][cell->index() + 1] = n_inner_dofs_per_cell;

            for (unsigned int j = 0; j < dof_handler.tria->n_raw_cells(i); ++j)
              dof_handler.object_dof_ptr[i][dim][j + 1] += dof_handler.object_dof_ptr[i][dim][j];

            dof_handler.object_dof_indices[i][dim].resize(dof_handler.object_dof_ptr[i][dim].back(),
                          ...
                          const T                   &cell_process)
      {
        ...
        for (const auto &cell : dof_handler.tria->cell_iterators())
          if (cell->is_active() && !cell->is_artificial())
            cell_process(cell, [&](const unsigned int n_dofs_per_entity, const unsigned int index) {
              auto &n_dofs_per_entity_target = dof_handler.object_dof_ptr[0][structdim][index + 1];

              // make sure that either the entity has not been visited or
              // the entity has the same number of dofs assigned
              Assert((n_dofs_per_entity_target == static_cast<typename DoFHandler<dim, spacedim>::offset_type>(-1) ||
        ...
        for (unsigned int i = 1; i < n_raw_entities + 1; ++i)
          {
            if (dof_handler.object_dof_ptr[0][structdim][i] ==
                static_cast<typename DoFHandler<dim, spacedim>::offset_type>(-1))
              dof_handler.object_dof_ptr[0][structdim][i] = dof_handler.object_dof_ptr[0][structdim][i - 1];
            else
              dof_handler.object_dof_ptr[0][structdim][i] += dof_handler.object_dof_ptr[0][structdim][i - 1];
          }

        // allocate memory for indices
        dof_handler.object_dof_indices[0][structdim].resize(dof_handler.object_dof_ptr[0][structdim].back(),
                                                            numbers::invalid_dof_index);
      }
      ...
      reserve_space(DoFHandler<dim, spacedim> &dof_handler)
      {
        ...
        reserve_cells(dof_handler,
                      dim == 1 ? fe.n_dofs_per_line() : (dim == 2 ? fe.n_dofs_per_quad(0) : fe.n_dofs_per_hex()));

        // vertices
        reserve_subentities(dof_handler, 0, dof_handler.tria->n_vertices(), [&](const auto &cell, const auto &process) {
          for (const auto vertex_index : cell->vertex_indices())
            process(fe.n_dofs_per_vertex(), cell->vertex_index(vertex_index));
        });
          ...
          reserve_subentities(dof_handler,
                              ...
                              [&](const auto &cell, const auto &process) {
                                for (const auto line_index : cell->line_indices())
                                  process(fe.n_dofs_per_line(), cell->line(line_index)->index());
                              });
          ...
          reserve_subentities(dof_handler,
                              ...
                              [&](const auto &cell, const auto &process) {
                                for (const auto face_index : cell->face_indices())
                                  process(fe.n_dofs_per_quad(face_index), cell->face(face_index)->index());
                              });
      }
      ...
      reserve_space_mg(DoFHandler<1, spacedim> &dof_handler)
      {
        ...
        const unsigned int                        dofs_per_line = dof_handler.get_fe().n_dofs_per_line();
        ...
        for (unsigned int i = 0; i < n_levels; ++i)
          {
            ...
            dof_handler.mg_levels.back()->dof_object.dofs =
              std::vector<types::global_dof_index>(tria.n_raw_lines(i) * dofs_per_line, numbers::invalid_dof_index);
          }
      ...
      reserve_space_mg(DoFHandler<2, spacedim> &dof_handler)
      {
        ...
        for (unsigned int i = 0; i < n_levels; ++i)
          {
            dof_handler.mg_levels.emplace_back(std::make_unique<internal::DoFHandlerImplementation::DoFLevel<2>>());
            dof_handler.mg_levels.back()->dof_object.dofs =
        ...
        dof_handler.mg_faces = std::make_unique<internal::DoFHandlerImplementation::DoFFaces<2>>();
        dof_handler.mg_faces->lines.dofs =
          std::vector<types::global_dof_index>(tria.n_raw_lines() * fe.n_dofs_per_line(), numbers::invalid_dof_index);
          ...
          if (tria.vertex_used(vertex))
            {
              Assert(min_level[vertex] < n_levels, ExcInternalError());
              Assert(max_level[vertex] >= min_level[vertex], ExcInternalError());
              dof_handler.mg_vertex_dofs[vertex].init(min_level[vertex], max_level[vertex], fe.n_dofs_per_vertex());
            }
      ...
      reserve_space_mg(DoFHandler<3, spacedim> &dof_handler)
      {
        ...
        for (unsigned int i = 0; i < n_levels; ++i)
          {
            dof_handler.mg_levels.emplace_back(std::make_unique<internal::DoFHandlerImplementation::DoFLevel<3>>());
            dof_handler.mg_levels.back()->dof_object.dofs =
              std::vector<types::global_dof_index>(tria.n_raw_hexs(i) * fe.n_dofs_per_hex(),
                                                   numbers::invalid_dof_index);
          }

        dof_handler.mg_faces = std::make_unique<internal::DoFHandlerImplementation::DoFFaces<3>>();
        dof_handler.mg_faces->lines.dofs =
          std::vector<types::global_dof_index>(tria.n_raw_lines() * fe.n_dofs_per_line(), numbers::invalid_dof_index);
        ...
        dof_handler.mg_faces->quads.dofs =
          std::vector<types::global_dof_index>(tria.n_raw_quads() * fe.n_dofs_per_quad(0 /*=face_no*/),
                                               numbers::invalid_dof_index);
    ...
    namespace DoFHandlerImplementation
    {
      ...
      struct Implementation
      {
        ...
        ensure_absence_of_future_fe_indices(DoFHandler<dim, spacedim> &dof_handler)
        {
          ...
          for (const auto &cell : dof_handler.active_cell_iterators())
            if (cell->is_locally_owned())
              Assert(!cell->future_fe_index_set(),
                     ExcMessage("There shouldn't be any cells flagged for p-adaptation when partitioning."));
        }
        ...
        reserve_space_vertices(DoFHandler<dim, spacedim> &dof_handler)
        {
          ...
          std::vector<std::vector<bool>> vertex_fe_association(dof_handler.fe_collection.size(),
                                                               std::vector<bool>(dof_handler.tria->n_vertices(),
                                                                                 false));
          ...
          dof_handler.hp_object_fe_ptr[d].reserve(dof_handler.tria->n_vertices() + 1);
              ...
              if (dof_handler.tria->vertex_used(v) && locally_used_vertices[v])
                {
                  for (unsigned int fe = 0; fe < dof_handler.fe_collection.size(); ++fe)
                    if (vertex_fe_association[fe][v] == true)
                      {
                        fe_slots_needed++;
                        vertex_slots_needed += dof_handler.get_fe(fe).n_dofs_per_vertex();
                      }
                }
            }
                  ...
                  if (vertex_fe_association[fe][v] == true)
                    {
                      ...
                      dof_handler.object_dof_ptr[l][d].push_back(dof_handler.object_dof_indices[l][d].size());

                      for (unsigned int i = 0; i < dof_handler.get_fe(fe).n_dofs_per_vertex(); i++)
                        dof_handler.object_dof_indices[l][d].push_back(numbers::invalid_dof_index);
                    }
              }
          ...
          AssertDimension(vertex_slots_needed, dof_handler.object_dof_indices[l][d].size());
          AssertDimension(fe_slots_needed, dof_handler.hp_object_fe_indices[d].size());
          AssertDimension(fe_slots_needed + 1, dof_handler.object_dof_ptr[l][d].size());
          AssertDimension(dof_handler.tria->n_vertices() + 1, dof_handler.hp_object_fe_ptr[d].size());

          dof_handler.object_dof_indices[l][d].assign(vertex_slots_needed, numbers::invalid_dof_index);
        }
        ...
        reserve_space_cells(DoFHandler<dim, spacedim> &dof_handler)
        {
          ...
          for (unsigned int level = 0; level < dof_handler.tria->n_levels(); ++level)
            {
              dof_handler.object_dof_ptr[level][dim] = std::vector<typename DoFHandler<dim, spacedim>::offset_type>(
                dof_handler.tria->n_raw_cells(level), static_cast<typename DoFHandler<dim, spacedim>::offset_type>(-1));

              types::global_dof_index next_free_dof = 0;
              for (auto cell : dof_handler.active_cell_iterators_on_level(level))
                if (cell->is_active() && !cell->is_artificial())
                  {
                    dof_handler.object_dof_ptr[level][dim][cell->index()] = next_free_dof;
        ...
        reserve_space_faces(DoFHandler<dim, spacedim> &dof_handler)
        {
          ...
          {
            std::vector<bool> face_touched(dim == 2 ? dof_handler.tria->n_raw_lines() :
                                                      dof_handler.tria->n_raw_quads());
            ...
            dof_handler.hp_object_fe_ptr[d].resize(dof_handler.tria->n_raw_faces() + 1);
                  ...
                  if (!face_touched[cell->face(face)->index()])
                    {
                      ...
                      if (cell->at_boundary(face) || cell->face(face)->has_children() ||
                           ...
                           (cell->active_fe_index() == cell->neighbor(face)->active_fe_index())))
                        {
                          fe_slots_needed = 1;
                          n_face_slots +=
                            dof_handler.get_fe(cell->active_fe_index()).template n_dofs_per_object<dim - 1>(face);
                        }
                      else
                        {
                          fe_slots_needed = 2;
                          n_face_slots +=
                            ...
                            dof_handler.get_fe(cell->neighbor(face)->active_fe_index())
                              .template n_dofs_per_object<dim - 1>(cell->neighbor_face_no(face));
                        }

                      // mark this face as visited
                      face_touched[cell->face(face)->index()] = true;

                      dof_handler.hp_object_fe_ptr[d][cell->face(face)->index() + 1] = fe_slots_needed;
                    }

            for (unsigned int i = 1; i < dof_handler.hp_object_fe_ptr[d].size(); i++)
              dof_handler.hp_object_fe_ptr[d][i] += dof_handler.hp_object_fe_ptr[d][i - 1];


            dof_handler.hp_object_fe_indices[d].resize(dof_handler.hp_object_fe_ptr[d].back());
            dof_handler.object_dof_ptr[l][d].resize(dof_handler.hp_object_fe_ptr[d].back() + 1);
                  ...
                  if (!face_touched[cell->face(face)->index()])
                    {
                      ...
                      if (cell->at_boundary(face) || cell->face(face)->has_children() ||
                           ...
                           (cell->active_fe_index() == cell->neighbor(face)->active_fe_index())))
                        {
                          ...
                          const unsigned int n_dofs = dof_handler.get_fe(fe).template n_dofs_per_object<dim - 1>(face);
                          const unsigned int offset = dof_handler.hp_object_fe_ptr[d][cell->face(face)->index()];
                      ...
                      else
                        {
                          ...
                          types::fe_index fe_2      = cell->neighbor(face)->active_fe_index();
                          ...
                          const unsigned int n_dofs_1 =
                            dof_handler.get_fe(fe_1).template n_dofs_per_object<dim - 1>(face_no_1);

                          const unsigned int n_dofs_2 =
                            dof_handler.get_fe(fe_2).template n_dofs_per_object<dim - 1>(face_no_2);

                          const unsigned int offset = dof_handler.hp_object_fe_ptr[d][cell->face(face)->index()];

                          dof_handler.hp_object_fe_indices[d].push_back(cell->active_fe_index());
                          dof_handler.object_dof_ptr[l][d].push_back(dof_handler.object_dof_indices[l][d].size());

                          dof_handler.hp_object_fe_indices[d][offset + 0] = fe_1;
                          dof_handler.hp_object_fe_indices[d][offset + 1] = fe_2;
                          dof_handler.object_dof_ptr[l][d][offset + 1]    = n_dofs_1;
                          dof_handler.object_dof_ptr[l][d][offset + 2]    = n_dofs_2;
            ...
            for (unsigned int i = 1; i < dof_handler.object_dof_ptr[l][d].size(); i++)
              dof_handler.object_dof_ptr[l][d][i] += dof_handler.object_dof_ptr[l][d][i - 1];
          }
        }
        ...
        {
          Assert(dof_handler.fe_collection.size() > 0, (typename DoFHandler<1, spacedim>::ExcNoFESelected()));
          Assert(dof_handler.tria->n_levels() > 0, ExcMessage("The current Triangulation must not be empty."));
          Assert(dof_handler.tria->n_levels() == dof_handler.hp_cell_future_fe_indices.size(), ExcInternalError());

          internal::DoFHandlerImplementation::Implementation::reset_to_empty_objects(dof_handler);

          Threads::TaskGroup<> tasks;
          tasks += Threads::new_task(&reserve_space_cells<1, spacedim>, dof_handler);
          tasks += Threads::new_task(&reserve_space_vertices<1, spacedim>, dof_handler);
          tasks.join_all();
        }
        ...
        reserve_space(DoFHandler<2, spacedim> &dof_handler)
        {
          ...
          Threads::TaskGroup<> tasks;
          tasks += Threads::new_task(&reserve_space_cells<2, spacedim>, dof_handler);
          tasks += Threads::new_task(&reserve_space_faces<2, spacedim>, dof_handler);
          tasks += Threads::new_task(&reserve_space_vertices<2, spacedim>, dof_handler);
          tasks.join_all();
        }
        ...
        reserve_space(DoFHandler<3, spacedim> &dof_handler)
        {
          ...
          Threads::TaskGroup<> tasks;
          tasks += Threads::new_task(&reserve_space_cells<3, spacedim>, dof_handler);
          tasks += Threads::new_task(&reserve_space_faces<3, spacedim>, dof_handler);
          tasks += Threads::new_task(&reserve_space_vertices<3, spacedim>, dof_handler);
          ...
          // reserve_space_vertices()
          {
            ...
            std::vector<std::vector<bool>> line_fe_association(dof_handler.fe_collection.size(),
            ...
            dof_handler.hp_object_fe_ptr[d].reserve(dof_handler.tria->n_raw_lines() + 1);
                ...
                if (line_is_used[line] == true)
                  {
                    for (unsigned int fe = 0; fe < dof_handler.fe_collection.size(); ++fe)
                      if (line_fe_association[fe][line] == true)
                        {
                          fe_slots_needed++;
                          line_slots_needed += dof_handler.get_fe(fe).n_dofs_per_line();
                        }
                  }
              }
            ...
            AssertDimension(dof_handler.hp_object_fe_ptr[d].size(), dof_handler.tria->n_raw_lines() + 1);
                    ...
                    if (line_fe_association[fe][line] == true)
                      {
                        ...
                        for (unsigned int i = 0; i < dof_handler.get_fe(fe).n_dofs_per_line(); i++)
                          dof_handler.object_dof_indices[l][d].push_back(numbers::invalid_dof_index);
                      }
                }
            ...
            // make sure that all entries have been set
            AssertDimension(dof_handler.hp_object_fe_indices[d].size(), fe_slots_needed);
            AssertDimension(dof_handler.object_dof_ptr[l][d].size(), fe_slots_needed + 1);
            AssertDimension(dof_handler.object_dof_indices[l][d].size(), line_slots_needed);
          }
        ...
        communicate_active_fe_indices(DoFHandler<dim, spacedim> &dof_handler)
        {
          ...
          if (const dealii::parallel::shared::Triangulation<dim, spacedim> *tr =
                dynamic_cast<const dealii::parallel::shared::Triangulation<dim, spacedim> *>(
                  &dof_handler.get_triangulation()))
            {
              ...
              std::vector<types::fe_index> active_fe_indices(tr->n_active_cells(), 0u);
              for (const auto &cell : dof_handler.active_cell_iterators())
                if (cell->is_locally_owned())
              ...
              for (const auto &cell : dof_handler.active_cell_iterators())
                if (!cell->is_locally_owned())
                  dof_handler.hp_cell_active_fe_indices[cell->level()][cell->index()] =
                    active_fe_indices[cell->active_cell_index()];
            }
          else if (const dealii::parallel::DistributedTriangulationBase<dim, spacedim> *tr =
                     dynamic_cast<const dealii::parallel::DistributedTriangulationBase<dim, spacedim> *>(
                       &dof_handler.get_triangulation()))
            {
              ...
              // objects
              auto pack = [](const typename DoFHandler<dim, spacedim>::active_cell_iterator &cell) -> types::fe_index {
                return cell->active_fe_index();
              };

              auto unpack = [&dof_handler](const typename DoFHandler<dim, spacedim>::active_cell_iterator &cell,
                ...
                dof_handler.hp_cell_active_fe_indices[cell->level()][cell->index()] = active_fe_index;
              };

              GridTools::exchange_cell_data_to_ghosts<types::fe_index, DoFHandler<dim, spacedim>>(dof_handler,
                                                                                                  pack,
                                                                                                  unpack);
            }
          else
            {
              ...
              Assert((dynamic_cast<const dealii::parallel::TriangulationBase<dim, spacedim> *>(
        ...
        communicate_future_fe_indices(DoFHandler<dim, spacedim> &dof_handler)
        {
          ...
          if (const dealii::parallel::shared::Triangulation<dim, spacedim> *tr =
                dynamic_cast<const dealii::parallel::shared::Triangulation<dim, spacedim> *>(
                  &dof_handler.get_triangulation()))
            {
              std::vector<types::fe_index> future_fe_indices(tr->n_active_cells(), 0u);
              for (const auto &cell : dof_handler.active_cell_iterators() | IteratorFilters::LocallyOwnedCell())
                future_fe_indices[cell->active_cell_index()] =
                  dof_handler.hp_cell_future_fe_indices[cell->level()][cell->index()];

              Utilities::MPI::sum(future_fe_indices, tr->get_communicator(), future_fe_indices);
                       ...
                       &dof_handler.get_triangulation()))
            {
              auto pack = [&dof_handler](
                            const typename DoFHandler<dim, spacedim>::active_cell_iterator &cell) -> types::fe_index {
                return dof_handler.hp_cell_future_fe_indices[cell->level()][cell->index()];
              };

              auto unpack = [&dof_handler](const typename DoFHandler<dim, spacedim>::active_cell_iterator &cell,
                                           const types::fe_index future_fe_index) -> void {
                dof_handler.hp_cell_future_fe_indices[cell->level()][cell->index()] = future_fe_index;
              };
        ...
        template <int dim, int spacedim>
        static void
        collect_fe_indices_on_cells_to_be_refined(DoFHandler<dim, spacedim> &dof_handler)
        {
                ...
                if (cell->refine_flag_set())
                  {
                    ...
                    fe_transfer->refined_cells_fe_index.insert({cell, cell->future_fe_index()});
                  }
                else if (cell->coarsen_flag_set())
                  {
                    ...
                    if (fe_transfer->coarsened_cells_fe_index.find(parent) ==
                        fe_transfer->coarsened_cells_fe_index.end())
                      {
                        ...
                        const types::fe_index fe_index = dealii::internal::hp::DoFHandlerImplementation::
                          Implementation::dominated_future_fe_on_children<dim, spacedim>(parent);

                        fe_transfer->coarsened_cells_fe_index.insert({parent, fe_index});
                      }
                  }
                else
                  {
                    ...
                    if (cell->future_fe_index_set() == true)
                      fe_transfer->persisting_cells_fe_index.insert({cell, cell->future_fe_index()});
                  }
              }
        }
        ...
        template <int dim, int spacedim>
        static void
        distribute_fe_indices_on_refined_cells(DoFHandler<dim, spacedim> &dof_handler)
        {
          const auto &fe_transfer = dof_handler.active_fe_index_transfer;
        ...
        determine_fe_from_children(const typename Triangulation<dim, spacedim>::cell_iterator &,
                                   ...
                                   const dealii::hp::FECollection<dim, spacedim> &fe_collection)
        {
          ...
          const std::set<unsigned int> children_fe_indices_set(children_fe_indices.begin(), children_fe_indices.end());

          const types::fe_index dominated_fe_index = fe_collection.find_dominated_fe_extended(children_fe_indices_set,
                                                                                              /*codim=*/0);

          Assert(dominated_fe_index != numbers::invalid_fe_index, ExcNoDominatedFiniteElementOnChildren());
        ...
        template <int dim, int spacedim>
        static types::fe_index
        dominated_future_fe_on_children(const typename DoFHandler<dim, spacedim>::cell_iterator &parent)
        {
          Assert(!parent->is_active(),
                 ExcMessage("You ask for information on children of this cell which is only "
                            "available for active cells. This cell has no children."));

          const auto &dof_handler = parent->get_dof_handler();
          Assert(dof_handler.has_hp_capabilities(), (typename DoFHandler<dim, spacedim>::ExcOnlyAvailableWithHP()));
          ...
          for (const auto &child : parent->child_iterators())
            {
              ...
              const types::fe_index future_fe_index_child =
                dealii::internal::DoFCellAccessorImplementation::Implementation::future_fe_index<dim, spacedim, false>(
                  *child);
          ...
          const types::fe_index future_fe_index =
            dof_handler.fe_collection.find_dominated_fe_extended(future_fe_indices_children,
                                                                 /*codim=*/0);

          Assert(future_fe_index != numbers::invalid_fe_index, ExcNoDominatedFiniteElementOnChildren());
      ...
      communicate_future_fe_indices(DoFHandler<dim, spacedim> &dof_handler)
      {
        Implementation::communicate_future_fe_indices<dim, spacedim>(dof_handler);
      }
      ...
      dominated_future_fe_on_children(const typename DoFHandler<dim, spacedim>::cell_iterator &parent)
      {
        return Implementation::dominated_future_fe_on_children<dim, spacedim>(parent);
      }
    } // namespace DoFHandlerImplementation
  }   // namespace hp
} // namespace internal
...
template <int dim, int spacedim>
DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
typename DoFHandler<dim, spacedim>::cell_iterator DoFHandler<dim, spacedim>::begin(const unsigned int level) const
{
...
template <int dim, int spacedim>
DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
typename DoFHandler<dim, spacedim>::cell_iterator DoFHandler<dim, spacedim>::end() const
{
...
template <int dim, int spacedim>
DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
typename DoFHandler<dim, spacedim>::cell_iterator DoFHandler<dim, spacedim>::end(const unsigned int level) const
{
...
template <int dim, int spacedim>
DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
typename DoFHandler<dim, spacedim>::level_cell_iterator DoFHandler<dim, spacedim>::end_mg() const
{
...
DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
IteratorRange<
  typename DoFHandler<dim, spacedim>::active_cell_iterator> DoFHandler<dim, spacedim>::active_cell_iterators() const
{
  return IteratorRange<typename DoFHandler<dim, spacedim>::active_cell_iterator>(begin_active(), end());
}
...
template <int dim, int spacedim>
DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
IteratorRange<typename DoFHandler<dim, spacedim>::level_cell_iterator> DoFHandler<dim, spacedim>::mg_cell_iterators()
  const
{
  return IteratorRange<typename DoFHandler<dim, spacedim>::level_cell_iterator>(begin_mg(), end_mg());
}
...
template <int dim, int spacedim>
DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
IteratorRange<typename DoFHandler<dim, spacedim>::cell_iterator> DoFHandler<dim, spacedim>::cell_iterators_on_level(
...
template <int dim, int spacedim>
DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
IteratorRange<typename DoFHandler<dim, spacedim>::active_cell_iterator> DoFHandler<dim, spacedim>::
  active_cell_iterators_on_level(const unsigned int level) const
{
  return IteratorRange<typename DoFHandler<dim, spacedim>::active_cell_iterator>(begin_active(level),
                                                                                 end_active(level));
}
...
template <int dim, int spacedim>
DEAL_II_CXX20_REQUIRES((concepts::is_valid_dim_spacedim<dim, spacedim>))
IteratorRange<typename DoFHandler<dim, spacedim>::level_cell_iterator> DoFHandler<dim, spacedim>::
  mg_cell_iterators_on_level(const unsigned int level) const
{
  return IteratorRange<typename DoFHandler<dim, spacedim>::level_cell_iterator>(begin_mg(level), end_mg(level));
}
...
{
  Assert(!(dim == 2 && spacedim == 3) || hp_capability_enabled == false, ExcNotImplementedWithHP());
        ...
        for (const auto iface : cell->face_indices())
          {
            ...
            if (face->at_boundary())
              {
                const unsigned int dofs_per_face = cell->get_fe().n_dofs_per_face(iface);
  ...
  DoFHandler<dim, spacedim>::n_boundary_dofs(const std::set<types::boundary_id> &boundary_ids) const
{
  ...
  Assert(this->fe_collection.size() > 0, ExcNoFESelected());
  Assert(boundary_ids.find(numbers::internal_face_boundary_id) == boundary_ids.end(), ExcInvalidBoundaryIndicator());
        ...
        for (const auto iface : cell->face_indices())
          {
            ...
            if (face->at_boundary() && (boundary_ids.find(boundary_id) != boundary_ids.end()))
              {
                const unsigned int dofs_per_face = cell->get_fe().n_dofs_per_face(iface);
                dofs_on_face.resize(dofs_per_face);
  ...
  else
    {
      ...
      for (unsigned int i = 0; i < this->mg_vertex_dofs.size(); ++i)
        mem += sizeof(MGVertexDoFs) +
               (1 + this->mg_vertex_dofs[i].get_finest_level() - this->mg_vertex_dofs[i].get_coarsest_level()) *
...
void DoFHandler<dim, spacedim>::distribute_dofs(const hp::FECollection<dim, spacedim> &ff)
{
  Assert(this->tria != nullptr,
         ExcMessage("You need to set the Triangulation in the DoFHandler using reinit() "
                    "or in the constructor before you can distribute DoFs."));
  Assert(this->tria->n_levels() > 0, ExcMessage("The Triangulation you are using is empty!"));

  // verify size of provided FE collection
  Assert(ff.size() > 0, ExcMessage("The given hp::FECollection is empty!"));
  Assert((ff.size() <= std::numeric_limits<types::fe_index>::max()) && (ff.size() != numbers::invalid_fe_index),
  ...
  if ((hp_cell_active_fe_indices.size() > 0) && (hp_cell_future_fe_indices.size() > 0))
    {
      Assert(hp_capability_enabled, ExcInternalError());

      for (const auto &cell : this->active_cell_iterators() | IteratorFilters::LocallyOwnedCell())
        {
          Assert(cell->active_fe_index() < ff.size(), ExcInvalidFEIndex(cell->active_fe_index(), ff.size()));
          Assert(cell->future_fe_index() < ff.size(), ExcInvalidFEIndex(cell->future_fe_index(), ff.size()));
        }
    }
#endif
  ...
  if (this->fe_collection != ff)
    {
      ...
      AssertThrow(hp_capability_enabled || !contains_multiple_fes,
                  ExcMessage("You cannot re-enable hp-capabilities after you registered a single "
  ...
  if (hp_capability_enabled)
    {
      ...
      internal::hp::DoFHandlerImplementation::Implementation::communicate_active_fe_indices(*this);
    }

  {
    ...
    const internal::parallel::shared::TemporarilyRestoreSubdomainIds<dim, spacedim> subdomain_modifier(
    ...
    if (hp_capability_enabled)
      internal::hp::DoFHandlerImplementation::Implementation::reserve_space(*this);
    else
  ...
  if (!hp_capability_enabled &&
      dynamic_cast<const parallel::DistributedTriangulationBase<dim, spacedim> *>(&*this->tria) == nullptr)
...
void DoFHandler<dim, spacedim>::distribute_mg_dofs()
{
  ...
  Assert(this->object_dof_indices.size() > 0,
         ExcMessage("Distribute active DoFs using distribute_dofs() before calling distribute_mg_dofs()."));

  Assert(((this->tria->get_mesh_smoothing() & Triangulation<dim, spacedim>::limit_level_difference_at_vertices) !=
  ...
  if (dynamic_cast<const parallel::TriangulationBase<dim, spacedim> *>(&*this->tria) == nullptr)
...
void DoFHandler<dim, spacedim>::setup_policy()
{
  ...
  if (dynamic_cast<const dealii::parallel::shared::Triangulation<dim, spacedim> *>(&this->get_triangulation()) !=
      nullptr)
    this->policy = std::make_unique<internal::DoFHandlerImplementation::Policy::ParallelShared<dim, spacedim>>(*this);
  else if (dynamic_cast<const dealii::parallel::DistributedTriangulationBase<dim, spacedim> *>(
             &this->get_triangulation()) == nullptr)
    this->policy = std::make_unique<internal::DoFHandlerImplementation::Policy::Sequential<dim, spacedim>>(*this);
  else
    this->policy =
      std::make_unique<internal::DoFHandlerImplementation::Policy::ParallelDistributed<dim, spacedim>>(*this);
}
  ...
  if (hp_capability_enabled)
    {
      ...
      if (this->n_locally_owned_dofs() == this->n_dofs())
        ...
        for (const auto new_number : new_numbers)
          Assert(new_number < this->n_dofs(), ExcMessage("New DoF index is not less than the total number of dofs."));
#endif
  ...
  else
    {
      ...
      if (dynamic_cast<const parallel::shared::Triangulation<dim, spacedim> *>(&*this->tria) != nullptr)
        {
          Assert(new_numbers.size() == this->n_dofs() || new_numbers.size() == this->n_locally_owned_dofs(),
                 ExcMessage("Incorrect size of the input array."));
        }
      else if (dynamic_cast<const parallel::DistributedTriangulationBase<dim, spacedim> *>(&*this->tria) != nullptr)
        {
          AssertDimension(new_numbers.size(), this->n_locally_owned_dofs());
        }
      else
                                              ...
                                              const std::vector<types::global_dof_index> &new_numbers)
{
  ...
  Assert(this->mg_levels.size() > 0 && this->object_dof_indices.size() > 0,
  ...
  AssertDimension(new_numbers.size(), this->locally_owned_mg_dofs(level).n_elements());
  ...
  if (this->n_locally_owned_dofs() == this->n_dofs())
    ...
    for (const auto new_number : new_numbers)
      Assert(new_number < this->n_dofs(level), ExcMessage("New DoF index is not less than the total number of dofs."));
#endif

  this->mg_number_cache[level] = this->policy->renumber_mg_dofs(level, new_numbers);
}
  ...
  switch (dim)
    {
      ...
      case 2:
        return (3 * this->fe_collection.max_dofs_per_vertex() + 2 * this->fe_collection.max_dofs_per_line());
      case 3:
        ...
        // satisfied
        return (19 * this->fe_collection.max_dofs_per_vertex() + 28 * this->fe_collection.max_dofs_per_line() +
...
void DoFHandler<dim, spacedim>::set_active_fe_indices(const std::vector<types::fe_index> &active_fe_indices)
{
  Assert(active_fe_indices.size() == this->get_triangulation().n_active_cells(),
         ExcDimensionMismatch(active_fe_indices.size(), this->get_triangulation().n_active_cells()));
...
{
  set_active_fe_indices(std::vector<types::fe_index>(active_fe_indices.begin(), active_fe_indices.end()));
}
...
{
  std::vector<types::fe_index> active_fe_indices(this->get_triangulation().n_active_cells(), numbers::invalid_fe_index);
...
void DoFHandler<dim, spacedim>::set_future_fe_indices(const std::vector<types::fe_index> &future_fe_indices)
{
  Assert(future_fe_indices.size() == this->get_triangulation().n_active_cells(),
         ExcDimensionMismatch(future_fe_indices.size(), this->get_triangulation().n_active_cells()));
  ...
  // tests which we would have to duplicate ourselves otherwise
  for (const auto &cell : this->active_cell_iterators())
    if (cell->is_locally_owned() && future_fe_indices[cell->active_cell_index()] != numbers::invalid_fe_index)
...
{
  std::vector<types::fe_index> future_fe_indices(this->get_triangulation().n_active_cells(), numbers::invalid_fe_index);
...
void DoFHandler<dim, spacedim>::connect_to_triangulation_signals()
{
  ...
  this->tria_listeners.push_back(this->tria->signals.create.connect([this]() { this->reinit(*(this->tria)); }));
  this->tria_listeners.push_back(this->tria->signals.clear.connect([this]() { this->clear(); }));
  ...
  if (dynamic_cast<const dealii::parallel::fullydistributed::Triangulation<dim, spacedim> *>(
        &this->get_triangulation()))
    {
      // no transfer of active FE indices for this class
    }
  else if (dynamic_cast<const dealii::parallel::distributed::Triangulation<dim, spacedim> *>(
             &this->get_triangulation()))
    {
      // repartitioning signals
      this->tria_listeners_for_transfer.push_back(this->tria->signals.pre_distributed_repartition.connect([this]() {
        internal::hp::DoFHandlerImplementation::Implementation::ensure_absence_of_future_fe_indices<dim, spacedim>(
          *this);
      }));
      this->tria_listeners_for_transfer.push_back(
        this->tria->signals.pre_distributed_repartition.connect([this]() { this->pre_distributed_transfer_action(); }));
      this->tria_listeners_for_transfer.push_back(
        this->tria->signals.post_distributed_repartition.connect([this] { this->post_distributed_transfer_action(); }));

      // refinement signals
      this->tria_listeners_for_transfer.push_back(
        this->tria->signals.post_p4est_refinement.connect([this]() { this->pre_distributed_transfer_action(); }));
      this->tria_listeners_for_transfer.push_back(this->tria->signals.post_distributed_refinement.connect(
        [this]() { this->post_distributed_transfer_action(); }));

      // serialization signals
      this->tria_listeners_for_transfer.push_back(
        this->tria->signals.post_distributed_save.connect([this]() { this->active_fe_index_transfer.reset(); }));
      this->tria_listeners_for_transfer.push_back(
        this->tria->signals.post_distributed_load.connect([this]() { this->update_active_fe_table(); }));
    }
  else if (dynamic_cast<const dealii::parallel::shared::Triangulation<dim, spacedim> *>(&this->get_triangulation()) !=
           nullptr)
    {
      // partitioning signals
      this->tria_listeners_for_transfer.push_back(this->tria->signals.pre_partition.connect([this]() {
        internal::hp::DoFHandlerImplementation::Implementation::ensure_absence_of_future_fe_indices(*this);
      }));

      // refinement signals
      this->tria_listeners_for_transfer.push_back(this->tria->signals.pre_refinement.connect(
        [this]() { internal::hp::DoFHandlerImplementation::Implementation::communicate_future_fe_indices(*this); }));
      this->tria_listeners_for_transfer.push_back(
        this->tria->signals.pre_refinement.connect([this] { this->pre_transfer_action(); }));
      this->tria_listeners_for_transfer.push_back(
        this->tria->signals.post_refinement.connect([this] { this->post_transfer_action(); }));
    }
  else
    {
      // refinement signals
...
void DoFHandler<dim, spacedim>::create_active_fe_table()
{
  ...
  for (unsigned int level = 0; level < this->hp_cell_future_fe_indices.size(); ++level)
    {
      if (this->hp_cell_active_fe_indices[level].empty() && this->hp_cell_future_fe_indices[level].empty())
        {
          this->hp_cell_active_fe_indices[level].resize(this->tria->n_raw_cells(level), 0);
          this->hp_cell_future_fe_indices[level].resize(this->tria->n_raw_cells(level), numbers::invalid_fe_index);
        }
      else
        {
          ...
          Assert(this->hp_cell_active_fe_indices[level].size() == this->tria->n_raw_cells(level) &&
                   this->hp_cell_future_fe_indices[level].size() == this->tria->n_raw_cells(level),
                 ExcInternalError());
        }
  ...
  for (unsigned int i = 0; i < this->hp_cell_future_fe_indices.size(); ++i)
    {
      ...
      this->hp_cell_future_fe_indices[i].assign(this->tria->n_raw_cells(i), numbers::invalid_fe_index);
    }
}
...
void DoFHandler<dim, spacedim>::pre_transfer_action()
{
  ...
  dealii::internal::hp::DoFHandlerImplementation::Implementation::collect_fe_indices_on_cells_to_be_refined(*this);
}
...
void DoFHandler<dim, spacedim>::pre_distributed_transfer_action()
{
  ...
  Assert((dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(&this->get_triangulation()) !=
          nullptr),
         ExcNotImplemented());
  ...
  active_fe_index_transfer->active_fe_indices.resize(get_triangulation().n_active_cells(), numbers::invalid_fe_index);

  for (const auto &cell : active_cell_iterators())
    if (cell->is_locally_owned())
      active_fe_index_transfer->active_fe_indices[cell->active_cell_index()] = cell->future_fe_index();

  // Create transfer object and attach to it.
  const auto *distributed_tria =
    dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(&this->get_triangulation());

  active_fe_index_transfer->cell_data_transfer =
    std::make_unique<parallel::distributed::CellDataTransfer<dim, spacedim, std::vector<types::fe_index>>>(
      *distributed_tria,
      /*transfer_variable_size_data=*/false,
      /*refinement_strategy=*/
      &dealii::AdaptationStrategies::Refinement::preserve<dim, spacedim, types::fe_index>,
      /*coarsening_strategy=*/
      [this](const typename Triangulation<dim, spacedim>::cell_iterator &parent,
             const std::vector<types::fe_index>                         &children_fe_indices) -> types::fe_index {
        return dealii::internal::hp::DoFHandlerImplementation::Implementation::determine_fe_from_children<dim,
                                                                                                          spacedim>(
          parent, children_fe_indices, fe_collection);
      });

  active_fe_index_transfer->cell_data_transfer->prepare_for_coarsening_and_refinement(
...
void DoFHandler<dim, spacedim>::post_transfer_action()
{
  ...
  dealii::internal::hp::DoFHandlerImplementation::Implementation::distribute_fe_indices_on_refined_cells(*this);
...
void DoFHandler<dim, spacedim>::post_distributed_transfer_action()
{
  ...
  this->active_fe_index_transfer->active_fe_indices.resize(this->get_triangulation().n_active_cells(),
                                                           numbers::invalid_fe_index);
  this->active_fe_index_transfer->cell_data_transfer->unpack(this->active_fe_index_transfer->active_fe_indices);

  // Update all locally owned active FE indices.
  this->set_active_fe_indices(this->active_fe_index_transfer->active_fe_indices);
...
void DoFHandler<dim, spacedim>::prepare_for_serialization_of_active_fe_indices()
{
  ...
  active_fe_index_transfer->cell_data_transfer->prepare_for_serialization(active_fe_index_transfer->active_fe_indices);
#endif
}
...
void DoFHandler<dim, spacedim>::deserialize_active_fe_indices()
{
  ...
  // Unpack active FE indices.
  active_fe_index_transfer->active_fe_indices.resize(get_triangulation().n_active_cells(), numbers::invalid_fe_index);
  active_fe_index_transfer->cell_data_transfer->deserialize(active_fe_index_transfer->active_fe_indices);

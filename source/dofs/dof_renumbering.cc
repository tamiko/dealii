...
namespace DoFRenumbering
{
  namespace boost
  {
    ...
    template <int dim, int spacedim>
    void
    Cuthill_McKee(DoFHandler<dim, spacedim> &dof_handler, const bool reversed_numbering, const bool use_constraints)
    {
      std::vector<types::global_dof_index> renumbering(dof_handler.n_dofs(), numbers::invalid_dof_index);
    ...
    template <int dim, int spacedim>
    void
    king_ordering(DoFHandler<dim, spacedim> &dof_handler, const bool reversed_numbering, const bool use_constraints)
    {
      std::vector<types::global_dof_index> renumbering(dof_handler.n_dofs(), numbers::invalid_dof_index);
    ...
    template <int dim, int spacedim>
    void
    minimum_degree(DoFHandler<dim, spacedim> &dof_handler, const bool reversed_numbering, const bool use_constraints)
    {
      std::vector<types::global_dof_index> renumbering(dof_handler.n_dofs(), numbers::invalid_dof_index);
      ...
      for (int i = 0; i < n; ++i)
        {
          ...
          Assert(std::find(inverse_perm.begin(), inverse_perm.end(), i) != inverse_perm.end(), ExcInternalError());
                ...
                const std::vector<types::global_dof_index> &starting_indices)
  {
    ...
    compute_Cuthill_McKee(renumbering, dof_handler, reversed_numbering, use_constraints, starting_indices);
  ...
  {
    const bool reorder_level_dofs = (level == numbers::invalid_unsigned_int) ? false : true;
    ...
    if (locally_owned_dofs.n_elements() == locally_owned_dofs.size())
      {
        ...
        DynamicSparsityPattern dsp(locally_owned_dofs.size(), locally_owned_dofs.size());
        if (reorder_level_dofs == false)
          {
            DoFTools::make_sparsity_pattern(dof_handler, dsp, constraints);
          }
    ...
    else
      {
        ...
        if (reorder_level_dofs == false)
          ...
          {
            DoFTools::extract_locally_active_level_dofs(dof_handler, locally_active_dofs, level);
          }
        ...
        for (const auto starting_index : starting_indices)
          {
            if ((needs_locally_active ==
              ...
              {
                Assert(locally_active_dofs.is_element(starting_index),
                       ExcMessage("You specified global degree of freedom " + std::to_string(starting_index) +
        ...
        const IndexSet index_set_to_use = (needs_locally_active ? locally_active_dofs : locally_owned_dofs);
        ...
        DynamicSparsityPattern dsp(index_set_to_use.size(), index_set_to_use.size(), index_set_to_use);
        if (reorder_level_dofs == false)
          {
            DoFTools::make_sparsity_pattern(dof_handler, dsp, constraints);
          }
        ...
        DynamicSparsityPattern local_sparsity(index_set_to_use.n_elements(), index_set_to_use.n_elements());
        ...
        SparsityTools::reorder_Cuthill_McKee(local_sparsity, my_new_indices, local_starting_indices);
        if (reversed_numbering)
        ...
        if (needs_locally_active == true)
          {
            ...
            Assert(erase_these_indices.size() == active_but_not_owned_dofs.n_elements(), ExcInternalError());
            Assert(static_cast<unsigned int>(
                     std::count(my_new_indices.begin(), my_new_indices.end(), numbers::invalid_dof_index)) ==
            ...
            {
              std::set<types::global_dof_index>::const_iterator next_erased_index = erase_these_indices.begin();
                ...
                const std::vector<types::global_dof_index> &starting_indices)
  {
    ...
    std::vector<types::global_dof_index> new_indices(dof_handler.locally_owned_mg_dofs(level).n_elements(),
                                                     numbers::invalid_dof_index);

    compute_Cuthill_McKee(new_indices, dof_handler, reversed_numbering, false, starting_indices, level);
  ...
  template <int dim, int spacedim>
  void
  component_wise(DoFHandler<dim, spacedim> &dof_handler, const std::vector<unsigned int> &component_order_arg)
  {
    std::vector<types::global_dof_index> renumbering(dof_handler.n_locally_owned_dofs(), numbers::invalid_dof_index);

    const types::global_dof_index result = compute_component_wise<dim, spacedim>(
      renumbering, dof_handler.begin_active(), dof_handler.end(), component_order_arg, false);
    (void)result;

    // If we don't have a renumbering (i.e., when there is 1 component) then
    // return
    if (Utilities::MPI::max(renumbering.size(), dof_handler.get_communicator()) == 0)
      return;
    ...
    Assert((result == dof_handler.n_locally_owned_dofs()) ||
             ((dof_handler.n_locally_owned_dofs() < dof_handler.n_dofs()) && (result <= dof_handler.n_dofs())),
           ExcInternalError());
                 ...
                 const std::vector<unsigned int> &component_order_arg)
  {
    ...
    std::vector<types::global_dof_index> renumbering(dof_handler.locally_owned_mg_dofs(level).n_elements(),
                                                     numbers::invalid_dof_index);
    ...
    const types::global_dof_index result =
      compute_component_wise<dim, spacedim>(renumbering, start, end, component_order_arg, true);
    (void)result;

    Assert(result == 0 || result == dof_handler.n_dofs(level), ExcInternalError());
  ...
  {
    const hp::FECollection<dim, spacedim> &fe_collection = start->get_dof_handler().get_fe_collection();
    ...
    const IndexSet &locally_owned_dofs = is_level_operation ?
    ...
    // take care of that
    std::vector<std::vector<types::global_dof_index>> component_to_dof_map(fe_collection.n_components());
    for (CellIterator cell = start; cell != end; ++cell)
      {
        ...
        const unsigned int    dofs_per_cell = fe_collection[fe_index].n_dofs_per_cell();
    ...
    if (const parallel::TriangulationBase<dim, spacedim> *tria =
            ...
            &start->get_dof_handler().get_triangulation())))
      {
        ...
        Utilities::MPI::sum(local_dof_count, tria->get_communicator(), global_dof_count);
    ...
    for (unsigned int component = 0; component < fe_collection.n_components(); ++component)
      {
        ...
        for (const types::global_dof_index dof_index : component_to_dof_map[component])
          {
            Assert(locally_owned_dofs.index_within_set(dof_index) < new_indices.size(), ExcInternalError());
            new_indices[locally_owned_dofs.index_within_set(dof_index)] = next_free_index;
  ...
  block_wise(DoFHandler<dim, spacedim> &dof_handler)
  {
    ...
    const types::global_dof_index result = compute_block_wise<dim,
                                                              spacedim,
  ...
  block_wise(DoFHandler<dim, spacedim> &dof_handler, const unsigned int level)
  {
    ...
    const types::global_dof_index result =
      compute_block_wise<dim,
                         spacedim,
                         typename DoFHandler<dim, spacedim>::level_cell_iterator,
                         typename DoFHandler<dim, spacedim>::level_cell_iterator>(renumbering, start, end, true);
    (void)result;
                     ...
                     const bool                            is_level_operation)
  {
    ...
    for (ITERATOR cell = start; cell != end; ++cell)
      {
        ...
        if (is_level_operation)
          Assert(cell->level() == start->level(),
                 ExcMessage("Multigrid renumbering in compute_block_wise() needs to be applied to a single level!"));
    ...
    for (unsigned int block = 0; block < fe_collection.n_blocks(); ++block)
      {
        ...
        block_to_dof_map[block].erase(std::unique(block_to_dof_map[block].begin(), block_to_dof_map[block].end()),
    ...
    for (unsigned int block = 0; block < fe_collection.n_blocks(); ++block)
      {
        const typename std::vector<types::global_dof_index>::const_iterator begin_of_component =
        ...
        for (typename std::vector<types::global_dof_index>::const_iterator dof_index = begin_of_component;
      ...
      if (cell->has_children())
        {
          ...
          for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; ++c)
            current_next_free_dof_offset = compute_hierarchical_recursive<dim>(
              current_next_free_dof_offset, my_starting_index, cell->child(c), locally_owned_dof_indices, new_indices);
        }
      else
        {
  ...
  hierarchical(DoFHandler<dim, spacedim> &dof_handler)
  {
    ...
    if (const parallel::TriangulationBase<dim, spacedim> *tria =
          dynamic_cast<const parallel::TriangulationBase<dim, spacedim> *>(&dof_handler.get_triangulation()))
      {
#ifdef DEAL_II_WITH_MPI
        types::global_dof_index locally_owned_size = dof_handler.locally_owned_dofs().n_elements();
        const int               ierr               = MPI_Exscan(
          &locally_owned_size, &my_starting_index, 1, DEAL_II_DOF_INDEX_MPI_TYPE, MPI_SUM, tria->get_communicator());
        AssertThrowMPI(ierr);
#endif
      }
    ...
    if (const parallel::distributed::Triangulation<dim, spacedim> *tria =
          dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(&dof_handler.get_triangulation()))
      {
        ...
        for (unsigned int c = 0; c < tria->n_cells(0); ++c)
          {
            const unsigned int coarse_cell_index = tria->get_p4est_tree_to_coarse_cell_permutation()[c];

            const typename DoFHandler<dim, spacedim>::level_cell_iterator this_cell(tria,
                                                                                    0,
                                                                                    coarse_cell_index,
                                                                                    &dof_handler);
    ...
    Assert(std::find(renumbering.begin(), renumbering.end(), numbers::invalid_dof_index) == renumbering.end(),
           ExcInternalError());
  ...
  template <int dim, int spacedim>
  void
  sort_selected_dofs_back(DoFHandler<dim, spacedim> &dof_handler, const std::vector<bool> &selected_dofs)
  {
    std::vector<types::global_dof_index> renumbering(dof_handler.n_dofs(), numbers::invalid_dof_index);
                          ...
                          const unsigned int         level)
  {
    ...
    std::vector<types::global_dof_index> renumbering(dof_handler.n_dofs(level), numbers::invalid_dof_index);
                                  ...
                                  const std::vector<bool>              &selected_dofs)
  {
    ...
    const types::global_dof_index n_selected_dofs = std::count(selected_dofs.begin(), selected_dofs.end(), false);
                                  ...
                                  const unsigned int                    level)
  {
    ...
    const unsigned int n_selected_dofs = std::count(selected_dofs.begin(), selected_dofs.end(), false);
  ...
  cell_wise(DoFHandler<dim, spacedim>                                                   &dof,
            const std::vector<typename DoFHandler<dim, spacedim>::active_cell_iterator> &cells)
  {
    std::vector<types::global_dof_index> renumbering(dof.n_locally_owned_dofs());
  ...
  template <int dim, int spacedim>
  void
  compute_cell_wise(std::vector<types::global_dof_index>                                                 &new_indices,
        ...
        for (const auto dof : cell_dofs)
          {
            ...
            if (local_dof != numbers::invalid_dof_index && !already_sorted[local_dof])
              {
                    ...
                    const typename std::vector<typename DoFHandler<dim, spacedim>::level_cell_iterator> &cells)
  {
    ...
    Assert(new_order.size() == dof.n_dofs(level), ExcDimensionMismatch(new_order.size(), dof.n_dofs(level)));
    Assert(reverse.size() == dof.n_dofs(level), ExcDimensionMismatch(reverse.size(), dof.n_dofs(level)));
  ...
  template <int dim, int spacedim>
  void
  downstream(DoFHandler<dim, spacedim> &dof, const Tensor<1, spacedim> &direction, const bool dof_wise_renumbering)
  {
                     ...
                     const bool                            dof_wise_renumbering)
  {
    ...
    if (dof_wise_renumbering == false)
      {
        std::vector<typename DoFHandler<dim, spacedim>::active_cell_iterator> ordered_cells;
        ordered_cells.reserve(dof.get_triangulation().n_active_cells());
        const CompareDownstream<typename DoFHandler<dim, spacedim>::active_cell_iterator, spacedim> comparator(
          direction);
    ...
    else
      {
        ...
        std::vector<std::pair<Point<spacedim>, unsigned int>> support_point_list(n_dofs);

        const hp::FECollection<dim> &fe_collection = dof.get_fe_collection();
        Assert(fe_collection[0].has_support_points(), typename FiniteElement<dim>::ExcFEHasNoSupportPoints());
        hp::QCollection<dim> quadrature_collection;
        for (unsigned int comp = 0; comp < fe_collection.size(); ++comp)
          {
            Assert(fe_collection[comp].has_support_points(), typename FiniteElement<dim>::ExcFEHasNoSupportPoints());
            quadrature_collection.push_back(Quadrature<dim>(fe_collection[comp].get_unit_support_points()));
          }
        hp::FEValues<dim, spacedim> hp_fe_values(fe_collection, quadrature_collection, update_quadrature_points);
                     ...
                     const bool                            dof_wise_renumbering)
  {
    if (dof_wise_renumbering == false)
      {
        std::vector<typename DoFHandler<dim, spacedim>::level_cell_iterator> ordered_cells;
        ordered_cells.reserve(dof.get_triangulation().n_cells(level));
        const CompareDownstream<typename DoFHandler<dim, spacedim>::level_cell_iterator, spacedim> comparator(
          direction);

        typename DoFHandler<dim, spacedim>::level_cell_iterator p   = dof.begin(level);
        typename DoFHandler<dim, spacedim>::level_cell_iterator end = dof.end(level);
      ...
      {
        Assert(dof.get_fe().has_support_points(), typename FiniteElement<dim>::ExcFEHasNoSupportPoints());
        const unsigned int                                    n_dofs = dof.n_dofs(level);
        std::vector<std::pair<Point<spacedim>, unsigned int>> support_point_list(n_dofs);

        Quadrature<dim>         q_dummy(dof.get_fe().get_unit_support_points());
        FEValues<dim, spacedim> fe_values(dof.get_fe(), q_dummy, update_quadrature_points);

        std::vector<bool> already_touched(dof.n_dofs(), false);

        const unsigned int                                      dofs_per_cell = dof.get_fe().n_dofs_per_cell();
        std::vector<types::global_dof_index>                    local_dof_indices(dofs_per_cell);
        typename DoFHandler<dim, spacedim>::level_cell_iterator begin = dof.begin(level);
        typename DoFHandler<dim, spacedim>::level_cell_iterator end   = dof.end(level);
        for (; begin != end; ++begin)
          {
            const typename Triangulation<dim, spacedim>::cell_iterator &begin_tria = begin;
  ...
  namespace internal
  {
    ...
    private:
      ...
      bool
      compare(const DHCellIterator &c1, const DHCellIterator &c2, std::integral_constant<int, xdim>) const
      {
  ...
  template <int dim, int spacedim>
  void
  clockwise_dg(DoFHandler<dim, spacedim> &dof, const Point<spacedim> &center, const bool counter)
  {
    std::vector<types::global_dof_index> renumbering(dof.n_dofs());
  ...
  template <int dim, int spacedim>
  void
  compute_random(std::vector<types::global_dof_index> &new_indices, const DoFHandler<dim, spacedim> &dof_handler)
  {
    const types::global_dof_index n_dofs = dof_handler.n_dofs();
    Assert(new_indices.size() == n_dofs, ExcDimensionMismatch(new_indices.size(), n_dofs));

    std::iota(new_indices.begin(), new_indices.end(), types::global_dof_index(0));
    ...
    for (unsigned int i = 1; i < n_dofs; ++i)
      {
        ...
        const unsigned int j = ::boost::random::uniform_int_distribution<>(0, i)(random_number_generator);
  ...
  {
    Assert((!dynamic_cast<const parallel::TriangulationBase<dim, spacedim> *>(&dof_handler.get_triangulation())),
                         ...
                         const DoFHandler<dim, spacedim>      &dof_handler)
  {
    ...
    const unsigned int n_subdomains = *std::max_element(subdomain_association.begin(), subdomain_association.end()) + 1;
  ...
  template <int dim, int spacedim>
  void
  compute_support_point_wise(std::vector<types::global_dof_index> &new_dof_indices,
                             const DoFHandler<dim, spacedim>      &dof_handler)
  {
    ...
    // Step 1:
    std::vector<types::global_dof_index> component_renumbering(n_dofs, numbers::invalid_dof_index);
    compute_component_wise<dim, spacedim>(
      component_renumbering, dof_handler.begin_active(), dof_handler.end(), std::vector<unsigned int>(), false);
    ...
    const types::global_dof_index dofs_per_component       = dof_handler.n_dofs() / n_components;
    const types::global_dof_index local_dofs_per_component = dof_handler.n_locally_owned_dofs() / n_components;
    ...
    std::vector<types::global_dof_index> component_dofs(local_dofs_per_component);
    for (unsigned int component = 0; component < n_components; ++component)
      {
        for (std::size_t i = 0; i < local_dofs_per_component; ++i)
          component_dofs[i] = component_renumbering[n_components * i + component];
        component_renumbered_dofs.add_indices(component_dofs.begin(), component_dofs.end());
      }
    component_renumbered_dofs.compress();
#ifdef DEBUG
    {
      IndexSet component_renumbered_dofs2(dof_handler.n_dofs());
      component_renumbered_dofs2.add_indices(component_renumbering.begin(), component_renumbering.end());
      Assert(component_renumbered_dofs2 == component_renumbered_dofs, ExcInternalError());
    }
#endif
    for (const FiniteElement<dim, spacedim> &fe : dof_handler.get_fe_collection())
      {
        AssertThrow(fe.dofs_per_cell == 0 || fe.has_support_points(), ExcNotImplemented());
        for (unsigned int i = 0; i < fe.n_base_elements(); ++i)
          AssertThrow(fe.base_element(0).get_unit_support_points() == fe.base_element(i).get_unit_support_points(),
                      ExcMessage("All base elements should have the same support points."));
      }

    std::vector<types::global_dof_index> component_to_nodal(dof_handler.n_locally_owned_dofs(),
                                                            numbers::invalid_dof_index);
          ...
          for (unsigned int i = 0; i < fe.dofs_per_cell; ++i)
            {
              if (locally_owned_dofs.is_element(cell_dofs[i]))
                {
                  const auto local_index            = locally_owned_dofs.index_within_set(cell_dofs[i]);
                  component_renumbered_cell_dofs[i] = component_renumbering[local_index];
                }
              else
                {
                  component_renumbered_cell_dofs[i] = numbers::invalid_dof_index;
                }
            }
          ...
          for (unsigned int i = 0; i < fe.dofs_per_cell; ++i)
            {
              if (fe.system_to_component_index(i).first == 0 &&
                  component_renumbered_dofs.is_element(component_renumbered_cell_dofs[i]))
                {
                  for (unsigned int component = 0; component < fe.n_components(); ++component)
                    {
                      ...
                      const auto local_index = component_renumbered_dofs.index_within_set(
                        component_renumbered_cell_dofs[i] + dofs_per_component * component);

                      if (component_to_nodal[local_index] == numbers::invalid_dof_index)
                        {
    ...
    for (std::size_t i = 0; i < dof_handler.n_locally_owned_dofs(); ++i)
      {
        const auto local_index = component_renumbered_dofs.index_within_set(component_renumbering[i]);
  ...
  {
    const std::vector<types::global_dof_index> new_global_numbers =
      compute_matrix_free_data_locality(dof_handler, constraints, matrix_free_data);
  ...
  compute_matrix_free_data_locality(const DoFHandler<dim, spacedim> &dof_handler,
                                    const AffineConstraints<Number> &constraints,
                                    const AdditionalDataType        &matrix_free_data)
  {
    ...
    separate_matrix_free.reinit(dealii::MappingQ1<dim>(), dof_handler, constraints, dealii::QGauss<1>(2), my_mf_data);
  ...
  namespace
  {
    ...
    group_dofs_by_rank_access(const dealii::Utilities::MPI::Partitioner &partitioner)
    {
      ...
      const std::vector<std::pair<unsigned int, unsigned int>> &import_targets = partitioner.import_targets();
      ...
      std::map<std::vector<unsigned int>,
        ...
        dofs_by_rank{[](const std::vector<unsigned int> &a, const std::vector<unsigned int> &b) {
          if (a.size() < b.size())
            return true;
          if (a.size() == b.size())
            {
    ...
    compute_mf_numbering(const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free, const unsigned int component)
    {
      const IndexSet    &owned_dofs = matrix_free.get_dof_info(component).vector_partitioner->locally_owned_range();
      const unsigned int n_comp     = matrix_free.get_dof_handler(component).get_fe().n_components();
      Assert(matrix_free.get_dof_handler(component).get_fe().n_base_elements() == 1, ExcNotImplemented());
      Assert(dynamic_cast<const FE_Q_Base<dim> *>(&matrix_free.get_dof_handler(component).get_fe().base_element(0)),
             ExcNotImplemented("Matrix-free renumbering only works for "
                               "FE_Q elements"));

      const unsigned int fe_degree = matrix_free.get_dof_handler(component).get_fe().degree;
      ...
      if (dim == 1)
      ...
      else if (dim == 3)
        {
          ...
          dofs_on_objects[4]  = std::make_pair(n_comp * (8 + 12 * nn + 4 * nn * nn), nn * nn);
          ...
          dofs_on_objects[10] = std::make_pair(n_comp * (8 + 12 * nn + 2 * nn * nn), nn * nn);
          dofs_on_objects[11] = std::make_pair(n_comp * (8 + 9 * nn), nn);
          dofs_on_objects[12] = std::make_pair(n_comp * (8 + 12 * nn), nn * nn);
          dofs_on_objects[13] = std::make_pair(n_comp * (8 + 12 * nn + 6 * nn * nn), nn * nn * nn);
          dofs_on_objects[14] = std::make_pair(n_comp * (8 + 12 * nn + 1 * nn * nn), nn * nn);
          dofs_on_objects[15] = std::make_pair(n_comp * (8 + 10 * nn), nn);
          dofs_on_objects[16] = std::make_pair(n_comp * (8 + 12 * nn + 3 * nn * nn), nn * nn);
          ...
          dofs_on_objects[22] = std::make_pair(n_comp * (8 + 12 * nn + 5 * nn * nn), nn * nn);
      ...
      const auto renumber_func = [](const types::global_dof_index dof_index,
        ...
        const types::global_dof_index local_dof_index = owned_dofs.index_within_set(dof_index);
      ...
      std::vector<types::global_dof_index> dof_indices(matrix_free.get_dof_handler(component).get_fe().dofs_per_cell);
      ...
      std::vector<unsigned int>  dof_numbers_mf_order(n_owned_dofs, dealii::numbers::invalid_unsigned_int);
      std::vector<unsigned int>  last_touch_by_cell_batch_range(n_owned_dofs, dealii::numbers::invalid_unsigned_int);
      std::vector<unsigned char> touch_count(n_owned_dofs);

      const auto operation_on_cell_range = [&](const MatrixFree<dim, Number, VectorizedArrayType> &data,
                                               unsigned int &,
                                               const unsigned int &,
        ...
        for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
          {
            ...
            for (unsigned int v = 0; v < data.n_active_entries_per_cell_batch(cell); ++v)
              {
                ...
                if (data.get_mg_level() == numbers::invalid_unsigned_int)
                  data.get_cell_iterator(cell, v, component)->get_dof_indices(dof_indices);
                else
                  data.get_cell_iterator(cell, v, component)->get_mg_dof_indices(dof_indices);
                      ...
                      for (unsigned int i1 = 0; i1 < nn; ++i1)
                        for (unsigned int i0 = 0; i0 < nn; ++i0)
                          for (unsigned int c = 0; c < n_comp; ++c)
                            renumber_func(dof_indices[r.first + r.second * c + i1 + i0 * nn],
                                          owned_dofs,
            ...
            data.get_dof_info(component).get_dof_indices_on_cell_batch(dofs_extracted, cell);
            for (const unsigned int dof_index : dofs_extracted)
              if (dof_index < n_owned_dofs && last_touch_by_cell_batch_range[dof_index] != cell_range.first)
                {
      ...
      Assert(matrix_free.get_task_info().scheme == dealii::internal::MatrixFreeFunctions::TaskInfo::none,
             ExcNotImplemented("Renumbering only available for non-threaded "
                               "version of MatrixFree::cell_loop"));

      matrix_free.template cell_loop<unsigned int, unsigned int>(operation_on_cell_range,
                                                                 counter_dof_numbers,
                                                                 counter_dof_numbers);
                                    ...
                                    const MatrixFree<dim, Number, VectorizedArrayType> &matrix_free)
  {
    ...
    Assert(component < matrix_free.n_components(), ExcMessage("Could not locate the given DoFHandler in MatrixFree"));
    ...
    const std::vector<std::vector<unsigned int>> dofs_by_rank_access =
      group_dofs_by_rank_access(*matrix_free.get_dof_info(component).vector_partitioner);

    const auto &[local_numbering, touch_count] = compute_mf_numbering(matrix_free, component);
    ...
    std::vector<dealii::types::global_dof_index> new_global_numbers(locally_owned_size);
    for (unsigned int i = 0; i < locally_owned_size; ++i)
      new_global_numbers[i] = owned_dofs.nth_index_in_set(numbering_categories[local_numbering[i]]);

...
namespace internal
{
  namespace DoFHandlerImplementation
  {
    ...
    NumberCache::NumberCache(const std::vector<IndexSet> &locally_owned_dofs_per_processor, const unsigned int my_rank)
    ...
    NumberCache::get_n_locally_owned_dofs_per_processor(const MPI_Comm mpi_communicator) const
    {
      if (n_global_dofs == 0)
        return std::vector<types::global_dof_index>();
      else if (n_locally_owned_dofs_per_processor.empty() == false)
        {
          AssertDimension(n_locally_owned_dofs_per_processor.size(), Utilities::MPI::n_mpi_processes(mpi_communicator));
          return n_locally_owned_dofs_per_processor;
        }
      else
        {
          return Utilities::MPI::all_gather(mpi_communicator, n_locally_owned_dofs);
        }
    }
    ...
    std::vector<IndexSet>
    NumberCache::get_locally_owned_dofs_per_processor(const MPI_Comm mpi_communicator) const
    {
      ...
      if (n_global_dofs == 0)
        ...
        {
          AssertDimension(locally_owned_dofs_per_processor.size(), Utilities::MPI::n_mpi_processes(mpi_communicator));
          return locally_owned_dofs_per_processor;
        }
      else
        {
          return Utilities::MPI::all_gather(mpi_communicator, locally_owned_dofs);
        }
    }

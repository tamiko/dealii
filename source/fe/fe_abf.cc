...
FE_ABF<dim>::FE_ABF(const unsigned int deg)
  : FE_PolyTensor<dim>(PolynomialsABF<dim>(deg),
  ...
  , rt_order(deg)
{
  ...
  std::vector<FullMatrix<double>> face_embeddings(1 << (dim - 1),
...
FE_ABF<dim>::initialize_support_points(const unsigned int deg)
{
  ...
  interior_weights_abf.reinit(TableIndices<3>(cell_quadrature.size(), polynomials_abf[0]->n() * dim, dim));
  ...
  for (unsigned int k = 0; k < cell_quadrature.size(); ++k)
    {
      for (unsigned int i = 0; i < polynomials_abf[0]->n() * dim; ++i)
        {
          poly_grad =
            polynomials_abf[i % dim]->compute_grad(i / dim, cell_quadrature.point(k)) * cell_quadrature.weight(k);
...
FE_ABF<dim>::initialize_restriction()
{
  ...
  for (const unsigned int face : GeometryInfo<dim>::face_indices())
    {
      ...
      // ordered by point
      Table<2, double> cached_values_face(this->n_dofs_per_cell(), q_face.size());
              ...
              for (unsigned int i_face = 0; i_face < this->n_dofs_per_face(face); ++i_face)
                {
                  ...
                  this->restriction[iso][child](face * this->n_dofs_per_face(face) + i_face, i_child) +=
                    Utilities::fixed_power<dim - 1>(.5) * q_sub.weight(k) * cached_values_face(i_child, k) *
  ...
  // evaluation suffers if not
  // ordered by point
  Table<3, double> cached_values_cell(this->n_dofs_per_cell(), q_cell.size(), dim);
...
template <int dim>
bool
FE_ABF<dim>::has_support_on_face(const unsigned int shape_index, const unsigned int face_index) const
{
      ...
      for (unsigned int fp = 0; fp < n_face_points; ++fp)
        {
          ...
          // have to be in 3d
          unsigned int k =
            QProjector<dim>::DataSetDescriptor::face(this->reference_cell(), face, false, false, false, n_face_points);

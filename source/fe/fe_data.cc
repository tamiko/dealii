...
namespace internal
{
  ...
  expand(const unsigned int dim, const std::vector<unsigned int> &dofs_per_object, const ReferenceCell cell_type)
  {
    ...
    // first_line_index
    const unsigned int first_line_index = (cell_type.n_vertices() * dofs_per_vertex);
    result.object_index[1][0]           = first_line_index;

    // first_quad_index
    const unsigned int first_quad_index = (first_line_index + cell_type.n_lines() * dofs_per_line);
    result.object_index[2][0]           = first_quad_index;

    // first_hex_index
    result.object_index[3][0] =
      (first_quad_index + (dim == 2 ? 1 : (dim == 3 ? cell_type.n_faces() : 0)) * dofs_per_quad);

    // first_face_line_index
    result.first_object_index_on_face[1][0] = (cell_type.face_reference_cell(face_no).n_vertices() * dofs_per_vertex);

    // first_face_quad_index
    result.first_object_index_on_face[2][0] =
      ((dim == 3 ? cell_type.face_reference_cell(face_no).n_vertices() * dofs_per_vertex :
    ...
    result.dofs_per_object_inclusive[dim - 1][0] =
      (cell_type.face_reference_cell(face_no).n_vertices() * dofs_per_vertex +
       cell_type.face_reference_cell(face_no).n_lines() * dofs_per_line + (dim == 3 ? 1 : 0) * dofs_per_quad);


    // dofs_per_cell
    result.dofs_per_object_inclusive[dim][0] =
      (cell_type.n_vertices() * dofs_per_vertex + cell_type.n_lines() * dofs_per_line +
       (dim == 2 ? 1 : (dim == 3 ? cell_type.n_faces() : 0)) * dofs_per_quad + (dim == 3 ? 1 : 0) * dofs_per_hex);
  ...
  {
    if (std::all_of(vector.begin(), vector.end(), [&](const auto &e) { return e == vector.front(); }))
      {
        return 1;
      }
    else
...
FiniteElementData<dim>::FiniteElementData(const std::vector<unsigned int> &dofs_per_object,
  ...
  : FiniteElementData(dofs_per_object,
                      dim == 0 ? ReferenceCells::Vertex :
                                 (dim == 1 ? ReferenceCells::Line :
                                             (dim == 2 ? ReferenceCells::Quadrilateral : ReferenceCells::Hexahedron)),
                      n_components,
                      degree,
                      conformity,
                      block_indices)
...
FiniteElementData<dim>::FiniteElementData(const internal::GenericDoFsPerObject &data,
  ...
  , number_of_unique_2d_subobjects(internal::number_unique_entries(data.dofs_per_object_inclusive[2]))
  , number_unique_faces(internal::number_unique_entries(data.dofs_per_object_inclusive[dim - 1]))
  ...
  , dofs_per_quad_max(*max_element(n_dofs_on_quad.begin(), n_dofs_on_quad.end()))
  ...
  , dofs_per_face_max(*max_element(n_dofs_on_face.begin(), n_dofs_on_face.end()))
...
FiniteElementData<dim>::operator==(const FiniteElementData<dim> &f) const
{
  return ((dofs_per_vertex == f.dofs_per_vertex) && (dofs_per_line == f.dofs_per_line) &&
          (dofs_per_quad == f.dofs_per_quad) && (dofs_per_hex == f.dofs_per_hex) && (components == f.components) &&

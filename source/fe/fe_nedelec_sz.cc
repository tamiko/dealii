...
FE_NedelecSZ<dim, spacedim>::FE_NedelecSZ(const unsigned int order)
  : FiniteElement<dim, dim>(
      ...
      std::vector<ComponentMask>(compute_num_dofs(order), ComponentMask(std::vector<bool>(dim, true))))
{
...
template <int dim, int spacedim>
double
FE_NedelecSZ<dim, spacedim>::shape_value(const unsigned int /*i*/, const Point<dim> & /*p*/) const
{
...
template <int dim, int spacedim>
double
FE_NedelecSZ<dim, spacedim>::shape_value_component(const unsigned int /*i*/,
                                                   const Point<dim> & /*p*/,
...
template <int dim, int spacedim>
Tensor<1, dim>
FE_NedelecSZ<dim, spacedim>::shape_grad(const unsigned int /*i*/, const Point<dim> & /*p*/) const
{
  Assert(false, (typename FiniteElement<dim, spacedim>::ExcFENotPrimitive()));
  return Tensor<1, dim>();
}
...
template <int dim, int spacedim>
Tensor<1, dim>
FE_NedelecSZ<dim, spacedim>::shape_grad_component(const unsigned int /*i*/,
                                                  const Point<dim> & /*p*/,
                                                  const unsigned int /*component*/) const
{
  Assert(false, ExcNotImplemented());
...
template <int dim, int spacedim>
Tensor<2, dim>
FE_NedelecSZ<dim, spacedim>::shape_grad_grad(const unsigned int /*i*/, const Point<dim> & /*p*/) const
{
  Assert(false, (typename FiniteElement<dim, spacedim>::ExcFENotPrimitive()));
  return Tensor<2, dim>();
}
...
template <int dim, int spacedim>
Tensor<2, dim>
FE_NedelecSZ<dim, spacedim>::shape_grad_grad_component(const unsigned int /*i*/,
                                                       const Point<dim> & /*p*/,
                                                       const unsigned int /*component*/) const
{
  Assert(false, ExcNotImplemented());
...
{
  std::unique_ptr<typename dealii::FiniteElement<dim, spacedim>::InternalDataBase> data_ptr =
  ...
  if (flags & update_values)
    {
      data.shape_values.resize(this->n_dofs_per_cell(), std::vector<Tensor<1, dim>>(n_q_points));
    }

  if (flags & update_gradients)
    {
      data.shape_grads.resize(this->n_dofs_per_cell(), std::vector<DerivativeForm<1, dim, dim>>(n_q_points));
    }

  if (flags & update_hessians)
    {
      data.shape_hessians.resize(this->n_dofs_per_cell(), std::vector<DerivativeForm<2, dim, dim>>(n_q_points));
    }
      ...
      case 2:
        {
          ...
          //   sigma_imj_sign[i][j].
          // Helper arrays:
          const int    sigma_comp_signs[GeometryInfo<2>::vertices_per_cell][2] = {{-1, -1}, {1, -1}, {-1, 1}, {1, 1}};
              ...
              for (unsigned int j = 0; j < vertices_per_cell; ++j)
                {
                  ...
                  data.sigma_imj_grads[i][j][sigma_imj_component[i][j]] = 2.0 * sigma_imj_sign[i][j];
                }
            }
              ...
              for (unsigned int q = 0; q < n_q_points; ++q)
                {
                  ...
                  // derivatives.
                  const unsigned int poly_length = (flags & update_hessians) ? 4 : ((flags & update_gradients) ? 3 : 2);
                      ...
                      for (unsigned int j = 0; j < degree; ++j)
                        {
                          ...
                          for (unsigned int i = 0; i < degree; ++i)
                            {
                              ...
                              // Type 1:
                              const unsigned int dof_index1(cell_type1_offset + shift_ij);
                              data.shape_hessians[dof_index1][q][0][0][0] = 8.0 * polyx[i][3] * polyy[j][0];
                              data.shape_hessians[dof_index1][q][1][0][0] = 8.0 * polyx[i][2] * polyy[j][1];

                              data.shape_hessians[dof_index1][q][0][1][0] = data.shape_hessians[dof_index1][q][1][0][0];
                              data.shape_hessians[dof_index1][q][1][1][0] = 8.0 * polyx[i][1] * polyy[j][2];

                              data.shape_hessians[dof_index1][q][0][0][1] = data.shape_hessians[dof_index1][q][1][0][0];
                              data.shape_hessians[dof_index1][q][1][0][1] = data.shape_hessians[dof_index1][q][1][1][0];

                              data.shape_hessians[dof_index1][q][0][1][1] = data.shape_hessians[dof_index1][q][1][1][0];
                              data.shape_hessians[dof_index1][q][1][1][1] = 8.0 * polyx[i][0] * polyy[j][3];
                          ...
                          data.shape_hessians[dof_index3_1][q][0][1][1] = 4.0 * polyy[j][2];
                          ...
                          data.shape_hessians[dof_index3_2][q][1][0][0] = 4.0 * polyx[j][2];
                          ...
                          for (unsigned int k = 0; k < degree; ++k)
                            {
                              ...
                              const unsigned int shift_j(k * degree); // Used below when subbing k for j
                                                                      // (type 3)
                              for (unsigned int j = 0; j < degree; ++j)
                                {
                                  ...
                                  // convenience):
                                  const unsigned int shift_ij(j + shift_j); // here we've subbed j for i,
                                                                            // k for j.
                                      ...
                                      for (unsigned int d1 = 0; d1 < dim; ++d1)
                                        {
                                          for (unsigned int d2 = 0; d2 < dim; ++d2)
                                            {
                                              data.shape_hessians[dof_index2_1][q][0][d1][d2] =
                                                data.shape_hessians[dof_index1][q][0][d1][d2];
                                              data.shape_hessians[dof_index2_1][q][1][d1][d2] =
                                                -1.0 * data.shape_hessians[dof_index1][q][1][d1][d2];
                                              data.shape_hessians[dof_index2_1][q][2][d1][d2] =
                                                data.shape_hessians[dof_index1][q][2][d1][d2];

                                              data.shape_hessians[dof_index2_2][q][0][d1][d2] =
                                                data.shape_hessians[dof_index1][q][0][d1][d2];
                                              data.shape_hessians[dof_index2_2][q][1][d1][d2] =
                                                -1.0 * data.shape_hessians[dof_index1][q][1][d1][d2];
                                              data.shape_hessians[dof_index2_2][q][2][d1][d2] =
                                                -1.0 * data.shape_hessians[dof_index1][q][2][d1][d2];
                                            }
                                        }
                                    }
                                  // Type 3: (note we re-use k and j for
                                      ...
                                      for (unsigned int d2 = 0; d2 < dim; ++d2)
                                        {
                                          for (unsigned int d3 = 0; d3 < dim; ++d3)
                                            {
                                              data.shape_hessians[dof_index3_1][q][d1][d2][d3] = 0.0;
                                              data.shape_hessians[dof_index3_2][q][d1][d2][d3] = 0.0;
                                              data.shape_hessians[dof_index3_3][q][d1][d2][d3] = 0.0;
                                            }
                                        }
                                    }
                                  data.shape_hessians[dof_index3_1][q][0][1][1] = 4.0 * polyy[j][2] * polyz[k][0];
                                  data.shape_hessians[dof_index3_1][q][0][1][2] = 4.0 * polyy[j][1] * polyz[k][1];

                                  data.shape_hessians[dof_index3_1][q][0][2][1] =
                                    data.shape_hessians[dof_index3_1][q][0][1][2];
                                  data.shape_hessians[dof_index3_1][q][0][2][2] = 4.0 * polyy[j][0] * polyz[k][2];


                                  data.shape_hessians[dof_index3_2][q][1][0][0] = 4.0 * polyx[j][2] * polyz[k][0];
                                  data.shape_hessians[dof_index3_2][q][1][0][2] = 4.0 * polyx[j][1] * polyz[k][1];

                                  data.shape_hessians[dof_index3_2][q][1][2][0] =
                                    data.shape_hessians[dof_index3_2][q][1][0][2];
                                  data.shape_hessians[dof_index3_2][q][1][2][2] = 4.0 * polyx[j][0] * polyz[k][2];


                                  data.shape_hessians[dof_index3_3][q][2][0][0] = 4.0 * polyx[j][2] * polyy[k][0];
                                  data.shape_hessians[dof_index3_3][q][2][0][1] = 4.0 * polyx[j][1] * polyy[k][1];

                                  data.shape_hessians[dof_index3_3][q][2][1][0] =
                                    data.shape_hessians[dof_index3_3][q][2][0][1];
                                  data.shape_hessians[dof_index3_3][q][2][1][1] = 4.0 * polyx[j][0] * polyy[k][2];
                                }
                            }
                        }
                    }
...
template <int dim, int spacedim>
void
FE_NedelecSZ<dim, spacedim>::fill_edge_values(const typename Triangulation<dim, dim>::cell_iterator &cell,
                                              const Quadrature<dim>                                 &quadrature,
                                              const InternalData                                    &fe_data) const
{
  ...
  Assert(!(flags & update_values) || fe_data.shape_values.size() == this->n_dofs_per_cell(),
         ExcDimensionMismatch(fe_data.shape_values.size(), this->n_dofs_per_cell()));
  ...
  std::vector<std::vector<unsigned int>> edge_order(lines_per_cell, std::vector<unsigned int>(vertices_per_line));
                          ...
                          for (unsigned int i = 1; i < degree + 1; ++i)
                            {
                              ...
                              fe_data.shape_grads[dof_index][q][0][0] = edge_sigma_grads[m][0] *
                              ...
                              fe_data.shape_grads[dof_index][q][1][1] = edge_sigma_grads[m][1] *
                              ...
                              for (unsigned int d2 = 0; d2 < dim; ++d2)
                                {
                                  for (unsigned int d3 = 0; d3 < dim; ++d3)
                                    {
                                      fe_data.shape_hessians[shift_m][q][d1][d2][d3] = 0;
                                    }
                                }
                            }
                                  ...
                                  for (unsigned int d2 = 0; d2 < dim; ++d2)
                                    {
                                      for (unsigned int d3 = 0; d3 < dim; ++d3)
                                        {
                                          fe_data.shape_hessians[dof_index][q][d1][d2][d3] =
                                            edge_sigma_grads[m][d1] * edge_sigma_grads[m][d2] *
                                              edge_sigma_grads[m][d3] * poly[i][3] * edge_lambda_values[m][q] +
                                            poly[i][2] * (edge_sigma_grads[m][d1] * edge_sigma_grads[m][d2] *
                                                            edge_lambda_grads[m][d3] +
                                                          edge_sigma_grads[m][d3] * edge_sigma_grads[m][d1] *
                                                            edge_lambda_grads[m][d2] +
                                                          edge_sigma_grads[m][d3] * edge_sigma_grads[m][d2] *
                              ...
                              for (unsigned int i = 0; i < degree; ++i)
                                {
                                  ...
                                  for (unsigned int d = 0; d < dim; ++d)
                                    {
                                      fe_data.shape_values[dof_index][q][d] =
                                        edge_sigma_grads[m][d] * poly[i][1] * edge_lambda_values[m][q] +
                                      ...
                                      for (unsigned int d2 = 0; d2 < dim; ++d2)
                                        {
                                          fe_data.shape_grads[dof_index][q][d1][d2] =
                                            edge_sigma_grads[m][d1] * edge_sigma_grads[m][d2] * poly[i][2] *
                                              edge_lambda_values[m][q] +
                                            (edge_sigma_grads[m][d1] * edge_lambda_grads[m][q][d2] +
                                             edge_sigma_grads[m][d2] * edge_lambda_grads[m][q][d1]) *
                                              poly[i][1] +
                              ...
                              for (unsigned int d2 = 0; d2 < dim; ++d2)
                                {
                                  for (unsigned int d3 = 0; d3 < dim; ++d3)
                                    {
                                      fe_data.shape_hessians[shift_m][q][d1][d2][d3] =
                                        0.5 * edge_sigma_grads[m][d1] * edge_lambda_gradgrads_3d[m][d3][d2];
                                    }
                                }
                            }
                                          ...
                                          for (unsigned int d3 = 0; d3 < dim; ++d3)
                                            {
                                              fe_data.shape_hessians[dof_index][q][d1][d2][d3] =
                                                edge_sigma_grads[m][d1] * edge_sigma_grads[m][d2] *
                                                  edge_sigma_grads[m][d3] * poly[i][3] * edge_lambda_values[m][q] +
                                                poly[i][2] * (edge_sigma_grads[m][d1] * edge_sigma_grads[m][d2] *
                                                                edge_lambda_grads[m][q][d3] +
                                                              edge_sigma_grads[m][d3] * edge_sigma_grads[m][d1] *
                                                                edge_lambda_grads[m][q][d2] +
                                                              edge_sigma_grads[m][d3] * edge_sigma_grads[m][d2] *
                                                                edge_lambda_grads[m][q][d1]) +
                                                poly[i][1] *
                                                  (edge_sigma_grads[m][d1] * edge_lambda_gradgrads_3d[m][d3][d2] +
                                                   edge_sigma_grads[m][d2] * edge_lambda_gradgrads_3d[m][d3][d1] +
                                                   edge_sigma_grads[m][d3] * edge_lambda_gradgrads_3d[m][d2][d1]);
                                            }
                                        }
                                    }
                                }
...
template <int dim, int spacedim>
void
FE_NedelecSZ<dim, spacedim>::fill_face_values(const typename Triangulation<dim, dim>::cell_iterator &cell,
                                              const Quadrature<dim>                                 &quadrature,
                                              const InternalData                                    &fe_data) const
{
      ...
      if (flags & (update_values | update_gradients | update_hessians))
        {
          ...
          // DoF info:
          const unsigned int n_line_dofs = this->n_dofs_per_line() * GeometryInfo<dim>::lines_per_cell;
          ...
          const unsigned int vertex_opposite_on_face[GeometryInfo<3>::vertices_per_face] = {3, 2, 1, 0};

          const unsigned int vertices_adjacent_on_face[GeometryInfo<3>::vertices_per_face][2] = {{1, 2},
                                                                                                 {0, 3},
                                                                                                 {0, 3},
                                                                                                 {1, 2}};
          ...
          for (unsigned int m = 0; m < faces_per_cell; ++m)
            {
              ...
              if (cell->vertex_index(
                    GeometryInfo<dim>::face_to_cell_vertices(m, vertices_adjacent_on_face[current_max][0])) >
                  cell->vertex_index(
                    GeometryInfo<dim>::face_to_cell_vertices(m, vertices_adjacent_on_face[current_max][1])))
                {
          ...
          std::vector<std::vector<double>> face_xi_values(faces_per_cell, std::vector<double>(n_q_points));
          std::vector<std::vector<double>> face_xi_grads(faces_per_cell, std::vector<double>(dim));
          std::vector<std::vector<double>> face_eta_values(faces_per_cell, std::vector<double>(n_q_points));
          std::vector<std::vector<double>> face_eta_grads(faces_per_cell, std::vector<double>(dim));

          std::vector<std::vector<double>> face_lambda_values(faces_per_cell, std::vector<double>(n_q_points));
          std::vector<std::vector<double>> face_lambda_grads(faces_per_cell, std::vector<double>(dim));
          for (unsigned int m = 0; m < faces_per_cell; ++m)
            {
              for (unsigned int q = 0; q < n_q_points; ++q)
                {
                                  ...
                                  for (unsigned int d2 = 0; d2 < dim; ++d2)
                                    {
                                      fe_data.shape_grads[dof_index1][q][d1][d2] =
                                        (face_xi_grads[m][d1] * face_xi_grads[m][d2] * polyxi[i][2] * polyeta[j][0] +
                              ...
                              for (unsigned int d2 = 0; d2 < dim; ++d2)
                                {
                                  fe_data.shape_grads[dof_index3_1][q][d1][d2] =
                                    face_xi_grads[m][d1] *
                                    (face_eta_grads[m][d2] * polyeta[j][1] * face_lambda_values[m][q] +
                                     face_lambda_grads[m][d2] * polyeta[j][0]);

                                  fe_data.shape_grads[dof_index3_2][q][d1][d2] =
                                    face_eta_grads[m][d1] *
                                    (face_xi_grads[m][d2] * polyxi[j][1] * face_lambda_values[m][q] +
                                  ...
                                  for (unsigned int d2 = 0; d2 < dim; ++d2)
                                    {
                                      for (unsigned int d3 = 0; d3 < dim; ++d3)
                                        {
                                          fe_data.shape_hessians[dof_index1][q][d1][d2][d3] =
                                            polyxi[i][1] * face_xi_grads[m][d3] *
                                              (face_eta_grads[m][d1] *
                                                 (polyeta[j][2] * face_eta_grads[m][d2] * face_lambda_values[m][q] +
                                                  polyeta[j][1] * face_lambda_grads[m][d2]) +
                                               polyeta[j][1] * face_eta_grads[m][d2] * face_lambda_grads[m][d1]) +
                                            polyxi[i][0] *
                                              (polyeta[j][3] * face_eta_grads[m][d1] * face_eta_grads[m][d2] *
                                                 face_eta_grads[m][d3] * face_lambda_values[m][q] +
                                               polyeta[j][2] * (face_eta_grads[m][d1] * face_eta_grads[m][d2] *
                                                                  face_lambda_grads[m][d3] +
                                                                face_eta_grads[m][d3] *
                                                                  (face_eta_grads[m][d1] * face_lambda_grads[m][d2] +
                                                                   face_eta_grads[m][d2] * face_lambda_grads[m][d1]))) +
                                            (polyxi[i][1] * polyeta[j][1] * face_eta_grads[m][d3] +
                                             polyxi[i][2] * polyeta[j][0] * face_xi_grads[m][d3]) *
                                              (face_xi_grads[m][d1] * face_lambda_grads[m][d2] +
                                               face_xi_grads[m][d2] * face_lambda_grads[m][d1]) +
                                            face_lambda_grads[m][d3] *
                                              (polyxi[i][2] * polyeta[j][0] * face_xi_grads[m][d1] *
                                                 face_xi_grads[m][d2] +
                                               polyxi[i][1] * polyeta[j][1] *
                                                 (face_xi_grads[m][d1] * face_eta_grads[m][d2] +
                                                  face_xi_grads[m][d2] * face_eta_grads[m][d1])) +
                                            face_lambda_values[m][q] *
                                              (polyxi[i][3] * polyeta[j][0] * face_xi_grads[m][d1] *
                                                 face_xi_grads[m][d2] * face_xi_grads[m][d3] +
                                               polyxi[i][1] * polyeta[j][2] * face_eta_grads[m][d3] *
                                                 (face_xi_grads[m][d1] * face_eta_grads[m][d2] +
                                                  face_xi_grads[m][d2] * face_eta_grads[m][d1]) +
                                               polyxi[i][2] * polyeta[j][1] *
                                                 (face_xi_grads[m][d3] * face_xi_grads[m][d2] * face_eta_grads[m][d1] +
                                                  face_xi_grads[m][d1] *
                                                    (face_xi_grads[m][d2] * face_eta_grads[m][d3] +
                                                     face_xi_grads[m][d3] * face_eta_grads[m][d2])));
                                        }
                                    }
                                }
                                  ...
                                  for (unsigned int d2 = 0; d2 < dim; ++d2)
                                    {
                                      for (unsigned int d3 = 0; d3 < dim; ++d3)
                                        {
                                          fe_data.shape_hessians[dof_index2][q][d1][d2][d3] =
                                            face_xi_grads[m][d1] *
                                              (polyxi[i][1] * polyeta[j][1] *
                                                 (face_eta_grads[m][d2] * face_lambda_grads[m][d3] +
                                                  face_eta_grads[m][d3] * face_lambda_grads[m][d2]) +
                                               polyxi[i][2] * polyeta[j][0] *
                                                 (face_xi_grads[m][d2] * face_lambda_grads[m][d3] +
                                                  face_xi_grads[m][d3] * face_lambda_grads[m][d2]) +
                                               face_lambda_values[m][q] *
                                                 (face_eta_grads[m][d2] *
                                                    (polyxi[i][1] * polyeta[j][2] * face_eta_grads[m][d3] +
                                                     polyxi[i][2] * polyeta[j][1] * face_xi_grads[m][d3]) +
                                                  face_xi_grads[m][d2] *
                                                    (polyxi[i][2] * polyeta[j][1] * face_eta_grads[m][d3] +
                                                     polyxi[i][3] * polyeta[j][0] * face_xi_grads[m][d3]))) -
                                            polyxi[i][0] * face_eta_grads[m][d1] *
                                              (face_eta_grads[m][d2] *
                                                 (polyeta[j][3] * face_eta_grads[m][d3] * face_lambda_values[m][q] +
                                                  polyeta[j][2] * face_lambda_grads[m][d3]) +
                                               polyeta[j][2] * face_eta_grads[m][d3] * face_lambda_grads[m][d2]) -
                                            face_eta_grads[m][d1] *
                                              (polyxi[i][1] * face_xi_grads[m][d3] *
                                                 (polyeta[j][2] * face_eta_grads[m][d2] * face_lambda_values[m][q] +
                                                  polyeta[j][1] * face_lambda_grads[m][d2]) +
                                               face_xi_grads[m][d2] *
                                                 (polyxi[i][1] *
                                                    (polyeta[j][2] * face_eta_grads[m][d3] * face_lambda_values[m][q] +
                                                     polyeta[j][1] * face_lambda_grads[m][d3]) +
                                                  polyxi[i][2] * polyeta[j][1] * face_xi_grads[m][d3] *
                                                    face_lambda_values[m][q]));
                                        }
                                    }
                              ...
                              for (unsigned int d2 = 0; d2 < dim; ++d2)
                                {
                                  for (unsigned int d3 = 0; d3 < dim; ++d3)
                                    {
                                      fe_data.shape_hessians[dof_index3_1][q][d1][d2][d3] =
                                        face_xi_grads[m][d1] *
                                        (face_eta_grads[m][d2] *
                                           (polyeta[j][2] * face_eta_grads[m][d3] * face_lambda_values[m][q] +
                                            polyeta[j][1] * face_lambda_grads[m][d3]) +
                                         face_lambda_grads[m][d2] * polyeta[j][1] * face_eta_grads[m][d3]);

                                      fe_data.shape_hessians[dof_index3_2][q][d1][d2][d3] =
                                        face_eta_grads[m][d1] *
                                        (face_xi_grads[m][d2] *
                                           (polyxi[j][2] * face_xi_grads[m][d3] * face_lambda_values[m][q] +
                                            polyxi[j][1] * face_lambda_grads[m][d3]) +
                                         face_lambda_grads[m][d2] * polyxi[j][1] * face_xi_grads[m][d3]);
                                    }
                                }
                            }
                        }
                  ...
                  for (unsigned int d2 = 0; d2 < dim; ++d2)
                    {
                      transformed_shape_grads[q][d1] -=
                        data.shape_values(first + d2, q) * mapping_data.jacobian_pushed_forward_grads[q][d2][d1];
                    }
                }
            }
                          ...
                          for (unsigned int d4 = 0; d4 < dim; ++d4)
                            {
                              transformed_shape_hessians[q][d1][d3][d4] -=
                                (data.shape_values(first + d2, q) *
                                 mapping_data.jacobian_pushed_forward_2nd_derivatives[q][d2][d1][d3][d4]) +
                                (data.shape_gradients[first + d1][q][d2] *
                                 mapping_data.jacobian_pushed_forward_grads[q][d2][d3][d4]) +
                                (data.shape_gradients[first + d2][q][d3] *
                                 mapping_data.jacobian_pushed_forward_grads[q][d2][d1][d4]) +
                                (data.shape_gradients[first + d2][q][d4] *
                                 mapping_data.jacobian_pushed_forward_grads[q][d2][d3][d1]);
                            }
                        }
                    }
                }
          ...
          for (unsigned int q = 0; q < n_q_points; ++q)
            {
              for (unsigned int d = 0; d < dim; ++d)
                {
                  data.shape_hessians[first + d][q] = transformed_shape_hessians[q][d];
                }
            }
        }
    }
  ...
  dealii::internal::FEValuesImplementation::FiniteElementRelatedData<dim, dim> &data) const
{
  ...
  // This will fill in the missing items in the InternalData
  // (fe_internal/fe_data) which was not filled in by get_data.
  fill_edge_values(cell, QProjector<dim>::project_to_all_faces(this->reference_cell(), quadrature[0]), fe_data);
  if (dim == 3 && this->degree > 1)
    {
      fill_face_values(cell, QProjector<dim>::project_to_all_faces(this->reference_cell(), quadrature[0]), fe_data);
    }

  const UpdateFlags  flags(fe_data.update_each);
  const unsigned int n_q_points = quadrature[0].size();
  const auto         offset     = QProjector<dim>::DataSetDescriptor::face(this->reference_cell(),
                                                               face_no,
...
template <int dim, int spacedim>
UpdateFlags
FE_NedelecSZ<dim, spacedim>::requires_update_flags(const UpdateFlags flags) const
{
  ...
  if (flags & update_gradients)
    out |= update_gradients | update_values | update_jacobian_pushed_forward_grads | update_covariant_transformation;

  if (flags & update_hessians)
    out |= update_hessians | update_values | update_gradients | update_jacobian_pushed_forward_grads |

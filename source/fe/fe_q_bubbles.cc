        ...
        switch (dim)
          {
            case 1:
              if (spacedim == 1)
                ...
                q_fine = std::make_unique<QAnisotropic<dim>>(QGauss<1>(degree + 1), q_dummy);
              else
                q_fine = std::make_unique<QAnisotropic<dim>>(QGauss<1>(degree + 1), q_dummy, q_dummy);
              break;
            case 2:
              if (spacedim == 2)
                q_fine = std::make_unique<QGauss<dim>>(degree + 1);
              else
                q_fine = std::make_unique<QAnisotropic<dim>>(QGauss<1>(degree + 1), QGauss<1>(degree + 1), q_dummy);
              break;
            case 3:
              q_fine = std::make_unique<QGauss<dim>>(degree + 1);
              break;
        ...
        for (; ref_case <= RefinementCase<dim>::isotropic_refinement; ++ref_case)
          {
            ...
            for (unsigned int i = 0; i < nc; ++i)
              {
                Assert(matrices[ref_case - 1][i].n() == dpc, ExcDimensionMismatch(matrices[ref_case - 1][i].n(), dpc));
                Assert(matrices[ref_case - 1][i].m() == dpc, ExcDimensionMismatch(matrices[ref_case - 1][i].m(), dpc));
              }
                    ...
                    for (unsigned int j = 0; j < dpc; ++j)
                      {
                        ...
                        fine_mass(gdi, gdj) += fine.shape_value(i, q) * fine.shape_value(j, q) * fine.JxW(q);
                        Point<dim> quad_tmp;
                        for (unsigned int k = 0; k < dim; ++k)
                          quad_tmp(k) = fine.quadrature_point(q)(k);
                        coarse_rhs_matrix(gdi, j) += fine.shape_value(i, q) * fe.shape_value(j, quad_tmp) * fine.JxW(q);
                      }
              }
...
FE_Q_Bubbles<dim, spacedim>::FE_Q_Bubbles(const Quadrature<1> &points)
  : FE_Q_Base<dim, spacedim>(
      ...
      FiniteElementData<dim>(get_dpo_vector(points.size() - 1), 1, points.size(), FiniteElementData<dim>::H1),
...
FE_Q_Bubbles<dim, spacedim>::get_name() const
{
  ...
  for (unsigned int j = 0; j < n_points; ++j)
    if (std::fabs(points[j] - static_cast<double>(j) / (this->degree - 1)) > 1e-15)
      {
        type = false;
        break;
      }
  ...
  if (type == true)
    {
      if (this->degree > 3)
        namebuf << "FE_Q_Bubbles<" << Utilities::dim_string(dim, spacedim) << ">(QIterated(QTrapezoid(),"
                << this->degree - 1 << "))";
      else
        namebuf << "FE_Q_Bubbles<" << Utilities::dim_string(dim, spacedim) << ">(" << this->degree - 1 << ")";
    }
  else
    {
...
template <int dim, int spacedim>
void
FE_Q_Bubbles<dim, spacedim>::get_interpolation_matrix(const FiniteElement<dim, spacedim> &x_source_fe,
...
template <int dim, int spacedim>
bool
FE_Q_Bubbles<dim, spacedim>::has_support_on_face(const unsigned int shape_index, const unsigned int face_index) const
{
...
FE_Q_Bubbles<dim, spacedim>::get_prolongation_matrix(const unsigned int         child,
...
template <int dim, int spacedim>
const FullMatrix<double> &
FE_Q_Bubbles<dim, spacedim>::get_restriction_matrix(const unsigned int         child,
                                                    const RefinementCase<dim> &refinement_case) const
{
  AssertIndexRange(refinement_case, RefinementCase<dim>::isotropic_refinement + 1);
  Assert(refinement_case != RefinementCase<dim>::no_refinement,
...
FE_Q_Bubbles<dim, spacedim>::compare_for_domination(const FiniteElement<dim, spacedim> &fe_other,

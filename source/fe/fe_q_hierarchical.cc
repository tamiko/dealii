...
FE_Q_Hierarchical<dim>::FE_Q_Hierarchical(const unsigned int degree)
  : FE_Poly<dim>(TensorProductPolynomials<dim>(Polynomials::Hierarchical::generate_complete_basis(degree)),
                 FiniteElementData<dim>(get_dpo_vector(degree), 1, degree, FiniteElementData<dim>::H1),
                 std::vector<bool>(FiniteElementData<dim>(get_dpo_vector(degree), 1, degree).n_dofs_per_cell(), false),
  ...
  , face_renumber(face_fe_q_hierarchical_to_hierarchic_numbering(degree))
{
  ...
  poly_space_derived_ptr->set_numbering(hierarchic_to_fe_q_hierarchical_numbering(*this));
  ...
  // for all dimensions.
  std::vector<FullMatrix<double>> dofs_cell(GeometryInfo<1>::max_children_per_cell,
...
template <int dim>
void
FE_Q_Hierarchical<dim>::get_interpolation_matrix(const FiniteElement<dim> &source, FullMatrix<double> &matrix) const
{
  // support interpolation between FE_Q_Hierarchical only.
  if (const FE_Q_Hierarchical<dim> *source_fe = dynamic_cast<const FE_Q_Hierarchical<dim> *>(&source))
    {
      // ok, source is a Q_Hierarchical element, so we will be able to do the
      // work
      Assert(matrix.m() == this->n_dofs_per_cell(), ExcDimensionMismatch(matrix.m(), this->n_dofs_per_cell()));
      Assert(matrix.n() == source.n_dofs_per_cell(), ExcDimensionMismatch(matrix.m(), source_fe->n_dofs_per_cell()));
...
template <int dim>
std::vector<std::pair<unsigned int, unsigned int>>
FE_Q_Hierarchical<dim>::hp_quad_dof_identities(const FiniteElement<dim> &fe_other, const unsigned int) const
{
  // we can presently only compute
  // these identities if both FEs are
  // FE_Q_Hierarchicals or if the other
...
FE_Q_Hierarchical<dim>::compare_for_domination(const FiniteElement<dim> &fe_other, const unsigned int codim) const
{
  ...
  // & cell domination
  // ----------------------------------------
  if (const FE_Q_Hierarchical<dim> *fe_hierarchical_other = dynamic_cast<const FE_Q_Hierarchical<dim> *>(&fe_other))
    {
...
{
  const unsigned int dofs_1d = 2 * this->n_dofs_per_vertex() + this->n_dofs_per_line();
      ...
      for (unsigned int k = 0; k < dofs_1d; ++k)
        {
          ...
          // upper right block
            ...
            {
              if (((c == 0) && (j == 1) && ((k % 2) == 0)) || ((c == 1) && (j == 0) && ((k % 2) == 0)))
                dofs_subcell[c](j, k) = -1.;
            }
          // upper diagonal block
          else if ((j >= 2) && (k >= 2) && (j <= k))
            {
              ...
              for (unsigned int i = 1; i <= j; ++i)
                factor *= (static_cast<double>(k - i + 1)) / (static_cast<double>(i));
              // factor == k * (k-1) * ... * (k-j+1) / j! = k! / (k-j)! / j!
              if (c == 0)
                {
                  dofs_subcell[c](j, k) = ((k + j) % 2 == 0) ? std::pow(.5, static_cast<double>(k)) * factor :
              ...
              else
                {
                  ...
                  dofs_cell[c](j, k)    = ((k + j) % 2 == 0) ? std::pow(2., static_cast<double>(j)) * factor :
          ...
          for (unsigned int c = 0; c < GeometryInfo<1>::max_children_per_cell; ++c)
            for (unsigned int i = 0; i < dofs_1d; ++i)
              for (unsigned int j = 2; j < dofs_1d; ++j)
                this->interface_constraints(1 + c * (this->degree - 1) + j - 2, i) = dofs_subcell[c](j, i);
          break;
        }
          ...
          for (unsigned int i = 0; i < dofs_1d * dofs_1d; ++i)
            {
              ...
              for (unsigned int j = 0; j < (this->degree - 1); ++j)
                {
                  this->interface_constraints(5 + j, face_renumber[i]) =
                    dofs_subcell[0](1, i % dofs_1d) * dofs_subcell[0](2 + j, (i - (i % dofs_1d)) / dofs_1d);
                  this->interface_constraints(5 + (this->degree - 1) + j, face_renumber[i]) =
                    dofs_subcell[0](1, i % dofs_1d) * dofs_subcell[1](2 + j, (i - (i % dofs_1d)) / dofs_1d);
                  this->interface_constraints(5 + 2 * (this->degree - 1) + j, face_renumber[i]) =
                    dofs_subcell[0](2 + j, i % dofs_1d) * dofs_subcell[1](0, (i - (i % dofs_1d)) / dofs_1d);
                  this->interface_constraints(5 + 3 * (this->degree - 1) + j, face_renumber[i]) =
                    dofs_subcell[1](2 + j, i % dofs_1d) * dofs_subcell[0](1, (i - (i % dofs_1d)) / dofs_1d);
                }
              ...
              for (unsigned int j = 0; j < (this->degree - 1); ++j)
                {
                  ...
                  this->interface_constraints(5 + 4 * (this->degree - 1) + j, face_renumber[i]) =
                    dofs_subcell[0](0, i % dofs_1d) * dofs_subcell[0](2 + j, (i - (i % dofs_1d)) / dofs_1d);
                  this->interface_constraints(5 + 4 * (this->degree - 1) + (this->degree - 1) + j, face_renumber[i]) =
                    dofs_subcell[0](0, i % dofs_1d) * dofs_subcell[1](2 + j, (i - (i % dofs_1d)) / dofs_1d);
                  // right edge
                  this->interface_constraints(5 + 4 * (this->degree - 1) + 2 * (this->degree - 1) + j,
                                              face_renumber[i]) =
                    dofs_subcell[1](1, i % dofs_1d) * dofs_subcell[0](2 + j, (i - (i % dofs_1d)) / dofs_1d);
                  this->interface_constraints(5 + 4 * (this->degree - 1) + 3 * (this->degree - 1) + j,
                                              face_renumber[i]) =
                    dofs_subcell[1](1, i % dofs_1d) * dofs_subcell[1](2 + j, (i - (i % dofs_1d)) / dofs_1d);
                  // bottom edge
                  this->interface_constraints(5 + 4 * (this->degree - 1) + 4 * (this->degree - 1) + j,
                                              face_renumber[i]) =
                    dofs_subcell[0](2 + j, i % dofs_1d) * dofs_subcell[0](0, (i - (i % dofs_1d)) / dofs_1d);
                  this->interface_constraints(5 + 4 * (this->degree - 1) + 5 * (this->degree - 1) + j,
                                              face_renumber[i]) =
                    dofs_subcell[1](2 + j, i % dofs_1d) * dofs_subcell[0](0, (i - (i % dofs_1d)) / dofs_1d);
                  // top edge
                  this->interface_constraints(5 + 4 * (this->degree - 1) + 6 * (this->degree - 1) + j,
                                              face_renumber[i]) =
                    dofs_subcell[0](2 + j, i % dofs_1d) * dofs_subcell[1](1, (i - (i % dofs_1d)) / dofs_1d);
                  this->interface_constraints(5 + 4 * (this->degree - 1) + 7 * (this->degree - 1) + j,
                                              face_renumber[i]) =
                    dofs_subcell[1](2 + j, i % dofs_1d) * dofs_subcell[1](1, (i - (i % dofs_1d)) / dofs_1d);
                }
              ...
              for (unsigned int j = 0; j < (this->degree - 1); ++j)
                for (unsigned int k = 0; k < (this->degree - 1); ++k)
                  {
                    // subcell 0
                    this->interface_constraints(5 + 12 * (this->degree - 1) + j + k * (this->degree - 1),
                                                face_renumber[i]) =
                      dofs_subcell[0](2 + j, i % dofs_1d) * dofs_subcell[0](2 + k, (i - (i % dofs_1d)) / dofs_1d);
                    // subcell 1
                    this->interface_constraints(5 + 12 * (this->degree - 1) + j + k * (this->degree - 1) +
                                                  (this->degree - 1) * (this->degree - 1),
                                                face_renumber[i]) =
                      dofs_subcell[1](2 + j, i % dofs_1d) * dofs_subcell[0](2 + k, (i - (i % dofs_1d)) / dofs_1d);
                    // subcell 2
                    this->interface_constraints(5 + 12 * (this->degree - 1) + j + k * (this->degree - 1) +
                                                  2 * (this->degree - 1) * (this->degree - 1),
                                                face_renumber[i]) =
                      dofs_subcell[0](2 + j, i % dofs_1d) * dofs_subcell[1](2 + k, (i - (i % dofs_1d)) / dofs_1d);
                    // subcell 3
                    this->interface_constraints(5 + 12 * (this->degree - 1) + j + k * (this->degree - 1) +
                                                  3 * (this->degree - 1) * (this->degree - 1),
                                                face_renumber[i]) =
                      dofs_subcell[1](2 + j, i % dofs_1d) * dofs_subcell[1](2 + k, (i - (i % dofs_1d)) / dofs_1d);
                  }
            }
          break;
        }
...
template <int dim>
void
FE_Q_Hierarchical<dim>::initialize_embedding_and_restriction(const std::vector<FullMatrix<double>> &dofs_cell,
                                                             const std::vector<FullMatrix<double>> &dofs_subcell)
{
  ...
  TensorProductPolynomials<dim> *poly_space_derived_ptr =
    dynamic_cast<TensorProductPolynomials<dim> *>(this->poly_space.get());
  const std::vector<unsigned int> &renumber = poly_space_derived_ptr->get_numbering();

  for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; ++c)
    {
      this->prolongation[iso][c].reinit(this->n_dofs_per_cell(), this->n_dofs_per_cell());
      this->restriction[iso][c].reinit(this->n_dofs_per_cell(), this->n_dofs_per_cell());
    }
              ...
              for (unsigned int c = 0; c < GeometryInfo<2>::max_children_per_cell; ++c)
                {
                  ...
                  this->restriction[iso][c](j, i) = dofs_cell[c0](renumber[j] % dofs_1d, renumber[i] % dofs_1d) *
              ...
              for (unsigned int c = 0; c < GeometryInfo<3>::max_children_per_cell; ++c)
                {
                  ...
                  this->prolongation[iso][c](j, i) =
                    ...
                    dofs_subcell[c2](((renumber[j] - (renumber[j] % dofs_1d)) / dofs_1d -
                  ...
                  this->restriction[iso][c](j, i) =
                    ...
                    dofs_cell[c1](((renumber[j] - (renumber[j] % dofs_1d)) / dofs_1d) % dofs_1d,
                                  ((renumber[i] - (renumber[i] % dofs_1d)) / dofs_1d) % dofs_1d) *
                    dofs_cell[c2](((renumber[j] - (renumber[j] % dofs_1d)) / dofs_1d -
                                   (((renumber[j] - (renumber[j] % dofs_1d)) / dofs_1d) % dofs_1d)) /
                                    dofs_1d,
                                  ((renumber[i] - (renumber[i] % dofs_1d)) / dofs_1d -
                                   (((renumber[i] - (renumber[i] % dofs_1d)) / dofs_1d) % dofs_1d)) /
...
FE_Q_Hierarchical<dim>::initialize_generalized_support_points()
{
  ...
  TensorProductPolynomials<dim> *poly_space_derived_ptr =
    dynamic_cast<TensorProductPolynomials<dim> *>(this->poly_space.get());
  const std::vector<unsigned int> &index_map_inverse = poly_space_derived_ptr->get_numbering_inverse();
...
template <>
void
FE_Q_Hierarchical<1>::get_face_interpolation_matrix(const FiniteElement<1, 1> & /*x_source_fe*/,
...
template <>
void
FE_Q_Hierarchical<1>::get_subface_interpolation_matrix(const FiniteElement<1, 1> & /*x_source_fe*/,
...
template <int dim>
void
FE_Q_Hierarchical<dim>::get_face_interpolation_matrix(const FiniteElement<dim> &x_source_fe,
                                                      FullMatrix<double>       &interpolation_matrix,
                                                      const unsigned int        face_no) const
{
  ...
  const FE_Q_Hierarchical<dim> &source_fe = dynamic_cast<const FE_Q_Hierarchical<dim> &>(x_source_fe);
  (void)source_fe;
          ...
          for (unsigned int i = 0; i < this->degree - 1; ++i)
            {
              for (unsigned int j = 0; j < GeometryInfo<3>::lines_per_face; ++j)
                interpolation_matrix(i + j * (x_source_fe.degree - 1) + GeometryInfo<3>::vertices_per_face,
                                     i + j * (this->degree - 1) + GeometryInfo<3>::vertices_per_face) = 1;

              for (unsigned int j = 0; j < this->degree - 1; ++j)
                interpolation_matrix((i + GeometryInfo<3>::lines_per_face) * (x_source_fe.degree - 1) + j +
...
template <int dim>
void
FE_Q_Hierarchical<dim>::get_subface_interpolation_matrix(const FiniteElement<dim> &x_source_fe,
                                                         const unsigned int        subface,
                                                         FullMatrix<double>       &interpolation_matrix,
                                                         const unsigned int        face_no) const
{
              ...
              case 0:
                {
                  ...
                  for (unsigned int dof = 2; dof < this->n_dofs_per_face(face_no);)
                    {
                  ...
                  for (unsigned int i = 2; i < this->n_dofs_per_face(face_no); ++i)
                    {
                      ...
                      for (unsigned int j = i + 1; j < this->n_dofs_per_face(face_no); ++j)
                        {
                          factorial_ij *= j - i;
                          factorial_j *= j;
                  ...
                  for (unsigned int i = 2; i <= this->degree; ++i)
                    {
                      ...
                      for (unsigned int j = 2; j <= this->degree; ++j)
                        {
                          interpolation_matrix(i + (j + 2) * source_fe.degree - j, i + (j + 2) * this->degree - j) =
                            std::pow(0.5, i + j);
                          factorial_k *= j;
                          int factorial_kl = 1;
                          ...
                          for (unsigned int k = j + 1; k < this->degree; ++k)
                            {
                              ...
                              if (((j + k) & 1) != 0u)
                                interpolation_matrix(i + (j + 2) * source_fe.degree - j,
                                                     i + (k + 2) * this->degree - k) =
                                  -1.0 * std::pow(0.5, i + k) * factorial_l / (factorial_k * factorial_kl);
                          ...
                          if (((i + j) & 1) != 0u)
                            {
                              ...
                              for (unsigned int k = 2; k <= this->degree; ++k)
                                {
                                  interpolation_matrix(i + (k + 2) * source_fe.degree - k,
                                                       j + (k + 2) * this->degree - k) = tmp * std::pow(0.5, k);
                                  factorial_k *= k;
                                  int factorial_l  = factorial_k;
                                  int factorial_kl = 1;

                                  for (unsigned int l = k + 1; l <= this->degree; ++l)
                                    {
                                      factorial_kl *= l - k;
                                      factorial_l *= l;
                                      ...
                                      if (((k + l) & 1) != 0u)
                                        interpolation_matrix(i + (k + 2) * source_fe.degree - k,
                                                             j + (l + 2) * this->degree - l) =
                                          -1.0 * tmp * std::pow(0.5, l) * factorial_l / (factorial_k * factorial_kl);

                                      else
                                        interpolation_matrix(i + (k + 2) * source_fe.degree - k,
                                                             j + (l + 2) * this->degree - l) =
                                          tmp * std::pow(0.5, l) * factorial_l / (factorial_k * factorial_kl);
                                    }
                                }
                              ...
                              for (unsigned int k = 0; k < this->degree - 1;)
                                {
                                  interpolation_matrix(i + source_fe.degree + 1, (j + 2) * this->degree + k + 2 - j) =
                                    tmp;
                      ...
                      for (unsigned int j = i + 1; j <= this->degree; ++j)
                        {
                          ...
                          for (unsigned int k = 2; k <= this->degree; ++k)
                            {
                              interpolation_matrix(i + (k + 2) * source_fe.degree - k, j + (k + 2) * this->degree - k) =
                                tmp * std::pow(0.5, k);
                              factorial_k *= k;
                              int factorial_l  = factorial_k;
                              int factorial_kl = 1;
                          ...
                          for (unsigned int k = 0; k < this->degree - 1;)
                            {
                              interpolation_matrix(i + 3 * source_fe.degree - 1, j + (k + 4) * this->degree - k - 2) =
                                tmp;
                              k = k + 2;
                            }
                          ...
                          for (unsigned int k = j + 1; k <= this->degree; ++k)
                            {
                              factorial_kl *= k - j;
                              factorial_l *= k;
                              interpolation_matrix(i + (j + 2) * source_fe.degree - j, i + (k + 2) * this->degree - k) =
                                std::pow(0.5, i + k) * factorial_l / (factorial_k * factorial_kl);
                            }
                        }
      ...
      for (unsigned int ix = 0; ix <= this->degree; ++ix)
        {
          ...
          this->generalized_face_support_points[face_no][face_renumber[k++]] = p;
        }
}
      ...
      case 2:
        {
          ...
          // inside quad
          Assert(fe.n_dofs_per_quad(0 /*only one quad in 2d*/) == fe.n_dofs_per_line() * fe.n_dofs_per_line(),
                 ExcInternalError());
          for (unsigned int i = 0; i < fe.n_dofs_per_line(); ++i)
      ...
      case 3:
        {
          ...
          // inside quads
          Assert(fe.n_dofs_per_quad(face_no) == fe.n_dofs_per_line() * fe.n_dofs_per_line(), ExcInternalError());
          // left face
          for (unsigned int i = 0; i < fe.n_dofs_per_line(); ++i)
            for (unsigned int j = 0; j < fe.n_dofs_per_line(); ++j)
          ...
          // inside hex
          Assert(fe.n_dofs_per_hex() == fe.n_dofs_per_quad(face_no) * fe.n_dofs_per_line(), ExcInternalError());
          for (unsigned int i = 0; i < fe.n_dofs_per_line(); ++i)
            for (unsigned int j = 0; j < fe.n_dofs_per_line(); ++j)
...
template <>
bool
FE_Q_Hierarchical<1>::has_support_on_face(const unsigned int shape_index, const unsigned int face_index) const
{
...
template <int dim>
bool
FE_Q_Hierarchical<dim>::has_support_on_face(const unsigned int shape_index, const unsigned int face_index) const
{
  AssertIndexRange(shape_index, this->n_dofs_per_cell());
  ...
  // the boundary
  if (((dim == 2) && (shape_index >= this->get_first_quad_index(0 /*only one quad in 2d*/))) ||
  ...
  if (shape_index < this->get_first_line_index())
    ...
    // dof is on a quad
    {
      const unsigned int quad_index =
        (shape_index - this->get_first_quad_index(0 /*first quad*/)) / this->n_dofs_per_quad(face_index);
  ...
  if ((dim == 2) || (dim == 3))
    {
      ...
      for (unsigned int i = 0; i < ((dim == 2) ? (sub_degree + 1) * (sub_degree + 1) :
                                                 (sub_degree + 1) * (sub_degree + 1) * (sub_degree + 1));
           ++i)
        {
          // vertex
          if (i < GeometryInfo<dim>::vertices_per_cell)
            embedding_dofs[i] = i;
          // line
          else if (i < (GeometryInfo<dim>::vertices_per_cell + GeometryInfo<dim>::lines_per_cell * (sub_degree - 1)))
            {
              ...
              embedding_dofs[i] = GeometryInfo<dim>::vertices_per_cell + line * (this->degree - 1) + j;
            }
          // quad
          else if (i < (GeometryInfo<dim>::vertices_per_cell + GeometryInfo<dim>::lines_per_cell * (sub_degree - 1)) +
                         GeometryInfo<dim>::quads_per_cell * (sub_degree - 1) * (sub_degree - 1))
            {
              const unsigned int j =
                (i - GeometryInfo<dim>::vertices_per_cell - GeometryInfo<dim>::lines_per_cell * (sub_degree - 1)) %
                (sub_degree - 1);
              const unsigned int k =
                ((i - GeometryInfo<dim>::vertices_per_cell - GeometryInfo<dim>::lines_per_cell * (sub_degree - 1) - j) /
                 (sub_degree - 1)) %
                (sub_degree - 1);
              ...
              embedding_dofs[i] = GeometryInfo<dim>::vertices_per_cell +
                                  ...
                                  face * (this->degree - 1) * (this->degree - 1) + k * (this->degree - 1) + j;
            }
          // hex
          else if (i < (GeometryInfo<dim>::vertices_per_cell + GeometryInfo<dim>::lines_per_cell * (sub_degree - 1)) +
                         GeometryInfo<dim>::quads_per_cell * (sub_degree - 1) * (sub_degree - 1) +
                         GeometryInfo<dim>::hexes_per_cell * (sub_degree - 1) * (sub_degree - 1) * (sub_degree - 1))
            {
              ...
              const unsigned int l =
                (i - GeometryInfo<dim>::vertices_per_cell - GeometryInfo<dim>::lines_per_cell * (sub_degree - 1) -
                 GeometryInfo<dim>::quads_per_cell * (sub_degree - 1) * (sub_degree - 1) - j - k * (sub_degree - 1)) /
                ((sub_degree - 1) * (sub_degree - 1));
...
FE_Q_Hierarchical<dim>::get_constant_modes() const
{
  ...
  for (unsigned int i = GeometryInfo<dim>::vertices_per_cell; i < this->n_dofs_per_cell(); ++i)

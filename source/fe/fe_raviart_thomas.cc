...
FE_RaviartThomas<dim>::FE_RaviartThomas(const unsigned int deg)
  : FE_PolyTensor<dim>(PolynomialsRaviartThomas<dim>(deg + 1, deg),
                       FiniteElementData<dim>(get_dpo_vector(deg), dim, deg + 1, FiniteElementData<dim>::Hdiv),
                       std::vector<bool>(PolynomialsRaviartThomas<dim>::n_polynomials(deg + 1, deg), true),
                       std::vector<ComponentMask>(PolynomialsRaviartThomas<dim>::n_polynomials(deg + 1, deg),
                                                  ComponentMask(std::vector<bool>(dim, true))))
{
  ...
  this->interface_constraints.reinit((1 << (dim - 1)) * this->n_dofs_per_face(face_no), this->n_dofs_per_face(face_no));
...
FE_RaviartThomas<dim>::initialize_support_points(const unsigned int deg)
{
  ...
  this->generalized_support_points.resize(GeometryInfo<dim>::faces_per_cell * n_face_points + n_interior_points);
  ...
  if (dim > 1)
    {
      ...
      TensorProductPolynomials<dim - 1> legendre = Polynomials::Legendre::generate_complete_basis(deg);
      ...
      for (unsigned int k = 0; k < n_face_points; ++k)
        {
          ...
          for (unsigned int i = 0; i < legendre.n(); ++i)
            {
              boundary_weights(k, i) = face_points.weight(k) * legendre.compute_value(i, face_points.point(k));
            }
        }

      Quadrature<dim> faces = QProjector<dim>::project_to_all_faces(this->reference_cell(), face_points);
      for (; current < GeometryInfo<dim>::faces_per_cell * n_face_points; ++current)
        {
          ...
          this->generalized_support_points[current] = faces.point(
            current +
            QProjector<dim>::DataSetDescriptor::face(this->reference_cell(), 0, true, false, false, n_face_points));
        }
    }
...
FE_RaviartThomas<dim>::initialize_quad_dof_index_permutation_and_sign_change()
{
  ...
  for (unsigned int local_face_dof = 0; local_face_dof < this->n_dofs_per_quad(face_no); ++local_face_dof)
    {
      // Row and column
      ...
      for (const bool face_orientation : {false, true})
        for (const bool face_flip : {false, true})
          for (const bool face_rotation : {false, true})
            {
              const auto case_no = internal::combined_face_orientation(face_orientation, face_rotation, face_flip);

              if (((!face_orientation) && (!face_rotation)) || ((face_orientation) && (face_rotation)))
                {
                  // We flip across the diagonal
                  // This is the local face dof offset
                  this->adjust_quad_dof_index_for_face_orientation_table[face_no](local_face_dof, case_no) =
                    j + i * n - local_face_dof;
                }
              else
                {
                  // Offset is zero
                  this->adjust_quad_dof_index_for_face_orientation_table[face_no](local_face_dof, case_no) = 0;
              ...
              if (!face_flip && face_rotation)
                {
                  this->adjust_quad_dof_sign_for_face_orientation_table[face_no](local_face_dof, case_no) =
                    ((j % 2) == 1);
                }
...
FE_RaviartThomas<dim>::initialize_restriction()
{
  ...
  for (const unsigned int face : GeometryInfo<dim>::face_indices())
    {
      ...
      // of a full face.
      Quadrature<dim> q_face = QProjector<dim>::project_to_face(this->reference_cell(), q_base, face);
      // Store shape values, since the
      // evaluation suffers if not
      // ordered by point
      Table<2, double> cached_values_on_face(this->n_dofs_per_cell(), q_face.size());
      for (unsigned int k = 0; k < q_face.size(); ++k)
        for (unsigned int i = 0; i < this->n_dofs_per_cell(); ++i)
          cached_values_on_face(i, k) =
            this->shape_value_component(i, q_face.point(k), GeometryInfo<dim>::unit_normal_direction[face]);
      ...
      for (unsigned int sub = 0; sub < GeometryInfo<dim>::max_children_per_face; ++sub)
        {
          ...
          // only.
          Quadrature<dim>    q_sub = QProjector<dim>::project_to_subface(this->reference_cell(), q_base, face, sub);
          const unsigned int child =
            GeometryInfo<dim>::child_cell_on_face(RefinementCase<dim>::isotropic_refinement, face, sub);
          ...
          // functions.
          for (unsigned int k = 0; k < n_face_points; ++k)
            for (unsigned int i_child = 0; i_child < this->n_dofs_per_cell(); ++i_child)
              for (unsigned int i_face = 0; i_face < this->n_dofs_per_face(face); ++i_face)
                {
                  // The quadrature
                  // weights on the
                  // subcell are NOT
                  // transformed, so we
                  // have to do it here.
                  this->restriction[iso][child](face * this->n_dofs_per_face(face) + i_face, i_child) +=
                    Utilities::fixed_power<dim - 1>(.5) * q_sub.weight(k) * cached_values_on_face(i_child, k) *
                    this->shape_value_component(face * this->n_dofs_per_face(face) + i_face,
                                                q_sub.point(k),
  ...
  const unsigned int start_cell_dofs = GeometryInfo<dim>::faces_per_cell * this->n_dofs_per_face(face_no);

  // Store shape values, since the
  // evaluation suffers if not
  // ordered by point
  Table<3, double> cached_values_on_cell(this->n_dofs_per_cell(), q_cell.size(), dim);
  ...
  for (unsigned int child = 0; child < GeometryInfo<dim>::max_children_per_cell; ++child)
    {
      Quadrature<dim> q_sub = QProjector<dim>::project_to_child(this->reference_cell(), q_cell, child);
...
template <int dim>
bool
FE_RaviartThomas<dim>::has_support_on_face(const unsigned int shape_index, const unsigned int face_index) const
{

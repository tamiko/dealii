...
namespace
{
    ...
    for (unsigned int q = 0; q < quadrature.size(); ++q)
      {
        ...
        sum += Lh(x_q, indices) * fe.shape_value_component(dof, x_q, component) * quadrature.weight(q);
      }
  ...
  void
  ensure_existence(const std::vector<unsigned int>     &n_coefficients_per_direction,
                   ...
                   std::vector<FullMatrix<double>>     &legendre_transform_matrices)
  {
    ...
    if (legendre_transform_matrices[fe].m() == 0)
      {
        legendre_transform_matrices[fe].reinit(n_coefficients_per_direction[fe], fe_collection[fe].n_dofs_per_cell());
        ...
        for (unsigned int k = 0; k < n_coefficients_per_direction[fe]; ++k)
          for (unsigned int j = 0; j < fe_collection[fe].n_dofs_per_cell(); ++j)
            legendre_transform_matrices[fe](k, j) =
              integrate(fe_collection[fe], q_collection[fe], TableIndices<1>(k), j, component);
      }
  }
                   ...
                   std::vector<FullMatrix<double>>     &legendre_transform_matrices)
  {
    ...
    if (legendre_transform_matrices[fe].m() == 0)
      {
        legendre_transform_matrices[fe].reinit(Utilities::fixed_power<2>(n_coefficients_per_direction[fe]),
                                               fe_collection[fe].n_dofs_per_cell());

        unsigned int k = 0;
        for (unsigned int k1 = 0; k1 < n_coefficients_per_direction[fe]; ++k1)
          for (unsigned int k2 = 0; k2 < n_coefficients_per_direction[fe]; ++k2, k++)
            for (unsigned int j = 0; j < fe_collection[fe].n_dofs_per_cell(); ++j)
              legendre_transform_matrices[fe](k, j) =
                integrate(fe_collection[fe], q_collection[fe], TableIndices<2>(k1, k2), j, component);
      }
  }
                   ...
                   std::vector<FullMatrix<double>>     &legendre_transform_matrices)
  {
    ...
    if (legendre_transform_matrices[fe].m() == 0)
      {
        legendre_transform_matrices[fe].reinit(Utilities::fixed_power<3>(n_coefficients_per_direction[fe]),
                                               fe_collection[fe].n_dofs_per_cell());

        unsigned int k = 0;
        for (unsigned int k1 = 0; k1 < n_coefficients_per_direction[fe]; ++k1)
          for (unsigned int k2 = 0; k2 < n_coefficients_per_direction[fe]; ++k2)
            for (unsigned int k3 = 0; k3 < n_coefficients_per_direction[fe]; ++k3, k++)
              for (unsigned int j = 0; j < fe_collection[fe].n_dofs_per_cell(); ++j)
                legendre_transform_matrices[fe](k, j) =
                  integrate(fe_collection[fe], q_collection[fe], TableIndices<3>(k1, k2, k3), j, component);
      }
  }
} // namespace
...
{
  template <int dim, int spacedim>
  Legendre<dim, spacedim>::Legendre(const std::vector<unsigned int>       &n_coefficients_per_direction,
    ...
    , component(component_ != numbers::invalid_unsigned_int ? component_ : 0)
  {
    ...
    if (fe_collection[0].n_components() > 1)
      Assert(component_ != numbers::invalid_unsigned_int,
             ExcMessage("For vector-valued problems, you need to explicitly specify for "
    ...
    const unsigned int max_n_coefficients_per_direction =
      *std::max_element(n_coefficients_per_direction.cbegin(), n_coefficients_per_direction.cend());
    unrolled_coefficients.reserve(Utilities::fixed_power<dim>(max_n_coefficients_per_direction));
  }
  ...
  template <int dim, int spacedim>
  inline bool
  Legendre<dim, spacedim>::operator==(const Legendre<dim, spacedim> &legendre) const
  {
    return ((n_coefficients_per_direction == legendre.n_coefficients_per_direction) &&
            (*fe_collection == *(legendre.fe_collection)) && (q_collection == legendre.q_collection) &&
            (legendre_transform_matrices == legendre.legendre_transform_matrices) && (component == legendre.component));
  }
  ...
  Legendre<dim, spacedim>::precalculate_all_transformation_matrices()
  {
    ...
    for (unsigned int fe = 0; fe < fe_collection->size(); ++fe)
      task_group += Threads::new_task([&, fe]() {
        ensure_existence(
          n_coefficients_per_direction, *fe_collection, q_collection, fe, component, legendre_transform_matrices);
      });
  ...
  template <int dim, int spacedim>
  unsigned int
  Legendre<dim, spacedim>::get_n_coefficients_per_direction(const unsigned int index) const
  {
  ...
  void
  Legendre<dim, spacedim>::calculate(const dealii::Vector<Number> &local_dof_values,
                                     const unsigned int            cell_active_fe_index,
                                     Table<dim, CoefficientType>  &legendre_coefficients)
  {
    for (unsigned int d = 0; d < dim; ++d)
      AssertDimension(legendre_coefficients.size(d), n_coefficients_per_direction[cell_active_fe_index]);
    ...
    const FullMatrix<CoefficientType> &matrix = legendre_transform_matrices[cell_active_fe_index];

    unrolled_coefficients.resize(Utilities::fixed_power<dim>(n_coefficients_per_direction[cell_active_fe_index]));
    std::fill(unrolled_coefficients.begin(), unrolled_coefficients.end(), CoefficientType(0.));

    Assert(unrolled_coefficients.size() == matrix.m(), ExcInternalError());

    Assert(local_dof_values.size() == matrix.n(), ExcDimensionMismatch(local_dof_values.size(), matrix.n()));

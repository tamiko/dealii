...
namespace internal
{
  ...
  setup_primitive_offset_table(const FESystem<dim, spacedim> &fe, const unsigned int base_no)
  {
    Assert(fe.base_element(base_no).is_primitive(), ExcInternalError());
    Table<2, unsigned int> table(fe.element_multiplicity(base_no), fe.base_element(base_no).n_dofs_per_cell());
    // 0 is a bad default value since it is a valid index
    table.fill(numbers::invalid_unsigned_int);

    unsigned int out_index = 0;
    for (unsigned int system_index = 0; system_index < fe.n_dofs_per_cell(); ++system_index)
      {
        if (fe.system_to_base_index(system_index).first.first == base_no)
          {
            Assert(fe.n_nonzero_components(system_index) == 1, ExcInternalError());
            const unsigned int base_component = fe.system_to_base_index(system_index).first.second;
            const unsigned int base_index     = fe.system_to_base_index(system_index).second;
            Assert(base_index < fe.base_element(base_no).n_dofs_per_cell(), ExcInternalError());
  ...
  setup_nonprimitive_offset_table(const FESystem<dim, spacedim> &fe, const unsigned int base_no)
  {
        ...
        if (fe.system_to_base_index(system_index).first.first == base_no)
          {
            ...
            table.back().n_nonzero_components = fe.n_nonzero_components(system_index);
    ...
    Assert(table.size() == base_fe.n_dofs_per_cell() * fe.element_multiplicity(base_no), ExcInternalError());
    return table;
  }
  ...
  template <int dim, int spacedim = dim>
  void
  copy_primitive_base_element_values(const FESystem<dim, spacedim> &fe,
                                     const unsigned int             base_no,
                                     const UpdateFlags              base_flags,
                                     const Table<2, unsigned int>  &base_to_system_table,
                                     const FEValuesImplementation::FiniteElementRelatedData<dim, spacedim> &base_data,
                                     FEValuesImplementation::FiniteElementRelatedData<dim, spacedim>       &output_data)
  {
    Assert(fe.base_element(base_no).is_primitive(), ExcInternalError());
    const unsigned int n_components    = fe.element_multiplicity(base_no);
    const unsigned int n_dofs_per_cell = fe.base_element(base_no).n_dofs_per_cell();

    auto copy_row = [](const auto row_in, auto row_out) { std::copy(row_in.begin(), row_in.end(), row_out.begin()); };

    if (base_flags & update_values)
      for (unsigned int component = 0; component < n_components; ++component)
        for (unsigned int b = 0; b < n_dofs_per_cell; ++b)
          copy_row(base_data.shape_values[b], output_data.shape_values[base_to_system_table[component][b]]);

    if (base_flags & update_gradients)
      for (unsigned int component = 0; component < n_components; ++component)
        for (unsigned int b = 0; b < n_dofs_per_cell; ++b)
          copy_row(base_data.shape_gradients[b], output_data.shape_gradients[base_to_system_table[component][b]]);

    if (base_flags & update_hessians)
      for (unsigned int component = 0; component < n_components; ++component)
        for (unsigned int b = 0; b < n_dofs_per_cell; ++b)
          copy_row(base_data.shape_hessians[b], output_data.shape_hessians[base_to_system_table[component][b]]);

    if (base_flags & update_3rd_derivatives)
      for (unsigned int component = 0; component < n_components; ++component)
        for (unsigned int b = 0; b < n_dofs_per_cell; ++b)
          copy_row(base_data.shape_3rd_derivatives[b],
                   output_data.shape_3rd_derivatives[base_to_system_table[component][b]]);
  }
        ...
        if (base_flags & update_values)
          for (unsigned int s = 0; s < offset.n_nonzero_components; ++s)
            for (unsigned int q = 0; q < n_q_points; ++q)
              output_data.shape_values[offset.out_index + s][q] = base_data.shape_values[offset.in_index + s][q];

        if (base_flags & update_gradients)
          for (unsigned int s = 0; s < offset.n_nonzero_components; ++s)
            for (unsigned int q = 0; q < n_q_points; ++q)
              output_data.shape_gradients[offset.out_index + s][q] = base_data.shape_gradients[offset.in_index + s][q];

        if (base_flags & update_hessians)
          for (unsigned int s = 0; s < offset.n_nonzero_components; ++s)
            for (unsigned int q = 0; q < n_q_points; ++q)
              output_data.shape_hessians[offset.out_index + s][q] = base_data.shape_hessians[offset.in_index + s][q];
...
template <int dim, int spacedim>
FESystem<dim, spacedim>::FESystem(const FiniteElement<dim, spacedim> &fe, const unsigned int n_elements)
...
FESystem<dim, spacedim>::FESystem(const FiniteElement<dim, spacedim> &fe1,
  ...
  : FiniteElement<dim, spacedim>(
      ...
      FETools::Compositing::compute_restriction_is_additive_flags(&fe1, n1, &fe2, n2, &fe3, n3, &fe4, n4),
...
FESystem<dim, spacedim>::FESystem(const FiniteElement<dim, spacedim> &fe1,
  ...
  : FiniteElement<dim, spacedim>(
      ...
      FETools::Compositing::compute_restriction_is_additive_flags(&fe1, n1, &fe2, n2, &fe3, n3, &fe4, n4, &fe5, n5),
      FETools::Compositing::compute_nonzero_components(&fe1, n1, &fe2, n2, &fe3, n3, &fe4, n4, &fe5, n5))
  , base_elements(static_cast<int>(n1 > 0) + static_cast<int>(n2 > 0) + static_cast<int>(n3 > 0) +
                  static_cast<int>(n4 > 0) + static_cast<int>(n5 > 0))
{
  std::vector<const FiniteElement<dim, spacedim> *> fes;
  fes.push_back(&fe1);
  fes.push_back(&fe2);
...
FESystem<dim, spacedim>::get_sub_fe(const unsigned int first_component, const unsigned int n_selected_components) const
{
  ...
  Assert(n_selected_components == this->n_components(), ExcMessage("You can not select a part of a FiniteElement."));
  return *this;
}
...
FESystem<dim, spacedim>::shape_value(const unsigned int i, const Point<dim> &p) const
{
  ...
  Assert(this->is_primitive(i), (typename FiniteElement<dim, spacedim>::ExcShapeFunctionNotPrimitive(i)));

  return (base_element(this->system_to_base_table[i].first.first).shape_value(this->system_to_base_table[i].second, p));
}
                                               ...
                                               const unsigned int component) const
{
  ...
  return (base_element(base).shape_value_component(this->system_to_base_table[i].second, p, component_in_base));
}
...
FESystem<dim, spacedim>::shape_grad(const unsigned int i, const Point<dim> &p) const
{
  ...
  return (base_element(this->system_to_base_table[i].first.first).shape_grad(this->system_to_base_table[i].second, p));
}
                                              ...
                                              const unsigned int component) const
{
  ...
  // need to check this here
  return (base_element(base).shape_grad_component(this->system_to_base_table[i].second, p, component_in_base));
}
...
FESystem<dim, spacedim>::shape_grad_grad(const unsigned int i, const Point<dim> &p) const
{
  ...
  return (
    base_element(this->system_to_base_table[i].first.first).shape_grad_grad(this->system_to_base_table[i].second, p));
}
                                                   ...
                                                   const unsigned int component) const
{
  ...
  // should the respective shape function not be primitive; thus, there is no
  // need to check this here
  return (base_element(base).shape_grad_grad_component(this->system_to_base_table[i].second, p, component_in_base));
}
                                                        ...
                                                        const unsigned int component) const
{
  ...
  // need to check this here
  return (
    base_element(base).shape_3rd_derivative_component(this->system_to_base_table[i].second, p, component_in_base));
}
                                                        ...
                                                        const unsigned int component) const
{
  ...
  // need to check this here
  return (
    base_element(base).shape_4th_derivative_component(this->system_to_base_table[i].second, p, component_in_base));
}
                                                  ...
                                                  FullMatrix<double>                 &interpolation_matrix) const
{
  ...
  Assert((interpolation_matrix.m() == this->n_dofs_per_cell()) || (x_source_fe.n_dofs_per_cell() == 0),
         ExcDimensionMismatch(interpolation_matrix.m(), this->n_dofs_per_cell()));
  Assert((interpolation_matrix.n() == x_source_fe.n_dofs_per_cell()) || (this->n_dofs_per_cell() == 0),
  ...
  const FESystem<dim, spacedim> &source_fe = dynamic_cast<const FESystem<dim, spacedim> &>(x_source_fe);
  ...
  for (unsigned int i = 0; i < this->n_base_elements(); ++i)
    {
      base_matrices[i].reinit(base_element(i).n_dofs_per_cell(), source_fe.base_element(i).n_dofs_per_cell());
    ...
    for (unsigned int j = 0; j < source_fe.n_dofs_per_cell(); ++j)
      if (this->system_to_base_table[i].first == source_fe.system_to_base_table[j].first)
        interpolation_matrix(i, j) =
          (base_matrices[this->system_to_base_table[i].first.first](this->system_to_base_table[i].second,
  ...
  if (this->restriction[refinement_case - 1][child].n() == 0)
    {
      ...
      if (do_restriction)
        {
          FullMatrix<double> restriction(this->n_dofs_per_cell(), this->n_dofs_per_cell());
  ...
  if (this->prolongation[refinement_case - 1][child].n() == 0)
    {
      ...
      if (do_prolongation)
        {
          FullMatrix<double> prolongate(this->n_dofs_per_cell(), this->n_dofs_per_cell());
  ...
  dealii::internal::FEValuesImplementation::FiniteElementRelatedData<dim, spacedim> & /*output_data*/) const
{
  ...
  std::unique_ptr<typename FiniteElement<dim, spacedim>::InternalDataBase> data_ptr =
    std::make_unique<InternalData>(this->n_base_elements());
  ...
  for (unsigned int base_no = 0; base_no < this->n_base_elements(); ++base_no)
    {
      ...
      auto base_fe_data = base_element(base_no).get_data(flags, mapping, quadrature, base_fe_output_object);
  ...
  for (unsigned int base_no = 0; base_no < this->n_base_elements(); ++base_no)
    {
      internal::FEValuesImplementation::FiniteElementRelatedData<dim, spacedim> &base_fe_output_object =
        data.get_fe_output_object(base_no);
      base_fe_output_object.initialize(quadrature.max_n_quadrature_points(),
                                       base_element(base_no),
                                       flags | base_element(base_no).requires_update_flags(flags));
      ...
      // but we can't because we can't know what the elements already
      // copied and/or will want to update on every cell
      auto base_fe_data = base_element(base_no).get_face_data(flags, mapping, quadrature, base_fe_output_object);
  ...
  for (unsigned int base_no = 0; base_no < this->n_base_elements(); ++base_no)
    {
      ...
      // but we can't because we can't know what the elements already
      // copied and/or will want to update on every cell
      auto base_fe_data = base_element(base_no).get_subface_data(flags, mapping, quadrature, base_fe_output_object);
    ...
    for (unsigned int base_no = 0; base_no < this->n_base_elements(); ++base_no)
      {
        ...
        typename FiniteElement<dim, spacedim>::InternalDataBase &base_fe_data = fe_data.get_fe_data(base_no);
        ...
        if (face_no == invalid_face_number)
          {
            cell_quadrature = dynamic_cast<const Quadrature<dim> *>(&quadrature);
        ...
        else if (sub_no == invalid_face_number)
          {
            ...
            face_quadrature = dynamic_cast<const hp::QCollection<dim - 1> *>(&quadrature);
            Assert(face_quadrature != nullptr, ExcInternalError());

            n_q_points = (*face_quadrature)[face_quadrature->size() == 1 ? 0 : face_no].size();
          }
        else
          {
            sub_face_quadrature = dynamic_cast<const Quadrature<dim - 1> *>(&quadrature);
        ...
        if (face_no == invalid_face_number)
          base_fe.fill_fe_values(
            cell, cell_similarity, *cell_quadrature, mapping, mapping_internal, mapping_data, base_fe_data, base_data);
        else if (sub_no == invalid_face_number)
          base_fe.fill_fe_face_values(
            cell, face_no, *face_quadrature, mapping, mapping_internal, mapping_data, base_fe_data, base_data);
        else
        ...
        if (base_fe.is_primitive())
          {
            internal::copy_primitive_base_element_values(
              *this, base_no, base_flags, primitive_offset_tables[base_no], base_data, output_data);
          }
        else
          {
            internal::copy_nonprimitive_base_element_values(
              *this, base_no, n_q_points, base_flags, nonprimitive_offset_tables[base_no], base_data, output_data);
          }
      }
}
    ...
    for (unsigned int m = 0; m < this->interface_constraints.m(); ++m)
      {
        ...
        const std::pair<std::pair<unsigned int, unsigned int>, unsigned int> n_index =
          this->face_system_to_base_table[face_no][n];
            ...
            case 2:
              {
                ...
                if (m < this->n_dofs_per_vertex())
                  ...
                  {
                    const unsigned int index_in_line = (m - this->n_dofs_per_vertex()) % this->n_dofs_per_line();
                    const unsigned int sub_line      = (m - this->n_dofs_per_vertex()) / this->n_dofs_per_line();
                    ...
                    const unsigned int tmp1 = 2 * this->n_dofs_per_vertex() + index_in_line;
                    m_index.first           = this->face_system_to_base_table[face_no][tmp1].first;
                    ...
                    // functions:
                    Assert(this->face_system_to_base_table[face_no][tmp1].second >=
                             2 * base_element(m_index.first.first).n_dofs_per_vertex(),
                           ExcInternalError());
                    const unsigned int tmp2 = this->face_system_to_base_table[face_no][tmp1].second -
                                              2 * base_element(m_index.first.first).n_dofs_per_vertex();
                    Assert(tmp2 < base_element(m_index.first.first).n_dofs_per_line(), ExcInternalError());
                    m_index.second = base_element(m_index.first.first).n_dofs_per_vertex() +
                                     base_element(m_index.first.first).n_dofs_per_line() * sub_line + tmp2;
                  }
                break;
              }
            ...
            case 3:
              {
                ...
                if (m < 5 * this->n_dofs_per_vertex())
                  ...
                  if (m < 5 * this->n_dofs_per_vertex() + 12 * this->n_dofs_per_line())
                    {
                      // for the meaning of all this, see the 2d part
                      const unsigned int index_in_line = (m - 5 * this->n_dofs_per_vertex()) % this->n_dofs_per_line();
                      const unsigned int sub_line      = (m - 5 * this->n_dofs_per_vertex()) / this->n_dofs_per_line();
                      Assert(sub_line < 12, ExcInternalError());

                      const unsigned int tmp1 = 4 * this->n_dofs_per_vertex() + index_in_line;
                      m_index.first           = this->face_system_to_base_table[face_no][tmp1].first;
                      ...
                      const unsigned int tmp2 = this->face_system_to_base_table[face_no][tmp1].second -
                                                4 * base_element(m_index.first.first).n_dofs_per_vertex();
                      Assert(tmp2 < base_element(m_index.first.first).n_dofs_per_line(), ExcInternalError());
                      m_index.second = 5 * base_element(m_index.first.first).n_dofs_per_vertex() +
                                       base_element(m_index.first.first).n_dofs_per_line() * sub_line + tmp2;
                    }
                  else
                    // on one of the four sub-quads
                    {
                      ...
                      const unsigned int index_in_quad =
                        (m - 5 * this->n_dofs_per_vertex() - 12 * this->n_dofs_per_line()) %
                        this->n_dofs_per_quad(face_no);
                      Assert(index_in_quad < this->n_dofs_per_quad(face_no), ExcInternalError());
                      ...
                      const unsigned int tmp1 =
                        4 * this->n_dofs_per_vertex() + 4 * this->n_dofs_per_line() + index_in_quad;
                      Assert(tmp1 < this->face_system_to_base_table[face_no].size(), ExcInternalError());
                      m_index.first = this->face_system_to_base_table[face_no][tmp1].first;
                      ...
                      const unsigned int tmp2 = this->face_system_to_base_table[face_no][tmp1].second -
                                                4 * base_element(m_index.first.first).n_dofs_per_vertex() -
                                                4 * base_element(m_index.first.first).n_dofs_per_line();
                      Assert(tmp2 < base_element(m_index.first.first).n_dofs_per_quad(face_no), ExcInternalError());
                      m_index.second = 5 * base_element(m_index.first.first).n_dofs_per_vertex() +
                                       12 * base_element(m_index.first.first).n_dofs_per_line() +
                                       base_element(m_index.first.first).n_dofs_per_quad(face_no) * sub_quad + tmp2;
                    }
                                    ...
                                    const std::vector<unsigned int>                         &multiplicities)
{
  ...
  Assert(count_nonzeros(multiplicities) > 0, ExcMessage("You only passed FiniteElements with multiplicity 0."));
...

  {
    ...
    for (unsigned int face_no = 0; face_no < this->n_unique_faces(); ++face_no)
      {
        this->face_system_to_component_table[face_no].resize(this->n_dofs_per_face(face_no));

        FETools::Compositing::build_face_tables(this->face_system_to_base_table[face_no],
  ...
  init_tasks += Threads::new_task([&]() {
    ...
    for (unsigned int base_el = 0; base_el < this->n_base_elements(); ++base_el)
      if (!base_element(base_el).has_support_points() && base_element(base_el).n_dofs_per_cell() != 0)
        {
  ...
  init_tasks += Threads::new_task([&]() {
    ...
    for (unsigned int base_no = 0; base_no < this->n_base_elements(); ++base_no)
      if (base_element(base_no).is_primitive())
        primitive_offset_tables[base_no] = internal::setup_primitive_offset_table(*this, base_no);
  });
  ...
  init_tasks += Threads::new_task([&]() {
    ...
    for (unsigned int base_no = 0; base_no < this->n_base_elements(); ++base_no)
      if (!base_element(base_no).is_primitive())
        nonprimitive_offset_tables[base_no] = internal::setup_nonprimitive_offset_table(*this, base_no);
  });
      ...
      for (unsigned int face_no = 0; face_no < this->n_unique_faces(); ++face_no)
        {
          ...
          for (unsigned int base_el = 0; base_el < this->n_base_elements(); ++base_el)
            if (!base_element(base_el).has_support_points() && (base_element(base_el).n_dofs_per_face(face_no) > 0))
              {
          ...
          // elements
          this->unit_face_support_points[face_no].resize(this->n_dofs_per_face(face_no));

          for (unsigned int i = 0; i < this->n_dofs_per_face(face_no); ++i)
            {
              const unsigned int base_i        = this->face_system_to_base_table[face_no][i].first.first;
              const unsigned int index_in_base = this->face_system_to_base_table[face_no][i].second;

              Assert(index_in_base < base_element(base_i).unit_face_support_points[face_no].size(), ExcInternalError());

              this->unit_face_support_points[face_no][i] =
                base_element(base_i).unit_face_support_points[face_no][index_in_base];
            }
        }
    });
  ...
  if (dim >= 3)
    init_tasks += Threads::new_task([&]() {
      for (unsigned int face_no = 0; face_no < this->n_unique_faces(); ++face_no)
        {
          ...
          // already.
          Assert(this->adjust_quad_dof_index_for_face_orientation_table[face_no].n_elements() ==
                   this->reference_cell().n_face_orientations(face_no) * this->n_dofs_per_quad(face_no),
                 ExcInternalError());
          ...
          for (unsigned int b = 0; b < this->n_base_elements(); ++b)
            {
              const Table<2, int> &temp =
                this->base_element(b).adjust_quad_dof_index_for_face_orientation_table[face_no];
              for (unsigned int c = 0; c < this->element_multiplicity(b); ++c)
                {
                  for (unsigned int i = 0; i < temp.size(0); ++i)
                    for (unsigned int j = 0; j < this->reference_cell().n_face_orientations(face_no); ++j)
                      this->adjust_quad_dof_index_for_face_orientation_table[face_no](index + i, j) = temp(i, j);
                  index += temp.size(0);
                }
            }
      ...
      Assert(this->adjust_line_dof_index_for_line_orientation_table.size() == this->n_dofs_per_line(),
             ExcInternalError());
      unsigned int index = 0;
      for (unsigned int b = 0; b < this->n_base_elements(); ++b)
        {
          const std::vector<int> &temp2 = this->base_element(b).adjust_line_dof_index_for_line_orientation_table;
          for (unsigned int c = 0; c < this->element_multiplicity(b); ++c)
            {
...
template <int dim, int spacedim>
void
FESystem<dim, spacedim>::get_face_interpolation_matrix(const FiniteElement<dim, spacedim> &x_source_fe,
                                                       ...
                                                       const unsigned int                  face_no) const
{
  ...
  Assert(interpolation_matrix.m() == x_source_fe.n_dofs_per_face(face_no),
         ExcDimensionMismatch(interpolation_matrix.m(), x_source_fe.n_dofs_per_face(face_no)));
      ...
      while (true)
        {
          ...
          base_to_base_interpolation.reinit(base_other.n_dofs_per_face(face_no), base.n_dofs_per_face(face_no));
          base.get_face_interpolation_matrix(base_other, base_to_base_interpolation, face_no);
          ...
          for (unsigned int i = 0; i < this->n_dofs_per_face(face_no); ++i)
            if (this->face_system_to_base_index(i, face_no).first == std::make_pair(base_index, multiplicity))
              for (unsigned int j = 0; j < fe_other_system->n_dofs_per_face(face_no); ++j)
                if (fe_other_system->face_system_to_base_index(j, face_no).first ==
                    std::make_pair(base_index_other, multiplicity_other))
                  interpolation_matrix(j, i) =
                    base_to_base_interpolation(fe_other_system->face_system_to_base_index(j, face_no).second,
...
template <int dim, int spacedim>
void
FESystem<dim, spacedim>::get_subface_interpolation_matrix(const FiniteElement<dim, spacedim> &x_source_fe,
                                                          ...
                                                          const unsigned int                  face_no) const
{
  ...
  // FESystem(FE_Q(p),1,FE_Q(q),2) vs
  // FESystem(FESystem(FE_Q(r),2),1,FE_Q(s),1)
  const FESystem<dim, spacedim> *fe_other_system = dynamic_cast<const FESystem<dim, spacedim> *>(&x_source_fe);
  if (fe_other_system != nullptr)
      ...
      while (true)
        {
          ...
          // get the interpolation from the bases
          base_to_base_interpolation.reinit(base_other.n_dofs_per_face(face_no), base.n_dofs_per_face(face_no));
          base.get_subface_interpolation_matrix(base_other, subface, base_to_base_interpolation, face_no);
  ...
  else
    {
      ...
      Assert(fe_other_system != nullptr, (typename FiniteElement<dim, spacedim>::ExcInterpolationNotImplemented()));
    }
}
...
FESystem<dim, spacedim>::hp_object_dof_identities(const FiniteElement<dim, spacedim> &fe_other,
                                                  const unsigned int                  face_no) const
{
  ...
  // FESystem(FE_Q(p),1,FE_Q(q),2) vs
  // FESystem(FESystem(FE_Q(r),2),1,FE_Q(s),1)
  if (const FESystem<dim, spacedim> *fe_other_system = dynamic_cast<const FESystem<dim, spacedim> *>(&fe_other))
    {
      ...
      while (true)
        {
          ...
          switch (structdim)
            {
              ...
              case 1:
                ...
                break;
              case 2:
                base_identities = base.hp_quad_dof_identities(base_other, face_no);
                break;
              default:
                Assert(false, ExcNotImplemented());
            }
          ...
          for (const auto &base_identity : base_identities)
            identities.emplace_back(base_identity.first + dof_offset, base_identity.second + dof_offset_other);
...
template <int dim, int spacedim>
std::vector<std::pair<unsigned int, unsigned int>>
FESystem<dim, spacedim>::hp_quad_dof_identities(const FiniteElement<dim, spacedim> &fe_other,
                                                const unsigned int                  face_no) const
{
...
FESystem<dim, spacedim>::compare_for_domination(const FiniteElement<dim, spacedim> &fe_other,
                                                const unsigned int                  codim) const
{
  ...
  if (const FESystem<dim, spacedim> *fe_sys_other = dynamic_cast<const FESystem<dim, spacedim> *>(&fe_other))
    {
      ...
      for (unsigned int b = 0; b < this->n_base_elements(); ++b)
        {
          Assert(this->base_element(b).n_components() == fe_sys_other->base_element(b).n_components(),
                 ExcNotImplemented());
          Assert(this->element_multiplicity(b) == fe_sys_other->element_multiplicity(b), ExcNotImplemented());
          ...
          const FiniteElementDomination::Domination base_domination =
            (this->base_element(b).compare_for_domination(fe_sys_other->base_element(b), codim));
...
template <int dim, int spacedim>
bool
FESystem<dim, spacedim>::has_support_on_face(const unsigned int shape_index, const unsigned int face_index) const
{
...
FESystem<dim, spacedim>::unit_support_point(const unsigned int index) const
{
  ...
  Assert((this->unit_support_points.size() == this->n_dofs_per_cell()) || (this->unit_support_points.empty()),
...
template <int dim, int spacedim>
Point<dim - 1>
FESystem<dim, spacedim>::unit_face_support_point(const unsigned int index, const unsigned int face_no) const
{
  AssertIndexRange(index, this->n_dofs_per_face(face_no));
  Assert((this->unit_face_support_points[this->n_unique_faces() == 1 ? 0 : face_no].size() ==
          this->n_dofs_per_face(face_no)) ||
           (this->unit_face_support_points[this->n_unique_faces() == 1 ? 0 : face_no].empty()),
         (typename FiniteElement<dim, spacedim>::ExcFEHasNoSupportPoints()));

  // let's see whether we have the information pre-computed
  if (this->unit_face_support_points[this->n_unique_faces() == 1 ? 0 : face_no].size() != 0)
    return this->unit_face_support_points[this->n_unique_faces() == 1 ? 0 : face_no][index];
  else
    // no. ask the base element whether it would like to provide this
    // information
    return (base_element(this->face_system_to_base_index(index, face_no).first.first)
              .unit_face_support_point(this->face_system_to_base_index(index, face_no).second, face_no));
}
  ...
  for (unsigned int i = 0; i < base_elements.size(); ++i)
    {
      ...
      for (unsigned int k = 0; k < this->n_dofs_per_cell(); ++k)
        {
          std::pair<std::pair<unsigned int, unsigned int>, unsigned int> ind = this->system_to_base_index(k);
      ...
      for (unsigned int r = 0; r < element_multiplicity; ++r)
        for (const unsigned int c : base_table.second)
          components.push_back(comp + r * this->base_elements[i].first->n_components() + c);
    }
...
{
  Assert(this->has_generalized_support_points(), ExcMessage("The FESystem does not have generalized support points"));
      ...
      if (base_element.has_generalized_support_points())
        {
          ...
          for (unsigned int m = 0; m < multiplicity; ++m, current_vector_component += n_base_components)
            {
              ...
              for (unsigned int j = 0; j < base_point_values.size(); ++j)
                {
                  ...
                  // vector of values:
                  const auto *const begin = std::begin(point_values[n]) + current_vector_component;
                  const auto *const end   = begin + n_base_components;
                  std::copy(begin, end, std::begin(base_point_values[j]));
                }

              base_element.convert_generalized_support_point_values_to_dof_values(base_point_values, base_dof_values);

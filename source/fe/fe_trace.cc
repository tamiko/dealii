  ...
  , fe_q(degree)
{
  Assert(degree > 0,
         ExcMessage("FE_Trace can only be used for polynomial degrees "
                    "greater than zero"));
  this->poly_space.set_numbering(FETools::hierarchic_to_lexicographic_numbering<dim - 1>(degree));
  ...
  for (unsigned int face_no = 0; face_no < this->n_unique_faces(); ++face_no)
    this->unit_face_support_points[face_no] = fe_q.get_unit_face_support_points(face_no);
...
template <int dim, int spacedim>
bool
FE_TraceQ<dim, spacedim>::has_support_on_face(const unsigned int shape_index, const unsigned int face_index) const
{
...
FE_TraceQ<dim, spacedim>::compare_for_domination(const FiniteElement<dim, spacedim> &fe_other,
                                                 const unsigned int                  codim) const
{
  ...
  // --------------------------------
  if (const FE_TraceQ<dim, spacedim> *fe_traceq_other = dynamic_cast<const FE_TraceQ<dim, spacedim> *>(&fe_other))
    {
...
template <int dim, int spacedim>
void
FE_TraceQ<dim, spacedim>::get_face_interpolation_matrix(const FiniteElement<dim, spacedim> &source_fe,
...
template <int dim, int spacedim>
void
FE_TraceQ<dim, spacedim>::get_subface_interpolation_matrix(const FiniteElement<dim, spacedim> &x_source_fe,
                                                           ...
                                                           const unsigned int                  face_no) const
{
  ...
  if (const FE_TraceQ<dim, spacedim> *source_fe = dynamic_cast<const FE_TraceQ<dim, spacedim> *>(&x_source_fe))
    {
      fe_q.get_subface_interpolation_matrix(source_fe->fe_q, subface, interpolation_matrix, face_no);
    }

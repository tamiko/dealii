...
namespace internal
{
  namespace FEValuesImplementation
  {
                                                        ...
                                                        const UpdateFlags                   flags)
    {
      ...
      Assert(n_nonzero_shape_components >= fe.n_dofs_per_cell(), ExcInternalError());
...
FEValues<dim, spacedim>::FEValues(const Mapping<dim, spacedim>       &mapping,
  ...
  : FEValuesBase<dim, spacedim>(q.size(), fe.n_dofs_per_cell(), update_default, mapping, fe)
  , quadrature(q)
{
...
FEValues<dim, spacedim>::initialize(const UpdateFlags update_flags)
{
  ...
  this->finite_element_output.initialize(this->max_n_quadrature_points, *this->fe, flags);
  ...
  Threads::Task<std::unique_ptr<typename FiniteElement<dim, spacedim>::InternalDataBase>> fe_get_data =
    Threads::new_task(
      [&]() { return this->fe->get_data(flags, *this->mapping, quadrature, this->finite_element_output); });

  Threads::Task<std::unique_ptr<typename Mapping<dim, spacedim>::InternalDataBase>> mapping_get_data;
  if (flags & update_mapping)
    mapping_get_data = Threads::new_task([&]() { return this->mapping->get_data(flags, quadrature); });
  ...
  if (flags & update_mapping)
    ...
    this->mapping_data = std::make_unique<typename Mapping<dim, spacedim>::InternalDataBase>();
}
...
FEValues<dim, spacedim>::reinit(const typename Triangulation<dim, spacedim>::cell_iterator &cell)
{
  ...
  Assert(this->get_mapping().is_compatible_with(cell->reference_cell()),
         ExcMessage("You are trying to call FEValues::reinit() with a cell of type " +
...
template <bool lda>
void
FEValues<dim, spacedim>::reinit(const TriaIterator<DoFCellAccessor<dim, spacedim, lda>> &cell)
{
...
FEValues<dim, spacedim>::do_reinit()
{
  ...
  if (this->update_flags & update_mapping)
    {
      this->cell_similarity = this->get_mapping().fill_fe_values(
        this->present_cell, this->cell_similarity, quadrature, *this->mapping_data, this->mapping_output);
    }
...
{
  return (FEValuesBase<dim, spacedim>::memory_consumption() + MemoryConsumption::memory_consumption(quadrature));
}
...
FEFaceValuesBase<dim, spacedim>::FEFaceValuesBase(const unsigned int                  dofs_per_cell,
  ...
  : FEFaceValuesBase<dim, spacedim>(dofs_per_cell, flags, mapping, fe, hp::QCollection<dim - 1>(quadrature))
{}
...
FEFaceValuesBase<dim, spacedim>::FEFaceValuesBase(const unsigned int dofs_per_cell,
  ...
  : FEValuesBase<dim, spacedim>(quadrature.max_n_quadrature_points(), dofs_per_cell, update_default, mapping, fe)
  , present_face_index(numbers::invalid_unsigned_int)
  , quadrature(quadrature)
{
  Assert(quadrature.size() == 1 || quadrature.size() == fe.reference_cell().n_faces(), ExcInternalError());
}
...
FEFaceValues<dim, spacedim>::FEFaceValues(const Mapping<dim, spacedim>       &mapping,
                                          ...
                                          const Quadrature<dim - 1>          &quadrature,
                                          const UpdateFlags                   update_flags)
  : FEFaceValues<dim, spacedim>(mapping, fe, hp::QCollection<dim - 1>(quadrature), update_flags)
{}
...
FEFaceValues<dim, spacedim>::FEFaceValues(const Mapping<dim, spacedim>       &mapping,
                                          ...
                                          const hp::QCollection<dim - 1>     &quadrature,
                                          const UpdateFlags                   update_flags)
  : FEFaceValuesBase<dim, spacedim>(fe.n_dofs_per_cell(), update_flags, mapping, fe, quadrature)
{
  initialize(update_flags);
}
...
FEFaceValues<dim, spacedim>::FEFaceValues(const FiniteElement<dim, spacedim> &fe,
                                          const Quadrature<dim - 1>          &quadrature,
                                          const UpdateFlags                   update_flags)
  : FEFaceValues<dim, spacedim>(fe, hp::QCollection<dim - 1>(quadrature), update_flags)
{}
...
FEFaceValues<dim, spacedim>::initialize(const UpdateFlags update_flags)
{
  ...
  std::unique_ptr<typename Mapping<dim, spacedim>::InternalDataBase> (
    Mapping<dim, spacedim>::*mapping_get_face_data)(const UpdateFlags, const hp::QCollection<dim - 1> &) const =
    &Mapping<dim, spacedim>::get_face_data;


  Threads::Task<std::unique_ptr<typename FiniteElement<dim, spacedim>::InternalDataBase>> fe_get_data =
    Threads::new_task(
      finite_element_get_face_data, *this->fe, flags, *this->mapping, this->quadrature, this->finite_element_output);
  Threads::Task<std::unique_ptr<typename Mapping<dim, spacedim>::InternalDataBase>> mapping_get_data;
  if (flags & update_mapping)
    mapping_get_data = Threads::new_task(mapping_get_face_data, *this->mapping, flags, this->quadrature);
...
template <bool lda>
void
FEFaceValues<dim, spacedim>::reinit(const TriaIterator<DoFCellAccessor<dim, spacedim, lda>> &cell,
                                    const unsigned int                                       face_no)
{
  // assert that the finite elements passed to the constructor and
  // used by the DoFHandler used by this cell, are the same
  Assert(static_cast<const FiniteElementData<dim> &>(*this->fe) ==
           static_cast<const FiniteElementData<dim> &>(cell->get_dof_handler().get_fe(cell->active_fe_index())),
         (typename FEValuesBase<dim, spacedim>::ExcFEDontMatch()));
...
FESubfaceValues<dim, spacedim>::initialize(const UpdateFlags update_flags)
{
  ...
  if (flags & update_mapping)
    mapping_get_data =
      Threads::new_task(&Mapping<dim, spacedim>::get_subface_data, *this->mapping, flags, this->quadrature[0]);
...
template <bool lda>
void
FESubfaceValues<dim, spacedim>::reinit(const TriaIterator<DoFCellAccessor<dim, spacedim, lda>> &cell,
                                       const unsigned int                                       face_no,
                                       const unsigned int                                       subface_no)
{
  ...
  Assert(cell->face(face_no)->has_children() || subface_no < GeometryInfo<dim>::max_children_per_face,
         ExcIndexRange(subface_no, 0, GeometryInfo<dim>::max_children_per_face));
  Assert(!cell->face(face_no)->has_children() || subface_no < cell->face(face_no)->n_active_descendants(),
         ExcIndexRange(subface_no, 0, cell->face(face_no)->n_active_descendants()));
...
{
  reinit(cell, cell->face_iterator_to_index(face), face->child_iterator_to_index(subface));
}
                                       ...
                                       const unsigned int                                          subface_no)
{
  ...
  AssertIndexRange(subface_no,
                   (cell->has_periodic_neighbor(face_no) ?
                      cell->periodic_neighbor(face_no)->face(cell->periodic_neighbor_face_no(face_no))->n_children() :
...
FESubfaceValues<dim, spacedim>::do_reinit(const unsigned int face_no, const unsigned int subface_no)
{
  ...
  if (this->update_flags & update_mapping)
    {
      this->get_mapping().fill_fe_subface_values(
        this->present_cell, face_no, subface_no, this->quadrature[0], *this->mapping_data, this->mapping_output);
    }

...
namespace internal
{
  ...
  {
    std::vector<unsigned int> shape_function_to_row_table(fe.n_dofs_per_cell() * fe.n_components(),
  ...
  namespace
  {
    ...
    struct CheckForZero<Number, std::enable_if_t<Differentiation::AD::is_ad_number<Number>::value>>
    {
      static bool
      value(const Number & /*value*/)
      {
...
FEValuesBase<dim, spacedim>::CellIteratorContainer::n_dofs_for_dof_handler() const
{
  Assert(is_initialized(), ExcNotReinited());
...
template <typename Number>
void
FEValuesBase<dim, spacedim>::CellIteratorContainer::get_interpolated_dof_values(const ReadVector<Number> &in,
                                                                                Vector<Number>           &out) const
{
  ...
  if (level_dof_access)
    DoFCellAccessor<dim, spacedim, true>(&cell->get_triangulation(), cell->level(), cell->index(), dof_handler)
      .get_interpolated_dof_values(in, out);
  else
    DoFCellAccessor<dim, spacedim, false>(&cell->get_triangulation(), cell->level(), cell->index(), dof_handler)
      .get_interpolated_dof_values(in, out);
}
...
template <int dim, int spacedim>
void
FEValuesBase<dim, spacedim>::CellIteratorContainer::get_interpolated_dof_values(const IndexSet               &in,
                                                                                Vector<IndexSet::value_type> &out) const
{
  ...
  const DoFCellAccessor<dim, spacedim, false> cell_dofs(&cell->get_triangulation(),
                                                        cell->level(),
                                                        cell->index(),
                                                        dof_handler);

  std::vector<types::global_dof_index> dof_indices(cell_dofs.get_fe().n_dofs_per_cell());
...
namespace internal
{
                     ...
                     std::vector<Number>            &values)
  {
    ...
    std::fill_n(values.begin(), n_quadrature_points, dealii::internal::NumberType<Number>::value(0.0));
  ...
  void
  do_function_values(const ArrayView<typename VectorType::value_type> &dof_values,
                     const dealii::Table<2, double>                   &shape_values,
                     const FiniteElement<dim, spacedim>               &fe,
                     ...
                     const unsigned int                                component_multiple        = 1)
  {
    ...
    const unsigned int n_quadrature_points = quadrature_points_fastest ? values[0].size() : values.size();
      ...
      for (unsigned int shape_func = 0; shape_func < dofs_per_cell; ++shape_func)
        {
          ...
          if (fe.is_primitive(shape_func))
            {
              const unsigned int comp = fe.system_to_component_index(shape_func).first + mc * n_components;
              const unsigned int row  = shape_function_to_row_table[shape_func * n_components + comp];
                          ...
                          const unsigned int                                      component_multiple        = 1)
  {
    ...
    const unsigned int n_quadrature_points = quadrature_points_fastest ? derivatives[0].size() : derivatives.size();
    const unsigned int n_components        = fe.n_components();
  ...
  void
  do_function_laplacians(const ArrayView<Number2>                    &dof_values,
                         ...
                         std::vector<Number>                         &laplacians)
  {
    ...
    // initialize with zero
    std::fill_n(laplacians.begin(), n_quadrature_points, dealii::internal::NumberType<Number>::value(0.0));
...
template <typename Number>
void
FEValuesBase<dim, spacedim>::get_function_values(const ReadVector<Number> &fe_function,
                                                 std::vector<Number>      &values) const
{
  ...
  internal::do_function_values(make_array_view(dof_values.begin(), dof_values.end()),
                                                 ...
                                                 std::vector<Number>                            &values) const
{
  ...
  internal::do_function_values(view, this->finite_element_output.shape_values, values);
}
...
template <typename Number>
void
FEValuesBase<dim, spacedim>::get_function_gradients(const ReadVector<Number>                 &fe_function,
                                                    std::vector<Tensor<1, spacedim, Number>> &gradients) const
{
  ...
  Vector<Number> dof_values(dofs_per_cell);
  present_cell.get_interpolated_dof_values(fe_function, dof_values);
  internal::do_function_derivatives(make_array_view(dof_values.begin(), dof_values.end()),
                                                    ...
                                                    std::vector<Tensor<1, spacedim, Number>>       &gradients) const
{
  ...
  auto                                        view = make_array_view(dof_values.begin(), dof_values.end());
  fe_function.extract_subvector_to(indices, view);
  internal::do_function_derivatives(view, this->finite_element_output.shape_gradients, gradients);
}
...
template <typename Number>
void
FEValuesBase<dim, spacedim>::get_function_hessians(const ReadVector<Number>                 &fe_function,
                                                   ...
                                                   std::vector<Tensor<2, spacedim, Number>>       &hessians) const
{
  ...
  auto                                        view = make_array_view(dof_values.begin(), dof_values.end());
  fe_function.extract_subvector_to(indices, view);
  internal::do_function_derivatives(view, this->finite_element_output.shape_hessians, hessians);
}
...
template <typename Number>
void
FEValuesBase<dim, spacedim>::get_function_laplacians(const ReadVector<Number> &fe_function,
                                                     std::vector<Number>      &laplacians) const
{
  ...
  Vector<Number> dof_values(dofs_per_cell);
  present_cell.get_interpolated_dof_values(fe_function, dof_values);
  internal::do_function_laplacians(make_array_view(dof_values.begin(), dof_values.end()),
                                   this->finite_element_output.shape_hessians,
                                   laplacians);
}
                                                     ...
                                                     std::vector<Number>                            &laplacians) const
{
  ...
  auto                                        view = make_array_view(dof_values.begin(), dof_values.end());
  fe_function.extract_subvector_to(indices, view);
  internal::do_function_laplacians(view, this->finite_element_output.shape_hessians, laplacians);
}
  ...
  std::vector<Tensor<3, spacedim, Number>>       &third_derivatives) const
{
  ...
  auto                                        view = make_array_view(dof_values.begin(), dof_values.end());
  fe_function.extract_subvector_to(indices, view);
  internal::do_function_derivatives(view, this->finite_element_output.shape_3rd_derivatives, third_derivatives);
}
...
FEValuesBase<dim, spacedim>::memory_consumption() const
{
  return (sizeof(this->update_flags) + MemoryConsumption::memory_consumption(n_quadrature_points) +
          MemoryConsumption::memory_consumption(max_n_quadrature_points) + sizeof(cell_similarity) +
          MemoryConsumption::memory_consumption(dofs_per_cell) + MemoryConsumption::memory_consumption(mapping) +
          MemoryConsumption::memory_consumption(mapping_data) + MemoryConsumption::memory_consumption(*mapping_data) +
          MemoryConsumption::memory_consumption(mapping_output) + MemoryConsumption::memory_consumption(fe) +
          MemoryConsumption::memory_consumption(fe_data) + MemoryConsumption::memory_consumption(*fe_data) +
  ...
  if (present_cell.is_initialized())
    {
      if (&cell->get_triangulation() !=
          &present_cell.operator typename Triangulation<dim, spacedim>::cell_iterator()->get_triangulation())
        {
          ...
          tria_listener_refinement =
            cell->get_triangulation().signals.any_change.connect([this]() { this->invalidate_present_cell(); });
          tria_listener_mesh_transform =
            cell->get_triangulation().signals.mesh_movement.connect([this]() { this->invalidate_present_cell(); });
        }
    }
  else
    {
      ...
      // changes
      tria_listener_refinement =
        cell->get_triangulation().signals.post_refinement.connect([this]() { this->invalidate_present_cell(); });
      tria_listener_mesh_transform =
        cell->get_triangulation().signals.mesh_movement.connect([this]() { this->invalidate_present_cell(); });
    }
}
...
template <int dim, int spacedim>
inline void
FEValuesBase<dim, spacedim>::check_cell_similarity(const typename Triangulation<dim, spacedim>::cell_iterator &cell)
{
  ...
  if ((dim < spacedim) && (cell_similarity == CellSimilarity::translation))
    {
      if (static_cast<const typename Triangulation<dim, spacedim>::cell_iterator &>(this->present_cell)

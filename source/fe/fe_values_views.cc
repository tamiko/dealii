...
{
  template <int dim, int spacedim>
  Scalar<dim, spacedim>::Scalar(const FEValuesBase<dim, spacedim> &fe_values, const unsigned int component)
    : fe_values(&fe_values)
    , component(component)
    ...
    for (unsigned int i = 0; i < fe.n_dofs_per_cell(); ++i)
      {
        ...
        if (shape_function_data[i].is_nonzero_shape_function_component == true)
          shape_function_data[i].row_index = shape_function_to_row_table[i * fe.n_components() + component];
        else
  ...
  template <int dim, int spacedim>
  Vector<dim, spacedim>::Vector(const FEValuesBase<dim, spacedim> &fe_values, const unsigned int first_vector_component)
    : fe_values(&fe_values)
        ...
        for (unsigned int i = 0; i < fe.n_dofs_per_cell(); ++i)
          {
            ...
            if (shape_function_data[i].is_nonzero_shape_function_component[d] == true)
              shape_function_data[i].row_index[d] = shape_function_to_row_table[i * fe.n_components() + component];
            else
        ...
        if (n_nonzero_components == 0)
        ...
        else
          {
            for (unsigned int d = 0; d < spacedim; ++d)
              if (shape_function_data[i].is_nonzero_shape_function_component[d] == true)
                {
                  shape_function_data[i].single_nonzero_component       = shape_function_data[i].row_index[d];
    ...
    , shape_function_data(this->fe_values->fe->n_dofs_per_cell())
  {
    ...
    Assert(first_tensor_component + (dim * dim + dim) / 2 - 1 < fe.n_components(),
           ExcIndexRange(first_tensor_component + dealii::SymmetricTensor<2, dim>::n_independent_components - 1,
                         0,
                         fe.n_components()));
    // TODO: we'd like to use the fields with the same name as these
    // variables from FEValuesBase, but they aren't initialized yet
  ...
  void
  Scalar<dim, spacedim>::get_function_values(const ReadVector<Number>                 &fe_function,
                                             std::vector<solution_value_type<Number>> &values) const
  {
    ...
    Assert(fe_values->present_cell.is_initialized(), (typename FEValuesBase<dim, spacedim>::ExcNotReinited()));
    ...
    internal::do_function_values<dim, spacedim>(make_const_array_view(dof_values),
  ...
  template <typename Number>
  void
  Scalar<dim, spacedim>::get_function_gradients(const ReadVector<Number>                    &fe_function,
                                                std::vector<solution_gradient_type<Number>> &gradients) const
  {
    ...
    dealii::Vector<Number> dof_values(fe_values->dofs_per_cell);
    fe_values->present_cell.get_interpolated_dof_values(fe_function, dof_values);
    internal::do_function_derivatives<1, dim, spacedim>(make_const_array_view(dof_values),
  ...
  template <typename Number>
  void
  Scalar<dim, spacedim>::get_function_hessians(const ReadVector<Number>                   &fe_function,
                                               std::vector<solution_hessian_type<Number>> &hessians) const
  {
    ...
    dealii::Vector<Number> dof_values(fe_values->dofs_per_cell);
    fe_values->present_cell.get_interpolated_dof_values(fe_function, dof_values);
    internal::do_function_derivatives<2, dim, spacedim>(make_const_array_view(dof_values),
  ...
  template <typename Number>
  void
  Scalar<dim, spacedim>::get_function_laplacians(const ReadVector<Number>                     &fe_function,
                                                 std::vector<solution_laplacian_type<Number>> &laplacians) const
  {
    ...
    dealii::Vector<Number> dof_values(fe_values->dofs_per_cell);
    fe_values->present_cell.get_interpolated_dof_values(fe_function, dof_values);
    internal::do_function_laplacians<dim, spacedim>(make_const_array_view(dof_values),
                                                    fe_values->finite_element_output.shape_hessians,
                                                    shape_function_data,
                                                    laplacians);
  }
    ...
    std::vector<solution_third_derivative_type<Number>> &third_derivatives) const
  {
    ...
    dealii::Vector<Number> dof_values(fe_values->dofs_per_cell);
    fe_values->present_cell.get_interpolated_dof_values(fe_function, dof_values);
    internal::do_function_derivatives<3, dim, spacedim>(make_const_array_view(dof_values),
  ...
  template <typename Number>
  void
  Vector<dim, spacedim>::get_function_values(const ReadVector<Number>                 &fe_function,
                                             std::vector<solution_value_type<Number>> &values) const
  {
    Assert(fe_values->update_flags & update_values,
           (typename FEValuesBase<dim, spacedim>::ExcAccessToUninitializedField("update_values")));
  ...
  template <typename Number>
  void
  Vector<dim, spacedim>::get_function_gradients(const ReadVector<Number>                    &fe_function,
                                                std::vector<solution_gradient_type<Number>> &gradients) const
  {
    Assert(fe_values->update_flags & update_gradients,
           (typename FEValuesBase<dim, spacedim>::ExcAccessToUninitializedField("update_gradients")));
    ...
    std::vector<solution_symmetric_gradient_type<Number>> &symmetric_gradients) const
  {
    ...
    dealii::Vector<Number> dof_values(fe_values->dofs_per_cell);
    fe_values->present_cell.get_interpolated_dof_values(fe_function, dof_values);
    internal::do_function_symmetric_gradients<dim, spacedim>(make_const_array_view(dof_values),
                                                             fe_values->finite_element_output.shape_gradients,
                                                             shape_function_data,
                                                             symmetric_gradients);
  }
  ...
  template <typename Number>
  void
  Vector<dim, spacedim>::get_function_divergences(const ReadVector<Number>                      &fe_function,
                                                  std::vector<solution_divergence_type<Number>> &divergences) const
  {
    ...
    dealii::Vector<Number> dof_values(fe_values->dofs_per_cell);
    fe_values->present_cell.get_interpolated_dof_values(fe_function, dof_values);
    internal::do_function_divergences<dim, spacedim>(make_const_array_view(dof_values),
                                                     fe_values->finite_element_output.shape_gradients,
                                                     shape_function_data,
                                                     divergences);
  }
  ...
  template <typename Number>
  void
  Vector<dim, spacedim>::get_function_curls(const ReadVector<Number>                &fe_function,
                                            std::vector<solution_curl_type<Number>> &curls) const
  {
    Assert(fe_values->update_flags & update_gradients,
           (typename FEValuesBase<dim, spacedim>::ExcAccessToUninitializedField("update_gradients")));
    Assert(fe_values->present_cell.is_initialized(), ExcMessage("FEValues object is not reinited to any cell"));
    AssertDimension(fe_function.size(), fe_values->present_cell.n_dofs_for_dof_handler());

    // get function values of dofs on this cell
    dealii::Vector<Number> dof_values(fe_values->dofs_per_cell);
    fe_values->present_cell.get_interpolated_dof_values(fe_function, dof_values);
    internal::do_function_curls<dim, spacedim>(make_const_array_view(dof_values),
                                               fe_values->finite_element_output.shape_gradients,
                                               shape_function_data,
                                               curls);
  }
  ...
  template <typename Number>
  void
  Vector<dim, spacedim>::get_function_hessians(const ReadVector<Number>                   &fe_function,
                                               std::vector<solution_hessian_type<Number>> &hessians) const
  {
    Assert(fe_values->update_flags & update_hessians,
           (typename FEValuesBase<dim, spacedim>::ExcAccessToUninitializedField("update_hessians")));
  ...
  template <typename Number>
  void
  Vector<dim, spacedim>::get_function_laplacians(const ReadVector<Number>                 &fe_function,
  ...
  template <typename Number>
  void
  SymmetricTensor<2, dim, spacedim>::get_function_values(const ReadVector<Number>                 &fe_function,
                                                         std::vector<solution_value_type<Number>> &values) const
  {
    Assert(fe_values->update_flags & update_values,
           (typename FEValuesBase<dim, spacedim>::ExcAccessToUninitializedField("update_values")));
  ...
  template <typename Number>
  void
  Tensor<2, dim, spacedim>::get_function_values(const ReadVector<Number>                 &fe_function,
                                                std::vector<solution_value_type<Number>> &values) const
  {
    Assert(fe_values->update_flags & update_values,
           (typename FEValuesBase<dim, spacedim>::ExcAccessToUninitializedField("update_values")));
  ...
  template <typename Number>
  void
  Tensor<2, dim, spacedim>::get_function_divergences(const ReadVector<Number>                      &fe_function,
                                                     std::vector<solution_divergence_type<Number>> &divergences) const
  {
    Assert(fe_values->update_flags & update_gradients,
           (typename FEValuesBase<dim, spacedim>::ExcAccessToUninitializedField("update_gradients")));
  ...
  template <typename Number>
  void
  Tensor<2, dim, spacedim>::get_function_gradients(const ReadVector<Number>                    &fe_function,
                                                   std::vector<solution_gradient_type<Number>> &gradients) const
  {
    Assert(fe_values->update_flags & update_gradients,
           (typename FEValuesBase<dim, spacedim>::ExcAccessToUninitializedField("update_gradients")));
    ...
    dealii::Vector<Number> dof_values(fe_values->dofs_per_cell);
    fe_values->present_cell.get_interpolated_dof_values(fe_function, dof_values);
    internal::do_function_gradients<dim, spacedim>(make_const_array_view(dof_values),
                                                   fe_values->finite_element_output.shape_gradients,
                                                   shape_function_data,
                                                   gradients);
  }
...
namespace internal
{
  namespace FEValuesViews
  {
    ...
    Cache<dim, spacedim>::Cache(const FEValuesBase<dim, spacedim> &fe_values)
    {
      ...
      const unsigned int n_vectors = (fe.n_components() >= Tensor<1, spacedim>::n_independent_components ?
      ...
      const unsigned int n_symmetric_second_order_tensors =
        (fe.n_components() >= SymmetricTensor<2, spacedim>::n_independent_components ?
           fe.n_components() - SymmetricTensor<2, spacedim>::n_independent_components + 1 :
           0);
      ...
      const unsigned int n_second_order_tensors =
        (fe.n_components() >= Tensor<2, spacedim>::n_independent_components ?
           fe.n_components() - Tensor<2, spacedim>::n_independent_components + 1 :
           0);

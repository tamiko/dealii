  ...
  namespace internal
  {
        ...
        if (shape_function_data[shape_function].is_nonzero_shape_function_component)
          {
            ...
            const double *shape_value_ptr = &shape_values(shape_function_data[shape_function].row_index, 0);
      ...
      std::vector<typename Scalar<dim, spacedim>::template solution_laplacian_type<Number>> &laplacians)
    {
      ...
      std::fill(laplacians.begin(),
                laplacians.end(),
                typename Scalar<dim, spacedim>::template solution_laplacian_type<Number>());
          ...
          if (snc != -1)
            ...
            for (unsigned int d = 0; d < spacedim; ++d)
              if (shape_function_data[shape_function].is_nonzero_shape_function_component[d])
                {
                  const double *shape_value_ptr = &shape_values(shape_function_data[shape_function].row_index[d], 0);
                  for (unsigned int q_point = 0; q_point < n_quadrature_points; ++q_point)
      ...
      for (unsigned int shape_function = 0; shape_function < dofs_per_cell; ++shape_function)
        {
          ...
          if (snc != -1)
            {
              const unsigned int comp = shape_function_data[shape_function].single_nonzero_component_index;
              const dealii::Tensor<order, spacedim> *shape_derivative_ptr = &shape_derivatives[snc][0];
              for (unsigned int q_point = 0; q_point < n_quadrature_points; ++q_point)
              ...
              for (unsigned int q_point = 0; q_point < n_quadrature_points; ++q_point)
                {
                  for (unsigned int d = 0; d < dim; ++d)
                    symmetric_gradients[q_point][comp][d] += 0.5 * value * (*shape_gradient_ptr)[d];
                  symmetric_gradients[q_point][comp][comp] += 0.5 * value * (*shape_gradient_ptr++)[comp];
                }
            }
          else
            for (unsigned int q_point = 0; q_point < n_quadrature_points; ++q_point)
              {
                ...
                for (unsigned int d = 0; d < spacedim; ++d)
                  if (shape_function_data[shape_function].is_nonzero_shape_function_component[d])
                    grad[d] = value * shape_gradients[shape_function_data[shape_function].row_index[d]][q_point];
      ...
      std::vector<typename Vector<dim, spacedim>::template solution_divergence_type<Number>> &divergences)
    {
      ...
      std::fill(divergences.begin(),
                divergences.end(),
                typename Vector<dim, spacedim>::template solution_divergence_type<Number>());
                  ...
                  if (snc != -1)
                    {
                      ...
                      Assert(shape_function_data[shape_function].single_nonzero_component >= 0, ExcInternalError());
      ...
      std::vector<typename Vector<dim, spacedim>::template solution_laplacian_type<Number>> &laplacians)
    {
      ...
      std::fill(laplacians.begin(),
                laplacians.end(),
                typename Vector<dim, spacedim>::template solution_laplacian_type<Number>());
      ...
      for (unsigned int shape_function = 0; shape_function < dofs_per_cell; ++shape_function)
        {
          ...
          if (snc != -1)
            {
              const TableIndices<2> comp = dealii::SymmetricTensor<2, spacedim>::unrolled_to_component_indices(
            ...
            for (unsigned int d = 0; d < dealii::SymmetricTensor<2, spacedim>::n_independent_components; ++d)
              if (shape_function_data[shape_function].is_nonzero_shape_function_component[d])
                {
                  const TableIndices<2> comp = dealii::SymmetricTensor<2, spacedim>::unrolled_to_component_indices(d);
                  const double *shape_value_ptr = &shape_values(shape_function_data[shape_function].row_index[d], 0);
                  for (unsigned int q_point = 0; q_point < n_quadrature_points; ++q_point)
                    values[q_point][comp] += value * (*shape_value_ptr++);
                }
      ...
      std::vector<typename SymmetricTensor<2, dim, spacedim>::template solution_divergence_type<Number>> &divergences)
    {
      ...
      std::fill(divergences.begin(),
                divergences.end(),
                typename SymmetricTensor<2, dim, spacedim>::template solution_divergence_type<Number>());
          ...
          if (snc != -1)
            {
              ...
              const unsigned int ii = dealii::SymmetricTensor<2, spacedim>::unrolled_to_component_indices(comp)[0];
              const unsigned int jj = dealii::SymmetricTensor<2, spacedim>::unrolled_to_component_indices(comp)[1];

              for (unsigned int q_point = 0; q_point < n_quadrature_points; ++q_point, ++shape_gradient_ptr)
                {
                       ...
                       std::vector<typename ProductType<Number, dealii::Tensor<2, spacedim>>::type> &values)
    {
      ...
      std::fill(values.begin(), values.end(), typename ProductType<Number, dealii::Tensor<2, spacedim>>::type());
      ...
      std::vector<typename Tensor<2, dim, spacedim>::template solution_divergence_type<Number>> &divergences)
    {
      ...
      std::fill(divergences.begin(),
                divergences.end(),
                typename Tensor<2, dim, spacedim>::template solution_divergence_type<Number>());
      ...
      std::vector<typename Tensor<2, dim, spacedim>::template solution_gradient_type<Number>> &gradients)
    {
      ...
      std::fill(gradients.begin(),
                gradients.end(),
                typename Tensor<2, dim, spacedim>::template solution_gradient_type<Number>());

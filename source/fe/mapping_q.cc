...
MappingQ<dim, spacedim>::InternalData::memory_consumption() const
{
  return (
    ...
    MemoryConsumption::memory_consumption(quadrature_points) + MemoryConsumption::memory_consumption(unit_tangentials) +
    MemoryConsumption::memory_consumption(aux) + MemoryConsumption::memory_consumption(mapping_support_points) +
...
template <int dim, int spacedim>
void
MappingQ<dim, spacedim>::InternalData::initialize(const UpdateFlags      update_flags,
  ...
  if (dim > 1)
    {
      ...
      if (tensor_product_quadrature)
        {
          const std::array<Quadrature<1>, dim> &quad_array = quadrature.get_tensor_basis();
          ...
          if (tensor_product_quadrature)
            {
              ...
              shape_info.lexicographic_numbering =
                FETools::lexicographic_to_hierarchic_numbering<dim>(polynomial_degree);
              shape_info.n_q_points                 = n_q_points;
              shape_info.dofs_per_component_on_cell = Utilities::pow(polynomial_degree + 1, dim);
            }
        }
    }
}
...
template <int dim, int spacedim>
void
MappingQ<dim, spacedim>::InternalData::initialize_face(const UpdateFlags      update_flags,
                                                       const Quadrature<dim> &quadrature,
                                                       const unsigned int     n_original_q_points)
{
  ...
  if (dim > 1 && tensor_product_quadrature)
    {
      ...
      const FE_DGQ<1>        fe(polynomial_degree);
      shape_info.reinit(quadrature.get_tensor_basis()[0], fe);
      shape_info.lexicographic_numbering = FETools::lexicographic_to_hierarchic_numbering<facedim>(polynomial_degree);
  ...
  if (dim > 1)
    {
      if (this->update_each & (update_boundary_forms | update_normal_vectors | update_JxW_values))
        {
          aux.resize(dim - 1);
          ...
          for (const unsigned int i : GeometryInfo<dim>::face_indices())
            {
              ...
              if (dim > 2)
                {
                  unit_tangentials[GeometryInfo<dim>::faces_per_cell + i].resize(n_original_q_points);
...
MappingQ<dim, spacedim>::MappingQ(const unsigned int p)
  : polynomial_degree(p)
  , line_support_points(QGaussLobatto<1>(this->polynomial_degree + 1).get_points())
  , polynomials_1d(Polynomials::generate_complete_Lagrange_basis(line_support_points))
  , renumber_lexicographic_to_hierarchic(FETools::lexicographic_to_hierarchic_numbering<dim>(p))
  , unit_cell_support_points(
      internal::MappingQImplementation::unit_support_points<dim>(line_support_points,
                                                                 renumber_lexicographic_to_hierarchic))
  , support_point_weights_perimeter_to_interior(
      internal::MappingQImplementation::compute_support_point_weights_perimeter_to_interior(this->polynomial_degree,
                                                                                            dim))
  , support_point_weights_cell(
      internal::MappingQImplementation::compute_support_point_weights_cell<dim>(this->polynomial_degree))
{
  Assert(p >= 1,
...
MappingQ<dim, spacedim>::MappingQ(const MappingQ<dim, spacedim> &mapping)
  ...
  , renumber_lexicographic_to_hierarchic(mapping.renumber_lexicographic_to_hierarchic)
...
template <int dim, int spacedim>
Point<spacedim>
MappingQ<dim, spacedim>::transform_unit_to_real_cell(const typename Triangulation<dim, spacedim>::cell_iterator &cell,
                                                     const Point<dim> &p) const
{
  return Point<spacedim>(internal::evaluate_tensor_product_value(polynomials_1d,
...
template <>
Point<1>
MappingQ<1, 1>::transform_real_to_unit_cell_internal(const Triangulation<1, 1>::cell_iterator &cell,
                                                     const Point<1>                           &p,
                                                     const Point<1>                           &initial_p_unit) const
{
  // dispatch to the various specializations for spacedim=dim,
  // spacedim=dim+1, etc
  return internal::MappingQImplementation::do_transform_real_to_unit_cell_internal<1>(
    p,
    initial_p_unit,
    this->compute_mapping_support_points(cell),
    polynomials_1d,
...
template <>
Point<2>
MappingQ<2, 2>::transform_real_to_unit_cell_internal(const Triangulation<2, 2>::cell_iterator &cell,
                                                     const Point<2>                           &p,
                                                     const Point<2>                           &initial_p_unit) const
{
  return internal::MappingQImplementation::do_transform_real_to_unit_cell_internal<2>(
    p,
    initial_p_unit,
    this->compute_mapping_support_points(cell),
    polynomials_1d,
...
template <>
Point<3>
MappingQ<3, 3>::transform_real_to_unit_cell_internal(const Triangulation<3, 3>::cell_iterator &cell,
                                                     const Point<3>                           &p,
                                                     const Point<3>                           &initial_p_unit) const
{
  return internal::MappingQImplementation::do_transform_real_to_unit_cell_internal<3>(
    p,
    initial_p_unit,
    this->compute_mapping_support_points(cell),
    polynomials_1d,
...
template <>
Point<1>
MappingQ<1, 2>::transform_real_to_unit_cell_internal(const Triangulation<1, 2>::cell_iterator &cell,
                                                     const Point<2>                           &p,
                                                     const Point<1>                           &initial_p_unit) const
{
  const int dim      = 1;
  ...
  auto mdata = Utilities::dynamic_unique_cast<InternalData>(get_data(update_flags, point_quadrature));

  mdata->mapping_support_points = this->compute_mapping_support_points(cell);

  // dispatch to the various specializations for spacedim=dim,
  // spacedim=dim+1, etc
  return internal::MappingQImplementation::do_transform_real_to_unit_cell_internal_codim1<1>(
    p, initial_p_unit, mdata->mapping_support_points, polynomials_1d, renumber_lexicographic_to_hierarchic);
}
...
template <>
Point<2>
MappingQ<2, 3>::transform_real_to_unit_cell_internal(const Triangulation<2, 3>::cell_iterator &cell,
                                                     const Point<3>                           &p,
                                                     const Point<2>                           &initial_p_unit) const
{
  const int dim      = 2;
  ...
  // dispatch to the various specializations for spacedim=dim,
  // spacedim=dim+1, etc
  return internal::MappingQImplementation::do_transform_real_to_unit_cell_internal_codim1<2>(
    p, initial_p_unit, mdata->mapping_support_points, polynomials_1d, renumber_lexicographic_to_hierarchic);
}
...
template <>
Point<1>
MappingQ<1, 3>::transform_real_to_unit_cell_internal(const Triangulation<1, 3>::cell_iterator &,
                                                     const Point<3> &,
                                                     const Point<1> &) const
{
...
template <int dim, int spacedim>
Point<dim>
MappingQ<dim, spacedim>::transform_real_to_unit_cell(const typename Triangulation<dim, spacedim>::cell_iterator &cell,
                                                     const Point<spacedim> &p) const
{
          ...
          switch (dim)
            {
              ...
              case 2:
                {
                  const Point<dim> point = internal::MappingQ1::transform_real_to_unit_cell(vertices, p);
                  ...
                  if (-eps <= point(1) && point(1) <= 1 + eps && -eps <= point(0) && point(0) <= 1 + eps)
                    {
                      return point;
                    }
                  else
  ...
  const Point<dim> p_unit = this->transform_real_to_unit_cell_internal(cell, p, initial_p_unit);
  AssertThrow(numbers::is_finite(p_unit[0]), (typename Mapping<dim, spacedim>::ExcTransformationFailed()));
  return p_unit;
}
  ...
  const ArrayView<Point<dim>>                                &unit_points) const
{
  ...
  if (dim < spacedim)
    {
      Mapping<dim, spacedim>::transform_points_real_to_unit_cell(cell, real_points, unit_points);
      return;
    }
  ...
  internal::MappingQImplementation::InverseQuadraticApproximation<dim, spacedim> inverse_approximation(
    support_points, unit_cell_support_points);
    ...
    if (n_points - i > 1)
      {
        ...
        Point<dim, VectorizedArray<double>> unit_point =
          internal::MappingQImplementation::do_transform_real_to_unit_cell_internal<dim, spacedim>(
            p_vec,
      ...
      unit_points[i] = internal::MappingQImplementation::do_transform_real_to_unit_cell_internal<dim, spacedim>(
        real_points[i],
  ...
  for (unsigned int i = 0; i < 5; ++i)
    {
      ...
      if (out & (update_covariant_transformation | update_jacobian_grads | update_jacobians | update_boundary_forms |
...
MappingQ<dim, spacedim>::get_data(const UpdateFlags update_flags, const Quadrature<dim> &q) const
{
...
template <int dim, int spacedim>
std::unique_ptr<typename Mapping<dim, spacedim>::InternalDataBase>
MappingQ<dim, spacedim>::get_face_data(const UpdateFlags update_flags, const hp::QCollection<dim - 1> &quadrature) const
{
  ...
  data.initialize_face(this->requires_update_flags(update_flags),
                       QProjector<dim>::project_to_all_faces(ReferenceCells::get_hypercube<dim>(), quadrature[0]),
                       quadrature[0].size());
...
template <int dim, int spacedim>
std::unique_ptr<typename Mapping<dim, spacedim>::InternalDataBase>
MappingQ<dim, spacedim>::get_subface_data(const UpdateFlags update_flags, const Quadrature<dim - 1> &quadrature) const
{
  std::unique_ptr<typename Mapping<dim, spacedim>::InternalDataBase> data_ptr =
    std::make_unique<InternalData>(polynomial_degree);
  auto &data = dynamic_cast<InternalData &>(*data_ptr);
  data.initialize_face(this->requires_update_flags(update_flags),
                       QProjector<dim>::project_to_all_subfaces(ReferenceCells::get_hypercube<dim>(), quadrature),
                       quadrature.size());
  ...
  internal::FEValuesImplementation::MappingRelatedData<dim, spacedim> &output_data) const
{
  ...
  const CellSimilarity::Similarity computed_cell_similarity =
    (polynomial_degree == 1 && this->preserves_vertex_locations() ? cell_similarity : CellSimilarity::none);

  if (dim > 1 && data.tensor_product_quadrature)
    {
      internal::MappingQImplementation::maybe_update_q_points_Jacobians_and_grads_tensor<dim, spacedim>(
    ...
    {
      internal::MappingQImplementation::maybe_update_q_points_Jacobians_generic(computed_cell_similarity,
                                                                                data,
                                                                                make_array_view(
      ...
      internal::MappingQImplementation::maybe_update_jacobian_grads<dim, spacedim>(computed_cell_similarity,
                                                                                   data,
                                                                                   make_array_view(
                                                                                     quadrature.get_points()),
                                                                                   polynomials_1d,
  ...
  internal::MappingQImplementation::maybe_update_jacobian_pushed_forward_grads<dim, spacedim>(
    computed_cell_similarity,
    data,
  ...
  internal::MappingQImplementation::maybe_update_jacobian_2nd_derivatives<dim, spacedim>(
    computed_cell_similarity,
    data,
    make_array_view(quadrature.get_points()),
    polynomials_1d,
  ...
  internal::MappingQImplementation::maybe_update_jacobian_pushed_forward_2nd_derivatives<dim, spacedim>(
    computed_cell_similarity,
    data,
    make_array_view(quadrature.get_points()),
    polynomials_1d,
  ...
  internal::MappingQImplementation::maybe_update_jacobian_3rd_derivatives<dim, spacedim>(
    computed_cell_similarity,
    data,
    make_array_view(quadrature.get_points()),
    polynomials_1d,
  ...
  internal::MappingQImplementation::maybe_update_jacobian_pushed_forward_3rd_derivatives<dim, spacedim>(
    computed_cell_similarity,
    data,
    make_array_view(quadrature.get_points()),
    polynomials_1d,
  ...
  internal::FEValuesImplementation::MappingRelatedData<dim, spacedim> &output_data) const
{
  ...
  internal::MappingQImplementation::do_fill_fe_face_values(
    *this,
    ...
    QProjector<dim>::DataSetDescriptor::face(ReferenceCells::get_hypercube<dim>(),
                                             face_no,
  ...
  internal::FEValuesImplementation::MappingRelatedData<dim, spacedim> &output_data) const
{
  ...
  internal::MappingQImplementation::do_fill_fe_face_values(
    *this,
    ...
    face_no,
    subface_no,
    QProjector<dim>::DataSetDescriptor::subface(ReferenceCells::get_hypercube<dim>(),
                                                face_no,
                                                subface_no,
                                                cell->face_orientation(face_no),
                                                cell->face_flip(face_no),
  ...
  internal::FEValuesImplementation::MappingRelatedData<dim, spacedim> &output_data) const
{
  ...
  internal::MappingQImplementation::maybe_update_q_points_Jacobians_generic(CellSimilarity::none,
                                                                            data,
                                                                            make_array_view(quadrature.get_points()),
                                                                            polynomials_1d,
                                                                            renumber_lexicographic_to_hierarchic,
  ...
  internal::MappingQImplementation::maybe_update_jacobian_grads<dim, spacedim>(CellSimilarity::none,
                                                                               data,
                                                                               make_array_view(quadrature.get_points()),
                                                                               polynomials_1d,
                                                                               renumber_lexicographic_to_hierarchic,
      ...
      for (unsigned int point = 0; point < n_q_points; ++point)
        {
          ...
          for (unsigned int d = 0; d < spacedim; d++)
            normal[d] = output_data.inverse_jacobians[point].transpose()[d] * quadrature.normal_vector(point);
  ...
  internal::FEValuesImplementation::MappingRelatedData<dim, spacedim> &output_data) const
{
  ...
  Assert(update_flags & update_inverse_jacobians || update_flags & update_jacobians ||
  ...
  auto internal_data =
    this->get_data(update_flags, Quadrature<dim>(std::vector<Point<dim>>(unit_points.begin(), unit_points.end())));
  const InternalData &data    = static_cast<const InternalData &>(*internal_data);
  data.output_data            = &output_data;
  data.mapping_support_points = this->compute_mapping_support_points(cell);
  ...
  internal::FEValuesImplementation::MappingRelatedData<dim, spacedim> &output_data) const
{
  ...
  internal::MappingQImplementation::do_fill_fe_face_values(*this,
                                                           cell,
                                                           face_no,
                                                           numbers::invalid_unsigned_int,
...
template <int dim, int spacedim>
void
MappingQ<dim, spacedim>::transform(const ArrayView<const Tensor<1, dim>>                   &input,
                                   const MappingKind                                        mapping_kind,
                                   const typename Mapping<dim, spacedim>::InternalDataBase &mapping_data,
                                   const ArrayView<Tensor<1, spacedim>>                    &output) const
{
  internal::MappingQImplementation::transform_fields(input, mapping_kind, mapping_data, output);
}
...
template <int dim, int spacedim>
void
MappingQ<dim, spacedim>::transform(const ArrayView<const DerivativeForm<1, dim, spacedim>> &input,
                                   const MappingKind                                        mapping_kind,
                                   const typename Mapping<dim, spacedim>::InternalDataBase &mapping_data,
                                   const ArrayView<Tensor<2, spacedim>>                    &output) const
{
  internal::MappingQImplementation::transform_differential_forms(input, mapping_kind, mapping_data, output);
}
...
template <int dim, int spacedim>
void
MappingQ<dim, spacedim>::transform(const ArrayView<const Tensor<2, dim>>                   &input,
                                   const MappingKind                                        mapping_kind,
                                   const typename Mapping<dim, spacedim>::InternalDataBase &mapping_data,
                                   const ArrayView<Tensor<2, spacedim>>                    &output) const
{
  switch (mapping_kind)
    {
      ...
      case mapping_covariant_gradient:
        internal::MappingQImplementation::transform_gradients(input, mapping_kind, mapping_data, output);
        return;
      default:
        Assert(false, ExcNotImplemented());
    }
...
template <int dim, int spacedim>
void
MappingQ<dim, spacedim>::transform(const ArrayView<const DerivativeForm<2, dim, spacedim>> &input,
                                   const MappingKind                                        mapping_kind,
                                   const typename Mapping<dim, spacedim>::InternalDataBase &mapping_data,
                                   const ArrayView<Tensor<3, spacedim>>                    &output) const
{
  ...
  const internal::FEValuesImplementation::MappingRelatedData<dim, spacedim> &data =
    *static_cast<const InternalData &>(mapping_data).output_data;
          ...
          for (unsigned int q = 0; q < output.size(); ++q)
            for (unsigned int i = 0; i < spacedim; ++i)
              for (unsigned int j = 0; j < spacedim; ++j)
                {
                  double                                 tmp[dim];
                  const DerivativeForm<1, dim, spacedim> covariant = data.inverse_jacobians[q].transpose();
...
template <int dim, int spacedim>
void
MappingQ<dim, spacedim>::transform(const ArrayView<const Tensor<3, dim>>                   &input,
                                   const MappingKind                                        mapping_kind,
                                   const typename Mapping<dim, spacedim>::InternalDataBase &mapping_data,
                                   const ArrayView<Tensor<3, spacedim>>                    &output) const
{
  switch (mapping_kind)
    {
      ...
      case mapping_covariant_hessian:
        internal::MappingQImplementation::transform_hessians(input, mapping_kind, mapping_data, output);
        return;
      default:
        Assert(false, ExcNotImplemented());
    }
      ...
      for (unsigned int line_no = 0; line_no < GeometryInfo<dim>::lines_per_cell; ++line_no)
        {
          const typename Triangulation<dim, spacedim>::line_iterator line =
            (dim == 1 ? static_cast<typename Triangulation<dim, spacedim>::line_iterator>(cell) : cell->line(line_no));

          const Manifold<dim, spacedim> &manifold =
            ((line->manifold_id() == numbers::flat_manifold_id) && (dim < spacedim) ? cell->get_manifold() :
                                                                                      line->get_manifold());
...
template <>
void
MappingQ<3, 3>::add_quad_support_points(const Triangulation<3, 3>::cell_iterator &cell, std::vector<Point<3>> &a) const
{
  ...
  for (unsigned int face_no = 0; face_no < faces_per_cell; ++face_no)
    {
...
#ifdef DEBUG
      const bool face_orientation = cell->face_orientation(face_no), face_flip = cell->face_flip(face_no),
      ...
      for (unsigned int i = 0; i < vertices_per_face; ++i)
        Assert(face->vertex_index(i) ==
                 cell->vertex_index(
                   GeometryInfo<3>::face_to_cell_vertices(face_no, i, face_orientation, face_flip, face_rotation)),
               ExcInternalError());
      ...
      for (unsigned int i = 0; i < lines_per_face; ++i)
        Assert(face->line(i) ==
                 cell->line(
                   GeometryInfo<3>::face_to_cell_lines(face_no, i, face_orientation, face_flip, face_rotation)),
               ExcInternalError());
#endif
      ...
      boost::container::small_vector<Point<3>, 200> tmp_points(
        GeometryInfo<2>::vertices_per_cell + GeometryInfo<2>::lines_per_cell * (polynomial_degree - 1));
...
template <>
void
MappingQ<2, 3>::add_quad_support_points(const Triangulation<2, 3>::cell_iterator &cell, std::vector<Point<3>> &a) const
{
  std::array<Point<3>, GeometryInfo<2>::vertices_per_cell> vertices;
  for (const unsigned int i : GeometryInfo<2>::vertex_indices())
    vertices[i] = cell->vertex(i);

  Table<2, double> weights(Utilities::fixed_power<2>(polynomial_degree - 1), GeometryInfo<2>::vertices_per_cell);
  for (unsigned int q = 0, q2 = 0; q2 < polynomial_degree - 1; ++q2)
    for (unsigned int q1 = 0; q1 < polynomial_degree - 1; ++q1, ++q)
      {
        Point<2> point(line_support_points[q1 + 1][0], line_support_points[q2 + 1][0]);
        for (const unsigned int i : GeometryInfo<2>::vertex_indices())
  ...
  a.resize(a.size() + n_rows);
  auto a_view = make_array_view(a.end() - n_rows, a.end());
  cell->get_manifold().get_new_points(make_array_view(vertices.begin(), vertices.end()), weights, a_view);
}
...
template <int dim, int spacedim>
void
MappingQ<dim, spacedim>::add_quad_support_points(const typename Triangulation<dim, spacedim>::cell_iterator &,
...
MappingQ<dim, spacedim>::get_bounding_box(const typename Triangulation<dim, spacedim>::cell_iterator &cell) const
{
...
template <int dim, int spacedim>
bool
MappingQ<dim, spacedim>::is_compatible_with(const ReferenceCell &reference_cell) const
{

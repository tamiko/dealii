...
MappingQCache<dim, spacedim>::MappingQCache(const unsigned int polynomial_degree)
...
template <int dim, int spacedim>
MappingQCache<dim, spacedim>::MappingQCache(const MappingQCache<dim, spacedim> &mapping)
...
template <int dim, int spacedim>
void
MappingQCache<dim, spacedim>::initialize(const Mapping<dim, spacedim>       &mapping,
                                         const Triangulation<dim, spacedim> &triangulation)
{
  // FE and FEValues in the case they are needed
  FE_Nothing<dim, spacedim>                                             fe;
  Threads::ThreadLocalStorage<std::unique_ptr<FEValues<dim, spacedim>>> fe_values_all;

  this->initialize(triangulation, [&](const typename Triangulation<dim, spacedim>::cell_iterator &cell) {
    const auto mapping_q = dynamic_cast<const MappingQ<dim, spacedim> *>(&mapping);
        ...
        if (fe_values.get() == nullptr)
          {
            ...
            for (const auto i : FETools::hierarchic_to_lexicographic_numbering<dim>(this->polynomial_degree))
              quadrature_points.push_back(quadrature_gl.point(i));
            Quadrature<dim> quadrature(quadrature_points);

            fe_values = std::make_unique<FEValues<dim, spacedim>>(mapping, fe, quadrature, update_quadrature_points);
          }
...
{
  clear_signal.disconnect();
  clear_signal = triangulation.signals.any_change.connect([&]() -> void { this->support_point_cache.reset(); });

  support_point_cache =
    std::make_shared<std::vector<std::vector<std::vector<Point<spacedim>>>>>(triangulation.n_levels());
  ...
  WorkStream::run(
    triangulation.begin(),
    triangulation.end(),
    [&](const typename Triangulation<dim, spacedim>::cell_iterator &cell, void *, void *) {
      (*support_point_cache)[cell->level()][cell->index()] = compute_points_on_cell(cell);
      AssertDimension((*support_point_cache)[cell->level()][cell->index()].size(),
  ...
  const bool                                                     function_describes_relative_displacement)
{
  ...
  this->initialize(tria, [&](const typename Triangulation<dim, spacedim>::cell_iterator &cell) {
...
namespace
{
  ...
  void
  copy_locally_owned_data_from(const VectorType                                                    &vector,
                               LinearAlgebra::distributed::Vector<typename VectorType::value_type> &vector_ghosted)
  {
                                         ...
                                         const bool                       vector_describes_relative_displacement)
{
  ...
  const unsigned int is_fe_q   = dynamic_cast<const FE_Q<dim, spacedim> *>(&fe.base_element(0)) != nullptr;
  const unsigned int is_fe_dgq = dynamic_cast<const FE_DGQ<dim, spacedim> *>(&fe.base_element(0)) != nullptr;

  const auto lexicographic_to_hierarchic_numbering =
    Utilities::invert_permutation(FETools::hierarchic_to_lexicographic_numbering<spacedim>(this->get_degree()));
  ...
  vector_ghosted.reinit(dof_handler.locally_owned_dofs(), locally_relevant_dofs, dof_handler.get_communicator());
  ...
  const bool interpolation_of_values_is_needed = ((is_fe_q || is_fe_dgq) && fe.degree == this->get_degree()) == false;

  // Step 2: loop over all cells
  this->initialize(
    dof_handler.get_triangulation(),
    [&](const typename Triangulation<dim, spacedim>::cell_iterator &cell_tria) -> std::vector<Point<spacedim>> {
      const bool is_active_non_artificial_cell =
        (cell_tria->is_active() == true) && (cell_tria->is_artificial() == false);

      const typename DoFHandler<dim, spacedim>::cell_iterator cell_dofs(&cell_tria->get_triangulation(),
                                                                        cell_tria->level(),
                                                                        cell_tria->index(),
                                                                        &dof_handler);
      ...
      if (((vector_describes_relative_displacement || (is_active_non_artificial_cell == false)) &&
           ((mapping_q != nullptr && this->get_degree() == mapping_q->get_degree()) ==
            false)) /*condition 1: points need to be computed via FEValues*/
          || (is_active_non_artificial_cell &&
              interpolation_of_values_is_needed) /*condition 2: interpolation of values is needed*/)
        {
          ...
          if (fe_values.get() == nullptr)
            {
              ...
              fe_values = std::make_unique<FEValues<dim, spacedim>>(
                mapping,
                interpolation_of_values_is_needed ? fe : static_cast<const FiniteElement<dim, spacedim> &>(fe_nothing),
                quadrature,
      ...
      if (vector_describes_relative_displacement || is_active_non_artificial_cell == false)
        ...
        {
          result.resize(Utilities::pow<unsigned int>(this->get_degree() + 1, dim));
        }
      ...
      if (interpolation_of_values_is_needed == false)
              ...
              if (is_fe_q)
                {
                  ...
                  if (vector_describes_relative_displacement)
                    result[id.second][id.first] += vector_ghosted(dof_indices[i]);
                  else
                    result[id.second][id.first] = vector_ghosted(dof_indices[i]);
                }
              else
                {
                  // case 1b: FE_DGQ
                  if (vector_describes_relative_displacement)
                    result[lexicographic_to_hierarchic_numbering[id.second]][id.first] +=
                      vector_ghosted(dof_indices[i]);
                  else
                    result[lexicographic_to_hierarchic_numbering[id.second]][id.first] = vector_ghosted(dof_indices[i]);
                }
            }
        }
      else
        {
          ...
          std::vector<Vector<typename VectorType::value_type>> values(
            fe_values->n_quadrature_points, Vector<typename VectorType::value_type>(spacedim));
                                         ...
                                         const bool                       vector_describes_relative_displacement)
{
  ...
  AssertDimension(dof_handler.get_triangulation().n_global_levels() - 1, vectors.max_level());
  ...
  // Step 1: copy global vector so that the ghost values are such that the
  // cache can be set up for all ghost cells
  MGLevelObject<LinearAlgebra::distributed::Vector<typename VectorType::value_type>> vectors_ghosted(
    vectors.min_level(), vectors.max_level());

  for (unsigned int l = vectors.min_level(); l <= vectors.max_level(); ++l)
    {
      IndexSet locally_relevant_dofs;
      DoFTools::extract_locally_relevant_level_dofs(dof_handler, l, locally_relevant_dofs);
  ...
  this->initialize(
    dof_handler.get_triangulation(),
    [&](const typename Triangulation<dim, spacedim>::cell_iterator &cell_tria) -> std::vector<Point<spacedim>> {
      const bool is_non_artificial_cell = cell_tria->level_subdomain_id() != numbers::artificial_subdomain_id;

      const typename DoFHandler<dim, spacedim>::level_cell_iterator cell_dofs(&cell_tria->get_triangulation(),
                                                                              cell_tria->level(),
                                                                              cell_tria->index(),
                                                                              &dof_handler);
      ...
      // Step 2a) set up and reinit FEValues (if needed)
      if (((vector_describes_relative_displacement || (is_non_artificial_cell == false)) &&
           ((mapping_q != nullptr && this->get_degree() == mapping_q->get_degree()) ==
            false)) /*condition 1: points need to be computed via FEValues*/
      ...
      if (interpolation_of_values_is_needed == false)
              ...
              if (is_fe_q)
                {
                  // case 1a: FE_Q
                  if (vector_describes_relative_displacement)
                    result[id.second][id.first] += vectors_ghosted[cell_tria->level()](dof_indices[i]);
                  else
                    result[id.second][id.first] = vectors_ghosted[cell_tria->level()](dof_indices[i]);
                }
              else
      ...
      else
        {
          ...
          std::vector<typename VectorType::value_type> dof_values(fe.n_dofs_per_cell());
            ...
            for (unsigned int i = 0; i < fe.n_dofs_per_cell(); ++i)
              for (unsigned int q = 0; q < fe_values->n_quadrature_points; ++q)
                if (vector_describes_relative_displacement == false && i == 0)
                  result[q][c] = dof_values[i] * fe_values->shape_value_component(i, q, c);
                else
                  result[q][c] += dof_values[i] * fe_values->shape_value_component(i, q, c);
        }
...
MappingQCache<dim, spacedim>::memory_consumption() const
{
  if (support_point_cache.get() != nullptr)
    return sizeof(*this) + MemoryConsumption::memory_consumption(*support_point_cache);
  else
    return sizeof(*this);
}
...
MappingQCache<dim, spacedim>::get_vertices(const typename Triangulation<dim, spacedim>::cell_iterator &cell) const
{
  Assert(support_point_cache.get() != nullptr,
         ExcMessage("Must call MappingQCache::initialize() before "
                    "using it or after mesh has changed!"));
  ...
  return boost::container::small_vector<Point<spacedim>, GeometryInfo<dim>::vertices_per_cell>(ptr,
                                                                                               ptr +
                                                                                                 cell->n_vertices());
}

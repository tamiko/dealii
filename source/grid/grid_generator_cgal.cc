...
#  ifdef DEAL_II_WITH_CGAL
    Assert(dealii_implicit_function.n_components == 1,
           ExcMessage("The implicit function must have exactly one component."));
    Assert(dealii_implicit_function.value(interior_point) < 0,
           ExcMessage("The implicit function must be negative at the interior point."));
    Assert(outer_ball_radius > 0, ExcMessage("The outer ball radius must be positive."));
    Assert(tria.n_active_cells() == 0, ExcMessage("The triangulation must be empty."));
    ...
    if constexpr (dim == 3)
      {
        ...
        using Tr          = CGAL::Mesh_triangulation_3<Mesh_domain, CGAL::Default, CGALWrappers::ConcurrencyTag>::type;
        using C3t3        = CGAL::Mesh_complex_3_in_triangulation_3<Tr, int, int>;
        using Mesh_criteria = CGAL::Mesh_criteria_3<Tr>;


        auto cgal_implicit_function = [&](const Point_3 &p) {
          return NumberType(dealii_implicit_function.value(Point<3>(p.x(), p.y(), p.z())));
        };

        Mesh_domain domain = Mesh_domain::create_implicit_mesh_domain(
          cgal_implicit_function,
          K::Sphere_3(Point_3(interior_point[0], interior_point[1], interior_point[2]),
        ...
        CGALWrappers::cgal_triangulation_to_dealii_triangulation(cgal_triangulation, tria);
      }
    else if constexpr (dim == 2)
      {
        ...
        auto cgal_implicit_function = [&](const Point_3 &p) {
          return FT(dealii_implicit_function.value(Point<3>(p.x(), p.y(), p.z())));
        };

        Surface_3 surface(cgal_implicit_function,
                          Sphere_3(Point_3(interior_point[0], interior_point[1], interior_point[2]),
                                   outer_ball_radius * outer_ball_radius));
        ...
        CGAL::Surface_mesh_default_criteria_3<Tr> criteria(data.angular_bound, data.radius_bound, data.distance_bound);
...
#  endif
  ...
  {
#  ifdef DEAL_II_WITH_CGAL
    Assert(surface_tria.n_cells() > 0,
           ExcMessage("The input triangulation cannot be empty when calling this function."));
    Assert(vol_tria.n_cells() == 0, ExcMessage("The output triangulation must be empty when calling this function."));
    using K       = CGAL::Exact_predicates_inexact_constructions_kernel;
    using Point_3 = K::Point_3;

    using Mesh_domain   = CGAL::Polyhedral_mesh_domain_with_features_3<K, CGAL::Surface_mesh<Point_3>>;
    using Tr            = CGAL::Mesh_triangulation_3<Mesh_domain, CGAL::Default, CGALWrappers::ConcurrencyTag>::type;
    using Mesh_criteria = CGAL::Mesh_criteria_3<Tr>;
    ...
    CGALWrappers::cgal_triangulation_to_dealii_triangulation(cgal_triangulation, vol_tria);

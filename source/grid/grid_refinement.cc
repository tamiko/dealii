...
namespace
{
  ...
  void
  refine_and_coarsen_fixed_fraction_via_l1_norm(Triangulation<dim, spacedim> &tria,
                                                ...
                                                const unsigned int            max_n_cells)
  {
    ...
    std::sort(criteria_sorted.begin(), criteria_sorted.end(), std::greater<double>());

    const double total_error = criteria_sorted.l1_norm();

    // compute thresholds
    typename Vector<Number>::const_iterator pp = criteria_sorted.begin();
    for (double sum = 0; (sum < top_fraction * total_error) && (pp != criteria_sorted.end()); ++pp)
      sum += *pp;
    double top_threshold = (pp != criteria_sorted.begin() ? (*pp + *(pp - 1)) / 2 : *pp);

    typename Vector<Number>::const_iterator qq = criteria_sorted.end() - 1;
    for (double sum = 0; (sum < bottom_fraction * total_error) && (qq != criteria_sorted.begin() - 1); --qq)
      sum += *qq;
    double bottom_threshold = ((qq != criteria_sorted.end() - 1) ? (*qq + *(qq + 1)) / 2 : 0.);
    ...
    // refinemnt as well.
    const unsigned int refine_cells = pp - criteria_sorted.begin(), coarsen_cells = criteria_sorted.end() - 1 - qq;

    if (static_cast<unsigned int>(
          tria.n_active_cells() + refine_cells * (GeometryInfo<dim>::max_children_per_cell - 1) -
          (coarsen_cells * (GeometryInfo<dim>::max_children_per_cell - 1) / GeometryInfo<dim>::max_children_per_cell)) >
        max_n_cells)
      {
        GridRefinement::refine_and_coarsen_fixed_number(
          tria, criteria, 1. * refine_cells / criteria.size(), 1. * coarsen_cells / criteria.size(), max_n_cells);
        return;
      }
    ...
    // top_fraction!=1
    const double max_criterion = *(criteria_sorted.begin()), min_criterion = *(criteria_sorted.end() - 1);
                       ...
                       const unsigned int            max_to_mark)
{
  ...
  for (const auto &cell : tria.active_cell_iterators())
    if ((dynamic_cast<parallel::DistributedTriangulationBase<dim, spacedim> *>(&tria) == nullptr ||
...
template <int dim, typename Number, int spacedim>
void
GridRefinement::coarsen(Triangulation<dim, spacedim> &tria, const Vector<Number> &criteria, const double threshold)
{
  Assert(criteria.size() == tria.n_active_cells(), ExcDimensionMismatch(criteria.size(), tria.n_active_cells()));
  Assert(criteria.is_non_negative(), ExcNegativeCriteria());
...
GridRefinement::adjust_refine_and_coarsen_number_fraction(const types::global_cell_index current_n_cells,
                                                          ...
                                                          const double                   bottom_fraction)
{
  ...
  Assert(top_fraction + bottom_fraction <= 1 + 10 * std::numeric_limits<double>::epsilon(), ExcInvalidParameterValue());
  ...
  else if (static_cast<types::global_cell_index>(current_n_cells + refine_cells * cell_increase_on_refine -
                                                ...
                                                const unsigned int            max_n_cells)
{
  ...
  const std::pair<double, double> adjusted_fractions =
    adjust_refine_and_coarsen_number_fraction<dim>(criteria.size(), max_n_cells, top_fraction, bottom_fraction);
      ...
      if (refine_cells)
        {
          if (static_cast<size_t>(refine_cells) == criteria.size())
            ...
            {
              std::nth_element(tmp.begin(), tmp.begin() + refine_cells - 1, tmp.end(), std::greater<double>());
  ...
  switch (norm_type)
    {
      case VectorTools::NormType::L1_norm:
        ...
        refine_and_coarsen_fixed_fraction_via_l1_norm(tria, criteria, top_fraction, bottom_fraction, max_n_cells);
        break;
      ...
      case VectorTools::NormType::L2_norm:
        {
          ...
          std::transform(criteria.begin(), criteria.end(), criteria_squared.begin(), [](Number c) { return c * c; });

          refine_and_coarsen_fixed_fraction_via_l1_norm(
            tria, criteria_squared, top_fraction * top_fraction, bottom_fraction * bottom_fraction, max_n_cells);
        }
        break;
                                            ...
                                            const unsigned int            order)
{
  ...
  std::sort(cell_indices.begin(), cell_indices.end(), [&criteria](const unsigned int left, const unsigned int right) {
  ...
  for (std::size_t M = 0; M < criteria.size(); ++M)
    {
      ...
      const double cost = std::pow(((std::pow(2., dim) - 1) * (1 + M) + N), static_cast<double>(order) / dim) *

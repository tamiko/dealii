...
namespace internal
{
  ...
  apply_exponential_map(const Tensor<1, 3> &u, const Tensor<1, 3> &dir)
  {
    ...
    if (theta < 1.e-10)
    ...
    else
      {
        ...
        const Tensor<1, 3> tmp     = std::cos(theta) * u + std::sin(theta) * dirUnit;
        return tmp / tmp.norm();
      }
  }
  ...
  compute_normal(const Tensor<1, 3> &vector, bool normalize = false)
  {
    ...
    else if (std::abs(vector[1]) >= std::abs(vector[0]) && std::abs(vector[1]) >= std::abs(vector[2]))
      {
        normal[0] = -1.;
        normal[2] = -1.;
...
namespace
{
  template <int dim, int spacedim>
  bool
  spherical_face_is_horizontal(const typename Triangulation<dim, spacedim>::face_iterator &face,
                               const Point<spacedim>                                      &manifold_center)
  {
    ...
    sqr_distances_to_center[0] = (face->vertex(0) - manifold_center).norm_square();
    for (unsigned int i = 1; i < n_vertices; ++i)
      {
        sqr_distances_to_center[i]           = (face->vertex(i) - manifold_center).norm_square();
        sqr_distances_to_first_vertex[i - 1] = (face->vertex(i) - face->vertex(0)).norm_square();
      }
    const auto minmax_sqr_distance =
      std::minmax_element(sqr_distances_to_center.begin(), sqr_distances_to_center.end());
    const auto min_sqr_distance_to_first_vertex =
      std::min_element(sqr_distances_to_first_vertex.begin(), sqr_distances_to_first_vertex.end());

    return (*minmax_sqr_distance.second - *minmax_sqr_distance.first < 1.e-10 * *min_sqr_distance_to_first_vertex);
  }
} // namespace
...
template <int dim, int spacedim>
Tensor<1, spacedim>
PolarManifold<dim, spacedim>::normal_vector(const typename Triangulation<dim, spacedim>::face_iterator &face,
...
template <int dim, int spacedim>
Tensor<1, spacedim>
SphericalManifold<dim, spacedim>::get_tangent_vector(const Point<spacedim> &p1, const Point<spacedim> &p2) const
{
  const double tol = 1e-10;
  (void)tol;
...
template <int dim, int spacedim>
Tensor<1, spacedim>
SphericalManifold<dim, spacedim>::normal_vector(const typename Triangulation<dim, spacedim>::face_iterator &face,
                                                const Point<spacedim>                                      &p) const
{
  // Let us first test whether we are on a "horizontal" face
  // (tangential to the sphere).  In this case, the normal vector is
...
template <int dim, int spacedim>
void
SphericalManifold<dim, spacedim>::get_new_points(const ArrayView<const Point<spacedim>> &surrounding_points,
                                                 ...
                                                 ArrayView<Point<spacedim>>              new_points) const
{
  ...
  if (surrounding_points.size() == 2)
    {
      for (unsigned int row = 0; row < weight_rows; ++row)
        new_points[row] =
          get_intermediate_point(surrounding_points[0], surrounding_points[1], weights[row * weight_columns + 1]);
      return;
    }
  ...
  for (unsigned int row = 0; row < weight_rows; ++row)
    {
      ...
      if (spacedim < 3)
        new_points[row] =
          polar_manifold.get_new_point(surrounding_points,
                                       ArrayView<const double>(&weights[row * weight_columns], weight_columns));
    }
  ...
  boost::container::small_vector<Tensor<1, spacedim>, 100> merged_directions(surrounding_points.size(),
                                                                             Point<spacedim>());
  ...
  // the get_new_point function call below.
  boost::container::small_vector<unsigned int, 100> merged_weights_index(new_points.size(),
    ...
    if (!accurate_point_was_found[row])
      {
        if (merged_weights_index[row] == numbers::invalid_unsigned_int)
          {
            const ArrayView<const double> array_merged_weights(&merged_weights[row * weight_columns],
                                                               n_unique_directions);
...
SphericalManifold<dim, spacedim>::guess_new_point(const ArrayView<const Tensor<1, spacedim>> &directions,
      ...
      if (n_merged_points == 2)
        {
          ...
          Assert(std::abs(weights[0] + weights[1] - 1.0) < 1e-13, ExcMessage("Weights do not sum up to 1"));
          Point<3> intermediate =
            unit_manifold.get_intermediate_point(Point<3>(directions[0]), Point<3>(directions[1]), weights[1]);
          return intermediate;
        }
...
// ============================================================
template <int dim, int spacedim>
CylindricalManifold<dim, spacedim>::CylindricalManifold(const unsigned int axis, const double tolerance)
  : CylindricalManifold<dim, spacedim>(Point<spacedim>::unit_vector(axis), Point<spacedim>(), tolerance)
{
...
template <int dim, int spacedim>
Point<spacedim>
CylindricalManifold<dim, spacedim>::get_new_point(const ArrayView<const Point<spacedim>> &surrounding_points,
                                                  const ArrayView<const double>          &weights) const
{
  Assert(spacedim == 3, ExcMessage("CylindricalManifold can only be used for spacedim==3!"));
...
EllipticalManifold<dim, spacedim>::EllipticalManifold(const Point<spacedim>     &center,
  ...
  : ChartManifold<dim, spacedim, spacedim>(EllipticalManifold<dim, spacedim>::get_periodicity())
  ...
  , sinh_u(std::sqrt(cosh_u * cosh_u - 1.0))
{
  ...
  Assert(std::signbit(cosh_u * cosh_u - 1.0) == false,
         ExcMessage("Invalid eccentricity: It must satisfy 0 < eccentricity < 1."));
  const double direction_norm = direction.norm();
  Assert(direction_norm != 0.0, ExcMessage("Invalid major axis direction vector: Null vector not allowed."));
...
EllipticalManifold<2, 2>::push_forward(const Point<2> &chart_point) const
{
  ...
  const Point<2> p(direction[0] * x - direction[1] * y, direction[1] * x + direction[0] * y);
  return p + center;
}
...
EllipticalManifold<2, 2>::pull_back(const Point<2> &space_point) const
{
  ...
  const double pt0 = std::sqrt((x * x) / (cosh_u * cosh_u) + (y * y) / (sinh_u * sinh_u));
...
template <int dim, int spacedim>
DerivativeForm<1, spacedim, spacedim>
EllipticalManifold<dim, spacedim>::push_forward_gradient(const Point<spacedim> &) const
{
  Assert(false, ExcNotImplemented());
  return {};
}
...
EllipticalManifold<2, 2>::push_forward_gradient(const Point<2> &chart_point) const
{
  const double cs = std::cos(chart_point[1]);
  const double sn = std::sin(chart_point[1]);
  Tensor<2, 2> dX;
  ...
  Tensor<2, 2, double> rot{{{+direction[0], -direction[1]}, {direction[1], direction[0]}}};
...
FunctionManifold<dim, spacedim, chartdim>::clone() const
{
  ...
  if (!(push_forward_expression.empty() && pull_back_expression.empty()))
    {
      return std::make_unique<FunctionManifold<dim, spacedim, chartdim>>(push_forward_expression,
    ...
    {
      return std::make_unique<FunctionManifold<dim, spacedim, chartdim>>(*push_forward_function,
                                                                         *pull_back_function,
                                                                         this->get_periodicity(),
                                                                         tolerance);
    }
...
FunctionManifold<dim, spacedim, chartdim>::push_forward(const Point<chartdim> &chart_point) const
{
  ...
  for (unsigned int i = 0; i < chartdim; ++i)
    Assert((chart_point.norm() > tolerance && (std::abs(pb[i] - chart_point[i]) < tolerance * chart_point.norm())) ||
...
TorusManifold<dim>::pull_back(const Point<3> &p) const
{
  ...
  double w     = std::sqrt(Utilities::fixed_power<2>(y - std::sin(phi) * R) +
...
template <int dim, int spacedim>
void
TransfiniteInterpolationManifold<dim, spacedim>::initialize(const Triangulation<dim, spacedim> &triangulation)
{
  ...
  std::vector<Point<dim>>      unit_points = QIterated<dim>(QTrapezoid<1>(), (dim == spacedim ? 2 : 1)).get_points();
...
namespace
{
  ...
  compute_transfinite_interpolation(const AccessorType &cell, const Point<1> &chart_point, const bool /*cell_is_flat*/)
  {
  ...
  template <typename AccessorType>
  Point<AccessorType::space_dimension>
  compute_transfinite_interpolation(const AccessorType &cell, const Point<2> &chart_point, const bool cell_is_flat)
  {
    ...
    const std::array<Point<spacedim>, 4> vertices{{cell.vertex(0), cell.vertex(1), cell.vertex(2), cell.vertex(3)}};
        ...
        for (unsigned int line = 0; line < GeometryInfo<2>::lines_per_cell; ++line)
          {
            ...
            if (line_manifold_id == my_manifold_id || line_manifold_id == numbers::flat_manifold_id)
              {
                weights_vertices[GeometryInfo<2>::line_to_cell_vertices(line, 0)] -= my_weight * (1. - line_point);
                weights_vertices[GeometryInfo<2>::line_to_cell_vertices(line, 1)] -= my_weight * line_point;
              }
            else
              {
                ...
                new_point += my_weight * tria.get_manifold(line_manifold_id).get_new_point(points_view, weights_view);
              }
          }
  ...
  template <typename AccessorType>
  Point<AccessorType::space_dimension>
  compute_transfinite_interpolation(const AccessorType &cell, const Point<3> &chart_point, const bool cell_is_flat)
  {
    const unsigned int                  dim            = AccessorType::dimension;
    const unsigned int                  spacedim       = AccessorType::space_dimension;
    const types::manifold_id            my_manifold_id = cell.manifold_id();
        ...
        for (const unsigned int face : GeometryInfo<3>::face_indices())
          {
            ...
            if (face_manifold_id == my_manifold_id || face_manifold_id == numbers::flat_manifold_id)
              {
                for (unsigned int line = 0; line < GeometryInfo<2>::lines_per_cell; ++line)
                  {
            ...
            else
              {
                ...
                new_point += my_weight * tria.get_manifold(face_manifold_id).get_new_point(points_view, weights_view);
              }
          }
        ...
        for (unsigned int line = 0; line < GeometryInfo<3>::lines_per_cell; ++line)
          {
            ...
            if (line_manifold_id == my_manifold_id || line_manifold_id == numbers::flat_manifold_id)
              {
                weights_vertices[GeometryInfo<3>::line_to_cell_vertices(line, 0)] -= my_weight * (1. - line_point);
                weights_vertices[GeometryInfo<3>::line_to_cell_vertices(line, 1)] -= my_weight * (line_point);
              }
            else
              {
                ...
                weights[1] = line_point;
                new_point -=
                  my_weight * tria.get_manifold(line_manifold_id).get_new_point(points_view_line, weights_view_line);
              }
          }
  ...
  const Point<dim>                                           &chart_point) const
{
  ...
  Assert(GeometryInfo<dim>::is_inside_unit_cell(chart_point, 5e-4), ExcMessage("chart_point is not in unit interval"));
  ...
  const Point<dim>                                           &initial_guess) const
{
  ...
  Tensor<1, spacedim> residual =
    point - compute_transfinite_interpolation(*cell, chart_point, coarse_cell_is_flat[cell->index()]);
  ...
  for (unsigned int i = 0; i < 100; ++i)
    {
      ...
      while (!GeometryInfo<dim>::is_inside_unit_cell(chart_point + alpha * update, 0.2) && alpha > 1e-7)
        alpha *= 0.5;
      ...
      while (alpha > 1e-4)
        {
          ...
          const Tensor<1, spacedim> residual_guess =
            point - compute_transfinite_interpolation(*cell, guess, coarse_cell_is_flat[cell->index()]);
  ...
  const ArrayView<const Point<spacedim>> &points) const
{
  ...
  Assert(triangulation->begin_active()->level() >= level_coarse,
         ExcMessage("The manifold was initialized with level " + std::to_string(level_coarse) + " but there are now" +
                    "active cells on a lower level. Coarsening the mesh is " + "currently not supported"));
  ...
  for (; cell != endc; ++cell)
    {
      ...
      for (unsigned int i = 0; i < points.size(); ++i)
        {
          Point<dim> point = quadratic_approximation[cell->index()].compute(points[i]);
          current_distance += GeometryInfo<dim>::distance_to_unit_cell(point);
        }
      distances_and_cells.push_back(std::make_pair(current_distance, cell->index()));
    }
  // no coarse cell could be found -> transformation failed
  AssertThrow(distances_and_cells.size() > 0, (typename Mapping<dim, spacedim>::ExcTransformationFailed()));
  ...
  ArrayView<Point<dim>>                   chart_points) const
{
  ...
  if (surrounding_points.size() == 8)
    {
      ...
      const double cosine = scalar_product(v06, v27) / std::sqrt(v06.norm_square() * v27.norm_square());
      if (0.707 < cosine)
  ...
  Assert((!use_structdim_2_guesses && !use_structdim_3_guesses) || (use_structdim_2_guesses ^ use_structdim_3_guesses),
         ExcInternalError());



  auto compute_chart_point = [&](const typename Triangulation<dim, spacedim>::cell_iterator &cell,
    ...
    if (point_index == 3 && surrounding_points.size() >= 8)
      ...
      {
        if (point_index < 20)
          guess = 0.5 * (chart_points[GeometryInfo<dim>::line_to_cell_vertices(point_index - 8, 0)] +
                         chart_points[GeometryInfo<dim>::line_to_cell_vertices(point_index - 8, 1)]);
        else
          guess = 0.25 * (chart_points[GeometryInfo<dim>::face_to_cell_vertices(point_index - 20, 0)] +
                          chart_points[GeometryInfo<dim>::face_to_cell_vertices(point_index - 20, 1)] +
                          chart_points[GeometryInfo<dim>::face_to_cell_vertices(point_index - 20, 2)] +
                          chart_points[GeometryInfo<dim>::face_to_cell_vertices(point_index - 20, 3)]);
      }
    else
      {
        guess                        = quadratic_approximation[cell->index()].compute(surrounding_points[point_index]);
        used_quadratic_approximation = true;
      }
    chart_points[point_index] = pull_back(cell, surrounding_points[point_index], guess);
    ...
    if (chart_points[point_index][0] == internal::invalid_pull_back_coordinate && !used_quadratic_approximation)
      {
        guess                     = quadratic_approximation[cell->index()].compute(surrounding_points[point_index]);
        chart_points[point_index] = pull_back(cell, surrounding_points[point_index], guess);
      }

    if (chart_points[point_index][0] == internal::invalid_pull_back_coordinate)
      {
        for (unsigned int d = 0; d < dim; ++d)
          guess[d] = 0.5;
        chart_points[point_index] = pull_back(cell, surrounding_points[point_index], guess);
  ...
  for (unsigned int c = 0; c < nearby_cells.size(); ++c)
    {
      typename Triangulation<dim, spacedim>::cell_iterator cell(triangulation, level_coarse, nearby_cells[c]);
      ...
      for (unsigned int i = 0; i < surrounding_points.size(); ++i)
        {
          ...
          // Tolerance 5e-4 chosen that the method also works with manifolds
          // that have some discretization error like SphericalManifold
          if (GeometryInfo<dim>::is_inside_unit_cell(chart_points[i], 5e-4) == false)
            {
      ...
      if (c == nearby_cells.size() - 1 || nearby_cells[c + 1] == numbers::invalid_unsigned_int)
        {
          ...
          for (unsigned int b = 0; b <= c; ++b)
            {
              typename Triangulation<dim, spacedim>::cell_iterator cell(triangulation, level_coarse, nearby_cells[b]);
              ...
              for (unsigned int i = 0; i < surrounding_points.size(); ++i)
                {
                  compute_chart_point(cell, i);
                  message << surrounding_points[i] << " -> " << chart_points[i] << std::endl;
                }
            }

          AssertThrow(false, (typename Mapping<dim, spacedim>::ExcTransformationFailed(message.str())));
        }
    }
  ...
  const ArrayView<const double>          &weights) const
{
  ...
  ArrayView<Point<dim>> chart_points_view = make_array_view(chart_points.begin(), chart_points.end());

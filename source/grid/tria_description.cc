  ...
  namespace Utilities
  {
    namespace
    {
      ...
      struct DescriptionTemp
      {
         ...
         */
        void
        collect(const std::vector<unsigned int>                   &relevant_processes,
                ...
                const bool                                         vertices_have_unique_ids)
        {
          const auto create_request = [&](const unsigned int other_rank) {
            const auto ptr = std::find(relevant_processes.begin(), relevant_processes.end(), other_rank);

            Assert(ptr != relevant_processes.end(), ExcInternalError());

            const auto other_rank_index = std::distance(relevant_processes.begin(), ptr);

            return description_temp[other_rank_index];
          };

          const auto process_request = [&](const unsigned int, const DescriptionTemp<dim, spacedim> &request) {
            this->merge(request, vertices_have_unique_ids);
          };

          dealii::Utilities::MPI::ConsensusAlgorithms::selector<DescriptionTemp<dim, spacedim>>(relevant_processes,
                                                                                                create_request,
                                                                                                process_request,
                                                                                                comm);
        }
         ...
         */
        void
        merge(const DescriptionTemp<dim, spacedim> &other, const bool vertices_have_unique_ids)
        {
          this->cell_infos.resize(std::max(other.cell_infos.size(), this->cell_infos.size()));

          if (vertices_have_unique_ids == false) // need to compare points
            {
              // map point to local vertex index
              std::map<Point<spacedim>, unsigned int, FloatingPointComparator<double>> map_point_to_local_vertex_index(
              ...
              for (unsigned int i = 0; i < this->coarse_cell_vertices.size(); ++i)
                map_point_to_local_vertex_index[coarse_cell_vertices[i].second] = i;

              // map local vertex indices within other to the new local indices
              std::map<unsigned int, unsigned int> map_old_to_new_local_vertex_index;
              ...
              for (const auto &p : other.coarse_cell_vertices)
                if (map_point_to_local_vertex_index.find(p.second) == map_point_to_local_vertex_index.end())
                  {
                    this->coarse_cell_vertices.emplace_back(counter, p.second);
                    map_point_to_local_vertex_index[p.second] = map_old_to_new_local_vertex_index[p.first] = counter++;
                  }
                else
                  map_old_to_new_local_vertex_index[p.first] = map_point_to_local_vertex_index[p.second];
            ...
            {
              this->coarse_cells.insert(this->coarse_cells.end(), other.coarse_cells.begin(), other.coarse_cells.end());
          ...
          this->coarse_cell_index_to_coarse_cell_id.insert(this->coarse_cell_index_to_coarse_cell_id.end(),
        ...
        reduce()
        {
          ...
          {
            std::vector<std::tuple<types::coarse_cell_id, dealii::CellData<dim>, unsigned int>> temp;

            for (unsigned int i = 0; i < this->coarse_cells.size(); ++i)
              temp.emplace_back(this->coarse_cell_index_to_coarse_cell_id[i], this->coarse_cells[i], i);
            ...
            for (unsigned int i = 0; i < temp.size(); ++i)
              {
                this->coarse_cell_index_to_coarse_cell_id[i] = std::get<0>(temp[i]);
          ...
          // make coarse cell vertices unique
          {
            ...
            this->coarse_cell_vertices.erase(std::unique(this->coarse_cell_vertices.begin(),
                                                         this->coarse_cell_vertices.end(),
                                                         [](const std::pair<unsigned int, Point<spacedim>> &a,
          ...
          for (unsigned int i = 0; i < this->cell_infos.size(); ++i)
            {
              ...
              std::sort(this->cell_infos[i].begin(), this->cell_infos[i].end(), [](const auto &a, const auto &b) {
                return a.id < b.id;
              });
                ...
                if (temp.back().id == cell_infos[i][j].id)
                  {
                    ...
                    temp.back().level_subdomain_id =
                      std::min(temp.back().level_subdomain_id, this->cell_infos[i][j].level_subdomain_id);
                  }
                else
                  {
        ...
        convert(const MPI_Comm                                             comm,
                const typename Triangulation<dim, spacedim>::MeshSmoothing mesh_smoothing,
                const TriangulationDescription::Settings                   settings)
        {
          ...
          for (unsigned int i = 0; i < this->coarse_cell_vertices.size(); ++i)
            {
              description.coarse_cell_vertices.push_back(this->coarse_cell_vertices[i].second);
          ...
          description.coarse_cell_index_to_coarse_cell_id = this->coarse_cell_index_to_coarse_cell_id;
      ...
      template <int dim, int spacedim>
      void
      mark_cell_and_its_parents(const TriaIterator<CellAccessor<dim, spacedim>> &cell,
      ...
      public:
        CreateDescriptionFromTriangulationHelper(
          ...
          const std::function<types::subdomain_id(const typename dealii::Triangulation<dim, spacedim>::cell_iterator &)>
          ...
          , comm(comm)
          , settings(settings)
          , construct_multigrid((settings & TriangulationDescription::Settings::construct_multigrid_hierarchy) != 0u)
        {
          Assert(!(settings & TriangulationDescription::Settings::construct_multigrid_hierarchy) ||
                   (tria.get_mesh_smoothing() & Triangulation<dim, spacedim>::limit_level_difference_at_vertices),
                 ExcMessage("Source triangulation has to be set up with "
        ...
        create_description_for_rank(const unsigned int my_rank) const
        {
          ...
          const auto add_vertices_of_cell_to_vertices_owned_by_locally_owned_cells =
            [this](const auto &cell, std::vector<bool> &vertices_owned_by_locally_owned_cells) {
              // add local vertices
              for (const auto v : cell->vertex_indices())
                {
                  vertices_owned_by_locally_owned_cells[cell->vertex_index(v)] = true;
                  const auto coinciding_vertex_group = vertex_to_coinciding_vertex_group.find(cell->vertex_index(v));
                  if (coinciding_vertex_group != vertex_to_coinciding_vertex_group.end())
                    for (const auto &co_vertex : coinciding_vertex_groups.at(coinciding_vertex_group->second))
          ...
          for (int level = tria.get_triangulation().n_global_levels() - 1; level >= 0; --level)
            {
              // collect vertices connected to a (on any level) locally owned
              // cell
              std::vector<bool> vertices_owned_by_locally_owned_cells_on_level(tria.n_vertices());
              for (const auto &cell : tria.cell_iterators_on_level(level))
                if (construct_multigrid && (level_subdomain_id_function(cell) == my_rank))
              ...
              const auto is_locally_relevant_on_level = [&](const auto &cell) {
                for (const auto v : cell->vertex_indices())
                  if (vertices_owned_by_locally_owned_cells_on_level[cell->vertex_index(v)])
                    return true;
                return false;
              };
            ...
            for (const auto &cell : tria.cell_iterators_on_level(0))
              {
                ...
                construction_data.coarse_cell_index_to_coarse_cell_id.push_back(cell->id().get_coarse_cell_id());
              }
          ...
          std::vector<bool> vertices_owned_by_locally_owned_active_cells(tria.n_vertices());
          for (const auto &cell : tria.active_cell_iterators())
            if (subdomain_id_function(cell) == my_rank)
              add_vertices_of_cell_to_vertices_owned_by_locally_owned_cells(
          ...
          const auto is_locally_relevant_on_active_level = [&](const auto &cell) {
            if (cell->is_active())
              for (const auto v : cell->vertex_indices())
                if (vertices_owned_by_locally_owned_active_cells[cell->vertex_index(v)])
                  return true;
            return false;
          };
                  ...
                  if (is_locally_relevant_on_active_level(cell))
                    {
                      ...
                      cell_info.level_subdomain_id = level_subdomain_id_function(cell);
                    }
      ...
      private:
        ...
        {
          std::vector<unsigned int> vertices_locally_relevant_indices(vertices_locally_relevant.size());
          ...
          for (unsigned int i = 0; i < vertices_locally_relevant.size(); ++i)
            if (vertices_locally_relevant[i])
              {
                construction_data.coarse_cell_vertices.push_back(tria.get_vertices()[i]);
          ...
          for (auto &cell : construction_data.coarse_cells)
            for (unsigned int v = 0; v < cell.vertices.size(); ++v)
              cell.vertices[v] = vertices_locally_relevant_indices[cell.vertices[v]];
        }
                     ...
                     const std::vector<bool>        &vertices_locally_relevant) const
        {
          for (unsigned int i = 0; i < vertices_locally_relevant.size(); ++i)
            if (vertices_locally_relevant[i])
              construction_data.coarse_cell_vertices.emplace_back(i, tria.get_vertices()[i]);
        }
    ...
    template <int dim, int spacedim>
    Description<dim, spacedim>
    create_description_from_triangulation(const dealii::Triangulation<dim, spacedim> &tria,
                                          const MPI_Comm                              comm,
                                          const TriangulationDescription::Settings    settings,
                                          const unsigned int                          my_rank_in)
    {
      if (const auto tria_pdt = dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(&tria))
        Assert(comm == tria_pdt->get_communicator(), ExcMessage("MPI communicators do not match."));
      ...
      Assert(my_rank == numbers::invalid_unsigned_int || my_rank < dealii::Utilities::MPI::n_mpi_processes(comm),
             ExcMessage("Rank has to be smaller than available processes."));

      if (auto tria_pdt = dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(&tria))
        {
          Assert(my_rank == numbers::invalid_unsigned_int || my_rank == dealii::Utilities::MPI::this_mpi_process(comm),
                 ExcMessage("For creation from a parallel::distributed::Triangulation, "
        ...
        {
          Assert(false, ExcMessage("This type of triangulation is not supported!"));
        }

      const auto subdomain_id_function = [](const auto &cell) { return cell->subdomain_id(); };

      const auto level_subdomain_id_function = [](const auto &cell) { return cell->level_subdomain_id(); };

      return CreateDescriptionFromTriangulationHelper<dim, spacedim>(
               tria, subdomain_id_function, level_subdomain_id_function, comm, settings)
        .template create_description_for_rank<Description<dim, spacedim>>(my_rank);
    }
      ...
      const TriangulationDescription::Settings                   settings)
    {
      ...
      const int mpi_tag = dealii::Utilities::MPI::internal::Tags::fully_distributed_create;
      ...
      if (my_rank == group_root)
        {
          ...
          dealii::Triangulation<dim, spacedim> tria(
            (settings & TriangulationDescription::Settings::construct_multigrid_hierarchy) ?
              static_cast<typename dealii::Triangulation<dim, spacedim>::MeshSmoothing>(
                smoothing | Triangulation<dim, spacedim>::limit_level_difference_at_vertices) :
              smoothing);
          ...
          if (settings & TriangulationDescription::Settings::construct_multigrid_hierarchy)
            GridTools::partition_multigrid_levels(tria);

          const unsigned int end_group =
            std::min(group_root + group_size, dealii::Utilities::MPI::n_mpi_processes(comm));
          ...
          for (unsigned int other_rank = group_root + 1; other_rank < end_group; ++other_rank)
            {
              // 3a) create construction data for other ranks
              const auto construction_data = create_description_from_triangulation(tria, comm, settings, other_rank);
              // 3b) pack
              std::vector<char> buffer;
              dealii::Utilities::pack(construction_data, buffer, false);

              // 3c) send TriangulationDescription::Description
              const auto ierr = MPI_Send(buffer.data(), buffer.size(), MPI_CHAR, other_rank, mpi_tag, comm);
              AssertThrowMPI(ierr);
            }

          // 4) create TriangulationDescription::Description for this process
          // (root of group)
          return create_description_from_triangulation(tria, comm, settings, my_rank);
        }
      else
        {
          ...
          ierr = MPI_Recv(buf.data(), len, MPI_CHAR, status.MPI_SOURCE, mpi_tag, comm, &status);
          AssertThrowMPI(ierr);

          // 3b) unpack TriangulationDescription::Description (counter-part of
          // 3b of root process)
          auto construction_data = dealii::Utilities::template unpack<Description<dim, spacedim>>(buf, false);
    ...
    template <int dim, int spacedim>
    Description<dim, spacedim>
    create_description_from_triangulation(const Triangulation<dim, spacedim>               &tria,
                                          const LinearAlgebra::distributed::Vector<double> &partition,
                                          const TriangulationDescription::Settings          settings)
    {
      const bool construct_multigrid =
        (partition.size() > 0) && (settings & TriangulationDescription::Settings::construct_multigrid_hierarchy);

      Assert(construct_multigrid == false ||
               (tria.get_mesh_smoothing() & Triangulation<dim, spacedim>::limit_level_difference_at_vertices),
             ExcMessage("Source triangulation has to be set up with "
                        "limit_level_difference_at_vertices if the construction of the "
                        "multigrid hierarchy is requested!"));
      ...
      if (construct_multigrid) // perform first child policy
        {
          const auto tria_parallel = dynamic_cast<const parallel::TriangulationBase<dim, spacedim> *>(&tria);
          ...
          for (unsigned int l = 0; l < tria.n_global_levels(); ++l)
            partitions_mg[l].reinit(tria_parallel->global_level_cell_index_partitioner(l).lock());
              ...
              for (const auto &cell : tria.cell_iterators_on_level(level))
                {
                  ...
                  if (cell->is_active())
                    partitions_mg[level][cell->global_level_cell_index()] = partition[cell->global_active_cell_index()];
                  else
                    partitions_mg[level][cell->global_level_cell_index()] =
                      partitions_mg[level + 1][cell->child(0)->global_level_cell_index()];
                }
      ...
      return create_description_from_triangulation(tria, partition, partitions_mg, settings);
    }
                                          ...
                                          const TriangulationDescription::Settings                       settings_in)
    {
      ...
      if (partition.size() == 0)
        {
          ...
          return create_description_from_triangulation(tria, tria.get_communicator(), settings_in);
        }
      ...
      const std::vector<unsigned int> relevant_processes = [&]() {
        ...
        for (unsigned int i = 0; i < partition.locally_owned_size(); ++i)
          relevant_processes.insert(static_cast<unsigned int>(partition.local_element(i)));

        for (const auto &partition : partitions_mg)
          for (unsigned int i = 0; i < partition.locally_owned_size(); ++i)
            relevant_processes.insert(static_cast<unsigned int>(partition.local_element(i)));

        return std::vector<unsigned int>(relevant_processes.begin(), relevant_processes.end());
      }();
      ...
      const auto subdomain_id_function = [&partition](const auto &cell) {
        if ((cell->is_active() && (cell->is_artificial() == false)))
          return static_cast<unsigned int>(partition[cell->global_active_cell_index()]);
        else
          return numbers::artificial_subdomain_id;
      };

      const auto level_subdomain_id_function = [&construct_multigrid, &partitions_mg](const auto &cell) {
        if (construct_multigrid && (cell->is_artificial_on_level() == false))
          return static_cast<unsigned int>(partitions_mg[cell->level()][cell->global_level_cell_index()]);
        else
          return numbers::artificial_subdomain_id;
      };

      CreateDescriptionFromTriangulationHelper<dim, spacedim> helper(
        tria, subdomain_id_function, level_subdomain_id_function, tria.get_communicator(), settings);
      ...
      for (const auto rank : relevant_processes)
        descriptions_per_rank.emplace_back(
          helper.template create_description_for_rank<DescriptionTemp<dim, spacedim>>(rank));
      ...
      description_merged.collect(relevant_processes,
                                 ...
                                 dynamic_cast<const parallel::fullydistributed::Triangulation<dim, spacedim> *>(
                                   &tria) == nullptr);
      ...
      return description_merged.convert(partition.get_mpi_communicator(), tria.get_mesh_smoothing(), settings);
    }

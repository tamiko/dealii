...
namespace CUDAWrappers
{
  ...
  {
    void
    cusparsecsr2dense(cusparseHandle_t cusparse_handle, const SparseMatrix<float> &matrix, float *dense_matrix_dev)
    {
      auto cusparse_matrix = matrix.get_cusparse_matrix();

      const cusparseStatus_t cusparse_error_code = cusparseScsr2dense(cusparse_handle,
                                                                      matrix.m(),
                                                                      matrix.n(),
    ...
    void
    cusparsecsr2dense(cusparseHandle_t cusparse_handle, const SparseMatrix<double> &matrix, double *dense_matrix_dev)
    {
      auto cusparse_matrix = matrix.get_cusparse_matrix();

      const cusparseStatus_t cusparse_error_code = cusparseDcsr2dense(cusparse_handle,
                                                                      matrix.m(),
                                                                      matrix.n(),
                                                                      std::get<3>(cusparse_matrix),
                                                                      std::get<0>(cusparse_matrix),
    ...
    {
      const cusolverStatus_t cusolver_error_code =
        cusolverDnSgetrf(cusolver_dn_handle, m, n, dense_matrix_dev, m, workspace_dev, pivot_dev, info_dev);
      AssertCusolver(cusolver_error_code);
    }
    ...
    {
      const cusolverStatus_t cusolver_error_code =
        cusolverDnDgetrf(cusolver_dn_handle, m, n, dense_matrix_dev, m, workspace_dev, pivot_dev, info_dev);
      AssertCusolver(cusolver_error_code);
    }
                    ...
                    int               *info_dev)
    {
      const int              n_rhs = 1;
      const cusolverStatus_t cusolver_error_code =
        cusolverDnSgetrs(cusolver_dn_handle, CUBLAS_OP_N, m, n_rhs, dense_matrix_dev, m, pivot_dev, b, m, info_dev);
      AssertCusolver(cusolver_error_code);
    }
                    ...
                    int               *info_dev)
    {
      const int              n_rhs = 1;
      const cusolverStatus_t cusolver_error_code =
        cusolverDnDgetrs(cusolver_dn_handle, CUBLAS_OP_N, m, n_rhs, dense_matrix_dev, m, pivot_dev, b, m, info_dev);
      AssertCusolver(cusolver_error_code);
    }
    ...
    {
      int                    singularity         = 0;
      const cusolverStatus_t cusolver_error_code = cusolverSpScsrlsvluHost(cusolver_sp_handle,
                                                                           n_rows,
                                                                           nnz,
                                                                           descr,
                                                                           val_host,
    ...
    {
      int                    singularity         = 0;
      const cusolverStatus_t cusolver_error_code = cusolverSpDcsrlsvluHost(cusolver_sp_handle,
                                                                           n_rows,
                                                                           nnz,
                                                                           descr,
                                                                           val_host,
                           ...
                           float                     *x)
    {
      ...
      const cusolverStatus_t cusolver_error_code = cusolverSpScsrlsvchol(cusolver_sp_handle,
                                                                         matrix.m(),
                           ...
                           double                     *x)
    {
      ...
      const cusolverStatus_t cusolver_error_code = cusolverSpDcsrlsvchol(cusolver_sp_handle,
                                                                         matrix.m(),
                                                                         matrix.n_nonzero_elements(),
                                                                         std::get<3>(cusparse_matrix),
                                                                         std::get<0>(cusparse_matrix),
                     ...
                     Number                     *x_dev)
    {
      ...
      cusolverDngetrf_buffer_size(cusolver_dn_handle, m, n, dense_matrix_dev, workspace_size);
      ...
      cusolverDngetrf(cusolver_dn_handle, m, n, dense_matrix_dev, workspace_dev, pivot_dev, info_dev);

#ifdef DEBUG
      int         info                  = 0;
      cudaError_t cuda_error_code_debug = cudaMemcpy(&info, info_dev, sizeof(int), cudaMemcpyDeviceToHost);
      AssertCuda(cuda_error_code_debug);
      Assert(info == 0, ExcMessage("There was a problem during the LU factorization"));
#endif

      // Solve Ax = b
      cudaError_t cuda_error_code = cudaMemcpy(x_dev, b_dev, m * sizeof(Number), cudaMemcpyDeviceToDevice);
      AssertCuda(cuda_error_code);
      cusolverDngetrs(cusolver_dn_handle, m, dense_matrix_dev, pivot_dev, x_dev, info_dev);
#ifdef DEBUG
                              ...
                              const LinearAlgebra::CUDAWrappers::Vector<Number> &b)
  {
    if (additional_data.solver_type == "Cholesky")
      cholesky_factorization(cuda_handle.cusolver_sp_handle, A, b.get_values(), x.get_values());
    else if (additional_data.solver_type == "LU_dense")
      lu_factorization(cuda_handle.cusparse_handle, cuda_handle.cusolver_dn_handle, A, b.get_values(), x.get_values());
    else if (additional_data.solver_type == "LU_host")
      lu_factorization(cuda_handle.cusolver_sp_handle, A, b.get_values(), x.get_values());
    else
      AssertThrow(false, ExcMessage("The provided solver name " + additional_data.solver_type + " is invalid."));

  ...
  for (size_type row = 0; row < original.rows; ++row)
    {
      ...
      for (size_type i = 1; i <= std::min(extra_off_diagonals, rows - row - 1); ++i, ++next_free_slot)
        *next_free_slot = row + i;

      // copy rest
      next_free_slot = std::copy(original_first_after_side_diagonals, original_row_end, next_free_slot);
      ...
      Assert(next_free_slot <= &colnums[rowstart[row + 1]], ExcNotEnoughSpace(0, rowstart[row + 1] - rowstart[row]));
    }
}
...
void
SparsityPattern::reinit(const size_type m, const size_type n, const ArrayView<const unsigned int> &row_lengths)
{
  ...
  for (size_type i = 0; i < m; ++i)
    vec_len +=
      std::min(static_cast<size_type>(store_diagonal_first_in_row ? std::max(row_lengths[i], 1U) : row_lengths[i]), n);
  ...
  max_row_length = (row_lengths.empty() ?
                      0 :
                      std::min(static_cast<size_type>(*std::max_element(row_lengths.begin(), row_lengths.end())), n));
  ...
  for (size_type i = 1; i <= rows; ++i)
    rowstart[i] = rowstart[i - 1] +
                  (store_diagonal_first_in_row ?
                     std::max(std::min(static_cast<size_type>(row_lengths[i - 1]), n), static_cast<size_type>(1U)) :
                     std::min(static_cast<size_type>(row_lengths[i - 1]), n));
  Assert((rowstart[rows] == vec_len) || ((vec_len == 1) && (rowstart[rows] == 0)), ExcInternalError());
...
void
SparsityPattern::reinit(const size_type m, const size_type n, const std::vector<unsigned int> &row_lengths)
{
...
SparsityPattern::compress()
{
  ...
  const std::size_t nonzero_elements = std::count_if(&colnums[rowstart[0]],
  ...
  for (size_type line = 0; line < n_rows(); ++line)
    {
      ...
      // so check this first
      Assert((!store_diagonal_first_in_row) || (row_length != 0 && new_colnums[rowstart[line]] == line),
             ExcInternalError());
      ...
      Assert((rowstart[line] == next_row_start) ||
             ...
             ExcInternalError());
      Assert((rowstart[line] == next_row_start) ||
               (std::adjacent_find(&new_colnums[rowstart[line] + 1], &new_colnums[next_row_start]) ==
                &new_colnums[next_row_start]),
             ExcInternalError());
    }
...
SparsityPattern::copy_from(const FullMatrix<number> &matrix)
{
  ...
  column_indices.reserve(
    entries_per_row.size() > 0 ? *std::max_element(entries_per_row.begin(), entries_per_row.end()) : 0);
  for (size_type row = 0; row < matrix.m(); ++row)
    {
...
SparsityPattern::matrix_position(const std::size_t global_index) const
{
  ...
  const size_type row = (std::upper_bound(rowstart.get(), rowstart.get() + rows, global_index) - rowstart.get() - 1);
            ...
            for (; it != end; ++it)
              {
                ...
                if (store_diagonal_first_in_row && *it == row)
                  continue;
                Assert(k <= rowstart[row + 1], ExcNotEnoughSpace(row, rowstart[row + 1] - rowstart[row]));
                colnums[k++] = *it;
              }
          else
...
SparsityPattern::print_svg(std::ostream &out) const
{
  ...
  for (const auto &entry : *this)
    {
      out << "  <rect class=\"pixel\" x=\"" << entry.column() + 1 << "\" y=\"" << entry.row() + 1
          << "\" width=\".9\" height=\".9\"/>\n";
    }
  out << "</svg>" << std::endl;
}
...
SparsityPattern::block_write(std::ostream &out) const
{
  ...
  out << '[' << max_dim << ' ' << n_rows() << ' ' << n_cols() << ' ' << max_vec_len << ' ' << max_row_length << ' '
...
SparsityPattern::block_read(std::istream &in)
{
  ...
  in >> c;
  AssertThrow(c == '[', ExcIO());
  in >> max_dim >> rows >> cols >> max_vec_len >> max_row_length >> compressed >> store_diagonal_first_in_row;
  ...
  in.read(reinterpret_cast<char *>(rowstart.get()),
          reinterpret_cast<char *>(rowstart.get() + max_dim + 1) - reinterpret_cast<char *>(rowstart.get()));
  in >> c;
  AssertThrow(c == ']', ExcIO());
  in >> c;
  AssertThrow(c == '[', ExcIO());
  in.read(reinterpret_cast<char *>(colnums.get()),
          reinterpret_cast<char *>(colnums.get() + max_vec_len) - reinterpret_cast<char *>(colnums.get()));
  in >> c;
  AssertThrow(c == ']', ExcIO());
}
...
#  endif
template void
SparsityPattern::add_entries<std::vector<SparsityPattern::size_type>::iterator>(const size_type,

...
namespace TrilinosWrappers
{
  ...
  {
    AssertThrow(!preconditioner.is_null(), ExcMessage("Trying to dereference a null pointer."));
  ...
  PreconditionJacobi::initialize(const SparseMatrix &matrix, const AdditionalData &additional_data)
  {
    ...
    preconditioner.reset();
    preconditioner.reset(
      Ifpack().Create("point relaxation", const_cast<Epetra_CrsMatrix *>(&matrix.trilinos_matrix()), 0));
  ...
  void
  PreconditionBlockJacobi::initialize(const SparseMatrix &matrix, const AdditionalData &additional_data)
  {
    // release memory before reallocation
    preconditioner.reset();
    ...
    preconditioner.reset(
      Ifpack().Create((matrix.trilinos_matrix().NumMyRows() == 0) ? "point relaxation" : "block relaxation",
                      const_cast<Epetra_CrsMatrix *>(&matrix.trilinos_matrix()),
                      0));
    ...
    int n_local_parts =
      (matrix.trilinos_matrix().NumMyRows() + additional_data.block_size - 1) / additional_data.block_size;
  ...
  void
  PreconditionBlockSSOR::initialize(const SparseMatrix &matrix, const AdditionalData &additional_data)
  {
    preconditioner.reset();
  ...
  void
  PreconditionBlockSOR::initialize(const SparseMatrix &matrix, const AdditionalData &additional_data)
  {
    preconditioner.reset();
  ...
  PreconditionIC::initialize(const SparseMatrix &matrix, const AdditionalData &additional_data)
  {
    preconditioner.reset();
    preconditioner.reset(
      Ifpack().Create("IC", const_cast<Epetra_CrsMatrix *>(&matrix.trilinos_matrix()), additional_data.overlap));
  ...
  PreconditionILU::initialize(const SparseMatrix &matrix, const AdditionalData &additional_data)
  {
    preconditioner.reset();
    preconditioner.reset(
      Ifpack().Create("ILU", const_cast<Epetra_CrsMatrix *>(&matrix.trilinos_matrix()), additional_data.overlap));
  ...
  PreconditionILUT::initialize(const SparseMatrix &matrix, const AdditionalData &additional_data)
  {
    preconditioner.reset();
    preconditioner.reset(
      Ifpack().Create("ILUT", const_cast<Epetra_CrsMatrix *>(&matrix.trilinos_matrix()), additional_data.overlap));
    ...
    parameter_list.set("fact: level-of-fill", static_cast<int>(additional_data.ilut_fill));
  ...
  void
  PreconditionBlockwiseDirect::initialize(const SparseMatrix &matrix, const AdditionalData &additional_data)
  {
    preconditioner.reset();
    preconditioner.reset(
      Ifpack().Create("Amesos", const_cast<Epetra_CrsMatrix *>(&matrix.trilinos_matrix()), additional_data.overlap));
  ...
  void
  PreconditionChebyshev::initialize(const SparseMatrix &matrix, const AdditionalData &additional_data)
  {
    preconditioner = Teuchos::rcp(new Ifpack_Chebyshev(&matrix.trilinos_matrix()));

    Ifpack_Chebyshev *ifpack = dynamic_cast<Ifpack_Chebyshev *>(preconditioner.get());
    Assert(ifpack != nullptr,
           ExcMessage("Trilinos could not create this "
                      "preconditioner"));
    ...
    parameter_list.set("chebyshev: degree", static_cast<int>(additional_data.degree));

...
namespace TrilinosWrappers
{
  ...
  SolverBase::SolverBase(const SolverBase::SolverName solver_name, SolverControl &cn, const AdditionalData &data)
  ...
  void
  SolverBase::solve(const SparseMatrix &A, MPI::Vector &x, const MPI::Vector &b, const PreconditionBase &preconditioner)
  {
    // We need an Epetra_LinearProblem object to let the AztecOO solver know
    // about the matrix and vectors.
    linear_problem = std::make_unique<Epetra_LinearProblem>(const_cast<Epetra_CrsMatrix *>(&A.trilinos_matrix()),
                                                            &x.trilinos_vector(),
                    ...
                    const PreconditionBase &preconditioner)
  {
    // We need an Epetra_LinearProblem object to let the AztecOO solver know
    // about the matrix and vectors.
    linear_problem = std::make_unique<Epetra_LinearProblem>(const_cast<Epetra_Operator *>(&A),
                                                            &x.trilinos_vector(),
                                                            const_cast<Epetra_MultiVector *>(&b.trilinos_vector()));
                    ...
                    const PreconditionBase       &preconditioner)
  {
    ...
    Assert(A.local_range().second == A.m(), ExcMessage("Can only work in serial when using deal.II vectors."));
    Assert(A.trilinos_matrix().Filled(), ExcMessage("Matrix is not compressed. Call compress() method."));

    Epetra_Vector ep_x(View, A.trilinos_matrix().DomainMap(), x.begin());
    Epetra_Vector ep_b(View, A.trilinos_matrix().RangeMap(), const_cast<double *>(b.begin()));

    // We need an Epetra_LinearProblem object to let the AztecOO solver know
    // about the matrix and vectors.
    linear_problem =
      std::make_unique<Epetra_LinearProblem>(const_cast<Epetra_CrsMatrix *>(&A.trilinos_matrix()), &ep_x, &ep_b);
                    ...
                    const PreconditionBase                                   &preconditioner)
  {
    // In case we call the solver with deal.II vectors, we create views of the
    // vectors in Epetra format.
    AssertDimension(x.locally_owned_size(), A.trilinos_matrix().DomainMap().NumMyElements());
    AssertDimension(b.locally_owned_size(), A.trilinos_matrix().RangeMap().NumMyElements());
  ...
  SolverBase::solve(Epetra_Operator                                          &A,
                    ...
                    const PreconditionBase                                   &preconditioner)
  {
    AssertDimension(x.locally_owned_size(), A.OperatorDomainMap().NumMyElements());
    AssertDimension(b.locally_owned_size(), A.OperatorRangeMap().NumMyElements());
  ...
  namespace internal
  {
    namespace
    {
      ...
      {
        Assert(residual_vector->NumVectors() == 1, ExcMessage("Residual multivector holds more than one vector"));
      ...
      public:
                    ...
                    bool                SolutionUpdated) override
        {
          ...
          current_residual = (CurrentResNormEst < 0.0 ? compute_residual(CurrentResVector) : CurrentResNormEst);
          if (CurrentIter == 0)
            initial_residual = current_residual;

          return status_test_collection->CheckStatus(CurrentIter, CurrentResVector, CurrentResNormEst, SolutionUpdated);
        }
      ...
      TrilinosReductionControl::TrilinosReductionControl(const int                   max_steps,
                                                         const double                tolerance,
                                                         const double                reduction,
                                                         const Epetra_LinearProblem &linear_problem)
        ...
        // criterion are met
        , status_test_collection(std::make_unique<AztecOO_StatusTestCombo>(AztecOO_StatusTestCombo::OR))
      {
        // Maximum number of iterations
        Assert(max_steps >= 0, ExcInternalError());
        status_test_max_steps = std::make_unique<AztecOO_StatusTestMaxIters>(max_steps);
        status_test_collection->AddStatusTest(*status_test_max_steps);

        Assert(linear_problem.GetRHS()->NumVectors() == 1, ExcMessage("RHS multivector holds more than one vector"));

        // Residual norm is below some absolute value
        status_test_abs_tol = std::make_unique<AztecOO_StatusTestResNorm>(*linear_problem.GetOperator(),
                                                                          *(linear_problem.GetLHS()->operator()(0)),
                                                                          *(linear_problem.GetRHS()->operator()(0)),
                                                                          tolerance);
        status_test_abs_tol->DefineResForm(AztecOO_StatusTestResNorm::Explicit, AztecOO_StatusTestResNorm::TwoNorm);
        status_test_abs_tol->DefineScaleForm(AztecOO_StatusTestResNorm::None, AztecOO_StatusTestResNorm::TwoNorm);
        ...
        status_test_rel_tol = std::make_unique<AztecOO_StatusTestResNorm>(*linear_problem.GetOperator(),
                                                                          *(linear_problem.GetLHS()->operator()(0)),
                                                                          *(linear_problem.GetRHS()->operator()(0)),
                                                                          reduction);
        status_test_rel_tol->DefineResForm(AztecOO_StatusTestResNorm::Explicit, AztecOO_StatusTestResNorm::TwoNorm);
    ...
    if (!status_test)
      {
        if (const ReductionControl *const reduction_control =
          ...
          {
            status_test = std::make_unique<internal::TrilinosReductionControl>(reduction_control->max_steps(),
  ...
  SolverBase::set_preconditioner(AztecOO &solver, const PreconditionBase &preconditioner)
  {
    ...
    if (preconditioner.preconditioner.strong_count() != 0)
      {
        const int ierr = solver.SetPrecOperator(const_cast<Epetra_Operator *>(preconditioner.preconditioner.get()));
        AssertThrow(ierr == 0, ExcTrilinosError(ierr));
      }
    else
  ...
  SolverDirect::AdditionalData::AdditionalData(const bool output_solver_details, const std::string &solver_type)
    : output_solver_details(output_solver_details)
  ...
  SolverDirect::initialize(const SparseMatrix &A)
  {
    ...
    AssertThrow(Factory.Query(additional_data.solver_type.c_str()),
                ExcMessage(std::string("You tried to select the solver type <") + additional_data.solver_type +
  ...
  void
  SolverDirect::solve(const SparseMatrix &A, dealii::Vector<double> &x, const dealii::Vector<double> &b)
  {
    // In case we call the solver with deal.II vectors, we create views of the
    // vectors in Epetra format.
    Assert(x.size() == A.n(), ExcDimensionMismatch(x.size(), A.n()));

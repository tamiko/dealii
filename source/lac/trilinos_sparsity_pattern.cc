...
namespace TrilinosWrappers
{
      ...
      if (colnum_cache->size() > 0)
        {
          ...
          int       ncols;
          const int ierr =
            sparsity_pattern->graph->ExtractGlobalRowCopy(this->a_row,
                                                          colnum_cache->size(),
                                                          ncols,
  ...
  {
    column_space_map = std::make_unique<Epetra_Map>(TrilinosWrappers::types::int_type(0),
                                                    TrilinosWrappers::types::int_type(0),
                                                    Utilities::Trilinos::comm_self());
    graph            = std::make_unique<Epetra_FECrsGraph>(View, *column_space_map, *column_space_map, 0);
    graph->FillComplete();
  }



  SparsityPattern::SparsityPattern(const size_type m, const size_type n, const size_type n_entries_per_row)
  {
  ...
  {
    (void)input_sparsity;
    Assert(input_sparsity.n_rows() == 0, ExcMessage("Copy constructor only works for empty sparsity patterns."));
  }
  ...
  {
    reinit(row_parallel_partitioning, col_parallel_partitioning, writable_rows, communicator, n_max_entries_per_row);
  }
  ...
  void
  SparsityPattern::reinit(const size_type m, const size_type n, const std::vector<size_type> &n_entries_per_row)
  {
    reinit(complete_index_set(m), complete_index_set(n), MPI_COMM_SELF, n_entries_per_row);
  }
  ...
  namespace
  {
              ...
              std::unique_ptr<Epetra_CrsGraph>   &nonlocal_graph)
    {
      ...
      AssertThrow((TrilinosWrappers::max_my_gid(row_map) - TrilinosWrappers::min_my_gid(row_map)) *
      ...
      if (row_map.Comm().NumProc() > 1)
        graph =
          std::make_unique<Epetra_FECrsGraph>(Copy, row_map, n_entries_per_row, false
        ...
        graph = std::make_unique<Epetra_FECrsGraph>(Copy, row_map, col_map, n_entries_per_row, false);
    }
              ...
              std::unique_ptr<Epetra_CrsGraph>   &nonlocal_graph)
    {
      ...
      nonlocal_graph.reset();
      graph.reset();
      AssertDimension(n_entries_per_row.size(), TrilinosWrappers::n_global_elements(row_map));
      ...
      AssertThrow(std::accumulate(local_entries_per_row.begin(), local_entries_per_row.end(), std::uint64_t(0)) <
                    static_cast<std::uint64_t>(std::numeric_limits<int>::max()),
                  ExcMessage("The TrilinosWrappers use Epetra internally which "
                             "uses 'signed int' to represent local indices. "
                             "Therefore, only 2,147,483,647 nonzero matrix "
      ...
      if (row_map.Comm().NumProc() > 1)
        graph =
          std::make_unique<Epetra_FECrsGraph>(Copy, row_map, local_entries_per_row.data(), false
                                              // TODO: Check which new Trilinos version supports this...
                                              // Remember to change tests/trilinos/assemble_matrix_parallel_07, too.
                                              // #if DEAL_II_TRILINOS_VERSION_GTE(11,14,0)
                                              //                , true
        ...
        graph = std::make_unique<Epetra_FECrsGraph>(Copy, row_map, col_map, local_entries_per_row.data(), false);
    }
              ...
              std::unique_ptr<Epetra_CrsGraph>   &nonlocal_graph)
    {
      ...
      AssertDimension(sp.n_rows(), TrilinosWrappers::n_global_elements(row_map));
      AssertDimension(sp.n_cols(), TrilinosWrappers::n_global_elements(col_map));

      column_space_map = std::make_unique<Epetra_Map>(col_map);

      Assert(row_map.LinearMap() == true, ExcMessage("This function only works if the row map is contiguous."));
      ...
      AssertThrow(std::accumulate(n_entries_per_row.begin(), n_entries_per_row.end(), std::uint64_t(0)) <
                    static_cast<std::uint64_t>(std::numeric_limits<int>::max()),
                  ExcMessage("The TrilinosWrappers use Epetra internally which "
                             "uses 'signed int' to represent local indices. "
                             "Therefore, only 2,147,483,647 nonzero matrix "
      ...
      if (row_map.Comm().NumProc() > 1)
        graph = std::make_unique<Epetra_FECrsGraph>(Copy, row_map, n_entries_per_row.data(), false);
      else
        graph = std::make_unique<Epetra_FECrsGraph>(Copy, row_map, col_map, n_entries_per_row.data(), false);
        ...
        for (size_type row = 0; row < sp.n_rows(); ++row)
          {
            ...
            graph->InsertGlobalIndices(1, &trilinos_row, row_length, row_indices.data());
          }

      // TODO A dynamic_cast fails here, this is suspicious.
      const auto &range_map = static_cast<const Epetra_Map &>(graph->RangeMap()); // NOLINT
  ...
  SparsityPattern::reinit(const IndexSet &parallel_partitioning,
                          ...
                          const size_type n_entries_per_row)
  {
    SparsityPatternBase::resize(parallel_partitioning.size(), parallel_partitioning.size());
  ...
  SparsityPattern::reinit(const IndexSet &row_parallel_partitioning,
                          ...
                          const size_type n_entries_per_row)
  {
    SparsityPatternBase::resize(row_parallel_partitioning.size(), col_parallel_partitioning.size());
...
#  endif
    ...
    if (Utilities::MPI::n_mpi_processes(communicator) > 1)
      {
        ...
        nonlocal_graph          = std::make_unique<Epetra_CrsGraph>(Copy, nonlocal_map, 0);
      }
    else
                          ...
                          const bool                 exchange_data)
  {
    ...
    Epetra_Map row_map = row_parallel_partitioning.make_trilinos_map(communicator, false);
    Epetra_Map col_map = col_parallel_partitioning.make_trilinos_map(communicator, false);
    reinit_sp(row_map, col_map, nontrilinos_sparsity_pattern, exchange_data, column_space_map, graph, nonlocal_graph);
  }
  ...
  SparsityPattern::reinit(const IndexSet            &parallel_partitioning,
                          ...
                          const bool                 exchange_data)
  {
    AssertDimension(nontrilinos_sparsity_pattern.n_rows(), parallel_partitioning.size());
    AssertDimension(nontrilinos_sparsity_pattern.n_cols(), parallel_partitioning.size());
    SparsityPatternBase::resize(parallel_partitioning.size(), parallel_partitioning.size());
    Epetra_Map map = parallel_partitioning.make_trilinos_map(communicator, false);
    reinit_sp(map, map, nontrilinos_sparsity_pattern, exchange_data, column_space_map, graph, nonlocal_graph);
  }
  ...
  {
    SparsityPatternBase::resize(sp.n_rows(), sp.n_cols());
    const Epetra_Map rows(TrilinosWrappers::types::int_type(sp.n_rows()), 0, Utilities::Trilinos::comm_self());
    const Epetra_Map columns(TrilinosWrappers::types::int_type(sp.n_cols()), 0, Utilities::Trilinos::comm_self());
  ...
  SparsityPattern::compress()
  {
    ...
    if (nonlocal_graph.get() != nullptr)
      {
        if (nonlocal_graph->IndicesAreGlobal() == false && nonlocal_graph->RowMap().NumMyElements() > 0 &&
            n_global_elements(*column_space_map) > 0)
          {
            // Insert dummy element at (row, column) that corresponds to row 0
            // in local index counting.
            TrilinosWrappers::types::int_type row    = TrilinosWrappers::global_index(nonlocal_graph->RowMap(), 0);
        ...
        Assert(nonlocal_graph->RowMap().NumMyElements() == 0 || n_global_elements(*column_space_map) == 0 ||
                 nonlocal_graph->IndicesAreGlobal() == true,
               ExcInternalError());

        ierr = nonlocal_graph->FillComplete(*column_space_map, graph->RangeMap());
        AssertThrow(ierr >= 0, ExcTrilinosError(ierr));
    ...
    try
      ...
      {
        AssertThrow(false,
                    ExcMessage("The Epetra_CrsGraph::OptimizeStorage() function "
    ...
    Assert(compare_for_equality(n_rows(), n_global_rows(*graph)), ExcInternalError());
    Assert(compare_for_equality(n_cols(), n_global_cols(*graph)), ExcInternalError());
  }
  ...
  {
    return graph->RowMap().LID(static_cast<TrilinosWrappers::types::int_type>(i)) != -1;
  }
        ...
        if (graph->Filled() == false)
          {
            ...
            int ierr = graph->ExtractGlobalRowView(trilinos_i, nnz_extracted, col_indices);
            (void)ierr;
    ...
    for (int i = 0; i < static_cast<int>(local_size()); ++i)
      {
        ...
        for (unsigned int j = 0; j < static_cast<unsigned int>(num_entries); ++j)
          {
            if (static_cast<size_type>(std::abs(i - indices[j])) > local_b)
              local_b = std::abs(i - indices[j]);
          }
      }
    graph->Comm().MaxAll(reinterpret_cast<TrilinosWrappers::types::int_type *>(&local_b), &global_b, 1);
  ...
  SparsityPattern::row_length(const size_type row) const
  {
    ...
    TrilinosWrappers::types::int_type local_row = graph->LRID(static_cast<TrilinosWrappers::types::int_type>(row));
  ...
  {
    // TODO A dynamic_cast fails here, this is suspicious.
    const auto &domain_map = static_cast<const Epetra_Map &>(graph->DomainMap()); // NOLINT
    return domain_map;
  }
  ...
  {
    const Epetra_MpiComm *mpi_comm = dynamic_cast<const Epetra_MpiComm *>(&graph->RangeMap().Comm());
        ...
        for (int i = 0; i < graph->NumMyRows(); ++i)
          {
            ...
            for (int j = 0; j < num_entries; ++j)
              out << "(" << TrilinosWrappers::global_index(graph->RowMap(), i) << ","
                  << TrilinosWrappers::global_index(graph->ColMap(), indices[j]) << ") " << std::endl;
          }
      }
...
#  ifndef DOXYGEN
  ...
  template void
  SparsityPattern::reinit(const IndexSet &, const dealii::SparsityPattern &, const MPI_Comm, bool);
  template void
  SparsityPattern::reinit(const IndexSet &, const dealii::DynamicSparsityPattern &, const MPI_Comm, bool);


  template void
  SparsityPattern::reinit(const IndexSet &, const IndexSet &, const dealii::SparsityPattern &, const MPI_Comm, bool);
  template void

...
namespace internal
{
  namespace MatrixFreeFunctions
  {
                                           ...
                                           const bool                 apply_constraints) const
    {
      ...
      const unsigned int fe_index        = dofs_per_cell.size() == 1 ? 0 : cell_active_fe_index[cell];
      ...
      AssertIndexRange(cell, n_vectorization_lanes_filled[dof_access_index].size());
      const unsigned int n_vectorization_actual = n_vectorization_lanes_filled[dof_access_index][cell];
      ...
      for (unsigned int v = 0; v < n_vectorization_actual; ++v)
        {
          const unsigned int ib = (cell * n_vectorization + v) * n_fe_components;
          const unsigned int ie = (cell * n_vectorization + v + 1) * n_fe_components;
          ...
          const bool has_constraints =
            (hanging_node_constraint_masks.size() != 0 && hanging_node_constraint_masks_comp.size() != 0 &&
             hanging_node_constraint_masks[cell * n_vectorization + v] != unconstrained_compressed_constraint_kind &&
             hanging_node_constraint_masks_comp[fe_index][0 /*TODO*/]) ||
            (row_starts[ib].second != row_starts[ib + n_fe_components].second);

          auto do_copy = [&](const unsigned int *begin, const unsigned int *end) {
          ...
          if (!has_constraints || apply_constraints)
            {
              const unsigned int *begin = dof_indices.data() + row_starts[ib].first;
              const unsigned int *end   = dof_indices.data() + row_starts[ie].first;
              do_copy(begin, end);
            }
          else
            {
              Assert(row_starts_plain_indices[cell * n_vectorization + v] != numbers::invalid_unsigned_int,
                     ExcNotInitialized());
              const unsigned int *begin =
                plain_dof_indices.data() + row_starts_plain_indices[cell * n_vectorization + v];
                           ...
                           const bool                       use_vector_data_exchanger_full)
    {
      ...
      const unsigned int n_owned = (vector_partitioner->local_range().second - vector_partitioner->local_range().first);
      ...
      if (n_ghosts > 0)
        {
          ...
          // dofs. the ghost index set should store the same number
          {
            ...
            for (std::size_t i = 0; i < n_ghosts; ++i)
              Assert(ghost_numbering[i] == ghost_indices.index_within_set(ghost_dofs[i]), ExcInternalError());
          }
          ...
          for (unsigned int i = 0; i < n_boundary_cells; ++i)
            {
              unsigned int       *data_ptr = dof_indices.data() + row_starts[boundary_cells[i] * n_components].first;
              const unsigned int *row_end =
                dof_indices.data() + row_starts[(boundary_cells[i] + 1) * n_components].first;
              for (; data_ptr != row_end; ++data_ptr)
                *data_ptr = ((*data_ptr < n_owned) ? *data_ptr : n_owned + ghost_numbering[*data_ptr - n_owned]);
              ...
              if (store_plain_indices == true)
                {
                  ...
                  const unsigned int fe_index = (cell_active_fe_index.empty() || dofs_per_cell.size() == 1) ?
                                                  0 :
                                                  cell_active_fe_index[boundary_cells[i]];
                  AssertIndexRange(fe_index, dofs_per_cell.size());

                  if (hanging_node_constraint_masks.size() > 0 && hanging_node_constraint_masks_comp.size() > 0 &&
                      hanging_node_constraint_masks[boundary_cells[i]] != unconstrained_compressed_constraint_kind)
                    for (unsigned int comp = 0; comp < n_components; ++comp)
                      has_hanging_nodes |= hanging_node_constraint_masks_comp[fe_index][comp];

                  if (has_hanging_nodes || row_starts[boundary_cells[i] * n_components].second !=
                                             row_starts[(boundary_cells[i] + 1) * n_components].second)
                    {
                      unsigned int *data_ptr = plain_dof_indices.data() + row_starts_plain_indices[boundary_cells[i]];
                      const unsigned int *row_end = data_ptr + dofs_per_cell[fe_index];
                      for (; data_ptr != row_end; ++data_ptr)
                        *data_ptr =
                          ((*data_ptr < n_owned) ? *data_ptr : n_owned + ghost_numbering[*data_ptr - n_owned]);
                    }
                }
            }
        }
      ...
      Utilities::MPI::Partitioner *vec_part = const_cast<Utilities::MPI::Partitioner *>(vector_partitioner.get());
      vec_part->set_ghost_indices(ghost_indices);

      if (use_vector_data_exchanger_full == false)
        vector_exchanger =
          std::make_shared<MatrixFreeFunctions::VectorDataExchange::PartitionerWrapper>(vector_partitioner);
      else
        vector_exchanger =
          std::make_shared<MatrixFreeFunctions::VectorDataExchange::Full>(vector_partitioner, communicator_sm);
    }
                           ...
                           const std::vector<unsigned char> &irregular_cells)
    {
      ...
      if (cell_active_fe_index.size() > 0)
        {
          ...
          for (unsigned int cell = 0; cell < task_info.cell_partition_data.back(); ++cell)
            {
              const unsigned int n_comp = (irregular_cells[cell] > 0 ? irregular_cells[cell] : vectorization_length);
      ...
      new_hanging_node_constraint_masks.reserve(hanging_node_constraint_masks.size());
      ...
      for (unsigned int i = 0; i < task_info.cell_partition_data.back(); ++i)
        {
          const unsigned int n_vect = (irregular_cells[i] > 0 ? irregular_cells[i] : vectorization_length);
          ...
          for (unsigned int j = 0; j < n_vect; ++j)
            {
              ...
              if (hanging_node_constraint_masks.size() > 0 && hanging_node_constraint_masks_comp.size() > 0)
                {
                  const auto mask = hanging_node_constraint_masks[renumbering[position_cell + j]];
                  new_hanging_node_constraint_masks.push_back(mask);

                  if (mask != unconstrained_compressed_constraint_kind)
                    for (unsigned int comp = 0; comp < n_components; ++comp)
                      has_hanging_nodes |=
                        hanging_node_constraint_masks_comp[have_hp ? cell_active_fe_index[i] : 0][comp];
                }

              for (unsigned int comp = 0; comp < n_components; ++comp)
                {
                  new_row_starts[(i * vectorization_length + j) * n_components + comp].first = new_dof_indices.size();
              ...
              if (store_plain_indices &&
                  ((row_starts[cell_no].second != row_starts[cell_no + n_components].second) || has_hanging_nodes))
                {
          ...
          for (unsigned int j = n_vect; j < vectorization_length; ++j)
            if (hanging_node_constraint_masks.size() > 0)
              new_hanging_node_constraint_masks.push_back(unconstrained_compressed_constraint_kind);
    ...
    DoFInfo::compute_cell_index_compression(const std::vector<unsigned char> &irregular_cells)
    {
      ...
      Assert(vectorization_length == 1 || row_starts.size() % vectorization_length == 1, ExcInternalError());
      if (vectorization_length > 1)
        AssertDimension(row_starts.size() / vectorization_length / n_components, irregular_cells.size());
      index_storage_variants[dof_access_cell].resize(irregular_cells.size(), IndexStorageVariants::full);
      ...
      dof_indices_interleave_strides[dof_access_cell].resize(irregular_cells.size() * vectorization_length,
                                                             numbers::invalid_unsigned_int);

      std::vector<unsigned int> index_kinds(
        static_cast<unsigned int>(IndexStorageVariants::interleaved_contiguous_mixed_strides) + 1);
          ...
          if (has_constraints)
          ...
          else
            {
              ...
              for (unsigned int j = 0; j < n_comp; ++j)
                {
                  const unsigned int  cell_no     = i * vectorization_length + j;
                  const unsigned int *dof_indices = this->dof_indices.data() + row_starts[cell_no * n_components].first;
              ...
              bool indices_are_interleaved_and_contiguous = (ndofs > 1 && n_comp == vectorization_length);

              {
                const unsigned int *dof_indices =
                  this->dof_indices.data() + row_starts[i * vectorization_length * n_components].first;
                for (unsigned int k = 0; k < ndofs && indices_are_interleaved_and_contiguous; ++k)
                  for (unsigned int j = 0; j < n_comp; ++j)
              ...
              if (indices_are_contiguous || indices_are_interleaved_and_contiguous)
                {
                  for (unsigned int j = 0; j < n_comp; ++j)
                    {
                      const unsigned int start_index = row_starts[(i * vectorization_length + j) * n_components].first;
                      AssertIndexRange(start_index, dof_indices.size());
                      dof_indices_contiguous[dof_access_cell][i * vectorization_length + j] =
                        this->dof_indices.empty() ? 0 : this->dof_indices[start_index];
                    }
                }
              ...
              if (indices_are_interleaved_and_contiguous)
                {
                  ...
                  index_storage_variants[dof_access_cell][i] = IndexStorageVariants::interleaved_contiguous;
                  for (unsigned int j = 0; j < n_comp; ++j)
                    dof_indices_interleave_strides[2][i * vectorization_length + j] = n_comp;
                }
              else if (indices_are_contiguous)
                {
                  index_storage_variants[dof_access_cell][i] = IndexStorageVariants::contiguous;
                  for (unsigned int j = 0; j < n_comp; ++j)
                    dof_indices_interleave_strides[2][i * vectorization_length + j] = 1;
                }
              else if (ndofs > 0)
                {
                  ...
                  for (unsigned int j = 0; j < n_comp; ++j)
                    offsets[j] = dof_indices[j * ndofs + 1] - dof_indices[j * ndofs];
                  for (unsigned int k = 0; k < ndofs && indices_are_interleaved_and_mixed != 0; ++k)
                    for (unsigned int j = 0; j < n_comp; ++j)
                      // the first if case is to avoid negative offsets
                      // (invalid)
                      if (dof_indices[j * ndofs + 1] < dof_indices[j * ndofs] ||
                          dof_indices[j * ndofs + k] != dof_indices[j * ndofs] + k * offsets[j])
                        {
                  ...
                  if (indices_are_interleaved_and_mixed == 2)
                    {
                      for (unsigned int j = 0; j < n_comp; ++j)
                        dof_indices_interleave_strides[dof_access_cell][i * vectorization_length + j] = offsets[j];
                      for (unsigned int j = 0; j < n_comp; ++j)
                        dof_indices_contiguous[dof_access_cell][i * vectorization_length + j] = dof_indices[j * ndofs];
                      for (unsigned int j = 0; j < n_comp; ++j)
                      ...
                      if (indices_are_interleaved_and_mixed == 1 || n_comp != vectorization_length)
                  ...
                  else
                    {
                      ...
                      for (const unsigned int *indices = dof_indices; indices != dof_indices + ndofs; ++indices)
                        {
                          bool         is_sorted = true;
      ...
      auto fix_single_interleaved_indices = [&](const IndexStorageVariants variant) {
        if (index_kinds[static_cast<unsigned int>(IndexStorageVariants::interleaved_contiguous_mixed_strides)] > 0 &&
          ...
          for (unsigned int i = 0; i < irregular_cells.size(); ++i)
            {
              if (index_storage_variants[dof_access_cell][i] ==
                  ...
                  (variant != IndexStorageVariants::contiguous ||
                   dof_indices_interleave_strides[dof_access_cell][i * vectorization_length] == 1))
                {
                  index_storage_variants[dof_access_cell][i] = variant;
                  index_kinds[static_cast<unsigned int>(IndexStorageVariants::interleaved_contiguous_mixed_strides)]--;
      ...
      unsigned int n_interleaved =
        index_kinds[static_cast<unsigned int>(IndexStorageVariants::interleaved_contiguous)] +
        index_kinds[static_cast<unsigned int>(IndexStorageVariants::interleaved_contiguous_strided)] +
        index_kinds[static_cast<unsigned int>(IndexStorageVariants::interleaved_contiguous_mixed_strides)];

      // Step 2: fix single contiguous cell among others with interleaved
      // storage
      if (n_interleaved > 0 && index_kinds[static_cast<unsigned int>(IndexStorageVariants::contiguous)] > 0)
        for (unsigned int i = 0; i < irregular_cells.size(); ++i)
          if (index_storage_variants[dof_access_cell][i] == IndexStorageVariants::contiguous)
            {
              index_storage_variants[dof_access_cell][i] = IndexStorageVariants::interleaved_contiguous_mixed_strides;
              index_kinds[static_cast<unsigned int>(IndexStorageVariants::contiguous)]--;
              index_kinds[static_cast<unsigned int>(IndexStorageVariants::interleaved_contiguous_mixed_strides)]++;
            }

      // Step 3: Interleaved cells are left but also some non-contiguous ones
      // -> revert all to full storage
      if (n_interleaved > 0 && index_kinds[static_cast<unsigned int>(IndexStorageVariants::full)] +
                                   index_kinds[static_cast<unsigned int>(IndexStorageVariants::interleaved)] >
                                 0)
        for (unsigned int i = 0; i < irregular_cells.size(); ++i)
          if (index_storage_variants[dof_access_cell][i] > IndexStorageVariants::contiguous)
            {
      ...
      for (unsigned int i = 0; i < irregular_cells.size(); ++i)
        if (index_storage_variants[dof_access_cell][i] == IndexStorageVariants::interleaved)
          {
            if (n_vectorization_lanes_filled[dof_access_cell][i] < vectorization_length)
              {
                index_storage_variants[dof_access_cell][i] = IndexStorageVariants::full;
            ...
            AssertDimension(this->dof_indices.size(), this->dof_indices_interleaved.size());
            AssertDimension(n_vectorization_lanes_filled[dof_access_cell][i], vectorization_length);
            AssertIndexRange(row_starts[i * vectorization_length * n_components].first,
                             this->dof_indices_interleaved.size() + 1);
            AssertIndexRange(row_starts[i * vectorization_length * n_components].first + ndofs * vectorization_length,
                             this->dof_indices_interleaved.size() + 1);
            for (unsigned int k = 0; k < ndofs; ++k)
              {
                const unsigned int *my_dof_indices = dof_indices + k;
                const unsigned int *end            = interleaved_dof_indices + vectorization_length;
                for (; interleaved_dof_indices != end; ++interleaved_dof_indices, my_dof_indices += ndofs)
    ...
    void
    DoFInfo::compute_tight_partitioners(const Table<2, ShapeInfo<double>>        &shape_info,
                                        ...
                                        const bool                                use_vector_data_exchanger_full)
    {
      ...
      std::vector<types::global_dof_index> ghost_indices;
      {
        ...
        for (unsigned int cell = 0; cell < n_owned_cells; ++cell)
          {
            ...
            const unsigned int fe_index       = dofs_per_cell.size() == 1 ? 0 : cell_active_fe_index[cell / n_lanes];
            const unsigned int dofs_this_cell = dofs_per_cell[fe_index];

            for (unsigned int i = row_starts_plain_indices[cell]; i < row_starts_plain_indices[cell] + dofs_this_cell;
                 ++i)
              if (plain_dof_indices[i] >= part.locally_owned_size())
                ghost_indices.push_back(part.local_to_global(plain_dof_indices[i]));
          }
        ...
        const bool all_ghosts_equal =
          Utilities::MPI::min<int>(static_cast<int>(compressed_set.n_elements() == part.ghost_indices().n_elements()),
        ...
        if (all_ghosts_equal)
          ...
          {
            temp_0 =
              std::make_shared<Utilities::MPI::Partitioner>(part.locally_owned_range(), part.get_mpi_communicator());
        ...
        if (use_vector_data_exchanger_full == false)
          vector_exchanger_face_variants[0] =
            std::make_shared<MatrixFreeFunctions::VectorDataExchange::PartitionerWrapper>(temp_0);
        else
          vector_exchanger_face_variants[0] =
            std::make_shared<MatrixFreeFunctions::VectorDataExchange::Full>(temp_0, communicator_sm);
      }

      // construct a numbering of faces
      std::vector<FaceToCellTopology<1>> all_faces(inner_faces);
      all_faces.insert(all_faces.end(), ghosted_faces.begin(), ghosted_faces.end());
      Table<2, unsigned int> cell_and_face_to_faces((row_starts.size() - 1) / start_components.back(),
      ...
      for (unsigned int f = 0; f < all_faces.size(); ++f)
        {
          cell_and_face_to_faces(all_faces[f].cells_interior[0], all_faces[f].interior_face_no) = f;
          Assert(all_faces[f].cells_exterior[0] != numbers::invalid_unsigned_int, ExcInternalError());
          cell_and_face_to_faces(all_faces[f].cells_exterior[0], all_faces[f].exterior_face_no) = f;
        }

      // lambda function to detect objects on face pairs
      const auto loop_over_faces =
        [&](const std::function<void(const unsigned int, const unsigned int, const bool)> &fu) {
            ...
            for (unsigned int d = 0; d < cell_and_face_to_faces.size(1); ++d)
              {
                ...
                const unsigned int face_no = ext ? all_faces[f].exterior_face_no : all_faces[f].interior_face_no;
      ...
      const auto process_values =
        [&](std::shared_ptr<const Utilities::MPI::Partitioner> &vector_partitioner_values,
            const std::function<void(const std::function<void(const unsigned int, const unsigned int, const bool)> &)>
              &loop) {
          ...
          if (all_nodal_and_tensorial)
            for (unsigned int c = 0; c < n_base_elements; ++c)
              {
                const auto &si = shape_info(global_base_element_offset + c, 0).data.front();
                if (!si.nodal_at_cell_boundaries || (si.element_type == MatrixFreeFunctions::ElementType::tensor_none))
          ...
          if (all_nodal_and_tensorial == false)
          ...
          else
            {
              ...
              loop([&](const unsigned int cell_no, const unsigned int face_no, const bool flag) {
                const unsigned int index = dof_indices_contiguous[dof_access_cell][cell_no];
                if (flag || (index != numbers::invalid_unsigned_int && index >= part.locally_owned_size()))
                  {
                    const unsigned int stride = dof_indices_interleave_strides[dof_access_cell][cell_no];
                    unsigned int       i      = 0;
                    for (unsigned int e = 0; e < n_base_elements; ++e)
                      for (unsigned int c = 0; c < n_components[e]; ++c)
                        {
                          const ShapeInfo<double> &shape = shape_info(global_base_element_offset + e, 0);
                          for (unsigned int j = 0; j < shape.dofs_per_component_on_face; ++j)
                            ghost_indices.push_back(
                              part.local_to_global(index + i + shape.face_to_cell_index_nodal(face_no, j) * stride));
              ...
              has_noncontiguous_cell =
                Utilities::MPI::min<int>(static_cast<int>(has_noncontiguous_cell), part.get_mpi_communicator()) != 0;
              ...
              compressed_set.subtract_set(part.locally_owned_range());
              const bool all_ghosts_equal =
                Utilities::MPI::min<int>(static_cast<int>(compressed_set.n_elements() ==
              ...
              if (all_ghosts_equal || has_noncontiguous_cell)
              ...
              else
                {
                  vector_partitioner_values =
                    std::make_shared<Utilities::MPI::Partitioner>(part.locally_owned_range(),
                                                                  part.get_mpi_communicator());
                  const_cast<Utilities::MPI::Partitioner *>(vector_partitioner_values.get())
                    ->set_ghost_indices(compressed_set, part.ghost_indices());
                }
            }
        };
      ...
      const auto process_gradients =
        [&](const std::shared_ptr<const Utilities::MPI::Partitioner> &vector_partitoner_values,
            std::shared_ptr<const Utilities::MPI::Partitioner>       &vector_partitioner_gradients,
            const std::function<void(const std::function<void(const unsigned int, const unsigned int, const bool)> &)>
              &loop) {
          ...
          if (all_hermite)
            for (unsigned int c = 0; c < n_base_elements; ++c)
              if (shape_info(global_base_element_offset + c, 0).element_type !=
                ...
                all_hermite = false;
          if (all_hermite == false || vector_partitoner_values.get() == vector_partitioner.get())
                      ...
                      for (unsigned int c = 0; c < n_components[e]; ++c)
                        {
                          ...
                          for (unsigned int j = 0; j < 2 * shape.dofs_per_component_on_face; ++j)
                            ghost_indices.push_back(
                              part.local_to_global(index + i + shape.face_to_cell_index_hermite(face_no, j) * stride));
                          i += shape.dofs_per_component_on_cell * stride;
                        }
                    AssertDimension(i, dofs_per_cell[0] * stride);
                  }
              ...
              if (all_ghosts_equal)
              ...
              else
                {
                  vector_partitioner_gradients =
                    std::make_shared<Utilities::MPI::Partitioner>(part.locally_owned_range(),
                                                                  part.get_mpi_communicator());
                  const_cast<Utilities::MPI::Partitioner *>(vector_partitioner_gradients.get())
                    ->set_ghost_indices(compressed_set, part.ghost_indices());
                }
            }
        };

      std::shared_ptr<const Utilities::MPI::Partitioner> temp_1, temp_2, temp_3, temp_4;
      ...
      if (use_vector_data_exchanger_full == false)
        {
          vector_exchanger_face_variants[1] =
            std::make_shared<MatrixFreeFunctions::VectorDataExchange::PartitionerWrapper>(temp_1);
          vector_exchanger_face_variants[2] =
            std::make_shared<MatrixFreeFunctions::VectorDataExchange::PartitionerWrapper>(temp_2);
          vector_exchanger_face_variants[3] =
            std::make_shared<MatrixFreeFunctions::VectorDataExchange::PartitionerWrapper>(temp_3);
          vector_exchanger_face_variants[4] =
            std::make_shared<MatrixFreeFunctions::VectorDataExchange::PartitionerWrapper>(temp_4);
        }
      else
        {
          vector_exchanger_face_variants[1] =
            std::make_shared<MatrixFreeFunctions::VectorDataExchange::Full>(temp_1, communicator_sm);
          vector_exchanger_face_variants[2] =
            std::make_shared<MatrixFreeFunctions::VectorDataExchange::Full>(temp_2, communicator_sm);
          vector_exchanger_face_variants[3] =
            std::make_shared<MatrixFreeFunctions::VectorDataExchange::Full>(temp_3, communicator_sm);
          vector_exchanger_face_variants[4] =
            std::make_shared<MatrixFreeFunctions::VectorDataExchange::Full>(temp_4, communicator_sm);
        }
    }
      ...
      std::array<std::vector<std::pair<unsigned int, unsigned int>>, 3> &cell_indices_contiguous_sm)
    {
      ...
      for (unsigned int i = 0; i < 3; ++i)
        {
          dof_indices_contiguous_sm[i].resize(cell_indices_contiguous_sm[i].size());

          for (unsigned int j = 0; j < cell_indices_contiguous_sm[i].size(); ++j)
            if (cell_indices_contiguous_sm[i][j].first != numbers::invalid_unsigned_int)
              dof_indices_contiguous_sm[i][j] = {cell_indices_contiguous_sm[i][j].first,
                                                 cell_indices_contiguous_sm[i][j].second * dofs_per_cell[0]};
            else
              dof_indices_contiguous_sm[i][j] = {numbers::invalid_unsigned_int, numbers::invalid_unsigned_int};
        }
    }
        ...
        for (unsigned int block = begin; block < end; ++block)
          {
            ...
            std::vector<unsigned int>::const_iterator end_unique = std::unique(scratch.begin(), scratch.end());
            ...
            while (it != end_unique)
              {
                ...
                const unsigned int          next_bucket = (*it / bucket_size_threading + 1) * bucket_size_threading;
                std::lock_guard<std::mutex> lock(mutexes[*it / bucket_size_threading]);
        ...
        for (unsigned int block = begin; block < end; ++block)
          {
            ...
            const unsigned int *it = dof_info.dof_indices.data() + dof_info.row_starts[block * n_components].first,
                               *end_cell =
                                 dof_info.dof_indices.data() + dof_info.row_starts[(block + 1) * n_components].first;
            for (; it != end_cell; ++it)
              {
    ...
    {
      unsigned int n_rows = (vector_partitioner->local_range().second - vector_partitioner->local_range().first) +
                            vector_partitioner->ghost_indices().n_elements();
      ...
      std::vector<std::mutex>   mutexes(n_rows / internal::bucket_size_threading + 1);
      dealii::parallel::apply_to_subranges(
        0,
        task_info.n_active_cells,
        [this, &mutexes, &row_lengths](const unsigned int begin, const unsigned int end) {
          internal::compute_row_lengths(begin, end, *this, mutexes, row_lengths);
        },
        20);
      ...
      dealii::parallel::apply_to_subranges(
        0,
        task_info.n_active_cells,
        [this, &row_lengths, &mutexes, &connectivity_dof](const unsigned int begin, const unsigned int end) {
          internal::fill_connectivity_dofs(begin, end, *this, row_lengths, mutexes, connectivity_dof);
        },
        20);
      ...
      dealii::parallel::apply_to_subranges(
        0,
        task_info.n_active_cells,
        [this, &reverse_numbering, &connectivity_dof, &connectivity](const unsigned int begin, const unsigned int end) {
          internal::fill_connectivity(begin, end, *this, reverse_numbering, connectivity_dof, connectivity);
        },
        20);
    }
    ...
    DoFInfo::compute_dof_renumbering(std::vector<types::global_dof_index> &renumbering)
    {
      ...
      const unsigned int      n_components   = start_components.back();
      const unsigned int      n_cell_batches = n_vectorization_lanes_filled[dof_access_cell].size();
      Assert(n_cell_batches <= (row_starts.size() - 1) / vectorization_length / n_components, ExcInternalError());
              ...
              row_starts[(cell_no + 1) * n_components * vectorization_length].second)
            {
              const unsigned int ndofs =
                dofs_per_cell.size() == 1 ?
                  dofs_per_cell[0] :
                  (dofs_per_cell[cell_active_fe_index.size() > 0 ? cell_active_fe_index[cell_no] : 0]);
              const unsigned int *dof_ind =
                dof_indices.data() + row_starts[cell_no * n_components * vectorization_length].first;
    ...
    DoFInfo::memory_consumption() const
    {
      ...
      memory += MemoryConsumption::memory_consumption(hanging_node_constraint_masks);
...
namespace internal
{
  ...
  {
    template void
    DoFInfo::read_dof_indices<double>(const std::vector<types::global_dof_index> &,
    ...
    template void
    DoFInfo::read_dof_indices<float>(const std::vector<types::global_dof_index> &,
                                     const std::vector<types::global_dof_index> &,
                                     const bool,
    ...
    template bool
    DoFInfo::process_hanging_node_constraints<1>(const HangingNodes<1> &,
                                                 const std::vector<std::vector<unsigned int>> &,
                                                 const unsigned int,
                                                 const TriaIterator<DoFCellAccessor<1, 1, false>> &,
                                                 std::vector<types::global_dof_index> &);
    template bool
    DoFInfo::process_hanging_node_constraints<2>(const HangingNodes<2> &,
                                                 const std::vector<std::vector<unsigned int>> &,
                                                 const unsigned int,
                                                 const TriaIterator<DoFCellAccessor<2, 2, false>> &,
                                                 std::vector<types::global_dof_index> &);
    template bool
    DoFInfo::process_hanging_node_constraints<3>(const HangingNodes<3> &,
                                                 const std::vector<std::vector<unsigned int>> &,
                                                 const unsigned int,
    ...
    template void
    DoFInfo::compute_face_index_compression<1>(const std::vector<FaceToCellTopology<1>> &);
    template void
    DoFInfo::compute_face_index_compression<2>(const std::vector<FaceToCellTopology<2>> &);
    template void
    DoFInfo::compute_face_index_compression<4>(const std::vector<FaceToCellTopology<4>> &);
    template void
    DoFInfo::compute_face_index_compression<8>(const std::vector<FaceToCellTopology<8>> &);
    template void
    DoFInfo::compute_face_index_compression<16>(const std::vector<FaceToCellTopology<16>> &);

    template void
    DoFInfo::compute_vector_zero_access_pattern<1>(const TaskInfo &, const std::vector<FaceToCellTopology<1>> &);
    template void
    DoFInfo::compute_vector_zero_access_pattern<2>(const TaskInfo &, const std::vector<FaceToCellTopology<2>> &);
    template void
    DoFInfo::compute_vector_zero_access_pattern<4>(const TaskInfo &, const std::vector<FaceToCellTopology<4>> &);
    template void
    DoFInfo::compute_vector_zero_access_pattern<8>(const TaskInfo &, const std::vector<FaceToCellTopology<8>> &);
    template void
    DoFInfo::compute_vector_zero_access_pattern<16>(const TaskInfo &, const std::vector<FaceToCellTopology<16>> &);

    template void
    DoFInfo::print_memory_consumption<std::ostream>(std::ostream &, const TaskInfo &) const;
    template void
    DoFInfo::print_memory_consumption<ConditionalOStream>(ConditionalOStream &, const TaskInfo &) const;

    template void
    DoFInfo::print<double>(const std::vector<double> &, const std::vector<unsigned int> &, std::ostream &) const;

    template void
    DoFInfo::print<float>(const std::vector<float> &, const std::vector<unsigned int> &, std::ostream &) const;
  } // namespace MatrixFreeFunctions
} // namespace internal

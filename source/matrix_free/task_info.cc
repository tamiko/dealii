  ...
  namespace MatrixFreeFunctions
  {
          ...
          for (unsigned int j = 0; j < evens; ++j)
            {
              worker[j] = new (root->allocate_child())
                CellWork(function, task_info.partition_row_index[partition] + 2 * j, task_info, false);
              if (j > 0)
                {
              ...
              if (j < evens - 1)
                {
                  blocked_worker[j] = new (worker[j]->allocate_child())
                    CellWork(function, task_info.partition_row_index[partition] + 2 * j + 1, task_info, true);
                }
              else
                {
                  if (odds == evens)
                    {
                      worker[evens] = new (worker[j]->allocate_child())
                        CellWork(function, task_info.partition_row_index[partition] + 2 * j + 1, task_info, false);
    ...
    namespace color
    {
      ...
      public:
        ...
        {
          const unsigned int start_index = task_info.cell_partition_data[partition] + task_info.block_size * r.begin();
          const unsigned int end_index   = std::min(start_index + task_info.block_size * (r.end() - r.begin()),
      ...
      public:
        ...
        execute() override
        {
          ...
          parallel_for(tbb::blocked_range<unsigned int>(0, n_chunks, 1), CellWork(worker, task_info, partition));
          if (is_blocked == true)
            tbb::empty_task::spawn(*dummy);
          return nullptr;
        }
    ...
    TaskInfo::loop(MFWorkerInterface &funct) const
    {
      ...
      if (scheme != none)
        ...
        funct.cell_loop_pre_range(partition_row_index[partition_row_index.size() - 2]);
          ...
          if (scheme == partition_partition && evens > 0)
            {
              ...
              MPICommunication *worker_compr = new (root->allocate_child()) MPICommunication(funct, true);
              worker_compr->set_ref_count(1);
              for (unsigned int j = 0; j < evens; ++j)
                {
                  if (j > 0)
                    {
                      worker[j] = new (root->allocate_child()) partition::PartitionWork(funct, 2 * j, *this, false);
                      worker[j]->set_ref_count(2);
                      blocked_worker[j - 1]->dummy = new (worker[j]->allocate_child()) tbb::empty_task;
                      tbb::task::spawn(*blocked_worker[j - 1]);
                    }
                    ...
                    {
                      worker[j] =
                        new (worker_compr->allocate_child()) partition::PartitionWork(funct, 2 * j, *this, false);
                      worker[j]->set_ref_count(2);
                      MPICommunication *worker_dist = new (worker[j]->allocate_child()) MPICommunication(funct, false);
                      tbb::task::spawn(*worker_dist);
                    }
                  if (j < evens - 1)
                    {
                      blocked_worker[j] =
                        new (worker[j]->allocate_child()) partition::PartitionWork(funct, 2 * j + 1, *this, true);
                    }
                  else
                    {
                      if (odds == evens)
                        {
                          worker[evens] =
                            new (worker[j]->allocate_child()) partition::PartitionWork(funct, 2 * j + 1, *this, false);
                          tbb::task::spawn(*worker[evens]);
                        }
                      else
                        {
                  ...
                  for (unsigned int part = 0; part < partition_row_index.size() - 1; part++)
                    {
                      if (part == 0)
                        worker[worker_index] =
                          new (worker_compr->allocate_child()) color::PartitionWork(funct, slice_index, *this, false);
                      else
                        worker[worker_index] =
                          new (root->allocate_child()) color::PartitionWork(funct, slice_index, *this, false);
                      slice_index++;
                      ...
                      if (part < partition_row_index.size() - 1)
                        ...
                        {
                          tbb::empty_task *final = new (worker[worker_index - 1]->allocate_child()) tbb::empty_task;
      ...
      if (scheme != none)
        ...
        funct.cell_loop_post_range(partition_row_index[partition_row_index.size() - 2]);
    }
    ...
    {
      Utilities::MPI::MinMaxAvg memory_c = Utilities::MPI::min_max_avg(1e-6 * data_length, communicator);
      if (n_procs < 2)
        out << memory_c.min;
      else
    ...
    void
    TaskInfo::make_boundary_cells_divisible(std::vector<unsigned int> &boundary_cells)
    {
      ...
      if (fillup_needed > 0 && boundary_cells.size() < n_active_cells)
        {
          ...
          while (fillup_needed > 0 && (new_boundary_cells.empty() || new_boundary_cells.back() < n_active_cells - 1))
      ...
      Assert(boundary_cells.size() % vectorization_length == 0 || boundary_cells.size() == n_active_cells,
             ExcInternalError());
    }
    ...
    void
    TaskInfo::create_blocks_serial(const std::vector<unsigned int> &cells_with_comm,
                                   ...
                                   std::vector<unsigned char>      &incompletely_filled_vectorization)
    {
        ...
        for (unsigned int j = n_categories - 1; j > 0; --j)
          {
            ...
            while ((renumbering_category[j].size() % n_lanes) != 0u)
              {
                while (((renumbering_category[j].size() % n_lanes) != 0u) && !renumbering_category[lower_index].empty())
                  {
      ...
      const unsigned int        n_cells_per_parent = std::count(parent_relation.begin(), parent_relation.end(), 0);
      ...
      for (unsigned int j = 0; j < n_categories; ++j)
        {
          ...
          for (const unsigned int cell : renumbering_category[j])
            if (parent_relation.empty() || parent_relation[cell] == numbers::invalid_unsigned_int)
          ...
          temporary_numbering.insert(temporary_numbering.end(), other_cells.begin(), other_cells.end());
      ...
      std::vector<bool>         batch_with_comm(temporary_numbering.size() / n_lanes, false);
      ...
      for (unsigned int i = 0; i < temporary_numbering.size(); i += n_lanes)
        {
          ...
          const unsigned int category_hp =
            categories_are_hp ?
              std::upper_bound(category_size.begin(), category_size.end(), i) - category_size.begin() :
              0;
      ...
      if (n_procs == 1)
      ...
      else
        {
          ...
          batch_order.insert(batch_order.begin() + comm_begin, batch_order_comm.begin(), batch_order_comm.end());
          ...
          for (unsigned int i = 0; i < n_ghost_cells; ++i)
            {
              const unsigned int index = std::lower_bound(used_categories_vector.begin(),
                                         ...
                                         used_categories_vector.begin();
              AssertIndexRange(index, used_categories_vector.size());
              tight_category_map_ghost.emplace_back(std::array<unsigned int, 2>{{index, i}});
          ...
          for (unsigned int i = 0; i < counters.size(); ++i)
            if (counters[i] % n_lanes != 0)
              for (unsigned int j = counters[i] % n_lanes; j < n_lanes; ++j)
                tight_category_map_ghost.emplace_back(std::array<unsigned int, 2>{{i, numbers::invalid_unsigned_int}});
      ...
      const unsigned int n_ghost_batches =
        ((tight_category_map_ghost.empty() ? n_ghost_cells : tight_category_map_ghost.size()) + n_lanes - 1) / n_lanes;
      ...
      for (unsigned int block = 0; block < blocks.size() - 1; ++block)
        {
          const unsigned int grain_size = std::max((2048U / dofs_per_cell) / 8 * 4, 2U);
          for (unsigned int k = blocks[block]; k < blocks[block + 1]; k += grain_size)
            cell_partition_data.push_back(std::min(k + grain_size, blocks[block + 1]));
          ...
          for (unsigned int k = blocks[block]; k < blocks[block + 1]; ++k)
            {
              ...
              unsigned int       j   = 0;
              for (; j < n_lanes && temporary_numbering[pos + j] != numbers::invalid_unsigned_int; ++j)
      ...
      if (tight_category_map_ghost.empty())
        ...
        {
          for (unsigned int k = 0, ptr = 0; k < n_ghost_batches; ++k, ptr += n_lanes)
            {
              unsigned int j = 0;

              for (; j < n_lanes && (ptr + j < tight_category_map_ghost.size()) &&
                     (tight_category_map_ghost[ptr + j][1] != numbers::invalid_unsigned_int);
                   ++j)
                renumbering[counter++] = n_active_cells + tight_category_map_ghost[ptr + j][1];
    ...
    void
    TaskInfo::initial_setup_blocks_tasks(const std::vector<unsigned int> &boundary_cells,
                                         std::vector<unsigned int>       &renumbering,
                                         std::vector<unsigned char>      &incompletely_filled_vectorization)
    {
      const unsigned int n_cell_batches = (n_active_cells + vectorization_length - 1) / vectorization_length;
      const unsigned int n_ghost_slots  = (n_ghost_cells + vectorization_length - 1) / vectorization_length;
      ...
      if (n_cell_batches * vectorization_length > n_active_cells)
        incompletely_filled_vectorization[n_cell_batches - 1] =
          vectorization_length - (n_cell_batches * vectorization_length - n_active_cells);
      ...
      if (n_procs > 1)
        {
          ...
          cell_partition_data.push_back((n_cell_batches - n_macro_boundary_cells) / 2);
                                                ...
                                                const bool)
    {
      ...
      make_partitioning(connectivity, cluster_size, cell_partition, partition_list, partition_size, partition);

      // Color the cells within each partition
      make_coloring_within_partitions_pre_blocked(
        connectivity, partition, cell_partition, partition_list, partition_size, partition_color_list);
      ...
      for (unsigned int block = 0; block < n_blocks; ++block)
        {
          ...
          for (unsigned int mcell = mcell_start; mcell < std::min(mcell_start + block_size, n_cell_batches); ++mcell)
            {
              unsigned int n_comp = (irregular_cells[mcell] > 0) ? irregular_cells[mcell] : vectorization_length;
      ...
      for (unsigned int block = 0; block < n_blocks; ++block)
        {
          ...
          for (unsigned int cell = block_start[present_block]; cell < block_start[present_block + 1]; ++cell)
            renumbering[counter++] = partition_list[cell];
          unsigned int this_block_size = (present_block == n_blocks - 1) ? block_size_last : block_size;
                                ...
                                const bool                       hp_bool)
    {
      ...
      if (scheme == partition_color || scheme == color)
        make_partitioning(connectivity_blocks, cluster_size, cell_partition, partition_list, partition_size, partition);
      else
        make_partitioning(connectivity, cluster_size, cell_partition, partition_list, partition_size, partition);
      ...
      if (scheme == partition_partition)
        ...
        {
          make_coloring_within_partitions_pre_blocked(
            connectivity_blocks, partition, cell_partition, partition_list, partition_size, partition_2layers_list);
        }
      ...
      if (scheme == partition_partition) // blocking_connectivity == false
      ...
      else
        {
          ...
          for (unsigned int block = 0; block < n_blocks; ++block)
            {
              block_start[block + 1] = block_start[block];
              for (unsigned int mcell = mcell_start; mcell < std::min(mcell_start + block_size, n_cell_batches);
                   ++mcell)
                {
                  unsigned int n_comp = (irregular_cells[mcell] > 0) ? irregular_cells[mcell] : vectorization_length;
                  block_start[block + 1] += n_comp;
    ...
    void
    TaskInfo::make_thread_graph_partition_partition(const std::vector<unsigned int> &cell_active_fe_index,
                                                    DynamicSparsityPattern          &connectivity,
                                                    std::vector<unsigned int>       &renumbering,
                                                    std::vector<unsigned char>      &irregular_cells,
                                                    const bool                       hp_bool)
                                                ...
                                                DynamicSparsityPattern           &connectivity_blocks) const
    {
      ...
      for (unsigned int i = 0, mcell = 0; i < n_blocks; ++i)
        {
          for (unsigned int c = 0; c < block_size && mcell < *(cell_partition_data.end() - 2); ++c, ++mcell)
            {
              unsigned int ncomp = (irregular_cells[mcell] > 0) ? irregular_cells[mcell] : vectorization_length;
      ...
      for (unsigned int i = 0; i < cell_blocks.size(); ++i)
        for (unsigned int col = 0; col < cell_blocks[i].size(); ++col)
          {
            for (DynamicSparsityPattern::iterator it = connectivity_cells.begin(cell_blocks[i][col]);
    ...
    void
    TaskInfo::make_partitioning_within_partitions_post_blocked(const DynamicSparsityPattern    &connectivity,
    ...
    {
      const unsigned int n_cell_batches = *(cell_partition_data.end() - 2);
      const unsigned int n_ghost_slots  = *(cell_partition_data.end() - 1) - n_cell_batches;
                    ...
                    for (const unsigned int neighbor : neighbor_list)
                      {
                        ...
                        Assert(cell_partition_l2[neighbor] == partition_l2 - 1, ExcInternalError());
                        ...
                        for (; neighbor_it != end_it; ++neighbor_it)
                          {
                            if (cell_partition[neighbor_it->column()] == part &&
                                cell_partition_l2[neighbor_it->column()] == numbers::invalid_unsigned_int)
                              {
                ...
                if (partition_counter > 0)
                  {
                    int index_before = neighbor_neighbor_list.size(), index = index_before;
                    {
                      ...
                      missing_macros = 0;
                      std::vector<unsigned int>              remaining_per_cell_batch(max_fe_index + 1);
                      ...
                      if (hp_bool == true)
                        {
                          ...
                          for (cell = counter - partition_counter; cell < counter; ++cell)
                            {
                              renumbering_fe_index[cell_active_fe_index.empty() ?
                                                     0 :
                          ...
                          for (unsigned int j = 0; j < max_fe_index + 1; ++j)
                            {
                              remaining_per_cell_batch[j] = renumbering_fe_index[j].size() % vectorization_length;
                              if (remaining_per_cell_batch[j] != 0)
                                filled = false;
                              missing_macros +=
                                ((renumbering_fe_index[j].size() + vectorization_length - 1) / vectorization_length);
                            }
                        }
                      else
                        {
                          remaining_per_cell_batch.resize(1);
                          remaining_per_cell_batch[0] = partition_counter % vectorization_length;
                          ...
                          for (; neighbor != end; ++neighbor)
                            {
                              if (cell_partition[neighbor->column()] == part &&
                                  cell_partition_l2[neighbor->column()] == numbers::invalid_unsigned_int)
                                {
                                  unsigned int this_index = 0;
                                  if (hp_bool == true)
                                    this_index =
                                      cell_active_fe_index.empty() ? 0 : cell_active_fe_index[neighbor->column()];
                                  ...
                                  if (missing_macros > 0 || remaining_per_cell_batch[this_index] > 0)
                                    {
                                      cell_partition_l2[neighbor->column()] = partition_l2;
                                      neighbor_neighbor_list.push_back(neighbor->column());
                                      if (hp_bool == true)
                                        renumbering_fe_index[this_index].push_back(neighbor->column());
                                      partition_partition_list[counter] = neighbor->column();
                                      counter++;
                                      partition_counter++;
                                      if (remaining_per_cell_batch[this_index] == 0 && missing_macros > 0)
                                        missing_macros--;
                                      remaining_per_cell_batch[this_index]++;
                                      if (remaining_per_cell_batch[this_index] == vectorization_length)
                                        {
                                      ...
                                      if (missing_macros == 0)
                                        {
                                          filled = true;
                                          for (unsigned int fe_ind = 0; fe_ind < max_fe_index + 1; ++fe_ind)
                      ...
                      if (hp_bool == true)
                      ...
                      else
                        {
                          ...
                          if (partition_counter % vectorization_length != 0)
                            {
                              irregular_cells[n_cell_batches_before] = partition_counter % vectorization_length;
                              n_cell_batches_before++;
                            }
                        }
                    }
    ...
    void
    TaskInfo::make_coloring_within_partitions_pre_blocked(const DynamicSparsityPattern    &connectivity,
                                                          const unsigned int               partition,
                                                          const std::vector<unsigned int> &cell_partition,
                                                          const std::vector<unsigned int> &partition_list,
                                                          const std::vector<unsigned int> &partition_size,
          ...
          for (unsigned int k = partition_size[part]; k < partition_size[part + 1]; k++)
            {
              ...
              for (unsigned int j = 0; j <= n_neighbors; ++j)
                color_finder[j] = true;
              DynamicSparsityPattern::iterator neighbor = connectivity.begin(cell), end = connectivity.end(cell);
              for (; neighbor != end; ++neighbor)
                {
                  // Mark the color that a neighbor within the partition has
                  // as taken
                  if (cell_partition[neighbor->column()] == part && cell_color[neighbor->column()] <= n_neighbors)
...

    {
      ...
      unsigned int start_nonboundary =
        cell_partition_data.size() == 5 ? vectorization_length * (cell_partition_data[2] - cell_partition_data[1]) : 0;
    ...
    TaskInfo::update_task_info(const unsigned int partition)
    {
      ...
      for (unsigned int part = 0; part < partition; ++part)
        {
          partition_evens[part] = (partition_row_index[part + 1] - partition_row_index[part] + 1) / 2;
          ...
          partition_n_workers[part] = partition_evens[part] + partition_odds[part] - partition_n_blocked_workers[part];
        }
    }
  } // namespace MatrixFreeFunctions
} // namespace internal
...
template void
internal::MatrixFreeFunctions::TaskInfo::print_memory_statistics<std::ostream>(std::ostream &, const std::size_t) const;
template void
internal::MatrixFreeFunctions::TaskInfo::print_memory_statistics<ConditionalOStream>(ConditionalOStream &,

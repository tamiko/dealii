      ...
      for (unsigned int level = 0; level < n_levels; ++level)
        {
          ...
          const IndexSet   &owned_level_dofs = dof_handler.locally_owned_mg_dofs(level);
          ...
          for (const auto &level_cell : dof_handler.active_cell_iterators_on_level(level))
            {
              ...
              unrolled_copy_indices.resize(owned_dofs.n_elements(), numbers::invalid_dof_index);
              ...
              for (unsigned int i = 0; i < dofs_per_cell; ++i)
                {
                  ...
                  bool global_mine = global_index_in_set != numbers::invalid_dof_index;
                  bool level_mine  = owned_level_dofs.is_element(level_dof_indices[i]);
                  ...
                  if (global_mine && level_mine)
                    {
                      ...
                      unrolled_copy_indices[global_index_in_set] = level_dof_indices[i];
                    }
                  else if (global_mine && dof_touched[global_index_in_set] == false)
                    {
                      copy_indices_global_mine[level].emplace_back(global_dof_indices[i], level_dof_indices[i]);

                      // send this to the owner of the level_dof:
                      send_data_temp.emplace_back(level, global_dof_indices[i], level_dof_indices[i]);
          ...
          if (!unrolled_copy_indices.empty())
            {
              ...
              for (unsigned int i = 0; i < unrolled_copy_indices.size(); ++i)
                if (unrolled_copy_indices[i] != numbers::invalid_dof_index)
                  copy_indices[level].emplace_back(owned_dofs.nth_index_in_set(i), unrolled_copy_indices[i]);
            }
        }

      const dealii::parallel::TriangulationBase<dim, spacedim> *tria =
        (dynamic_cast<const dealii::parallel::TriangulationBase<dim, spacedim> *>(&dof_handler.get_triangulation()));
      AssertThrow(send_data_temp.empty() || tria != nullptr,
                  ExcMessage("We should only be sending information with a parallel Triangulation!"));

#ifdef DEAL_II_WITH_MPI
      if (tria && Utilities::MPI::sum(send_data_temp.size(), tria->get_communicator()) > 0)
        {
          const std::set<types::subdomain_id> &neighbors = tria->level_ghost_owners();
          std::map<int, std::vector<DoFPair>>  send_data;

          std::sort(send_data_temp.begin(), send_data_temp.end(), [](const DoFPair &lhs, const DoFPair &rhs) {
          ...
          send_data_temp.erase(std::unique(send_data_temp.begin(),
                                           send_data_temp.end(),
          ...
          for (unsigned int level = 0; level < n_levels; ++level)
            {
              ...
              is_ghost.add_indices(level_dof_indices.begin(), level_dof_indices.end());

              AssertThrow(level_dof_indices.size() == is_ghost.n_elements(), ExcMessage("Size does not match!"));

              const auto index_owner =
                Utilities::MPI::compute_index_owner(owned_level_dofs, is_ghost, tria->get_communicator());

              AssertThrow(level_dof_indices.size() == index_owner.size(), ExcMessage("Size does not match!"));

              for (unsigned int i = 0; i < index_owner.size(); ++i)
                send_data[index_owner[i]].emplace_back(level, global_dof_indices[i], level_dof_indices[i]);
            }
          ...
          const int mpi_tag = Utilities::MPI::internal::Tags::mg_transfer_fill_copy_indices;
            ...
            for (unsigned int counter = 0; counter < neighbors.size(); ++counter)
              {
                ...
                if (len == 0)
                  {
                    ierr = MPI_Recv(
                      nullptr, 0, MPI_BYTE, status.MPI_SOURCE, status.MPI_TAG, tria->get_communicator(), &status);
                    AssertThrowMPI(ierr);
                    continue;
                  }
                ...
                ierr =
                  MPI_Recv(ptr, len, MPI_BYTE, status.MPI_SOURCE, status.MPI_TAG, tria->get_communicator(), &status);
                AssertThrowMPI(ierr);

                for (const auto &dof_pair : receive_buffer)
                  {
                    copy_indices_level_mine[dof_pair.level].emplace_back(dof_pair.global_dof_index,
                                                                         dof_pair.level_dof_index);
                  }
              }
          }
...
#  endif
    ...
    void
    reinit_level_partitioner(const IndexSet                                           &locally_owned,
                             ...
                             Table<2, unsigned int>                                   &copy_indices_global_mine)
    {
      ...
      ghosted_dofs.add_indices(ghosted_level_dofs.begin(), ghosted_level_dofs.end());
      ghosted_dofs.compress();

      // Add possible ghosts from the previous content in the vector
      if (target_partitioner.get() != nullptr && target_partitioner->size() == locally_owned.size())
        {
      ...
      const int ghosts_locally_contained = (external_partitioner.get() != nullptr &&
                                            (external_partitioner->ghost_indices() & ghosted_dofs) == ghosted_dofs) ?
                                             1 :
                                             0;
      if (external_partitioner.get() != nullptr && Utilities::MPI::min(ghosts_locally_contained, communicator) == 1)
        {
          ...
          if (target_partitioner.get() != nullptr && target_partitioner->size() == locally_owned.size())
            for (unsigned int i = 0; i < copy_indices_global_mine.n_cols(); ++i)
              copy_indices_global_mine(1, i) = external_partitioner->global_to_local(
                target_partitioner->local_to_global(copy_indices_global_mine(1, i)));
          ...
          if (target_partitioner.get() != nullptr && target_partitioner->size() == locally_owned.size())
            for (unsigned int i = 0; i < copy_indices_global_mine.n_cols(); ++i)
              copy_indices_global_mine(1, i) =
                ...
                ghosted_dofs.index_within_set(target_partitioner->local_to_global(copy_indices_global_mine(1, i)));
          target_partitioner = std::make_shared<Utilities::MPI::Partitioner>(locally_owned, ghosted_dofs, communicator);
        }
    }
            ...
            for (unsigned int i = 0; i < (fe_degree + 1); ++i, ++m)
              {
                const unsigned int index =
                  c * n_scalar_cell_dofs + k * n_child_dofs_1d * n_child_dofs_1d + j * n_child_dofs_1d + i;
    ...
    void
    setup_element_info(ElementInfo<Number> &elem_info, const FiniteElement<1> &fe, const DoFHandler<dim> &dof_handler)
    {
      ...
      AssertDimension(Utilities::fixed_power<dim>(fe.n_dofs_per_cell()) * elem_info.n_components,
      ...
      std::vector<unsigned int> renumbering(fe.n_dofs_per_cell());
      {
        ...
        for (unsigned int i = 0; i < fe.n_dofs_per_line(); ++i)
          renumbering[i + fe.n_dofs_per_vertex()] = GeometryInfo<1>::vertices_per_cell * fe.n_dofs_per_vertex() + i;
      ...
      Assert(fe.n_dofs_per_vertex() == 0 || fe.n_dofs_per_vertex() == 1, ExcNotImplemented());
      const unsigned int shift = fe.n_dofs_per_cell() - fe.n_dofs_per_vertex();
      const unsigned int n_child_dofs_1d =
        (fe.n_dofs_per_vertex() > 0 ? (2 * fe.n_dofs_per_cell() - 1) : (2 * fe.n_dofs_per_cell()));
      ...
      elem_info.prolongation_matrix_1d.resize(fe.n_dofs_per_cell() * n_child_dofs_1d);
    ...
    setup_transfer(const DoFHandler<dim>                                                 &dof_handler,
                   ...
                   MGLevelObject<std::shared_ptr<const Utilities::MPI::Partitioner>>     &target_partitioners)
    {
      ...
      std::vector<types::global_dof_index> local_dof_indices(dof_handler.get_fe().n_dofs_per_cell());
      dirichlet_indices.resize(n_levels - 1);

      AssertDimension(target_partitioners.max_level(), n_levels - 1);
      Assert(external_partitioners.empty() || external_partitioners.size() == n_levels,
      ...
      for (unsigned int level = n_levels - 1; level > 0; --level)
        {
          ...
          typename DoFHandler<dim>::cell_iterator cell, endc = dof_handler.end(level - 1);
          for (cell = dof_handler.begin(level - 1); cell != endc; ++cell)
            {
              ...
              bool consider_cell = (tria.locally_owned_subdomain() == numbers::invalid_subdomain_id ||
              ...
              next_indices.resize(start_index + elem_info.n_child_cell_dofs, numbers::invalid_dof_index);
              for (unsigned int c = 0; c < GeometryInfo<dim>::max_children_per_cell; ++c)
                {
                  if (cell_is_remote && cell->child(c)->level_subdomain_id() != tria.locally_owned_subdomain())
                    continue;
                  cell->child(c)->get_mg_dof_indices(local_dof_indices);

                  resolve_identity_constraints(mg_constrained_dofs, level, local_dof_indices);
                  ...
                  if (cell->child(c)->has_children() &&
                       ...
                       cell->child(c)->level_subdomain_id() == tria.locally_owned_subdomain()))
                    {
                      ...
                      AssertIndexRange(dof_handler.get_fe().n_dofs_per_cell(), static_cast<unsigned short>(-1));
              ...
              if (!cell_is_remote)
                {
                  AssertIndexRange(static_cast<unsigned int>(cell->index()), coarse_level_indices[level - 1].size());
              ...
              if (level == 1 && !cell_is_remote)
                {
                  ...
                  resolve_identity_constraints(mg_constrained_dofs, level - 1, local_dof_indices);

                  const IndexSet &owned_level_dofs_l0 = dof_handler.locally_owned_mg_dofs(0);
                  for (const auto local_dof_index : local_dof_indices)
          ...
          parent_child_connect[level - 1].resize(counter,
          ...
          if (level < n_levels - 1)
            for (std::vector<std::pair<unsigned int, unsigned int>>::iterator i = parent_child_connect[level].begin();
          ...
          reinit_level_partitioner(dof_handler.locally_owned_mg_dofs(level),
                                   ghosted_level_dofs,
                                   external_partitioners.empty() ? nullptr : external_partitioners[level],
                                   tria.get_communicator(),
          ...
          if (level == 1)
            {
              ...
              reinit_level_partitioner(dof_handler.locally_owned_mg_dofs(0),
                                       ...
                                       external_partitioners.empty() ? nullptr : external_partitioners[0],
                                       tria.get_communicator(),
                                       target_partitioners[0],
                                       copy_indices_global_mine[0]);

              copy_indices_to_mpi_local_numbers(*target_partitioners[0],
      ...
      const unsigned int n_child_dofs_1d = fe->degree + 1 + fe->n_dofs_per_cell() - fe->n_dofs_per_vertex();
      ...
      for (unsigned int level = 1; level < n_levels; ++level)
        {
          LinearAlgebra::distributed::Vector<Number> touch_count(target_partitioners[level]);
          for (unsigned int c = 0; c < n_owned_level_cells[level - 1]; ++c)
            for (unsigned int j = 0; j < elem_info.n_child_cell_dofs; ++j)
              touch_count.local_element(level_dof_indices[level][elem_info.n_child_cell_dofs * c + j]) += Number(1.);
              ...
              for (unsigned int j = 0; j < (dim > 1 ? n_child_dofs_1d : 1); ++j)
                {
                  ...
                  for (unsigned int i = 0; i < n_child_dofs_1d; ++i, ++m)
                    weights_on_refined[level - 1][c * Utilities::fixed_power<dim>(3) + shift + degree_to_3[i]] =
                      Number(1.) /
                      touch_count.local_element(level_dof_indices[level][elem_info.n_child_cell_dofs * c + m]);
                }
        }
    }
    ...
    {
      if (mg_constrained_dofs != nullptr && mg_constrained_dofs->get_level_constraints(level).n_constraints() > 0)
        for (auto &ind : dof_indices)
          if (mg_constrained_dofs->get_level_constraints(level).is_identity_constrained(ind))
            {
              Assert(mg_constrained_dofs->get_level_constraints(level).get_constraint_entries(ind)->size() == 1,
                     ExcInternalError());
              ind = mg_constrained_dofs->get_level_constraints(level).get_constraint_entries(ind)->front().first;
            }
    }

  ...
  const std::vector<std::shared_ptr<const Utilities::MPI::Partitioner>> &external_partitioners)
{
  Assert(dof_handler.has_level_dofs(),
         ExcMessage("The underlying DoFHandler object has not had its "
  ...
  if (external_partitioners.size() > 0)
    {
      ...
      this->initialize_dof_vector = [external_partitioners](const unsigned int                          level,
  ...
  vector_partitioners.resize(0, dof_handler.get_triangulation().n_global_levels() - 1);
  for (unsigned int level = 0; level <= this->ghosted_level_vector.max_level(); ++level)
    vector_partitioners[level] = this->ghosted_level_vector[level].get_partitioner();

  std::vector<std::vector<Number>> weights_unvectorized;

  internal::MGTransfer::ElementInfo<Number> elem_info;

  internal::MGTransfer::setup_transfer<dim, Number>(dof_handler,
  ...
  for (unsigned int level = 0; level <= vector_partitioners.max_level(); ++level)
  ...
  for (unsigned int level = 1; level < n_levels; ++level)
    {
      weights_on_refined[level - 1].resize(((n_owned_level_cells[level - 1] + vec_size - 1) / vec_size) *
                                           n_weights_per_cell);
...
template <int dim, typename Number>
void
MGTransferMatrixFree<dim, Number>::prolongate_and_add(const unsigned int                                to_level,
                                                      LinearAlgebra::distributed::Vector<Number>       &dst,
                                                      const LinearAlgebra::distributed::Vector<Number> &src) const
{
  ...
  const bool src_inplace = src.get_partitioner().get() == this->vector_partitioners[to_level - 1].get();
  ...
  if (dst_inplace == false)
    {
      if (this->ghosted_level_vector[to_level].get_partitioner().get() != this->vector_partitioners[to_level].get())
        this->ghosted_level_vector[to_level].reinit(this->vector_partitioners[to_level]);
      AssertDimension(this->ghosted_level_vector[to_level].locally_owned_size(), dst.locally_owned_size());
  ...
  LinearAlgebra::distributed::Vector<Number> &dst_vec = dst_inplace ? dst : this->ghosted_level_vector[to_level];
...
template <int dim, typename Number>
void
MGTransferMatrixFree<dim, Number>::restrict_and_add(const unsigned int                                from_level,
                                                    LinearAlgebra::distributed::Vector<Number>       &dst,
                                                    const LinearAlgebra::distributed::Vector<Number> &src) const
{
  ...
  const bool src_inplace = src.get_partitioner().get() == this->vector_partitioners[from_level].get();
  if (src_inplace == false)
    {
      if (this->ghosted_level_vector[from_level].get_partitioner().get() != this->vector_partitioners[from_level].get())
  ...
  const bool dst_inplace = dst.get_partitioner().get() == this->vector_partitioners[from_level - 1].get();
  if (dst_inplace == false)
    {
      ...
      AssertDimension(this->ghosted_level_vector[from_level - 1].locally_owned_size(), dst.locally_owned_size());
      this->ghosted_level_vector[from_level - 1] = 0.;
    }

  const LinearAlgebra::distributed::Vector<Number> &src_vec =
    src_inplace ? src : this->ghosted_level_vector[from_level];
  LinearAlgebra::distributed::Vector<Number> &dst_vec = dst_inplace ? dst : this->ghosted_level_vector[from_level - 1];
...
template <int degree>
void
MGTransferMatrixFree<dim, Number>::do_prolongate_add(const unsigned int                                to_level,
                                                     LinearAlgebra::distributed::Vector<Number>       &dst,
                                                     const LinearAlgebra::distributed::Vector<Number> &src) const
{
  const unsigned int     vec_size           = VectorizedArray<Number>::size();
  ...
  if (this->mg_constrained_dofs != nullptr &&
      this->mg_constrained_dofs->get_user_constraint_matrix(to_level - 1).get_local_lines().size() > 0)
    {
      LinearAlgebra::distributed::Vector<Number> copy_src(src);

      // Distribute any user defined constraints
      this->mg_constrained_dofs->get_user_constraint_matrix(to_level - 1).distribute(copy_src);
  ...
  for (unsigned int cell = 0; cell < n_owned_level_cells[to_level - 1]; cell += vec_size)
    {
      const unsigned int n_lanes =
        (cell + vec_size > n_owned_level_cells[to_level - 1]) ? (n_owned_level_cells[to_level - 1] - cell) : vec_size;

      // read from source vector
      for (unsigned int v = 0; v < n_lanes; ++v)
        {
          const unsigned int shift = internal::MGTransfer::compute_shift_within_children<dim>(
            parent_child_connect[to_level - 1][cell + v].second, fe_degree + 1 - element_is_continuous, fe_degree);
          const unsigned int *indices =
            &level_dof_indices[to_level - 1]
                              [parent_child_connect[to_level - 1][cell + v].first * n_child_cell_dofs + shift];
              ...
              for (unsigned int k = 0; k < (dim > 2 ? degree_size : 1); ++k)
                for (unsigned int j = 0; j < (dim > 1 ? degree_size : 1); ++j)
                  for (unsigned int i = 0; i < degree_size; ++i, ++m)
                    evaluation_data[m][v] =
                      to_use->local_element(indices[c * n_scalar_cell_dofs + k * n_child_dofs_1d * n_child_dofs_1d +
                                                    j * n_child_dofs_1d + i]);

              // apply Dirichlet boundary conditions on parent cell
              for (std::vector<unsigned short>::const_iterator i = dirichlet_indices[to_level - 1][cell + v].begin();
          ...
          for (int c = n_components - 1; c >= 0; --c)
            internal::FEEvaluationImplBasisChange<internal::evaluate_general,
                                                  ...
                                                  2 * degree + 1>::do_forward(1,
                                                                              ...
                                                                              fe_degree + 1,
                                                                              2 * fe_degree + 1);
          internal::weight_fe_q_dofs_by_entity<dim, degree != -1 ? 2 * degree + 1 : -1, VectorizedArray<Number>>(
...
template <int degree>
void
MGTransferMatrixFree<dim, Number>::do_restrict_add(const unsigned int                                from_level,
                                                   LinearAlgebra::distributed::Vector<Number>       &dst,
                                                   const LinearAlgebra::distributed::Vector<Number> &src) const
{
  const unsigned int     vec_size           = VectorizedArray<Number>::size();
  ...
  for (unsigned int cell = 0; cell < n_owned_level_cells[from_level - 1]; cell += vec_size)
    {
      ...
      for (unsigned int v = 0; v < n_lanes; ++v)
        {
          const unsigned int shift = internal::MGTransfer::compute_shift_within_children<dim>(
            parent_child_connect[from_level - 1][cell + v].second, fe_degree + 1 - element_is_continuous, fe_degree);
          ...
          const unsigned int *indices =
            &level_dof_indices[from_level - 1]
                              [parent_child_connect[from_level - 1][cell + v].first * n_child_cell_dofs + shift];
          for (unsigned int c = 0, m = 0; c < n_components; ++c)
            {
              // apply Dirichlet boundary conditions on parent cell
              for (std::vector<unsigned short>::const_iterator i = dirichlet_indices[from_level - 1][cell + v].begin();
              ...
              for (unsigned int k = 0; k < (dim > 2 ? degree_size : 1); ++k)
                for (unsigned int j = 0; j < (dim > 1 ? degree_size : 1); ++j)
                  for (unsigned int i = 0; i < degree_size; ++i, ++m)
                    dst.local_element(indices[c * n_scalar_cell_dofs + k * n_child_dofs_1d * n_child_dofs_1d +
...
MGTransferBlockMatrixFree<dim, Number>::MGTransferBlockMatrixFree(const MGConstrainedDoFs &mg_c)
  : MGTransferBlockMatrixFreeBase<dim, Number, MGTransferMatrixFree<dim, Number>>(true)
{
...
template <int dim, typename Number>
MGTransferBlockMatrixFree<dim, Number>::MGTransferBlockMatrixFree(const std::vector<MGConstrainedDoFs> &mg_c)
  : MGTransferBlockMatrixFreeBase<dim, Number, MGTransferMatrixFree<dim, Number>>(false)
{
...
template <int dim, typename Number>
void
MGTransferBlockMatrixFree<dim, Number>::initialize_constraints(const std::vector<MGConstrainedDoFs> &mg_c)
{
  Assert(!this->same_for_all,
...
template <int dim, typename Number>
void
MGTransferBlockMatrixFree<dim, Number>::build(const DoFHandler<dim> &dof_handler)
{
  AssertDimension(this->matrix_free_transfer_vector.size(), 1);
...
template <int dim, typename Number>
void
MGTransferBlockMatrixFree<dim, Number>::build(const std::vector<const DoFHandler<dim> *> &dof_handler)
{
...
MGTransferBlockMatrixFree<dim, Number>::get_matrix_free_transfer(const unsigned int b) const
{

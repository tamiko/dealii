...
namespace NonMatching
{
  namespace internal
  {
    ...
    qpoints_over_locally_owned_cells(const GridTools::Cache<dim0, spacedim> &cache,
            ...
            if (tria_is_parallel)
              {
                ...
                std::vector<
                  std::pair<BoundingBox<spacedim>, typename Triangulation<dim0, spacedim>::active_cell_iterator>>
                  out_vals;
                cache.get_cell_bounding_boxes_rtree().query(boost::geometry::index::intersects(bbox),
            ...
            if (use_cell)
              {
                ...
                fe_v.reinit(cell);
                const std::vector<Point<spacedim>> &x_points = fe_v.get_quadrature_points();

                // Insert the points to the vector
                points_over_local_cells.insert(points_over_local_cells.end(), x_points.begin(), x_points.end());
              }
            ++cell_id;
          }
      }
                                ...
                                const FiniteElement<dim1, spacedim> &fe1)
    {
      ...
      const ComponentMask mask0 = (comps0.size() == 0 ? ComponentMask(fe0.n_components(), true) : comps0);

      const ComponentMask mask1 = (comps1.size() == 0 ? ComponentMask(fe1.n_components(), true) : comps1);

      AssertDimension(mask0.size(), fe0.n_components());
      AssertDimension(mask1.size(), fe1.n_components());

      // Global to local indices
      std::vector<unsigned int> gtl0(fe0.n_components(), numbers::invalid_unsigned_int);
      std::vector<unsigned int> gtl1(fe1.n_components(), numbers::invalid_unsigned_int);
                                   ...
                                   const AffineConstraints<number>        &immersed_constraints)
  {
    ...
    const bool tria_is_parallel =
      (dynamic_cast<const parallel::TriangulationBase<dim1, spacedim> *>(&space_dh.get_triangulation()) != nullptr);
    ...
    const ComponentMask immersed_c =
      (immersed_comps.size() == 0 ? ComponentMask(immersed_fe.n_components(), true) : immersed_comps);
    ...
    const unsigned int n_active_c = immersed_dh.get_triangulation().n_active_cells();

    const auto qpoints_cells_data =
      internal::qpoints_over_locally_owned_cells(cache, immersed_dh, quad, immersed_mapping, tria_is_parallel);
    ...
    const auto  cpm       = GridTools::compute_point_locations(cache, points_over_local_cells);
    const auto &all_cells = std::get<0>(cpm);
    const auto &maps      = std::get<2>(cpm);

    std::vector<std::set<typename Triangulation<dim0, spacedim>::active_cell_iterator>> cell_sets(n_active_c);
        ...
        for (const auto &cell_c : cells)
          {
            ...
            typename DoFHandler<dim0, spacedim>::cell_iterator ocell(*cell_c, &space_dh);
            ...
            if (ocell->is_locally_owned())
              {
                ...
                constraints.add_entries_local_to_global(odofs,
                                                        immersed_constraints,
                                                        dofs,
                              ...
                              const AffineConstraints<typename Matrix::value_type> &immersed_constraints)
  {
    ...
    FullMatrix<typename Matrix::value_type> cell_matrix(space_dh.get_fe().n_dofs_per_cell(),
    ...
    std::vector<std::vector<typename Triangulation<dim0, spacedim>::active_cell_iterator>> cell_container(n_active_c);
    std::vector<std::vector<std::vector<Point<dim0>>>>  qpoints_container(n_active_c);
    std::vector<std::vector<std::vector<unsigned int>>> maps_container(n_active_c);
        ...
        for (unsigned int j = 0; j < all_maps[o].size(); ++j)
          {
            ...
            if (cell_container[cell_id].empty())
              {
                ...
                qpoints_container[cell_id].emplace_back(std::vector<Point<dim0>>{all_qpoints[o][j]});
            ...
            // at the last inserted cell, which is more likely:
            ...
            else
              {
                ...
                const auto cell_p =
                  std::find(cell_container[cell_id].begin(), cell_container[cell_id].end() - 1, all_cells[o]);
            ...
            if (ocell->is_locally_owned())
              {
                ...
                FEValues<dim0, spacedim> o_fe_v(cache.get_mapping(), space_dh.get_fe(), qps, update_values);
                o_fe_v.reinit(ocell);
                ocell->get_dof_indices(odofs);
                ...
                for (unsigned int i = 0; i < space_dh.get_fe().n_dofs_per_cell(); ++i)
                  {
                    const auto comp_i = space_dh.get_fe().system_to_component_index(i).first;
                    if (space_gtl[comp_i] != numbers::invalid_unsigned_int)
                      for (unsigned int j = 0; j < immersed_dh.get_fe().n_dofs_per_cell(); ++j)
                        {
                            ...
                            for (unsigned int oq = 0; oq < o_fe_v.n_quadrature_points; ++oq)
                              {
                                ...
                                cell_matrix(i, j) += (fe_v.shape_value(j, q) * o_fe_v.shape_value(i, oq) * fe_v.JxW(q));
                              }
                        }
                  }

                // Now assemble the matrices
                constraints.distribute_local_to_global(cell_matrix, odofs, immersed_constraints, dofs, matrix);
              }
          }
      }
  }
  ...
  template <int dim0, int dim1, int spacedim, typename Number>
  void
  create_coupling_sparsity_pattern(const double                           &epsilon,
                                   ...
                                   const ComponentMask                    &comps1)
  {
    if (epsilon == 0.0)
      {
        ...
        create_coupling_sparsity_pattern(
          cache0, dh0, dh1, quad, sparsity, constraints0, comps0, comps1, cache1.get_mapping());
        return;
      }
    ...
    const bool zero_is_distributed =
      (dynamic_cast<const parallel::distributed::Triangulation<dim1, spacedim> *>(&dh0.get_triangulation()) != nullptr);
    const bool one_is_distributed =
      (dynamic_cast<const parallel::distributed::Triangulation<dim1, spacedim> *>(&dh1.get_triangulation()) != nullptr);
    ...
    const bool outer_loop_on_zero =
      ...
      (dh1.get_triangulation().n_active_cells() > dh0.get_triangulation().n_active_cells());
    ...
    if (outer_loop_on_zero)
      {
        ...
        std::vector<std::pair<BoundingBox<spacedim>, typename Triangulation<dim1, spacedim>::active_cell_iterator>>
          intersection;

        for (const auto &cell0 : dh0.active_cell_iterators() | IteratorFilters::LocallyOwnedCell())
          {
            intersection.resize(0);
            BoundingBox<spacedim> box0 = cache0.get_mapping().get_bounding_box(cell0);
            box0.extend(epsilon);
            boost::geometry::index::query(tree1,
                                          boost::geometry::index::intersects(box0),
            ...
            if (!intersection.empty())
              {
                ...
                for (const auto &entry : intersection)
                  {
                    typename DoFHandler<dim1, spacedim>::cell_iterator cell1(*entry.second, &dh1);
                    cell1->get_dof_indices(dofs1);
                    constraints0.add_entries_local_to_global(dofs0, dofs1, sparsity);
                  }
              }
          }
      }
    else
      {
        ...
        std::vector<std::pair<BoundingBox<spacedim>, typename Triangulation<dim0, spacedim>::active_cell_iterator>>
          intersection;

        for (const auto &cell1 : dh1.active_cell_iterators() | IteratorFilters::LocallyOwnedCell())
          {
            intersection.resize(0);
            BoundingBox<spacedim> box1 = cache1.get_mapping().get_bounding_box(cell1);
            box1.extend(epsilon);
            boost::geometry::index::query(tree0,
                                          boost::geometry::index::intersects(box1),
                                          std::back_inserter(intersection));
            if (!intersection.empty())
              {
                cell1->get_dof_indices(dofs1);
                for (const auto &entry : intersection)
                  {
                    typename DoFHandler<dim0, spacedim>::cell_iterator cell0(*entry.second, &dh0);
                    cell0->get_dof_indices(dofs0);
                    constraints0.add_entries_local_to_global(dofs0, dofs1, sparsity);
                  }
              }
  ...
  template <int dim0, int dim1, int spacedim, typename Matrix>
  void
  create_coupling_mass_matrix(Functions::CutOffFunctionBase<spacedim>              &kernel,
                              ...
                              const ComponentMask                                  &comps1)
  {
    if (epsilon == 0)
      {
        ...
        create_coupling_mass_matrix(
          cache0, dh0, dh1, quadrature1, matrix, constraints0, comps0, comps1, cache1.get_mapping());
        return;
      }
    ...
    // Local Matrix
    FullMatrix<typename Matrix::value_type> cell_matrix(fe0.n_dofs_per_cell(), fe1.n_dofs_per_cell());

    // Global to local indices
    const auto  p    = internal::compute_components_coupling(comps0, comps1, fe0, fe1);
          ...
          for (unsigned int j = 0; j < fe1.n_dofs_per_cell(); ++j)
            {
              ...
              for (unsigned int q1 = 0; q1 < quadrature1.size(); ++q1)
                sum_q1 += fev1.shape_value(j, q1) * kernel_values[q1] * fev1.JxW(q1);
              sum_q1 *= fev0.JxW(q0);
              ...
              for (unsigned int i = 0; i < fe0.n_dofs_per_cell(); ++i)
                {
                  ...
                  if (gtl0[comp_i] != numbers::invalid_unsigned_int && gtl1[comp_j] == gtl0[comp_i])
                    cell_matrix(i, j) += fev0.shape_value(i, q0) * sum_q1;
                }
            }
        }

      constraints0.distribute_local_to_global(cell_matrix, dofs0, dofs1, matrix);
    };

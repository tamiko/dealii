...
namespace NonMatching
{
  namespace internal
  {
    namespace QuadratureGeneratorImplementation
    {
      ...
      {
        Assert(start < end, ExcMessage("Interval start must be less than interval end."));

        const double length = end - start;
        for (unsigned int j = 0; j < quadrature1D.size(); ++j)
          {
            const double x = start + length * quadrature1D.point(j)[0];
            quadrature.push_back(dealii::internal::create_higher_dim_point(point, component_in_dim, x),
        ...
        for (unsigned int j = 0; j < lower.size(); ++j)
          {
            tensor_point_with_1D_quadrature(
              lower.point(j), lower.weight(j), quadrature1D, start, end, component_in_dim, quadrature);
          }
      }
      ...
      template <int dim>
      Definiteness
      pointwise_definiteness(const std::vector<std::reference_wrapper<const Function<dim>>> &functions,
                             const Point<dim>                                               &point)
      {
        Assert(functions.size() > 0, ExcMessage("The incoming vector must contain at least one function."));

        const int sign_of_first = boost::math::sign(functions[0].get().value(point));
      ...
      template <int dim>
      void
      estimate_function_bounds(const std::vector<std::reference_wrapper<const Function<dim>>> &functions,
                               const BoundingBox<dim>                                         &box,
        ...
        for (const Function<dim> &function : functions)
          {
            ...
            FunctionTools::taylor_estimate_function_bounds<dim>(function, box, bounds.value, bounds.gradient);
      ...
      {
        Assert(function_bounds.first <= function_bounds.second, ExcMessage("Function bounds reversed, max < min."));
      ...
      find_best_height_direction(const std::vector<FunctionBounds<dim>> &all_function_bounds)
      {
                ...
                if (!min_lower_abs_grad)
                  {
                    ...
                    for (unsigned int d = 0; d < dim; ++d)
                      {
                        (*min_lower_abs_grad)[d] = lower_bound_on_abs(bounds.gradient[d]);
                      }
                  }
                else
                  {
                    for (unsigned int d = 0; d < dim; ++d)
                      {
                        (*min_lower_abs_grad)[d] =
                          std::min((*min_lower_abs_grad)[d], lower_bound_on_abs(bounds.gradient[d]));
                      }
                  }
              }
          }
        ...
        if (min_lower_abs_grad)
          {
            const auto max_element = std::max_element(min_lower_abs_grad->begin(), min_lower_abs_grad->end());

            HeightDirectionData data;
            data.direction    = std::distance(min_lower_abs_grad->begin(), max_element);
      ...
      template <int dim>
      inline bool
      one_positive_one_negative_definite(const std::vector<FunctionBounds<dim>> &all_function_bounds)
      {
      ...
      template <int dim>
      void
      restrict_to_top_and_bottom(const std::vector<std::reference_wrapper<const Function<dim>>> &functions,
                                 const BoundingBox<dim>                                         &box,
                                 ...
                                 std::vector<Functions::CoordinateRestriction<dim - 1>>         &restrictions)
      {
        ...
        for (const auto &function : functions)
          {
            restrictions.push_back(Functions::CoordinateRestriction<dim - 1>(function, direction, bottom));
            restrictions.push_back(Functions::CoordinateRestriction<dim - 1>(function, direction, top));
          }
      }
      ...
      template <int dim>
      void
      restrict_to_point(const std::vector<std::reference_wrapper<const Function<dim>>> &functions,
                        ...
                        std::vector<Functions::PointRestriction<dim - 1>>              &restrictions)
      {
        ...
        for (const auto &function : functions)
          {
            restrictions.push_back(Functions::PointRestriction<dim - 1>(function, open_direction, point));
          }
      }
      ...
      template <int dim>
      void
      distribute_points_between_roots(const Quadrature<1>       &quadrature1D,
        ...
        for (int i = -1; i < n_roots; ++i)
          {
            ...
            const double end   = i + 1 < n_roots ? roots[i + 1] : interval.upper_bound(0);
            ...
            if (length > additional_data.min_distance_between_roots)
              {
                ...
                const Definiteness         definiteness      = pointwise_definiteness(level_sets, center);
                ExtendableQuadrature<dim> &target_quadrature = q_partitioning.quadrature_by_definiteness(definiteness);

                tensor_point_with_1D_quadrature(
                  point, weight, quadrature1D, start, end, height_function_direction, target_quadrature);
              }
          }
      }
      ...
      RootFinder::AdditionalData::AdditionalData(const double       tolerance,
      ...
      void
      RootFinder::find_roots(const std::vector<std::reference_wrapper<const Function<1>>> &functions,
                             const BoundingBox<1>                                         &interval,
                             std::vector<double>                                          &roots)
      {
        ...
        roots.erase(unique(roots.begin(), roots.end(), roots_are_equal), roots.end());
      }
                             ...
                             std::vector<double>  &roots)
      {
        ...
        if (boost::math::sign(left_value) != boost::math::sign(right_value))
          {
            const auto lambda = [&function](const double x) { return function.value(Point<1>(x)); };

            const auto stopping_criteria = [this](const double &a, const double &b) {
              return std::abs(a - b) < additional_data.tolerance;
            };

            boost::uintmax_t iterations = additional_data.max_iterations;

            const std::pair<double, double> root_bracket = boost::math::tools::toms748_solve(lambda,
        ...
        else
          {
            ...
            FunctionTools::taylor_estimate_function_bounds<1>(function, interval, value_bounds, gradient_bounds);

            // Since we already know the function values at the interval ends we
            // might as well check these for min/max too.
            const double function_min = std::min(std::min(left_value, right_value), value_bounds.first);

            // If the functions is positive there are no roots.
            if (function_min > 0)
              return;

            const double function_max = std::max(std::max(left_value, right_value), value_bounds.second);
            ...
            if (recursion_depth < additional_data.max_recursion_depth)
              {
                find_roots(function, interval.child(0), recursion_depth + 1, roots);
                find_roots(function, interval.child(1), recursion_depth + 1, roots);
              }
          }
      }
      ...
      template <int dim>
      ExtendableQuadrature<dim>::ExtendableQuadrature(const Quadrature<dim> &quadrature)
      ...
      template <int dim>
      void
      ExtendableQuadrature<dim>::push_back(const Point<dim> &point, const double weight)
      {
      ...
      QPartitioning<dim>::quadrature_by_definiteness(const Definiteness definiteness)
      {
        switch (definiteness)
          {
                                           ...
                                           const Function<dim>    &level_set)
      {
        const Point<dim> bottom_point =
          dealii::internal::create_higher_dim_point(point, direction, box.lower_bound(direction));
        const double bottom_value = level_set.value(bottom_point);

        const Point<dim> top_point =
          dealii::internal::create_higher_dim_point(point, direction, box.upper_bound(direction));
        const double top_value = level_set.value(top_point);

        // The end point closest to the zero-contour is the one with smallest
        // absolute value.
        return std::abs(bottom_value) < std::abs(top_value) ? bottom_point : top_point;
      }
      ...
      UpThroughDimensionCreator<dim, spacedim>::UpThroughDimensionCreator(
        ...
        , root_finder(
            RootFinder::AdditionalData(additional_data.root_finder_tolerance, additional_data.max_root_finder_splits))
      {
        q_index = 0;
      }
        ...
        for (unsigned int q = 0; q < low_dim_quadrature.size(); ++q)
          {
            ...
            restrict_to_point(level_sets, point, height_function_direction, point_restrictions);

            // We need a vector of references to do the recursive call.
            const std::vector<std::reference_wrapper<const Function<1>>> restrictions(point_restrictions.begin(),
                                                                                      point_restrictions.end());

            const BoundingBox<1> bounds_in_direction = box.bounds(height_function_direction);
            ...
            if (dim == spacedim)
              create_surface_point(point, weight, level_sets, box, height_function_direction, q_partitioning.surface);
          }
        ...
        ImmersedSurfaceQuadrature<dim>                                 &surface_quadrature)
      {
        ...
        if (roots.size() == 1)
          {
            surface_point = dealii::internal::create_higher_dim_point(point, height_function_direction, roots[0]);
          }
        else
          {
            ...
            surface_point = face_projection_closest_zero_contour(point, height_function_direction, box, level_set);
          }
        ...
        const double surface_weight = weight * gradient.norm() / std::abs(gradient[height_function_direction]);
      ...
      template <int dim, int spacedim>
      void
      UpThroughDimensionCreator<dim, spacedim>::set_1D_quadrature(unsigned int q_index)
      {
      ...
      template <int dim, int spacedim>
      QGeneratorBase<dim, spacedim>::QGeneratorBase(const hp::QCollection<1>       &q_collection1D,
                                                    const AdditionalQGeneratorData &additional_data)
        : additional_data(additional_data)
        , q_collection1D(&q_collection1D)
      {
      ...
      template <int dim, int spacedim>
      QGenerator<dim, spacedim>::QGenerator(const hp::QCollection<1>       &q_collection1D,
                                            const AdditionalQGeneratorData &additional_data)
      ...
      template <int dim, int spacedim>
      void
      QGenerator<dim, spacedim>::generate(const std::vector<std::reference_wrapper<const Function<dim>>> &level_sets,
                                          const BoundingBox<dim>                                         &box,
                                          const unsigned int                                              n_box_splits)
      {
        ...
        const std::pair<double, double> extreme_values = find_extreme_values(all_function_bounds);

        if (extreme_values.first > this->additional_data.limit_to_be_definite)
          {
            map_quadrature_to_box(tensor_products[this->q_index], box, this->q_partitioning.positive);
          }
        else if (extreme_values.second < -(this->additional_data.limit_to_be_definite))
          {
            map_quadrature_to_box(tensor_products[this->q_index], box, this->q_partitioning.negative);
          }
        else if (one_positive_one_negative_definite(all_function_bounds))
          {
            map_quadrature_to_box(tensor_products[this->q_index], box, this->q_partitioning.indefinite);
          }
        else
          {
            const std::optional<HeightDirectionData> data = find_best_height_direction(all_function_bounds);

            // Check larger than a constant to avoid that min_abs_dfdx is only
            // larger by 0 by floating point precision.
            if (data && data->min_abs_dfdx > this->additional_data.lower_bound_implicit_function)
              {
                create_low_dim_quadratures(data->direction, level_sets, box, n_box_splits);
      ...
      direction_of_largest_extent(const BoundingBox<dim> &box)
      {
        ...
        if (boost::math::epsilon_difference(side_lengths[dim - 1].first, side_lengths[dim - 2].first) < 100)
      ...
      template <int dim>
      unsigned int
      compute_split_direction(const BoundingBox<dim>                   &box,
                              const std::optional<HeightDirectionData> &height_direction_data)
      {
        const std::optional<unsigned int> direction = direction_of_largest_extent(box);
        ...
        const unsigned int                                              n_box_splits)
      {
        if (this->additional_data.split_in_half)
          {
            const unsigned int direction = compute_split_direction(box, direction_data);
      ...
      QGenerator<dim, spacedim>::create_low_dim_quadratures(
        ...
        const unsigned int                                              n_box_splits)
      {
        std::vector<Functions::CoordinateRestriction<dim - 1>> face_restrictions;
        restrict_to_top_and_bottom(level_sets, box, height_function_direction, face_restrictions);

        // We need a vector of references to do the recursive call.
        const std::vector<std::reference_wrapper<const Function<dim - 1>>> restrictions(face_restrictions.begin(),
                                                                                        face_restrictions.end());

        const BoundingBox<dim - 1> cross_section = box.cross_section(height_function_direction);
      ...
      QGenerator<dim, spacedim>::create_high_dim_quadratures(
        ...
        const BoundingBox<dim>                                         &box)
      {
        const QPartitioning<dim - 1> &low_dim_quadratures = low_dim_algorithm.get_quadratures();

        const Quadrature<1> &quadrature1D = (*this->q_collection1D)[this->q_index];
        ...
        up_through_dimension_creator.generate(
          level_sets, box, low_dim_quadratures.indefinite, height_function_direction, this->q_partitioning);
      }
        ...
        const BoundingBox<dim>                                         &box)
      {
        ...
        ExtendableQuadrature<dim> &quadrature = this->q_partitioning.quadrature_by_definiteness(definiteness);
      ...
      QGenerator<1, spacedim>::QGenerator(const hp::QCollection<1>       &q_collection1D,
                                          const AdditionalQGeneratorData &additional_data)
        : QGeneratorBase<1, spacedim>(q_collection1D, additional_data)
        , root_finder(
            RootFinder::AdditionalData(additional_data.root_finder_tolerance, additional_data.max_root_finder_splits))
      {
        Assert(q_collection1D.size() > 0, ExcMessage("Incoming quadrature collection is empty."));
      }
      ...
      template <int spacedim>
      void
      QGenerator<1, spacedim>::generate(const std::vector<std::reference_wrapper<const Function<1>>> &level_sets,
        ...
        for (const double root : roots)
          {
            ...
            Assert(gradient_norm > 1e-11, ExcMessage("The level set function has a gradient almost equal to 0."));
    ...
    {
      DeclExceptionMsg(ExcCellNotSet, "The set_active_cell function has to be called before calling this function.");

      DeclExceptionMsg(ExcReferenceCellNotHypercube, "The reference cell of the incoming cell must be a hypercube.");
      ...
      public:
         ...
         */
        void
        set_active_cell(const typename Triangulation<dim>::active_cell_iterator &cell) override;

        /**
         * @copydoc CellWiseFunction::set_subcell()
         */
        void
        set_subcell(const std::vector<unsigned int> &mask, const BoundingBox<dim> &subcell_box) override;
      ...
      RefSpaceFEFieldFunction<dim, VectorType>::RefSpaceFEFieldFunction(const DoFHandler<dim> &dof_handler,
      ...
      {
        Assert(&cell->get_triangulation() == &dof_handler->get_triangulation(),
               ExcMessage("The incoming cell must belong to the triangulation associated with "
                          "the DoFHandler passed to the constructor."));

        const auto dof_handler_cell = cell->as_dof_handler_iterator(*dof_handler);
        ...
        if (element != &dof_handler_cell->get_fe())
          {
            ...
            if (const FE_Q_iso_Q1<dim> *fe_q_iso_q1 =
                  dynamic_cast<const FE_Q_iso_Q1<dim> *>(&dof_handler_cell->get_fe()))
              {
            ...
            if (fe->n_base_elements() == 1 && dealii::internal::FEPointEvaluation::is_fast_path_supported(*fe, 0))
              {
                ...
                poly     = dealii::internal::FEPointEvaluation::get_polynomial_space(fe->base_element(0));

                polynomials_are_hat_functions =
                  (poly.size() == 2 && poly[0].value(0.) == 1. && poly[0].value(1.) == 0. && poly[1].value(0.) == 0. &&
                   poly[1].value(1.) == 1.);
              }
          }
        else
        ...
        for (unsigned int i = 0; i < local_dof_indices.size(); ++i)
          local_dof_values[i] =
            dealii::internal::ElementAccess<VectorType>::get(*global_dof_values, local_dof_indices[i]);
      }
      ...
      template <int dim, typename VectorType>
      void
      RefSpaceFEFieldFunction<dim, VectorType>::set_subcell(const std::vector<unsigned int> &mask,
      ...
      template <int dim, typename VectorType>
      double
      RefSpaceFEFieldFunction<dim, VectorType>::value(const Point<dim> &point, const unsigned int component) const
      {
        ...
        if (!poly.empty() && component == 0)
          {
            ...
            return dealii::internal::evaluate_tensor_product_value(
              poly,
              this->is_fe_q_iso_q1() ? local_dof_values_subcell : local_dof_values,
        ...
        else
          {
            double value = 0;
            for (unsigned int i = 0; i < local_dof_indices.size(); ++i)
              value += local_dof_values[i] * element->shape_value_component(i, point, component);
      ...
      template <int dim, typename VectorType>
      Tensor<1, dim>
      RefSpaceFEFieldFunction<dim, VectorType>::gradient(const Point<dim> &point, const unsigned int component) const
      {
        AssertIndexRange(component, this->n_components);
        Assert(cell_is_set(), ExcCellNotSet());
        ...
        if (!poly.empty() && component == 0)
        ...
        else
          {
            ...
            for (unsigned int i = 0; i < local_dof_indices.size(); ++i)
              gradient += local_dof_values[i] * element->shape_grad_component(i, point, component);
      ...
      template <int dim, typename VectorType>
      SymmetricTensor<2, dim>
      RefSpaceFEFieldFunction<dim, VectorType>::hessian(const Point<dim> &point, const unsigned int component) const
      {
        AssertIndexRange(component, this->n_components);
        Assert(cell_is_set(), ExcCellNotSet());
        ...
        if (!poly.empty() && component == 0)
        ...
        else
          {
            ...
            for (unsigned int i = 0; i < local_dof_indices.size(); ++i)
              hessian += local_dof_values[i] * element->shape_grad_grad_component(i, point, component);
        ...
        for (unsigned int d = 0; d < dim; ++d)
          {
            ...
            const double split_box_side_length = (1. / n_subdivisions) * unit_box.side_length(d);
      ...
      setup_lexicographic_mask(const std::array<unsigned int, dim> &subcell_indices, unsigned int dofs_per_line)
      {
  ...
  AdditionalQGeneratorData::AdditionalQGeneratorData(const unsigned int max_box_splits,
  ...
  template <int dim>
  QuadratureGenerator<dim>::QuadratureGenerator(const hp::QCollection<1> &q_collection,
                                                const AdditionalData     &additional_data)
    : q_generator(q_collection, additional_data)
  {
    Assert(q_collection.size() > 0, ExcMessage("Incoming hp::QCollection<1> is empty."));
  }
  ...
  template <int dim>
  void
  QuadratureGenerator<dim>::generate(const Function<dim> &level_set, const BoundingBox<dim> &box)
  {
    clear_quadratures();
  ...
  template <int dim>
  void
  QuadratureGenerator<dim>::generate_append(const Function<dim> &level_set, const BoundingBox<dim> &box)
  {
    Assert(level_set.n_components == 1,
           ExcMessage("The incoming function should be a scalar level set function,"
    ...
    Assert(q_generator.get_quadratures().indefinite.empty(),
           ExcMessage("Generation of quadrature rules failed. This can mean that the level "
  ...
  template <int dim>
  FaceQuadratureGenerator<dim>::FaceQuadratureGenerator(const hp::QCollection<1> &quadratures1D,
                                                        const AdditionalData     &additional_data)
                                                ...
                                                const unsigned int      face_index)
  {
    ...
    // the face normal.
    const unsigned int face_normal_direction = GeometryInfo<dim>::unit_normal_direction[face_index];

    const Point<dim> vertex0          = box.vertex(GeometryInfo<dim>::face_to_cell_vertices(face_index, 0));
    const double     coordinate_value = vertex0(face_normal_direction);

    const Functions::CoordinateRestriction<dim - 1> face_restriction(level_set,
                                                                     face_normal_direction,
                                                                     coordinate_value);

    // Reuse the lower dimensional QuadratureGenerator on the face.
    const BoundingBox<dim - 1> cross_section = box.cross_section(face_normal_direction);
    ...
    // Recompute these.
    const ImmersedSurfaceQuadrature<dim - 1, dim - 1> &surface_quadrature_wrong_normal =
    ...
    for (unsigned int i = 0; i < surface_quadrature_wrong_normal.size(); ++i)
      {
        const Point<dim> point = dealii::internal::create_higher_dim_point(surface_quadrature_wrong_normal.point(i),
                                                                           face_normal_direction,
                                                                           coordinate_value);
    ...
    surface_quadrature = ImmersedSurfaceQuadrature<dim - 1, dim>(surface_quadrature_wrong_normal.get_points(),
  ...
  FaceQuadratureGenerator<1>::FaceQuadratureGenerator(const hp::QCollection<1> &quadratures1D,
                                                      const AdditionalData     &additional_data)
  {
    (void)quadratures1D;
                                              ...
                                              const unsigned int    face_index)
  {
    ...
    const Point<1> vertex = box.vertex(GeometryInfo<1>::face_to_cell_vertices(face_index, 0));
  ...
  DiscreteQuadratureGenerator<dim>::DiscreteQuadratureGenerator(const hp::QCollection<1> &quadratures1D,
                                                                const DoFHandler<dim>    &dof_handler,
                                                                const VectorType         &level_set,
                                                                const AdditionalData     &additional_data)
    : QuadratureGenerator<dim>(quadratures1D, additional_data)
    , reference_space_level_set(
        std::make_unique<internal::DiscreteQuadratureGeneratorImplementation::RefSpaceFEFieldFunction<dim, VectorType>>(
          dof_handler,
          level_set))
  {}
  ...
  template <int dim>
  void
  DiscreteQuadratureGenerator<dim>::generate_fe_q_iso_q1(const BoundingBox<dim> &unit_box)
  {
    const unsigned int n_subdivisions = reference_space_level_set->n_subdivisions();
        ...
        for (unsigned int i = 1; i < n_subdivisions; ++i)
          {
            for (unsigned int j = 0; j < old_size; ++j)
              {
                std::array<unsigned int, dim> next_entry = all_subcell_indices[j];
                next_entry[d]                            = i;
    ...
    for (const auto &subcell_indices : all_subcell_indices)
      {
        const std::vector<unsigned int> lexicographic_mask =
          internal::DiscreteQuadratureGeneratorImplementation::setup_lexicographic_mask<dim>(subcell_indices,
                                                                                             dofs_per_line);

        const auto subcell_box =
          internal::DiscreteQuadratureGeneratorImplementation::create_subcell_box<dim>(unit_box,
                                                                                       subcell_indices,
                                                                                       n_subdivisions);

        reference_space_level_set->set_subcell(lexicographic_mask, subcell_box);

        QuadratureGenerator<dim>::generate_append(*reference_space_level_set, subcell_box);
      }
  }
  ...
  template <int dim>
  void
  DiscreteQuadratureGenerator<dim>::generate(const typename Triangulation<dim>::active_cell_iterator &cell)
  {
    Assert(cell->reference_cell().is_hyper_cube(),
           internal::DiscreteQuadratureGeneratorImplementation::ExcReferenceCellNotHypercube());
  ...
  template <int dim>
  template <typename VectorType>
  DiscreteFaceQuadratureGenerator<dim>::DiscreteFaceQuadratureGenerator(const hp::QCollection<1> &quadratures1D,
                                                                        const DoFHandler<dim>    &dof_handler,
                                                                        const VectorType         &level_set,
                                                                        const AdditionalData     &additional_data)
  ...
  template <int dim>
  void
  DiscreteFaceQuadratureGenerator<dim>::generate_fe_q_iso_q1(const BoundingBox<dim> &unit_box, unsigned int face_index)
  {
    const unsigned int n_subdivisions = reference_space_level_set->n_subdivisions();
    ...
    all_subcell_indices.reserve(Utilities::pow(n_subdivisions, dim));
    all_subcell_indices.emplace_back();
    all_subcell_indices[0][coordinate_direction_face] = coordinate_orientation_face == 0 ? 0 : n_subdivisions - 1;
    ...
    for (const auto &subcell_indices : all_subcell_indices)
      {
        ...
        FaceQuadratureGenerator<dim>::generate_append(*reference_space_level_set, subcell_box, face_index);
      }
  }
  ...
  template <int dim>
  void
  DiscreteFaceQuadratureGenerator<dim>::generate(const typename Triangulation<dim>::active_cell_iterator &cell,
                                                 const unsigned int                                       face_index)
  {
    Assert(cell->reference_cell().is_hyper_cube(),
           internal::DiscreteQuadratureGeneratorImplementation::ExcReferenceCellNotHypercube());
    ...
    if (reference_space_level_set->is_fe_q_iso_q1())
      ...
      FaceQuadratureGenerator<dim>::generate(*reference_space_level_set, unit_box, face_index);
  }
} // namespace NonMatching

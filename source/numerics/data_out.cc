...
DataOut<dim, spacedim>::DataOut()
{
  set_cell_selection(
    [this](const Triangulation<dim, spacedim> &) {
      typename Triangulation<dim, spacedim>::active_cell_iterator cell = this->triangulation->begin_active();
    ...
    [this](const Triangulation<dim, spacedim> &, const cell_iterator &old_cell) {
      typename Triangulation<dim, spacedim>::active_cell_iterator cell = old_cell;
      ++cell;
      while ((cell != this->triangulation->end()) && !cell->is_locally_owned())
        ++cell;
      return cell;
...
template <int dim, int spacedim>
void
DataOut<dim, spacedim>::build_one_patch(const std::pair<cell_iterator, unsigned int>                 *cell_and_index,
                                        ...
                                        const CurvedCellRegion curved_cell_region)
{
  ...
  const FEValuesBase<dim, spacedim> &fe_patch_values = scratch_data.get_present_fe_values(0);

  const auto vertices = fe_patch_values.get_mapping().get_vertices(cell_and_index->first);
  ...
  for (unsigned int dataset = 0; dataset < scratch_data.postprocessed_values.size(); ++dataset)
  ...
  if (curved_cell_region == curved_inner_cells ||
      (curved_cell_region == curved_boundary && (cell_and_index->first->at_boundary() || (dim != spacedim))) ||
      (cell_and_index->first->reference_cell() != ReferenceCells::get_hypercube<dim>()))
    {
      ...
      const std::vector<Point<spacedim>> &q_points = fe_patch_values.get_quadrature_points();
  ...
  if (scratch_data.n_datasets > 0)
    {
      ...
      for (const auto &dataset : this->dof_data)
        {
          const FEValuesBase<dim, spacedim> &this_fe_patch_values = scratch_data.get_present_fe_values(dataset_number);
          const unsigned int                 n_components         = this_fe_patch_values.get_fe().n_components();

          const DataPostprocessor<spacedim> *postprocessor = dataset->postprocessor;
          ...
          if (postprocessor != nullptr)
            {
              ...
              if ((n_components == 1) && (dataset->is_complex_valued() == false))
                {
                  ...
                  const typename DoFHandler<dim, spacedim>::cell_iterator dh_cell(
                    &cell_and_index->first->get_triangulation(),
                    cell_and_index->first->level(),
                    cell_and_index->first->index(),
                    dataset->dof_handler);
                  scratch_data.patch_values_scalar.template set_cell<dim>(dh_cell);
              ...
              else
                {
                  ...
                  else
                    {
                      ...
                      if (n_components == 1)
                          ...
                          if (update_flags & update_values)
                            {
                              ...
                              for (unsigned int i = 0; i < scratch_data.patch_values_scalar.solution_values.size(); ++i)
                                {
                                  AssertDimension(scratch_data.patch_values_system.solution_values[i].size(), 2);
                                  scratch_data.patch_values_system.solution_values[i][0] =
                                    scratch_data.patch_values_scalar.solution_values[i];
                                }
                            }
                          ...
                          if (update_flags & update_gradients)
                            {
                              ...
                              for (unsigned int i = 0; i < scratch_data.patch_values_scalar.solution_gradients.size();
                                   ++i)
                                {
                                  AssertDimension(scratch_data.patch_values_system.solution_values[i].size(), 2);
                                  scratch_data.patch_values_system.solution_gradients[i][0] =
                                    scratch_data.patch_values_scalar.solution_gradients[i];
                                }
                            }
                          ...
                          if (update_flags & update_hessians)
                            {
                              ...
                              for (unsigned int i = 0; i < scratch_data.patch_values_scalar.solution_hessians.size();
                                   ++i)
                                {
                                  AssertDimension(scratch_data.patch_values_system.solution_hessians[i].size(), 2);
                                  scratch_data.patch_values_system.solution_hessians[i][0] =
                          ...
                          if (update_flags & update_values)
                            {
                              ...
                              for (unsigned int i = 0; i < scratch_data.patch_values_scalar.solution_values.size(); ++i)
                                {
                                  AssertDimension(scratch_data.patch_values_system.solution_values[i].size(), 2);
                                  scratch_data.patch_values_system.solution_values[i][1] =
                                    scratch_data.patch_values_scalar.solution_values[i];
                                }
                            }
                          ...
                          if (update_flags & update_gradients)
                            {
                              ...
                              for (unsigned int i = 0; i < scratch_data.patch_values_scalar.solution_gradients.size();
                                ...
                                {
                                  AssertDimension(scratch_data.patch_values_system.solution_values[i].size(), 2);
                                  scratch_data.patch_values_system.solution_gradients[i][1] =
                                    scratch_data.patch_values_scalar.solution_gradients[i];
                                }
                            }
                          ...
                          if (update_flags & update_hessians)
                            {
                              ...
                              for (unsigned int i = 0; i < scratch_data.patch_values_scalar.solution_hessians.size();
                                ...
                                {
                                  AssertDimension(scratch_data.patch_values_system.solution_hessians[i].size(), 2);
                                  scratch_data.patch_values_system.solution_hessians[i][1] =
                                    scratch_data.patch_values_scalar.solution_hessians[i];
                                }
                            }
                        }
                      else
                        {
                          ...
                          if (update_flags & update_values)
                            {
                              std::vector<Vector<double>> tmp(scratch_data.patch_values_system.solution_values.size(),
                              ...
                              for (unsigned int i = 0; i < scratch_data.patch_values_system.solution_values.size(); ++i)
                                {
                                  AssertDimension(scratch_data.patch_values_system.solution_values[i].size(),
                                                  2 * n_components);
                                  for (unsigned int j = 0; j < n_components; ++j)
                                    scratch_data.patch_values_system.solution_values[i][j] = tmp[i][j];
                                }
                              ...
                              for (unsigned int i = 0; i < scratch_data.patch_values_system.solution_values.size(); ++i)
                                {
                                  for (unsigned int j = 0; j < n_components; ++j)
                                    scratch_data.patch_values_system.solution_values[i][j + n_components] = tmp[i][j];
                                }
                            }
                          ...
                          if (update_flags & update_gradients)
                            {
                              std::vector<std::vector<Tensor<1, spacedim>>> tmp(
                                scratch_data.patch_values_system.solution_gradients.size(),
                              ...
                              for (unsigned int i = 0; i < scratch_data.patch_values_system.solution_gradients.size();
                                   ++i)
                                {
                                  AssertDimension(scratch_data.patch_values_system.solution_gradients[i].size(),
                                                  2 * n_components);
                                  for (unsigned int j = 0; j < n_components; ++j)
                                    scratch_data.patch_values_system.solution_gradients[i][j] = tmp[i][j];
                                }
                              ...
                              for (unsigned int i = 0; i < scratch_data.patch_values_system.solution_gradients.size();
                                ...
                                {
                                  for (unsigned int j = 0; j < n_components; ++j)
                                    scratch_data.patch_values_system.solution_gradients[i][j + n_components] =
                                      tmp[i][j];
                                }
                            }
                          ...
                          if (update_flags & update_hessians)
                            {
                              ...
                              for (unsigned int i = 0; i < scratch_data.patch_values_system.solution_hessians.size();
                                   ++i)
                                {
                                  AssertDimension(scratch_data.patch_values_system.solution_hessians[i].size(),
                                                  2 * n_components);
                                  for (unsigned int j = 0; j < n_components; ++j)
                                    scratch_data.patch_values_system.solution_hessians[i][j] = tmp[i][j];
                                }
                              ...
                              for (unsigned int i = 0; i < scratch_data.patch_values_system.solution_hessians.size();
                                ...
                                {
                                  for (unsigned int j = 0; j < n_components; ++j)
                                    scratch_data.patch_values_system.solution_hessians[i][j + n_components] = tmp[i][j];
                                }
                            }
                        }
                    }
                  ...
                  const typename DoFHandler<dim, spacedim>::cell_iterator dh_cell(
                    ...
                    cell_and_index->first->index(),
                    dataset->dof_handler);
                  scratch_data.patch_values_system.template set_cell<dim>(dh_cell);
              ...
              for (unsigned int q = 0; q < n_q_points; ++q)
                for (unsigned int component = 0; component < dataset->n_output_variables; ++component)
                  patch.data(offset + component, q) = scratch_data.postprocessed_values[dataset_number][q](component);
              ...
              else
                {
                  ...
                  if (dataset->is_complex_valued() == false)
                    ...
                    // DataOut_DoFData::get_nonscalar_data_ranges().
                    {
                      ...
                      {
                        Assert(dataset->data_component_interpretation.size() == n_components, ExcInternalError());

                        unsigned int destination = offset;
                        for (unsigned int component = 0; component < n_components;
                             /* component is updated below */)
                          {
                            switch (dataset->data_component_interpretation[component])
                              {
                                    ...
                                    for (unsigned int c = 0; c < size; ++c)
                                      for (unsigned int q = 0; q < n_q_points; ++q)
                                        patch.data(destination + c, q) =
                                          scratch_data.patch_values_system.solution_values[q](component + c);
  ...
  for (const unsigned int f : cell_and_index->first->face_indices())
    {
      ...
      Assert(static_cast<unsigned int>(neighbor->level()) < scratch_data.cell_to_patch_index_map->size(),
             ExcInternalError());
      ...
      // neighbor index
      patch.neighbors[f] = (*scratch_data.cell_to_patch_index_map)[neighbor->level()][neighbor->index()];
    }
...
template <int dim, int spacedim>
void
DataOut<dim, spacedim>::build_patches(const hp::MappingCollection<dim, spacedim> &mapping,
                                      const unsigned int                          n_subdivisions_,
                                      const CurvedCellRegion                      curved_region)
{
  ...
  for (unsigned int l = 0; l < this->triangulation->n_levels(); ++l)
    {
      ...
      for (cell_iterator cell = first_cell_function(*this->triangulation); cell != this->triangulation->end();
      ...
      cell_to_patch_index_map[l].resize(max_index + 1, dealii::DataOutBase::Patch<dim, spacedim>::no_neighbor);
    }
  ...
  std::vector<std::pair<cell_iterator, unsigned int>> all_cells;
  {
    ...
    for (; cell != this->triangulation->end(); cell = next_cell_function(*this->triangulation, cell))
      {
        ...
        Assert(static_cast<unsigned int>(cell->level()) < cell_to_patch_index_map.size(), ExcInternalError());
        Assert(static_cast<unsigned int>(cell->index()) < cell_to_patch_index_map[cell->level()].size(),
               ExcInternalError());
  ...
  for (unsigned int i = 0; i < this->cell_data.size(); ++i)
    n_datasets += (this->cell_data[i]->is_complex_valued() && (this->cell_data[i]->postprocessor == nullptr) ? 2 : 1);
  for (unsigned int i = 0; i < this->dof_data.size(); ++i)
    n_datasets += (this->dof_data[i]->n_output_variables *
                   (this->dof_data[i]->is_complex_valued() && (this->dof_data[i]->postprocessor == nullptr) ? 2 : 1));
  ...
  const CurvedCellRegion curved_cell_region = (n_subdivisions < 2 ? no_curved_cells : curved_region);
  ...
  internal::DataOutImplementation::ParallelData<dim, spacedim> thread_data(n_datasets,
                                                                           n_subdivisions,
                                                                           n_postprocessor_outputs,
                                                                           mapping,
                                                                           this->get_fes(),
  ...
  auto worker = [this, n_subdivisions, curved_cell_region](
                  ...
                  int) { this->build_one_patch(cell_and_index, scratch_data, n_subdivisions, curved_cell_region); };
...
template <int dim, int spacedim>
void
DataOut<dim, spacedim>::set_cell_selection(const FilteredIterator<cell_iterator> &filtered_iterator)
{
  const auto first_cell = [filtered_iterator](const Triangulation<dim, spacedim> &triangulation) {
  ...
  const auto next_cell = [filtered_iterator](const Triangulation<dim, spacedim> &, const cell_iterator &cell) {
    // Create a copy of the filtered iterator so that we can
    // call a non-const function -- though we are really only
    // interested in the return value of that function, not the
    // state of the object
...
template <int dim, int spacedim>
std::pair<typename DataOut<dim, spacedim>::FirstCellFunctionType, typename DataOut<dim, spacedim>::NextCellFunctionType>

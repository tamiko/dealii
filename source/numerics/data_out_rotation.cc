...
namespace internal
{
  namespace DataOutRotationImplementation
  {
    ...
    append_patch_to_list(
      const std::vector<DataOutBase::Patch<DataOutRotation<dim, spacedim>::patch_dim,
                                           DataOutRotation<dim, spacedim>::patch_spacedim>> &new_patches,
      std::vector<DataOutBase::Patch<DataOutRotation<dim, spacedim>::patch_dim,
                                     DataOutRotation<dim, spacedim>::patch_spacedim>>       &patches)
    {
  ...
  std::vector<DataOutBase::Patch<patch_dim, patch_spacedim>>           &my_patches)
{
  ...
  for (unsigned int i = 0; i <= n_patches_per_circle; ++i)
    {
      angle_directions[i][dim - 1] = std::cos(2 * numbers::PI * i / n_patches_per_circle);
      angle_directions[i][dim]     = std::sin(2 * numbers::PI * i / n_patches_per_circle);
    }
              ...
              for (const unsigned int vertex : GeometryInfo<dim>::vertex_indices())
                {
                  ...
                  my_patches[angle].vertices[vertex + GeometryInfo<dim>::vertices_per_cell] =
                    v(0) * angle_directions[angle + 1];
                  my_patches[angle].vertices[vertex + GeometryInfo<dim>::vertices_per_cell][0] = v(1);
                }
      ...
      if (data.n_datasets > 0)
        {
          ...
          for (unsigned int dataset = 0; dataset < this->dof_data.size(); ++dataset)
            {
              const FEValuesBase<dim>      &fe_patch_values = data.get_present_fe_values(dataset);
                              ...
                              for (unsigned int y = 0; y < n_points; ++y)
                                for (unsigned int z = 0; z < n_points; ++z)
                                  my_patches[angle].data(offset + component,
                                                         x * n_points * n_points + y * n_points + z) =
                  ...
                  switch (dim)
                    {
                      ...
                      case 2:
                        for (unsigned int x = 0; x < n_points; ++x)
                          for (unsigned int y = 0; y < n_points; ++y)
                            for (unsigned int z = 0; z < n_points; ++z)
                              my_patches[angle].data(offset, x * n_points * n_points + y + z * n_points) =
          ...
          for (unsigned int dataset = 0; dataset < this->cell_data.size(); ++dataset)
            {
              ...
              Assert((*cell)->is_active(), ExcMessage("Cell must be active for cell data"));
              ...
              switch (dim)
                {
                  ...
                  case 2:
                    for (unsigned int x = 0; x < n_points; ++x)
                      for (unsigned int y = 0; y < n_points; ++y)
                        for (unsigned int z = 0; z < n_points; ++z)
                          my_patches[angle].data(dataset + offset, x * n_points * n_points + y * n_points + z) = value;
                    break;
...
template <int dim, int spacedim>
void
DataOutRotation<dim, spacedim>::build_patches(const unsigned int n_patches_per_circle,
                                              const unsigned int nnnn_subdivisions)
{
  ...
  const unsigned int n_subdivisions = (nnnn_subdivisions != 0) ? nnnn_subdivisions : this->default_subdivisions;
  ...
  for (cell_iterator cell = first_cell(); cell != this->triangulation->end(); cell = next_cell(cell))
  ...
  internal::DataOutRotationImplementation::ParallelData<dim, spacedim> thread_data(
    n_datasets,
    n_subdivisions,
    ...
    this->get_fes(),
    update_flags);
  std::vector<DataOutBase::Patch<patch_dim, patch_spacedim>> new_patches(n_patches_per_circle);
  for (unsigned int i = 0; i < new_patches.size(); ++i)
    {
      new_patches[i].n_subdivisions = n_subdivisions;
      new_patches[i].reference_cell = ReferenceCells::get_hypercube<dim + 1>();

      new_patches[i].data.reinit(n_datasets, Utilities::fixed_power<patch_dim>(n_subdivisions + 1));
    }
  ...
  WorkStream::run(
    ...
    [this](const std::vector<DataOutBase::Patch<patch_dim, patch_spacedim>> &new_patches) {
      internal::DataOutRotationImplementation::append_patch_to_list<dim, spacedim>(new_patches, this->patches);
    },
    thread_data,
    new_patches);
}
...
DataOutRotation<dim, spacedim>::next_cell(const cell_iterator &cell)
{
  ...
  typename Triangulation<dim, spacedim>::active_cell_iterator active_cell = cell;
  ++active_cell;
  return active_cell;
}

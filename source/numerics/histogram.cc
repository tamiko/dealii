...
{
  return (((n1 < n2) && (n1 > 0)) || ((n1 < n2) && (n2 <= 0)) || ((n2 < n1) && (n1 > 0) && (n2 <= 0)));
}
...
{
  Assert(values.size() > 0, ExcMessage("Your input data needs to contain at least one input vector."));
  ...
  Assert(values.size() == y_values_.size(), ExcIncompatibleArraySize(values.size(), y_values_.size()));
  ...
  switch (interval_spacing)
    {
      case linear:
        {
          ...
          for (unsigned int i = 1; i < values.size(); ++i)
            {
              min_value = std::min(min_value, *std::min_element(values[i].begin(), values[i].end()));
              max_value = std::max(max_value, *std::max_element(values[i].begin(), values[i].end()));
            }
      ...
      case logarithmic:
        {
          const auto logarithmic_less_function = &Histogram::template logarithmic_less<number>;

          min_value = *std::min_element(values[0].begin(), values[0].end(), logarithmic_less_function);

          max_value = *std::max_element(values[0].begin(), values[0].end(), logarithmic_less_function);
    ...
    for (int i = intervals.size() - 1; i >= 0; --i)
      {
        for (unsigned int n = 0; n < intervals[i].size(); ++n)
          out << intervals[i][n].left_point << ' '
              << (i < static_cast<int>(intervals.size()) - 1 ? y_values[i + 1] :
        ...
        out << std::endl;
        for (unsigned int n = 0; n < intervals[i].size(); ++n)
          out << intervals[i][n].left_point << ' ' << y_values[i] << ' ' << intervals[i][n].content << std::endl
              << intervals[i][n].right_point << ' ' << y_values[i] << ' ' << intervals[i][n].content << std::endl;
...
{
  return (MemoryConsumption::memory_consumption(intervals) + MemoryConsumption::memory_consumption(y_values));
}

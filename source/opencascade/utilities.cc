...
namespace OpenCASCADE
{
  ...
  point(const gp_Pnt &p, const double tolerance)
  {
    ...
    switch (spacedim)
      {
        case 1:
          Assert(std::abs(p.Y()) <= tolerance, ExcMessage("Cannot convert OpenCASCADE point to 1d if p.Y() != 0."));
          Assert(std::abs(p.Z()) <= tolerance, ExcMessage("Cannot convert OpenCASCADE point to 1d if p.Z() != 0."));
          return Point<spacedim>(p.X());
        case 2:
          Assert(std::abs(p.Z()) <= tolerance, ExcMessage("Cannot convert OpenCASCADE point to 2d if p.Z() != 0."));
  ...
  template <int dim>
  bool
  point_compare(const Point<dim> &p1, const Point<dim> &p2, const Tensor<1, dim> &direction, const double tolerance)
  {
            ...
            const bool          in_parallel)
  {
    ...
    const bool   mesh_is_present     = std::none_of(faces.begin(), faces.end(), [&Loc](const TopoDS_Face &face) {
    ...
    if (!mesh_is_present)
      {
        ...
        // `shape_to_be_written`.
        BRepMesh_IncrementalMesh mesh_im(shape_to_be_written, deflection, is_relative, angular_deflection, in_parallel);
      }
...
#    if !DEAL_II_OPENCASCADE_VERSION_GTE(7, 2, 0)
    AssertThrow(error == StlAPI_StatusOK, ExcMessage("Error writing STL from shape."));
#    else
...
#  endif
                      ...
                      const double             tolerance)
  {
    ...
    if (direction * direction > 0)
      {
        std::sort(curve_points.begin(), curve_points.end(), [&](const Point<dim> &p1, const Point<dim> &p2) {
  ...
  project_point_and_pull_back(const TopoDS_Shape &in_shape, const Point<dim> &origin, const double tolerance)
  {
    TopExp_Explorer exp;
  ...
  {
    std::tuple<Point<dim>, TopoDS_Shape, double, double> ref = project_point_and_pull_back(in_shape, origin, tolerance);
    return std::get<0>(ref);
  }

  std::tuple<Point<3>, Tensor<1, 3>, double, double>
  closest_point_and_differential_forms(const TopoDS_Shape &in_shape, const Point<3> &origin, const double tolerance)

...
namespace Particles
{
  ...
  DataOut<dim, spacedim>::build_patches(
    ...
    const std::vector<DataComponentInterpretation::DataComponentInterpretation> &data_component_interpretations_)
  {
    Assert(data_component_names.size() == data_component_interpretations_.size(),
           ExcMessage("When calling Particles::DataOut::build_patches with data component "
    ...
    if ((data_component_names.size() > 0) && (particles.n_locally_owned_particles() > 0))
      {
        Assert(particles.begin()->has_properties(),
               ExcMessage("You called Particles::DataOut::build_patches with data component "
                          "names and interpretations, but the particles do not seem to own "
                          "any properties."));

        Assert(data_component_names.size() == particles.begin()->get_properties().size(),
               ExcMessage("When calling Particles::DataOut::build_patches with data component "
                          "names and interpretations you need to provide as many data component "
    ...
    dataset_names.insert(dataset_names.end(), data_component_names.begin(), data_component_names.end());

    data_component_interpretations.clear();
    data_component_interpretations.emplace_back(DataComponentInterpretation::component_is_scalar);
  ...
  DataOut<dim, spacedim>::get_nonscalar_data_ranges() const
  {
    ...
    Assert(dataset_names.size() == data_component_interpretations.size(), ExcInternalError());

    // collect the ranges of particle data
    const unsigned int n_output_components = data_component_interpretations.size();
          ...
          case DataComponentInterpretation::component_is_part_of_vector:
            {
              ...
              Assert(i + spacedim <= n_output_components,
                     Exceptions::DataOutImplementation::ExcInvalidVectorDeclaration(i, dataset_names[i]));
              for (unsigned int dd = 1; dd < spacedim; ++dd)
                Assert(data_component_interpretations[i + dd] ==
          ...
          case DataComponentInterpretation::component_is_part_of_tensor:
            {
              ...
              Assert(i + size <= n_output_components,
                     Exceptions::DataOutImplementation::ExcInvalidTensorDeclaration(i, dataset_names[i]));

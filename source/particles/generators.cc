...
namespace Particles
{
  namespace Generators
  {
    namespace
    {
      ...
      std::vector<double>
      compute_local_cumulative_cell_weights(const Triangulation<dim, spacedim> &triangulation,
                                            const Mapping<dim, spacedim>       &mapping,
                                            const Function<spacedim>           &probability_density_function)
      {
        std::vector<double> cumulative_cell_weights(triangulation.n_active_cells());
            ...
            if (cell->is_locally_owned())
              {
                ...
                const double quadrature_point_weight =
                  probability_density_function.value(fe_values.get_quadrature_points()[0]);

                AssertThrow(quadrature_point_weight >= 0.0,
                            ProbabilityFunctionNegative<dim>(quadrature_formula.point(0)));

                // get_cell_weight makes sure to return positive values
                cumulative_weight += quadrature_point_weight * fe_values.JxW(0);
              }
            cumulative_cell_weights[cell->active_cell_index()] = cumulative_weight;
          }
      ...
      random_location_in_cell(const typename Triangulation<dim, spacedim>::active_cell_iterator &cell,
                              const Mapping<dim, spacedim>                                      &mapping,
                              std::mt19937 &random_number_generator)
      {
        ...
        const std::pair<Point<spacedim>, Point<spacedim>> &cell_bounds(cell_bounding_box.get_boundary_points());
        ...
        for (unsigned int i = 0; i < n_attempts; ++i)
          {
            ...
            for (unsigned int d = 0; d < spacedim; ++d)
              {
                position[d] =
                  uniform_distribution_01(random_number_generator) * (cell_bounds.second[d] - cell_bounds.first[d]) +
                  cell_bounds.first[d];
              }

            try
              {
                const Point<dim> reference_position = mapping.transform_real_to_unit_cell(cell, position);
        ...
        for (unsigned int d = 0; d < dim; ++d)
          reference_position[d] = uniform_distribution_01(random_number_generator);

        const Point<spacedim> position = mapping.transform_unit_to_real_cell(cell, reference_position);
    ...
    template <int dim, int spacedim>
    void
    regular_reference_locations(const Triangulation<dim, spacedim> &triangulation,
                                ...
                                const Mapping<dim, spacedim>       &mapping)
    {
      ...
      if (const auto tria = dynamic_cast<const parallel::TriangulationBase<dim, spacedim> *>(&triangulation))
        {
          n_particles_to_generate = tria->n_locally_owned_active_cells() * particle_reference_locations.size();
      ...
      particle_handler.reserve(particle_handler.n_locally_owned_particles() + n_particles_to_generate);
      ...
      for (const auto &cell : triangulation.active_cell_iterators() | IteratorFilters::LocallyOwnedCell())
        {
          for (const auto &reference_location : particle_reference_locations)
            {
              const Point<spacedim> position_real = mapping.transform_unit_to_real_cell(cell, reference_location);

              particle_handler.insert_particle(position_real, reference_location, particle_index, cell);
              ++particle_index;
            }
        }
    ...
    random_particle_in_cell(const typename Triangulation<dim, spacedim>::active_cell_iterator &cell,
                            const types::particle_index                                        id,
                            std::mt19937                                                      &random_number_generator,
                            const Mapping<dim, spacedim>                                      &mapping)
    {
      const auto position_and_reference_position = random_location_in_cell(cell, mapping, random_number_generator);
      return Particle<dim, spacedim>(position_and_reference_position.first, position_and_reference_position.second, id);
    }
    ...
    random_particle_in_cell_insert(const typename Triangulation<dim, spacedim>::active_cell_iterator &cell,
                                   const types::particle_index                                        id,
                                   std::mt19937                   &random_number_generator,
                                   ParticleHandler<dim, spacedim> &particle_handler,
                                   const Mapping<dim, spacedim>   &mapping)
    {
      const auto position_and_reference_position = random_location_in_cell(cell, mapping, random_number_generator);
      return particle_handler.insert_particle(position_and_reference_position.first,
    ...
    template <int dim, int spacedim>
    void
    probabilistic_locations(const Triangulation<dim, spacedim> &triangulation,
                            const Function<spacedim>           &probability_density_function,
                            ...
                            const unsigned int                  random_number_seed)
    {
      ...
      std::vector<types::particle_index> particles_per_cell(triangulation.n_active_cells(), 0);
      ...
      // First determine how many particles to generate in which cell
      {
        ...
        const std::vector<double> cumulative_cell_weights =
          compute_local_cumulative_cell_weights(triangulation, mapping, probability_density_function);

        // Sum the local integrals over all nodes
        double local_weight_integral = (cumulative_cell_weights.size() > 0) ? cumulative_cell_weights.back() : 0.0;

        double global_weight_integral;

        if (const auto tria = dynamic_cast<const parallel::TriangulationBase<dim, spacedim> *>(&triangulation))
          {
            global_weight_integral = Utilities::MPI::sum(local_weight_integral, tria->get_communicator());
          }
        else
          {
        ...
        if (const auto tria = dynamic_cast<const parallel::TriangulationBase<dim, spacedim> *>(&triangulation))
          {
            const int ierr =
              MPI_Exscan(&local_weight_integral, &local_start_weight, 1, MPI_DOUBLE, MPI_SUM, tria->get_communicator());
            AssertThrowMPI(ierr);
          }
#endif
        ...
        // Calculate start id
        start_particle_id =
          std::llround(static_cast<double>(n_particles_to_create) * local_start_weight / global_weight_integral);
        ...
        const types::particle_index end_particle_id =
          std::llround(static_cast<double>(n_particles_to_create) *
                       ((local_start_weight + local_weight_integral) / global_weight_integral));
        ...
        if (random_cell_selection)
          {
            ...
            for (types::particle_index current_particle_index = 0; current_particle_index < n_local_particles;
                 ++current_particle_index)
              {
                // Draw the random number that determines the cell of the
                // particle
                const double random_weight = uniform_distribution(random_number_generator);

                const std::vector<double>::const_iterator selected_cell =
                  std::lower_bound(cumulative_cell_weights.begin(), cumulative_cell_weights.end(), random_weight);
                const unsigned int cell_index = std::distance(cumulative_cell_weights.begin(), selected_cell);
            ...
            for (const auto &cell : triangulation.active_cell_iterators() | IteratorFilters::LocallyOwnedCell())
              {
                const types::particle_index cumulative_particles_to_create =
                  std::llround(static_cast<double>(n_local_particles) *
                               cumulative_cell_weights[cell->active_cell_index()] / local_weight_integral);
                ...
                particles_per_cell[cell->active_cell_index()] = cumulative_particles_to_create - particles_created;
                particles_created += particles_per_cell[cell->active_cell_index()];
              }
          }
      }
      ...
      {
        particle_handler.reserve(particle_handler.n_locally_owned_particles() + n_local_particles);
        unsigned int current_particle_index = start_particle_id;

        for (const auto &cell : triangulation.active_cell_iterators() | IteratorFilters::LocallyOwnedCell())
          {
            for (unsigned int i = 0; i < particles_per_cell[cell->active_cell_index()]; ++i)
              {
                random_particle_in_cell_insert(
                  cell, current_particle_index, random_number_generator, particle_handler, mapping);
                       ...
                       const std::vector<std::vector<double>>                &properties)
    {
      ...
      const ComponentMask mask = (components.size() == 0 ? ComponentMask(fe.n_components(), true) : components);

      const std::map<types::global_dof_index, Point<spacedim>> support_points_map =
      ...
      particle_handler.insert_global_particles(support_points_vec, global_bounding_boxes, properties);
    }
    ...
    quadrature_points(const Triangulation<dim, spacedim> &triangulation,
                      ...
                      const std::vector<std::vector<double>>                &properties)
    {
      const std::vector<Point<dim>> &particle_reference_locations = quadrature.get_points();
      std::vector<Point<spacedim>>   points_to_generate;
      points_to_generate.reserve(particle_reference_locations.size() * triangulation.n_active_cells());
      ...
      particle_handler.insert_global_particles(points_to_generate, global_bounding_boxes, properties);
    }
  } // namespace Generators
} // namespace Particles

...
struct FunctionsTestSymmetricTensorTensorCoupled
{
  ...
  static NumberType
  psi(const SymmetricTensor<2, dim, NumberType> &t1, const Tensor<2, dim, NumberType> &t2)
  {
    return pow(det_t(t1), 2) * pow(det_t(t2), 3);
  };

  static SymmetricTensor<2, dim, NumberType>
  dpsi_dt1(const SymmetricTensor<2, dim, NumberType> &t1, const Tensor<2, dim, NumberType> &t2)
  {
    return SymmetricTensor<2, dim, NumberType>(2.0 * pow(det_t(t1), 1) * ddet_t_dt(t1) * pow(det_t(t2), 3));
  };

  static Tensor<2, dim, NumberType>
  dpsi_dt2(const SymmetricTensor<2, dim, NumberType> &t1, const Tensor<2, dim, NumberType> &t2)
  {
    return pow(det_t(t1), 2) * 3.0 * pow(det_t(t2), 2) * ddet_t_dt(t2);
  };

  static SymmetricTensor<4, dim, NumberType>
  d2psi_dt1_dt1(const SymmetricTensor<2, dim, NumberType> &t1, const Tensor<2, dim, NumberType> &t2)
  {
  ...
  static Tensor<4, dim, NumberType>
  d2psi_dt2_dt1(const SymmetricTensor<2, dim, NumberType> &t1, const Tensor<2, dim, NumberType> &t2)
  {
    return 2.0 * pow(det_t(t1), 1) * 3.0 * pow(det_t(t2), 2) *
           outer_product(static_cast<Tensor<2, dim, NumberType>>(ddet_t_dt(t1)), ddet_t_dt(t2));
  };

  static Tensor<4, dim, NumberType>
  d2psi_dt1_dt2(const SymmetricTensor<2, dim, NumberType> &t1, const Tensor<2, dim, NumberType> &t2)
  {
    return 2.0 * pow(det_t(t1), 1) * 3.0 * pow(det_t(t2), 2) *
           outer_product(ddet_t_dt(t2), static_cast<Tensor<2, dim, NumberType>>(ddet_t_dt(t1)));
  };

  static Tensor<4, dim, NumberType>
  d2psi_dt2_dt2(const SymmetricTensor<2, dim, NumberType> &t1, const Tensor<2, dim, NumberType> &t2)
  {
...
test_symmetric_tensor_tensor_coupled()
{
  ...
  const FEValuesExtractors::Tensor<2>          t2_dof(SymmetricTensor<2, dim>::n_independent_components);
  const unsigned int                           n_AD_components =
    SymmetricTensor<2, dim>::n_independent_components + Tensor<2, dim>::n_independent_components;
  ...
  Tensor<2, dim, ScalarNumberType> t2 = unit_symmetric_tensor<dim, ScalarNumberType>();
  ...
  if (is_recording == true)
    {
      ...
      const Tensor<2, dim, ADNumberType>          t2_ad = ad_helper.get_sensitive_variables(t2_dof);
  ...
  const Tensor<2, dim, ScalarNumberType>          dpsi_dt2 = ad_helper.extract_gradient_component(Dpsi, t2_dof);
  ...
  if (AD::ADNumberTraits<ADNumberType>::n_supported_derivative_levels >= 2)
    {
      ...
      const Tensor<4, dim, ScalarNumberType> d2psi_dt2_dt1 = ad_helper.extract_hessian_component(D2psi, t1_dof, t2_dof);
      const Tensor<4, dim, ScalarNumberType> d2psi_dt1_dt2 = ad_helper.extract_hessian_component(D2psi, t2_dof, t1_dof);
      const Tensor<4, dim, ScalarNumberType> d2psi_dt2_dt2 = ad_helper.extract_hessian_component(D2psi, t2_dof, t2_dof);

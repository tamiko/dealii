...
struct FunctionsTestSymmetricTensorSymmetricTensorCoupled
{
  ...
  static NumberType
  psi(const SymmetricTensor<2, dim, NumberType> &t1, const SymmetricTensor<2, dim, NumberType> &t2)
  {
    return pow(det_t(t1), 2) * pow(det_t(t2), 3);
  };

  static SymmetricTensor<2, dim, NumberType>
  dpsi_dt1(const SymmetricTensor<2, dim, NumberType> &t1, const SymmetricTensor<2, dim, NumberType> &t2)
  {
    return SymmetricTensor<2, dim, NumberType>(2.0 * pow(det_t(t1), 1) * ddet_t_dt(t1) * pow(det_t(t2), 3));
  };

  static SymmetricTensor<2, dim, NumberType>
  dpsi_dt2(const SymmetricTensor<2, dim, NumberType> &t1, const SymmetricTensor<2, dim, NumberType> &t2)
  {
    return SymmetricTensor<2, dim, NumberType>(pow(det_t(t1), 2) * 3.0 * pow(det_t(t2), 2) * ddet_t_dt(t2));
  };

  static SymmetricTensor<4, dim, NumberType>
  d2psi_dt1_dt1(const SymmetricTensor<2, dim, NumberType> &t1, const SymmetricTensor<2, dim, NumberType> &t2)
  {
    return SymmetricTensor<4, dim, NumberType>(
      2.0 * pow(det_t(t2), 3) *
      (pow(det_t(t1), 0) * outer_product(ddet_t_dt(t1), ddet_t_dt(t1)) + pow(det_t(t1), 1) * d2det_t_dt_dt(t1)));
  };

  static SymmetricTensor<4, dim, NumberType>
  d2psi_dt2_dt1(const SymmetricTensor<2, dim, NumberType> &t1, const SymmetricTensor<2, dim, NumberType> &t2)
  {
    return SymmetricTensor<4, dim, NumberType>(2.0 * pow(det_t(t1), 1) * 3.0 * pow(det_t(t2), 2) *
                                               outer_product(ddet_t_dt(t1), ddet_t_dt(t2)));
  };

  static SymmetricTensor<4, dim, NumberType>
  d2psi_dt1_dt2(const SymmetricTensor<2, dim, NumberType> &t1, const SymmetricTensor<2, dim, NumberType> &t2)
  {
    return SymmetricTensor<4, dim, NumberType>(2.0 * pow(det_t(t1), 1) * 3.0 * pow(det_t(t2), 2) *
  ...
  static SymmetricTensor<4, dim, NumberType>
  d2psi_dt2_dt2(const SymmetricTensor<2, dim, NumberType> &t1, const SymmetricTensor<2, dim, NumberType> &t2)
  {
    return SymmetricTensor<4, dim, NumberType>(
      pow(det_t(t1), 2) * 3.0 *
      (2.0 * pow(det_t(t2), 1) * outer_product(ddet_t_dt(t2), ddet_t_dt(t2)) + pow(det_t(t2), 2) * d2det_t_dt_dt(t2)));
  };
};
...
test_symmetric_tensor_symmetric_tensor_coupled()
{
  ...
  std::cout << "*** Test variables: SymmetricTensor + SymmetricTensor (coupled), "
  ...
  // Function and its derivatives
  using func_ad = FunctionsTestSymmetricTensorSymmetricTensorCoupled<dim, ADNumberType>;

  // Setup the variable components and choose a value at which to
  // evaluate the tape
  const FEValuesExtractors::SymmetricTensor<2> t1_dof(0);
  const FEValuesExtractors::SymmetricTensor<2> t2_dof(SymmetricTensor<2, dim>::n_independent_components);
  const unsigned int                           n_AD_components =
    SymmetricTensor<2, dim>::n_independent_components + SymmetricTensor<2, dim>::n_independent_components;
  ADHelper ad_helper(n_AD_components);
  ad_helper.set_tape_buffer_sizes(); // Increase the buffer size from the
                                     // default values

  SymmetricTensor<2, dim, ScalarNumberType> t1 = unit_symmetric_tensor<dim, ScalarNumberType>();
  for (unsigned int i = 0; i < t1.n_independent_components; ++i)
    t1[t1.unrolled_to_component_indices(i)] += 0.135 * (i + 0.195);
  SymmetricTensor<2, dim, ScalarNumberType> t2 = unit_symmetric_tensor<dim, ScalarNumberType>();
  ...
  if (is_recording == true)
    {
      ...
      const SymmetricTensor<2, dim, ADNumberType> t1_ad = ad_helper.get_sensitive_variables(t1_dof);
      const SymmetricTensor<2, dim, ADNumberType> t2_ad = ad_helper.get_sensitive_variables(t2_dof);
  ...
  const SymmetricTensor<2, dim, ScalarNumberType> dpsi_dt1 = ad_helper.extract_gradient_component(Dpsi, t1_dof);
  const SymmetricTensor<2, dim, ScalarNumberType> dpsi_dt2 = ad_helper.extract_gradient_component(Dpsi, t2_dof);
  std::cout << "extracted Dpsi (t1): " << dpsi_dt1 << "\n"
            << "extracted Dpsi (t2): " << dpsi_dt2 << "\n";

  // Verify the result
  using func                        = FunctionsTestSymmetricTensorSymmetricTensorCoupled<dim, ScalarNumberType>;
  ...
  std::cout << "diff: " << std::abs((dpsi_dt1 - func::dpsi_dt1(t1, t2)).norm()) << std::endl;
  std::cout << "dpsi_dt2:            " << dpsi_dt2 << std::endl;
  std::cout << "func::dpsi_dt2(t1,t2): " << func::dpsi_dt2(t1, t2) << std::endl;
  std::cout << "diff: " << std::abs((dpsi_dt2 - func::dpsi_dt2(t1, t2)).norm()) << std::endl;
  Assert(std::abs(psi - func::psi(t1, t2)) < tol, ExcMessage("No match for function value."));
  Assert(std::abs((dpsi_dt1 - func::dpsi_dt1(t1, t2)).norm()) < tol, ExcMessage("No match for first derivative."));
  Assert(std::abs((dpsi_dt2 - func::dpsi_dt2(t1, t2)).norm()) < tol, ExcMessage("No match for first derivative."));
  if (AD::ADNumberTraits<ADNumberType>::n_supported_derivative_levels >= 2)
    {
      ...
      std::cout << "func::d2psi_dt1_dt1(t1,t2): " << func::d2psi_dt1_dt1(t1, t2) << std::endl;
      std::cout << "diff: " << std::abs((d2psi_dt1_dt1 - func::d2psi_dt1_dt1(t1, t2)).norm()) << std::endl;
      std::cout << "d2psi_dt2_dt1:            " << d2psi_dt2_dt1 << std::endl;
      std::cout << "func::d2psi_dt2_dt1(t1,t2): " << func::d2psi_dt2_dt1(t1, t2) << std::endl;
      std::cout << "diff: " << std::abs((d2psi_dt2_dt1 - func::d2psi_dt2_dt1(t1, t2)).norm()) << std::endl;
      std::cout << "d2psi_dt1_dt2:            " << d2psi_dt1_dt2 << std::endl;
      std::cout << "func::d2psi_dt1_dt2(t1,t2): " << func::d2psi_dt1_dt2(t1, t2) << std::endl;
      std::cout << "diff: " << std::abs((d2psi_dt1_dt2 - func::d2psi_dt1_dt2(t1, t2)).norm()) << std::endl;
      std::cout << "d2psi_dt2_dt2:            " << d2psi_dt2_dt2 << std::endl;
      std::cout << "func::d2psi_dt2_dt2(t1,t2): " << func::d2psi_dt2_dt2(t1, t2) << std::endl;
      std::cout << "diff: " << std::abs((d2psi_dt2_dt2 - func::d2psi_dt2_dt2(t1, t2)).norm()) << std::endl;
      Assert(std::abs((d2psi_dt1_dt1 - func::d2psi_dt1_dt1(t1, t2)).norm()) < tol,
             ExcMessage("No match for second derivative."));
      Assert(std::abs((d2psi_dt2_dt1 - func::d2psi_dt2_dt1(t1, t2)).norm()) < tol,
             ExcMessage("No match for second derivative."));
      Assert(std::abs((d2psi_dt1_dt2 - func::d2psi_dt1_dt2(t1, t2)).norm()) < tol,
             ExcMessage("No match for second derivative."));
      Assert(std::abs((d2psi_dt2_dt2 - func::d2psi_dt2_dt2(t1, t2)).norm()) < tol,
             ExcMessage("No match for second derivative."));
    }
}

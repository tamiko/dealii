...
namespace Step44
{
  ...
  public:
    ...
    get_tau(const Tensor<2, dim, NumberType> &F, const NumberType &p_tilde) const
    {
  ...
  protected:
    ...
    template <typename NumberType>
    SymmetricTensor<2, dim, NumberType>
    get_tau_vol(const Tensor<2, dim, NumberType> &F, const NumberType &p_tilde) const
    {
  ...
  template <int dim, typename number_t, enum AD::NumberTypes ad_type_code>
  void
  Solid<dim, number_t, ad_type_code>::print_conv_footer(const BlockVector<double> &solution_delta)
  {
    static const unsigned int l_width = 144;
    for (unsigned int i = 0; i < l_width; ++i)
      std::cout << "_";
  ...
  Solid<dim, number_t, ad_type_code>::get_error_dilation(const BlockVector<double> &solution_total) const
  {
    ...
    FEValues<dim>               fe_values_ref(fe, qf_cell, update_values | update_gradients | update_JxW_values);
    ...
    for (typename DoFHandler<dim>::active_cell_iterator cell = dof_handler_ref.begin_active();
         ...
         ++cell)
      {
        ...
        fe_values_ref[J_fe].get_function_values(solution_total, solution_values_J_total);
  ...
  template <int dim, typename number_t, enum AD::NumberTypes ad_type_code>
  void
  Solid<dim, number_t, ad_type_code>::assemble_system(const BlockVector<double> &solution_delta)
  {
  ...
  template <int dim, typename number_t, enum AD::NumberTypes ad_type_code>
  void
  Solid<dim, number_t, ad_type_code>::copy_local_to_global_system(const PerTaskData_ASM &data)
  {
    if (data.cell_matrix.frobenius_norm() > 1e-12)
      constraints.distribute_local_to_global(
        data.cell_matrix, data.cell_rhs, data.local_dof_indices, tangent_matrix, system_rhs);
  }
  template <int dim, typename number_t, enum AD::NumberTypes ad_type_code>
  void
    ...
    if (is_recording == true)
      {
            ...
            for (unsigned int k = 0; k < dofs_per_cell; ++k)
              {
                const unsigned int k_group = fe.system_to_base_index(k).first.first;
                if (k_group == u_dof)
                  Grad_u[q_point] += dof_values_ad[k] * scratch.fe_values_ref[u_fe].gradient(k, q_point);
                else if (k_group == p_dof)
                  p_tilde[q_point] += dof_values_ad[k] * scratch.fe_values_ref[p_fe].value(k, q_point);
                else if (k_group == J_dof)
                  J_tilde[q_point] += dof_values_ad[k] * scratch.fe_values_ref[J_fe].value(k, q_point);
                else
                  Assert(k_group <= J_dof, ExcInternalError());
              }
          }
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
          {
            const Tensor<2, dim, ADNumberType> F     = unit_symmetric_tensor<dim>() + Grad_u[q_point];
            const Tensor<2, dim, ADNumberType> F_inv = invert(F);
            const ADNumberType                 det_F = determinant(F);
            Assert(numbers::value_is_greater_than(det_F, 0.0), ExcMessage("Negative jacobian detected!"));

            const SymmetricTensor<2, dim, ADNumberType> tau         = lqph[q_point]->get_tau(F, p_tilde[q_point]);
            const ADNumberType                          dPsi_vol_dJ = lqph[q_point]->get_dPsi_vol_dJ(J_tilde[q_point]);

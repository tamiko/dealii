...
struct IncompressibleNeoHookeanPrincipalStretches
{
  // Incompressible Neo-Hookean material
  static NumberType
  psi(const std::array<std::pair<NumberType, Tensor<1, dim, NumberType>>, dim> eig_C)
  {
    // Make things work with AD types
    using std::log;
    using std::sqrt;
...
template <int dim, typename number_t, enum AD::NumberTypes ad_type_code>
void
test_NH_eigen_energy(const enum SymmetricTensorEigenvectorMethod method, const bool nontrivial_initial_values)
{
  using ADHelper         = AD::ScalarFunction<dim, ad_type_code, number_t>;
  using ADNumberType     = typename ADHelper::ad_type;
  using ScalarNumberType = typename ADHelper::scalar_type;

  std::cout << "*** Principal stretch definition of incompressible NeoHookean material (from free energy function), "
            << "dim = " << Utilities::to_string(dim) << ", "
            << "Type code: " << static_cast<int>(ad_type_code) << ", "
  ...
  static const ScalarNumberType tol_val =
    (nontrivial_initial_values ? 1e-6 : 1e5 * std::numeric_limits<ScalarNumberType>::epsilon());
  static const ScalarNumberType tol_grad =
    (nontrivial_initial_values ? 1e-4 : 1e5 * std::numeric_limits<ScalarNumberType>::epsilon());
  static const ScalarNumberType tol_hess =
    (nontrivial_initial_values ? 2e-2 /*1e-2,5e-3*/ : 1e5 * std::numeric_limits<ScalarNumberType>::epsilon());
  std::cout << "psi:              " << psi << std::endl;
  std::cout << "func::psi(C):     " << func::psi(C) << std::endl;
  std::cout << "DIFF NORM: " << std::abs(psi - func::psi(C)) << std::endl;
  Assert(std::abs(psi - func::psi(C)) < tol_val, ExcMessage("No match for function value."));
  std::cout << "dpsi_dC:              " << dpsi_dC << std::endl;
  std::cout << "func::dpsi_dC(C):     " << func::dpsi_dC(C) << std::endl;
  std::cout << "DIFF NORM: " << std::abs((dpsi_dC - func::dpsi_dC(C)).norm()) << std::endl;
  Assert(std::abs((dpsi_dC - func::dpsi_dC(C)).norm()) < tol_grad, ExcMessage("No match for first derivative."));
  if (AD::ADNumberTraits<ADNumberType>::n_supported_derivative_levels >= 2)
    {
      ...
      //      std::endl;
      std::cout << "DIFF NORM: " << std::abs((d2psi_dC_dC - func::d2psi_dC_dC(C)).norm()) << std::endl;
...
test_symmetric_tensor()
{
  ...
  // Non-trivial initial values (unequal eigenvalues):
  {
    ...
    test_NH_eigen_energy<dim, number_t, ad_type_code>(SymmetricTensorEigenvectorMethod::hybrid,
                                                      nontrivial_initial_values);
    test_NH_eigen_energy<dim, number_t, ad_type_code>(SymmetricTensorEigenvectorMethod::ql_implicit_shifts,
                                                      nontrivial_initial_values);
    test_NH_eigen_energy<dim, number_t, ad_type_code>(SymmetricTensorEigenvectorMethod::jacobi,
                                                      nontrivial_initial_values);
  }

...
FEFaceValues<3>
  fe_face_values1(mapping, fe, quadrature, update_quadrature_points | update_JxW_values | update_normal_vectors);
FESubfaceValues<3>
  fe_face_values2(mapping, fe, quadrature, update_quadrature_points | update_JxW_values | update_normal_vectors);
  ...
  for (; cell != dof_handler.end(); ++cell)
    for (const unsigned int face_no : GeometryInfo<3>::face_indices())
      if (!cell->at_boundary(face_no) && cell->face(face_no)->has_children())
        for (unsigned int subface_no = 0; subface_no < cell->face(face_no)->n_active_descendants(); ++subface_no)
          {
            ...
            for (unsigned int q = 0; q < quadrature.size(); ++q)
              {
                ...
                AssertThrow(std::fabs(fe_face_values1.JxW(q) - fe_face_values2.JxW(q)) < 1e-14, ExcInternalError());
                AssertThrow((fe_face_values1.normal_vector(q) + fe_face_values2.normal_vector(q)).norm_square() < 1e-20,
                            ExcInternalError());
              }
          }
}

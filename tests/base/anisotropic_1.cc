  ...
  for (unsigned int k = 0; k < n; ++k)
    {
      ...
      if ((grad1 - gradients1[k]).norm() > 5.0E-12)
        deallog << 'P' << k << ": gradients differ " << grad1 << " != " << gradients1[k] << std::endl;
      Tensor<1, dim> grad2 = q.template compute_derivative<1>(k, x);
      if ((grad2 - gradients2[k]).norm() > 5.0E-12)
        deallog << 'Q' << k << ": gradients differ " << grad1 << " != " << gradients2[k] << std::endl;
      ...
      if ((grad_grad1 - second1[k]).norm() > 5.0E-12)
        deallog << 'P' << k << ": second derivatives differ " << grad_grad1 << " != " << second1[k] << std::endl;
      Tensor<2, dim> grad_grad2 = q.template compute_derivative<2>(k, x);
      if ((grad_grad2 - second2[k]).norm() > 5.0E-12)
        deallog << 'Q' << k << ": second derivatives differ " << grad_grad2 << " != " << second2[k] << std::endl;
      if ((grad_grad2 - grad_grad1).norm() > 5.0E-12)
        deallog << "PQ" << k << ": second derivatives differ " << grad_grad1 << " != " << grad_grad2 << std::endl;

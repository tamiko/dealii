...
test()
{
  ...
  // test on the std::array class template
  {
    ...
    static_assert(is_const_reference<decltype(*a.begin())>(), "type needs to be const");
    static_assert(is_const_reference<decltype(*a.end())>(), "type needs to be const");
  ...
  // test for a vector (preserve iterator constness)
  {
    ...
    static_assert(!std::is_const_v<decltype(a)>, "a should not be const (but has const value)");
    static_assert(std::is_const_v<decltype(a)::value_type>, "a::value_type needs to be const");
    static_assert(is_const_reference<decltype(*a.begin())>(), "type needs to be const");
    static_assert(is_const_reference<decltype(*a.end())>(), "type needs to be const");
    v[2] = 10.0;
  ...
  // test for a (new in boost 1.54) vector class
  {
    ...
    try
      ...
      // to make CTest happy
#ifndef DEBUG
    deallog << "ExcMessage(\"The beginning of the array view needs to be before the end.\")" << std::endl;
    deallog << "ExcMessage(\"The beginning of the array view needs to be before the end.\")" << std::endl;
#endif
  }

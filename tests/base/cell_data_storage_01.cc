...
check_qph(Triangulation<dim>                                                     &tr,
          CellDataStorage<typename Triangulation<dim, dim>::cell_iterator, DATA> &manager,
    ...
    if (cell->is_locally_owned())
      {
        ...
        const std::vector<std::shared_ptr<const DATA>>                qpd_const     = manager_const.get_data(cell);
        const std::optional<std::vector<std::shared_ptr<DATA>>>       qpd_optional  = manager.try_get_data(cell);
        const std::optional<std::vector<std::shared_ptr<const DATA>>> qpd_const_optional =
          manager_const.try_get_data(cell);
      ...
      if (cell->is_locally_owned())
        {
            ...
            qpd[q]->value = my_func.value(q_points[q]);
          {
            ...
            if (next_cell != tr.end())
              {
                const std::optional<std::vector<std::shared_ptr<MyQData>>> nonexisting_data =
                  data_storage.try_get_data(next_cell);

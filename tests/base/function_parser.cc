  ...
  for (unsigned int i = 1; i <= expressions.size(); ++i)
    {
        ...
        {
          {
            ...
            deallog << "Initialize Succeeded with dim = 2, " << i << " components, " << expressions.size()
                    << " expressions, " << variables << " as variables." << std::endl;
          }
        }
      catch (...)
        {
          deallog << "Initialize Failed with dim = 2, " << i << " components, " << expressions.size()
                  << " expressions, " << variables << " as variables." << std::endl;
        }
      ...
      try
        ...
        {
          deallog << "Initialize Failed with dim = 2, " << i << " components, " << concatenated << " as function and "

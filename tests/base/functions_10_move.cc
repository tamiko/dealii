...
check()
{
  ...
  Functions::InterpolatedTensorProductGridData<dim> f(std::move(coordinates), std::move(data));
  ...
  for (unsigned int i = 0; i < 10; ++i)
    {
      ...
      for (unsigned int d = 0; d < dim; ++d)
        p[d] = coordinates_backup[d][0] +
               (random_value<double>()) * (coordinates_backup[d].back() - coordinates_backup[d][0]);

    ...
    for (unsigned int degree = 1; degree < 40; degree += 3)
      {
        ...
        for (unsigned int k = 1; k < degree; ++k)
          {
            eigenvalue_mat(k - 1, k) = std::sqrt(4. * k * (k + alpha) * (k + beta) * (k + alpha + beta) /
        ...
        eigenvalue_mat.compute_eigenvalues_symmetric(-1., 1., 1.e-20, roots_reference, eigenvectors);
        deallog << "Roots (implemented vs reference)" << std::endl;
        for (unsigned int i = 0; i < degree; ++i)
          deallog << std::setw(22) << std::setprecision(19) << roots[i] << " (fval = " << std::setw(9)
                  << std::setprecision(3) << jacobi_polynomial_value(degree, alpha, beta, roots[i]) << " ) "
                  << std::setw(22) << std::setprecision(19) << 0.5 + 0.5 * roots_reference[i]
                  << " (fval = " << std::setw(9) << std::setprecision(3)
                  << jacobi_polynomial_value(degree, alpha, beta, 0.5 + 0.5 * roots_reference[i]) << " )" << std::endl;
        deallog << std::endl;
      }
}

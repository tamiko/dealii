  ...
  for (unsigned int k = 0; k < n; ++k)
    {
      ...
      if (diff.norm_square() > eps * eps)
        deallog << 'P' << k << ": second derivatives differ " << grad2 << " != " << second[k] << std::endl;
      ...
      if ((grad3 - third[k]).norm_square() > 5e-15 * 5e-15)
        deallog << 'P' << k << ": third derivatives differ " << grad3 << " != " << third[k] << std::endl;
      ...
      if ((grad3 - third[k]).norm_square() > eps * eps)
        deallog << 'P' << k << ": fourth derivatives differ " << grad4 << " != " << fourth[k] << std::endl;

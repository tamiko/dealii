...
test(TimeStepping::RungeKutta<Vector<double>>                                         &solver,
     ...
     std::function<Vector<double>(const double, const double, const Vector<double> &)> id_minus_tau_J_inv,
...
void
test_convergence(TimeStepping::RungeKutta<Vector<double>>                                         &solver,
                 std::function<Vector<double>(const double, const Vector<double> &)>               f,
                 std::function<Vector<double>(const double, const double, const Vector<double> &)> id_minus_tau_J_inv,
                 std::function<double(const double)>                                               my)
{
  ...
  for (unsigned int cycle = 0; cycle < 8; ++cycle)
    {
      ...
      if (cycle > 1)
        deallog << std::log(std::fabs(errors[cycle - 1] / errors[cycle])) / std::log(2.) << std::endl;
    }
}
...
main()
{
  ...
  // deallog.precision(4);
  {
    ...
    deallog << "Strong Stability Preserving Runge-Kutta third order" << std::endl;
    TimeStepping::ExplicitRungeKutta<Vector<double>> ssp_rk3(TimeStepping::SSP_THIRD_ORDER);
    ...
    TimeStepping::LowStorageRungeKutta<Vector<double>> lsrk33(TimeStepping::LOW_STORAGE_RK_STAGE3_ORDER3);
    ...
    TimeStepping::LowStorageRungeKutta<Vector<double>> lsrk74(TimeStepping::LOW_STORAGE_RK_STAGE7_ORDER4);
    test(lsrk74, f4, id_minus_tau_J_inv4, my4);

    deallog << "Low-storage Runge-Kutta stage 9 order 5" << std::endl;
    TimeStepping::LowStorageRungeKutta<Vector<double>> lsrk95(TimeStepping::LOW_STORAGE_RK_STAGE9_ORDER5);
...

  {
    ...
    TimeStepping::ExplicitRungeKutta<Vector<double>> rk1(TimeStepping::FORWARD_EULER);
    test_convergence(rk1, my_rhs_function, id_minus_tau_J_inv1, my_exact_solution);

    deallog << "Runge-Kutta third order" << std::endl;
    TimeStepping::ExplicitRungeKutta<Vector<double>> rk3(TimeStepping::RK_THIRD_ORDER);
    test_convergence(rk3, my_rhs_function, id_minus_tau_J_inv3, my_exact_solution);

    deallog << "Strong Stability Preserving Runge-Kutta third order" << std::endl;
    TimeStepping::ExplicitRungeKutta<Vector<double>> ssp_rk3(TimeStepping::SSP_THIRD_ORDER);
    test_convergence(ssp_rk3, my_rhs_function, id_minus_tau_J_inv3, my_exact_solution);

    deallog << "Runge-Kutta fourth order" << std::endl;
    TimeStepping::ExplicitRungeKutta<Vector<double>> rk4(TimeStepping::RK_CLASSIC_FOURTH_ORDER);
    test_convergence(rk4, my_rhs_function, id_minus_tau_J_inv4, my_exact_solution);

    deallog << "Low-storage Runge-Kutta stage 3 order 3" << std::endl;
    TimeStepping::LowStorageRungeKutta<Vector<double>> lsrk33(TimeStepping::LOW_STORAGE_RK_STAGE3_ORDER3);
    test_convergence(lsrk33, my_rhs_function, id_minus_tau_J_inv3, my_exact_solution);

    deallog << "Low-storage Runge-Kutta stage 5 order 4" << std::endl;
    TimeStepping::LowStorageRungeKutta<Vector<double>> lsrk54(TimeStepping::LOW_STORAGE_RK_STAGE5_ORDER4);
    test_convergence(lsrk54, my_rhs_function, id_minus_tau_J_inv4, my_exact_solution);

    deallog << "Low-storage Runge-Kutta stage 7 order 4" << std::endl;
    TimeStepping::LowStorageRungeKutta<Vector<double>> lsrk74(TimeStepping::LOW_STORAGE_RK_STAGE7_ORDER4);
    test_convergence(lsrk74, my_rhs_function, id_minus_tau_J_inv4, my_exact_solution);

    deallog << "Low-storage Runge-Kutta stage 9 order 5" << std::endl;
    TimeStepping::LowStorageRungeKutta<Vector<double>> lsrk95(TimeStepping::LOW_STORAGE_RK_STAGE9_ORDER5);
    test_convergence(lsrk95, my_rhs_function, id_minus_tau_J_inv5, my_exact_solution);

    deallog << "Backward Euler first order" << std::endl;
    TimeStepping::ImplicitRungeKutta<Vector<double>> be(TimeStepping::BACKWARD_EULER);
    test_convergence(be, my_rhs_function, id_minus_tau_J_inv1, my_exact_solution);

    deallog << "Implicit midpoint second order" << std::endl;
    TimeStepping::ImplicitRungeKutta<Vector<double>> im(TimeStepping::IMPLICIT_MIDPOINT);
    test_convergence(im, my_rhs_function, id_minus_tau_J_inv2, my_exact_solution);

    deallog << "Crank-Nicolson second order" << std::endl;
    TimeStepping::ImplicitRungeKutta<Vector<double>> cn(TimeStepping::CRANK_NICOLSON);
    test_convergence(cn, my_rhs_function, id_minus_tau_J_inv2, my_exact_solution);

    deallog << "SDIRK second order" << std::endl;
    TimeStepping::ImplicitRungeKutta<Vector<double>> sdirk(TimeStepping::SDIRK_TWO_STAGES);
    test_convergence(sdirk, my_rhs_function, id_minus_tau_J_inv2, my_exact_solution);
  }

...
check(const double (&array)[N], const Point<dim>(&point))
{
  ...
  Utilities::unpack(array_uncompressed.cbegin(), array_uncompressed.cend(), unpacked_array_uncompressed, false);
  ...
  try
    {
      Point<dim> forbidden = Utilities::unpack<Point<dim>>(point_compressed.cbegin(), point_compressed.cend(), false);
    }

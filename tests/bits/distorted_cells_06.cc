...
class MyManifold : public Manifold<dim>
{
  ...
  {
    deallog << "Finding point between " << line->vertex(0) << " and " << line->vertex(1) << std::endl;

    return Point<dim>(0, 0.5, 0.9);
  }

  virtual Point<dim>
  get_new_point_on_quad(const typename Triangulation<dim>::quad_iterator &) const override
  {
...
check()
{
  ...
  try
  ...
  catch (typename Triangulation<dim>::DistortedCellList &dcv)
    {
      ...
      typename Triangulation<dim>::DistortedCellList subset = GridTools::fix_up_distorted_child_cells(dcv, coarse_grid);
      deallog << "Found " << subset.distorted_cells.size() << " cells that are still distorted" << std::endl;

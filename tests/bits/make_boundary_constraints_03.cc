      ...
      for (unsigned int i = 0; i < dofs_per_cell; ++i)
        {
          if (constraints.is_constrained(local_dof_indices[i]))
            deallog << "DoF " << local_dof_indices[i] << ", copy " << fe.system_to_base_index(i).first.second
                    << ", base element " << fe.system_to_base_index(i).first.first << ", index "
                    << fe.system_to_base_index(i).second << ", position " << locations[i] << std::endl;
        }
    }
}

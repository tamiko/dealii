...
public:
  ...
  virtual void
  vector_value(const Point<dim> &p, Vector<VectorType::value_type> &values) const
  {
    values(0) = value(p, 0);
  }
};
...
check()
{
  ...
  const IndexSet locally_relevant_dofs = DoFTools::extract_locally_relevant_dofs(dof);
    ...
    {
      {
        ...
        value(0)                         = 0;
        {
          auto cell_and_ref_point = GridTools::find_active_cell_around_point(mapping, dof, p[i]);
          if (cell_and_ref_point.first.state() == IteratorState::valid)
            {
              point_in_locally_owned_cell = cell_and_ref_point.first->is_locally_owned();
            }
        }

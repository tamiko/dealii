...
test_intersection_inside(Triangulation<3> &tria0, Triangulation<1, 3> &tria1)
{
  ...
  const double expected_measure   = (tria1.begin_active()->vertex(1) - tria1.begin_active()->vertex(0)).norm();

  const auto vec_of_arrays = CGALWrappers::compute_intersection_of_cells(tria0.begin_active(),
                                                                         tria1.begin_active(),
                                                                         MappingQ1<3>(),
                                                                         MappingQ1<1, 3>());
...
test_intersection(Triangulation<3> &tria0, Triangulation<1, 3> &tria1)
{
  ...
  const auto vec_of_arrays =
    CGALWrappers::compute_intersection_of_cells(tria0.begin_active(), cell1, MappingQ1<3>(), MappingQ1<1, 3>());
...
test_failing_intersection(Triangulation<3> &tria0, Triangulation<1, 3> &tria1)
{
  ...
  const auto vec_of_arrays =
    CGALWrappers::compute_intersection_of_cells(cell0, cell1, MappingQ1<3>(), MappingQ1<1, 3>());

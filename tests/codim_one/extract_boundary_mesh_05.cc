...
test()
{
  ...
  for (DoFHandler<dim - 1, dim>::active_cell_iterator cell = boundary_dh.begin_active(), endc = boundary_dh.end();
       cell != endc;
       ++cell)
    {
      ...
      for (unsigned int v = 0; v < GeometryInfo<dim - 1>::vertices_per_cell; ++v)
        {
          ...
          deallog << "vertex: " << v << ", global: " << cell->vertex_index(v) << " index: " << index << std::endl;
        }

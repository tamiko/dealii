...

{
  ...
  FEValues<dim, spacedim> fe_values(fe,
                                    q_midpoint,
                                    update_values | update_JxW_values | update_normal_vectors | update_gradients);
  ...
  for (; cell != endc; ++cell)
    {
      ...
      // element surface.
      cell_tangentials[0][0] = cell_normals[0][1] / sqrt(pow(cell_normals[0][0], 2) + pow(cell_normals[0][1], 2));
      cell_tangentials[0][1] = -cell_normals[0][0] / sqrt(pow(cell_normals[0][0], 2) + pow(cell_normals[0][1], 2));
      if (spacedim == 3)
      ...
      deallog << "cell no. " << cell->index() << "; "
              << "dir.deriv. " << projected_directional_derivative(cell->index()) << "; " << std::endl;
      if (spacedim == 2)
        deallog << "exact solution " << cos(2 * numbers::PI * (cell->index() + .5) / triangulation.n_cells())
                << std::endl;
    }

...
main()
{
  ...
  for (unsigned int m = 1; m < 7; ++m)
    {
      deallog << " =========Quadrature Order: " << m << " =============================== " << std::endl;
      deallog << " ============================================================ " << std::endl;
      for (unsigned int index = 0; index < 4; ++index)
        {
          deallog << " ===============Vertex Index: " << index << " ============================= " << std::endl;
                  ...
                  for (unsigned int q = 0; q < quad.size(); ++q)
                    {
                      ...
                      approx_integral_2 += (pow(x, (double)i) * pow(y, (double)j) * quad_de.weight(q) /
                                            (quad_de.point(q) - GeometryInfo<2>::unit_cell_vertex(index)).norm());
                    }

                  deallog << "f(x,y) = x^" << i << " y^" << j << ", Error = " << approx_integral - exact_integral;
                  if (std::abs(approx_integral - approx_integral_2) < eps)
                    deallog << std::endl;
                  else
                    deallog << ", desing: " << approx_integral_2 - exact_integral << std::endl;
                }
            }
        }
    }

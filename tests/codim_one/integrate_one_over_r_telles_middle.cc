  ...
  for (unsigned int m = 1; m < 6; ++m)
    {
      ...
      deallog << " ============================================================ " << std::endl;
      deallog << " ===============Vertex: " << center << " ============================= " << std::endl;
              ...
              for (unsigned int q = 0; q < quad_2.size(); ++q)
                {
                  ...
                  double R = sqrt((x - center[0]) * (x - center[0]) + (y - center[1]) * (y - center[1]));
                  approx_integral_2 += (pow(x, (double)i) * pow(y, (double)j) / R * quad_2.weight(q));
                }


              deallog << "f(x,y) = x^" << i << " y^" << j << ", Errors = " << approx_integral - exact_integral << ", "

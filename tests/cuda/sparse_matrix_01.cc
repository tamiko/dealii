...
void
check_matrix(const SparseMatrix<double> &A, CUDAWrappers::SparseMatrix<double> &A_dev)
{
  cudaError_t cuda_error_code;
  double     *val_dev                                                        = nullptr;
  int        *column_index_dev                                               = nullptr;
  int        *row_ptr_dev                                                    = nullptr;
  std::tie(val_dev, column_index_dev, row_ptr_dev, std::ignore, std::ignore) = A_dev.get_cusparse_matrix();
  ...
  cuda_error_code = cudaMemcpy(&column_index_host[0], column_index_dev, nnz * sizeof(int), cudaMemcpyDeviceToHost);
  AssertCuda(cuda_error_code);

  const int        n_rows = A_dev.m() + 1;
  std::vector<int> row_ptr_host(n_rows + 1);
  cuda_error_code = cudaMemcpy(&row_ptr_host[0], row_ptr_dev, (A_dev.m() + 1) * sizeof(int), cudaMemcpyDeviceToHost);
  AssertCuda(cuda_error_code);
...
void
check_vector(const Vector<double> &a, const LinearAlgebra::ReadWriteVector<double> &b)
{

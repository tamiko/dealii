...
check_this(const DoFHandler<dim> &dof_handler, const Vector<double> &v_node, const Vector<double> &v_cell)
{
  std::vector<DataComponentInterpretation::DataComponentInterpretation> data_component_interpretation(
    dof_handler.get_fe().n_components(), DataComponentInterpretation::component_is_scalar);
  // if possible, declare the last
  // dim components as vectors
  if (dof_handler.get_fe().n_components() >= dim)
    for (unsigned int i = dof_handler.get_fe().n_components() - dim; i < dof_handler.get_fe().n_components(); ++i)
  ...
  for (unsigned int i = 0; i < dof_handler.get_fe().n_components(); ++i)
    deallog << (data_component_interpretation[i] == DataComponentInterpretation::component_is_scalar ? "scalar " :
                                                                                                       "vector ");
  deallog << std::endl;

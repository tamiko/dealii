...
test(const unsigned int flag)
{
  ...
  if (false)
    {
      std::cout << "-------------------- " << Utilities::int_to_string(dim) << Utilities::int_to_string(flag)
                << std::endl;
      cm.print(std::cout);
      ...
      const std::string filename = "grid" + Utilities::int_to_string(dim) + Utilities::int_to_string(flag) + ".gp";
      std::ofstream     f(filename);

      f << "set terminal png size 400,410 enhanced font \"Helvetica,8\"" << std::endl
        << "set output \"grid" << Utilities::int_to_string(dim) << Utilities::int_to_string(flag) << ".png\""
        << std::endl

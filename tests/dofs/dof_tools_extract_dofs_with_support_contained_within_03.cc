...
test(const bool left = true)
{
  ...
  IndexSet support =
    left ? DoFTools::extract_dofs_with_support_contained_within(
             dh, std::function<bool(const typename DoFHandler<dim>::active_cell_iterator &)>(&pred_left<dim>), cm) :
           DoFTools::extract_dofs_with_support_contained_within(
             dh, std::function<bool(const typename DoFHandler<dim>::active_cell_iterator &)>(&pred_right<dim>), cm);

      ...
      for (unsigned int q = 0; q <= p + 2; ++q)
        {
          ...
          VectorTools::integrate_difference(
            dof_handler, interpolant, F<dim>(q, adj1), error, QGauss<dim>(q + 2), VectorTools::L2_norm, &select_mask1);
          if (q <= p)
            Assert(error.l2_norm() < 1e-12 * interpolant.l2_norm(), ExcInternalError());

          deallog << fe.get_name() << ", Mask 1, P_" << q << ", rel. error=" << error.l2_norm() / interpolant.l2_norm()
                  << std::endl;

          // then compute the interpolation error for mask 2
          VectorTools::integrate_difference(
            dof_handler, interpolant, F<dim>(q, adj2), error, QGauss<dim>(q + 2), VectorTools::L2_norm, &select_mask2);
          if (q <= p)
            Assert(error.l2_norm() < 1e-12 * interpolant.l2_norm(), ExcInternalError());

          deallog << fe.get_name() << ", Mask 2, P_" << q << ", rel. error=" << error.l2_norm() / interpolant.l2_norm()
                  << std::endl;
        }
    }
}

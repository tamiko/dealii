...
test(const Triangulation<dim> &tr)
{
  ...
  // matrices
  deallog << "Mass matrices " << (equal(mass_matrix[0], mass_matrix[1]) ? "are" : "are not") << " equal." << std::endl;
  deallog << "Laplace matrices " << (equal(laplace_matrix[0], laplace_matrix[1]) ? "are" : "are not") << " equal."
          << std::endl;

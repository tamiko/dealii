  ...
  for (; cell != endc; ++cell)
    {
      ...
      std::vector<std::vector<Tensor<1, 2>>> grads_here(n_q_points, std::vector<Tensor<1, 2>>(n_components));
  ...
  for (unsigned int elm = 0; elm < 3; ++elm)
    {
      ...
      project(
        map_default, *dof_handler, hn_constraints, QGauss<2>(6), TestMap1<2>(2), solution, min_convergence_steps[elm]);

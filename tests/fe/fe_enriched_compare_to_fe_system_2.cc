                  ...
                  const Tensor<2, dim> &h_s3)
{
  ...
  const Tensor<1, dim> g_s = g_s0 + g_s1 * v_f1 + v_s1 * g_f1 + g_s2 * v_f2 + v_s2 * g_f2 + g_s3 * v_f3 + v_s3 * g_f3;
  Tensor<2, dim> op1 = outer_product(g_s1, g_f1), op2 = outer_product(g_s2, g_f2), op3 = outer_product(g_s3, g_f3);
  ...
  const Tensor<2, dim> h_s =
    h_s0 + h_s1 * v_f1 + sp1 + v_s1 * h_f1 + h_s2 * v_f2 + sp2 + v_s2 * h_f2 + h_s3 * v_f3 + sp3 + v_s3 * h_f3;
     ...
     const bool                 distort)
{
  ...
  std::vector<
    std::vector<std::function<const Function<dim> *(const typename Triangulation<dim, dim>::cell_iterator &)>>>
    functions(3);
  ...
  FEValues<dim> fe_values_enriched(fe_enriched, volume_quad, update_values | update_gradients | update_hessians);

  FEValues<dim> fe_values_system(fe_system,
                                 volume_quad,
                                 update_values | update_gradients | update_hessians | update_quadrature_points);

  FEFaceValues<dim> fe_face_values_enriched(fe_enriched, face_quad, update_values | update_gradients | update_hessians);

  FEFaceValues<dim> fe_face_values_system(
    fe_system, face_quad, update_values | update_gradients | update_hessians | update_quadrature_points);
  ...
  typename DoFHandler<dim>::active_cell_iterator cell_enriched = dof_handler_enriched.begin_active(),
      ...
      for (const unsigned int face : GeometryInfo<dim>::face_indices())
        {
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            for (const auto q_point : fe_face_values_system.quadrature_point_indices())
              check_consistency(q_points[q_point],
                                fun1,
                                fun2,
                                fun3,

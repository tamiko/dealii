...
namespace Step22
{
    ...
    switch (component)
      {
        ...
        case 0:
          gradient[0] =
            x * (x * (x * y * (y * (16 * y - 24) + 8) + y * ((36 - 24 * y) * y - 12)) + y * (y * (8 * y - 12) + 4));
        ...
        case 1:
          gradient[0] = x * (x * ((24 - 12 * y) * y - 12) * y * y + (y * (12 * y - 24) + 12) * y * y) +
                        ((4 - 2 * y) * y - 2) * y * y;
          gradient[1] =
            x * (x * (x * y * ((24 - 16 * y) * y - 8) + y * (y * (24 * y - 36) + 12)) + y * ((12 - 8 * y) * y - 4));
          break;
        // pressure
        case 2:
          // discontinuous Boffi
    ...
    switch (component)
      {
        // velocity
        case 0:
          return x * (x * (x * (x * (24 * y - 12) - 48 * y + 24) + y * (y * (48 * y - 72) + 48) - 12) +
                      y * ((72 - 48 * y) * y - 24)) +
                 y * (y * (8 * y - 12) + 4);
  ...
  template <class Preconditioner>
  SchurComplement<Preconditioner>::SchurComplement(const BlockSparseMatrix<double> &system_matrix,
                                                   const InverseMatrix<SparseMatrix<double>, Preconditioner> &A_inverse)
  ...
  StokesProblem<dim>::setup_dofs()
  {
...

    {
      ...
      VectorTools::interpolate_boundary_values(dof_handler, 0, ExactSolution<dim>(), constraints, component_mask);
                        ...
                        for (unsigned int i = 0; i < dofs_per_cell; ++i)
                          {
                            ...
                            if (component_i < dim)
                              local_rhs(i) +=
                                fe_v_face.shape_value(i, q) * jump * normals[q][component_i] * fe_v_face.JxW(q);
                          }
                      }
                  }
              }
  ...
  StokesProblem<dim>::solve()
  {
...

    {
      ...
      InverseMatrix<SparseMatrix<double>, SparseILU<double>> m_inverse(system_matrix.block(1, 1), preconditioner);
  ...
  StokesProblem<dim>::output_results(const unsigned int refinement_cycle)
  {
    ...
    VectorTools::integrate_difference(
      dof_handler, solution, exactsolution, difference_per_cell, quadrature, VectorTools::L2_norm, &velocity_mask);
    const double L2_error_velocity = difference_per_cell.l2_norm();

    VectorTools::integrate_difference(
      dof_handler, solution, exactsolution, difference_per_cell, quadrature, VectorTools::H1_seminorm, &velocity_mask);
    const double H1_error_velocity = difference_per_cell.l2_norm();

    VectorTools::integrate_difference(
      dof_handler, solution, exactsolution, difference_per_cell, quadrature, VectorTools::Linfty_norm, &velocity_mask);
    ...
    VectorTools::integrate_difference(
      dof_handler, solution, exactsolution, difference_per_cell, quadrature, VectorTools::L2_norm, &pressure_mask);
    ...
    VectorTools::integrate_difference(
      dof_handler, solution, exactsolution, difference_per_cell, quadrature, VectorTools::mean, &pressure_mask);
    ...
    VectorTools::integrate_difference(
      dof_handler, solution, exactsolution, difference_per_cell, quadrature, VectorTools::H1_seminorm, &pressure_mask);
    const double H1_error_pressure = difference_per_cell.l2_norm();

    VectorTools::integrate_difference(
      dof_handler, solution, exactsolution, difference_per_cell, quadrature, VectorTools::Linfty_norm, &pressure_mask);

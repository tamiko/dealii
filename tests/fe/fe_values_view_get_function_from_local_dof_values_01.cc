...
test_view(const Vector<double>             &solution,
          ...
          const std::vector<NumberType>    &local_dof_values)
{
  using View                      = std::remove_reference_t<std::remove_const_t<decltype(fe_values[extractor])>>;
  ...
  // Values
  std::vector<typename View::template solution_value_type<NumberType>> qp_values_local(n_q_points);
  ...
  // Gradients
  std::vector<typename View::template solution_gradient_type<NumberType>> qp_grads_local(n_q_points);
  ...
  // Hessians
  std::vector<typename View::template solution_hessian_type<NumberType>> qp_hess_local(n_q_points);
  ...
  // Laplacians
  std::vector<typename View::template solution_laplacian_type<NumberType>> qp_laplace_local(n_q_points);
  ...
  // Third derivatives
  std::vector<typename View::template solution_third_derivative_type<NumberType>> qp_third_deriv_local(n_q_points);
  ...
  for (unsigned int q = 0; q < n_q_points; ++q)
    {
      if (std::abs(qp_values_local[q] - qp_values_global[q]) > tolerance * std::abs(qp_values_global[q]))
        deallog << "NOT OK: Value @ " << q << std::endl;

      if ((qp_grads_local[q] - qp_grads_global[q]).norm() > tolerance * qp_grads_global[q].norm())
        deallog << "NOT OK: Grad @ " << q << std::endl;

      if ((qp_hess_local[q] - qp_hess_global[q]).norm() > tolerance * qp_hess_global[q].norm())
        deallog << "NOT OK: Hess @ " << q << std::endl;

      if (std::abs(qp_laplace_local[q] - qp_laplace_global[q]) > tolerance * std::abs(qp_laplace_global[q]))
        deallog << "NOT OK: Laplace @ " << q << std::endl;

      if ((qp_third_deriv_local[q] - qp_third_deriv_global[q]).norm() > tolerance * qp_third_deriv_global[q].norm())
          ...
          const std::vector<NumberType>    &local_dof_values)
{
  ...
  using value_type              = typename ProductType<typename View::value_type, NumberType>::type;
  using gradient_type           = typename ProductType<typename View::gradient_type, NumberType>::type;
  using symmetric_gradient_type = typename ProductType<typename View::symmetric_gradient_type, NumberType>::type;
  using divergence_type         = typename ProductType<typename View::divergence_type, NumberType>::type;
  using curl_type               = typename ProductType<typename View::curl_type, NumberType>::type;
  using hessian_type            = typename ProductType<typename View::hessian_type, NumberType>::type;
  using laplacian_type          = typename ProductType<typename View::value_type, NumberType>::type;
  using third_derivative_type   = typename ProductType<typename View::third_derivative_type, NumberType>::type;
  ...
  // Symmetric gradients
  std::vector<typename View::template solution_symmetric_gradient_type<NumberType>> qp_symm_grads_local(n_q_points);
  ...
  // Divergences
  std::vector<typename View::template solution_divergence_type<NumberType>> qp_divs_local(n_q_points);
  ...
  // Curls
  std::vector<typename View::template solution_curl_type<NumberType>> qp_curls_local(n_q_points);
  ...
  for (unsigned int q = 0; q < n_q_points; ++q)
    {
      if ((qp_values_local[q] - qp_values_global[q]).norm() > tolerance * qp_values_global[q].norm())
        deallog << "NOT OK: Value @ " << q << std::endl;

      if ((qp_grads_local[q] - qp_grads_global[q]).norm() > tolerance * qp_grads_global[q].norm())
        deallog << "NOT OK: Grad @ " << q << std::endl;

      if ((qp_symm_grads_local[q] - qp_symm_grads_global[q]).norm() > tolerance * qp_symm_grads_global[q].norm())
        deallog << "NOT OK: Symm grad @ " << q << std::endl;

      if (std::abs(qp_divs_local[q] - qp_divs_global[q]) > tolerance * std::abs(qp_divs_global[q]))
      ...
      if ((qp_curls_local[q] - qp_curls_global[q]).norm() > tolerance * qp_curls_global[q].norm())
        deallog << "NOT OK: Curl @ " << q << std::endl;

      if ((qp_hess_local[q] - qp_hess_global[q]).norm() > tolerance * qp_hess_global[q].norm())
        deallog << "NOT OK: Hess @ " << q << std::endl;

      if ((qp_laplace_local[q] - qp_laplace_global[q]).norm() > tolerance * qp_laplace_global[q].norm())
        deallog << "NOT OK: Laplace @ " << q << std::endl;
  ...
  for (unsigned int q = 0; q < n_q_points; ++q)
    {
      ...
      if ((qp_divs_local[q] - qp_divs_global[q]).norm() > tolerance * qp_divs_global[q].norm())
        deallog << "NOT OK: Div @ " << q << std::endl;
    }
}

...
MixedElastoPlasticity<dim>::make_grid_and_dofs()
{
  ...
  deallog << "Number of active cells: " << triangulation.n_active_cells() << std::endl
          ...
          << "Number of degrees of freedom: " << dof_handler.n_dofs() << " = (" << n_stress_dof << " + " << n_gamma_dof
          << ')' << std::endl;
...
MixedElastoPlasticity<dim>::assemble_system()
{
  ...
  deallog << "fe.dofs_per_cell: " << fe.dofs_per_cell << "\tquadrature_formula.size(): " << quadrature_formula.size()
          << std::endl;

    ...
    for (unsigned int child_2 = 0; child_2 < GeometryInfo<dim>::max_children_per_cell; ++child_2)
      {
        ...
        // then to finer FE
        {
          ...
          coarse_fe.get_prolongation_matrix(child_2).mmult(tmp1, coarse_fe.get_prolongation_matrix(child_1));
        ...
        AssertThrow(injection_2.frobenius_norm() <= 1e-12 * injection_1.frobenius_norm(), ExcInternalError());
      }
}

...

  {
    ...
    for (unsigned int qpoint = 0; qpoint < q_points.size(); ++qpoint)
      for (unsigned int i = 0; i < n_dofs; ++i)
        cell_vector(i) += fiv.average_of_shape_gradients(i, qpoint).norm() * fiv.get_JxW_values()[qpoint];
    deallog << "average_of_shape_gradients.norm(): " << cell_vector << std::endl;

    cell_vector = 0.0;
    for (unsigned int qpoint = 0; qpoint < q_points.size(); ++qpoint)
      for (unsigned int i = 0; i < n_dofs; ++i)
        cell_vector(i) += fiv.average_of_shape_hessians(i, qpoint).norm() * fiv.get_JxW_values()[qpoint];
    deallog << "average_of_shape_hessians.norm(): " << cell_vector << std::endl;

    cell_vector = 0.0;
    for (unsigned int qpoint = 0; qpoint < q_points.size(); ++qpoint)
      for (unsigned int i = 0; i < n_dofs; ++i)
        cell_vector(i) += fiv.jump_in_shape_gradients(i, qpoint).norm() * fiv.get_JxW_values()[qpoint];

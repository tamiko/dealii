...
namespace StokesTests
{
    ...
    for (auto &cdf : c.face_data)
      {
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i)
          for (unsigned int k = 0; k < dofs_per_cell; ++k)
            system_matrix.add(cdf.joint_dof_indices[i], cdf.joint_dof_indices[k], cdf.cell_matrix(i, k));
      }
  }
  ...
  RightHandSide<2>::value(const Point<2> &p, const unsigned int component) const
  {
    ...
    // RHS for 0 BD's
    if (component == 0)
      return -nu * 2.0 * PI * PI * PI * (-2.0 * sin(PI * x) * sin(PI * x) + cos(2. * PI * x)) * sin(2.0 * PI * y) -
             PI * sin(PI * x) * sin(PI * y);
    if (component == 1)
      return nu * 2.0 * PI * PI * PI * (2.0 * cos(2.0 * PI * y) - 1) * sin(2.0 * PI * x) +
  ...
  RightHandSide<3>::value(const Point<3> &p, const unsigned int component) const
  {
    ...
    if (component == 0)
      return 4. * PI * PI * PI * (4. * sin(PI * x) * sin(PI * x) - cos(2. * PI * x)) * sin(2. * PI * y) *
               sin(2. * PI * z) +
             PI * cos(PI * x) * cos(PI * y) * sin(PI * z);
    if (component == 1)
      return -2. * PI * PI * PI * (4. * sin(PI * y) * sin(PI * y) - cos(2. * PI * y)) * sin(2. * PI * x) *
               sin(2. * PI * z) +
             PI * (-1) * sin(PI * y) * sin(PI * x) * sin(PI * z);
    if (component == 2)
      return -2. * PI * PI * PI * (4. * sin(PI * z) * sin(PI * z) - cos(2. * PI * z)) * sin(2. * PI * x) *
               sin(2. * PI * y) +
  ...
  template <int dim>
  StokesProblem<dim>::StokesProblem(FiniteElement<dim> &fe, const unsigned int pressure_degree)
  ...
  StokesProblem<dim>::assemble_system_mesh_loop()
  {
    ...
    auto penalty_parameter = [](const double degree, const double extent1, const double extent2) -> double {
      return 4.0 * degree * (degree + 1.0) * 0.5 * (1.0 / extent1 + 1.0 / extent2);
    };

    auto cell_worker = [&](const Iterator &cell, MeshWorker::ScratchData<dim> &scratch_data, CopyData &copy_data) {
          ...
          for (unsigned int i = 0; i < fe_v.dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < fe_v.dofs_per_cell; ++j)
                copy_data.cell_matrix(i, j) +=
                  (
                    // nu \nabla v : \nabla u
                    nu * scalar_product(fe_v[velocities].gradient(i, point), fe_v[velocities].gradient(j, point))
                    // -q, div u
                    - fe_v[pressure].value(i, point) * fe_v[velocities].divergence(j, point)
                    // -p, div v
                    - fe_v[pressure].value(j, point) * fe_v[velocities].divergence(i, point)
                    // p,q
                    + fe_v[pressure].value(j, point) * fe_v[pressure].value(i, point)) *
                  JxW[point];
    ...
    auto boundary_worker = [&](const Iterator               &cell,
      ...
      std::vector<Vector<double>> g_values(q_points.size(), Vector<double>(dim + 1));
      ...
      for (unsigned int point = 0; point < q_points.size(); ++point)
        {
          ...
          for (unsigned int i = 0; i < fe_fv.dofs_per_cell; ++i)
            for (unsigned int j = 0; j < fe_fv.dofs_per_cell; ++j)
              copy_data.cell_matrix(i, j) +=
                (
                  // - nu (\nabla u n) . v
                  -nu * ((fe_fv[velocities].gradient(j, point) * normals[point]) * fe_fv[velocities].value(i, point))

                  // - nu u . (\nabla v n)  // NIPG: use +
                  - nu * (fe_fv[velocities].value(j, point) * (fe_fv[velocities].gradient(i, point) * normals[point]))

                  // + nu * penalty u . v
                  + nu * penalty * (fe_fv[velocities].value(j, point) * fe_fv[velocities].value(i, point))

                  // p (v.n)
                  + fe_fv[pressure].value(j, point) * scalar_product(fe_fv[velocities].value(i, point), normals[point])

                  // q (u.n)
                  + fe_fv[pressure].value(i, point) * scalar_product(fe_fv[velocities].value(j, point), normals[point])
          ...
          for (unsigned int i = 0; i < fe_fv.dofs_per_cell; ++i)
            copy_data.cell_rhs(i) += (
                                       // -nu g . (\nabla v n) // NIPG: use +
                                       -nu * scalar_product(g, (fe_fv[velocities].gradient(i, point) * normals[point]))

                                       // +nu penalty g . v
                                       + nu * penalty * scalar_product(g, fe_fv[velocities].value(i, point))

                                       // q (g.n) (weak normal component of boundary condition)
                                       + fe_fv[pressure].value(i, point) * scalar_product(g, normals[point])) *
                                     JxW[point];
        }
    };
    ...
    auto face_worker = [&](const Iterator               &cell,
                           ...
                           MeshWorker::ScratchData<dim> &scratch_data,
                           CopyData                     &copy_data) {
      const FEInterfaceValues<dim> &fe_fv = scratch_data.reinit(cell, f, sf, ncell, nf, nsf);
      FEInterfaceViews::Scalar<dim> interface_scalar(fe_fv, pressure.component);
      FEInterfaceViews::Vector<dim> interface_vector(fe_fv, velocities.first_vector_component);
            ...
            for (unsigned int j = 0; j < dofs_per_cell; ++j)
              copy_data_face.cell_matrix(i, j) +=
                (
                  ...
                  -nu * (fe_fv[velocities].average_of_gradients(j, point) * normals[point]) *
                    fe_fv[velocities].jump_in_values(i, point)

                  // - nu [u] . {\nabla v}n  (symmetry) // NIPG: use +
                  - nu * fe_fv[velocities].jump_in_values(j, point) *
                      (fe_fv[velocities].average_of_gradients(i, point) * normals[point])
                  ...
                  + fe_fv[pressure].average_of_values(j, point) *
                      scalar_product(fe_fv[velocities].jump_in_values(i, point), normals[point])

                  // {q} ([u].n)
                  + fe_fv[pressure].average_of_values(i, point) *
                      scalar_product(fe_fv[velocities].jump_in_values(j, point), normals[point])) *
                JxW[point];
        }
    };

    auto copier = [&](const CopyData &c) { copy(c, constraints, system_matrix, system_rhs); };

    const unsigned int n_gauss_points = pressure_degree + 2;
    const UpdateFlags  cell_flags     = update_values | update_gradients | update_quadrature_points | update_JxW_values;
  ...
  StokesProblem<dim>::compute_errors(unsigned int k)
  {
    ...
    const double mean_pressure =
      VectorTools::compute_mean_value(dof_handler, QGauss<dim>(pressure_degree + 3), solution, dim);
    ...
    deallog << " At " << k + 1 << "th mesh" << std::endl
            ...
            << " Hdiv_Conv_rate1: " << std::setw(6) << (k == 0 ? 0 : last_Hdiv_error1 / Velocity_Hdiv_error1)
            << std::endl
            << " L2 pressure:  " << std::setw(12) << Pressure_L2_error << std::setw(0) << " rate: " << std::setw(6)
            << (k == 0 ? 0 : last_Pressure_L2_error / Pressure_L2_error) << std::endl
            << std::setw(0) << std::endl;
  ...
  try
    {
      ...
      fe = std::make_unique<FESystem<dim>>(FESystem<dim>(FE_DGQ<dim>(degree), dim), 1, FE_DGQ<dim>(degree - 1), 1);

      deallog << fe->get_name() << ": degree=" << fe->degree << " tensor_degree=" << fe->tensor_degree() << std::endl;

...
main()
{
  ...
  // 2D test
  {
    ...
    GridGenerator::concentric_hyper_shells(triangulation, center, 1.0, 4.0, 3u, 2.0, 4, true);
        ...
        for (const unsigned int face_n : GeometryInfo<2>::face_indices())
          if (cell->face(face_n)->at_boundary())
            deallog << "boundary face center distance to origin: "
                    << (cell->face(face_n)->center(/*respect_manifold*/ true) - center).norm()
  ...
  // 3D test
  {
    ...
    GridGenerator::concentric_hyper_shells(triangulation, center, 1.0, 5.0, 2u, 2.0, 0, true);

  ...
  for (unsigned int c = 0; c < n_cells; ++c)
    {
      ...
      const auto pos_cell = std::find(serial_cells.begin(), serial_cells.end(), cell);
      for (auto r : ranks)
        if (r != 0)
      ...
      if (pos_cell == serial_cells.end())
      ...
      else
        {
          ...
          for (const auto &p_idx : local_map)
            {
              auto serial_pt_pos = std::find(serial_local_map.begin(), serial_local_map.end(), p_idx);
                  ...
                  if ((quad[pt_num] - serial_qpoints[serial_cell_idx][serial_pt_idx]).norm() > 1e-10)
                    {
                      ...
                      deallog << "qpoint in serial : " << quad[pt_num]
                              << " Point in distributed: " << serial_qpoints[serial_cell_idx][serial_pt_idx]
                              << std::endl;
                    }
                }
              ++pt_num;

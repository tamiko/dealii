  ...
  for (auto &cell : sphere.active_cell_iterators())
    {
      ...
      if (my_pair.first.state() == IteratorState::valid && my_pair.first->is_locally_owned())
        {
          computed_pts++;
          ...
          if (output_points[c].size() != computed_points[c_cell].size())
            {
              ...
              for (unsigned int pt_idx = 0; pt_idx < output_points[c].size(); pt_idx++)
                deallog << output_points[c][pt_idx] << " from process " << output_ranks[c][pt_idx] << " to " << my_rank
                        << std::endl;
              ...
              for (unsigned int pt_idx = 0; pt_idx < output_points[c].size(); pt_idx++)
                {
                  ...
                  auto        pt_it = std::find(computed_points[c_cell].begin(), computed_points[c_cell].end(), pt);

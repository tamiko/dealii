  ...
  for (; cell_v2 != end_v2; ++cell_v2, ++cell_v4)
    {
      ...
      for (const unsigned int i : cell_v2->vertex_indices())
        {
          AssertThrow((cell_v2->vertex(i) - cell_v4->vertex(i)).norm() < 1.e-10, ExcInternalError());
        }
      for (const unsigned int i : GeometryInfo<dim>::face_indices())
        {
          AssertThrow(cell_v2->face(i)->boundary_id() == cell_v4->face(i)->boundary_id(), ExcInternalError());
        }
      for (unsigned int i = 0; i < GeometryInfo<dim>::lines_per_cell; ++i)
        {
          AssertThrow(cell_v2->line(i)->boundary_id() == cell_v4->line(i)->boundary_id(), ExcInternalError());
        }
    }
  deallog << "  OK" << std::endl;
}
  ...
  try
    {
      ...
      gmsh_grid<2>(SOURCE_DIR "/grids/grid_in_msh_01.2d.msh", SOURCE_DIR "/grids/grid_in_msh_01.2d.v4.msh");
      deallog << "/grid_in_msh_01.2da.v4.msh" << std::endl;
      gmsh_grid<2>(SOURCE_DIR "/grids/grid_in_msh_01.2da.msh", SOURCE_DIR "/grids/grid_in_msh_01.2da.v4.msh");
      deallog << "/grid_in_msh_01.3d.v4.msh" << std::endl;
      gmsh_grid<3>(SOURCE_DIR "/grids/grid_in_msh_01.3d.msh", SOURCE_DIR "/grids/grid_in_msh_01.3d.v4.msh");
      deallog << "/grid_in_msh_01.3da.v4.msh" << std::endl;
      gmsh_grid<3>(SOURCE_DIR "/grids/grid_in_msh_01.3da.msh", SOURCE_DIR "/grids/grid_in_msh_01.3da.v4.msh");
      deallog << "/grid_in_msh_01.3d_neg.v4.msh" << std::endl;
      gmsh_grid<3>(SOURCE_DIR "/grids/grid_in_msh_01.3d_neg.msh", SOURCE_DIR "/grids/grid_in_msh_01.3d_neg.v4.msh");
    }

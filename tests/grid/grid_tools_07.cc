...
generate_grid(Triangulation<2> &triangulation)
{
  ...
  GridGenerator::merge_triangulations(triangulation1, triangulation2, triangulation, 0., false, true);
}
  ...
  for (const auto &cell : triangulation.active_cell_iterators())
    {
      for (unsigned int face = 0; face < cell->n_faces(); ++face)
        {
          if (cell->face(face)->at_boundary() && cell->face(face)->boundary_id() == boundary_id)
            {
...
main()
{
             ...
             "point:\n";
  {
    ...
    const auto        cell_idxs  = mark_vertices_at_boundary(10, triangulation, marked_vertices);
    const int         found_cell = find_cell_at_point(p, triangulation, marked_vertices, cell_hint);
             ...
             "point but is not marked:\n\n";
  {
    ...
    // mark cell 4
    const auto cell_idxs = mark_vertices_at_boundary(9, triangulation, marked_vertices);

  ...
  for (unsigned int proc = 0; proc < Utilities::MPI::n_mpi_processes(mpi_communicator); ++proc)
    {
      if (proc == Utilities::MPI::this_mpi_process(mpi_communicator))
        {
          for (typename std::vector<cell_iterator>::const_iterator it_1 = ghost_cells_tria.begin(),
               ...
               ++it_1, ++it_2)
            {
              ...
              deallog << "Ghost " << cell_1->level() << ' ' << cell_1->index() << ' ' << cell_1->id() << ' '
                      << cell_1->id().to_string() << ' ' << "Halo " << cell_2->level() << ' ' << cell_2->index() << ' '
                      << cell_2->id() << ' ' << cell_2->id().to_string() << std::endl;

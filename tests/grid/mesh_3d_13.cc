        ...
        for (unsigned int subface_no = 0; subface_no < GeometryInfo<3>::max_children_per_face; ++subface_no)
          {
            ...
            const Triangulation<3>::active_cell_iterator neighbor_child = neighbor->child(neighbor_child_index);

            AssertThrow(neighbor_child->face(neighbor_neighbor) == cell->face(face_no)->child(subface_no),
                        ExcInternalError());

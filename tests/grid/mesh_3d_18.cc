...
test_with_wrong_face_orientation()
{
  ...
  deallog << "cell->face_orientation(3)=" << (cell->face_orientation(3) ? "true" : "false") << std::endl;

  const Triangulation<3>::active_cell_iterator neighbor_child = cell->neighbor_child_on_subface(3, 1);

  deallog << "cell->neighbor_child_on_subface(3,1)=" << neighbor_child << std::endl;
  deallog << "cell->neighbor_child_on_subface(3,1)->neighbor(5)=" << neighbor_child->neighbor(5) << std::endl;
  ...
  // status we expect.
  AssertThrow(cell->face(3)->child(1) == neighbor_child->face(cell->neighbor_of_neighbor(3)), ExcInternalError());
}

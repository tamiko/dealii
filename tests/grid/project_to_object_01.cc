...
main()
{
  ...
  // test the internal cross derivative function: the stencil should be order 2.
  {
    ...
    const std::array<double, 3> exact{{2.0 * std::cos(3.0) * std::cos(2.0) + 3.0 * std::sin(3.0) * std::sin(2.0),
    ...
    for (unsigned int cross_derivative_n = 0; cross_derivative_n < cross_derivatives.size(); ++cross_derivative_n)
      {
        ...
        for (unsigned int step_n = 5; step_n < 10; ++step_n)
          {
            ...
            errors.push_back(std::log(
              std::abs(cross_stencil<structdim>(cross_derivatives[cross_derivative_n], c0, current_step, objective) -
        ...
        deallog << "slope is nearly 3: " << (std::abs(numerator / denominator - 3.0) < 0.05) << std::endl;
      }
  }
  ...
  // Test project_to_object in 2D with an annulus
  {
    ...
    // geodesic
    {
      ...
      deallog << std::endl
              ...
              << std::endl
              << "vertex 0 distance from origin: " << face->vertex(0).distance(Point<2>()) << std::endl
              << "vertex 1 distance from origin: " << face->vertex(1).distance(Point<2>()) << std::endl
              << "trial point distance from origin: " << trial_point.distance(Point<2>()) << std::endl
              << "projected point distance from origin: " << projected_point.distance(Point<2>()) << std::endl
              << "projected point:        " << projected_point << std::endl
              << "true geodesic midpoint: " << face->get_manifold().get_new_point(vertices_view, weights_view)
              << std::endl;
    }
    ...
    // harder test: project a vertex onto the geodesic
    {
      ...
      const Point<2> projected_point = GridTools::project_to_object(cell->face(face_n), trial_point);

      deallog << std::endl
              << "Project a vertex onto the geodesic:" << std::endl
              << std::endl
              << "vertex distance from origin:          " << face->vertex(1).distance(Point<2>()) << std::endl
              << "trial point distance from origin:     " << trial_point.distance(Point<2>()) << std::endl
              << "projected point distance from origin: " << projected_point.distance(Point<2>()) << std::endl
              << "projected point: " << projected_point << std::endl
    ...
    for (auto &cell : triangulation.active_cell_iterators())
      {
        ...
        Assert((torus_manifold.push_forward(torus_manifold.pull_back(projected_point)) - projected_point).norm() <
                 1e-14,
               ExcInternalError());
  ...
  for (unsigned int n_refinements = 4; n_refinements < 7; ++n_refinements)
    {
      ...
      // easy test: project the arithmetic mean of the vertices onto the face
      {
        ...
        deallog << std::endl
                ...
                << std::endl
                << "vertex 0 distance from origin: " << face->vertex(0).distance(Point<3>()) << std::endl
                << "vertex 1 distance from origin: " << face->vertex(1).distance(Point<3>()) << std::endl
                << "trial point distance from origin: " << trial_point.distance(Point<3>()) << std::endl
                << "projected point distance from origin: " << projected_point.distance(Point<3>()) << std::endl;

        const std::array<Point<3>, 4> vertices{{face->vertex(0), face->vertex(1), face->vertex(2), face->vertex(3)}};
        const std::array<double, 4>   weights{{0.25, 0.25, 0.25, 0.25}};
        const auto                    vertices_view = make_array_view(vertices.begin(), vertices.end());
        const auto                    weights_view  = make_array_view(weights.begin(), weights.end());

        const Point<3> true_midpoint = face->get_manifold().get_new_point(vertices_view, weights_view);
        deallog << "projected point:    " << projected_point << std::endl
                << "true line midpoint: " << true_midpoint << std::endl
                << "distance less than 5.0e-6: " << (projected_point.distance(true_midpoint) < 5.0e-6) << std::endl;
      }
      ...
      // project (nearly) a vertex onto the surface
      {
        ...
        const Point<3> trial_point     = face->get_manifold().get_new_point(vertices_view, weights_view);
        const Point<3> projected_point = GridTools::project_to_object(face, trial_point);
      ...
      // test that we can recover a point that is on the face
      {
        ...
        deallog << std::endl
                ...
                << "trial point distance from origin:     " << trial_point.distance(Point<3>()) << std::endl
                << "projected point distance from origin: " << projected_point.distance(Point<3>()) << std::endl
                << "Error is less than 1.0e-3:           " << (projected_point.distance(trial_point) < 1.0e-3)
                << std::endl;
      }
      ...
      // test that we can recover a point that is on a line in 3D
      {
        ...
        deallog << std::endl
                ...
                << "trial point distance from origin:     " << trial_point.distance(Point<3>()) << std::endl
                << "projected point distance from origin: " << projected_point.distance(Point<3>()) << std::endl
                << "Error less than 1.0e-7:               " << (projected_point.distance(trial_point) < 1.0e-7)
                << std::endl;
      }
    }
  deallog << "OK" << std::endl;

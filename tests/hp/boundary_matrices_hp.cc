...
check()
{
  ...
  for (unsigned int i = 0; i < 5 - dim; ++i)
    element.push_back(FESystem<dim>(FE_RaviartThomasNodal<dim>(i), 1, FE_DGQ<dim>(i), 1, FE_Nothing<dim>(), dim));

  // ... also add Q-(i+1) ^ dim
  // to this system
  for (unsigned int i = 1; i < 3; ++i)
    element.push_back(FESystem<dim>(FE_Nothing<dim>(dim), 1, FE_Nothing<dim>(), 1, FE_Q<dim>(i), dim));

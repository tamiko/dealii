      ...
      if (cell->line(line)->n_active_fe_indices() > 1)
        {
          ...
          for (unsigned int i = 0; i < cell->line(line)->n_active_fe_indices(); ++i)
            {
              deallog << "  active_fe_index=" << cell->line(line)->nth_active_fe_index(i) << " ("
                      << fe_collection[cell->line(line)->nth_active_fe_index(i)].get_name() << ')' << std::endl;

              cell->line(line)->get_dof_indices(line_dof_indices_1, cell->line(line)->nth_active_fe_index(i));
              deallog << "  dof indices=";
          ...
          for (unsigned int i = 0; i < cell->line(line)->n_active_fe_indices(); ++i)
            for (unsigned int j = i + 1; j < cell->line(line)->n_active_fe_indices(); ++j)
              {
                cell->line(line)->get_dof_indices(line_dof_indices_1, cell->line(line)->nth_active_fe_index(i));
                cell->line(line)->get_dof_indices(line_dof_indices_2, cell->line(line)->nth_active_fe_index(j));

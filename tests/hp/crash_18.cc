    ...
    for (unsigned int index = 0; cell != endc; ++cell, ++index)
      if (cell->refine_flag_set() && (smoothness_indicators(index) > cutoff_smoothness) &&
          !(cell->active_fe_index() == fe_collection.size() - 1))
        {
          ...
          cell->set_active_fe_index(std::min<types::fe_index>(cell->active_fe_index() + 1, fe_collection.size() - 1));
        }
  }
  ...
  for (unsigned int cycle = 0; cycle < 2; ++cycle)
    {
      ...
      deallog << "   Number of constraints       : " << hanging_node_constraints.n_constraints() << std::endl;

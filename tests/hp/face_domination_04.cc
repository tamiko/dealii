  ...
  for (unsigned int i = 0; i < 2; ++i)
    {
      ...
      fe_collection.push_back(FESystem<dim>(FE_Nothing<dim>(dim), 1, FE_Nothing<dim>(), 1, FE_Q<dim>(i + 1), 1));
      fe_collection.push_back(FESystem<dim>(FE_RaviartThomasNodal<dim>(i), 1, FE_DGQ<dim>(i), 1, FE_Q<dim>(i + 2), 1));

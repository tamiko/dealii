...
test()
{
  ...
  typename DoFHandler<dim>::active_cell_iterator cell = dof_handler.begin_active(0), endc = dof_handler.end();
  for (; cell != endc; ++cell)
    if (cell->index() % 2 == 0)
    ...
    for (; cell != endc; ++cell)
      {
        ...
        for (unsigned int i = 0; i < cell->get_fe().dofs_per_cell; ++i)
          deallog << local_dof_indices[i] << (constraints.is_constrained(local_dof_indices[i]) ? "*" : "") << ' ';
        deallog << std::endl;
      }
  }
}

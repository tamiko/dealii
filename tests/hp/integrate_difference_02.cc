...
test()
{
  ...
  VectorTools::interpolate(dof_handler,
                           Functions::Monomial<dim>(
                             dim == 1 ? Point<dim>(1.) : (dim == 2 ? Point<dim>(1., 0.) : Point<dim>(1., 0., 0.))),
                           vec);
  ...
  {
    VectorTools::integrate_difference(
      dof_handler, vec, Functions::ZeroFunction<dim>(), diff, q_collection, VectorTools::H1_seminorm);
  ...
  {
    VectorTools::integrate_difference(
      dof_handler, vec, Functions::ZeroFunction<dim>(), diff, q_collection, VectorTools::W1infty_seminorm);
  ...
  {
    VectorTools::integrate_difference(
      dof_handler, vec, Functions::ZeroFunction<dim>(), diff, q_collection, VectorTools::W1infty_norm);

      ...
      for (unsigned int q = 0; q <= p + 2; ++q)
        {
          ...
          VectorTools::integrate_difference(
            dof_handler, interpolant, F<dim>(q), error, hp::QCollection<dim>(QGauss<dim>(q + 2)), VectorTools::L2_norm);
          if (q <= p)
            AssertThrow(error.l2_norm() < 1e-12 * interpolant.l2_norm(), ExcInternalError());

          deallog << fe.get_name() << ", P_" << q << ", rel. error=" << error.l2_norm() / interpolant.l2_norm()
                  << std::endl;
        }
    }
}

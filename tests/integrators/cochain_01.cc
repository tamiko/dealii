...
test_cochain(const Triangulation<dim> &tr, const FiniteElement<dim> &fe)
{
  ...
  if (debugging)
    for (unsigned int i = 0; i < matrices.size(); ++i)
      deallog << "Block " << '(' << matrices.block(i).row << ',' << matrices.block(i).column << ") " << std::setw(3)
              << std::right << matrices.matrix(i).m() << std::left << 'x' << std::setw(3) << matrices.matrix(i).n()
  ...
  MeshWorker::loop<dim, dim, MeshWorker::DoFInfo<dim>, MeshWorker::IntegrationInfoBox<dim>>(
    dof.begin_active(), dof.end(), dof_info, info_box, cell_matrix<dim>, nullptr, nullptr, assembler);
  ...
  for (unsigned int d = 0; d < dim; ++d)
    {
      ...
      if (debugging)
        deallog << "u " << source.block(d).l2_norm() << ' ' << matrices.name(m + 2) << ' '
                << result1.block(d + 1).l2_norm() << ' ' << matrices.name(m + 3) << ' ' << aux.block(d + 1).l2_norm()
                << ' ' << matrices.name(m + 2) << "^T " << result1.block(d).l2_norm() << ' ' << matrices.name(m + 1)

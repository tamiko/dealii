...
main()
{
  ...
  if (true)
    {
      LaplaceProblem<BlockVector<double>, BlockSparseMatrix<double>, BlockSparsityPattern> laplace_problem(2);
      laplace_problem.run();
  ...
  if (true)
    {
      LaplaceProblem<BlockVector<double>, BlockSparseMatrix<double>, BlockSparsityPattern> laplace_problem(3);
      laplace_problem.run();
  ...
  for (unsigned int i = 1; i < n_datasets; ++i)
    {
      ...
      for (unsigned int j = 0; j < solutions[0].size(); ++j)
        if (std::fabs(solutions[i][j] - solutions[0][j]) > accuracy * std::fabs(solutions[i][j] + solutions[0][j]))
          {

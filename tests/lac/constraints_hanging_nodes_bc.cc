...
test()
{
...

  {
    ...
    VectorTools::interpolate_boundary_values(dof, 0, Functions::ConstantFunction<dim>(1.), boundary_values);
  ...
  VectorTools::interpolate_boundary_values(dof, 0, Functions::ConstantFunction<dim>(1.), library_constraints);
  ...
  for (unsigned int i = 0; i < dof.n_dofs(); ++i)
    {
      AssertThrow(correct_constraints.is_constrained(i) == library_constraints.is_constrained(i), ExcInternalError());
      using constraint_format = const std::vector<std::pair<types::global_dof_index, double>> &;
          ...
          for (unsigned int q = 0; q < correct.size(); ++q)
            {
              ...
              AssertThrow(std::fabs(correct[q].second - library[q].second) < 1e-14, ExcInternalError());
            }
          AssertThrow(std::fabs(correct_constraints.get_inhomogeneity(i) - library_constraints.get_inhomogeneity(i)) <
                        1e-14,
                      ExcInternalError());
        }
    }

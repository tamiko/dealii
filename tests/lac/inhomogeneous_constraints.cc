  ...
  for (; cell != endc; ++cell)
    {
      ...
      test_all_constraints.distribute_local_to_global(cell_matrix, cell_rhs, local_dof_indices, test_matrix, test_rhs);
    }
  ...
  for (unsigned int cycle = 0; cycle < 3; ++cycle)
    {
      ...
      deallog << std::endl
              ...
              << "   Number of hanging node constraints: " << hanging_nodes_only.n_constraints() << std::endl
              << "   Total number of constraints:        " << test_all_constraints.n_constraints() << std::endl
              << "   Number of inhomogenous constraints: " << test_all_constraints.n_inhomogeneities() << std::endl
              << "   Number of identity constraints:     " << test_all_constraints.n_identities() << std::endl;
  ...
  for (unsigned int index = 0; cell != endc; ++cell, ++index)
    {
      ...
      const double mu =
        (1. / (sum_1 * sum_ln_k_square - sum_ln_k * sum_ln_k) * (sum_ln_k * sum_ln_U - sum_1 * sum_ln_U_ln_k));

...
test()
{
  ...
  Kokkos::View<double *, MemorySpace::Default::kokkos_space> v_device(v.get_values(), local_relevant.n_elements());
  ...
  deallog << myid << ":"
          << "first owned entry: " << print_value(v.get_values(), partitioner->global_to_local(myid * 2)) << std::endl;
  deallog << myid << ":"
          << "second owned entry: " << print_value(v.get_values(), partitioner->global_to_local(myid * 2 + 1))
          << std::endl;

  // set ghost dof on owning processor and maximize
  if (myid != 0)
    Kokkos::deep_copy(Kokkos::subview(v_device, partitioner->global_to_local(1)), 7. * myid);
  ...
  // check
  deallog << myid << ":"
          << "ghost entry after max from owner: " << print_value(v.get_values(), partitioner->global_to_local(1))
          << std::endl;
  ...
  // check
  deallog << myid << ":"
          << "ghost entry after min from zero: " << print_value(v.get_values(), partitioner->global_to_local(1))
          << std::endl;
  ...
  // check
  deallog << myid << ":"
          << "ghost entry after min from : " << print_value(v.get_values(), partitioner->global_to_local(1))
          << std::endl;
  ...
  deallog << myid << ":"
          << "ghost entry after max and partly init: " << print_value(v.get_values(), partitioner->global_to_local(1))
          << std::endl;
  ...
  v.update_ghost_values();
  deallog << myid << ":"
          << "ghost entry after max and full init: " << print_value(v.get_values(), partitioner->global_to_local(1))
          << std::endl;
  ...
  v.compress(VectorOperation::max);
  deallog << myid << ":"
          << "ghost entry after first max: " << print_value(v.get_values(), partitioner->global_to_local(1))
          << std::endl;
  v.compress(VectorOperation::max);
  deallog << myid << ":"
          << "ghost entry after second max: " << print_value(v.get_values(), partitioner->global_to_local(1))
          << std::endl;

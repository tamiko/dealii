...
#define PRINTME(name, var) deallog << "Solution vector: " << name << ": " << var << std::endl;
  ...
  // deal.II BlockSparseMatrix
  {
...

    {
      ...
      // Preconditinoed by S_approx_inv
      {
        ...
        IterationNumberControl                   solver_control_S_approx(1,
                                                       1.0e-10,
                                                       false,
                                                       false); // Perform only a limited number of sweeps

...
do_test()
{
  ...
  auto convert_to_vectorized = [](const Vector<double> &in, AlignedVector<VectorizedArray<double>> &out) {
    std::transform(in.begin(), in.end(), out.begin(), [](const double &val) { return make_vectorized_array(val); });
  };
  ...
  auto subtract_and_assign = [](AlignedVector<VectorizedArray<double>>       &lhs,
    ...
    std::transform(lhs.begin(),
                   ...
                   rhs.begin(),
                   lhs.begin(),
                   [](const VectorizedArray<double> lval, const VectorizedArray<double> rval) { return lval - rval; });
  };
              ...
              for (unsigned int kk = 0; kk < size; ++kk, ++cc)
                if (dim == 1)
                  ...
                  full(c, cc) = laplace[2](i, ii)[0] * mass[1](j, jj)[0] * mass[0](k, kk)[0] +
                                mass[2](i, ii)[0] *
                                  (laplace[1](j, jj)[0] * mass[0](k, kk)[0] + mass[1](j, jj)[0] * laplace[0](k, kk)[0]);
  full.vmult(w2, w1);

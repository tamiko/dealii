...
do_test()
{
  ...
  auto convert_to_vectorized = [](const Vector<float> &in, AlignedVector<VectorizedArray<float>> &out) {
    std::transform(in.begin(), in.end(), out.begin(), [](const float &val) { return make_vectorized_array(val); });
  };
  ...
  auto subtract_and_assign = [](AlignedVector<VectorizedArray<float>>       &lhs,
    ...
    std::transform(lhs.begin(),
                   ...
                   rhs.begin(),
                   lhs.begin(),
                   [](const VectorizedArray<float> lval, const VectorizedArray<float> rval) { return lval - rval; });
  };

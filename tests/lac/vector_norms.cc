  ...
  for (unsigned int size = 1; size < 200000; size += skip)
    {
      ...
      AssertThrow(std::abs(l1_norm - 0.5 * size * (size + 1)) < acc * 0.5 * size * (size + 1), ExcInternalError());
      ...
      AssertThrow(std::abs(lp_norma - value * std::pow(static_cast<number>(size), static_cast<number>(1. / 3.))) <
                    std::max(acc, number(1e-15)) * std::pow(static_cast<number>(size), static_cast<number>(1. / 3.)) *
                      value,
                  ExcInternalError());
    }
}
  ...
  for (unsigned int size = 1; size < 100000; size += skip)
    {
      ...
      AssertThrow(std::abs(lp_norma -
                           std::abs(value) * std::pow(static_cast<number>(size), static_cast<number>(1. / 3.))) <

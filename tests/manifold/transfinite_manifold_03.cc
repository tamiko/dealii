  ...
  for (unsigned int degree = 1; degree < 5; ++degree)
    {
      ...
      double                    volume = 0;
      for (typename Triangulation<dim, spacedim>::cell_iterator cell = tria.begin_active(); cell != tria.end(); ++cell)
        {
      ...
      deallog << std::setw(4) << tria.n_active_cells() << " cells, degree=" << degree << ": " << std::setprecision(12)

...
main()
{
  ...
  // of inner cell
  std::vector<Point<3>> vertices{{1.6666666666666663, 0., -0.69035593728849154},
  ...
  tria.set_manifold(2, CylindricalManifold<dim>(Point<dim>{std::sqrt(0.5), 0., std::sqrt(0.5)}, Point<dim>()));
  TransfiniteInterpolationManifold<dim> transfinite;
  transfinite.initialize(tria);
  tria.set_manifold(1, transfinite);

  deallog << "Cell center " << tria.begin_active()->center(true, true) << std::endl;

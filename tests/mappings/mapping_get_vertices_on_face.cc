             ...
             const Mapping<dim>                               &mapping)
{
  ...
  for (unsigned int i = 0; i < face->n_vertices(); ++i)
    {
      vertices[i] =
        mapping.transform_unit_to_real_cell(cell, mapping.transform_real_to_unit_cell(cell, face->vertex(i)));
    }
  return vertices;
}
...
test_vertex_order()
{
  ...
  for (const auto &cell : tria.active_cell_iterators())
    for (unsigned int f = 0; f < cell->n_faces(); ++f)
      {
        const auto &vertices_detour = get_vertices(cell, cell->face(f), mapping);

...
regression_slope(const std::vector<double> &x, const std::vector<double> &y)
{
  ...
  Assert(x.size() >= 2, dealii::ExcMessage("at least two points are required for linear regression fit"));
  ...
  for (unsigned mapping_p = 2; mapping_p < fe.degree + 3; ++mapping_p)
    {
      ...
      for (unsigned int refinement_n = 1; refinement_n < 4; ++refinement_n)
        {
          ...
          VectorTools::project(
            mapping, dof_handler, constraints, QGauss<dim>(fe.degree + (mapping_p + 2) / 2), fe_function, v);
      ...
      deallog << "regression slope: " << regression_slope(log_refinements, log_l2_errors) << std::endl;
    }
}

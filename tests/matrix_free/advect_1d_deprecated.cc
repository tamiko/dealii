        ...
        for (unsigned int q = 0; q < phi_m.n_q_points; ++q)
          {
            ...
            const VectorizedArray<number> normal_times_advection = advection * phi_m.get_normal_vector(q);
        ...
        for (unsigned int q = 0; q < fe_eval.n_q_points; ++q)
          {
            ...
            const VectorizedArray<number> normal_times_advection = advection * fe_eval.get_normal_vector(q);
            const value_type              flux_times_normal =
              make_vectorized_array<number>(0.5) *
              ((u_minus + u_plus) * normal_times_advection + std::abs(normal_times_advection) * (u_minus - u_plus));

    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        ...
        for (unsigned int q = 0; q < phi.n_q_points; ++q)
          {
            Tensor<1, n_components, Tensor<1, dim, VectorizedArray<number>>> grad;
...
public:
  ...
  value(const Point<dim> &p, const unsigned int component) const override
  {
    ...
    return -std::cos(3 * numbers::PI * p[0] / 0.8) * advection[0] * 3 * numbers::PI / 0.8 * (component + 1.2);
  }
};
...
test(const unsigned int n_refine)
{
  ...
  MatrixFreeAdvectionBasic<dim, fe_degree, fe_degree + 1, double, LinearAlgebra::distributed::Vector<double>, 3> mf2(
    mf_data);
  mf2.vmult(out, in);

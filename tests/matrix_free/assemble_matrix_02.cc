...
do_test(const DoFHandler<dim> &dof)
{
  ...
  // compute matrix with (\nabla v, \nabla u) + (v, 10 * u)
  {
    ...
    FEEvaluation<dim, degree_p + 1, degree_p + 2, dim> phi_u(
      mapping, dof.get_fe(), QGauss<1>(degree_p + 2), update_values | update_gradients | update_JxW_values, 0);
    ...
    for (; cell != endc; ++cell)
      {
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
              {
                for (unsigned int j = 0; j < dofs_per_cell; ++j)
                  {
                    cell_matrix(i, j) +=
                      (phi_grads_u[i] * phi_grads_u[j] - div_phi_u[i] * phi_pres[j] - phi_pres[i] * div_phi_u[j]) *
                      fe_values.JxW(q);
                  }
              }
          }
        ...
        for (unsigned int i = 0; i < dofs_per_cell_u; i += VectorizedArray<double>::size())
          {
            const unsigned int n_items = i + VectorizedArray<double>::size() > dofs_per_cell_u ?
                                           (dofs_per_cell_u - i) :
            ...
            for (unsigned int v = 0; v < n_items; ++v)
              {
                for (unsigned int j = 0; j < dofs_per_cell_u; ++j)
                  test_matrix(phi_u.get_internal_dof_numbering()[j], phi_u.get_internal_dof_numbering()[i + v]) =
                    phi_u.begin_dof_values()[j][v];
                for (unsigned int j = 0; j < dofs_per_cell_p; ++j)
                  test_matrix(phi_p.get_internal_dof_numbering()[j], phi_u.get_internal_dof_numbering()[i + v]) =
                    phi_p.begin_dof_values()[j][v];
              }
          }

        for (unsigned int i = 0; i < dofs_per_cell_p; i += VectorizedArray<double>::size())
          {
            const unsigned int n_items = i + VectorizedArray<double>::size() > dofs_per_cell_p ?
                                           (dofs_per_cell_p - i) :
                                           VectorizedArray<double>::size();
            for (unsigned int j = 0; j < dofs_per_cell_p; ++j)
            ...
            for (unsigned int v = 0; v < n_items; ++v)
              for (unsigned int j = 0; j < dofs_per_cell_u; ++j)
                test_matrix(phi_u.get_internal_dof_numbering()[j], phi_p.get_internal_dof_numbering()[i + v]) =
                  phi_u.begin_dof_values()[j][v];
          }

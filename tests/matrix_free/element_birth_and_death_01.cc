...
public:
  ...
  {
    return std::sin(point[0] * 2 * numbers::PI) * std::sin(point[1] * 2 * numbers::PI);
  }
};
...
test(const unsigned int n_refinements)
{
  ...
  using FEFaceIntegrator    = FEFaceEvaluation<dim, -1, 0, 1, Number, VectorizedArrayType>;

  for (unsigned int i = 0; i < 2; ++i)
    {
      parallel::distributed::Triangulation<dim> tria(MPI_COMM_WORLD);
      if (i == 0)
        GridGenerator::subdivided_hyper_rectangle(tria, {2, 2}, {0.0, 0.0}, {1.0, 1.0}, true);
      else
        GridGenerator::subdivided_hyper_rectangle(tria, {2, 1}, {0.0, 0.0}, {1.0, 0.5}, true);
      ...
      dof_handler.distribute_dofs(hp::FECollection<dim>(FE_Q<dim>(1), FE_Nothing<dim>(1)));
      ...
      const auto cell_operation = [&](const auto &matrix_free, auto &dst, const auto &src, const auto range) {
      ...
      const auto face_operation = [&](const auto &, auto &, const auto &, const auto) {
        // nothing to do here but in the DG case
      };

      const auto boundary_operation =
        [&](const auto &matrix_free, auto &, const auto &, const auto range, const bool is_interior_face = true) {
          ...
          for (unsigned int face = range.first; face < range.second; ++face)
            {
              ...
              const double scaling = phi.at_boundary() ? (phi.boundary_id() + 1.0) : (2.0 * dim);
      ...
      if (i == 0)
        {
          MatrixFreeTools::ElementActivationAndDeactivationMatrixFree<dim, Number, VectorizedArrayType>
            element_birth_and_death;

          element_birth_and_death.reinit(matrix_free);

          element_birth_and_death.template cell_loop<VectorType, VectorType>(cell_operation, dst, src, true);
        ...
        {
          matrix_free.template cell_loop<VectorType, VectorType>(cell_operation, dst, src, true);

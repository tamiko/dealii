...
public:
          ...
          for (unsigned int q = 0; q < fe_eval.n_q_points; ++q)
            {
              ...
              for (unsigned int d = 0; d < dim; ++d)
                cell_errors[1] += std::abs(fe_eval.get_gradient(q)[d][j] - function.gradient(p, 0)[d]);
              for (unsigned int d = 0; d < dim; ++d)
                for (unsigned int e = 0; e < dim; ++e)
                  cell_errors[2] += std::abs(fe_eval.get_hessian(q)[d][e][j] - function.hessian(p, 0)[d][e]);
            }
      }
  }
    ...
    for (unsigned int face = face_range.first; face < face_range.second; ++face)
      {
        ...
        fe_evalm.evaluate(EvaluationFlags::values | EvaluationFlags::gradients | EvaluationFlags::hessians);
        fe_evalp.reinit(face);
        fe_evalp.read_dof_values(src);
        fe_evalp.evaluate(EvaluationFlags::values | EvaluationFlags::gradients | EvaluationFlags::hessians);
            ...
            for (unsigned int q = 0; q < fe_evalm.n_q_points; ++q)
              {
                ...
                for (unsigned int d = 0; d < dim; ++d)
                  facem_errors[1] += std::abs(fe_evalm.get_gradient(q)[d][j] - function.gradient(p, 0)[d]);
                for (unsigned int d = 0; d < dim; ++d)
                  for (unsigned int e = 0; e < dim; ++e)
                    facem_errors[2] += std::abs(fe_evalm.get_hessian(q)[d][e][j] - function.hessian(p, 0)[d][e]);
                double normal_derivative = 0;
                for (unsigned int d = 0; d < dim; ++d)
                  normal_derivative += function.gradient(p, 0)[d] * fe_evalm.normal_vector(q)[d][j];
                facem_errors[3] += std::abs(fe_evalm.get_normal_derivative(q)[j] - normal_derivative);

                // exterior face
                facep_errors[0] += std::abs(fe_evalp.get_value(q)[j] - function.value(p, 0));
                for (unsigned int d = 0; d < dim; ++d)
                  facep_errors[1] += std::abs(fe_evalp.get_gradient(q)[d][j] - function.gradient(p, 0)[d]);
                for (unsigned int d = 0; d < dim; ++d)
                  for (unsigned int e = 0; e < dim; ++e)
                    facep_errors[2] += std::abs(fe_evalp.get_hessian(q)[d][e][j] - function.hessian(p, 0)[d][e]);
                facep_errors[3] += std::abs(fe_evalp.get_normal_derivative(q)[j] - normal_derivative);
              }
          }
      }
  }
            ...
            for (unsigned int q = 0; q < fe_evalm.n_q_points; ++q)
              {
                ...
                for (unsigned int d = 0; d < dim; ++d)
                  boundary_errors[1] += std::abs(fe_evalm.get_gradient(q)[d][j] - function.gradient(p, 0)[d]);
                for (unsigned int d = 0; d < dim; ++d)
                  for (unsigned int e = 0; e < dim; ++e)
                    boundary_errors[2] += std::abs(fe_evalm.get_hessian(q)[d][e][j] - function.hessian(p, 0)[d][e]);
                double normal_derivative = 0;
                for (unsigned int d = 0; d < dim; ++d)
                  normal_derivative += function.gradient(p, 0)[d] * fe_evalm.normal_vector(q)[d][j];
                boundary_errors[3] += std::abs(fe_evalm.get_normal_derivative(q)[j] - normal_derivative);
              }
          }
      }
  }
  ...
  test_functions(const Vector<Number> &src) const
  {
    ...
    data.loop(&MatrixFreeTest::cell, &MatrixFreeTest::face, &MatrixFreeTest::boundary, this, dst_dummy, src);
    ...
    deallog << "Error face+ grad*normal:" << facep_errors[3] / facep_times << std::endl;
...
do_test(const DoFHandler<dim> &dof, const AffineConstraints<double> &constraints)
{
  ...
  MatrixFree<dim, number> mf_data;
  {
    ...
    data.mapping_update_flags                = update_gradients | update_hessians | update_quadrature_points;
    data.mapping_update_flags_boundary_faces = update_gradients | update_hessians | update_quadrature_points;
    data.mapping_update_flags_inner_faces    = update_gradients | update_hessians | update_quadrature_points;

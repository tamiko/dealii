...
test()
{
  ...
  auto mf_data = std::make_shared<MatrixFree<dim, number, VectorizedArrayType>>();
  {
  ...
  MatrixFreeOperators::
    LaplaceOperator<dim, fe_degree, fe_degree + 1, 1, LinearAlgebra::distributed::Vector<number>, VectorizedArrayType>
      mf;

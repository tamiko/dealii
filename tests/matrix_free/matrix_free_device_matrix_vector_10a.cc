...
test()
{
  ...
  MatrixFreeTest<dim, fe_degree, Number, LinearAlgebra::distributed::Vector<Number, MemorySpace::Default>> mf(mf_data,
                                                                                                              coef_size,
                                                                                                              false);
      ...
      if (cell->is_locally_owned())
        {
          ...
          for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
            {
              const auto coef = VaryingCoefficient<dim>(fe_values.quadrature_point(q_point));

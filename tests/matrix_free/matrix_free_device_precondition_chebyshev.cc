...
test()
{
  ...
  LinearAlgebra::distributed::Vector<Number, MemorySpace::Default> in_dev(owned_set, MPI_COMM_WORLD);
  LinearAlgebra::distributed::Vector<Number, MemorySpace::Default> out_dev(owned_set, MPI_COMM_WORLD);
  ...
  LinearAlgebra::distributed::Vector<Number, MemorySpace::Host> matrix_diagonal(ref.get_partitioner());
  for (const auto index : matrix_diagonal.locally_owned_elements())
    matrix_diagonal[index] = sparse_matrix(index, index);

  using HostPreconditionerType = PreconditionChebyshev<TrilinosWrappers::SparseMatrix,
  ...
  host_preconditioner_data.preconditioner =
    std::make_shared<DiagonalMatrix<LinearAlgebra::distributed::Vector<Number, MemorySpace::Host>>>(matrix_diagonal);
  host_preconditioner_data.constraints.copy_from(constraints);
  precondition_chebyshev_host.initialize(sparse_matrix, host_preconditioner_data);

  using DevicePreconditionerType = PreconditionChebyshev<
    MatrixFreeTest<dim, fe_degree, Number, LinearAlgebra::distributed::Vector<Number, MemorySpace::Default>>,
  ...
  device_preconditioner_data.preconditioner =
    std::make_shared<DiagonalMatrix<LinearAlgebra::distributed::Vector<Number, MemorySpace::Default>>>(
      LinearAlgebra::distributed::Vector<Number, MemorySpace::Default>(ref.get_partitioner()));
  device_preconditioner_data.preconditioner->get_vector().import_elements(matrix_diagonal, VectorOperation::insert);

...
public:
  DEAL_II_HOST_DEVICE
  HelmholtzOperatorQuad(const typename CUDAWrappers::MatrixFree<dim, Number>::Data *gpu_data, Number *coef, int cell)
    : gpu_data(gpu_data)
    , coef(coef)
    , cell(cell)
  {}
  ...
  operator()(CUDAWrappers::FEEvaluation<dim, fe_degree, n_q_points_1d, 1, Number> *fe_eval, int q_point) const;
  ...
  Number                                                     *dst) const
{
  ...
  fe_eval.apply_for_each_quad_point(HelmholtzOperatorQuad<dim, fe_degree, Number, n_q_points_1d>(gpu_data, coef, cell));
...
public:
  ...
  DEAL_II_HOST_DEVICE void
  operator()(const typename CUDAWrappers::MatrixFree<dim, Number>::Data *gpu_data,
             const unsigned int                                          cell,
...
template <int dim,
          int fe_degree,
          typename Number,
          typename VectorType = LinearAlgebra::distributed::Vector<double, MemorySpace::Default>,
...
MatrixFreeTest<dim, fe_degree, Number, VectorType, n_q_points_1d>::MatrixFreeTest(
  const CUDAWrappers::MatrixFree<dim, Number> &data_in,
  const unsigned int                           size,
  ...
  : data(data_in)
{
  ...
  if (constant_coeff)
    ...
    {
      VaryingCoefficientFunctor<dim, fe_degree, Number, n_q_points_1d> functor(coef.get_values());
...
template <int dim, int fe_degree, typename Number, typename VectorType, int n_q_points_1d>
Number
MatrixFreeTest<dim, fe_degree, Number, VectorType, n_q_points_1d>::el(const unsigned int row,
...
template <int dim, int fe_degree, typename Number, typename VectorType, int n_q_points_1d>
void
MatrixFreeTest<dim, fe_degree, Number, VectorType, n_q_points_1d>::vmult(VectorType &dst, const VectorType &src) const
{
  dst = static_cast<Number>(0.);
  HelmholtzOperator<dim, fe_degree, Number, n_q_points_1d> helmholtz_operator(coef.get_values());

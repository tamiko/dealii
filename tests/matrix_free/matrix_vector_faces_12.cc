...
f_x(double x_m)
{
  ...
  if (x <= 9.0)
    ...
    y_m = 0.001 *
          (-28.0 + 4.046435022819e1 - 1.379581654948 * x + 1.945884504128e-2 * x * x - 2.070318932190e-4 * x * x * x);
    ...
    if (x[0] <= x_max / 2.0)
      {
        ...
        AssertThrow(iter < maxiter, ExcMessage("Newton within PullBack did not find the solution. "));
        xi[1] = Y;
      }
    else if (x[0] > x_max / 2.0)
      {

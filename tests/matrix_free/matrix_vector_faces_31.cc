...
do_test(const unsigned int fe_degree)
{
  ...
  for (unsigned int test = 0; test < (dim == 3 && fe_degree > 3 ? 1 : 2); ++test)
    {
      ...
      MatrixFreeTest<dim, -1, 0, double, LinearAlgebra::distributed::Vector<double>, dim> mf(mf_data);
      mf.vmult(out, in);

      MatrixFreeVariant<dim, -1, 0, double, LinearAlgebra::distributed::Vector<double>, dim> mf2(mf_data, true);
      ...
      for (unsigned int d = 0; d < dim; ++d)
        {
          MatrixFreeVariant<dim, -1, 0, double, LinearAlgebra::distributed::Vector<double>, 1> mf3(mf_data, true, d);
          if (d == 0)
      ...
      if (dim == 3)
        {
          MatrixFreeVariant<dim, -1, 0, double, LinearAlgebra::distributed::Vector<double>, 2> mf3(mf_data, true, 0);
          mf3.vmult(out_dist, in);
          MatrixFreeVariant<dim, -1, 0, double, LinearAlgebra::distributed::Vector<double>, 1> mf4(mf_data, true, 2);
      ...
      if (dim == 3)
        {
          MatrixFreeVariant<dim, -1, 0, double, LinearAlgebra::distributed::Vector<double>, 1> mf3(mf_data, true, 0);
          mf3.vmult(out_dist, in);
          MatrixFreeVariant<dim, -1, 0, double, LinearAlgebra::distributed::Vector<double>, 2> mf4(mf_data, true, 1);
          mf4.vmult_add(out_dist, in);
      ...
      MatrixFreeVariant<dim, -1, 0, double, LinearAlgebra::distributed::Vector<double>, 1> mf4(mf_data_scalar, true);

...
test()
{
  ...
  const std::vector<types::global_dof_index> dofs_per_block = DoFTools::count_dofs_per_fe_block(dof, stokes_sub_blocks);
  {
  ...
  deallog << "Verification fe degree " << degree << ": " << error / relative << std::endl << std::endl;
}

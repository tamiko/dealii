...
public:
  ...
  initialize(const Mapping<dim>                                        &mapping,
             ...
             const std::map<types::boundary_id, const Function<dim> *> &dirichlet_boundary,
            ...
            const LinearAlgebra::distributed::Vector<number> &src) const
  {
    ...
    for (unsigned int i = 0; i < edge_constrained_indices.size(); ++i)
      {
        const_cast<LinearAlgebra::distributed::Vector<double> &>(src).local_element(edge_constrained_indices[i]) =
    ...
    for (unsigned int i = 0; i < edge_constrained_indices.size(); ++i)
      {
        ...
        const_cast<LinearAlgebra::distributed::Vector<double> &>(src).local_element(edge_constrained_indices[i]) = 0.;
        edge_constrained_values[i] = std::pair<number, number>(src_val, dst.local_element(edge_constrained_indices[i]));
      }
  ...
  initialize_dof_vector(LinearAlgebra::distributed::Vector<number> &vector) const
  {
    ...
    Assert(vector.partitioners_are_globally_compatible(*data.get_dof_info(0).vector_partitioner), ExcInternalError());
  }
...
public:
  ...
  void
  Tvmult(LinearAlgebra::distributed::Vector<double> &dst, const LinearAlgebra::distributed::Vector<double> &src) const
  {
...
do_test(const DoFHandler<dim> &dof)
{
  ...
  fine_matrix.initialize(mapping, dof, mg_constrained_dofs, dirichlet_boundary, numbers::invalid_unsigned_int);
  ...
  using LevelMatrixType = LaplaceOperator<dim, fe_degree, n_q_points_1d, number>;
  ...
  for (unsigned int level = 0; level < dof.get_triangulation().n_global_levels(); ++level)
    {
      ...
      smoother_data[level].preconditioner.reset(new DiagonalMatrix<LinearAlgebra::distributed::Vector<number>>());
      smoother_data[level].preconditioner->get_vector() = mg_matrices[level].get_matrix_diagonal_inverse();
    }
  ...
  PreconditionMG<dim, LinearAlgebra::distributed::Vector<double>, MGTransferMatrixFree<dim, double>> preconditioner(
    dof, mg, mg_transfer);

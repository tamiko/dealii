...
public:
  ...
  print_n_calls_special()
  {
    ...
    if (n_calls_vmult > 0)
      deallog << "Approx. number of calls to special vmult for Operator of size " << m() << ": "
              << (n_calls_vmult + 5) / 10 * 10 << std::endl;
  }
             ...
             const unsigned int                  level = numbers::invalid_unsigned_int)
  {
    ...
    DoFRenumbering::matrix_free_data_locality(dof_handler, constraints, addit_data);
...
do_test(DoFHandler<dim> &dof)
{
  ...
  in = 1.;
  {
    ...
    VectorTools::interpolate_boundary_values(dof, 0, Functions::ZeroFunction<dim>(), boundary_values);
  ...
  MGCoarseGridApplySmoother<LinearAlgebra::distributed::Vector<number>> mg_coarse(mg_smoother);

...
test()
{
  ...
  auto cell_worker = [](const Iterator &cell, ScratchData &s, CopyData &c) {
    deallog << "Cell worker on : " << cell->id() << " is_locally_owned_on_level? " << cell->is_locally_owned_on_level()
            << std::endl;
    ++c.n_cells;
  ...
  auto face_worker = [](const Iterator     &cell,
    ...
    deallog << "Face worker on : " << cell << ", Neighbor cell : " << ncell << ", Face : " << f
            ...
            << ", neighbor->is_locally_owned_on_level(): " << ncell->is_locally_owned_on_level() << std::endl;
  };
  ...
  std::function<void(const Iterator &, const unsigned int &, ScratchData &, CopyData &)> empty_boundary_worker;

...
test()
{
  ...
  hp::FECollection<dim, spacedim> fe(FE_DGQ<dim, spacedim>(1), FE_DGQ<dim, spacedim>(2));
  ...
  using CopyData =
    MeshWorker::CopyData<1 + GeometryInfo<dim>::faces_per_cell, 1, 1 + GeometryInfo<dim>::faces_per_cell>;
            ...
            for (unsigned int j = 0; j < fev.dofs_per_cell; ++j)
              {
                c.matrices[0](i, j) +=
                  (-fev.shape_grad(i, q) * n[q] * fev.shape_value(j, q) +
                   ...
                   gamma / cell->face(f)->diameter() * fev.shape_value(i, q) * fev.shape_value(j, q)) *
                  JxW[q];
              }
            c.vectors[0](i) +=
              ((gamma / cell->face(f)->diameter() * fev.shape_value(i, q) - fev.shape_grad(i, q) * n[q]) *
    ...
    for (unsigned int q = 0; q < p.size(); ++q)
      {
        for (unsigned int i = 0; i < fev.dofs_per_cell; ++i)
          {
            for (unsigned int j = 0; j < fev.dofs_per_cell; ++j)
              c.matrices[0](i, j) += (-.5 * fev.shape_grad(i, q) * n[q] * fev.shape_value(j, q) +
            ...
            for (unsigned int j = 0; j < nfev.dofs_per_cell; ++j)
              c.matrices[f + 1](i, j) += (-.5 * fev.shape_grad(i, q) * nn[q] * nfev.shape_value(j, q) +
  ...
  mesh_loop(cell,
            ...
            scratch,
            copy,
            assemble_own_cells | assemble_boundary_faces | assemble_own_interior_faces_both,
            boundary_worker,
            face_worker);

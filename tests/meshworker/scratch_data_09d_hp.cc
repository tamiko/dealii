...
run(const ExtractorType &extractor)
{
  ...
  hp::FECollection<dim, spacedim> fe(
    FESystem<dim, spacedim>(FE_Q<dim, spacedim>(1), SymmetricTensor<2, dim>::n_independent_components),
    FESystem<dim, spacedim>(FE_Q<dim, spacedim>(3), SymmetricTensor<2, dim>::n_independent_components));

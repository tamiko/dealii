...
namespace Step50
{
  ...
  template <class IteratorType>
  void
  LaplaceProblem<dim>::assemble_cell(const IteratorType &cell, ScratchData<dim> &scratch_data, CopyData &copy_data)
  {
      ...
      for (unsigned int i = 0; i < dofs_per_cell; ++i)
        {
          ...
          if (!cell->is_level_cell())
            copy_data.cell_rhs(i) +=
              (scratch_data.fe_values.shape_value(i, q_point) * 10.0 * scratch_data.fe_values.JxW(q_point));
        }
  }
  ...
  LaplaceProblem<dim>::assemble_system_and_multigrid()
  {
    ...
    auto cell_worker_active = [&](const decltype(mg_dof_handler.begin_active()) &cell,
                                  ...
                                  CopyData &copy_data) { this->assemble_cell(cell, scratch_data, copy_data); };
    ...
    auto copier_mg = [&](const CopyData &c) {
      boundary_constraints[c.level].distribute_local_to_global(c.cell_matrix,
                                                               c.local_dof_indices,
                                                               mg_matrices[c.level]);
    ...
    for (unsigned int level = 0; level < triangulation.n_global_levels(); ++level)
      {
        ...
        deallog << "mg_interface_matrices[" << level << "]: " << mg_interface_matrices[level].frobenius_norm()
                << std::endl;
      }
  }

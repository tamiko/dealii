...
test()
{
  ...
  if (myid == 0)
    deallog << "#vertices = " << tr.n_vertices() << std::endl
            ...
            << std::count(locally_owned_vertices.begin(), locally_owned_vertices.end(), true) << std::endl;
  ...
  Assert(Utilities::MPI::sum(n_vertices_moved, MPI_COMM_WORLD) == (dim == 2 ? 25 : 125), ExcInternalError());

...
SeventhProblem<dim>::setup_second_system()
{
  ...
  second_locally_relevant_solution.reinit(second_locally_owned_dofs, second_locally_relevant_dofs, mpi_communicator);
...
seventh_grid()
{
  ConditionalOStream pcout(deallog.get_file_stream(), (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0));

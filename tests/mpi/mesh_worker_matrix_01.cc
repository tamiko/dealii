...
test_simple(DoFHandler<dim> &dofs, bool faces)
{
  ...
  FilteredIterator<typename DoFHandler<dim>::active_cell_iterator> cell(IteratorFilters::LocallyOwnedCell(),
                                                                        dofs.begin_active());
  FilteredIterator<typename DoFHandler<dim>::active_cell_iterator> end(IteratorFilters::LocallyOwnedCell(), dofs.end());
  ...
  if (numprocs == 1) // renumber DoFs
    {
      ...
      for (typename DoFHandler<dim>::active_cell_iterator cell = dofs.begin_active(); cell != dofs.end(); ++cell)
        {
          ...
          if (myid == i)
            {
              ...
              for (typename DoFHandler<dim>::active_cell_iterator cell = dofs.begin_active(); cell != dofs.end();
                   ++cell)
                {
                  if (!cell->is_locally_owned())
                    continue;

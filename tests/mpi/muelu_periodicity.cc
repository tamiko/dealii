...
namespace Step22
{
  ...
  StokesProblem<dim>::setup_dofs()
  {
...

    {
      ...
      GridTools::collect_periodic_faces(dof_handler, 2, 3, 1, periodicity_vector, Tensor<1, dim>(), matrix);
              ...
              for (unsigned int i = 0; i < dofs_per_cell; ++i)
                {
                  for (unsigned int j = 0; j <= i; ++j)
                    {
                      local_matrix(i, j) += (symgrad_phi_u[i] * symgrad_phi_u[j] - div_phi_u[i] * phi_p[j] -
  ...
  StokesProblem<dim>::solve()
  {
...

    {
      ...
      SchurComplement<TrilinosWrappers::PreconditionJacobi> schur_complement(
        system_matrix, A_inverse, owned_partitioning[0], relevant_partitioning[0], mpi_communicator);
      ...
      InverseMatrix<TrilinosWrappers::SparseMatrix, TrilinosWrappers::PreconditionAMGMueLu> m_inverse(
  ...
  StokesProblem<dim>::run()
  {
    ...
    GridTools::collect_periodic_faces(triangulation, 2, 3, 1, periodicity_vector, Tensor<1, dim>(), matrix);

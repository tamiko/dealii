...
test()
{
  ...
  VectorTools::compute_normal_flux_constraints(dofh, 0, no_normal_flux_boundaries, constraints, MappingQ<dim>(degree));
  constraints.close();
  ...
  MPI_Gather(&n_constraints, 1, MPI_UNSIGNED, &n_constraints_glob[0], 1, MPI_UNSIGNED, 0, MPI_COMM_WORLD);
  if (myid == 0)

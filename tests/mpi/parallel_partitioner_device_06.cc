...
test()
{
  ...
  Kokkos::View<unsigned *, MemorySpace::Default::kokkos_space> locally_owned_data("locally_owned_data", local_size);
  ArrayView<unsigned int, MemorySpace::Default> locally_owned_data_view(locally_owned_data.data(), local_size);
  Kokkos::deep_copy(locally_owned_data,
                    Kokkos::View<unsigned *, Kokkos::HostSpace>(cpu_locally_owned_data.data(),
                                                                cpu_locally_owned_data.size()));

  // set up a ghost array
  Kokkos::View<unsigned *, MemorySpace::Default::kokkos_space> ghosts("ghosts", v.n_ghost_indices());
  ...
  v.export_to_ghosted_array_finish<unsigned int, MemorySpace::Default>(ghosts_view, requests);
  deallog << "All ghosts: ";
  ...
  Kokkos::View<unsigned *, MemorySpace::Default::kokkos_space> temp_array2("temp_array2", x.n_import_indices());
  ArrayView<unsigned int, MemorySpace::Default>                temp_array2_view(temp_array2.data(), temp_array2.size());

  Kokkos::View<unsigned *, MemorySpace::Default::kokkos_space> ghosts2("ghosts2", x.n_ghost_indices());
  ArrayView<unsigned int, MemorySpace::Default>                ghosts2_view(ghosts2.data(), ghosts2.size());
  ...
  w.export_to_ghosted_array_finish<unsigned int, MemorySpace::Default>(ghosts_view, requests);
  ...
  x.export_to_ghosted_array_finish<unsigned int, MemorySpace::Default>(ghosts_view, requests);
  deallog << "Ghosts on reduced 2: ";
  print_device_view(ghosts);

  x.export_to_ghosted_array_finish<unsigned int, MemorySpace::Default>(ghosts2_view, requests2);

...
periodicity_tests<dim>::check_periodicity()
{
  ...
  using cell_face_map_it = typename std::map<cell_face_pair, cell_face_pair>::iterator;
                  ...
                  if (cell_it->has_periodic_neighbor(i_face))
                    {
                      ...
                      if (cell_it->periodic_neighbor_is_coarser(i_face))
                        {
                          ...
                          Assert(nb_it->periodic_neighbor_child_on_subface(face_subface.first, face_subface.second) ==
                                   cell_it,
                                 ExcInternalError());
                        }
                      ...
                      deallog << "If the periodic neighbor is not coarser, "
                              << nb_of_nb_it->face(nb_of_nb_i_face)->center() << " should be equal to "
                              << cell_it->face(i_face)->center() << std::endl;
                      deallog << "-------------------------------------------------------" << std::endl;
                      Assert((cell_it->face(i_face)->center() - nb_of_nb_it->face(nb_of_nb_i_face)->center()).norm() <
                                 1e-10 ||
                               cell_it->periodic_neighbor_is_coarser(i_face),
                             ExcInternalError());

                      Assert(cell_it == nb_of_nb_it || cell_it->periodic_neighbor_is_coarser(i_face),
                             ExcInternalError());
                    }
                }
            }

...
namespace Step22
{
  ...
  private:
    ...
    const SmartPointer<const InverseMatrix<TrilinosWrappers::SparseMatrix, Preconditioner>> A_inverse;
  ...
  StokesProblem<dim>::setup_dofs()
  {
...

    {
      ...
      GridTools::collect_periodic_faces(dof_handler, 4, 0, 1, periodicity_vector, offset, rot_matrix);
      ...
      VectorTools::interpolate_boundary_values(
        dof_handler, 1, Functions::ZeroFunction<dim>(dim + 1), constraints, fe.component_mask(velocities));

      VectorTools::interpolate_boundary_values(
        dof_handler, 2, Functions::ZeroFunction<dim>(dim + 1), constraints, fe.component_mask(velocities));

      VectorTools::interpolate_boundary_values(
        dof_handler, 3, Functions::ZeroFunction<dim>(dim + 1), constraints, fe.component_mask(velocities));

      VectorTools::interpolate_boundary_values(
        dof_handler, 5, Functions::ZeroFunction<dim>(dim + 1), constraints, fe.component_mask(velocities));
    }
    constraints.close();
  ...
  StokesProblem<dim>::solve()
  {
    ...
    const InverseMatrix<TrilinosWrappers::SparseMatrix, TrilinosWrappers::PreconditionJacobi> A_inverse(
...

    {
      ...
      SchurComplement<TrilinosWrappers::PreconditionJacobi> schur_complement(system_matrix,
                                                                             A_inverse,
                                                                             owned_partitioning[0],
                                                                             mpi_communicator);

      SolverControl solver_control(solution.block(1).size(), 1e-6 * schur_rhs.l2_norm(), false, false);
      SolverCG<TrilinosWrappers::MPI::Vector> cg(solver_control);
  ...
  template <int dim>
  void
  StokesProblem<dim>::get_point_value(const Point<dim> point, const int proc, Vector<double> &value) const
  {
    ...
    if (cell.state() == IteratorState::valid)
      ...
      pcout << "Point: " << point << " is not inside a non-artificial cell!" << std::endl;
                ...
                if (face->boundary_id() == 0)
                  {
                    ...
                    const std::vector<Point<dim>> tmp_points = fe_face_values.get_quadrature_points();
    ...
    if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
      {
        ...
        for (unsigned int i = 0; i < Utilities::MPI::n_mpi_processes(MPI_COMM_WORLD); ++i)
          filenames.push_back(std::string("solution-") + Utilities::int_to_string(refinement_cycle, 2) + "." +
                              Utilities::int_to_string(i, 2) + ".vtu");
        const std::string pvtu_filename = ("solution-" + Utilities::int_to_string(refinement_cycle, 2) + ".pvtu");
  ...
  StokesProblem<dim>::run()
  {
    ...
    GridTools::collect_periodic_faces(triangulation, 4, 0, 1, periodicity_vector, offset, rot_matrix);

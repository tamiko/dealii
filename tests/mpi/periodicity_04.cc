...
check(const unsigned int orientation, bool reverse)
{
  ...
  DoFTools::map_dofs_to_support_points(MappingQ<dim>(1), dof_handler, support_points);
  ...
  if (myid == 0)
    deallog << "n_constraints: " << n_constraints << " n_expected_constraints: " << n_expected_constraints << std::endl;
  AssertThrow(n_constraints == n_expected_constraints, ExcDimensionMismatch(n_constraints, n_expected_constraints));
  ...
  using CellFace = std::pair<typename Triangulation<dim>::cell_iterator, unsigned int>;
  ...
  for (it = face_map.begin(); it != face_map.end(); ++it)
    {
      ...
      Assert(std::min(std::abs(face_center_1(dim - 1) - 3.), std::abs(face_center_1(dim - 1) + 3.)) < 1.e-8,
             ExcInternalError());
      Assert(std::min(std::abs(face_center_2(dim - 1) - 3.), std::abs(face_center_2(dim - 1) + 3.)) < 1.e-8,
             ExcInternalError());
          ...
          if (std::abs(face_center_1(c) - face_center_2(c)) > 1.e-8)
            {
              ...
              for (it = triangulation.get_periodic_face_map().begin();
                   ...
                   ++it)
                {
                  std::cout << "The cell with center " << it->first.first->center() << " has on face "
                            << it->first.second << " as neighbor the cell with center "
                            << it->second.first.first->center() << " on face " << it->second.first.second << std::endl;
                }

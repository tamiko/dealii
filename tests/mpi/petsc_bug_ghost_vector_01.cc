...
test()
{
...

  {
    ...
    const double length = solid_cells_x * cell_size_x, height = static_cast<double>(h_cells * cell_size_y),
    ...
    GridGenerator::subdivided_hyper_rectangle(
      tmp_rectangle, step_sizes, Point<2>(h_distance, 0), Point<2>(length + h_distance, v_distance), false);
    ...
    GridGenerator::subdivided_hyper_rectangle(
      tmp_rectangle, step_sizes, Point<2>(h_distance, solid_top), Point<2>(h_distance + length, height), false);
    ...
    for (unsigned int i = 0; i < h_cells; ++i)
      step_sizes[1].push_back(cell_size_y);
    GridGenerator::subdivided_hyper_rectangle(
      tmp_rectangle, step_sizes, Point<2>(h_distance + length, 0), Point<2>(total_length, height), false);

...
test()
{
  ...
  vector.reinit(dofh.locally_owned_dofs(), MPI_COMM_WORLD);
  {
    ...
    FEValues<dim>      fe_values(fe, quadrature_formula, update_quadrature_points | update_JxW_values | update_values);
          ...
          for (unsigned int q = 0; q < n_q_points; ++q)
            {
              for (unsigned int i = 0; i < dofs_per_cell; ++i)
                {
                  local_vector(i) += fe_values.shape_value(i, q) * 1.0 * local_dof_indices[i] *

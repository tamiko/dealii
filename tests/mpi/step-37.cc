...
namespace Step37
{
  ...
  private:
    ...
    using SystemMatrixType = MatrixFreeOperators::LaplaceOperator<dim,
    ...
    using LevelMatrixType = MatrixFreeOperators::LaplaceOperator<dim,
                                                                 degree_finite_element,
                                                                 degree_finite_element + 1,
                                                                 1,
  ...
  LaplaceProblem<dim>::setup_system()
  {
    ...
    mapping = std::make_shared<MappingFEField<dim, dim, LinearAlgebra::distributed::Vector<double>>>(dof_euler,
                                                                                                     euler_positions);
...

    {
      ...
      system_mf_storage->reinit(*mapping.get(), dof_handler, constraints, QGauss<1>(fe.degree + 1), additional_data);
    ...
    if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
      {
        ...
        for (unsigned int i = 0; i < Utilities::MPI::n_mpi_processes(MPI_COMM_WORLD); ++i)
          filenames.emplace_back("solution-" + std::to_string(cycle) + "." + std::to_string(i) + ".vtu");

        std::string   pvtu_filename = "solution-" + Utilities::to_string(cycle) + ".pvtu";
    ...
    if (dim == 2 && Utilities::MPI::n_mpi_processes(MPI_COMM_WORLD) == 1)
      {
        ...
        const std::string base_filename =
          "grid" + dealii::Utilities::int_to_string(dim) + "_" + dealii::Utilities::int_to_string(cycle);

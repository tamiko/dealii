...
namespace Step39
{
  ...
  Estimator<dim>::boundary(MeshWorker::DoFInfo<dim> &dinfo, typename MeshWorker::IntegrationInfo<dim> &info) const
  {
    ...
    for (unsigned k = 0; k < fe.n_quadrature_points; ++k)
      dinfo.value(0) += penalty * (boundary_values[k] - uh[k]) * (boundary_values[k] - uh[k]) * fe.JxW(k);
    dinfo.value(0) = std::sqrt(dinfo.value(0));
  }
  ...
  template <int dim>
  void
  ErrorIntegrator<dim>::cell(MeshWorker::DoFInfo<dim> &dinfo, typename MeshWorker::IntegrationInfo<dim> &info) const
  {
    const FEValuesBase<dim>    &fe = info.fe_values();
  ...
  template <int dim>
  void
  ErrorIntegrator<dim>::boundary(MeshWorker::DoFInfo<dim> &dinfo, typename MeshWorker::IntegrationInfo<dim> &info) const
  {
    const FEValuesBase<dim> &fe = info.fe_values();
  ...
  InteriorPenaltyProblem<dim>::assemble_matrix()
  {
    ...
    FilteredIterator<typename DoFHandler<dim>::active_cell_iterator> begin(IteratorFilters::LocallyOwnedCell(),
                                                                           dof_handler.begin_active());
    FilteredIterator<typename DoFHandler<dim>::active_cell_iterator> end(IteratorFilters::LocallyOwnedCell(),
                                                                         dof_handler.end());
  ...
  InteriorPenaltyProblem<dim>::solve()
  {
    ...
    using Smoother = RelaxationBlockJacobi<TrilinosWrappers::SparseMatrix, double, TrilinosWrappers::MPI::Vector>;
    ...
    Multigrid<TrilinosWrappers::MPI::Vector> mg(mgmatrix, coarse_grid_solver, mg_transfer, mg_smoother, mg_smoother);
  ...
  InteriorPenaltyProblem<dim>::estimate()
  {
    ...
    MeshWorker::integration_loop<dim, dim>(
      dof_handler.begin_active(), dof_handler.end(), dof_info, info_box, integrator, assembler, lctrl);

...
test()
{
  ...
  {
    const std::string prefix = Utilities::int_to_string(dim, 1) + "d-" + Utilities::int_to_string(spacedim, 1) + "d";
  ...
  for (int n_loop = 0;
       ...
       ++n_loop)
    {
      ...
      deallog << "n_loop: " << n_loop << ", n_cell_gap: " << count_cell_via_signal.n_active_cell_gap() << std::endl;
    }

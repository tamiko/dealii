...
namespace Step27
{
    ...
    , pcout(std::cout, (Utilities::MPI::this_mpi_process(mpi_communicator) == 0))
  {
    ...
    const std::vector<unsigned int> n_coefficients_per_direction(fe_collection.size(), max_degree);
    fourier =
      std::make_unique<FESeries::Fourier<dim>>(n_coefficients_per_direction, fe_collection, fourier_q_collection);
  }
  ...
  LaplaceProblem<dim>::setup_system()
  {
    ...
    std::vector<IndexSet> locally_owned_dofs_per_processor =
      Utilities::MPI::all_gather(dof_handler.get_communicator(), dof_handler.locally_owned_dofs());
  ...
  LaplaceProblem<dim>::solve()
  {
    ...
    check_solver_within_range(cg.solve(system_matrix, completely_distributed_solution, system_rhs, preconditioner),
  ...
  LaplaceProblem<dim>::postprocess()
  {
    ...
    hp::Refinement::p_adaptivity_from_relative_threshold(dof_handler, smoothness_indicators, 0.5, 0.);
  ...
  LaplaceProblem<2>::create_coarse_grid()
  {
    ...
    const std::vector<Point<2>> vertices = {{-1.0, -1.0}, {-0.5, -1.0}, {+0.0, -1.0}, {+0.5, -1.0}, {+1.0, -1.0}, //
                                            {-1.0, -0.5}, {-0.5, -0.5}, {+0.0, -0.5}, {+0.5, -0.5}, {+1.0, -0.5}, //
                                            {-1.0, +0.0}, {-0.5, +0.0}, {+0.5, +0.0}, {+1.0, +0.0},               //
                                            {-1.0, +0.5}, {-0.5, +0.5}, {+0.0, +0.5}, {+0.5, +0.5}, {+1.0, +0.5}, //
                                            {-1.0, +1.0}, {-0.5, +1.0}, {+0.0, +1.0}, {+0.5, +1.0}, {+1.0, +1.0}};

    const std::vector<std::array<int, GeometryInfo<dim>::vertices_per_cell>> cell_vertices = {{{0, 1, 5, 6}},
                                                                                              {{1, 2, 6, 7}},
                                                                                              {{2, 3, 7, 8}},
                                                                                              {{3, 4, 8, 9}},
                                                                                              {{5, 6, 10, 11}},
    ...
    for (unsigned int cycle = 0; cycle < 5; ++cycle)
      {
        ...
        pcout << "   Number of active cells      : " << triangulation.n_global_active_cells() << std::endl
              ...
              << Utilities::MPI::sum(constraints.n_constraints(), mpi_communicator) << std::endl;

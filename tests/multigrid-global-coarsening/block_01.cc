...
test(const unsigned int n_refinements, const unsigned int fe_degree)
{
  ...
  MGLevelObject<std::shared_ptr<const Utilities::MPI::Partitioner>> partitioners(min_level, max_level);
  MGLevelObject<std::shared_ptr<const Utilities::MPI::Partitioner>> partitioners_system(min_level, max_level);
  MGLevelObject<MGTwoLevelTransfer<dim, VectorType>>                transfers(min_level, max_level);
  MGLevelObject<MGTwoLevelTransfer<dim, VectorType>>                transfers_system(min_level, max_level);
  ...
  for (auto l = min_level; l <= max_level; ++l)
    {
      ...
      partitioners[l] =
        std::make_shared<const Utilities::MPI::Partitioner>(dof_handlers[l].locally_owned_dofs(),
                                                            DoFTools::extract_locally_relevant_dofs(dof_handlers[l]),
                                                            MPI_COMM_WORLD);


      dof_handlers_system[l].reinit(*trias[l]);
      dof_handlers_system[l].distribute_dofs(FESystem<dim>(FE_Q<dim>(fe_degree), dim));
      DoFRenumbering::block_wise(dof_handlers_system[l]);
      partitioners_system[l] =
        std::make_shared<const Utilities::MPI::Partitioner>(dof_handlers_system[l].locally_owned_dofs(),
  ...
  for (unsigned int l = min_level; l < max_level; ++l)
    {
      ...
      transfers_system[l + 1].reinit(dof_handlers_system[l + 1], dof_handlers_system[l]);
    }



  MGTransferGlobalCoarsening<dim, VectorType> transfer(transfers,
                                                       [&](const auto l, auto &vec) { vec.reinit(partitioners[l]); });

  MGTransferGlobalCoarsening<dim, VectorType> transfer_system(transfers_system, [&](const auto l, auto &vec) {
    vec.reinit(partitioners_system[l]);
  });
  ...
  MGLevelObject<LinearAlgebra::distributed::BlockVector<Number>> vec_levels_block(min_level, max_level);

  LinearAlgebra::distributed::Vector<Number>                vec_system(partitioners_system.back());
  MGLevelObject<LinearAlgebra::distributed::Vector<Number>> vec_levels_system(min_level, max_level);
  ...
  transfer_system.copy_to_mg(dof_handlers_system.back(), vec_levels_system, vec_system);
  ...
  transfer_system.copy_from_mg(dof_handlers_system.back(), vec_system, vec_levels_system);
  ...
  for (unsigned int l = max_level; l > min_level; --l)
    {
      transfer_block.restrict_and_add(l, vec_levels_block[l - 1], vec_levels_block[l]);
      transfer_system.restrict_and_add(l, vec_levels_system[l - 1], vec_levels_system[l]);
  ...
  for (unsigned int l = min_level; l < max_level; ++l)
    {
      transfer_block.prolongate(l + 1, vec_levels_block[l + 1], vec_levels_block[l]);
      transfer_system.prolongate(l + 1, vec_levels_system[l + 1], vec_levels_system[l]);

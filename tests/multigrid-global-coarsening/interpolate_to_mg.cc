...
test(const unsigned int n_glob_ref = 2, const unsigned int n_ref = 0)
{
  ...
  VectorTools::project(dof_handler, constraints, QGauss<dim>(n_q_points + 2), function, fine_projection);
  ...
  for (unsigned int level = min_level; level <= max_level; ++level)
    {
      ...
      level_projection[level].reinit(dof_handler.locally_owned_mg_dofs(level), set, mpi_communicator);
    }
                ...
                if (std::abs(diff) > 1e-10)
                  {
                    ...
                    AssertThrow(false,
                                ExcMessage("Level " + std::to_string(level) + " Diff " + std::to_string(diff) +
                                           " center_x " + std::to_string(cell->center()[0]) + " center_y " +

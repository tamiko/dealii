...
void
test(const unsigned int n_refinements, const unsigned int fe_degree_fine, const double factor)
{
  ...
  for (auto l = min_level; l <= max_level; ++l)
    {
      ...
      std::unique_ptr<FiniteElement<dim>> fe       = std::make_unique<FE_Q<dim>>(fe_degree_fine);
      std::unique_ptr<Quadrature<dim>>    quad     = std::make_unique<QGauss<dim>>(fe_degree_fine + 1);
      std::unique_ptr<Mapping<dim>>       _mapping = std::make_unique<MappingFE<dim>>(FE_Q<dim>(1));


      // set up triangulation
      GridGenerator::subdivided_hyper_cube(tria, 2);
      tria.refine_global(l);
      GridTools::distort_random(factor, tria, true, boost::random::mt19937::default_seed);
      ...
      VectorTools::interpolate_boundary_values(*_mapping, dof_handler, 0, Functions::ZeroFunction<dim>(), constraint);
      constraint.close();
  ...
  deallog << dim << ' ' << fe_degree_fine << ' ' << n_refinements << ' ' << "quad" << ' ' << solver_control.last_step()
          << std::endl;
}

...
private:
  MatrixFreeOperators::LaplaceOperator<dim, fe_degree_1, n_q_points_1d, 1, typename BlockVectorType::BlockType>
    laplace1;
  MatrixFreeOperators::LaplaceOperator<dim, fe_degree_2, n_q_points_1d, 1, typename BlockVectorType::BlockType>
    laplace2;
};
...
do_test(const std::vector<const DoFHandler<dim> *> &dof)
{
  ...
  BlockLaplace<dim, fe_degree_1, fe_degree_2, n_q_points_1d, LinearAlgebra::distributed::BlockVector<number>>
    fine_matrix;
  ...
  for (unsigned int level = 0; level < dof[0]->get_triangulation().n_global_levels(); ++level)
    {
      ...
      std::vector<const AffineConstraints<double> *> level_constraints_ptrs(dof.size());
      for (unsigned int i = 0; i < dof.size(); ++i)
        {
  ...
  PreconditionMG<dim, LinearAlgebra::distributed::BlockVector<number>, MGTransferBlockMF<dim, number>> preconditioner(
    dof, mg, mg_transfer);

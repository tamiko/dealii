...
namespace Step50
{
  ...
  LaplaceProblem<dim>::assemble_system()
  {
    ...
    typename DoFHandler<dim>::active_cell_iterator cell = mg_dof_handler.begin_active(), endc = mg_dof_handler.end();
    for (; cell != endc; ++cell)
  ...
  LaplaceProblem<dim>::assemble_multigrid()
  {
    ...
    typename DoFHandler<dim>::cell_iterator cell = mg_dof_handler.begin(), endc = mg_dof_handler.end();
      ...
      if (cell->level_subdomain_id() == triangulation.locally_owned_subdomain())
        {
          ...
          const IndexSet    &interface_dofs_on_level = mg_constrained_dofs.get_refinement_edge_indices(cell->level());
  ...
  LaplaceProblem<dim>::vcycle(const MGCoarseGridBase<vector_t> &coarse_grid_solver)
  {
    ...
    auto print_coarse_solve = [](const bool start, const unsigned int level) {
      deallog << "coarse_solve " << (start ? "started" : "finished") << " on level " << level << std::endl;
    };

    auto print_restriction = [](const bool start, const unsigned int level) {
      deallog << "restriction " << (start ? "started" : "finished") << " on level " << level << std::endl;
    };

    auto print_prolongation = [](const bool start, const unsigned int level) {
      deallog << "prolongation " << (start ? "started" : "finished") << " on level " << level << std::endl;
    };

    auto print_pre_smoother_step = [](const bool start, const unsigned int level) {
      deallog << "pre_smoother_step " << (start ? "started" : "finished") << " on level " << level << std::endl;
    };

    auto print_post_smoother_step = [](const bool start, const unsigned int level) {
      deallog << "post_smoother_step " << (start ? "started" : "finished") << " on level " << level << std::endl;
    };

    mg.set_edge_matrices(mg_interface_down, mg_interface_up);

    PreconditionMG<dim, vector_t, MGTransferPrebuilt<vector_t>> preconditioner(mg_dof_handler, mg, mg_transfer);
  ...
  LaplaceProblem<dim>::solve()
  {
    ...
    MGCoarseGridIterativeSolver<vector_t, SolverCG<vector_t>, matrix_t, PreconditionIdentity> coarse_grid_solver(
    ...
    for (unsigned int cycle = 0; cycle < 3; ++cycle)
      {
        ...
        for (unsigned int level = 0; level < triangulation.n_global_levels(); ++level)
          deallog << mg_dof_handler.n_dofs(level) << (level == triangulation.n_global_levels() - 1 ? ")" : ", ");
        deallog << std::endl;

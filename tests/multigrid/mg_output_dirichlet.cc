...
initialize(const DoFHandler<dim> &dof, MGLevelObject<Vector<double>> &u)
{
  ...
  for (unsigned int l = 0; l < dof.get_triangulation().n_levels(); ++l)
    {
      for (typename DoFHandler<dim>::cell_iterator cell = dof.begin_mg(l); cell != dof.end_mg(l); ++cell)
        {
...
check_simple(const FiniteElement<dim> &fe)
{
  ...
  mg_constrained_dofs_renumbered.make_zero_boundary_constraints(mgdof_renumbered, {0});

  ...
  for (unsigned int level = 0; level < n_levels; ++level)
    {
      ...
      MGTools::make_sparsity_pattern(mg_dof_handler, csp, level, boundary_constraints[level], false);
      mg_sparsity_patterns[level].copy_from(csp);

      csp.reinit(mg_dof_handler.n_dofs(level), mg_dof_handler.n_dofs(level));
      MGTools::make_sparsity_pattern(mg_dof_handler, csp, level, empty_constraints, true);

...
namespace Step39
{
  ...
  InteriorPenaltyProblem<dim>::assemble_mg_matrix()
  {
    ...
    for (unsigned int level = mg_matrix_in_out.min_level(); level <= mg_matrix_in_out.min_level(); ++level)
    ...
    for (unsigned int s = 0; s < n_steps; ++s)
      {
        ...
        if (estimates.block(0).size() == 0)
          ...
          {
            GridRefinement::refine_and_coarsen_fixed_fraction(triangulation, estimates.block(0), 0.5, 0.0);
            triangulation.execute_coarsening_and_refinement();
          }

        deallog << "Triangulation " << triangulation.n_active_cells() << " cells, " << triangulation.n_levels()
                << " levels" << std::endl;

      ...
      for (unsigned int level = 1; level < mgdof.get_triangulation().n_global_levels(); ++level)
        {
          ...
          DoFTools::extract_locally_relevant_level_dofs(mgdof, level, relevant_set);
          v2.reinit(mgdof.locally_owned_mg_dofs(level), relevant_set, MPI_COMM_WORLD);
          ...
          v2_cpy = v2;
          v3 -= v2_cpy;
          deallog << "Diff prolongate   l" << level << ": " << filter_out_small_numbers(v3.l2_norm(), tolerance)
                  << std::endl;
          deallog << "Vectors have ghosts: " << v2.has_ghost_elements() << ' ' << v3.has_ghost_elements() << std::endl;
        }
      ...
      for (unsigned int level = 1; level < mgdof.get_triangulation().n_global_levels(); ++level)
        {
          ...
          v2_cpy = v2;
          v3 -= v2_cpy;
          deallog << "Diff restrict     l" << level << ": " << filter_out_small_numbers(v3.l2_norm(), tolerance)
                  << std::endl;
          ...
          v2_cpy = v2;
          v3 -= v2_cpy;
          deallog << "Diff restrict add l" << level << ": " << filter_out_small_numbers(v3.l2_norm(), tolerance)
                  << std::endl;
        }
      deallog << std::endl;
    }

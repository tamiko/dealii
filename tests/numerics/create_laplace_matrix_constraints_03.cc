...
check()
{
  ...
  VectorTools::interpolate_boundary_values(dof, 0, Functions::ConstantFunction<dim>(1, 1.), constraints);
  constraints.close();
  ...
  MatrixTools::create_laplace_matrix(mapping, dof, quadrature, matrix_ref, rhs_function, rhs_ref, &rhs_function);
  constraints.condense(matrix_ref, rhs_ref);

  MatrixTools::create_laplace_matrix(mapping, dof, quadrature, matrix, rhs_function, rhs, &rhs_function, constraints);

...
test()
{
  ...
  static const Point<3> vertices_1[] = {
    ...
    // first extension
    // points 15-18
    Point<dim>(0, 0, 7) + Point<dim>(std::cos(2 * numbers::PI / 6), std::sin(2 * numbers::PI / 6), 0) * 4.0,
    ...
    // second extension
    // points 19-22
    Point<dim>(0, 0, 7) + Point<dim>(std::cos(6 * numbers::PI / 6), std::sin(6 * numbers::PI / 6), 0) * 4.0,
    Point<dim>(std::cos(4 * numbers::PI / 6), std::sin(4 * numbers::PI / 6), 7) +
    ...
    // third extension
    // points 23-26
    Point<dim>(0, 0, 7) + Point<dim>(std::cos(10 * numbers::PI / 6), std::sin(10 * numbers::PI / 6), 0) * 4.0,
    Point<dim>(std::cos(8 * numbers::PI / 6), std::sin(8 * numbers::PI / 6), 7) +
  ...
  for (Triangulation<dim>::active_cell_iterator cell = triangulation.begin_active(); cell != triangulation.end();
       ++cell)
    for (const unsigned int f : GeometryInfo<dim>::face_indices())
      if ((cell->face(f)->center()[2] != -4) && (cell->face(f)->center()[2] != 7) && (cell->face(f)->at_boundary()))

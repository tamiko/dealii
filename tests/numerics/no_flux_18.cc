...
namespace StokesClass
{
  ...
  namespace StokesSolver
  {
    ...
    template <class StokesMatrixType, class MassMatrixType, class PreconditionerA, class PreconditionerMp>
    unsigned int
    BlockSchurPreconditioner<StokesMatrixType, MassMatrixType, PreconditionerA, PreconditionerMp>::n_iterations_A()
      const
    {
    ...
    template <class StokesMatrixType, class MassMatrixType, class PreconditionerA, class PreconditionerMp>
    unsigned int
    BlockSchurPreconditioner<StokesMatrixType, MassMatrixType, PreconditionerA, PreconditionerMp>::n_iterations_S()
      const
    {
    ...
    template <class StokesMatrixType, class MassMatrixType, class PreconditionerA, class PreconditionerMp>
    void
    BlockSchurPreconditioner<StokesMatrixType, MassMatrixType, PreconditionerA, PreconditionerMp>::vmult(
        ...
        // exception on all other processors
        catch (const std::exception &exc)
          {
            if (Utilities::MPI::this_mpi_process(src.block(0).get_mpi_communicator()) == 0)
              AssertThrow(false,
                          ExcMessage(
                            std::string("The iterative (bottom right) solver in BlockSchurPreconditioner::vmult "
  ...
  template <int dim>
  void
  ExactSolution_BoundaryValues_u<dim>::vector_value(const Point<dim> &p, Vector<double> &values) const
  {
    (void)p;
    for (unsigned int i = 0; i < values.size(); ++i)
      values(i) = 0.0;
  ...
  class StokesOperator : public MatrixFreeOperators::Base<dim, LinearAlgebra::distributed::BlockVector<number>>
  {
  public:
    StokesOperator()
  ...
  template <int dim, int degree_v, typename number>
  void
  StokesOperator<dim, degree_v, number>::evaluate_2_x_viscosity(const Viscosity<dim> &viscosity_function)
  {
  ...
  template <int dim, int degree_v, typename number>
  void
  StokesOperator<dim, degree_v, number>::local_apply(const dealii::MatrixFree<dim, number>                 &data,
                                                     LinearAlgebra::distributed::BlockVector<number>       &dst,
                                                     const LinearAlgebra::distributed::BlockVector<number> &src,
                                                     const std::pair<unsigned int, unsigned int> &cell_range) const
  {
  ...
  template <int dim, int degree_v, typename number>
  void
  StokesOperator<dim, degree_v, number>::apply_add(LinearAlgebra::distributed::BlockVector<number>       &dst,
                                                   const LinearAlgebra::distributed::BlockVector<number> &src) const
  {
    MatrixFreeOperators::Base<dim, LinearAlgebra::distributed::BlockVector<number>>::data->cell_loop(
      &StokesOperator::local_apply, this, dst, src);
  }
  template <int dim, int degree_v, typename number>
  void
  ...
  class MassMatrixOperator : public MatrixFreeOperators::Base<dim, LinearAlgebra::distributed::Vector<number>>
  {
  public:
    MassMatrixOperator()
  ...
  template <int dim, int degree_p, typename number>
  void
  MassMatrixOperator<dim, degree_p, number>::evaluate_1_over_viscosity(const Viscosity<dim> &viscosity_function)
  {
    const unsigned int                                   n_cells = this->data->n_cell_batches();
  ...
  template <int dim, int degree_p, typename number>
  void
  MassMatrixOperator<dim, degree_p, number>::local_apply(const dealii::MatrixFree<dim, number>            &data,
                                                         LinearAlgebra::distributed::Vector<number>       &dst,
                                                         const LinearAlgebra::distributed::Vector<number> &src,
                                                         const std::pair<unsigned int, unsigned int> &cell_range) const
  {
  ...
  template <int dim, int degree_p, typename number>
  void
  MassMatrixOperator<dim, degree_p, number>::apply_add(LinearAlgebra::distributed::Vector<number>       &dst,
                                                       const LinearAlgebra::distributed::Vector<number> &src) const
  {
  ...
  MassMatrixOperator<dim, degree_p, number>::compute_diagonal()
  {
    ...
    const unsigned int locally_owned_size = inverse_diagonal.locally_owned_size();
  ...
  template <int dim, int degree_v, typename number>
  class ABlockOperator : public MatrixFreeOperators::Base<dim, LinearAlgebra::distributed::Vector<number>>
  {
  public:
    ABlockOperator()
      : MatrixFreeOperators::Base<dim, LinearAlgebra::distributed::Vector<number>>()
  ...
  template <int dim, int degree_v, typename number>
  void
  ABlockOperator<dim, degree_v, number>::evaluate_2_x_viscosity(const Viscosity<dim> &viscosity_function)
  {
    const unsigned int                                     n_cells = this->data->n_cell_batches();
    FEEvaluation<dim, degree_v, degree_v + 1, dim, number> velocity(*this->data, 0);
    viscosity_x_2.reinit(n_cells, velocity.n_q_points);
  ...
  template <int dim, int degree_v, typename number>
  void
  ABlockOperator<dim, degree_v, number>::local_apply(const dealii::MatrixFree<dim, number>            &data,
                                                     LinearAlgebra::distributed::Vector<number>       &dst,
                                                     const LinearAlgebra::distributed::Vector<number> &src,
                                                     const std::pair<unsigned int, unsigned int>      &cell_range) const
  {
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        ...
        for (unsigned int q = 0; q < velocity.n_q_points; ++q)
          {
            velocity.submit_symmetric_gradient(viscosity_x_2(cell, q) * velocity.get_symmetric_gradient(q), q);
          }
        velocity.integrate(EvaluationFlags::gradients);
  ...
  template <int dim, int degree_v, typename number>
  void
  ABlockOperator<dim, degree_v, number>::apply_add(LinearAlgebra::distributed::Vector<number>       &dst,
                                                   const LinearAlgebra::distributed::Vector<number> &src) const
  {
    MatrixFreeOperators::Base<dim, LinearAlgebra::distributed::Vector<number>>::data->cell_loop(
  ...
  StokesProblem<dim>::setup_system()
  {
    ...
    VectorTools::compute_no_normal_flux_constraints(dof_handler_u, 0, dirichlet_boundary, constraints_u);
    constraints_u.close();
    ...
    stokes_mf_storage->reinit(
      MappingQ1<dim>{}, stokes_dofs, stokes_constraints, QGauss<1>(degree_u + 1), additional_data_stokes);
    ...
    additional_data_mass.mapping_update_flags  = (update_values | update_JxW_values | update_quadrature_points);
    std::shared_ptr<MatrixFree<dim, double>> mass_mf_storage(new MatrixFree<dim, double>());
    mass_mf_storage->reinit(
      MappingQ1<dim>{}, dof_handler_p, constraints_p, QGauss<1>(degree_u + 1), additional_data_mass);
    ...
    for (unsigned int level = 0; level < n_levels; ++level)
      {
        ...
        const auto   &refinement_edge_indices = mg_constrained_dofs.get_refinement_edge_indices(level);

        VectorTools::compute_no_normal_flux_constraints_on_level(
          dof_handler_u, 0, dirichlet_boundary, level_constraints, mapping, refinement_edge_indices, level);
        ...
        mg_mf_storage_level->reinit(
          MappingQ1<dim>{}, dof_handler_u, level_constraints, QGauss<1>(degree_u + 1), additional_data);
  ...
  StokesProblem<dim>::assemble_rhs()
  {
    ...
    matrix_free_homogeneous->reinit(MappingQ1<dim>{}, dofs, constraints_no_dirchlet, QGauss<1>(degree_u + 1), data);
    ...
    FEEvaluation<dim, velocity_degree - 1, velocity_degree + 1, 1, double> pressure(*stokes_matrix.get_matrix_free(),
                                                                                    1);

    for (unsigned int cell = 0; cell < stokes_matrix.get_matrix_free()->n_cell_batches(); ++cell)
      {
        velocity.reinit(cell);
        pressure.reinit(cell);
        for (unsigned int q = 0; q < velocity.n_q_points; ++q)
  ...
  StokesProblem<dim>::solve()
  {
    ...
    MGCoarseGridIterativeSolver<vector_t, SolverCG<vector_t>, LevelMatrixType, PreconditionIdentity> mg_coarse;
    ...
    try
      {
        SolverFGMRES<block_vector_t> solver(solver_control_cheap,
                                            mem,
                                            SolverFGMRES<block_vector_t>::AdditionalData(50));

        solver.solve(stokes_matrix, distributed_stokes_solution, distributed_stokes_rhs, preconditioner_cheap);
      }
    ...
    deallog << "Solved-in "
            << (solver_control_cheap.last_step() != numbers::invalid_unsigned_int ? solver_control_cheap.last_step() :
                                                                                    0)
  ...
  StokesProblem<dim>::run()
  {
    ...
    deallog << "n_sinker: " << sinker.n_sinkers << "       max/min viscosity ratio: " << sinker.DR_mu << std::endl
            << std::endl;
    ...
    for (unsigned int cycle = 0; cycle < n_cycles; ++cycle)
      {
        ...
        deallog << "Number of degrees of freedom: " << dof_handler_u.n_dofs() + dof_handler_p.n_dofs() << " ("
                << dof_handler_u.n_dofs() << '+' << dof_handler_p.n_dofs() << ')' << std::endl;

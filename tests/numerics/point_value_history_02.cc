                                        ...
                                        std::vector<Vector<double>>                &computed_quantities) const
{
  ...
  for (unsigned int i = 0; i < computed_quantities.size(); ++i)
    {
      Assert(computed_quantities[i].size() == 4, ExcDimensionMismatch(computed_quantities[i].size(), 3));
      ...
      computed_quantities[i](2) =
        inputs.solution_gradients[i][0].norm() + inputs.solution_hessians[i][1].norm(); // norm of y hessian

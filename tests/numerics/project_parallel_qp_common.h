  ...
  for (unsigned int q = 0; q <= p; ++q)
    {
      ...
      double     field_l2_norm = 0.;
      {
        ...
        field_relevant.reinit(dof_handler.locally_owned_dofs(), locally_relevant_dofs, MPI_COMM_WORLD);
      ...
      deallog << fe.get_name() << ", P_" << q << ", rel. error=" << L2_norm / field_l2_norm << std::endl;

...
test()
{
    ...
    for (auto a1 : values)
      {
        const auto res = LineMinimization::line_search<double>(
          func, fg0.first, fg0.second, LineMinimization::poly_fit<double>, a1, 0.1, 0.001);
...

  {
    ...
    for (auto a1 : values)
      {
        const auto res = LineMinimization::line_search<double>(
          func, fg0.first, fg0.second, LineMinimization::poly_fit<double>, a1, 0.100001, 0.1);
    ...
    for (auto p : params)
      {
        ...
        for (auto a1 : values)
          {
            const auto res = LineMinimization::line_search<double>(
              func, fg0.first, fg0.second, LineMinimization::poly_fit<double>, a1, 0.00100001, 0.001);

...
namespace Step44
{
  ...
  namespace Parameters
  {
    ...
    NonlinearSolver::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Nonlinear solver");
      {
        ...
        prm.declare_entry("Use line search", "false", Patterns::Bool(), "Use line-search minimization algorithm");
        prm.declare_entry("Use line search approximate gradient",
                          "false",
                          Patterns::Bool(),
    ...
    NonlinearSolver::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Nonlinear solver");
      {
        ...
        use_line_search_ptrb = prm.get_bool("Use line search approximate gradient");
      }
      prm.leave_subsection();
    }
    struct Time
  ...
  private:
    ...
    void
    assemble_system_tangent(BlockSparseMatrix<double> &tangent_matrix, const bool print = true);
    void
    ...
    void
    assemble_system_rhs(BlockVector<double> &system_rhs, const bool print = true);
    void
    ...
    setup_qph();
    void
    update_qph_incremental(const BlockVector<double> &solution_delta, const bool print = true);
    void
  ...
  struct Solid<dim>::PerTaskData_K
  {
    ...
    BlockSparseMatrix<double>           *tangent_matrix;
    PerTaskData_K(const unsigned int dofs_per_cell, BlockSparseMatrix<double> &tangent_matrix)
  ...
  struct Solid<dim>::PerTaskData_RHS
  {
    ...
    std::vector<types::global_dof_index> local_dof_indices;
    BlockVector<double>                 *system_rhs;
    PerTaskData_RHS(const unsigned int dofs_per_cell, BlockVector<double> &system_rhs)
  ...
  template <int dim>
  void
  Solid<dim>::update_qph_incremental(const BlockVector<double> &solution_delta, const bool print)
  {
    ...
    for (; newton_iteration < parameters.max_iterations_NR; ++newton_iteration)
      {
        ...
        const double tang_mtrx_norm_old = tangent_matrix.block(0, 0).frobenius_norm();

        // Minimization function (exact)
        auto ls_minimization_function =
          [this, &solution_delta, &newton_update, &tang_mtrx_norm_old](const double ss /*step size*/) {
            ...
            const double tang_mtrx_norm_new = tangent_matrix.block(0, 0).frobenius_norm();

            (void)tang_mtrx_norm_new;
            (void)tang_mtrx_norm_old;
            if (ss != 0.0)
              Assert(tang_mtrx_norm_new != tang_mtrx_norm_old, ExcInternalError());
        ...
        auto ls_minimization_function_ptrb = [&ls_minimization_function](const double ss) {
          ...
          return std::make_pair(f_m.first, (f_plus.first - f_minus.first) / (2. * ptrb));
        };

        // Invoke line search
        const bool use_ptrb           = parameters.use_line_search_ptrb;
        auto       perform_linesearch = [&ls_minimization_function, &ls_minimization_function_ptrb, &use_ptrb]() {
          const auto res_0 = (use_ptrb ? ls_minimization_function_ptrb(0.0) : ls_minimization_function(0.0));
          Assert(res_0.second < 0.0,
                 ExcMessage("Gradient should be negative. Current value: " + std::to_string(res_0.second)));
          const auto res_1 = (use_ptrb ? ls_minimization_function_ptrb(1.0) : ls_minimization_function(1.0));
          ...
          const auto         res = (use_ptrb ? LineMinimization::line_search<double>(ls_minimization_function_ptrb,
                                                                             res_0.first,
                                                                             res_0.second,
                                                                             ...
                                                                             max_evals,
                                                                             debug_linesearch) :
                                                     LineMinimization::line_search<double>(ls_minimization_function,
                                                                             res_0.first,
                                                                             res_0.second,
                                                                             LineMinimization::poly_fit<double>,
                                                                             a1,
        ...
        const double linesearch_step_size = (parameters.use_line_search ? perform_linesearch() : 1.0);
        ...
        std::cout << " | " << std::fixed << std::setprecision(3) << std::setw(7) << std::scientific
                  << lin_solver_output.first << "  " << lin_solver_output.second << "  " << linesearch_step_size << "  "
                  << error_residual_norm.norm << "  " << error_residual_norm.u << "  " << error_residual_norm.p << "  "
                  << error_residual_norm.J << "  " << error_update_norm.norm << "  " << error_update_norm.u << "  "
  ...
  template <int dim>
  void
  Solid<dim>::assemble_system_tangent(BlockSparseMatrix<double> &tangent_matrix, const bool print)
  {
  ...
  template <int dim>
  void
  Solid<dim>::assemble_system_rhs(BlockVector<double> &system_rhs, const bool print)
  {

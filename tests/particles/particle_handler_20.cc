        ...
        if (Utilities::MPI::this_mpi_process(tr.get_communicator()) == 0)
          {
            ...
            Particles::Particle<dim, spacedim> particle(
              position, reference_position, Utilities::MPI::this_mpi_process(tr.get_communicator()) * n_particles + p);
    ...
    for (auto particle = particle_handler.begin(); particle != particle_handler.end(); ++particle)
      {
        particle->get_properties()[0] =
          1000 + 100 * Utilities::MPI::this_mpi_process(tr.get_communicator()) + 10 * particle->get_id();
        particle->get_properties()[1] =
          2000 + 100 * Utilities::MPI::this_mpi_process(tr.get_communicator()) + 10 * particle->get_id();
        counter++;
      }

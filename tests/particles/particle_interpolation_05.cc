...
test()
{
  ...
  // processor
  Particles::ParticleHandler<dim, spacedim> particle_handler(space_tria, mapping);

  FE_Q<dim, spacedim> space_fe(1);

  ComponentMask space_mask(space_fe.n_components(), true);

  Particles::Generators::regular_reference_locations<dim, spacedim>(space_tria,
  ...
  DynamicSparsityPattern dsp(particle_handler.n_global_particles() * n_comps, space_dh.n_dofs());

  AffineConstraints<double> constraints;

  // Build the interpolation sparsity
  Particles::Utilities::create_interpolation_sparsity_pattern(space_dh, particle_handler, dsp, constraints, space_mask);
  ...
  SparsityTools::distribute_sparsity_pattern(dsp, global_particles_index_set, MPI_COMM_WORLD, local_particle_index_set);
  // Create a SparseMatrix
  TrilinosWrappers::SparseMatrix matrix;
  matrix.reinit(local_particle_index_set, locally_owned_dofs, dsp, MPI_COMM_WORLD);

  // Build the interpolation matrix
  Particles::Utilities::create_interpolation_matrix(space_dh, particle_handler, matrix, constraints, space_mask);

  // Create dofs and particle vectors
  TrilinosWrappers::MPI::Vector field(locally_owned_dofs, MPI_COMM_WORLD);
  TrilinosWrappers::MPI::Vector interpolation(local_particle_index_set, MPI_COMM_WORLD);

...
test_standard_tensors()
{
  ...
  AssertThrow(
    std::fabs((StandardTensors<dim>::dev_P * t - (t - (trace(t) / dim) * unit_symmetric_tensor<dim>())).norm()) < 1e-14,
    ExcInternalError());
  AssertThrow(
    std::fabs((t * StandardTensors<dim>::dev_P - (t - (trace(t) / dim) * unit_symmetric_tensor<dim>())).norm()) < 1e-14,
    ExcInternalError());
  ...
  //       det(F)^{-2/dim} F^{T}.F .
  AssertThrow(std::fabs((symmetrize(std::pow(determinant(F), 2.0 / dim) * F * static_cast<Tensor<2, dim>>(s_x_Dev_P) *
                                    transpose(F)) -
                         StandardTensors<dim>::dev_P * t)
                          .norm()) < 1e-14,
              ExcInternalError());
  AssertThrow(std::fabs((symmetrize(std::pow(determinant(F), 2.0 / dim) * F * static_cast<Tensor<2, dim>>(Dev_P_T_x_s) *
                                    transpose(F)) -
                         StandardTensors<dim>::dev_P * t)
                          .norm()) < 1e-14,
              ExcInternalError());
  ...
  const SymmetricTensor<2, dim> tau_bar =
    symmetrize(F_bar * static_cast<Tensor<2, dim>>(S_bar) * transpose(F_bar)); // Note: tau_bar = tau(F) |_{F = F_bar}

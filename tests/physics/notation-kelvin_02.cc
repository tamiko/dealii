...
test_rank_3_tensors()
{
  // Non-symmetric tensor: Version 1
  {
    ...
    initialize(A);
    initialize(C);
    const Tensor<1, dim, double> B = double_contract<0, 0, 1, 1>(C,
  ...
  // Non-symmetric tensor: Version 2
  {
    ...
    const FullMatrix<double> mC =
      Notation::Kelvin::to_matrix<dim, Tensor<1, dim>, Tensor<2, dim>>(C); // Define subtensor representation
                                                                           // Tensor<1,dim> \otimes Tensor<2,dim>
  ...
  // Symmetric tensor: Version 1
  {
    ...
    const Vector<double>     vA = Notation::Kelvin::to_vector(A);
    const FullMatrix<double> mC = Notation::Kelvin::to_matrix<dim, SymmetricTensor<2, dim>, Tensor<1, dim>>(
  ...
  // Symmetric tensor: Version 2
  {
    ...
    const Vector<double>     vA = Notation::Kelvin::to_vector(A);
    const FullMatrix<double> mC = Notation::Kelvin::to_matrix<dim, Tensor<1, dim>, SymmetricTensor<2, dim>>(

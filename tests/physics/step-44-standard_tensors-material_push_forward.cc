...
namespace Step44
{
  ...
  public:
    ...
    {}
    void
    update_material_data(const Tensor<2, dim> &F_in, const double p_tilde_in, const double J_tilde_in)
    {
      F                          = F_in;
      det_F                      = determinant(F);
      ...
      if (std::abs(det_F - 1.0) > 1e-9)
        {
          ...
          Assert((Physics::Transformations::Contravariant::push_forward(get_S_vol(), F) - get_tau_vol()).norm() /
                     get_tau_vol().norm() <
                   tol,
                 ExcInternalError());
          Assert((Physics::Transformations::Contravariant::push_forward(get_S_iso(), F) - get_tau_iso()).norm() /
                     get_tau_iso().norm() <
                   tol,
                 ExcInternalError());

          // Verify the pull-back transformation
          Assert((Physics::Transformations::Contravariant::pull_back(get_tau_vol(), F) - get_S_vol()).norm() /
                     get_S_vol().norm() <
                   tol,
                 ExcInternalError());
          Assert((Physics::Transformations::Contravariant::pull_back(get_tau_iso(), F) - get_S_iso()).norm() /
                     get_S_iso().norm() <
                   tol,
                 ExcInternalError());
        }
      else
        {
          ...
          // Verify the push-forward transformation
          Assert((Physics::Transformations::Contravariant::push_forward(get_S_vol(), F) - get_tau_vol()).norm() < tol,
                 ExcInternalError());
          Assert((Physics::Transformations::Contravariant::push_forward(get_S_iso(), F) - get_tau_iso()).norm() < tol,
                 ExcInternalError());

          // Verify the pull-back transformation
          Assert((Physics::Transformations::Contravariant::pull_back(get_tau_vol(), F) - get_S_vol()).norm() < tol,
                 ExcInternalError());
          Assert((Physics::Transformations::Contravariant::pull_back(get_tau_iso(), F) - get_S_iso()).norm() < tol,
                 ExcInternalError());
        }
      ...
      if (std::abs(det_F - 1.0) > 1e-9)
        {
          ...
          // Verify the push-forward transformation
          Assert((Physics::Transformations::Contravariant::push_forward(get_H_vol(), F) - get_Jc_vol()).norm() /
                     get_Jc_vol().norm() <
                   tol,
                 ExcInternalError());
          Assert((Physics::Transformations::Contravariant::push_forward(get_H_iso(), F) - get_Jc_iso()).norm() /
                     get_Jc_iso().norm() <
                   tol,
                 ExcInternalError());

          // Verify the pull-back transformation
          Assert((Physics::Transformations::Contravariant::pull_back(get_Jc_vol(), F) - get_H_vol()).norm() /
                     get_H_vol().norm() <
                   tol,
                 ExcInternalError());
          Assert((Physics::Transformations::Contravariant::pull_back(get_Jc_iso(), F) - get_H_iso()).norm() /
                     get_H_iso().norm() <
                   tol,
                 ExcInternalError());
        }
      else
        {
          ...
          // Verify the push-forward transformation
          Assert((Physics::Transformations::Contravariant::push_forward(get_H_vol(), F) - get_Jc_vol()).norm() < tol,
                 ExcInternalError());
          Assert((Physics::Transformations::Contravariant::push_forward(get_H_iso(), F) - get_Jc_iso()).norm() < tol,
                 ExcInternalError());

          // Verify the pull-back transformation
          Assert((Physics::Transformations::Contravariant::pull_back(get_Jc_vol(), F) - get_H_vol()).norm() < tol,
                 ExcInternalError());
          Assert((Physics::Transformations::Contravariant::pull_back(get_Jc_iso(), F) - get_H_iso()).norm() < tol,
                 ExcInternalError());
        }
  ...
  protected:
    ...
    get_H_vol() const
    {
      ...
      return p_tilde * det_F * (C_inv_x_C_inv + (2.0 * Physics::Elasticity::StandardTensors<dim>::dC_inv_dC(F)));
    }
    SymmetricTensor<4, dim>
    get_H_iso() const
    {
    ...
    get_Jc_vol() const
    {
      return p_tilde * det_F *
             (Physics::Elasticity::StandardTensors<dim>::IxI - (2.0 * Physics::Elasticity::StandardTensors<dim>::S));
    }
    SymmetricTensor<4, dim>
    get_Jc_iso() const
    {
      ...
      const SymmetricTensor<4, dim> tau_iso_x_I = outer_product(tau_iso, Physics::Elasticity::StandardTensors<dim>::I);
      const SymmetricTensor<4, dim> I_x_tau_iso = outer_product(Physics::Elasticity::StandardTensors<dim>::I, tau_iso);
                                              ...
                                              PerTaskData_UQPH & /*data*/)
  {
    ...
    for (const unsigned int q_point : scratch.fe_values_ref.quadrature_point_indices())
  ...
  {
    pcout << std::endl << "Timestep " << time.get_timestep() << " @ " << time.current() << 's' << std::endl;
  ...
  Solid<dim>::compute_vol_current() const
  {
    ...
    for (typename Triangulation<dim>::active_cell_iterator cell = triangulation.begin_active();
         ...
         ++cell)
      {
        ...
        const std::vector<std::shared_ptr<const PointHistory<dim>>> lqph = quadrature_point_history.get_data(cell);
        Assert(lqph.size() == n_q_points, ExcInternalError());
        for (const unsigned int q_point : fe_values_ref.quadrature_point_indices())
          {
                                               ...
                                               PerTaskData_K                                        &data) const
  {
        ...
        for (const unsigned int i : scratch.fe_values_ref.dof_indices())
          {
            ...
            for (const unsigned int j : scratch.fe_values_ref.dof_indices_ending_at(i))
              {
                ...
                if ((i_group == j_group) && (i_group == u_dof))
                ...
                else if ((i_group == p_dof) && (j_group == u_dof))
                  {
                    data.cell_matrix(i, j) +=
                      N[i] * det_F * (symm_grad_Nx[j] * Physics::Elasticity::StandardTensors<dim>::I) * JxW;
                  }
    ...
    for (const unsigned int i : scratch.fe_values_ref.dof_indices())
      for (const unsigned int j : scratch.fe_values_ref.dof_indices_starting_at(i + 1))
      ...
      if (cell->face(face)->at_boundary() == true && cell->face(face)->boundary_id() == 6)
        {
          ...
          for (const unsigned int f_q_point : scratch.fe_face_values_ref.quadrature_point_indices())
            {
        ...
        if (parameters.type_lin == "CG")
          {
            ...
            ReductionControl solver_control_K_Jp_inv(static_cast<unsigned int>(tangent_matrix.block(J_dof, p_dof).m() *
            ...
            ReductionControl solver_control_K_con_inv(static_cast<unsigned int>(tangent_matrix.block(u_dof, u_dof).m() *
                                                                                parameters.max_iterations_lin),
                                                      1.0e-30,
                                                      parameters.tol_lin);

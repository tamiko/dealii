...
test_symmetric_tensor(const Tensor<2, dim> &F)
{
  // Rank-2 Symmetric tensors
  {
    ...
    const Tensor<2, dim> T_trans = Physics::Transformations::Contravariant::push_forward(T, F);
    Assert((T_calc - T_trans).norm() < 1e-9,
           ExcMessage("Rank 2 symmetric tensor: Contraction using push_forward() function is incorrect."));
  }
  ...
  // Rank-4 Symmetric tensors
  {
    ...
    Assert((T_calc - T_trans).norm() < 1e-9,
           ExcMessage("Rank 4 symmetric tensor: Contraction using push_forward() function is incorrect."));
  }
}

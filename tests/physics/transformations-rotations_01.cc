...
test_rotation_matrix_3d_z_axis(const double angle)
{
  ...
  Assert(std::abs(determinant(R_z) - 1.0) < 1e-9, ExcMessage("Rodrigues rotation matrix determinant is not unity"));
  const Tensor<2, 3> R = Transformations::Rotations::rotation_matrix_3d(Tensor<1, 3>({0., 0., 1.}), angle);
  Assert(std::abs(determinant(R) - 1.0) < 1e-9, ExcMessage("Rotation matrix determinant is not unity"));

  Assert((transpose(R) * R - unit_symmetric_tensor<3>()).norm() < 1e-9, ExcMessage("Matrix is not a rotation matrix"));
...
test_rotation_matrix_3d(const Tensor<1, 3> &axis, const double angle)
{
  ...
  const Tensor<2, 3> R_rodrigues =
    u_dyad_u + std::cos(angle) * (static_cast<Tensor<2, 3>>(unit_symmetric_tensor<3>()) - u_dyad_u) +
...
main()
{
  ...
  // 2-d
  {
    ...
    Assert((out - Tensor<1, 2>({0, 1})).norm() < 1e-12, ExcMessage("Incorrect computation of 90 degree R in 2d"));
  }
  {
  ...
  test_rotation_matrix_3d(normalise(Tensor<1, 3>({1., 1., 1.})), 90.0 * deg_to_rad);
  test_rotation_matrix_3d(normalise(Tensor<1, 3>({0., 2., 1.})), 45.0 * deg_to_rad);
  test_rotation_matrix_3d(normalise(Tensor<1, 3>({-1., 3., 2.})), 60.0 * deg_to_rad);

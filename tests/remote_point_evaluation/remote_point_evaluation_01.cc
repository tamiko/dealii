...
{
  FEValues<dim, spacedim> fe_eval(mapping, dof_handler.get_fe(), quadrature, update_gradients);
  FEValues<dim, spacedim> fe_eval_dim(mapping, dof_handler_dim.get_fe(), quadrature, update_gradients);
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      TriaIterator<DoFCellAccessor<dim, spacedim, false>> dof_cell_dim(&dof_handler_dim.get_triangulation(),
                                                                       cell->level(),
                                                                       cell->index(),
                                                                       &dof_handler_dim);
...
template <int dim, int spacedim>
void
print(std::tuple<std::vector<typename Triangulation<dim, spacedim>::active_cell_iterator>,
  ...
  for (unsigned int i = 0; i < std::get<0>(result).size(); ++i)
    {
      ...
      std::cout << std::get<0>(result)[i]->level() << ' ' << std::get<0>(result)[i]->index() << " x " << n_points
                << std::endl;

      for (unsigned int j = 0; j < n_points; ++j)
        {
          std::cout << std::get<1>(result)[i][j] << " - " << std::get<2>(result)[i][j] << " - "
                    << std::get<3>(result)[i][j] << " - " << std::get<4>(result)[i][j] << std::endl;
        }
    }
}
...
template <int dim, int spacedim, typename VectorType>
void
compute_force_vector_sharp_interface(const Mapping<dim, spacedim>    &surface_mapping,
                                     ...
                                     VectorType                      &force_vector)
{
  ...
  const auto integration_points = [&]() {
    ...
    for (const auto &cell : surface_dofhandler.get_triangulation().active_cell_iterators())
      {
        if (cell->is_locally_owned() == false)
          continue;
    ...
    for (const auto &cell : tria_surface.active_cell_iterators())
      {
        ...
        TriaIterator<DoFCellAccessor<dim, spacedim, false>> dof_cell(&tria_surface,
                                                                     ...
                                                                     cell->index(),
                                                                     &surface_dofhandler);
        TriaIterator<DoFCellAccessor<dim, spacedim, false>> dof_cell_dim(&tria_surface,
                                                                         cell->level(),
                                                                         cell->index(),
        ...
        for (const auto q : fe_eval_dim.quadrature_point_indices())
          {
            ...
            for (unsigned int i = 0; i < spacedim; ++i)
              result[i] = -curvature_values[q] * normal_values[q][i] * fe_eval.JxW(q) * surface_tension;
  ...
  const auto fu = [&](const auto &values, const auto &cell_data) {
    ...
    FEPointEvaluation<spacedim, spacedim> phi_force(mapping, dof_handler.get_fe(), update_values);
    ...
    for (unsigned int i = 0; i < cell_data.cells.size(); ++i)
      {
        ...
        const ArrayView<const T> force_JxW(values.data() + cell_data.reference_point_ptrs[i],
...
test()
{
  ...
  compute_curvature(mapping, dof_handler_dim, dof_handler, quadrature, normal_vector, curvature_vector);
  ...
  VectorType force_vector_sharp_interface(create_partitioner(background_dof_handler));
  ...
  if (true)
    {
      ...
      data_out.add_data_vector(background_dof_handler, force_vector_sharp_interface, "force");

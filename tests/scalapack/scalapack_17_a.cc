...
test(const unsigned int block_size_i, const unsigned int block_size_j)
{
  ...
  std::shared_ptr<Utilities::MPI::ProcessGrid> grid =
    std::make_shared<Utilities::MPI::ProcessGrid>(mpi_communicator, size, size, block_size_i, block_size_i);
  ScaLAPACKMatrix<NumberType> scalapack_matrix(size, size, grid, block_size_i, block_size_i);

  pcout << "2D grid matrix: dim=" << scalapack_matrix.m() << 'x' << scalapack_matrix.n() << ";  blocks=" << block_size_i
        << 'x' << block_size_i << ";  grid=" << grid->get_process_grid_rows() << 'x' << grid->get_process_grid_columns()
        << std::endl
        << std::endl;

  const bool       process_is_in_grid  = grid->is_process_active();
  const int        entry               = process_is_in_grid ? this_mpi_process : -1;
  std::vector<int> valid_process_ranks = Utilities::MPI::gather(mpi_communicator, entry, 0);
  int              random_rank         = 0;
  ...
  if (this_mpi_process == rank)
    {
      ...
      for (unsigned int i = 0; i < size; ++i)
        for (unsigned int j = 0; j < size; ++j)
          diff += (full(i, j) - comparison(i, j)) * (full(i, j) - comparison(i, j));
      diff = sqrt(diff);

      ...
      for (; cell != endc; ++cell)
        {
          ...
          typename Triangulation<dim>::cell_iterator tria_shared_cell =
            shared_dof_handler.get_triangulation().create_cell_iterator(cell->id());
       ...
       ++cell)
    {
      ...
      if (dim == 2)
        if (cell->center()[0] < 0.5 && cell->center()[1] < 0.5)
          cell->set_refine_flag();
      if (dim == 3)
        if (cell->center()[0] < 0.5 && cell->center()[1] < 0.5 && cell->center()[2] < 0.5)
          cell->set_refine_flag();
    }
       ...
       ++cell)
    {
      ...
      if (dim == 2)
        if (cell->is_locally_owned() && cell->center()[0] < 0.5 && cell->center()[1] < 0.5)
          cell->set_refine_flag();
      if (dim == 3)
        if (cell->is_locally_owned() && cell->center()[0] < 0.5 && cell->center()[1] < 0.5 && cell->center()[2] < 0.5)
          cell->set_refine_flag();
    }

...
test()
{
  ...
  Assert(dof_handler.n_locally_owned_dofs() ==
           Utilities::MPI::all_gather(MPI_COMM_WORLD,
                                      dof_handler.n_locally_owned_dofs())[triangulation.locally_owned_subdomain()],
         ExcInternalError());
  ...
  const std::vector<types::global_dof_index> n_owned_dofs =
    Utilities::MPI::all_gather(MPI_COMM_WORLD, dof_handler.n_locally_owned_dofs());
  Assert(std::accumulate(n_owned_dofs.begin(), n_owned_dofs.end(), 0U) == N, ExcInternalError());

  const std::vector<IndexSet> owned_dofs = Utilities::MPI::all_gather(MPI_COMM_WORLD, dof_handler.locally_owned_dofs());
  IndexSet                    all(N);

...
main()
{
  ...
  // test some basic algebra with barycentric polynomials
  {
    ...
    deallog << "bp1_0 * 2 * bp1_1 / 2 = " << bp1_0 * 2 * bp1_1 / 2 << std::endl << std::endl;
  }

  {
    ...
    deallog << "bp2_0 * bp2_0 + bp2_1 * bp2_0 = " << bp2_0 * bp2_0 + bp2_1 * bp2_0 << std::endl;
    deallog << "bp2_1 * bp2_0 + bp2_0 * bp2_0 = " << bp2_1 * bp2_0 + bp2_0 * bp2_0 << std::endl;
    ...
    for (unsigned int i = 0; i < 6; ++i)
      {
        ...
        for (unsigned int j = 0; j < 6; ++j)
          {
            Assert(std::abs(p2[i].value(fe.get_unit_support_points()[j]) - double(i == j)) < 1e-12, ExcInternalError());
          }
        deallog << std::endl;
      }
  }
    ...
    for (unsigned int i = 0; i < 4; ++i)
      {
        ...
        for (unsigned int j = 0; j < 4; ++j)
          {
            Assert(std::abs(tet4.compute_value(i, points[j]) - double(i == j)) < 1e-12, ExcInternalError());

            // first derivatives should be constant
            Assert((tet4.compute_grad(i, points[0]) - tet4.compute_grad(i, points[j])).norm() == 0.0,
                   ExcInternalError());
            Assert(tet4.compute_2nd_derivative(i, points[j]).norm() == 0.0, ExcInternalError());
            Assert(tet4.compute_3rd_derivative(i, points[j]).norm() == 0.0, ExcInternalError());
            Assert(tet4.compute_4th_derivative(i, points[j]).norm() == 0.0, ExcInternalError());
          }
      }
    ...
    for (unsigned int i = 0; i < 10; ++i)
      {
        ...
        for (unsigned int j = 0; j < 10; ++j)
          {
            Assert(std::abs(tet10.compute_value(i, points[j]) - double(i == j)) < 1e-12, ExcInternalError());

            // second derivatives should be constant
            Assert((tet10.compute_2nd_derivative(i, points[0]) - tet10.compute_2nd_derivative(i, points[j])).norm() ==
                     0.0,
                   ExcInternalError());

            Assert(tet10.compute_3rd_derivative(i, points[j]).norm() == 0.0, ExcInternalError());
            Assert(tet10.compute_4th_derivative(i, points[j]).norm() == 0.0, ExcInternalError());
          }
      }

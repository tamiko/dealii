      ...
      for (unsigned int p = 1; p <= 2; ++p)
        {
          ...
          test<dim>({0, 0}, {0, 1}, hp::FECollection<dim>(FE_Q<dim>(q), FE_SimplexP<dim>(p)), cube_and_pyramid);
          test<dim>({0, 0}, {1, 0}, hp::FECollection<dim>(FE_SimplexP<dim>(p), FE_Q<dim>(q)), cube_and_pyramid);
        }
  }
  deallog.pop();
}

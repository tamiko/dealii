...
public:
  ...
  HEX(unsigned int degree, unsigned int initial_refinement, unsigned int number_refinement)
  {
  ...
  static std::unique_ptr<DGHeat<dim>>
  TET(unsigned int degree, unsigned int initial_refinement, unsigned int number_refinement)
  {
    return std::make_unique<DGHeat<dim>>(false,
                                         new FE_SimplexDGP<dim>(degree),
                                         new MappingFE<dim>(FE_SimplexP<dim>(1)),
...
DGHeat<dim>::make_grid(int refinements)
{
  ...
  const unsigned int ref = refinements == -1 ? initial_refinement_level : refinements;

  if (hex)
    GridGenerator::subdivided_hyper_cube(triangulation, Utilities::pow(2, ref), -1.0, +1.0);
  else
    GridGenerator::subdivided_hyper_cube_with_simplices(triangulation, Utilities::pow(2, ref), -1.0, +1.0);
        ...
        for (unsigned int i = 0; i < n_dofs; ++i)
          {
            for (unsigned int j = 0; j < n_dofs; ++j)
              {
                copy_data.cell_matrix(i, j) += fe_v.shape_grad(i, point)   // \nabla \phi_i
                                               * fe_v.shape_grad(j, point) // \nabla \phi_j
                                               * JxW[point];               // dx
              }

            // Right Hand Side
            copy_data.cell_rhs(i) += (fe_v.shape_value(i, point) * f[point] * JxW[point]);
          }
      }
  };
      ...
      for (unsigned int point = 0; point < q_points.size(); ++point)
        for (unsigned int i = 0; i < n_facet_dofs; ++i)
          for (unsigned int j = 0; j < n_facet_dofs; ++j)
            {
              copy_data.cell_matrix(i, j) += -normals[point] * fe_face.shape_grad(i, point) // n*\nabla \phi_i
              ...
              copy_data.cell_matrix(i, j) += beta * 1. / h * fe_face.shape_value(i, point) // \phi_i
        ...
        for (unsigned int i = 0; i < n_dofs; ++i)
          {
            for (unsigned int j = 0; j < n_dofs; ++j)
              {
                copy_data_face.cell_matrix(i, j) += -normals[qpoint] * fe_iv.average_of_shape_gradients(i, qpoint) *
                ...
                copy_data_face.cell_matrix(i, j) += beta * 1. / h * fe_iv.jump_in_shape_values(i, qpoint) *
                                                    fe_iv.jump_in_shape_values(j, qpoint) * JxW[qpoint];
              }
          }
      }
...
DGHeat<dim>::output_results(unsigned int it) const
{
  ...
  std::string dimension(dim == 2 ? "solution-2d-" + type + "-case-" : "solution-3d-" + type + "-case-");
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
        {
          const double u_exact =
            dim == 2 ?
              -std::sin(M_PI * fe_values.quadrature_point(q)[0]) * std::sin(M_PI * fe_values.quadrature_point(q)[1]) :
              -std::sin(M_PI * fe_values.quadrature_point(q)[0]) * std::sin(M_PI * fe_values.quadrature_point(q)[1]) *
...
DGHeat<dim>::run()
{
  ...
  error_table.evaluate_all_convergence_rates(ConvergenceTable::reduction_rate_log2);

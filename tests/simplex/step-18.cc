...
namespace Step18
{
  ...
  {
    const Tensor<1, 3> curl({grad_u[2][1] - grad_u[1][2], grad_u[0][2] - grad_u[2][0], grad_u[1][0] - grad_u[0][1]});
  ...
  template <int dim>
  IncrementalBoundaryValues<dim>::IncrementalBoundaryValues(const double present_time, const double present_timestep)
    : Function<dim>(dim)
    , velocity(.08)
  ...
  template <int dim>
  void
  IncrementalBoundaryValues<dim>::vector_value(const Point<dim> & /*p*/, Vector<double> &values) const
  {
    Assert(values.size() == dim, ExcDimensionMismatch(values.size(), dim));
...
#ifdef HEX
    GridGenerator::subdivided_hyper_rectangle(triangulation, {1 * n, 1 * n, 3 * n}, {-0.5, -0.5, 0}, {+0.5, +0.5, +3});
#else
  ...
  TopLevel<dim>::assemble_system()
  {
              ...
              for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
                {
                  ...
                  cell_rhs(i) += (body_force_values[q_point](component_i) * fe_values.shape_value(i, q_point) -
    ...
    VectorTools::interpolate_boundary_values(
      mapping, dof_handler, 0, Functions::ZeroFunction<dim>(dim), boundary_values);
  ...
  TopLevel<dim>::solve_linear_problem()
  {
    ...
    solver.solve(system_matrix, distributed_incremental_displacement, system_rhs, PreconditionIdentity());
#endif

    deallog.depth_file(previous_depth);

    deallog << "norm: " << distributed_incremental_displacement.linfty_norm() << ' '
            << distributed_incremental_displacement.l1_norm() << ' ' << distributed_incremental_displacement.l2_norm()
            << std::endl;
  ...
  TopLevel<dim>::output_results() const
  {
    ...
    std::vector<DataComponentInterpretation::DataComponentInterpretation> solution_interpretation;

    solution_names.assign(dim, "delta");
    solution_interpretation.assign(
      dim, DataComponentInterpretation::DataComponentInterpretation::component_is_part_of_vector);
  ...
  TopLevel<dim>::do_initial_timestep(const bool do_output)
  {
    ...
    ++timestep_no;
    deallog << "Timestep " << timestep_no << " at time " << present_time << std::endl;
    ...
    for (unsigned int cycle = 0; cycle < 1; ++cycle)
      {
        ...
        deallog << "    Number of active cells:       " << triangulation.n_active_cells() << " (by partition:";
        for (unsigned int p = 0; p < n_mpi_processes; ++p)
          deallog << (p == 0 ? ' ' : '+') << (GridTools::count_cells_with_subdomain_association(triangulation, p));
        deallog << ')' << std::endl;
  ...
  TopLevel<dim>::setup_quadrature_point_history()
  {
    ...
    for (auto &cell : triangulation.active_cell_iterators() | IteratorFilters::LocallyOwnedCell())
      {
  ...
  TopLevel<dim>::update_quadrature_point_history()
  {
    ...
    std::vector<std::vector<Tensor<1, dim>>> displacement_increment_grads(quadrature_formula.size(),
        ...
        for (unsigned int q = 0; q < quadrature_formula.size(); ++q)
          {
            ...
            const SymmetricTensor<2, dim> rotated_new_stress =
              symmetrize(transpose(rotation) * static_cast<Tensor<2, dim>>(new_stress) * rotation);

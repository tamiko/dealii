...
namespace Step74
{
  ...
  template <int dim>
  Tensor<1, dim>
  SmoothSolution<dim>::gradient(const Point<dim> &point, const unsigned int /*component*/) const
  {
  ...
  double
  compute_penalty(const unsigned int fe_degree, const double cell_extend_left, const double cell_extend_right)
  {
    const double degree = std::max(1., static_cast<double>(fe_degree));
    return degree * (degree + 1.) * 0.5 * (1. / cell_extend_left + 1. / cell_extend_right);
  }
  ...
  {
    std::string filename =
      "sol_Q" + Utilities::int_to_string(degree, 1) + "-" + Utilities::int_to_string(cycle, 2) + ".vtu";
  ...
  SIPGLaplace<dim>::compute_error_estimate()
  {
    ...
    const auto cell_worker = [&](const auto &cell, auto &scratch_data, auto &copy_data) {
      ...
      for (unsigned int point = 0; point < n_q_points; ++point)
        {
          const double residual = rhs[point] + diffusion_coefficient * trace(hessians[point]);
      ...
      for (unsigned int point = 0; point < n_q_points; ++point)
        {
          ...
          flux_jump_square += diffusion_coefficient * flux_jump * flux_jump * JxW[point];
        }
  ...
  SIPGLaplace<dim>::compute_energy_norm()
  {
    ...
    UpdateFlags cell_flags = update_gradients | update_quadrature_points | update_JxW_values;
    UpdateFlags face_flags = update_values | update_quadrature_points | update_JxW_values;

    ScratchData scratch_data(mapping, fe, quadrature_2, cell_flags, face_quadrature_2, face_flags);
    ...
    for (unsigned int cycle = 0; cycle < max_cycle; ++cycle)
      {
        ...
        if (test_case == Test_Case::l_singularity)
          {
            compute_error_estimate();
            deallog << "  Estimated error              : " << std::sqrt(estimated_error_square_per_cell.l1_norm())
                    << std::endl;

            convergence_table.add_value("Estimator", std::sqrt(estimated_error_square_per_cell.l1_norm()));
          }
        deallog << std::endl;
      }
    {
      ...
      convergence_table.write_text(deallog.get_file_stream(), TableHandler::TextOutputFormat::org_mode_table);
    }
  }
} // namespace Step74

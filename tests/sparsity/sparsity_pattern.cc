...
main()
{
  ...
  sp1.compress();
  deallog << sp1.n_rows() << ' ' << sp1.n_cols() << ' ' << sp1.bandwidth() << ' ' << sp1.n_nonzero_elements()
          << std::endl;
  ...
  sp3.symmetrize();
  sp3.compress();
  deallog << sp3.n_rows() << ' ' << sp3.n_cols() << ' ' << sp3.bandwidth() << ' ' << sp3.n_nonzero_elements()
          << std::endl;
  ...
  for (unsigned int loop = 1; loop <= 4; ++loop)
    {
      ...
      for (unsigned int i = 0; i < sp.n_nonzero_elements(); ++i)
        AssertThrow(sp(sp.matrix_position(i).first, sp.matrix_position(i).second) == i, ExcInternalError());
      for (types::global_dof_index row = 0; row < sp.n_rows(); ++row)
        for (types::global_dof_index col = 0; col < sp.n_cols(); ++col)
          if (sp(row, col) != SparsityPattern::invalid_entry)
            AssertThrow(sp.matrix_position(sp(row, col)) == std::make_pair(row, col), ExcInternalError());
    };
  ...
  deallog << sp3.n_rows() - sp5.n_rows() << ' ' << sp3.n_cols() - sp5.n_cols() << ' '
          << (sp3.is_compressed() ^ sp5.is_compressed()) << ' ' << (sp3.is_compressed() ^ sp5.is_compressed()) << ' '
          << std::endl;

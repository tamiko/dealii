...
main(int argc, char *argv[])
{
  ...
  std::cout << "Timing ratio (subs/lambda_double): "
            << (diff_symm_subs) / (diff_lambda_real_double_setup + diff_lambda_real_double_call) << std::endl;
  ...
  std::cout << "LLVMDoubleVisitor run-time       :"
            << std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() << " us : Value: " << res2
            << std::endl;

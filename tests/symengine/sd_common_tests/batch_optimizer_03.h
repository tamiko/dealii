...
namespace Diff_Test
{
  ...
  test_standard()
  {
    ...
    const SymmetricTensor<2, dim, SD::Expression> C_SD(SD::make_symmetric_tensor_of_symbols<2, dim>("C"));

    const SD::Expression symbolic_psi = 0.5 * mu_e_SD * (trace(C_SD) - 3) + kappa_e_SD * (std::log(determinant(C_SD)));
    ...
    optimizer.register_functions(symbolic_psi, symbolic_S,
    ...
    const SymmetricTensor<2, dim, NumberType> C       = 2.0 * unit_symmetric_tensor<dim>();
    ...
    // employed later
    const SD::types::substitution_map sub_vals = SD::resolve_explicit_dependencies(sub_vals_unresolved);
  ...
  test_explicit()
  {
    ...
    const SymmetricTensor<2, dim, SD::Expression> Qi_SD(SD::make_symmetric_tensor_of_symbols<2, dim>("Qi"));

    // Compute the dependent variable
    const SD::Expression symbolic_psi_CQi =
      0.5 * mu_e_SD * (trace(C_SD) - 3) + kappa_e_SD * (std::log(determinant(C_SD))) +
      0.5 * mu_v_SD * ((C_SD * Qi_SD) - 3) + kappa_v_SD * (std::log(C_SD * Qi_SD));
    ...
    SD::add_to_substitution_map(sub_vals_explicit, std::make_pair(Qi_SD, 2 * C_SD));

    // Note: After performing these operations we should not longer
    // need to consider the internal variable Q at all
    const SD::Expression                          symbolic_psi = SD::substitute(symbolic_psi_CQi, sub_vals_explicit);
    const SymmetricTensor<2, dim, SD::Expression> symbolic_S   = SD::substitute(symbolic_S_CQi, sub_vals_explicit);
    const SymmetricTensor<4, dim, SD::Expression> symbolic_HH =
      2.0 * SD::differentiate(symbolic_S, C_SD); // HH = 2*dS_dC = 4*d2psi_dC_dC

    // Configure optimizer
    SD::types::substitution_map sub_vals_optim;
    SD::add_to_symbol_map(sub_vals_optim, mu_e_SD, kappa_e_SD, C_SD, mu_v_SD, kappa_v_SD);
  ...
  test_implicit()
  {
    ...
    const SD::types::substitution_map             sub_vals_func_symb_dependencies = SD::make_symbol_map(C_SD);
    const SymmetricTensor<2, dim, SD::Expression> Qd_SD =
      SD::make_symmetric_tensor_of_symbolic_functions<2, dim>("Qd", sub_vals_func_symb_dependencies);
    const SymmetricTensor<4, dim, SD::Expression> dQd_SD_dC = SD::differentiate(Qd_SD, C_SD);
    ...
    const SD::types::substitution_map sub_vals_implicit = SD::make_substitution_map(std::make_pair(Qi_SD, Qd_SD));
    const SymmetricTensor<2, dim, SD::Expression> symbolic_S_subs_Q = SD::substitute(symbolic_S, sub_vals_implicit);
    ...
    const SymmetricTensor<4, dim, SD::Expression> dQ_dC_i_SD = SD::make_symmetric_tensor_of_symbols<4, dim>("dQ_dC_i");
    const SD::types::substitution_map             sub_vals_explicit =
      SD::make_substitution_map(std::make_pair(Qd_SD, Qi_SD),
                                std::make_pair(SD::differentiate(Qd_SD, C_SD), dQ_dC_i_SD));
    ...
    const SD::types::substitution_map sub_vals_optim =
      SD::make_symbol_map(mu_e_SD, kappa_e_SD, C_SD, mu_v_SD, kappa_v_SD, Qi_SD, dQ_dC_i_SD);
    ...
    const double                              mu_v    = 5;
    const double                              kappa_v = 2;
    const SymmetricTensor<2, dim, NumberType> Q       = 1.5 * unit_symmetric_tensor<dim>();
    const SymmetricTensor<4, dim, NumberType> dQ_dC   = 3.0 * identity_tensor<dim>();

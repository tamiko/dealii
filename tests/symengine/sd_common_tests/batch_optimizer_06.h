...
#define CREATE_SYMBOLS_FUNCTIONS_SUBSTITUTION_MAP                                                         \
  ...
  // Serialize
  {
    ...
    // timer.enter_subsection("Serialization");
    {
      ...
      if (!(opt_method == SD::OptimizerType::llvm && numbers::NumberTraits<NumberType>::is_complex))
        {
          // We cannot do this check under all circumstances, as internally the
          // optimizer type switches.
          Assert(optimizer.optimization_method() == opt_method, ExcInternalError());
        }

      {
        ...
        const Tensor<2, dim, NumberType>          new_val_ds_dy = optimizer.evaluate(symb_ds_dy);
        const SymmetricTensor<2, dim, NumberType> new_val_ds_dz = optimizer.evaluate(symb_ds_dz);

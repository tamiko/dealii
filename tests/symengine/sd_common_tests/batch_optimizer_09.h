...
test_extract_evaluate()
{
  ...
  deallog.push("Extraction");
  {
    ...
    const Tensor<2, dim, NumberType>          extr_val_G = extraction_optimizer.extract(G, values);
    const SymmetricTensor<2, dim, NumberType> extr_val_H = extraction_optimizer.extract(H, values);
  ...
  deallog.push("Invalid evaluation");
  {
    ...
    try
      ...
      {
        deallog << "Caught invalid evaluation before substitution: Scalar" << std::endl;
      }

    try
      {
        const Tensor<2, dim, NumberType> val_G = extraction_optimizer.evaluate(G);
        deallog << "G(x,y,T): " << val_G << std::endl << std::endl;
      }
    catch (std::exception &)
      {
        deallog << "Caught invalid evaluation before substitution: Tensor" << std::endl;
      }

    try
      {
        const SymmetricTensor<2, dim, NumberType> val_H = extraction_optimizer.evaluate(H);
        deallog << "H(x,y,S): " << val_H << std::endl << std::endl;
      }
    catch (std::exception &)
      {
        deallog << "Caught invalid evaluation before substitution: SymmetricTensor" << std::endl;
      }
    ...
    try
      ...
      {
        deallog << "Caught invalid substitution before optimization." << std::endl;
      }
    ...
    try
      ...
      {
        deallog << "Caught invalid evaluation before optimization: Scalar" << std::endl;
      }
    ...
    try
      ...
      {
        deallog << "Caught invalid evaluation before optimization: Tensor" << std::endl;
      }
    ...
    try
      ...
      {
        deallog << "Caught invalid evaluation before optimization: SymmetricTensor" << std::endl;
      }
  }
  deallog.pop();

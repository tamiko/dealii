...
main()
{
  ...
  try
    {
      const SD::types::substitution_map sub_vals_resolved = SD::resolve_explicit_dependencies(sub_vals_unresolved);
      std::cout << "Resolved map:" << std::endl;
      SD::Utilities::print_substitution_map(std::cout, sub_vals_resolved);
      deallog << "Cyclic dependency in substitution map was unexpectedly resolved." << std::endl;
    }
  catch (...)
    {

...
main()
{
  ...
  const SD::types::substitution_map symbol_map_1 = SD::make_symbol_map(SD::Expression("x1"));
  const SD::types::substitution_map symbol_map_2 =
    SD::make_symbol_map(SD::types::symbol_vector{SD::Expression("x2"), SD::Expression("x3")});
  // Add via map, but check that associated values are ignored.
  const SD::types::substitution_map symbol_map_3 =
    SD::make_symbol_map(SD::types::substitution_map{{SD::Expression("x4"), SD::Expression(4)},
                                                    {SD::Expression("x5"), SD::Expression(5)},
                                                    {SD::Expression("x6"), SD::Expression(6)}});
  const SD::types::substitution_map symbol_map_4 =
    SD::make_symbol_map(SD::Expression("x7"),
  ...
  SD::types::substitution_map symbol_map =
    merge_substitution_maps(symbol_map_1, symbol_map_2, symbol_map_3, symbol_map_4);
  SD::Utilities::print_substitution_map(deallog, symbol_map);


  deallog << "Set values in symbol map" << std::endl;
  SD::set_value_in_symbol_map(symbol_map, SD::Expression("x1"), SD::Expression(1));
  SD::set_value_in_symbol_map(symbol_map, SD::Expression("x2"), SD::Expression(2.0));
  SD::set_value_in_symbol_map(symbol_map, SD::Expression("x3"), std::complex<double>(3.0));
  ...
  SD::set_value_in_symbol_map(symbol_map,
                              SD::types::symbol_vector{SD::Expression("x6"), SD::Expression("x7")},
                              std::vector<double>{6.0, 7.0});
  SD::set_value_in_symbol_map(symbol_map, std::make_pair(SD::Expression("x8"), 8.0));
  SD::set_value_in_symbol_map(symbol_map,
                              std::vector<std::pair<SD::Expression, double>>{std::make_pair(SD::Expression("x9"), 9.0),
                                                                             std::make_pair(SD::Expression("x10"),
                                                                                            10.0)});
  SD::set_value_in_symbol_map(symbol_map,
                              SD::types::substitution_map{{SD::Expression("x10"), SD::Expression(10)},
                                                          {SD::Expression("x11"), SD::Expression(11)},

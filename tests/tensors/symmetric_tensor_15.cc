...
test()
{
  ...
  {
    const SymmetricTensor<4, dim> T = outer_product<dim>(unit_symmetric_tensor<dim>(), unit_symmetric_tensor<dim>());

    // T*t should yield a diagonal tensor
    // where the diagonal elements are the
    // traces of t
    SymmetricTensor<2, dim> x = T * t;
    AssertThrow((x - trace(t) * unit_symmetric_tensor<dim>()).norm() < 1e-15 * t.norm(), ExcInternalError());

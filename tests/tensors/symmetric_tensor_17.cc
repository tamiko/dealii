...
test()
{
    ...
    for (unsigned int j = 0; j < dim; ++j)
      for (unsigned int k = 0; k < dim; ++k)
        for (unsigned int l = 0; l < dim; ++l)
          t[i][j][k][l] =
            10000 * (i == j && k == l ? 1 : 0) + 100 * ((i == k && j == l ? 1 : 0) + (i == l && j == k ? 1 : 0));
  ...
  for (unsigned int i = 0; i < dim; ++i)
    for (unsigned int j = 0; j < dim; ++j)
      for (unsigned int k = 0; k < dim; ++k)
        for (unsigned int l = 0; l < dim; ++l)
          deallog << i << ' ' << j << ' ' << k << ' ' << l << ": " << (int)t[i][j][k][l] << std::endl;
  ...
  // given to it
  AssertThrow((t_times_1 - (dim * 10000 + 2 * 100) * unit_symmetric_tensor<dim>()).norm() < 1e-14 * t_times_1.norm(),
              ExcInternalError());
}

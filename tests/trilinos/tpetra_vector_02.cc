...
test()
{
  ...
  LinearAlgebra::TpetraWrappers::Vector<Number> a(parallel_partitioner_1, MPI_COMM_WORLD);
  LinearAlgebra::TpetraWrappers::Vector<Number> b(parallel_partitioner_1, MPI_COMM_WORLD);
  LinearAlgebra::TpetraWrappers::Vector<Number> c(parallel_partitioner_2, MPI_COMM_WORLD);
  ...
  if (rank == 0)
    {
      for (unsigned int i = 0; i < 5; ++i)
        AssertThrow(Number(1.) + read_write_1[i] == read_write_3[i], ExcMessage("Problem in add(scalar)."));
    }
  else
    {
      for (unsigned int i = 5; i < 10; ++i)
  ...
  if (rank == 0)
    {
      for (unsigned int i = 0; i < 5; ++i)
        AssertThrow(Number(1.) + read_write_1[i] + Number(2.) * read_write_2[i] == read_write_3[i],
  ...
  if (rank == 0)
    {
      for (unsigned int i = 0; i < 5; ++i)
        AssertThrow(Number(4.) + Number(4.) * read_write_1[i] + Number(10.) * read_write_2[i] == read_write_3[i],
  ...
  if (rank == 0)
    {
      for (unsigned int i = 0; i < 5; ++i)
        AssertThrow(Number(3.) * read_write_1[i] + Number(2.) * read_write_2[i] == read_write_3[i],
  ...
  if (rank == 0)
    {
      for (unsigned int i = 0; i < 5; ++i)
        AssertThrow(read_write_1[i] * read_write_2[i] == read_write_3[i], ExcMessage("Problem in scale."));
    }
  else
    {
      for (unsigned int i = 5; i < 10; ++i)
  ...
  if (rank == 0)
    {
      for (unsigned int i = 0; i < 5; ++i)
        AssertThrow(Number(2.) * read_write_2[i] == read_write_3[i], ExcMessage("Problem in scale."));
    }
  else
    {
      for (unsigned int i = 5; i < 10; ++i)
        AssertThrow((Number)2. * read_write_2[i] == read_write_3[i], ExcMessage("Problem in equ."));
    }

...
test(VectorTools::NormType norm, double value)
{
  ...
  VectorTools::integrate_difference(
    dofh, solution, Functions::ZeroFunction<dim>(2 * dim), cellwise_errors, QGauss<dim>(5), norm);

  double error = cellwise_errors.l2_norm();

  const double difference_1 = std::abs(error - value);
  deallog << "computed: " << error << " expected: " << value << " difference: " << difference_1 << std::endl;
  Assert(difference_1 < 1e-10, ExcMessage("Error in integrate_difference, first components"));

  VectorTools::integrate_difference(
    dofh, solution, Functions::ZeroFunction<dim>(2 * dim), cellwise_errors, QGauss<dim>(5), norm, &mask_2);

  error                     = cellwise_errors.l2_norm();
  const double difference_2 = std::abs(error - 2.0 * value);
  deallog << "computed: " << error << " expected: " << 2.0 * value << " difference: " << difference_2 << std::endl;
